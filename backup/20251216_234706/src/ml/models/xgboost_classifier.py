...
class XGBoostClassifier:
    ...
    def load_model(cls, model_path: Union[str, Path]) -> 'XGBoostClassifier':
        ...
        try:
            ...
            # 加载模型权重
            classifier.model.load_model(str(model_path))
...
if __name__ == "__main__":
    # 模块测试
    print("🧪 XGBoost分类器模块测试")

    try:
        # 创建配置
        config = XGBoostModelConfig.create_default()
        print(f"✅ 配置创建成功: {config.model_name} v{config.model_version}")
        ...
        print(f"   验证准确率: {metrics['val_accuracy']:.3f}")
        ...
        for i, (pred, prob) in enumerate(zip(predictions, probabilities)):
            print(f"   样本{i+1}: 预测={pred}, 概率={prob}")

        # 特征重要性
        importance_df = classifier.get_feature_importance()
        print(f"📈 前5个重要特征:")
        for _, row in importance_df.head().iterrows():
            print(f"   {row['feature_display_name']}: {row['importance']:.4f}")
