...
class EnhancedFeatureExtractor:
    ...
    def extract_features(self, match_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"特征提取失败: {e}, match_id: {match_data.get('id', 'unknown')}"
            )
            return {}
            ...
            try:
                ...
                for (section, stat_key), feature_name in basic_stats_map.items():
                    ...
                    if (
                        ...
                        and isinstance(stat_value, list)
                        and len(stat_value) >= 2
                    ):
    ...
    def _extract_advanced_stats(self, match_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if home_xg is not None and away_xg is not None:
            ...
            if home_score is not None and away_score is not None:
                ...
                features["total_xg_vs_actual"] = (home_xg + away_xg) - (
            ...
            try:
                ...
                if odds_data:
                    ...
                    if (
                        ...
                        and odds_data.get("draw")
                        and odds_data.get("away_win")
                    ):
                        home_imp_prob = (
                            1 / odds_data["home_win"]
                            if odds_data["home_win"] > 0
                            else 0
                        )
                        draw_imp_prob = (
                            1 / odds_data["draw"] if odds_data["draw"] > 0 else 0
                        )
                        away_imp_prob = (
                            1 / odds_data["away_win"]
                            if odds_data["away_win"] > 0
                            else 0
                        )
                        ...
                        if total_imp_prob > 0:
                            features["home_win_implied_prob"] = (
                                home_imp_prob / total_imp_prob
                            )
                            features["draw_implied_prob"] = (
                                draw_imp_prob / total_imp_prob
                            )
                            features["away_win_implied_prob"] = (
                                away_imp_prob / total_imp_prob
                            )
                        else:
    ...
    def get_feature_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                ...
                / max(self.feature_stats["total_matches"], 1)
                * 100
            ),
