...
class FootballFeatureAnalyzer:
    ...
    def _analyze_target_relation(self, df: pd.DataFrame, target_col: str) -> Dict:
        ...
        target_relation = {
            ...
            'top_features_by_corr': sorted(feature_target_corr.items(),
                                         key=lambda x: abs(x[1]['correlation']), reverse=True)[:10]
        }
    ...
    def suggest_feature_engineering(self, analysis_results: Dict) -> Dict:
        ...
        if missing['features_with_missing'] > 0:
            ...
            moderate_missing = [k for k, v in missing['missing_percentage_by_feature'].items()
                              if 5 < v <= 20]
            if moderate_missing:
                suggestions.append({
                    'type': 'missing_values',
    ...
    def create_new_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'mutual_info_scores' in self.feature_stats:
            top_features = sorted(self.feature_stats['mutual_info_scores'].items(),
    ...
    def visualize_feature_analysis(self, df: pd.DataFrame, target_col: str = 'target'):
        ...
        if self.correlation_matrix is not None:
            ...
            sns.heatmap(self.correlation_matrix, mask=mask, cmap='coolwarm', center=0,
                       annot=False, fmt='.2f', cbar_kws={"shrink": .8})
            plt.title('特征相关性热力图')
            plt.tight_layout()
