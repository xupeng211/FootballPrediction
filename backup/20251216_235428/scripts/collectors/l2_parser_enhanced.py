...
class CompleteL2Parser:
    ...
    def parse_api_response(self, api_data: Dict[str, Any]) -> L2MatchStats:
        ...
        try:
                        ...
                        if (
                            ...
                            if key in self.field_mapping:
                                ...
                                if key == "accurate_passes":
                                    ...
                                    # 同时处理总长传数
                                    self._parse_total_fields(
                                        l2_stats, home_value, away_value, "long_balls"
                                    )
                                    ...
                                    # 同时处理总传中数
                                    self._parse_total_fields(
                                        l2_stats, home_value, away_value, "crosses"
                                    )
            ...
            if filled_fields < 60:
                self.logger.warning(
                    f"⚠️ 字段填充数量较少: {filled_fields}，可能存在问题"
                )
    ...
    def _process_stat_values(
        self, values: List[Union[str, int, float, None]]
    ...
    def _parse_percentage_fields(
        self,
        l2_stats: L2MatchStats,
        home_value: Optional[Union[int, float]],
        away_value: Optional[Union[int, float]],
        base_field: str,
    ) -> None:
        """解析百分比字段并设置相关属性"""
        try:
    ...
    def _count_filled_fields(self, l2_stats: L2MatchStats) -> int:
        ...
        for attr_name in dir(l2_stats):
            if (
                ...
                and not attr_name.startswith("l2_")
                and hasattr(l2_stats, attr_name)
            ):
    ...
    def get_field_summary(self) -> Dict[str, int]:
        ...
        return {
            ...
            "defined_dataclass_fields": len(
                [field for field in dir(L2MatchStats) if not field.startswith("_")]
            )
            - 1,  # 减去l2_stats_raw
        }
    ...
    def validate_data_quality(self, l2_stats: L2MatchStats) -> Dict[str, Any]:
        ...
        # 计算质量分数
        critical_score = (
            validation_results["critical_fields_filled"] / len(critical_fields)
        ) * 50
        advanced_score = (
            validation_results["advanced_fields_filled"] / len(advanced_fields)
        ) * 30
        coverage_score = min(
            (validation_results["total_fields"] / 70) * 20, 20
        )  # 假设最多70个字段

        validation_results["data_quality_score"] = (
            critical_score + advanced_score + coverage_score
        )
