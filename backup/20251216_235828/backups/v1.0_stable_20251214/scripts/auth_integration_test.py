...
class AuthIntegrationTester:
    ...
    async def setup(self) -> None:
        ...
        self.token_manager = create_token_manager(
            default_ttl=self.token_ttl, cache_refresh_threshold=self.refresh_threshold
        )
        ...
        if self.fotmob:
            ...
            mock_provider = MockAuthProvider(
                "demo_provider", f"demo_token_{int(time.time())}", self.token_ttl
            )
        ...
        if self.use_proxies:
            ...
            print(
                f"✅ 设置代理池: {len(proxy_urls)} 个代理，策略: {self.proxy_strategy}"
            )
        else:
    ...
    async def simulate_request(self, request_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            async with self.rate_limiter.acquire("api_requests"):
                ...
                request_result = await self._mock_http_request(
                    token, proxy if self.use_proxies else None
                )
    ...
    async def run_requests(self) -> None:
        ...
        if self.concurrent:
            # 并发请求
            print("🔄 使用并发请求模式...")
            tasks = [self.simulate_request(i) for i in range(self.num_requests)]
    ...
    async def _print_results_summary(
        self, successful: list[dict], failed: list[dict], all_results: list[Any]
    ) -> None:
        """打印结果摘要"""
        print("\n📊 请求结果摘要:")
        print(f"   总请求数: {len(all_results)}")
        print(
            f"   成功请求: {len(successful)} ({len(successful) / len(all_results) * 100:.1f}%)"
        )
        print(
            f"   失败请求: {len(failed)} ({len(failed) / len(all_results) * 100:.1f}%)"
        )
        ...
        print(
            f"   限流率: {self.stats['rate_limited_requests'] / self.stats['total_requests'] * 100:.1f}%"
        )
...
示例用法:
  ...
  python scripts/auth_integration_test.py --demo --requests 50 --use-proxies
    ...
    # 认证配置
    parser.add_argument(
        "--fotmob", action="store_true", help="使用FotMob认证（默认使用Demo认证）"
    )
    parser.add_argument("--demo", action="store_true", help="使用Demo认证（默认）")
    parser.add_argument(
        "--token-ttl", type=float, default=300.0, help="令牌生存时间（秒） (默认: 300)"
    )
    parser.add_argument(
        "--refresh-threshold",
        type=float,
    ...
    parser.add_argument("--force-refresh", action="store_true", help="强制刷新令牌")

    # 请求配置
    parser.add_argument(
        "--requests", "-n", type=int, default=10, help="请求数量 (默认: 10)"
    )
    parser.add_argument("--concurrent", action="store_true", help="并发执行请求")
    parser.add_argument(
        "--rate-limit", type=float, default=5.0, help="速率限制 QPS (默认: 5.0)"
    )
    parser.add_argument("--burst", type=int, default=10, help="突发容量 (默认: 10)")
    parser.add_argument(
        "--request-timeout",
        type=float,
        default=10.0,
    ...
    # 代理配置
    parser.add_argument("--use-proxies", action="store_true", help="启用代理池")
    parser.add_argument(
        "--proxy-strategy",
    ...
    parser.add_argument("--token-info", action="store_true", help="显示令牌详细信息")
    parser.add_argument(
        "--timeout", type=float, default=10.0, help="网络请求超时时间（秒） (默认: 10)"
    )
