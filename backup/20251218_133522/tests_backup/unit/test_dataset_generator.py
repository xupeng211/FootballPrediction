...
from src.ml.dataset.target_labels import (
    MatchOutcome,
    score_to_label,
    label_to_numeric,
    numeric_to_label,
...
class TestClassificationDatasetGenerator:
    ...
    def mock_feature_extractor(self):
        ...
        def create_mock_feature_set(match_id: str, completeness: float = 0.9):
            return MatchFeatureSet(
                ...
                home_form_last5=TeamFormFeatures(
                    matches_played=5,
                    wins=3,
                    draws=1,
                    losses=1,
                ...
                away_form_last5=TeamFormFeatures(
                    matches_played=5,
                    wins=2,
                    draws=2,
                    losses=1,
                ...
                home_form_last3=TeamFormFeatures(
                    matches_played=3,
                    wins=2,
                    draws=0,
                    losses=1,
                ...
                away_form_last3=TeamFormFeatures(
                    matches_played=3,
                    wins=1,
                    draws=1,
                    losses=1,
                ...
                home_xg_last5=XGFeatures(
                    total_xg=8.5,
                    total_goals=8,
                    xg_efficiency=0.94,
                    avg_xg_per_match=1.7,
                    avg_goals_per_match=1.6,
                    big_chances_created=12,
                    big_chances_converted=8,
                    big_chance_conversion_rate=0.67,
                    xg_trend_5=0.1,
                    xg_trend_3=0.2,
                ),
                away_xg_last5=XGFeatures(
                    total_xg=7.2,
                    total_goals=6,
                    xg_efficiency=0.83,
                    avg_xg_per_match=1.44,
                    avg_goals_per_match=1.2,
                    big_chances_created=10,
                    big_chances_converted=6,
                    big_chance_conversion_rate=0.6,
                    xg_trend_5=-0.05,
                    xg_trend_3=0.1,
                ),
                home_xg_last3=XGFeatures(
                    total_xg=5.1,
                    total_goals=5,
                    xg_efficiency=0.98,
                    avg_xg_per_match=1.7,
                    avg_goals_per_match=1.67,
                    big_chances_created=7,
                    big_chances_converted=5,
                    big_chance_conversion_rate=0.71,
                    xg_trend_5=0.1,
                    xg_trend_3=0.2,
                ),
                away_xg_last3=XGFeatures(
                    total_xg=4.2,
                    total_goals=4,
                    xg_efficiency=0.95,
                    avg_xg_per_match=1.4,
                    avg_goals_per_match=1.33,
                    big_chances_created=6,
                    big_chances_converted=4,
                    big_chance_conversion_rate=0.67,
                    xg_trend_5=-0.05,
                    xg_trend_3=0.1,
                ),
                odds=OddsFeatures(
                    ...
                    draw_odds=3.4,
                    away_win_odds=3.8,
                    home_win_implied_prob=0.48,
                    draw_implied_prob=0.29,
                    away_win_implied_prob=0.26,
                    home_win_normalized=0.48,
                    draw_normalized=0.29,
                    away_win_normalized=0.26,
                ),
                ...
                data_quality_flag=(
                    "HIGH"
                    if completeness >= 0.8
                    else ("MEDIUM" if completeness >= 0.5 else "LOW")
                ),
            )
    ...
    def dataset_generator(self, mock_feature_extractor, mock_db_pool):
        ...
        return ClassificationDatasetGenerator(
            feature_extractor=mock_feature_extractor, db_pool=mock_db_pool
        )
    ...
    async def test_generate_dataset_success(self, dataset_generator):
        ...
        result_df = await dataset_generator.generate_dataset(
            league_id="premier_league", start_date="2024-01-01"
        )
    ...
    async def test_generate_dataset_filter_low_completeness(self, dataset_generator):
        ...
        result_df = await dataset_generator.generate_dataset(
            league_id="test_league", start_date="2024-01-01"
        )
    ...
    async def test_generate_feature_extraction_error(self, dataset_generator):
        """测试特征提取错误处理"""
        # 模拟特征提取失败
        dataset_generator.feature_extractor.extract_features.side_effect = Exception(
            "特征提取失败"
        )

        # 应该跳过失败的比赛，不抛出异常
        result_df = await dataset_generator.generate_dataset(
            "test_league", "2024-01-01"
        )
        assert isinstance(result_df, pd.DataFrame)
...
class TestIntegrationScenarios:
    ...
    async def test_end_to_end_dataset_generation(self):
        ...
        mock_features.get_feature_names.return_value = [
            "feature1",
            "feature2",
            "feature3",
        ]
