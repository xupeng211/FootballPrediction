#!/usr/bin/env python3
"""
IntegrityError修复验证测试 - 修复版
Test IntegrityError Fix - Fixed Version
"""
...
async def test_null_time_handling():
    ...
    try:
        ...
        test_match_data = {
            "header": {
                "status": {"reason": {"short": "TBD", "long": "Time To Be Determined"}},
                "teams": [
        ...
        success = await save_null_time_match(match_data)
...
async def save_null_time_match(match_data):
    try:
        async for session in get_async_db_session():
            ...
            if saved_match:
                ...
                logger.info(f"   match_time: {saved_match.match_time}")
                logger.info(f"   match_date: {saved_match.match_date}")
                return True
            else:
