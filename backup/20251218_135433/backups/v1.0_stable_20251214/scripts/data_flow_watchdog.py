        ...
        try:
                ...
                try:
                    ...
                    if self.process_name in cmdline:
                        ...
                        if proc.status() in [psutil.STATUS_ZOMBIE, psutil.STATUS_DEAD]:
                            logger.error(
                                f"❌ 进程 {proc.info['pid']} 处于异常状态: {proc.status()}"
                            )
                            return False
            ...
            if not process_found:
                await self.send_alert(
                    "PROCESS_DOWN", f"❌ 未找到采集器进程: {self.process_name}"
                )
                return False
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.get(self.api_health_url) as response:
                    if response.status == 200:
                        ...
                        await self.send_alert(
                            "API_UNHEALTHY", f"⚠️ API响应异常: HTTP {response.status}"
                        )
    ...
    async def send_alert(self, alert_type: str, message: str):
        ...
        if alert_type in self.last_alert_time:
            ...
            if time_since_last < self.alert_cooldown:
                logger.info(
                    f"🔕 告警冷却中: {alert_type} (剩余 {self.alert_cooldown - time_since_last:.0f} 秒)"
                )
                return
