...
class LeagueConfigPatcher:
    ...
    def patch_league_ids(self) -> dict[str, Any]:
        ...
        patch_stats = {"updated": 0, "added": 0, "unchanged": 0, "failed": 0}
        ...
        for league_name, patch_data in self.patch_leagues.items():
            try:
                if league_name in name_to_index:
                    ...
                    # 检查是否需要更新
                    if (
                        existing_league["id"] != patch_data["id"]
                        or existing_league.get("tier") != patch_data["tier"]
                    ):
                        # 保存原始数据用于日志
                        ...
                        logger.info(
                            f"✅ 无需更新: {league_name} (ID: {patch_data['id']})"
                        )
                else:
                    # 添加新联赛
                    new_league = {"name": league_name, **patch_data}
                    leagues_list.append(new_league)
                    patch_stats["added"] += 1
                    logger.info(
                        f"➕ 添加: {league_name} -> ID {patch_data['id']} (Tier {patch_data['tier']})"
                    )
    ...
    def print_final_overview(self):
        ...
        for tier in sorted(tiers.keys()):
            ...
            for league in tier_leagues:
                ...
                id_display = (
                    league.get("id", 0) if league.get("id", 0) > 0 else "未找到"
                )
                logger.info(
                    f"   {status} {league['name']} -> ID {id_display} ({league.get('country', 'N/A')})"
                )
        ...
        logger.info(
            f"   成功率: {(metadata['successful_ids'] / metadata['total_leagues'] * 100):.1f}%"
        )
