...
# 导入待测试的API模块
from src.api.predictions.predict_router import (
    router,
    PredictionResult,
...
class MockInferenceService:
    ...
    def get_service_stats(self) -> Dict[str, Any]:
        """获取服务统计"""
        return {**self.stats, "cache_size": 0, "is_initialized": self.is_initialized}
...
class TestPerformance:
    ...
    def test_batch_prediction_performance(self, client: TestClient):
        ...
        # 批量预测应该比单次预测平均更快
        assert (
            avg_time_per_prediction < 100
        ), f"批量预测平均时间过长: {avg_time_per_prediction:.2f}ms"
...
class TestResponseValidation:
    ...
    def test_probability_sum_validation(self, client: TestClient):
        ...
        for i in range(10):
            ...
            data = response.json()
            prob_sum = (
                data["HOME_WIN_PROBA"] + data["DRAW_PROBA"] + data["AWAY_WIN_PROBA"]
            )
