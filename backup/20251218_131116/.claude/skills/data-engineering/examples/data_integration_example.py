...
class EnhancedDataLayer:
    ...
    async def get_team_statistics(self, team_id: int, last_n_matches: int = 10) -> Optional[Dict[str, Any]]:
        """获取球队统计信息（带缓存）"""
    ...
    async def batch_update_features(self, features_data: List[Dict[str, Any]]) -> int:
        ...
        try:
            ...
            await self.db_optimizer.execute_query(
                "DELETE FROM match_features WHERE match_id = ANY($1::int[])",
                {"match_ids": match_ids}
            )

            # 批量插入新特征
            inserted_count = await self.db_optimizer.batch_insert(
                "match_features",
                features_data,
                batch_size=1000
            )
    ...
    async def get_upcoming_matches(self, hours_ahead: int = 48) -> List[Dict[str, Any]]:
        ...
        async def fetch_from_db():
            ...
            JOIN teams ht ON m.home_team_id = ht.id
            JOIN teams at ON m.away_team_id = at.id
            WHERE m.date BETWEEN NOW() AND NOW() + INTERVAL '{hours} hours'
    ...
    async def warm_up_cache(self):
        ...
        for match in upcoming_matches:
            ...
            for team_id in [match["home_team_id"], match["away_team_id"]]:
                ...
                if team_stats:
                    warm_up_data.append({
                        "key": {"team_id": team_id, "last_n_matches": 10},
                        "value": team_stats
                    })
