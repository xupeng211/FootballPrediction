...
本项目采用分层的工作流架构：**4 个核心流程**驱动日常 CI/CD/MLOps，外加若干配套工作流（安全扫描、质量门禁、项目同步等）用于专项任务。所有工作流均位于 `.github/workflows/`，历史方案保存在 `archive/` 目录备查。
...
### 4. **project-maintenance-pipeline.yml** - 定期维护

---

## 🔒 支持性工作流

- **security-scan.yml**：按需/定时运行安全基线检查（硬编码密钥、Bandit、Safety、Trivy 等），并在 PR 中回显结果。
- **quality-gate.yml**：作为复用/手动质量门禁，可在其它工作流通过 `workflow_call` 引用或手动触发一次性质量体检。
- **project-sync-pipeline.yml / issue-tracking-pipeline.yml**：保持看板、Issue 与仓库状态同步。
- **deploy.yml**：轻量的发布入口，用于人工触发特定环境的部署。

其余历史或实验性流程均迁移到 `archive/`，如需恢复请先评估是否能以现有核心工作流实现。
