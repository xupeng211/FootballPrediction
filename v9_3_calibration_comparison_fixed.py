...
"""
V9.3 大规模校准方法对比 (修正版)
使用 2188 场比赛的数据集，对比 Platt Scaling vs Isotonic Regression
"""
...
class LargeScaleCalibrationComparator:
    """大规模校准方法对比器"""
    ...
    def prepare_data(self, data_path):
        ...
        # 排除非特征列
        exclude_cols = ['real_home_odds', 'real_draw_odds', 'real_away_odds',
                       'real_match_date', 'real_season', 'actual_home_goals',
                       'actual_away_goals', 'actual_result', 'season', 'match_date']
    ...
    def plot_reliability_curves(self):
        ...
        fig.suptitle('Large-Scale Probability Calibration Comparison (V9.3)', fontsize=16, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig('/home/user/projects/FootballPrediction/V9_3_LARGE_SCALE_CALIBRATION_COMPARISON.png',
                   dpi=150, bbox_inches='tight')
        plt.close()

        print("  ✅ 大规模可靠性曲线图已保存: V9_3_LARGE_SCALE_CALIBRATION_COMPARISON.png")
    ...
    def save_results(self):
        ...
        results_df.to_csv('/home/user/projects/FootballPrediction/V9_3_LARGE_SCALE_CALIBRATION_RESULTS.csv', index=False)
        print("  ✅ 结果已保存: V9_3_LARGE_SCALE_CALIBRATION_RESULTS.csv")


def main():
    """主函数"""
    print("=" * 60)
    print("🎯 V9.3 大规模校准方法对比 - 2188 场比赛")
    print("=" * 60)

    comparator = LargeScaleCalibrationComparator()
    ...
    # 最终建议
    print("\n" + "=" * 60)
    print("💡 V9.3 大规模校准方法建议")
    print("=" * 60)
    print(f"推荐方法: {best_method}")
    print(f"原因: 在大规模数据集中表现最佳 (Brier Score: {comparator.results['best_brier']:.4f})")
    print("\n📋 Brier Score 参考:")
    print("  < 0.25: 优秀")
    print("  0.25-0.30: 良好")
    print("  0.30-0.35: 一般")
    ...
    print(f"\n🎉 V9.3 大规模数据集校准完成!")
    print(f"   数据规模: 2188 场比赛")
    print(f"   最佳方法: {best_method}")
    print(f"   Brier Score: {comparator.results['best_brier']:.4f}")
