...
def test_repositories_create_prediction():
    """测试创建预测端点"""
    response = client.post("/api/v1/repositories/predictions", json={
        "match_id": 123,
        "user_id": 456,
        "predicted_home": 2,
        "predicted_away": 1,
...
def test_repositories_update_prediction():
    """测试更新预测端点"""
    response = client.put("/api/v1/repositories/predictions/123", json={
        "predicted_home": 3,
        "predicted_away": 1,
        "confidence": 0.90
    })
...
def test_repositories_create_user():
    """测试创建用户端点"""
    response = client.post("/api/v1/repositories/users", json={
        "username": "test_user",
...
def test_repositories_date_range_matches():
    """测试获取日期范围内的比赛端点"""
    response = client.get("/api/v1/repositories/matches/date-range?start_date=2024-01-01&end_date=2024-12-31")
    assert response.status_code in [200, 500]
...
def test_repositories_finish_match():
    """测试结束比赛端点"""
    response = client.post("/api/v1/repositories/matches/789/finish", json={
        "actual_home": 2,
        "actual_away": 1
    })
    assert response.status_code in [200, 404, 422, 500]
...
def test_all_repositories_endpoints():
    ...
    endpoints = [
        ...
        ("GET", "/api/v1/repositories/matches/date-range?start_date=2024-01-01&end_date=2024-12-31", [200, 500]),
