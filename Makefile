...
cov.enforce: ## Test: Run coverage with strict 39% threshold
	@$(ACTIVATE) && \
	echo "$(YELLOW)Running coverage with 39% threshold...$(RESET)" && \
	pytest -m "unit" --cov=src --cov-report=term-missing:skip-covered --cov-fail-under=39 && \
	echo "$(GREEN)✅ Coverage passed (>=39%)$(RESET)"
