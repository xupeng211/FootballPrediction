...
def test_observers_alerts_post():
    """测试手动触发告警端点"""
    response = client.post("/api/v1/observers/alerts", json={
...
def test_observers_metrics_update():
    """测试更新指标端点"""
    response = client.post("/api/v1/observers/metrics/update", json={
...
def test_observers_predictions_record():
    """测试记录预测事件端点"""
    response = client.post("/api/v1/observers/predictions/record", json={
...
def test_observers_cache_hit():
    """测试记录缓存命中端点"""
    response = client.post("/api/v1/observers/cache/hit", json={
        "cache_key": "test_key",
...
def test_observers_cache_miss():
    """测试记录缓存未命中端点"""
    response = client.post("/api/v1/observers/cache/miss", json={
        "cache_key": "test_key_missing",
        "cache_type": "predictions"
    })
    assert response.status_code in [200, 201, 422, 500]
