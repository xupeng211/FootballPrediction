...
Independent reviewer 必须是新的 Codex 子进程/会话，运行于 detached read-only worktree；Builder 的 reasoning 不能算 review，reviewer 不得修改、commit、push 或 merge。`make agent-review ...` 使用通用 `codex exec` 的 stdin prompt、只接受受信任且不可由当前用户改写的 Codex CLI、独立 schema、final-message 输出以及 Codex-owned persisted session/index 生成外部 machine-readable receipt；receipt 必须绑定完整 base/head、diff hash、Codex invocation、成功完成事件、persisted final answer、Codex session provenance 和原始输出。`make agent-merge-ready ...` 只回答 `MERGE_READY=YES|NO`，永不 merge；YES 后必须停止并交回 Execution Controller 做 merge/gate 判断。
