...
### ⚡ 安全性修复
- [ ] **1.1** 移除测试文件 `src/bad_example.py`
  - 验收：文件已删除，测试仍然通过
  - 预计时间：2分钟

- [ ] **1.2** 修复硬编码的 API 密钥
  - 文件：`src/collectors/scores_collector.py:31`
  - 改为：从环境变量读取
  - 验收：代码中无硬编码密钥
  - 预计时间：10分钟

- [ ] **1.3** 创建 `.env.example` 模板文件
  - 包含所有必要的环境变量
  - 移除真实密钥值，使用占位符
  - 验收：新开发者可以基于此快速配置
  - 预计时间：15分钟
