...
class MockDataLoader:
    ...
    def _generate_teams(self) -> list:
        """生成球队列表"""
        team_names = [f"球队{i}" for i in range(1, self.num_teams + 1)]
    ...
    async def load_data(self) -> pd.DataFrame:
        ...
        for match_date in pd.date_range("2022-01-01", "2024-12-01", freq="3D"):
            if (
                len(matches) >= self.num_matches * 2
            ):  # 每场比赛生成两条记录（主队+客队）
                break
            ...
            # 基于实力生成期望进球
            team1_xg = min(
                max(team1_strength + home_advantage + np.random.normal(0, 0.1), 0.1),
                0.9,
            )
            team2_xg = min(
                max(team2_strength - home_advantage + np.random.normal(0, 0.1), 0.1),
                0.9,
            )
            ...
            home_result = (
                ...
                else ("draw" if team1_goals == team2_goals else "loss")
            )
            matches.append(
                {
                    "match_id": match_id,
            ...
            away_result = (
                ...
                else ("draw" if team2_goals == team1_goals else "loss")
            )
            matches.append(
                {
                    "match_id": match_id,
...
async def train_mock_model():
    ...
    try:
        ...
        logger.info(
            f"✅ 特征工程完成，新增 {len(df_features.columns) - len(df.columns)} 个特征"
        )
        ...
        logger.info(
            f"🧹 数据清洗: {initial_rows} -> {cleaned_rows} 行 (删除 {initial_rows - cleaned_rows} 行)"
        )
        ...
        logger.info(
            f"📈 胜率 - 训练集: {y_train.mean():.2%}, 测试集: {y_test.mean():.2%}"
        )
        ...
        model_info = {
            ...
            "performance": {"accuracy": float(accuracy), "precision": float(precision)},
            "training_data": {
                "total_samples": len(X),
