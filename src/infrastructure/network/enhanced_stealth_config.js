...
function getAntiDetectionScripts(workerId = 1) {
    // V194.3: 时间种子 - 基于当前 UTC 小时，每小时指纹变化一次
    const currentHour = Math.floor(Date.now() / (1000 * 60 * 60)); // UTC 小时
    ...
    const vendor = WEBGL_CONFIG.vendors[vendorIndex];
    const renderer = WEBGL_CONFIG.renderers[rendererIndex];
    ...
    return `
        ...
        const getParameterProxyHandler = {
            apply: function(target, thisArg, args) {
                ...
                if (param === 34024) {
                    return ${WEBGL_CONFIG.maxRenderbufferSize};
                }
                // MAX_SAMPLES
                if (param === 36183) {
                    return ${WEBGL_CONFIG.maxSamples};
                }
            ...
            if (type === 'image/png' && isFingerprintSize) {
                ...
                if (context) {
                    ...
                    for (let i = 0; i < imageData.data.length; i += 4) {
                        const noise = ((i * 7 + this.width * 13) % 5) - 2; // -2 to 2
        ...
        console.log('[V193.1] 高级隐身装甲已激活: WebGL=' + webglRenderer + ', CPU=' + ${hardwareConcurrency} + '核, RAM=' + ${deviceMemory} + 'GB');
    `;
}
