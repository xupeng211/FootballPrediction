...
const { issueAllocationAuthority, allocationAuthorityFor, bindAllocationEvents, allocationDescriptor, markResolvedDecision, bindRegistryDecisionLedger } = require('./VerifiedAllocationAuthority');
...
function validateAllocationSnapshot(allocation, rawSha256) {
    ...
    if (allocation.fixtures.some(row => !/^fx_[A-Za-z0-9]+$/.test(row.canonical_fixture_id) || !/^evt_[A-Za-z0-9]+$/.test(row.canonical_event_id) || /^(?:fx|evt)_(?:toa|fotmob|odds|provider)/i.test(row.canonical_fixture_id) || /^(?:fx|evt)_(?:toa|fotmob|odds|provider)/i.test(row.canonical_event_id))) throw new Error('allocation snapshot canonical fixture IDs are invalid');
...
function seedFotMobFixtureUniverse({ rawHtml, rawSha256, manifest, allocation = null, allocate = null, mode = 'INITIAL_SEED' }) {
    ...
    // INITIAL_SEED is evidence creation, never an authority bypass: allocator
    // output must satisfy the same immutable artifact contract as replay.
    validateAllocationSnapshot(allocationSnapshot, rawSha256);
...
function resolveOddsEvents({ oddsRawText, oddsRawSha256, universe, decidedAt, decisionLedger = null, priorDecisions = null, authorizedSupersessions = new Set() }) {
    ...
    if (priorDecisions !== null && decisionLedger !== null) throw new Error('decisionLedger and priorDecisions cannot both be supplied');
    const priorLatest = decisionLedger ? decisionLedger.latestDecisions() : priorDecisions === null ? new Map() : require('./IdentityDecisionLedger').projectIdentityDecisionState(priorDecisions, allocationAuthorityFor(universe)).latest;
    const decisions = [], quarantines = [], aliases = [], seenProviderIds = new Set();
    for (const event of odds) {
        if (!event || typeof event.id !== 'string' || !event.id || seenProviderIds.has(event.id)) throw new Error('duplicate or invalid provider event identity in identity batch');
        seenProviderIds.add(event.id);
        const base = { candidate_provider: 'the-odds-api', candidate_provider_event_id: event.id, ruleset_version: RULESET_VERSION, resolver_version: RESOLVER_VERSION, decided_at: decidedAt, raw_sha256: oddsRawSha256, evidence_refs: [snapshot.snapshot_id, event.id] };
        let reason = null;
        ...
        const prior = priorLatest.get(`the-odds-api\u0000${event.id}`) || null;
        if (reason) {
            ...
            if (prior?.decision === 'MATCHED' && prior.canonical_event_id === candidate.canonical_event_id) {
...
// Transaction-v1 uses this entrypoint exclusively.  It is deliberately the
// same resolver/ruleset as the legacy path, but has no persistence capability.
function resolveOddsEventsProspectively(args = {}) {
    if (args.decisionLedger !== undefined && args.decisionLedger !== null) throw new Error('transaction-v1 prospective resolver cannot accept a decision ledger');
    return resolveOddsEvents({ ...args, decisionLedger: null, priorDecisions: args.priorDecisions || [] });
}
