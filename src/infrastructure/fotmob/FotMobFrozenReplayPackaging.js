...
function archiveGroups(rows, repositoryRoot) { const groups = new Map(); for (const row of rows.values()) { if (row.kind !== 'EXISTING_PACKAGE') continue; const id = safeId(row.package_id, 'package_id'); const b = { path: verifyRepositoryExternalRegularFile(row.archive_path, { repositoryRoot }), sha256: row.archive_sha256.toLowerCase() }; const old = groups.get(id); if (old && (old.path !== b.path || old.sha256 !== b.sha256)) fail(`conflicting archive binding for package_id ${id}`, 'SAFETY_ERROR'); groups.set(id, b); } return groups; }
...
function buildReplaySourceIndex(args = {}) {
    ...
    try {
        ...
        if (loose.length) { const members = loose.flatMap(x => { x.payloadMember = `pairs/${x.row.fotmob_match_id}.payload.json`; x.manifestMember = `pairs/${x.row.fotmob_match_id}.manifest.json`; return [{ name: x.payloadMember, bytes: x.payloadBytes }, { name: x.manifestMember, bytes: x.manifestBytes }]; }); const rel = 'packages/historical-reuse-replay.tar.gz'; const tempArchive = path.join(tempRoot, rel); const bytes = deterministicTarGz(members); writeExclusive(tempArchive, bytes, repositoryRoot); const hash = sha256(bytes); verifyArchive(tempArchive, hash, { repositoryRoot }); archives.set('historical-reuse-replay', { path: path.join(outputRoot, rel), sha256: hash, members: inspectArchive(tempArchive, { repositoryRoot }).members, first: loose[0] }); }
        for (const [id, binding] of groupedArchives) { verifyArchive(binding.path, binding.sha256, { repositoryRoot }); archives.set(id, { ...binding, members: inspectArchive(binding.path, { repositoryRoot }).members, first: null }); }
