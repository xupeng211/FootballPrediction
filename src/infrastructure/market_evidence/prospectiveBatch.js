...
function sourceForCapture(capture) {
    ...
    const receiptSha256 = sha256Text(stableStringify(capture));
...
function buildProspectiveMarketEvidenceTransaction(options = {}) {
    ...
    const capture = snapshotPlainData(captureReceipt, 'captureReceipt'); const safeSupportedMarketKeys = snapshotPlainData(supportedMarketKeys, 'supportedMarketKeys'); const safeSupersessions = snapshotPlainData(authorizedSupersessions, 'authorizedSupersessions');
...
function finalizeProspectiveMarketEvidenceTransactionForPublication(candidate) {
    ...
    const latestEvidenceTime = evidenceTimes.length ? Math.max(...evidenceTimes) : now;
    if (!Number.isFinite(latestEvidenceTime) || now < latestEvidenceTime) throw new Error('publisher clock precedes captured evidence');
