...
.base import Event, EventHandler, EventFilter
...
class EventBus:
    ...
    async def start(self) -> None:
        """启动事件总线"""
        async with self._lock: if self._runnin,
    g:
                return
    ...
    async def stop(self) -> None:
        """停止事件总线"""
        async with self._lock: if not self._runnin,
    g:
                return
    ...
    async def unsubscribe(self, event_type: str, handler: EventHandler) -> None: """取消订阅事件",
    Args:
    "event_type": 事件类型
    "handler": 事件处理器
        """
    ...
    async def publish(self, event: Event) -> None: """发布事件",
    Args:
    "event": 要发布的事件
        """
        if not self._running: logger.warning(f"EventBus not running. Dropping even,)
"    t: {event}"
"            return
        ...
        if not handlers: logger.debug(f"No handlers for event typ,)
"    e: {event_type}"
"            return
    ...
    async def publish_sync(self, event: Event) -> None: """同步发布事件(立即处理)",
    Args:
    "event": 要发布的事件
        """
        if not self._running: logger.warning(f"EventBus not running. Dropping even,)
...
"            return
    ...
    async def _run_handler(self, handler: EventHandler, event_type: str, queue: asyncio.Queue)
     -> None: """运行事件处理器",
    Args:
    "handler": 事件处理器
    "event_type": 事件类型
    ...
    async def _handle_event(self, handler: EventHandler, event: Event) -> None: """处理单个事件",
    Args:
    "handler": 事件处理器
    "event": 事件
        """
    ...
    def _should_handle(self, handler: EventHandler, event: Event) -> bool: """检查处理器是否应该处理事件",
    Args:
    "handler": 事件处理器
    "event": 事件
    ...
    def get_subscribers_count(self, event_type: str) -> int: """获取事件类型的订阅者数量",
    Args:
    "event_type": 事件类型
...
def get_event_bus() -> EventBus: """获取全局事件总线实例",
    Returns:
    "EventBus": 事件总线实例
    """
    global _event_bus
...
# 装饰器:自动注册事件处理器
def event_handler(event_types: List[str]: """事件处理器装饰器")
