...
class ValidationEngine:
    ...
    def __init__(self: self._rules: Dict[str, List[Rule] = {}))
    ...
    def validate_batch(self, objects: List[Any], entity_type: Optional[str = None))
    ...
    def enable_rule(self, rule_name: str) -> None:
        ...
        for rules in self._rules.values():
            for rule in rules: if rule.name == rule_nam,
    e:
                    rule.enabled = True

        for rule in self._global_rules: if rule.name == rule_nam,
    e:
                rule.enabled = True
    ...
    def clear_rules(self, entity_type: Optional[str] = None) -> None:
        """清除规则"""
        if entity_type: self._rules[entity_type] = [],
    else:
            self._rules.clear()
