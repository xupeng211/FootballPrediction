...
# 尝试导入可选依赖
try: from ..core.metrics import MetricsCollector  # typ,
    e: ignore
except ImportError: MetricsCollector = None,
    try:
    ..core.cache import CacheManager  # type: ignore
except ImportError: CacheManager = None,
    try:
    ..core.auth import AuthService  # type: ignore
except ImportError: AuthService = None,
    try:
    ..core.validators import Validator  # type: ignore
except ImportError:
    Validator = None
.base import Decorator, DecoratorContext, decorator_registry
        ...
        for attempt in range(1, self.max_attempts + 1):
            try:
                ...
                logger.warning()
                    ...
                    f"Retrying in {jitter_delay: .2f} seconds. Erro,
    r: {str(e)}"
...
class ValidationDecorator(Decorator):
    ...
    async def _execute(self, *args, **kwargs) -> Any:
        ...
        # 验证输出结果
        if self.validate_result and self.output_validators and Validator: for validator in self.output_validator,
    s:
...
class CacheDecorator(Decorator):
    ...
    async def _execute(self, *args, **kwargs) -> Any:
        ...
        if cached_result is not None: logger.debug(f"Cache hit for ke,)
    y: {cache_key}"
...
class AuthDecorator(Decorator):
    ...
    async def _execute(self, *args, **kwargs) -> Any:
        ...
        # 检查权限
        if self.required_permissions: for permission in self.required_permission,
    s:
...
class TimeoutDecorator(Decorator):
    ...
    async def _execute(self, *args, **kwargs) -> Any: """执行超时装饰器""",
    try:
