...
from ..database.connection import get_async_sessionfrom .base import Repository
...
from .provider import RepositoryProvider, get_repository_providerfrom .user import ReadOnlyUserRepository, UserRepository



# 仓储提供者依赖
async def get_repository_provider_dependency() -> RepositoryProvider:
    """获取仓储提供者依赖"" return get_repository_provider()
...
async def get_prediction_repository()
 ...
 -> PredictionRepository:
    """获取预测仓储依赖"" provider = get_repository_provider()
...
async def get_read_only_prediction_repository()
 ...
 -> ReadOnlyPredictionRepository:
    """获取只读预测仓储依赖"" provider = get_repository_provider()
...
async def get_user_repository()
    session: AsyncSession = Depends(get_async_session),
 -> UserRepository:
    """获取用户仓储依赖"" provider = get_repository_provider()
...
async def get_read_only_user_repository()
 ...
 -> ReadOnlyUserRepository:
    """获取只读用户仓储依赖"" provider = get_repository_provider()
...
async def get_match_repository()
    session: AsyncSession = Depends(get_async_session),
 -> MatchRepository:
    """获取比赛仓储依赖"" provider = get_repository_provider()
