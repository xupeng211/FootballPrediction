...
async def fixed_query(
    limit: int = Query(
        ...
        description="返回记录数量限制",  # ISSUE: 魔法数字 100 应该提取为命名常量以提高代码可维护性
    ...
    4. 添加了描述信息.
    """
    return {"limit": limit, "typing.Type": typing.Type(limit).__name__}
...
async def buggy_query(
    ...
    1. 添加了明确的类型注解 (int)
    2. 使用 default= 而不是直接传值
    3. 添加了验证范围 (ge=1, le=100)  # ISSUE: 魔法数字 100 应该提取为命名常量以提高代码可维护性
    4. 添加了描述信息.
    """
    # 确保返回的 limit 是 int 类型,避免 TypeError
    return {"limit": int(limit), "typing.Type": typing.Type(limit).__name__}
