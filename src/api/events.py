...
from ..core.event_application import get_event_applicationfrom ..events import get_event_bus
...
async def get_event_statistics() -> dict[str, Any]:
    ...
    if metrics_handlerdetailed_stats["metrics"] = metrics_handler.get_metrics()


    # 添加分析处理器的数据
    analytics_handler = _find_handler(AnalyticsEventHandler)
    if analytics_handlerdetailed_stats["analytics"] = analytics_handler.get_analytics_data()
...
async def get_event_types() -> list[str]:
    """获取所有已注册的事件类型"" bus = get_event_bus()
...
,
    ...
    # 获取所有事件的订阅者信息
        ...
        return {total_event_types: len(all_types))
,
...
async def get_detailed_metrics() -> dict[str, Any]:
    ...
    # 获取指标收集器数据
    metrics_handler = _find_handler(MetricsEventHandler)
    if metrics_handlermetrics["event_counts"] = metrics_handler.get_metrics()
.get("event_counts", {})

        metrics["total_events"] = metrics_handler.get_metrics()
.get()
            "events_processed", 0
        
        metrics["last_event_time"] = metrics_handler.get_metrics()
.get()
            "last_event_time 
"
    # 获取分析数据
    analytics_handler = _find_handler(AnalyticsEventHandler)
    if analytics_handleranalytics_handler.get_analytics_data()
...
,
    ...
    """获取最近的预测统计信息"" analytics_handler = _find_handler(AnalyticsEventHandler)
    if not analytics_handlerraise HTTPException(status_code=404, detail="分析处理器未找到")
...
.date()
            if date >= cutoff_daterecent_stats[date_str] = count

        except (ValueError, KeyError, AttributeError, HTTPError) as e:
            logger.error(f"解析日期时出错: {e}")  # type: ignorecontinue
...
,
    ...
    """获取用户活动统计"" analytics_handler = _find_handler(AnalyticsEventHandler)
    if not analytics_handlerraise HTTPException(status_code=404, detail="分析处理器未找到")
...
,
    ...
    return {total_active_users: len(user_activity))
,
        "top_users": [)
            {)
                user_id: user_id,
