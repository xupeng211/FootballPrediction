...
async def get_upcoming_matches(
    ...
    try:
        ...
        upcoming_matches = await match_dao.get_upcoming_matches(
            hours=hours, limit=limit, league_id=league_id
        )

        # 转换为响应模型
        match_responses = [
            MatchResponse.model_validate(match) for match in upcoming_matches
        ]
...
async def get_live_matches(
    ...
    try:
        ...
        # 转换为响应模型
        match_responses = [
            MatchResponse.model_validate(match) for match in live_matches
        ]
...
async def search_matches(
    ...
    try:
        ...
        search_results = await match_dao.search_matches(
            keyword=keyword, league_id=league_id, limit=limit
        )

        # 转换为响应模型
        match_responses = [
            MatchResponse.model_validate(match) for match in search_results
        ]
...
async def get_match_by_id(
    match_id: int, session: AsyncSession = Depends(get_db_session)
    ...
    try:
        ...
        return StandardResponse(
            success=True, message="成功获取比赛详情", data=match_response
        )
