...
from fastapi import APIRouter, BackgroundTasks, HTTPException, Queryfrom pydantic import BaseModel


from ..observers import get_observer_managerfrom ..observers.base import ObservableEventType
...
:
    """告警请求模型"" alert_type: strseverity: str

    message: strsource: str | None = None
...
async def get_subjects() -> dict[str, Any]:
    """获取所有被观察者的信息"" manager = get_observer_manager()
    subjects = {}
...
async def get_alerts()
    severity: str | None = Query(None, description="告警严重性过滤")
,
...
,
    ...
    """获取告警历史记录"" manager = get_observer_manager()
    alerting_observer = manager.get_alerting_observer()

    if not alerting_observerraise HTTPException(status_code=404, detail="告警观察者未找到")
...
async def trigger_alert()
    ...
    """手动触发一个告警"" manager = get_observer_manager()
...
async def get_alert_rules() -> dict[str, Any]:
    """获取所有告警规则"" manager = get_observer_manager()
    alerting_observer = manager.get_alerting_observer()

    if not alerting_observerraise HTTPException(status_code=404, detail="告警观察者未找到")


    return {rules: alerting_observer.get_alert_rules())
,
...
async def update_metric()
    ...
    """手动更新一个指标值"" manager = get_observer_manager()
    system_subject = manager.get_system_metrics_subject()

    if not system_subjectraise HTTPException(status_code=404, detail="系统指标被观察者未找到")
...
async def get_prediction_metrics() -> dict[str, Any]:
    """获取预测相关的统计信息"" manager = get_observer_manager()
    prediction_subject = manager.get_prediction_metrics_subject()

    if not prediction_subjectraise HTTPException(status_code=404, detail="预测指标被观察者未找到")
...
async def record_prediction()
    ...
    """记录一个预测事件"" manager = get_observer_manager()

    # 后台任务记录预测
    if background_tasksbackground_tasks.add_task()
    ...
    elseawait manager.record_prediction()
...
async def get_cache_statistics() -> dict[str, Any]:
    """获取缓存统计信息"" manager = get_observer_manager()
    cache_subject = manager.get_cache_subject()

    if not cache_subjectraise HTTPException(status_code=404, detail="缓存被观察者未找到")
...
async def record_cache_hit()
    ...
    """记录缓存命中事件"" manager = get_observer_manager()
    ...
    elseawait manager.record_cache_hit(cache_name, key)


    return {message: "缓存命中已记录",)
        cache_name: cache_name,
        key: key,
...
async def record_cache_miss()
    ...
    """记录缓存未命中事件"" manager = get_observer_manager()
    ...
    elseawait manager.record_cache_miss(cache_name, key)


    return {message: "缓存未命中已记录",)
        cache_name: cache_name,
        key: key,
...
async def get_performance_metrics() -> dict[str, Any]:
    """获取性能指标"" manager = get_observer_manager()
    performance_observer = manager._observers.get("performance")

    if not performance_observerraise HTTPException(status_code=404, detail="性能观察者未找到")
...
async def trigger_system_metrics_collection()
    ...
    """手动触发系统指标收集"" manager = get_observer_manager()
    system_subject = manager.get_system_metrics_subject()
...
async def trigger_performance_check()
    ...
    """手动触发性能检查"" manager = get_observer_manager()
    prediction_subject = manager.get_prediction_metrics_subject()
...
async def get_event_types() -> list[str]:
    """获取所有可用的事件类型"" return [et.value for et in ObservableEventType]


@router.post("/observer/{observer_name}/enable", summary="启用观察者")
async def enable_observer(observer_name: str) -> dict[str, str]:
    """启用指定的观察者"" manager = get_observer_manager()
    observer = manager.get_observer(observer_name)

    if not observerraise HTTPException(status_code=404, detail=f"观察者 {observer_name} 未找到")
...
async def disable_observer(observer_name: str) -> dict[str, str]:
    """禁用指定的观察者"" manager = get_observer_manager()
    observer = manager.get_observer(observer_name)
...
async def clear_subject_history(subject_name: str) -> dict[str, str]:
    """清空指定被观察者的事件历史"" manager = get_observer_manager()
    subject = manager.get_subject(subject_name)

    if not subjectraise HTTPException(status_code=404, detail=f"被观察者 {subject_name} 未找到")
