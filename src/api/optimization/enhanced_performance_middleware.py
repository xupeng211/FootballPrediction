...
class PerformanceMetrics:
    ...
    def record_request(self, method: str, url: str, status_code: int,
                      process_time: float, endpoint: str):
        """记录请求指标"""
    ...
    def get_endpoint_stats(self, endpoint: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'p50_response_time': recent_times[len(recent_times) // 2] if recent_times else 0,
            'p95_response_time': recent_times[int(len(recent_times) * 0.95)] if recent_times else 0,
            'p99_response_time': recent_times[int(len(recent_times) * 0.99)] if recent_times else 0,
    ...
    def get_global_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'error_rate': (self.global_stats['error_count'] / total_requests * 100) if total_requests > 0 else 0,
            'avg_response_time': (self.global_stats['total_time'] / total_requests) if total_requests > 0 else 0,
...
class EnhancedPerformanceMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        # 记录性能指标
        self.metrics.record_request(method, url_path, status_code, process_time, endpoint)
            ...
            if suggestions:
                ...
                for suggestion in suggestions:
                    if suggestion['severity'] in ['critical', 'high']:
                        logger.warning(f"Performance optimization suggestion: {suggestion['message']}")
        except Exception as e:
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'uptime_hours': (time.time() - self.metrics.global_stats['start_time']) / 3600
        }
...
# 全局性能中间件实例
_performance_middleware: Optional[EnhancedPerformanceMiddleware] = None


def get_performance_middleware() -> Optional[EnhancedPerformanceMiddleware]:
    """获取全局性能中间件实例"""
...
def create_performance_middleware(app, enabled: bool = True, detailed_logging: bool = False) -> EnhancedPerformanceMiddleware:
    """创建性能中间件实例"""
