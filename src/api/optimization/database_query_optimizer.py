...
class QueryMetrics:
    ...
    def record_execution(self, execution_time: float, rows_returned: int = 0,
                        rows_examined: int = 0, error: bool = False):
        """记录查询执行"""
...
class DatabasePerformanceAnalyzer:
    ...
    async def analyze_query(self, query_text: str, execution_time: float,
    ...
    async def _record_slow_query(self, query_hash: str, query_text: str, execution_time: float):
        """记录慢查询"""
        slow_query = {
            ...
            'suggestions': await self._generate_slow_query_suggestions(query_text, execution_time)
        }
    ...
    async def _generate_slow_query_suggestions(self, query_text: str, execution_time: float) -> List[str]:
        """生成慢查询优化建议"""
        suggestions = []
    ...
    async def get_queries_with_high_error_rate(self, min_error_rate: float = 5.0) -> List[Dict[str, Any]]:
        """获取错误率高的查询"""
        high_error_queries = []

        for metrics in self.query_metrics.values():
            if metrics.get_error_rate() >= min_error_rate and metrics.execution_count >= 5:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        if not self.query_metrics:
            return {
                ...
                'analysis_duration': (datetime.utcnow() - self.analysis_start_time).total_seconds()
            }
        ...
        slow_queries_count = len([q for q in self.query_metrics.values() if q.avg_time > 1.0])
        high_error_rate_queries = len([q for q in self.query_metrics.values() if q.get_error_rate() > 5.0])
        ...
        for query_hash, metrics in self.query_metrics.items():
            if metrics.avg_time > 1.0:
                suggestion = {
                    ...
                    'suggestions': await self._generate_slow_query_suggestions(metrics.query_text, metrics.avg_time),
    ...
    async def clear_old_data(self, days_to_keep: int = 7):
        ...
        old_queries = []
        for query_hash, metrics in self.query_metrics.items():
            if (metrics.last_executed and
                metrics.last_executed < cutoff_date and
        ...
        logger.info(f"Cleaned up old performance data: removed {len(old_queries)} old queries")


# 全局数据库性能分析器实例
_db_analyzer: Optional[DatabasePerformanceAnalyzer] = None
...
async def initialize_database_analyzer(max_query_history: int = 1000) -> DatabasePerformanceAnalyzer:
    """初始化数据库性能分析器"""
    global _db_analyzer
