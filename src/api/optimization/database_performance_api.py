...
from src.api.optimization.database_performance_middleware import (
    get_database_middleware, get_optimization_advisor
)
...
class QueryAnalysisRequest(BaseModel):
    ...
    analyze_options: Optional[Dict[str, Any]] = Field(None, description="分析选项")


class DatabaseOptimizationRequest(BaseModel):
    """数据库优化请求模型"""
    optimization_type: str = Field(..., description="优化类型: query, pool, index, all")
    target_pool: Optional[str] = Field(None, description="目标连接池名称")
...
async def get_database_performance_status():
    ...
    middleware_metrics = await db_middleware.get_real_time_metrics()
    ...
    status = {
        ...
        "components": {
            ...
            "connection_pool_optimizer": {
                ...
                "total_connections": pools_status.get('summary', {}).get('total_connections', 0),
                "avg_utilization": pools_status.get('summary', {}).get('avg_utilization', 0.0)
            },
            "execution_analyzer": {
                "enabled": True,
                "status": "active",
...
async def analyze_query_performance(request: QueryAnalysisRequest):
    ...
    try:
        ...
        # 执行查询优化建议分析
        optimization_analysis = await optimization_advisor.analyze_query_for_optimization(request.query_text)
...
async def get_high_error_queries(min_error_rate: float = Query(5.0, ge=0.0, le=100.0)):
    ...
    try:
        error_queries = await db_analyzer.get_queries_with_high_error_rate(min_error_rate)
...
async def optimize_connection_pool(pool_name: str, optimization_type: str, parameters: Optional[Dict[str, Any]] = None):
    """优化特定连接池"""
    pool_optimizer = get_connection_pool_optimizer()
...
async def optimize_database_performance(request: DatabaseOptimizationRequest, background_tasks: BackgroundTasks):
    """优化数据库性能"""
    ...
    try:
        if request.optimization_type == "query":
            ...
            result["query_optimization"] = {
                ...
                "high_priority_suggestions": len([s for s in suggestions if s['priority'] in ['critical', 'high']]),
            ...
            if request.target_pool:
                ...
                optimization_result = await pool_optimizer.optimize_pool_manually(
                    request.target_pool,
                    request.parameters.get('operation', 'auto') if request.parameters else 'auto'
                )
            ...
            # 全局优化
            background_tasks.add_task(_run_global_database_optimization, optimization_id, request.parameters)
    ...
    try:
        ...
        report = {
            ...
            "optimization_suggestions": {
                ...
                "critical_priority": len([s for s in optimization_suggestions if s['priority'] == 'critical']),
...
async def _run_global_database_optimization(optimization_id: str, parameters: Optional[Dict[str, Any]]):
    """运行全局数据库优化"""
    try:
...
async def database_performance_health():
    ...
    components = {
        "performance_middleware": "healthy" if db_middleware.is_monitoring else "disabled",
        "connection_pool_optimizer": "healthy" if pool_optimizer.is_monitoring else "disabled",
