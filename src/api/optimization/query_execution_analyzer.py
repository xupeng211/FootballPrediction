...
class ExecutionPlanAnalysis:
    ...
    def _serialize_plan_nodes(self, nodes: List[ExecutionPlanNode]) -> List[Dict[str, Any]]:
        """序列化执行计划节点"""
        result = []
        for node in nodes:
            node_dict = {
...
class QueryExecutionAnalyzer:
    ...
    async def analyze_query_execution_plan(self, query: str, session: AsyncSession,
                                         analyze_options: Optional[Dict[str, Any]] = None) -> ExecutionPlanAnalysis:
        """分析查询执行计划"""
        ...
        try:
            # 获取执行计划
            explain_result = await self._get_execution_plan(query, session, analyze_options)
    ...
    async def _get_execution_plan(self, query: str, session: AsyncSession,
                                options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """获取执行计划"""
        analyze_format = options.get('format', 'JSON') if options else 'JSON'
        analyze_options = options.get('options', ['ANALYZE', 'BUFFERS']) if options else ['ANALYZE', 'BUFFERS']

        # 构建EXPLAIN查询
        explain_query = f"EXPLAIN (FORMAT {analyze_format}, {', '.join(analyze_options)}) {query}"
    ...
    def _parse_text_explain(self, explain_text: str) -> Dict[str, Any]:
        ...
        for line in lines:
            if 'Execution Time:' in line:
                execution_time = float(re.search(r'Execution Time:\s*([\d.]+)', line).group(1))
            elif 'Planning Time:' in line:
                planning_time = float(re.search(r'Planning Time:\s*([\d.]+)', line).group(1))
    ...
    def _parse_execution_plan(self, explain_data: Dict[str, Any]) -> List[ExecutionPlanNode]:
        """解析执行计划"""
    ...
    async def _analyze_execution_performance(self, query: str, query_hash: str,
                                           execution_plan: List[ExecutionPlanNode],
                                           explain_data: Dict[str, Any]) -> ExecutionPlanAnalysis:
        """分析执行性能"""
        # 计算总成本
        ...
        # 识别性能问题
        performance_issues = self._identify_performance_issues(execution_plan, execution_time)
    ...
    def _identify_used_indexes(self, execution_plan: List[ExecutionPlanNode]) -> List[str]:
        """识别使用的索引"""
        indexes = set()
    ...
    async def _detect_missing_indexes(self, query: str, execution_plan: List[ExecutionPlanNode]) -> List[Dict[str, Any]]:
        """检测缺失的索引"""
        missing_indexes = []
            ...
            if node.node_type == 'Seq Scan' and node.relation_name:
                ...
                if where_fields:
                    missing_indexes.append({
                        ...
                        'estimated_impact': 'high' if node.actual_total_time > 1.0 else 'medium'
                    })
    ...
    def _extract_where_fields(self, query: str) -> List[str]:
        ...
        if match:
            ...
            return [field.lower() for field in fields if field.lower() not in ['id', 'created_at', 'updated_at']]
    ...
    def _identify_performance_issues(self, execution_plan: List[ExecutionPlanNode],
    ...
    def _check_node_performance_issues(self, node: ExecutionPlanNode, issues: List[Dict[str, Any]]):
        """检查节点的性能问题"""
        # 检查全表扫描
    ...
    async def _generate_optimization_suggestions(self, query: str,
                                               execution_plan: List[ExecutionPlanNode],
                                               performance_issues: List[Dict[str, Any]],
                                               missing_indexes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """生成优化建议"""
        suggestions = []
    ...
    def get_cached_analysis(self, query_hash: str) -> Optional[ExecutionPlanAnalysis]:
        """获取缓存的分析结果"""
    ...
    def get_analysis_statistics(self) -> Dict[str, Any]:
        ...
        avg_execution_time = sum(record['execution_time'] for record in self.analysis_history) / total_analyzed
        avg_total_cost = sum(record['total_cost'] for record in self.analysis_history) / total_analyzed
...
# 全局查询执行分析器实例
_execution_analyzer: Optional[QueryExecutionAnalyzer] = None
