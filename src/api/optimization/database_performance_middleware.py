...
class DatabasePerformanceMiddleware:
    ...
    def __init__(self, enable_query_tracking: bool = True,
    ...
    def _register_query_listeners(self):
        ...
        def after_cursor_execute_async(conn, cursor, statement, parameters, context, executemany):
            ...
            try:
                ...
                if start_time:
                    ...
                    # 检查慢查询
                    if (self.enable_slow_query_detection and
                        execution_time > self.slow_query_threshold):
        ...
        def handle_error_async(exc, context):
            if self.is_monitoring and hasattr(context, 'statement'):
                asyncio.create_task(
                    self.db_analyzer.analyze_query(
                        context.statement, 0.0, error=True
                    )
                )
    ...
    def _handle_slow_query(self, statement: str, execution_time: float, parameters: Any):
        """处理慢查询"""
        logger.warning(
    ...
    async def analyze_current_performance(self) -> Dict[str, Any]:
        ...
        # 获取高错误率查询
        high_error_queries = await self.db_analyzer.get_queries_with_high_error_rate(5.0)
...
class QueryOptimizationAdvisor:
    ...
    async def analyze_query_for_optimization(self, query_text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'overall_priority': self._determine_overall_priority(suggestions, complexity_score)
        }
    ...
    def _suggest_indexes(self, query_text: str) -> List[str]:
        ...
        if where_match:
            ...
            for field in fields:
                if field not in ['id', 'created_at', 'updated_at']:  # 常见已有索引的字段
    ...
    def _determine_overall_priority(self, suggestions: list, complexity_score: int) -> str:
        """确定整体优先级"""
        if any(s['priority'] == 'high' for s in suggestions) or complexity_score > 10:
            return 'high'
        elif any(s['priority'] == 'medium' for s in suggestions) or complexity_score > 5:
            return 'medium'
        else:
            return 'low'
...
# 全局中间件实例
_db_middleware: Optional[DatabasePerformanceMiddleware] = None
_optimization_advisor: Optional[QueryOptimizationAdvisor] = None
