...
class ConnectionPoolOptimizer:
    ...
    def __init__(self, config: Optional[PoolOptimizationConfig] = None):
            ...
            try:
                ...
                if hasattr(pool, 'size'):
                    ...
                    if metrics.pool_size > 0:
                        metrics.utilization_rate = (metrics.checked_out / metrics.pool_size) * 100
    ...
    async def _scale_up_pool(self, pool_name: str, pool: Pool, metrics: PoolMetrics):
        ...
        try:
            ...
            if isinstance(pool, QueuePool):
                ...
                new_overflow = min(current_overflow + 2, self.config.max_pool_size - pool._pool_size)

                if new_overflow > current_overflow:
                    logger.info(f"Scaling up pool {pool_name}: overflow {current_overflow} -> {new_overflow}")
    ...
    async def _scale_down_pool(self, pool_name: str, pool: Pool, metrics: PoolMetrics):
        ...
        try:
            ...
            if isinstance(pool, QueuePool):
                ...
                if new_overflow < current_overflow:
                    logger.info(f"Scaling down pool {pool_name}: overflow {current_overflow} -> {new_overflow}")
    ...
    async def get_pool_status(self, pool_name: str) -> Optional[Dict[str, Any]]:
        ...
        status = {
            ...
            'metrics': {
                ...
                'creation_time': metrics.creation_time.isoformat() if metrics.creation_time else None,
    ...
    def _evaluate_pool_health(self, metrics: PoolMetrics) -> Dict[str, Any]:
        ...
        if metrics.utilization_rate > 90:
            ...
            health_status['issues'].append(f"连接池利用率过高: {metrics.utilization_rate:.1f}%")
        elif metrics.utilization_rate > 80:
            health_status['overall'] = 'warning'
            health_status['warnings'].append(f"连接池利用率较高: {metrics.utilization_rate:.1f}%")
        ...
        if (metrics.last_activity and
            datetime.utcnow() - metrics.last_activity > timedelta(hours=1)):
    ...
    def _generate_pool_recommendations(self, metrics: PoolMetrics) -> List[str]:
        ...
        if not metrics.last_activity or datetime.utcnow() - metrics.last_activity > timedelta(hours=2):
    ...
    async def get_all_pools_status(self) -> Dict[str, Any]:
        ...
        for pool_name in self.pools:
            ...
            if status:
                ...
                all_status['summary']['total_connections'] += metrics['total_connections']
        ...
        if pool_count > 0:
            all_status['summary']['avg_utilization'] = round(total_utilization / pool_count, 2)

        return all_status

    async def optimize_pool_manually(self, pool_name: str, optimization_type: str, **kwargs) -> Dict[str, Any]:
        """手动优化连接池"""
        if pool_name not in self.pools:
        ...
        try:
            if optimization_type == 'reset_overflow':
                if isinstance(pool, QueuePool):
                    ...
                    result['changes']['overflow'] = {'old': old_overflow, 'new': self.config.max_overflow}
                ...
                result['message'] = f'Unsupported optimization type: {optimization_type}'
