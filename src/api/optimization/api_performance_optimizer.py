...
from src.api.optimization.enhanced_performance_middleware import get_performance_middleware
from src.api.optimization.smart_cache_system import get_cache_manager, get_cache_middleware
...
class PerformanceAnalysisRequest(BaseModel):
    ...
    endpoint_filter: Optional[str] = Field(None, description="端点过滤器")
...
class CacheOptimizationRequest(BaseModel):
    ...
    warm_keys: Optional[List[str]] = Field(None, description="预热的键列表")


class PerformanceOptimizationRequest(BaseModel):
    """性能优化请求模型"""
    optimization_type: str = Field(..., description="优化类型: response_time, cache, database, global")
    target_response_time_ms: Optional[float] = Field(200.0, description="目标响应时间（毫秒）")
...
async def get_performance_status():
    ...
    if perf_middleware:
        status["performance_monitoring"].update(perf_middleware.get_performance_report())
...
async def get_performance_report(
    ...
    if endpoint_filter:
        filtered_endpoints = {}
        for endpoint, stats in perf_middleware.metrics.endpoint_stats.items():
            if endpoint_filter in endpoint:
                filtered_endpoints[endpoint] = perf_middleware.metrics.get_endpoint_stats(endpoint)
...
async def optimize_cache(request: CacheOptimizationRequest, background_tasks: BackgroundTasks):
    """优化缓存性能"""
    cache_manager = get_cache_manager()
    ...
    if request.operation == "clear":
        ...
        result["message"] = f"缓存清理完成，清理了 {result.get('cleared_count', 0)} 个条目"

    elif request.operation == "warm":
        if request.warm_keys:
            # 这里应该实现具体的预热逻辑
            result["message"] = f"缓存预热任务已启动，预热 {len(request.warm_keys)} 个键"
        ...
        # 启动后台优化任务
        background_tasks.add_task(_optimize_cache_performance, cache_manager, operation_id)
...
async def optimize_performance(request: PerformanceOptimizationRequest, background_tasks: BackgroundTasks):
    """优化API性能"""
    perf_middleware = get_performance_middleware()
    if not perf_middleware:
        raise HTTPException(
    ...
    if request.optimization_type == "response_time":
        ...
        # 全局优化
        background_tasks.add_task(
            _run_global_optimization, perf_middleware, optimization_id, request.parameters
        )
...
async def _optimize_cache_performance(cache_manager, operation_id: str):
    ...
    try:
        ...
        logger.info(f"Cache optimization completed for {operation_id}: {optimization_result}")
    except Exception as e:
        logger.error(f"Cache optimization error for {operation_id}: {e}")


async def _run_global_optimization(perf_middleware, optimization_id: str, parameters: Optional[Dict[str, Any]]):
    """运行全局性能优化"""
    try:
        # 获取当前性能报告
        report = perf_middleware.get_performance_report()
