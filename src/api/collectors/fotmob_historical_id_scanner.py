...
class FotMobHistoricalIDScanner:
    ...
    def validate_payload(self, data: Dict, team_id: int) -> Tuple[bool, str]:
        ...
        if len(all_matches) == 0:
            return False, f"Empty 'allMatches' array (0 matches)"
    ...
    def _log_discovery_fingerprint(self, team_id: int, team_name: str,
                                   match_count: int, seasons_found: List[str]):
        """记录发现指纹"""
    ...
    async def discover_league_season(
        ...
        for i, team in enumerate(seed_teams, 1):
            ...
            # 提取目标联赛-赛季的比赛
            target_matches = self.extract_target_matches(
                matches,
                league_id,
                [season]
            )
    ...
    async def discover_all_target_seasons(
        ...
        for league_id in league_ids:
            ...
            for season in seasons:
                matches = await self.discover_league_season(
                    league_id,
                    season,
                    seed_teams
                )
...
async def main():
    ...
    async with FotMobHistoricalIDScanner(concurrency=2) as scanner:
        # 执行发现
        results = await scanner.discover_all_target_seasons(
