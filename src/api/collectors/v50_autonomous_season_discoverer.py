...
class AutonomousSeasonDiscoverer:
        ...
        for attempt in range(self.retry_count):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"请求失败 (尝试 {attempt + 1}/{self.retry_count}): 状态码 {response.status_code}")
    ...
    def discover_league_seasons(self, league_id: int, league_name: str = "") -> LeagueSeasonManifest:
        """
        发现联赛的所有可用赛季
        ...
        logger.info(f"✓ 发现联赛 {league_id} ({manifest.league_name}) 共 {len(manifest.seasons)} 个赛季")

        return manifest

    def _parse_seasons_data(self, data: dict, league_id: int, league_name: str) -> LeagueSeasonManifest:
        """
        解析 API 响应中的赛季数据
        ...
        return LeagueSeasonManifest(league_id=league_id, league_name=league_name, seasons=seasons)
    ...
    def get_historical_seasons(self, league_id: int, league_name: str = "", years_back: int = 5) -> list[SeasonInfo]:
        """
        获取过去 N 年的赛季列表
        ...
        # 筛选最近 N 年的赛季
        historical = [s for s in manifest.seasons if s.start_year >= cutoff_year]

        logger.info(f"✓ 联赛 {league_id} 过去 {years_back} 年 共 {len(historical)} 个赛季")
...
def discover_seasons_for_leagues(
    league_ids: list[int], league_names: dict[int, str] = None, years_back: int = 5
    ...
    try:
        for league_id in league_ids:
            ...
            seasons = discoverer.get_historical_seasons(league_id, league_name, years_back)
...
if __name__ == "__main__":
    ...
    for season in epl_manifest.seasons:
        ...
        print(f"  - {season.season_name} (ID: {season.season_id}, 代码: {season.season_code}){current}")
