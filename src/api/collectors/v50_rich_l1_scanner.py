...
class RichL1Scanner:
    ...
    async def _fetch_with_retry(self, url: str, method: str = "GET") -> dict | None:
        """
        带重试机制的 HTTP 请求
        ...
        for attempt in range(self.retry_count):
            try:
                ...
                async with self.session.request(method, url, headers=self._get_random_headers()) as response:
                    ...
                    if response.status == 200:
                        ...
                        logger.warning(f"请求失败 (尝试 {attempt + 1}/{self.retry_count}): 状态码 {response.status}")
    ...
    async def fetch_league_matches(
        self, league_id: int, league_name: str, season_info: SeasonInfo
    ) -> list[RichL1Match]:
        """
        ...
        logger.info(f"🔍 扫描 {league_name} - {season_info.season_name} (赛季代码: {season_info.season_code})")
        ...
        # 解析比赛数据
        matches = self._parse_league_matches(data, league_id, league_name, season_info)
    ...
    def _parse_league_matches(
        self, data: dict, league_id: int, league_name: str, season_info: SeasonInfo
    ) -> list[RichL1Match]:
        """
        解析联赛比赛数据（Rich L1）
    ...
    async def scan_league_historical(
        self, league_id: int, league_name: str = "", years_back: int = 5
    ) -> list[RichL1Match]:
        """
        扫描联赛的历史赛季数据
        ...
        """
        # 获取历史赛季
        seasons = self.season_discoverer.get_historical_seasons(league_id, league_name, years_back)

        if not seasons:
            logger.warning(f"联赛 {league_id} 没有可用赛季")
            return []

        logger.info(f"🏆 开始扫描 {league_name} 过去 {years_back} 年 共 {len(seasons)} 个赛季")
        ...
        logger.info(f"✅ {league_name} 扫描完成: {len(all_matches)} 场比赛")
    ...
    async def scan_multiple_leagues(
        ...
        Returns:
            Rich L1 比赛列表
        """
        logger.info(f"🌍 开始全球广域扫描: {len(league_configs)} 个联赛 × 过去 {years_back} 年")
...
# 便捷函数
async def quick_scan_league(league_id: int, league_name: str = "", years_back: int = 5) -> list[dict]:
    """
    快速扫描单个联赛
...
async def quick_scan_multiple_leagues(league_configs: list[tuple[int, str]], years_back: int = 5) -> list[dict]:
    """
    快速扫描多个联赛
    ...
    """
    async with RichL1Scanner() as scanner:
        matches = await scanner.scan_multiple_leagues(league_configs, years_back)
        scanner.print_summary()
        return [m.to_dict() for m in matches]
