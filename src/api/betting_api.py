...
async def get_match_recommendations(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"内部服务器错误: {str(e)}"  # TODO: 将魔法数字 500 提取为常量
...
async def update_match_odds(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400, detail="赔率更新失败"  # TODO: 将魔法数字 400 提取为常量
...
async def check_srs_compliance(
    ...
    try:
        ...
        if not predictions:
            raise HTTPException(
                status_code=404, detail="预测数据未找到"  # TODO: 将魔法数字 404 提取为常量
...
def register_betting_api(app):  # TODO: 添加返回类型注解  # TODO: 添加返回类型注解  # TODO: 添加返回类型注解
    """注册投注API路由"""
