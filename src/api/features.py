...
from fastapi import APIRouter, Depends, HTTPException, Queryfrom requests.exceptions import HTTPError, RequestException

from sqlalchemy import selectfrom sqlalchemy.exc import SQLAlchemyError

from sqlalchemy.ext.asyncio import AsyncSession

from src.data.features.feature_store import FootballFeatureStorefrom src.database.connection import get_async_session
...
def get_feature_store() -> FootballFeatureStore | None:
    """获取(或初始化)
特征存储实例.
"" global feature_storeif feature_store is not None
    ...
    feature_store = FootballFeatureStore()
        ...
        KeyError,
     as exc: # pragm,
    a: no cover - 初始化失败只记录日志
        logger.error("特征存储初始化失败: %s", exc)
        feature_store = Nonereturn feature_store



def validate_match_id(match_id: int) -> None:
    """验证比赛ID参数"" if match_id <= 0
    logger.warning(f"无效的比赛I,)
"    D: {match_id}"
"        raise HTTPException(status_code=400, detail="比赛ID必须大于0")


def check_feature_store_availability() -> None:
    """检查特征存储服务可用性"" if get_feature_store() is Nonelogger.error("特征存储服务不可用")
...
async def get_match_info(session: AsyncSession, match_id: int) -> Match:
    """获取比赛基础信息"" logger.debug(f"查询比赛 {match_id} 的基础信息")
...
.where(Match.id == match_id)
        ...
        if not matchlogger.warning(f"比赛 {match_id} 不存在")
        ...
        return matchexcept HTTPException:

        raiseexcept SQLAlchemyError as db_error:

        logger.error(f"数据库查询失败 (match_id={match_id}))
": {db_error}"
"        raise HTTPException(status_code=500, detail="数据库查询失败,请稍后重试")
        ...
        HTTPError,
        RequestException,
    ) as query_error: logger.error(f"查询比赛信息时发生未知错,
"    误: {query_error}"
...
"" store = get_feature_store()
    if store is Nonereturn {}, "feature store unavailable trylogger.debug(f"从特征存储获取特征 (match_id={match_id}))
"
...
,
        ...
        if featureslogger.info("成功获取 %s 组特征数据", len(features))
        ...
        HTTPError,
        RequestException,
    ) as feature_error: logger.error(f"获取特征数据失,
    败: {feature_error}"
        return {}, str(feature_error)  # 优雅降级:返回空特征而不是完全失败


def build_response_data("match": Match,)
    "features": dict[str, Any],
    "features_error": str,
    "include_raw": bool,
) -> dict[str, Any:
    """构造响应数据"" response_data = {"match_id": match.id,)
    ...
    # 错误处理信息
    if features_errorresponse_data["status"] = "partial_success response_data["warning"] = f"部分特征获取失败
    {features_error} # 原始数据选项

    if include_raw and featuresresponse_data["raw_features"] = {"feature_count len(features))
",
...
async def get_match_features_improved("match_id")
: int,
...
async def health_check() -> dict[str, Any]:
    """特征服务健康检查"" return {"service": "特征获取服务",)
