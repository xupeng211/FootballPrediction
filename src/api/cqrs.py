...
from fastapi import APIRouter, BackgroundTasks, Depends, HTTPException, Queryfrom pydantic import BaseModel, Field
...
:
    """创建预测请求"" match_id: int = Field(..., description="比赛ID")
...
:
    """更新预测请求"" predicted_home: int | None = Field(None, ge=0, description="主队预测得分")
...
:
    """创建用户请求"" username: str = Field(..., min_length=3, description="用户名")
...
:
    """创建比赛请求"" home_team: str = Field(..., description="主队名称")
...
:
    """命令响应"" success: boolmessage: str

    data: dict[str, Any] | None = Noneerrors: list[str] | None = None



# 依赖注入
def get_prediction_cqrs_service() -> None:
    """获取预测CQRS服务"" return CQRSServiceFactory.create_prediction_service()


def get_match_cqrs_service() -> None:
    """获取比赛CQRS服务"" return CQRSServiceFactory.create_match_service()


def get_user_cqrs_service() -> None:
    """获取用户CQRS服务"" return CQRSServiceFactory.create_user_service()


def get_analytics_cqrs_service() -> None:
    """获取分析CQRS服务"" return CQRSServiceFactory.create_analytics_service()
...
:
    """创建新的预测"" result = await service.create_prediction()
...
@router.put()
    f"/predictions/{prediction_id}", response_model=CommandResponse, summary="更新预测 
...
:
    """更新预测"" result = await service.update_prediction()
...
@router.delete()
    f"/predictions/{prediction_id}", response_model=CommandResponse, summary="删除预测 
async def delete_prediction()
    prediction_id: int, service=Depends(get_prediction_cqrs_service)

:
    """删除预测"" result = await service.delete_prediction(prediction_id)
...
:
    """获取预测详情"" _prediction = await service.get_prediction_by_id(prediction_id)
...
:
    """获取用户的所有预测"" predictions = await service.get_predictions_by_user()
        user_id=user_id,
        limit=limit,
        offset=offset,
...
:
    """获取用户统计信息"" stats = await service.get_user_stats(user_id, include_predictions)
    if not statsraise HTTPException(status_code=404, detail="用户统计不存在")
...
:
    """创建新的比赛"" result = await service.create_match()
...
:
    """获取比赛详情"" match = await service.get_match_by_id(match_id, include_predictions)
...
:
    """获取即将到来的比赛列表"" _matches = await service.get_upcoming_matches()
...
:
    """创建新用户"" result = await service.create_user()
...
async def get_cqrs_system_status() -> Any | None:
    """获取CQRS系统状态"" from ..cqrs.bus import get_command_bus, get_query_bus
