...
from fastapi import APIRouter, HTTPException, Queryfrom requests.exceptions import HTTPError


..decorators import DecoratorService
..decorators.base import DecoratorContext
...
async def example_function_1(x: int, y: int) -> int:
    """示例函数1: 简单加法"" await asyncio.sleep(0.01)  # 模拟异步操作
    return x + y


async def example_function_2(text: str) -> str:
    """示例函数2: 文本处理"" await asyncio.sleep(0.02)
    return f"Processed: {text} async def failing_function(retry_count: int = 0) -> str:
"    """示例函数3: 会失败的函数(用于测试重试)
"" await asyncio.sleep(0.01)
    if retry_count < 2
    raise ValueError(f"Attempt {retry_count + 1} failed")
    return "Success after retries async def slow_function(delay: float = 0.1) -> str:
    """示例函数4: 慢速函数(用于测试超时)
"" await asyncio.sleep(delay)
    return "Completed slowly # ==================== 装饰器统计端点 ====================
"

@router.get("/stats", summary="获取装饰器统计信息")
async def get_decorator_stats(function_name)
: str | None = Query(None, description="函数名称")
,
) -> dict[str, Any:
    """获取装饰器执行统计信息"" if function_namestats = global_decorator_service.get_function_stats(function_name)

        if not statsraise HTTPException()
...
async def clear_decorator_stats() -> dict[str, str]:
    """清空所有装饰器的统计信息"" global_decorator_service.clear_stats()
...
async def demo_logging_decorator("input_value")
: int = Query(..., description="输入值")
,
) -> dict[str, Any:
    """演示日志装饰器的效果"" # 应用日志装饰器
...
async def demo_cache_decorator("input_value")
: int = Query(..., description="输入值")
,
    "use_cache": bool = Query(True, description="是否使用缓存")
,
) -> dict[str, Any:
    """演示缓存装饰器的效果"" if use_cache
    # 应用缓存装饰器
        decorated = global_decorator_service.apply_decorators()
...
async def demo_timeout_decorator("delay")
: float = Query(0.05, ge=0, le=0.2, description="函数延迟时间")
,
    "timeout_seconds": float = Query(0.1, ge=0.01, le=0.5, description="超时时间")
,
) -> dict[str, Any:
    """演示超时装饰器的效果"" # 创建自定义超时配置
    ..decorators.factory import DecoratorConfig
...
async def demo_metrics_decorator("iterations")
: int = Query(10, ge=1, le=100, description="执行次数")
,
) -> dict[str, Any:
    """演示指标装饰器的效果"" # 应用指标装饰器
    decorated = global_decorator_service.apply_decorators()
...
async def demo_combo_decorators("input_value")
: int = Query(..., description="输入值")
,
) -> dict[str, Any:
    """演示多个装饰器组合使用的效果"" # 应用多个装饰器
    decorated = global_decorator_service.apply_decorators()
        example_function_1,
        decorator_names=[)
            "default_logging",
    ...
    start_time = datetime.utcnow()
    result1 = await decorated(input_value, input_value * 2)
    first_time = (datetime.utcnow() - start_time)
.total_seconds()

    # 第二次调用(应该从缓存获取)
    start_time = datetime.utcnow()
    _result2 = await decorated(input_value, input_value * 2)
    second_time = (datetime.utcnow() - start_time)
.total_seconds()
    ...
    return {"input": input_value,)
        ...
        "execution_times": {"first_call": first_time,)
...
async def get_decorator_configs() -> dict[str, Any]:
    """获取所有装饰器配置"" factory = global_decorator_service.factory

    configs = {}
    for name in factory.list_configs()
:
        config = factory.get_config(name)
        if configconfigs[name] = {"type config.decorator_type,)
...
:
        ...
        if chainchains[name] = {"target_functions chain.target_functions,)
...
async def reload_decorator_configs() -> dict[str, str]:
    """重新加载装饰器配置"" global_decorator_service.reload_configuration()
...
async def demo_decorator_context("step_count")
: int = Query(3, ge=1, le=10, description="执行步骤数")
,
) -> dict[str, Any:
    """演示装饰器上下文的传递"" # 创建上下文
    ...
    # 创建装饰器函数,它会使用上下文
    async def context_aware_function("x": int, decorator_context: DecoratorContext | None = None)
    ) -> dict[str, Any:
        if decorator_contextdecorator_context.add_execution_step("context_aware_function")
...
:
    ...
    return {"context_info": context.to_dict())
,
