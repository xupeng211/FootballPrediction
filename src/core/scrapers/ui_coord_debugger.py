...
class UIComponentTester:
    ...
    def _validate_url_format(self, url: str) -> None:
        ...
        for indicator in summary_indicators:
            if indicator in normalized_url:
                print("[!] Component [0]: 检测到 Summary 页面 URL")
                print("[!] Component [0]: 建议使用包含比赛哈希的 Detail 页面 URL")
                return
        ...
        if "-" not in last_segment or len(last_segment) < 8:
            print("[!] Component [0]: URL 可能不是 Detail 页面")
            print("[!] Component [0]: 建议确认 URL 格式是否正确")
    ...
    async def discover_components(self, headless: bool = False) -> list[ComponentDiscovery]:
        ...
        async with async_playwright() as p:
            ...
            # 预扩展环境取证
            pre_screenshot = await self._capture_execution_snapshot(
                page, phase=ExpansionPhase.PRE_EXPANSION
            )

            # V41.212: 递归式 UI 激活
            print("[*] Component [0]: 启动递归式 UI 激活")
            ...
            for depth in range(expansion_config["max_depth"]):
                ...
                # 脱敏输出
                print(
                    f"[*] Component [0]: 扩展轮次 {depth + 1}: "
                    f"扫描={record.element_count}, 点击={record.clicked_count}, "
            ...
            # 后扩展环境取证
            post_screenshot = await self._capture_execution_snapshot(
                page, phase=ExpansionPhase.POST_EXPANSION
            )

            print(f"[*] Component [0]: 预扩展取证: {pre_screenshot}")
            print(f"[*] Component [0]: 后扩展取证: {post_screenshot}")

            # 增强型降级定位
            for idx, label in enumerate(labels, start=1):
                # 脱敏输出 - 不显示具体标签名称
                print(f"[*] Component [{idx}]: 搜索中...")
                ...
                if coords is None:
                    ...
                    print(f"    Component [{idx}]: Found (坐标计算失败)")
                    continue
                ...
                print(f"    Component [{idx}]: Found -> ({coords['x']:.1f}, {coords['y']:.1f})")
    ...
    def _save_discovery_report(self, discoveries: list[ComponentDiscovery]) -> None:
        ...
        print(f"\n[*] Component [0]: 报告已保存: {output_file.absolute()}")
...
async def main():
    ...
    print("=" * 60)
    print("V41.212 Dynamic Content Inventory Expansion")
    print("=" * 60)
    ...
    try:
        ...
        print("执行摘要")
        print("=" * 60)
        found = sum(1 for d in discoveries if d.found)
        total = len(discoveries)
        print(f"发现率: {found}/{total} ({found / total * 100:.1f}%)")
