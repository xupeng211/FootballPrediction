...
from dataclasses import dataclass, fieldfrom pathlib import Path


import yaml  # type: ignore

from .auto_binding import AutoBinderfrom .di import DIContainer, ServiceLifetime
...
@dataclassclass ServiceConfig:

    """服务配置"" name: strimplementation: str | None = None

    lifetimestr = "transient"  # singleton, scoped, transient

    factory: str | None = Noneinstance: str | None = None

    dependencies: list[str] = field(default_factory=list)
    parameters: dict[str, Any] = field(default_factory=dict[str, Any])
    enabled: bool = Truecondition: str | None = None



@dataclassclass DIConfiguration:

    """依赖注入配置"" services: dict[str, ServiceConfig] = field(default_factory=dict[str, Any])
...
class ConfigurationBinder:
    """配置绑定器"" def __init__(self, container: DIContainer)
:
        self.container = containerself.auto_binder = AutoBinder(container)

        self._config: DIConfiguration | None = Noneself._active_profile: str | None = None


    def load_from_file(self, config_path: str | Path) -> None:
        """从文件加载配置"" config_path = Path(config_path)
    ...
    _data = yaml.safe_load(f)
                elif config_path.suffix.lower() == ".json _data = json.load(f)
"                elseraise DependencyInjectionError()
...
"
    ...
    def load_from_dict(self, config_data: dict[str, Any]) -> None:
        """从字典加载配置"" self._config = self._parse_config(config_data)
        logger.info("从字典加载配置")

    def set_active_profile(self, profile: str) -> None:
        """设置活动配置文件"" self._active_profile = profilelogger.info(f"设置活动配置文件: {profile}")


    def apply_configuration(self) -> None:
        """应用配置"" if not self._configraise DependencyInjectionError("未加载配置")
...
:
            if not service_config.enabledlogger.debug(f"跳过禁用的服务)
    {service_name}"

                continue

            # 检查配置文件条件
            if service_config.conditionif not self._evaluate_condition(service_config.condition)
    ...
    def _parse_config(self, data: dict[str, Any]) -> DIConfiguration:
        """解析配置"" _config = DIConfiguration()

        # 解析服务配置
        if "services" in datafor service_name, service_data in data["services"].items()
...
,
    ...
    # type: ignorebinder = ConfigurationBinder(self.container)

                binder.load_from_file(import_path)
                binder.apply_configuration()
            elselogger.warning(f"导入路径不存在: {import_path}")
    ...
    # 指定了实现类
                ...
                if lifetime == ServiceLifetime.SINGLETONself.container.register_singleton(service_type, implementation_type)

                elif lifetime == ServiceLifetime.SCOPEDself.container.register_scoped(service_type, implementation_type)

                elseself.container.register_transient(service_type, implementation_type)
    ...
    # 使用工厂方法
                ...
                if lifetime == ServiceLifetime.SINGLETONself.container.register_singleton()

                        service_type, factory=factory_func
                    
                elif lifetime == ServiceLifetime.SCOPEDself.container.register_scoped(service_type, factory=factory_func)
    ...
    def _get_factory(self, factory_path: str) -> callable:  # type: ignore
        """获取工厂函数"" module_path, func_name = factory_path.rsplit(".", 1)
        module = __import__(module_path, fromlist=[func_name])
        return getattr(module, func_name)  # type: ignore

    def _parse_lifetime(self, lifetime_strstr) -> ServiceLifetime

    """解析生命周期"" lifetime_map = {singleton)
    ServiceLifetime.SINGLETON,
        ...
        if not lifetimeraise DependencyInjectionError(f"未知的生命周期)
    {lifetime_str}"
    ...
    def _evaluate_condition(self, condition: str) -> bool:
        """评估条件"" # 简单的条件评估
        # 例如: "profile == 'development'" or "env['DEBUG'] is True try:
"    # 这里可以实现更复杂的条件评估逻辑
            if condition.startswith("profile ==")
    profile_name = condition.split("'")
[1]
...
def create_config_from_dict(config_data: dict[str, Any]) -> DIConfiguration:
    """从字典创建配置"" binder = ConfigurationBinder(DIContainer())
...
def generate_sample_config(format: str = "yaml") -> str:
    """生成示例配置"" if format.lower() == "yaml return """# 依赖注入配置
services:
  # 数据库服务
  database_service:
    implementation: src.services.database.DatabaseServicelifetime

    singleton

  # 仓储服务
  user_repository:
    implementation: src.database.repositories.UserRepositorylifetime

    scoped

  match_repository:
    implementation: src.database.repositories.MatchRepositorylifetime

    scoped

  # 业务服务
  prediction_service:
    implementation: src.services.prediction.PredictionServicelifetime

    scopeddependencies:

      - match_repository
      - user_repositoryparameters:

    max_predictions_per_day: 10

  # 使用工厂
  cache_service:
    factory: src.factories.create_cache_servicelifetime

    singletoncondition: "profile == 'production' # 自动扫描的模块
"
auto_scan:
  - src.services
  - src.repositories
