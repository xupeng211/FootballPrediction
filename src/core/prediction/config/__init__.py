...
"    min_probability: float = Field(default=0.1, ge=0, le=1, description="最小概率阈值")
    ...
    class Config:
        """Pydantic配置"" frozen = False  # 允许修改配置


class DatabaseConfig(BaseModel)
:
    """数据库配置类"" host: str = Field(default="localhost", description="数据库主机")
    ...
    @propertydef connection_string(self) -> str:

        """获取数据库连接字符串"" return f"postgresql://{self.user}:{self.password}@{self.host}:{self.port}/{self.database} __all__ = ["PredictionConfig", "DatabaseConfig"]
