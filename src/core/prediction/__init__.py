...
:
        """更新统计"" self.total_predictions += 1
        if is_correctself.correct_predictions += 1

        self.accuracy = ()
            self.correct_predictions / self.total_predictionsif self.total_predictions > 0
...
:
        """初始化预测引擎"" self._config = config or PredictionConfig()
        self.statistics = PredictionStatistics()
        self._initialized = False

    async def initialize(self)
:
        """初始化引擎"" self._initialized = True
