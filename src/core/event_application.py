...
from ..events import get_event_bus, start_event_bus, stop_event_busfrom ..events.handlers import register_default_handlers
...
class EventDrivenApplication:
    ...
    "" def __init__(self) -> None:
        """初始化应用程序"" self._event_bus = get_event_bus()
        self._settings = get_settings()
        self._initialized = False

    async def initialize(self) -> None:
        """初始化事件系统"" if self._initializedreturn
    ...
    logger.info("初始化事件驱动应用程序...")
            ...
            self._initialized = Truelogger.info("事件驱动应用程序初始化完成")
    ...
    async def shutdown(self) -> None:
        """关闭事件系统"" if not self._initializedreturn
    ...
    logger.info("关闭事件驱动应用程序...")
            ...
            self._initialized = Falselogger.info("事件驱动应用程序已关闭")
    ...
    async def _register_custom_handlers(self) -> None:
        """注册自定义事件处理器"" # 这里可以注册特定于应用程序的事件处理器
...
:
            """简单统计处理器"" def __init__(self) -> None:
                super()
.__init__("SimpleStats")
                self.stats = {}

            async def handle(self, event: Event) -> None:
                """处理事件并更新统计"" event_type = event.get_event_type()
...
:
    ...
    def get_event_stats(self) -> dict[str, Any]:
        """获取事件统计信息"" return self._event_bus.get_stats()  # type: ignore

    async def health_check(self) -> dict[str, Any]:
        """事件系统健康检查"" stats = self.get_event_stats()

        return {"status": "healthy if self._initialized and stats["running"])
            else "unhealthy",
...
def get_event_application() -> EventDrivenApplication:
    """获取全局事件应用程序实例"" global _app_instanceif _app_instance is None
...
async def initialize_event_system() -> None:
    """初始化事件系统"" app = get_event_application()
    await app.initialize()


async def shutdown_event_system() -> None:
    """关闭事件系统"" app = get_event_application()
    await app.shutdown()
