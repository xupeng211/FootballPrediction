...
class V19_3HardenedPipeline:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                v18_features = self._extract_v18_features(
                    row, home_standings=home_standings, away_standings=away_standings
                )
    ...
    def _extract_v18_features(
        self, row: dict, home_standings: dict | None = None, away_standings: dict | None = None
    ) -> dict[str, float] | None:
        """提取 V18 基础特征"""
        try:
            ...
            if "home_stats" in raw_data and "away_stats" in raw_data:
                return self._extract_from_simple_format(
                    raw_data, home_standings=home_standings, away_standings=away_standings
                )
                    ...
                    if "Periods" in stats_data and "All" in stats_data["Periods"]:
                        ...
                        if "stats" in period_stats:
                            ...
                            return self._extract_from_stats_list(
                                stats_list, home_standings=home_standings, away_standings=away_standings
                            )
    ...
    def _extract_from_simple_format(
        self, raw_data: dict, home_standings: dict | None = None, away_standings: dict | None = None
    ) -> dict[str, float] | None:
        """从简化格式提取特征"""
        try:
    ...
    def _extract_from_stats_list(
        self, stats_list: list, home_standings: dict | None = None, away_standings: dict | None = None
    ) -> dict[str, float] | None:
    ...
    def compute_feature_importance(self, X: np.ndarray, y: np.ndarray, feature_cols: list[str]) -> pd.DataFrame:
        """
        计算特征重要性
    ...
    def filter_features(self, importance_df: pd.DataFrame, feature_df: pd.DataFrame) -> tuple[list[str], list[str]]:
        """
        过滤特征：保留高重要性特征
    ...
    def train_final_model(
        self, feature_df: pd.DataFrame, feature_cols: list[str], train_size: int = 650, test_size: int = 110
    ) -> HardenedMetrics:
        """
        训练最终生产模型
        ...
        # 计算 Brier Score
        brier = brier_score_loss(pd.get_dummies(y_test).values, test_proba, sample_weight=None)
...
def main():
    ...
    try:
        ...
        for fold in metrics.cv_fold_results:
            print(
                f"  Fold {fold['fold']}: Train={fold['train_accuracy']:.2%}, Test={fold['test_accuracy']:.2%}, ROI={fold['roi']:.2f}%"
            )
...
def generate_comparison_report(metrics: HardenedMetrics):
    ...
    report = {
        ...
        "improvement": {
            ...
            "stability_improvement": 12.02 - metrics.cv_std_roi
            if metrics.cv_std_roi < 12.02
            else -(metrics.cv_std_roi - 12.02),
        },
    }
    ...
    print("-" * 65)
    print(
        f"{'5折平均 ROI':<20} {report['v19_2_audit']['avg_roi']:<15.2f} {report['v19_3_hardened']['avg_roi']:<15.2f} {report['improvement']['roi_improvement']:<15.2f}"
    )
    print(
        f"{'ROI 标准差':<20} {report['v19_2_audit']['roi_std']:<15.2f} {report['v19_3_hardened']['roi_std']:<15.2f} {report['improvement']['stability_improvement']:<15.2f}"
    )
    print(
        f"{'训练集准确率':<20} {report['v19_2_audit']['train_accuracy']:<15.2%} {report['v19_3_hardened']['train_accuracy']:<15.2%} {'N/A':<15}"
    )
    print("=" * 80)
