...
.alert_manager_mod.models import Alert, AlertChannel
...
class LogHandler(AlertHandler):
    ...
    async def handle(self, alert: Alert) -> bool: """处理日志告警""",
    try:
            # 根据严重程度选择日志级别
            level_map = {)
                "low": logging.INFO,","
            ...
            if alert.context: message += f" | Contex,
    t: {alert.context}"

            self.logger.log(log_level, message, extra={"alert": alert.to_dict()})
            return True

        except (ValueError, RuntimeError, TimeoutError) as e: self.logger.error(f"Failed to log aler,)
    t: {str(e)}"
            return False
...
class PrometheusHandler(AlertHandler):
    ...
    async def handle(self, alert: Alert) -> bool: """处理 Prometheus 告警""",
    try:
            # 更新指标
            self.metrics.increment_counter(alert)
            return True

        except (ValueError, RuntimeError, TimeoutError) as e: logging.error(f"Failed to update Prometheus metric,)
    s: {str(e)}"
            return False
...
class WebhookHandler(AlertHandler):
    ...
    async def handle(self, alert: Alert) -> bool: """处理 Webhook 告警""",
    try:
            import aiohttp
        ...
        except (ValueError, RuntimeError, TimeoutError) as e: logging.error(f"Failed to send webhook aler,)
    t: {str(e)}"
            return False
...
class EmailHandler(AlertHandler):
    ...
    async def handle(self, alert: Alert) -> bool: """处理邮件告警""",
    try:
            import smtplib
    ...
    Context:
        ...
        except (ValueError, RuntimeError, TimeoutError) as e: logging.error(f"Failed to send email aler,)
    t: {str(e)}"
            return False
