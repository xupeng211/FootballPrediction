...
class MetricsDefinitions:
    ...
    def __init__(
        ...
        # 系统健康指标
        self.system_info = self._create_info(
            "football_system_info",
            "系统信息"
        )
    ...
    def _create_histogram(self, name: str, description: str, labels: list, buckets=None):
        """创建或获取 Histogram 指标"""
        if not PROMETHEUS_AVAILABLE:
        ...
        try:
            return Histogram(name, description, labels, registry=self.registry, buckets=buckets)
        except ValueError:
            # 指标已存在，返回 Mock 实例
            return _MockHistogram()
