...
class DatabaseMetrics:
    ...
    async def update_table_row_counts(self, table_counts: Optional[Dict[str, int]] = None) -> None:
        """
        更新数据表行数统计
        ...
        if table_counts:
            ...
            for table_name, count in table_counts.items():
                try:
                    self.metrics.table_row_count.labels(
            ...
            async with get_async_session() as session:
                ...
                for table_name in self.tables_to_monitor:
                    try:
                        ...
                        self.metrics.table_row_count.labels(
                            table_name=table_name
                        ).set(float(row_count) if row_count is not None else 0.0)
