...
class GreedyMiner:
    ...
    def extract_all_features(self, json_data: Dict, match_id: Optional[int] = None) -> Dict[str, Any]:
        """
        无差别提取所有特征
    ...
    def _extract_string_feature(self, value: str, path: str, key: str, features: Dict) -> None:
        ...
        extractable_patterns = [
            'name', 'team', 'player', 'referee', 'stadium', 'venue',
            'league', 'season', 'competition', 'country'
        ]
    ...
    def _get_all_keys(self, obj: Any, keys: Optional[set] = None) -> set:
        """递归获取所有键"""
        if keys is None:
            keys = set()
