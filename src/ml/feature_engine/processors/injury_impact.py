...
class InjuryImpactProcessor(BaseProcessor[MatchData]):
    ...
    def __init__(self, config: Optional[InjuryImpactProcessorConfig] = None) -> None:
    ...
    def _get_history_lineups(
        self, context: Any, side: HomeAway
    ) -> List[LineupSnapshot]:
        """
        获取历史阵容数据
    ...
    def _analyze_team_impact(
        ...
        if expected_starters:
            ...
            features[f"{prefix}_expected_starters_present"] = round(
                present_count / len(expected_starters) if expected_starters else 1.0,
                4
            )
        else:
            # 无历史数据，使用默认值
        ...
        if history:
            ...
            features[f"{prefix}_lineup_continuity"] = round(
                overlap / total if total > 0 else 0.0,
                4
            )
        else:
    ...
    def _identify_expected_starters(
        ...
        expected_starters = [
            ...
            if p.player_id and p.player_id in expected_ids
        ]
    ...
    def _analyze_missing_starters(
        ...
        if total_expected_value > 0:
            features[f"{prefix}_missing_starter_value_ratio"] = round(
                missing_value / total_expected_value,
                4
            )
        else:
            features[f"{prefix}_missing_starter_value_ratio"] = 0.0
    ...
    def _analyze_stability(
        ...
        if similarities:
            features[f"{prefix}_availability_stability"] = round(
                statistics.mean(similarities),
                4
            )
            # 变动率（1 - 稳定性）
            features[f"{prefix}_turnover_rate"] = round(
                1 - features[f"{prefix}_availability_stability"],
                4
            )
        else:
            features[f"{prefix}_availability_stability"] = 0.5
    ...
    def _analyze_bench_impact(
        ...
        # 替补平均评分（如果可用）
        ratings = [
            p.team_rating for p in bench_players
            if p.team_rating is not None
        ]
        if ratings:
    ...
    def _compute_impact_scores(self, features: Dict[str, float]) -> Dict[str, float]:
        ...
        scores["home_composite_health"] = round(
            (1 - home_missing) * 0.5 + home_stability * 0.3 + home_bench * 0.2,
            4
        )
        ...
        scores["away_composite_health"] = round(
            (1 - away_missing) * 0.5 + away_stability * 0.3 + away_bench * 0.2,
            4
        )

        # 健康差值
        scores["diff_composite_health"] = round(
            scores["home_composite_health"] - scores["away_composite_health"],
            4
        )
