...
if __name__ == "__main__":
    ...
    # 测试场景 A: 势均力敌，双强防守
    print("\n📊 场景 A: 势均力敌 + 双强防守 (高平局概率)")
    print("-" * 60)

    result_a = calc.calculate(
        home_xg_l5=[1.2, 1.0, 1.1, 0.9, 1.3],   # 平均 1.1
        away_xg_l5=[1.1, 1.0, 1.2, 1.1, 1.0],   # 平均 1.08
        home_def_rating=8.0,
        away_def_rating=7.5,
        home_recent_scores=[[1, 1], [0, 0], [1, 0]],
        away_recent_scores=[[0, 0], [1, 1], [0, 1]],
    )

    print(f"  xG 平衡指数:     {result_a.xg_balance_index:.3f}")
    print(f"  双强防守指数:    {result_a.double_wall_score:.3f}")
    print(f"  低比分动量:      {result_a.scoreless_momentum:.3f}")
    print(f"  综合平局概率:    {result_a.combined_draw_probability:.3f}")
    ...
    result_b = calc.calculate(
        home_xg_l5=[2.5, 2.8, 3.0, 2.2, 2.6],   # 平均 2.62
        away_xg_l5=[0.8, 0.6, 0.9, 0.7, 0.5],   # 平均 0.7
        home_def_rating=6.0,
        away_def_rating=4.0,
        home_recent_scores=[[3, 0], [2, 1], [4, 1]],
        away_recent_scores=[[0, 2], [1, 3], [0, 4]],
    )

    print(f"  xG 平衡指数:     {result_b.xg_balance_index:.3f}")
    print(f"  双强防守指数:    {result_b.double_wall_score:.3f}")
    print(f"  低比分动量:      {result_b.scoreless_momentum:.3f}")
    print(f"  综合平局概率:    {result_b.combined_draw_probability:.3f}")
    if result_b.warnings:
        print(f"  警告: {result_b.warnings}")
    ...
    result_c = calc.calculate(
        home_xg_l5=[1.5, 1.3, 1.7, 1.2, 1.4],   # 平均 1.42
        away_xg_l5=[1.0, 1.2, 0.9, 1.1, 1.0],   # 平均 1.04
        home_def_rating=6.5,
        away_def_rating=6.0,
        home_recent_scores=[[2, 1], [1, 1], [1, 2]],
        away_recent_scores=[[1, 1], [2, 2], [1, 0]],
    )

    print(f"  xG 平衡指数:     {result_c.xg_balance_index:.3f}")
    print(f"  双强防守指数:    {result_c.double_wall_score:.3f}")
    print(f"  低比分动量:      {result_c.scoreless_momentum:.3f}")
    print(f"  综合平局概率:    {result_c.combined_draw_probability:.3f}")
    if result_c.warnings:
        print(f"  警告: {result_c.warnings}")
