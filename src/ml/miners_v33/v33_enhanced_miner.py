...
class V33EnhancedMiner:
    ...
    def _calc_shotmap_stats(self, home_shots: list, away_shots: list) -> dict[str, float]:
        ...
        features["shot_distance_diff"] = home_stats.get("shot_home_avg_distance", 0) - away_stats.get(
            "shot_away_avg_distance", 0
        )
    ...
    def _extract_environment_features(self, json_data: dict) -> dict[str, float]:
        ...
        try:
            ...
            # 天气综合指标
            features["env_is_bad_weather"] = float(
                1 if features["env_weather_rain"] > 5 or features["env_weather_wind"] > 20 else 0
            )
    ...
    def _calculate_interaction_features(self, all_features: dict) -> dict[str, float]:
        ...
        try:
            ...
            if "shot_count_diff" in all_features and "momentum_mean" in all_features:
                features["inter_shot_momentum"] = all_features["shot_count_diff"] * all_features["momentum_mean"]
            ...
            if "momentum_positive_ratio" in all_features and "env_weather_rain" in all_features:
                features["inter_positive_momentum_rain"] = all_features["momentum_positive_ratio"] * (
                    1 + all_features["env_weather_rain"] / 10
                )

            # 复合交互特征
            if "shot_home_count" in all_features and "tech_expected_goals_home" in all_features:
                features["inter_shot_xg_efficiency"] = all_features["tech_expected_goals_home"] / max(
                    1, all_features["shot_home_count"]
                )
            if "shot_away_count" in all_features and "tech_expected_goals_away" in all_features:
                features["inter_away_shot_xg_efficiency"] = all_features["tech_expected_goals_away"] / max(
                    1, all_features["shot_away_count"]
                )
            ...
            shot_keys = [
                "shot_home_count",
                "shot_away_count",
                "shot_avg_distance_diff",
                "shot_home_avg_distance",
                "shot_away_avg_distance",
            ]
    ...
    def _extract_percentage(self, value: Any) -> float | None:
        """从字符串中提取百分比"""
