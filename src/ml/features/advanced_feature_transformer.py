...
class AdvancedFeatureTransformer:
    ...
    def __init__(self, config: Optional[AdvancedFeatureConfig] = None):
        ...
        logger.info(f"Phase 5 特征: 场馆={self.config.enable_venue_features}, "
                   f"H2H={self.config.enable_h2h_features}, 积分={self.config.enable_points_features}")
        logger.info(f"Phase 8 特征: PlayerRatings={self.config.enable_player_ratings_features}, "
    ...
    def transform_for_prediction(self, match_data: pd.DataFrame,
    ...
    def _update_feature_list(self, feature_type: str):
        ...
        if feature_type == 'venue':
            venue_features = [
                'venue_home_goals_rolling_3', 'venue_home_goals_rolling_5',
                'venue_away_goals_rolling_3', 'venue_away_goals_rolling_5',
                'venue_home_vs_away_diff_3', 'venue_home_vs_away_diff_5',
                'venue_home_advantage_3', 'venue_home_advantage_5'
            ]
            ...
            player_ratings_features = [
                # 首发评分特征
                'xi_rating_diff', 'xi_rating_ratio',
                # 球星评分特征
                'star_rating_diff', 'star_rating_ratio',
                # 替补实力特征
                'bench_rating_diff', 'bench_rating_ratio',
                # 综合实力特征
                'home_team_strength', 'away_team_strength', 'strength_diff', 'strength_ratio'
            ]
            ...
            metadata_features = [
                # 上座率特征
                'attendance_normalized', 'high_attendance_flag', 'medium_attendance_flag', 'low_attendance_flag',
                # 裁判特征
                'referee_encoded', 'has_referee_data',
                # 体育场特征
                'stadium_encoded', 'has_stadium_data',
                # 主场优势特征
            ...
            discipline_features = [
                # 红牌特征
                'total_red_cards', 'red_cards_diff', 'home_red_cards_flag', 'away_red_cards_flag', 'any_red_cards_flag',
                # 半场进球特征
                'total_goals_ht', 'goals_ht_diff', 'goals_ht_ratio',
                # 比赛动态特征
                'home_goals_ft_vs_ht', 'away_goals_ft_vs_ht', 'ht_scoring_intensity',
                'ht_leading_to_ft_win', 'comeback_flag'
            ]
    ...
    def _add_player_ratings_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 5. 离散化特征 (评分等级)
        df_result['home_xi_rating_tier'] = pd.cut(df_result['home_xi_rating'],
                                                  bins=[0, 6, 7, 8, 10],
                                                  labels=['Weak', 'Average', 'Strong', 'Elite'])
        df_result['away_xi_rating_tier'] = pd.cut(df_result['away_xi_rating'],
                                                  bins=[0, 6, 7, 8, 10],
                                                  labels=['Weak', 'Average', 'Strong', 'Elite'])
    ...
    def _add_metadata_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_result['medium_attendance_flag'] = ((df_result['attendance'] >= 30000) & (df_result['attendance'] <= 50000)).astype(int)
    ...
    def _add_discipline_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_result['ht_leading_to_ft_win'] = (
            ((df_result['home_goals_ht'] > df_result['away_goals_ht']) & (df_result['home_score'] > df_result['away_score'])) |
            ((df_result['away_goals_ht'] > df_result['home_goals_ht']) & (df_result['away_score'] > df_result['home_score']))
        ).astype(int)
