...
class V26_5_ProductionAdapter(BaseFeatureAdapter):
    ...
    def adapt(self, raw_features: dict[str, Any]) -> AdaptationResult:
        ...
        try:
            ...
            logger.debug(f"滚动统计 [{home_team}]: xg={home_stats['rolling_xg']:.2f}, "
                        f"shots={home_stats['rolling_shots_on_target']:.1f}, "
                        f"poss={home_stats['rolling_possession']:.1f} ({home_stats['matches_count']} 场)")
            logger.debug(f"滚动统计 [{away_team}]: xg={away_stats['rolling_xg']:.2f}, "
            ...
            if home_xg is None or away_xg is None:
                home_xg, away_xg = self._extract_fotmob_stat(raw_features, "expected_goals", default_home=1.0, default_away=1.0)
                home_possession, away_possession = self._extract_fotmob_stat(raw_features, "BallPossesion", default_home=50.0, default_away=50.0)
                home_shots, away_shots = self._extract_fotmob_stat(raw_features, "total_shots", default_home=10.0, default_away=10.0)
            else:
                # 使用简单格式
                home_possession = self._safe_get(
                    raw_features, "content", "stats", "home", "possession", "percentage", default=50
    ...
    def _extract_fotmob_stat(self, raw_data: dict, stat_key: str, default_home: float = 0.0, default_away: float = 0.0) -> tuple[float, float]:
        """
...
class FeatureAdapterFactory:
    ...
    @classmethod
    def register_adapter(cls, model_type: ModelType, adapter: BaseFeatureAdapter):
        """注册新的特征适配器"""
