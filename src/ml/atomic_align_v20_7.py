...
V20_7_FULL_SCHEMA = {
    # === shotmap 合成特征 (66 维) ===
    # All 时段 (22 个)
    "home_all_synth_total_shots",
    "home_all_synth_xg",
    "home_all_synth_avg_distance",
    "home_all_synth_shots_on_target",
    "home_all_synth_big_chances",
    "home_all_synth_peak_xg",
    "home_all_synth_peak_minute",
    "home_all_synth_volatility",
    "away_all_synth_total_shots",
    "away_all_synth_xg",
    "away_all_synth_avg_distance",
    "away_all_synth_shots_on_target",
    "away_all_synth_big_chances",
    "away_all_synth_peak_xg",
    "away_all_synth_peak_minute",
    "away_all_synth_volatility",
    "diff_all_synth_total_shots",
    "diff_all_synth_xg",
    "diff_all_synth_shots_on_target",
    "diff_all_synth_big_chances",
    "total_all_synth_total_shots",
    "total_all_synth_xg",
    # FirstHalf 时段 (22 个)
    ...
    "away_FH_synth_total_shots",
    "away_FH_synth_xg",
    "away_FH_synth_avg_distance",
    "away_FH_synth_shots_on_target",
    "away_FH_synth_big_chances",
    "away_FH_synth_peak_xg",
    "away_FH_synth_peak_minute",
    "away_FH_synth_volatility",
    "diff_FH_synth_total_shots",
    "diff_FH_synth_xg",
    "diff_FH_synth_shots_on_target",
    "diff_FH_synth_big_chances",
    "total_FH_synth_total_shots",
    "total_FH_synth_xg",
    # SecondHalf 时段 (22 个)
    "home_SH_synth_total_shots",
    "home_SH_synth_xg",
    "home_SH_synth_avg_distance",
    "home_SH_synth_shots_on_target",
    "home_SH_synth_big_chances",
    "home_SH_synth_peak_xg",
    "home_SH_synth_peak_minute",
    "home_SH_synth_volatility",
    "away_SH_synth_total_shots",
    "away_SH_synth_xg",
    "away_SH_synth_avg_distance",
    "away_SH_synth_shots_on_target",
    "away_SH_synth_big_chances",
    "away_SH_synth_peak_xg",
    "away_SH_synth_peak_minute",
    "away_SH_synth_volatility",
    "diff_SH_synth_total_shots",
    "diff_SH_synth_xg",
    "diff_SH_synth_shots_on_target",
    "diff_SH_synth_big_chances",
    "total_SH_synth_total_shots",
    "total_SH_synth_xg",
}
...
class AtomicBackfillEngine:
        ...
        for period_name in missing_periods:
            if period_name in periods_data:
                ...
                if isinstance(shots, list) and shots:
                    ...
                    period_features = self.shotmap_aggregator.synthesize_period_features(shots, period_name)

                    # 关键: 将合成特征映射回原 stats 字段名
                    # home_FH_synth_xg -> home_FirstHalf_expected_goals
                    mapped_features = self._map_synth_to_stats_fields(period_features, period_name)
    ...
    def _map_synth_to_stats_fields(self, synth_features: dict, period_name: str) -> dict[str, Any]:
        """
        将合成特征映射回原 stats 字段名
...
class PlayerTeamCascadeAggregator:
    ...
    def extract_player_team_aggregates(self, content: dict, features: dict) -> dict[str, Any]:
        """
        提取球员-球队级联聚合特征
        ...
        for metric in self.CORE_PLAYER_METRICS:
            home_total = self._aggregate_metric_for_team(player_stats, home_player_ids, metric)
            away_total = self._aggregate_metric_for_team(player_stats, away_player_ids, metric)
    ...
    def _aggregate_metric_for_team(self, player_stats: dict, player_ids: list[str], metric: str) -> int:
        """为指定球队的球员聚合指标"""
        total = 0
...
class MomentumDeepAnalyzer:
    ...
    def extract_deep_momentum_features(self, content: dict) -> dict[str, Any]:
        """
        提取深度动量特征
        ...
        if home_values and away_values:
            joint_features = self._calculate_joint_momentum_features(home_values, away_values)
            features.update(joint_features)

        logger.info(f"📈 动量深度解剖: 生成 {len(features)} 个特征")
        return features

    def _find_momentum_data(self, content: dict) -> dict | None:
    ...
    def _calculate_momentum_derivatives(self, values: list[float], team: str) -> dict[str, Any]:
        """
        计算动量的导数特征
        ...
        features[f"{team}_momentum_neg_acceleration_mean"] = (
            float(np.mean(neg_accelerations)) if neg_accelerations else 0.0
        )
    ...
    def _calculate_joint_momentum_features(self, home_values: list[float], away_values: list[float]) -> dict[str, Any]:
        """计算主客队联合动量特征"""
        features = {}
...
class SchemaAlignmentEnforcer:
    ...
    def enforce_schema_alignment(self, features: dict[str, Any]) -> dict[str, Any]:
        """
        强制执行 Schema 对齐
...
def atomic_align_v20_7(
    content: dict, features: dict, home_team_id: int, away_team_id: int
...
if __name__ == "__main__":
    ...
    # 执行对齐
    initial_features = {}
    aligned_features, metrics = atomic_align_v20_7(content, initial_features, home_team_id, away_team_id)
