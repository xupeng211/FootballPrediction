...
class ClassificationDatasetGenerator:
    ...
    def _merge_features_and_raw_data(self, features_data: List[Dict[str, Any]],
                                   raw_match_data: pd.DataFrame) -> pd.DataFrame:
        """
        合并特征数据和原始比赛数据
    ...
    def _apply_filters(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        self.logger.info(f"过滤完成: {final_size}/{initial_size} 场比赛保留 (过滤率: {total_filtered/initial_size*100:.1f}%)")
    ...
    def _prepare_final_dataset(self, filtered_data: pd.DataFrame) -> pd.DataFrame:
        ...
        final_columns = [
            ...
            'final_home_score', 'final_away_score', 'target_label', 'target_numeric',
            'feature_completeness_score', 'data_quality_flag'
        ] + feature_columns
...
# 便捷函数
async def create_classification_dataset(league_id: str,
                                      start_date: str,
                                      output_path: Optional[str] = None,
                                      min_completeness_score: float = 0.8) -> pd.DataFrame:
    """
    创建分类数据集的便捷函数
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        try:
            ...
            print("使用示例:")
            print(">>> dataset = await generator.generate_dataset('premier_league', '2024-01-01')")
            print(">>> await generator.save_dataset_to_parquet(dataset, 'training_data.parquet')")
