...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/feature_scout_v24.log'),
...
class FeatureScout:
    ...
    def load_data(self) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        try:
            with conn.cursor() as cur:
                ...
                version_counts = {}
                for r in records:
                    v = r.get('meta_data', {}).get('extraction_version', 'unknown') if isinstance(r.get('meta_data'), dict) else 'unknown'
    ...
    def select_spartans(
        self, result: FeatureImportanceResult
    ) -> List[str]:
        """
        筛选 300 强特征
        ...
        logger.info(f"300 强特征筛选完成")

        return spartans

    def save_results(
        self, result: FeatureImportanceResult, spartans: List[str]
    ) -> None:
        """
        保存结果到文件
...
def main():
    ...
    parser.add_argument(
        ...
        help='输出 Top N 特征（默认: 50）'
    )
    parser.add_argument(
        '--spartans', type=int, default=300,
        help='300 强特征数量（默认: 300）'
    )
