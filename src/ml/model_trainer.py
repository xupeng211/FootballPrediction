...
class ModelTrainer:
    ...
    def load_training_data(self) -> Tuple[pd.DataFrame, Optional[pd.Series]]:
        """加载训练数据"""
        try:
    ...
    def _create_target_variable(self, df: pd.DataFrame) -> pd.Series:
        ...
        # 创建三分类结果
        y = pd.Series(np.where(score_diff > 0.5, 'home_win',
                      np.where(score_diff < -0.5, 'away_win', 'draw')))
    ...
    def save_model(self, model: lgb.LGBMClassifier) -> bool:
        ...
        try:
            ...
            with open(report_path, 'w', encoding='utf-8') as f:
                f.write(f"FootballPrediction V7.0 训练报告\n")
