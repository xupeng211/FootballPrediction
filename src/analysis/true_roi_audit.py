...
class TrueROIAuditor:
    ...
    def load_odds_csv(self) -> pd.DataFrame:
        ...
        # 选择需要的列
        combined = combined[[
            'date', 'home_team_db', 'away_team_db',
    ...
    def plot_profit_curve(self, save_path: Optional[str] = None):
        """绘制盈利曲线"""
        ...
        if betting_df1 is not None:
            ax1.plot(range(len(betting_df1)), betting_df1['cumulative_profit'],
                    linewidth=2, color='#E63946', label='策略1: 赔率隐含概率', alpha=0.7)
        if betting_df2 is not None:
            ax1.plot(range(len(betting_df2)), betting_df2['cumulative_profit'],
                    linewidth=2, color='#2E86AB', label='策略2: 理想模型 (58.95%)', alpha=0.7)
        ...
        ax3 = axes[1, 0]
        if betting_df1 is not None:
            ax3.hist(betting_df1['profit'], bins=30, color='#E63946', alpha=0.5,
                    label='策略1', edgecolor='black')
        if betting_df2 is not None:
            ax3.hist(betting_df2['profit'], bins=30, color='#2E86AB', alpha=0.5,
                    label='策略2', edgecolor='black')
                ...
                for j in range(5):
                    ...
                    if j == 3:
                        ...
                        table[(i, j)].set_text_props(
                            color='green' if roi_val > 0 else 'red',
                            weight='bold'
                        )
