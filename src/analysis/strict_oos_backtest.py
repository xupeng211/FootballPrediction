...
class StrictOOSBacktest:
    ...
    def __init__(self):
        ...
        self.scaler = None
        self.metadata = None
        self.odds_data: Optional[pd.DataFrame] = None
    ...
    def load_match_data_with_features(self) -> pd.DataFrame:
        ...
        # 检查是否有特征相关列
        feature_cols = [row['column_name'] for _, row in check_df.iterrows()
                       if 'feature' in row['column_name'].lower() or
                          'rolling' in row['column_name'].lower() or
                          'vector' in row['column_name'].lower()]
    ...
    def prepare_split_data(self, odds_df: pd.DataFrame, match_df: pd.DataFrame):
        ...
        if overlap:
            print(f"  ⚠️ 警告: 训练集和测试集有日期重叠!")
        else:
            print(f"  ✓ 无日期重叠 - 时序隔离验证通过")
    ...
    def run_baseline_backtest(
        ...
        def is_prediction_correct(row):
            ...
            result = row['result']
            return (
                (bet_on == 'h' and result == 'H') or
                (bet_on == 'd' and result == 'D') or
                (bet_on == 'a' and result == 'A')
            )
    ...
    def plot_profit_curve(self, results: Dict, save_path: Optional[str] = None):
        """绘制盈利曲线"""
        ...
        # 1. 累计收益曲线
        ax1 = axes[0, 0]
        ax1.plot(range(len(betting_df)), betting_df['cumulative_profit'],
                linewidth=2, color='#2E86AB', label='累计收益')
        ...
        ax3.axvline(x=betting_df['profit'].mean(), color='red', linestyle='-', linewidth=2, label=f'均值: {betting_df["profit"].mean():.3f}')
        ax3.set_xlabel('收益')
        ax3.set_ylabel('频次')
