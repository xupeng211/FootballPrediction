...
print(f"  ✅ 模型加载成功: V19.4 Draw Sensitivity")
...
def extract_features_from_raw(row):
    ...
    try:
        ...
        # 处理 diff/total 格式
        if np.isnan(features['home_rolling_shots_on_target']) and 'diff_shots_on_target' in tech and 'total_shots_on_target' in tech:
...
audit_df = pd.merge(
    df_predictions,
    merged_df[['home_team', 'away_team', 'result_score', 'b365_home_odds', 'b365_draw_odds', 'b365_away_odds', 'match_time']],
...
audit_df['predicted_result'] = audit_df.apply(
    lambda row: get_prediction(row['model_prob_h'], row['model_prob_d'], row['model_prob_a']),
    axis=1
)
