...
class FeatureFactory:
    ...
    def _extract_starting_11_features(
        ...
        # 计算首发平均评分
        home_ratings = [
            self.path_resolver.get_player_rating(p) for p in home_starters
        ]
        away_ratings = [
            self.path_resolver.get_player_rating(p) for p in away_starters
        ]
        ...
        home_missing_stars = sum(
            1 for p in home_unavailable
            if self.path_resolver.get_player_rating(p, 0) >= 7.5
        )
        away_missing_stars = sum(
            1 for p in away_unavailable
            if self.path_resolver.get_player_rating(p, 0) >= 7.5
        )
