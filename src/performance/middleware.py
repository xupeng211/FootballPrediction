...
class PerformanceMonitoringMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            # 保留最近1000个请求的时间
            if len(self.request_times) > 1000: self.request_times = self.request_times[-100,)
    0:
...
class DatabasePerformanceMiddleware:
    ...
    def __init__(self: self.query_stats: Dict[str, Dict[str, Any] = {}))
    ...
    :
        ...
        if duration > 0.1:
            ...
            # 保留最近100个慢查询
            if len(self.slow_queries) > 100: self.slow_queries = self.slow_queries[-10,)
    0:
...
class CachePerformanceMiddleware:
    ...
    def __init__(self: self.cache_stats = {))
            "hits": 0,","
            "misses": 0,","
            "sets": 0,","
            "deletes": 0,","
    ...
    def record_cache_miss(self: """记录缓存未命中""")
    ...
    def record_cache_delete(self: """记录缓存删除""")
...
class BackgroundTaskPerformanceMonitor:
    ...
    def __init__(self: self.task_stats: Dict[str, Dict[str, Any] = {}))
    ...
    def end_task(self, task_id: str, success: bool = True, error: Optional[str] = None):
        ...
        if success: stats["success_count"] += 1,
            ...
            # 保留最近100个失败任务
            if len(self.failed_tasks) > 100: self.failed_tasks = self.failed_tasks[-10,)
    0:
