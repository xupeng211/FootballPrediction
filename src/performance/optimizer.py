...
from .monitoring import get_performance_analyzer, get_system_monitor
...
class PerformanceOptimizer:
            ...
            try:
                ...
                if "already exists" in str(e).lower():
                    ...
                    results["failed"].append(
                        {"name": index_config["name"], "error": str(e)}
                    )
                    logger.error(f"Failed to create index {index_config['name']}: {e}")
                    await self.db_session.rollback()

        logger.info(
            f"Database index optimization completed: "
    ...
    async def implement_query_cache(self) -> dict[str, Any]:
        ...
        cache_results = {"cached": [], "failed": [], "total": len(cacheable_queries)}

        for query_name, query_info in cacheable_queries.items():
            try:
                # 执行查询
                result = await self.db_session.execute(
                    text(query_info["sql"]), query_info["params"]
                )
                ...
                if result.returns_rows:
                    ...
                    data_list = [dict(zip(columns, row, strict=False)) for row in data]
                else:
                    data_list = []
                ...
                logger.info(
                    f"Cached query result: {query_name} ({len(data_list)} rows)"
                )

            except Exception as e:
                cache_results["failed"].append({"name": query_name, "error": str(e)})
                logger.error(f"Failed to cache query {query_name}: {e}")

        logger.info(
            f"Query caching completed: "
    ...
    async def optimize_cache_configuration(self) -> dict[str, Any]:
        ...
        optimization_actions = {
            "increase_cache_sizes": ["prediction_result", "team_stats", "match_data"],
            "decrease_cache_sizes": ["season_data", "league_data"],
            "add_warmup_queries": ["popular_teams", "active_matches"],
            "implement_cache_invalidation": ["user_predictions", "team_standings"],
        }
    ...
    async def monitor_performance_improvements(self) -> dict[str, Any]:
        ...
        return {
            ...
            "recommendations": recommendations,
            "alerts": alerts,
            "improvement_status": self._calculate_improvement_status(
                performance_summary
            ),
        }
