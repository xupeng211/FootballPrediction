...
class PerformanceMonitoringIntegration:
    ...
    def integrate_with_fastapi(self, app: FastAPI) -> None:
        ...
        try:
            # 导入中间件
            .api import router as performance_router
            .middleware import PerformanceMonitoringMiddleware
        ...
        except (ValueError, RuntimeError, TimeoutError) as e: logger.error(f"Failed to integrate performance monitorin,)
"    g: {str(e)}"
"
    ...
    try:
            .profiler import DatabaseQueryProfiler, get_profiler
        ...
        except (ValueError, RuntimeError, TimeoutError) as e: logger.error(f"Failed to initialize database monitorin,)
"    g: {str(e)}"
"
    ...
    try:
        ...
        except (ValueError, RuntimeError, TimeoutError) as e: logger.error(f"Failed to initialize cache monitorin,)
"    g: {str(e)}"
"
    ...
    try:
        ...
        except (ValueError, RuntimeError, TimeoutError) as e: logger.error(f"Failed to initialize task monitorin,)
"    g: {str(e)}"
"
    def start_profiling(self) -> None:
        ...
        try:
            .profiler import start_profiling

            start_profiling()
            logger.info("Performance profiling started")

        except (ValueError, RuntimeError, TimeoutError) as e: logger.error(f"Failed to start profilin,)
"    g: {str(e)}"
"
    def stop_profiling(self) -> None: """停止性能分析""",
    try:
            .profiler import stop_profiling
        ...
        except (ValueError, RuntimeError, TimeoutError) as e: logger.error(f"Failed to stop profilin,)
"    g: {str(e)}"
"
    ...
    def update_config(self, config: dict[str, Any]) -> None: """更新性能监控配置""",
    try:
            # 更新采样率
        ...
        except (ValueError, RuntimeError, TimeoutError) as e: logger.error(f"Failed to update performance monitoring confi,)
"    g: {str(e)}"
"
    ...
    "task_stats": dict[str, Any] = {}  # 从task_monitor获取  # type: ignore
        ...
        except (ValueError, RuntimeError, TimeoutError) as e: logger.error(f"Failed to create performance repor,)
"    t: {str(e)}"
"            return None
    ...
    try:
        ...
        except (ValueError, RuntimeError, TimeoutError) as e: logger.error(f"Failed to setup performance alertin,)
"    g: {str(e)}"
"
    ...
    try:
        ...
        except (ValueError, RuntimeError, TimeoutError) as e: logger.error(f"Failed to cleanup performance monitorin,)
"    g: {str(e)}"
"
...
def setup_performance_monitoring("app": FastAPI | None = None,)
