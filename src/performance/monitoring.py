    ...
    def check_performance_alerts(self) -> List[Dict[str, Any]]:
        ...
        if current_metrics.response_time_avg > 2.0:
            alerts.append({
                ...
                'severity': 'warning' if current_metrics.response_time_avg < 5.0 else 'critical',
        ...
        if current_metrics.error_rate > 0.05:  # 5%
            alerts.append({
                ...
                'severity': 'warning' if current_metrics.error_rate < 0.1 else 'critical',
...
class PerformanceAnalyzer:
    ...
    def analyze_trends(self, hours: int = 1) -> Dict[str, Any]:
        ...
        return {
            ...
            'trends': {
                'cpu': {
                    'trend': cpu_trend,
                    'change': cpu_values[-1] - cpu_values[0] if len(cpu_values) > 1 else 0
                },
                'memory': {
                    'trend': memory_trend,
                    'change': memory_values[-1] - memory_values[0] if len(memory_values) > 1 else 0
                },
                'response_time': {
                    'trend': response_trend,
                    'change': response_times[-1] - response_times[0] if len(response_times) > 1 else 0
                }
            }
        }
    ...
    def get_performance_recommendations(self, metrics_summary: Dict[str, Any]) -> List[str]:
        """获取性能优化建议"""
        recommendations = []
        ...
        # 响应时间建议
        avg_response = metrics_summary.get('performance', {}).get('avg_response_time', 0)
        if avg_response > 1.0:
