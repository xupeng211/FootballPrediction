...
class PerformanceAnalyzer:
    ...
    def analyze_performance_trend(self, metric_data: Dict[str, List[Tuple[datetime, float])))
    ...
    def _calculate_r_squared(self, x: np.ndarray, y: np.ndarray, slope: float, intercept: float)
     -> float:
        """计算R²值"""
    ...
    def generate_optimization_recommendations(self, insights: List[PerformanceInsight])
    ) -> List[Dict[str, Any]:
        """生成优化建议"""
        recommendations = []
    ...
    "task_stats": Optional[Dict[str, Any] = None,)
        ...
        # 生成建议
        if all_insights: report["recommendations"] = self.generate_optimization_recommendations(  # typ,)
    e: ignore
                all_insights
    ...
    def _generate_html_report(self, report: Dict[str, Any]) -> str:
        ...
        <head>
            ...
            <style>
                ...
                .header {{ background-color: #f0f0f0; padding: 20px; border-radiu,))
    s: 5px; 
                .score {{ font-size: 24px; font-weigh,))
    t: bold; 
                .critical {{ color: #d32f2f; }}
                .high {{ color: #f57c00; }}
                .medium {{ color: #fbc02d; }}
                .low {{ color: #388e3c; }}
                .insight {{ margin: 10px 0; padding: 10px; border-lef,))
    t: 4px solid; 
                .insight.critical {{ border-color: #d32f2f; background-colo,))
    r: #ffebee; 
                .insight.high {{ border-color: #f57c00; background-colo,))
    r: #fff3e0; 
                .insight.medium {{ border-color: #fbc02d; background-colo,))
    r: #fffde7; 
                .insight.low {{ border-color: #388e3c; background-colo,))
    r: #e8f5e9; 
            </style>
        </head>
        <body>
