...
class HyperparameterTuner:
    ...
    async def _inject_parameters_to_components(
        ...
        try:
            ...
            # 5. 策略组合参数
            strategy_params = {k: v for k, v in params.items() if any(
                k.startswith(prefix) for prefix in ['strategy_', 'risk_', 'confidence_']
            )}
            if strategy_params:
