...
"""
    ...
    def _generate_visualizations(self, df: pd.DataFrame, metrics: Dict):
        ...
        fig.suptitle(f'V19.4 每周实战对账单 ({metrics["week_start"]} ~ {metrics["week_end"]})', fontsize=16, fontweight='bold')
        ...
        ax1.fill_between(range(len(valid_df)), valid_df['cumulative_profit'], 0,
                        where=(valid_df['cumulative_profit'] >= 0), alpha=0.3, color='green')
        ax1.fill_between(range(len(valid_df)), valid_df['cumulative_profit'], 0,
                        where=(valid_df['cumulative_profit'] < 0), alpha=0.3, color='red')
        ax1.set_xlabel('比赛序号')
        ...
        ax2.bar(prediction_counts.index, prediction_counts.values, color=[colors.get(x, 'blue') for x in prediction_counts.index])
        ax2.set_xlabel('预测结果')
        ax2.set_ylabel('场次')
        ...
        if 'confusion_matrix' in metrics:
            sns.heatmap(metrics['confusion_matrix'], annot=True, fmt='d', cmap='Blues', ax=ax3, cbar_kws={'label': '场次'})
    ...
    def compare_with_backtest(self, actual_metrics: Dict, backtest_path: str) -> Dict:
        ...
        comparison['is_significantly_different'] = (
            abs(comparison['accuracy_diff']) > 0.10 or  # 准确率偏离超过10%
            abs(comparison['yield_diff']) > 0.20  # Yield偏离超过20%
        )
