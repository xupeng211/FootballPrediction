...
from src.ml.fault_tolerance import (
    CircuitBreaker,
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/backfill_v20.5.log'),
...
class BackfillOrchestrator:
    ...
    def __init__(self):
        ...
        logger.info(f"熔断器阈值: 20 次连续失败")
        logger.info(f"断点文件: data/backfill_progress_v20.5.json")
        ...
        try:
            ...
            cur.execute("""
                ...
                ON CONFLICT (match_id) DO UPDATE SET
                    ...
                    updated_at = NOW()
            """, (match_id, match['league_id'], match['season_id'], match['home_team'], match['away_team'], json.dumps(final_features), datetime.now()))
    ...
    def run_backfill(self):
            ...
            for season in self.config.SEASONS:
                ...
                for match in matches:
                    ...
                    if processed % self.config.BATCH_SIZE == 0:
                        ...
                        logger.info(f"")
                        logger.info(f"批次 #{processed // self.config.BATCH_SIZE}: 已处理 {processed}/{total_matches} 场")
                        # V20.5.1 FIX: 区分 [Session] 和 [Global DB]
                        logger.info(f"  [Session] 本次运行: 成功 {session_success}, 跳过 {session_skipped}, 失败 {session_failed}")
                        logger.info(f"  [Global DB] 累积总计: 成功 {progress['successful']}, 跳过 {progress['skipped']}, 失败 {progress['failed']}")
                        logger.info(f"  会话速率: {session_rate:.2f} 场/秒 | 熔断状态: {self.circuit_breaker.state.value}, 失败计数: {self.circuit_breaker.failure_count}")
        ...
        # V20.5.1 FIX: 区分 [Session] 和 [Global DB]
        logger.info(f"")
        logger.info(f"[Session] 本次运行统计:")
        logger.info(f"  已处理: {session_processed} 场")
        logger.info(f"  成功: {session_success}, 跳过: {session_skipped}, 失败: {session_failed}")
        logger.info(f"  会话成功率: {session_success / session_processed * 100:.1f}%" if session_processed > 0 else "  会话成功率: N/A")
        logger.info(f"")
        logger.info(f"[Global DB] 累积统计:")
        logger.info(f"  Checkpoint: {progress['processed']}/{progress['total_matches']} 场")
        logger.info(f"  累积成功: {progress['successful']}, 累积跳过: {progress['skipped']}, 累积失败: {progress['failed']}")
