...
class V19_4ProductionPredictor:
    ...
    def generate_daily_picks(self, df: pd.DataFrame, min_confidence: float = 0.60, min_roi: float = 5.0) -> str:
        ...
        # 筛选高价值比赛
        filtered = df[
            (df["confidence"] >= min_confidence) & (df["estimated_roi"] >= min_roi) & (~df["price_deprecated"])
        ]
...
if __name__ == "__main__":
    ...
    predictor.run(limit=args.limit, min_confidence=args.min_confidence, min_roi=args.min_roi)
