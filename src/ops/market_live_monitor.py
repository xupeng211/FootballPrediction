...
# 导入现有模块
from src.ops.risk_monitor import RiskMonitor, RiskLevel
...
class MarketLiveMonitor:
    ...
    def __init__(
        ...
        # 数据存储
        self.entity_status: Optional[EntityStatus] = None
        self.reference_price: Optional[ReferencePrice] = None
        self.model_prediction: Optional[ModelPrediction] = None
        self.delta_metrics: Optional[DeltaMetrics] = None

        # 信号状态
        self.current_signal: Optional[SignalExecution] = None
    ...
    async def _perform_monitoring_cycle(self) -> None:
        ...
        if self.reference_price:
            ...
            logger.info(f"   主胜: {self.reference_price.home_odds:.2f} (隐含: {self.reference_price.home_implied_prob:.2%})")
            logger.info(f"   平局: {self.reference_price.draw_odds:.2f} (隐含: {self.reference_price.draw_implied_prob:.2%})")
            logger.info(f"   客胜: {self.reference_price.away_odds:.2f} (隐含: {self.reference_price.away_implied_prob:.2%})")
        ...
        if self.model_prediction:
            ...
            logger.info(f"   预测: {self.model_prediction.predicted_label} (置信度: {self.model_prediction.confidence:.2%})")
    ...
    async def _fetch_entity_status(self) -> Optional[EntityStatus]:
        """
        获取实体状态数据
    ...
    async def _fetch_reference_price(self) -> Optional[ReferencePrice]:
        """
        获取市场参考价格
    ...
    async def _fetch_model_prediction(self) -> Optional[ModelPrediction]:
        """
        获取模型预测
    ...
    async def _update_signal(self) -> None:
        ...
        if risk_level == RiskLevel.EMERGENCY:
            logger.warning(f"⚠️ 风控等级为紧急，不生成信号")
            return
    ...
    def _save_monitoring_snapshot(self) -> None:
        ...
        snapshot_file = self.data_dir / f"snapshot_{self.target_match_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
