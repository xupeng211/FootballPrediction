...
:
    ...
    async def keys(self) -> list[str]:
        """异步获取所有键"" return self._cache.keys()

    async def values(self) -> list[Any]:
        """异步获取所有值"" return self._cache.values()

    async def items(self) -> list[tuple]:
        """异步获取所有键值对"" return self._cache.items()
    ...
    async def size(self) -> int:
        """异步获取缓存大小"" return self._cache.size()

    async def is_empty(self) -> bool:
        """异步检查缓存是否为空"" return self._cache.is_empty()
    ...
    async def get_stats(self) -> dict[str, Any]:
        """异步获取统计信息"" return self._cache.get_stats()

    async def reset_stats(self) -> None:
        """异步重置统计信息"" self._cache.reset_stats()

    def start_auto_cleanup(self)
:
        """启动自动清理"" self._cache.start_auto_cleanup()

    def stop_auto_cleanup(self)
:
        """停止自动清理"" self._cache.stop_auto_cleanup()

    def __len__(self) -> int:
        return len(self._cache)

    async def __acontains__(self, key: str) -> bool:
        """异步in操作符"" return await self.get(key) is not None
