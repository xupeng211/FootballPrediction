...
"" from .async_cache import AsyncTTLCachefrom .ttl_cache import TTLCache



class CacheFactory:
    """缓存工厂类 提供创建不同类型缓存的静态方法.
    Provides static methods for creating different types of caches.
    "" @staticmethoddef create_cache()
    ...
    ValueError: 当cache_type不支持时
        "" if cache_type == "async return AsyncTTLCache(max_size, default_ttl, cleanup_interval)
        elif cache_type == "sync return TTLCache(max_size, default_ttl, cleanup_interval)
        elseraise ValueError(f"不支持的缓存类型: {cache_type}")


    @staticmethoddef create_lru_cache(max_size: int = 128) -> TTLCache:
    ...
    @staticmethoddef create_ttl_cache()
    ...
    @staticmethoddef create_async_lru_cache(max_size: int = 128) -> AsyncTTLCache:
    ...
    @staticmethoddef create_async_ttl_cache()
    ...
    @staticmethoddef create_prediction_cache() -> TTLCache:
    ...
    @staticmethoddef create_feature_cache() -> TTLCache:
    ...
    @staticmethoddef create_odds_cache() -> TTLCache:
    ...
    @staticmethoddef create_session_cache() -> TTLCache:
    ...
    @staticmethoddef create_config_cache() -> TTLCache:
    ...
    @staticmethoddef get_available_types() -> list:
    ...
    @staticmethoddef get_preset_configs() -> dict[str, Any]:
