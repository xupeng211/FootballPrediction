    ...
    def _evict_entries(self, count: int = 1):
        ...
        while evicted < count and self.cache:
            if self.config.eviction_policy == CacheEvictionPolicy.LRU:
                key = self.access_order[0] if self.access_order else next(iter(self.cache))
...
class DistributedCacheManager:
    ...
    async def get(self, key: str, level: Optional[CacheLevel] = None,
        ...
        try:
            ...
            for cache_level in [CacheLevel.L1_MEMORY, CacheLevel.L2_REDIS]:
                ...
                if value is not None:
                    ...
                    await self.performance_monitor.record_hit(cache_level, response_time)
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None,
        ...
        try:
            ...
            if level:
                ...
                await self.performance_monitor.record_write(level, time.time() - start_time)
                return result
            ...
            if self.enable_write_through:
                ...
                for cache_level in [CacheLevel.L1_MEMORY, CacheLevel.L2_REDIS]:
                    result = await self._set_to_level(key, value, ttl, cache_level, session_id)
                    success = success and result

            elif self.enable_write_back:
                # Write-back: 只写入L1，异步写入其他级别
                await self._set_to_level(key, value, ttl, CacheLevel.L1_MEMORY, session_id)
            ...
            await self.performance_monitor.record_write(level or CacheLevel.L1_MEMORY, time.time() - start_time)
            return success
    ...
    async def _get_from_level(self, key: str, level: CacheLevel,
                             session_id: Optional[str] = None) -> Optional[Any]:
        """从特定缓存级别获取值"""
    ...
    async def _set_to_level(self, key: str, value: Any, ttl: Optional[int],
                           level: CacheLevel, session_id: Optional[str] = None) -> bool:
        """向特定缓存级别设置值"""
        if level == CacheLevel.L1_MEMORY:
    ...
    async def _populate_higher_levels(self, key: str, value: Any, source_level: CacheLevel):
        """回填到更高级别的缓存"""
        # 如果是从L2获取的，回填到L1
        if source_level == CacheLevel.L2_REDIS:
            await self._set_to_level(key, value, None, CacheLevel.L1_MEMORY)

    async def _async_write_back(self, key: str, value: Any, ttl: Optional[int],
...
class LoadBalancer:
    ...
    def select_node(self, nodes: List[str], key: Optional[str] = None) -> str:
        """选择节点"""
        if not nodes:
...
class CachePerformanceMonitor:
    ...
    async def record_hit(self, level: CacheLevel, response_time: float):
        ...
        if len(self.metrics['response_times'][level.value]) > 1000:
            self.metrics['response_times'][level.value] = self.metrics['response_times'][level.value][-1000:]
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_response_times': {k: round(v, 4) for k, v in avg_response_times.items()},
...
def get_distributed_cache_manager() -> Optional[DistributedCacheManager]:
    """获取全局分布式缓存管理器实例"""
    global _distributed_cache_manager
    return _distributed_cache_manager


async def initialize_distributed_cache(config: Dict[str, Any]) -> DistributedCacheManager:
    """初始化分布式缓存管理器"""
    global _distributed_cache_manager
