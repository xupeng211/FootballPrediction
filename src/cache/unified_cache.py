...
class UnifiedCacheManager:
    ...
    async def invalidate_pattern(self, pattern: str, cache_type: str = None):
        ...
        try:
            ...
            logger.info(f"Cache invalidation completed: pattern='{pattern}', type='{cache_type}'")
...
def get_cache_manager() -> UnifiedCacheManager:
    ...
    if _cache_manager is None:
        ...
        try:
            ...
            logger.warning(f"Redis client initialization failed: {e}, using local cache only")
            redis_client = None
...
def cached(cache_type: str = 'default', ttl: Optional[int] = None, key_func: Optional[Callable] = None):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            logger.debug(f"Function {func.__name__} executed in {execution_time:.3f}s, cached for {ttl or 'default'}s")
            return result
        return wrapper
    return decorator
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if execution_time > threshold:
                    logger.warning(f"Slow function detected: {func.__name__} took {execution_time:.3f}s (threshold: {threshold}s)")
