...
def expensive_computation(x: int, y: int) -> int:
    """模拟耗时的计算"" logger.info(f"执行计算: {x} + {y}")
    return x + y
...
async def fetch_user_data(user_id: int) -> Dict[str, Any]:
    """模拟API调用获取用户数据"" logger.info(f"获取用户数据: {user_id}")
    await asyncio.sleep(1)  # 模拟网络延迟
    return {"id": user_id, "name": f"User {user_id}", "age": 25}" # 示例3: 基于用户的缓存
...
"" logger.info(f"获取用户档案: {user_id}, 敏感信息: {include_sensitive}")
    ...
    if include_sensitiveprofile["phone"] = f"123-456-{user_i,)

    d:04d} profile["address"] = f"Street {user_id, City return profile
...
:
    """生成缓存失效键的函数"" # 如果是更新操作,失效相关缓存
    if "update" in func.__name__
    user_id = kwargs.get("user_id") or args[0] if args else Noneif user_id

    return [)
                f"user_profile: use,
    r:{user_id}:*",
                f"user_predictions: use,
    r:{user_id}:*",
...
"" logger.info(f"获取比赛详情: {match_id}")
    ...
    return {)
        "match_id": match_id,", "home_team": "Team A",", "away_team": "Team B",", "kickof": "2024-01-01T20: 0,
    0:00Z""",
...
"" logger.info(f"计算球队统计: team_id={team_id}, season={season}")
    return {)
        "team_id": team_id,", "season": season,", "played": 20,", "won": 12,", "drawn": 5,", "lost": 3,", "goals_for": 35,", "goals_against": 18,", "points": 41," 
"

# 示例7: 自定义键生成器
def custom_key_generator(func, args, kwargs, user_id=None)
:
    """自定义缓存键生成器"" func_name = func.__qualname__
    # 使用特定的参数组合生成键
    key_parts = [func_name]

    # 添加特定的参数
    if "team_id" in kwargskey_parts.append(f"team_{kwargs[team_id]}")

    if "season" in kwargskey_parts.append(f"season_{kwargs[season]}")
...
class PredictionService:
    """预测服务类,展示装饰器的组合使用"" @cache_match_data(ttl_seconds=600)
    async def get_match_odds(self, match_id: int) -> Dict[str, float]:
        """获取比赛赔率"" await asyncio.sleep(0.3)
        return {)
            "home_win": 2.10,", "draw": 3.40,", "away_win": 3.20," 
"
    @cache_by_user(ttl=300, user_param="user_id")
    async def calculate_user_prediction(self, user_id: int, match_id: int, model_type: str = "default ) -> Dict[str, Any]:
"        """计算用户预测(基于用户缓存)
"" # 获取比赛数据
        ...
        return {)
            "user_id": user_id,", "match_id": match_id,", "model_type": model_type,", "prediction": "2-1",", "confidence": 0.78,", "used_odds": odds," 

    @cache_invalidate(pattern="predictions: use,)
    r:{user_id}:*"
    async def submit_prediction(self, user_id: int, match_id: int, prediction: str)
    ) -> Dict[str, Any:
        """提交预测(会失效用户相关缓存)
"" logger.info()
...
"
        ...
        return {)
            "success": True,", "prediction_id": f"pred_{match_id}_{user_id}",
"            "invalidated_cache": f"predictions: use,
"    r:{user_id}:*",
"        
