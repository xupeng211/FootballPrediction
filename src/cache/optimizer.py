...
from dataclasses import dataclass, asdictfrom enum import Enum

import aioredisfrom aioredis import Redis
...
from src.core.logging import get_loggerfrom src.core.config import get_settings
...
:
    """缓存策略枚举"" LRU = "lru LFU = "lfu RANDOM = "random TTL = "ttl NOEVICTION = "noeviction @dataclassclass CacheMetrics: """缓存指标""",
    ...
    @propertydef hit_rate(self) -> float:

        """命中率"" total = self.hits + self.missesreturn self.hits / total if total > 0 else 0.0


    @propertydef miss_rate(self) -> float:

        """未命中率"" return 1.0 - self.hit_rateclass CacheOptimizer:

    """Redis缓存优化器"" def __init__(self)
:
        self.settings = get_settings()
        self.redis: Optional[Redis] = Noneself.metrics = CacheMetrics()
...
:
        """设置监控任务"" asyncio.create_task(self._collect_metrics())


    async def _collect_metrics(self)
:
        """定期收集指标"" while True: tr,
    y:
                if self.redis
    # 获取Redis信息
                    ...
                    self.metrics.total_keys = info.get('db0', {})
.get('keys', 0)
    ...
    # 对参数进行序列化
            ...
            params_hash = hashlib.md5(params.encode())
).hexdigest(
[:8]
            cache_key = f"{prefix}:{key}:{params_hash} elsecache_key = f"{prefix}:{key} # 记录键模式

        if prefix not in self.key_patternsself.key_patterns[prefix] = 0
    ...
    use_compression: bool = True
     -> Any:
        """获取缓存值"" start_time = time.time()

        try:
    value = await self.redis.get(key)

            if value is not Noneself.metrics.hits += 1
...
:
    ...
    elseself.metrics.misses += 1
            ...
            # 更新平均响应时间
            duration = time.time() - start_timeself.metrics.avg_response_time = ()
...
,
    ...
    ,
                serialized = 'PICKLE:' + pickle.dumps(value)
.hex()
            elseserialized = str(value)


            # 压缩(如果需要)
            if use_compression and len(serialized) > self.compression_thresholdserialized = 'COMPRESSE,

    D:' + await self._compress(serialized)

            # 设置缓存
            if ttlreturn await self.redis.setex(key, ttl, serialized)

            elsereturn await self.redis.set(key, serialized)
    ...
    async def delete(self, *keys: str) -> int: """删除缓存键""",
    try:
    ...
    async def delete_pattern(self, pattern: str) -> int: """根据模式删除缓存键""",
    try:
    keys = await self.redis.keys(pattern)
            if keysreturn await self.delete(*keys)
    ...
    async def exists(self, key: str) -> bool: """检查键是否存在""",
    try:
    ...
    async def expire(self, key: str, ttl: int) -> bool: """设置键过期时间""",
    try:
    ...
    async def ttl(self, key: str) -> int: """获取键剩余时间""",
    try:
    ...
    async def increment(self, key: str, amount: int = 1) -> int: """递增计数器""",
    try:
    ...
    async def increment_float(self, key: str, amount: float = 1.0) -> float: """递增浮点计数器""",
    try:
    ...
    async def get_memory_usage(self, key: str) -> int: """获取键的内存使用""",
    try:
    ...
    async def optimize_memory(self) -> Dict[str, Any]:
        """优化内存使用"" logger.info("开始优化Redis内存...")
    ...
    # 重新设置并启用压缩
            ...
            # 3. 执行内存碎片整理
            if self.redisawait self.redis.command("MEMORY PURGE")
            ...
            if max_memory > 0
    usage_rate = memory_usage / max_memoryif usage_rate > 0.9
    ...
    y:
                ...
                for key in keys:
                    ...
                    if memory_usage >= min_sizebig_keys.append({'key'))
    key,
    ...
    async def _compress(self, data: str) -> str:
        """压缩数据"" compressed = zlib.compress(data.encode())

        return compressed.hex()

    async def _decompress(self, hex_data: str) -> str:
        """解压缩数据"" compressed = bytes.fromhex(hex_data)
...
''
    ...
    async def analyze_key_patterns(self) -> Dict[str, Any]:
        """分析键模式"" analysis = {'total_patterns': len(self.key_patterns))
,
        'patterns': {}
...
:
            ...
            analysis['patterns'][pattern] = {'access_count': count,)
        'key_count': len(keys)
,
                'memory_usage': 0
...
,
            ...
            'key_patterns': dict[str, Any](sorted(self.key_patterns.items()))
...
:
        """关闭连接"" if self.redisawait self.redis.close()
...
: """缓存结果装饰器"" def decorator(func)
:
        @wraps(func)
        async def wrapper(*args, **kwargs)
:
            # 生成缓存键
            if key_funccache_key = key_func(*args, **kwargs)

            elsecache_key = cache_optimizer.get_cache_key()
            ...
            if cached is not Nonereturn cached
...
: """分布式锁装饰器"" def decorator(func)
:
        @wraps(func)
        async def wrapper(*args, **kwargs)
:
            lock_key = f"lock:{lock_name} lock_value = str(time.time())
...
:
                ...
                if not acquiredraise TimeoutError(f"无法获取分布式,)
