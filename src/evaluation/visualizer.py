...
class EvaluationVisualizer:
    ...
    def plot_roc_curves(
        ...
        for i, (class_name, color) in enumerate(
            zip(self.class_names, self.class_colors, strict=False)
        ):
    ...
    def plot_calibration_curves(
        ...
        n_bins: int = 10,
        title: str = "概率校准曲线",
        save_name: str = "calibration_curves",
    ) -> list[str]:
        """
        绘制校准曲线
    ...
    def plot_prediction_distribution(
        ...
        y_proba: np.ndarray,
        title: str = "预测分布分析",
        save_name: str = "prediction_distribution",
    ) -> list[str]:
        """
        绘制预测分布图
        ...
        if accuracy_by_conf:
            ...
            ax_acc.plot(
                [0.5, 1.0], [0.5, 1.0], "k--", linewidth=1, alpha=0.7, label="完美校准"
            )
    ...
    def plot_backtest_results(
        ...
        backtest_result,
        title: str = "回测结果分析",
        save_name: str = "backtest_results",
    ) -> list[str]:
        """
        绘制回测结果图表
        ...
        ax_equity.axhline(
            y=backtest_result.initial_bankroll,
            color="red",
            linestyle="--",
            linewidth=1,
            alpha=0.7,
        ...
        ax_equity.set_title(
            f"资金曲线 (ROI: {backtest_result.roi:.2f}%)",
            fontsize=14,
            fontweight="bold",
        )
        ax_equity.legend()
        ...
        ax_cumulative.plot(
            ...
            color=(
                self.colors["success"]
                if backtest_result.net_profit > 0
        ...
        ax_cumulative.set_title(
            f"累计收益曲线 (总收益: {backtest_result.net_profit:.2f})",
            fontsize=14,
            fontweight="bold",
        )
        ...
        ax_profits.hist(
            ...
            label=f"盈利 ({len(positive_profits)}笔)",
            edgecolor="black",
        )
        ax_profits.hist(
            negative_profits,
            ...
            label=f"亏损 ({len(negative_profits)}笔)",
            edgecolor="black",
        )
        ...
        ax_profits.set_title(
            f"收益分布 (平均: {backtest_result.avg_profit_per_bet:.2f})",
            fontsize=14,
            fontweight="bold",
        )
        ax_profits.legend()
        ...
        bars = ax_bet_types.bar(
            bet_type_counts.index, bet_type_counts.values, color=colors
        )
        ...
        for _i, class_name in enumerate(["H", "D", "A"]):
            class_bets = [
                bet for bet in backtest_result.bets if bet.bet_type.value == class_name
            ]
            if class_bets:
                class_win_rate = sum(1 for bet in class_bets if bet.won) / len(
                    class_bets
                )
        ...
        risk_metrics = {
            ...
            "盈利因子": (
                ...
                if backtest_result.profit_factor != float("inf")
                else 10
            ),
        }
    ...
    def plot_metrics_summary(
        ...
        # 添加数值标签
        for i, (_cat, val) in enumerate(
            zip(categories[:-1], values[:-1], strict=False)
        ):
    ...
    def create_comprehensive_report(
        ...
        # 1. ROC曲线
        try:
            roc_files = self.plot_roc_curves(
                y_true,
                y_proba,
        ...
        # 2. 校准曲线
        try:
            calibration_files = self.plot_calibration_curves(
                y_true,
                y_proba,
        ...
        # 3. 预测分布
        try:
            distribution_files = self.plot_prediction_distribution(
                y_true,
                y_pred,
                y_proba,
        ...
        if backtest_result:
            try:
                backtest_files = self.plot_backtest_results(
                    backtest_result,
        ...
        if metrics_result:
            try:
                metrics_files = self.plot_metrics_summary(
                    metrics_result,
        ...
        logger.info(
            f"Comprehensive report created with {len(report_files)} chart types"
        )
        return report_files
