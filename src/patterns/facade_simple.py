...
"    e: {report_type}"
"
    async def _get_overview_stats(self, days: int, user_id: Optional[int])
    ) -> Dict[str, Any:
        """获取概览统计"""
        return {"total_predictions": 100, "accuracy": 65.0, "active_users": 25}

    async def _get_prediction_analytics(self, days: int, user_id: Optional[int])
    ) -> Dict[str, Any:
        """获取预测分析"""
        return {)
    ...
    async def _get_performance_metrics(self, days: int, user_id: Optional[int])
    ) -> Dict[str, Any:
        """获取性能指标"""
        return {)
            "roi": 15.5,","
...
class SystemFacade:
    ...
    async def get_dashboard_summary(self, user_id: Optional[int] = None)
    ) -> Dict[str, Any:
        """获取仪表板摘要"""
