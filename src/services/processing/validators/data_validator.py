...
# from src.data.processing.missing_data_handler import MissingDataHandler  # type: ignoreclass DataValidator:

    """数据验证器"" def __init__(self) -> None:
        """初始化验证器"" self.logger = logging.getLogger(f"processing.{self.__class__.__name__}")
        # self.missing_handler = MissingDataHandler()  # type: ignore

        # 验证规则配置
        self.validation_rules = {)
            "match_data": {", "required_fields": [" "match_id"," "home_team"," "away_team"," "match_date"," ],)
                "date_fields": ["match_date",", "numeric_fields": ["home_score", "away_score"],", "string_fields": ["home_team", "away_team", "venue", "competition"]," },
"            "odds_data": {", "required_fields": [" "match_id", "bookmaker", "home_win", "draw", "away_win", ],)
                "numeric_fields": [" "home_win", "draw", "away_win", "bookmaker_margin", ],
"                "float_fields": ["home_win", "draw", "away_win"]," ,
"            "features_data": {", "numeric_range": {", "form": (0, 1)))
,", "win_rate": (0, 1)
",", "goals_avg": (0, 10)
"," ,
"            ,
    ...
    "data_type": 数据类型
        ...
        Returns:
            验证结果报告
        """" try:
"    # 转换为DataFrameif isinstance(data, Dict[str, Any])
    ...
    df = pd.DataFrame(data)
            ...
            if df.emptyreturn {)
                    ...
                    "valid": False,", "errors": ["数据为空"],", "warnings": []",
"                    "statistics": {}," 
"
            # 执行各类验证
            validation_results = {)
...
"
            ...
            validation_results["errors"].extend(content_result["errors"])  # type: ignorevalidation_results["warnings"].extend(content_result["warnings"])  # type: ignore


            # 3. 业务规则验证
            business_result = await self._validate_business_rules(df, data_type)
            validation_results["errors"].extend(business_result["errors"])  # type: ignorevalidation_results["warnings"].extend(business_result["warnings"])  # type: ignore
    ...
    validation_results["valid"] = False
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: self.logger.error(f"数据验证失,)
"    败: {e}", exc_info=True
"            return {)
                "valid": False,", "errors": [f"验证过程出错: {str(e)))
",
"                "warnings": []",
"                "statistics": {}," 
"
    async def _validate_structure(self, df: pd.DataFrame, data_type: str)
    ) -> Dict[str, Any:
        """" 验证数据结构
...
"
        ...
        if missing_fieldsresult["errors"].append(f"缺少必需字,)
"
    段: {missing_fields}"
"
        # 检查字段类型
        date_fields = rules.get("date_fields", [])
        for field in date_fields: if field in df.column,
    s:
...
:
        ...
        for field in numeric_fields: if field in df.column,
    s:
...
:
    ...
    async def _validate_content(self, df: pd.DataFrame, data_type: str)
    ) -> Dict[str, Any:
        """" 验证数据内容
...
"
        ...
        if duplicates.any()
    result["warnings"].append(f"发现 {duplicates.sum()))
" 条重复记录"
"
        # 检查缺失值
        # missing_report = self.missing_handler.analyze_missing_data(df)  # type: ignoremissing_report = {"missing_percentage": 0},
...
.columnsfor col in numeric_cols:
    ...
    # 使用IQR方法检测异常值
                ...
                if not outliers.emptyoutlier_percentage = len(outliers) / len(df) * 100

                    if outlier_percentage > 10
    result["warnings"].append()
                            f"字段 {col} 异常值比例过高: {outlier_percentag,)
"    e:.1% 

        return result

    async def _validate_business_rules(self, df: pd.DataFrame, data_type: str)
    ) -> Dict[str, Any:
        """" 验证业务规则
    ...
    "data_type": 数据类型
        ...
        """" "result": Dict[str, Any] = {"errors": [], "warnings": []}
"
        if data_type == "match_data result = await self._validate_match_business_rules(df)
"        elif data_type == "odds_data result = await self._validate_odds_business_rules(df)
"        elif data_type == "features_data result = await self._validate_features_business_rules(df)
"
        return result

    async def _validate_match_business_rules(self, df: pd.DataFrame) -> Dict[str, Any]:
        """验证比赛数据业务规则"" "result": Dict[str, Any] = {"errors": [], "warnings": []}

        # 检查同一日期的重复比赛
        if ()
            "match_date" in df.columns and "home_team" in df.columnsand "away_team" in df.columns

        
:
            date_team_pairs = df.groupby("match_date")
.apply()
                lambda x: x.duplicated(subset=["home_team", "away_team"])
.any()
...
"
        ...
        # 检查日期范围
        if "match_date" in df.columnsdf["match_date"] = pd.to_datetime(df["match_date"])
    ...
    async def _validate_odds_business_rules(self, df: pd.DataFrame) -> Dict[str, Any]:
        """验证赔率数据业务规则"" "result": Dict[str, Any] = {"errors": [], "warnings": []}
    ...
    # 检查赔率小于等于1
                ...
                if invalid_odds.any()
    result["errors"].append(f"{outcome" 赔率值不能小于等于1") # 检查过高的赔率)
...
"
    ...
    async def _validate_features_business_rules(self, df: pd.DataFrame)
    ) -> Dict[str, Any:
        """验证特征数据业务规则"" "result": Dict[str, Any] = {"errors": [], "warnings": []}
...
," 
        ...
        # 数值列统计
        numeric_cols = df.select_dtypes(include=["number"])
.columnsif not numeric_cols.empty
    ...
    date_stats = {}
            for col in date_cols:
                date_stats[col] = {)
                    "min": df[col].min()
,", "max": df[col].max()
,", "range_days": (df[col].max() - df[col].min())
.days," 
            stats["date_statistics"] = date_stats

        return stats

    async def _log_validation_results(self, results: Dict[str, Any], data_type: str)
     -> None:
        """记录验证结果"" if results["valid"]
    self.logger.info(f"{data_type" 数据验证通过") elseself.logger.error())
"
...
"
    ...
    async def validate_batch_consistency(self, batches: List[pd.DataFrame])
    ) -> Dict[str, Any:
        """" 验证批次数据的一致性
...
"            "issues": []",
"            "statistics": {", "total_batches": len(batches))
",", "total_records": sum(len(batch) for batch in batches)
"," ,
"        

        if not batchesresult["consistent"] = False
...
:
            ...
            if batch_cols != first_batch_colsresult["consistent"] = False
        ...
        for col in first_batch_cols:
            col_types = []
            for batch in batches: if col in batch.column,
    s:
                    col_types.append(str(batch[col].dtype))


            if len(set(col_types))
 > 1
