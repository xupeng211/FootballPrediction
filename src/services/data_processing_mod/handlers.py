...
class MissingDataHandler:
    ...
    def _drop_missing(self, data: pd.DataFrame, analysis: Dict[str, Any]) -> pd.DataFrame:
    ...
    def _fill_missing(self, data: pd.DataFrame, analysis: Dict[str, Any]) -> pd.DataFrame:
        """填充缺失数据 / Fill Missing Data"""
        # 数值列用中位数填充
        numeric_columns = data.select_dtypes(include=[np.number]).columns
        for col in numeric_columns:
            if col in analysis["missing_by_column"] and analysis["missing_by_column"][col] > 0:
    ...
    def _interpolate_missing(self, data: pd.DataFrame, analysis: Dict[str, Any]) -> pd.DataFrame:
...
class MissingScoresHandler:
    ...
    async def handle_missing_scores(
    ...
    def _infer_scores_from_other_data(
        ...
        if "home_ht_score" in data.columns and "away_ht_score" in data.columns:
            # 使用半场比分作为全场比分的参考
            ht_complete = data.loc[missing_mask, "home_ht_score"].notna() & \
                         data.loc[missing_mask, "away_ht_score"].notna()

            if ht_complete.sum() > 0:
                # 简单策略：半场比分可能接近最终比分
                data.loc[missing_mask & ht_complete, "home_score"] = \
                    data.loc[missing_mask & ht_complete, "home_ht_score"]
                data.loc[missing_mask & ht_complete, "away_score"] = \
                    data.loc[missing_mask & ht_complete, "away_ht_score"]
...
class MissingTeamDataHandler:
    ...
    def _fill_missing_team_data(self, data: pd.DataFrame, team_type: str) -> pd.DataFrame:
        ...
        if team_name_col in data.columns:
            ...
            if missing_mask.sum() > 0:
                # 创建队名到ID的映射
                team_mapping = {}
                complete_data = data[data[team_id_col].notna() & data[team_name_col].notna()]
