    ...
    def _create_derived_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "home_team_strength" in df.columns and "away_team_strength" in df.columns:
            df["strength_difference"] = (
                df["home_team_strength"] - df["away_team_strength"]
            )

        # 示例：创建历史交锋胜率
        if (
            "head_to_head_home_wins" in df.columns
            and "head_to_head_total" in df.columns
        ):
            df["h2h_home_win_rate"] = (
                df["head_to_head_home_wins"] / df["head_to_head_total"]
            )
