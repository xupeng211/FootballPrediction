...
class FotMobCollectionService(BaseService):
    ...
    async def _save_match_data(
        self, data: Dict[str, Any], task: FotMobCollectionTask, *, transaction_ctx: TransactionContext
    ) -> None:
        """保存比赛数据到数据库 (事务保护版本)"""
        try:
            # 记录操作开始
            ...
            # 使用事务上下文中的连接执行SQL
            await transaction_ctx.connection.execute(
                sql,
                task.match_id,
                data.get("home_team"),
                data.get("away_team"),
            ...
            if self.settings.application.debug:
                summary = transaction_ctx.get_summary()
            ...
            self.logger.error(f"❌ 比赛数据保存失败 (事务 {transaction_ctx.transaction_id}): {task.match_id}, 错误: {e}")
            raise
        ...
        try:
            ...
            for match_info in matches_data:
                try:
                    ...
                    transaction_ctx.add_operation(
                        operation_type="SUCCESS",
                        description=f"批量保存成功: {task.match_id}",
                        match_id=task.match_id
                    )
                    ...
                    # 在批量操作中，单个失败不应该回滚整个事务
                    # 但记录错误以便后续处理
                    self.logger.warning(f"⚠️  批量保存中单个失败: {error_msg}")
            ...
            transaction_ctx.add_operation(
                ...
                description=f"批量保存完成: 成功 {successful_count}, 失败 {failed_count}",
            ...
            self.logger.info(
                ...
                f"{successful_count}/{len(matches_data)} 成功, 耗时: {duration:.2f}s"
            )
    ...
    async def fetch_fotmob_data_batch(
        ...
        async def fetch_with_semaphore(match_id: str) -> tuple[str, Optional[Dict[str, Any]]]:
            async with semaphore:
                try:
    ...
    def _parse_kickoff_time(self, time_data: Dict[str, Any]) -> Optional[datetime]:
        """解析比赛开始时间"""
        try:
            # FotMob时间格式可能是多种情况
    ...
    def _extract_initial_odds(self, match_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for source_key, default_value in odds_sources:
