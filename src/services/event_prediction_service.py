...
.strategy_prediction_service import StrategyPredictionService
..domain.models import Match, Prediction
..events import ()
    PredictionMadeEvent,
...
..events.types import MatchCreatedEventData, UserRegisteredEventData
...
:
    """事件驱动的预测服务"" 继承自策略预测服务,添加事件发布功能.
    Inherits from strategy prediction service, adding event publishing capabilities.
    """" def __init__(self, *args, **kwargs) -> None:
"        """初始化事件驱动预测服务"" super()
.__init__(*args, **kwargs)
        self._event_bus = get_event_bus()
        self._event_source = "prediction_service async def predict_match(self,)
    "match_id": int,
    "user_id": int,
    "strategy_name": Optional[str] = None,
    "confidence": Optional[float] = None,
    "notes": Optional[str] = None,
     -> Prediction: """预测单场比赛并发布事件""",
    Args:
    "match_id": 比赛ID
    "user_id": 用户ID
...
.predict_match()
    ...
    "Prediction": 更新后的预测
        ...
        if not original_predictionraise ValueError(f"预测不存,)

    在: {prediction_id}"

        # 保存原有数据用于事件
        previous_prediction = {)
            "predicted_home": original_prediction.predicted_home,", "predicted_away": original_prediction.predicted_away,", "confidence": original_prediction.confidence,", "notes": original_prediction.notes," 

        # 更新预测
        if new_predicted_home is not Noneoriginal_prediction.predicted_home = new_predicted_home

        if new_predicted_away is not Noneoriginal_prediction.predicted_away = new_predicted_away

        if new_confidence is not Noneoriginal_prediction.confidence = new_confidence

        if new_notes is not Noneoriginal_prediction.notes = new_notes
    ...
    "strategy_name": 使用的策略名称
        ...
        """" # 执行批量预测
        predictions = await super()
.batch_predict(match_ids, user_id, strategy_name)
    ...
    async def _publish_prediction_made_event(self, prediction: Prediction, strategy_name: Optional[str])
     -> None: """发布预测创建事件""",
    Args:
    "prediction": 创建的预测
    "strategy_name": 使用的策略名称
...
"    # 创建事件数据
            eventdata= PredictionMadeEventData()
                ...
                if prediction.predicted_home_scoreelse None,

                predicted_away=float(prediction.predicted_away_score)
                if prediction.predicted_away_scoreelse None,

                confidence=float(prediction.confidence_score)
                if prediction.confidence_scoreelse None,
...
,", "notes": prediction.notes," ,
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.error(f"发布预测创建事件失,)
"    败: {e}"
"
    async def _publish_prediction_updated_event(self,)
...
,", "notes": prediction.notes," ,
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.error(f"发布预测更新事件失,)
    败: {e}"


# 事件驱动的比赛服务
class EventDrivenMatchService:
    """事件驱动的比赛服务"" 处理比赛相关的事件发布.
    Handles match-related event publishing.
    """" def __init__(self, match_repository) -> None: """初始化比赛服务""",
    Args:
    "match_repository": 比赛仓储
        """" self._match_repository = match_repositoryself._event_bus = get_event_bus()
"
        self._event_source = "match_service async def create_match(self,)
"    "home_team_id": int,
    "away_team_id": int,
    "league_id": int,
    "match_time": datetime,
    ...
    async def _publish_match_created_event(self,)
    "match": Match,
    ...
    try:
    # 创建事件数据
            eventdata= MatchCreatedEventData()
                match_id=match.id,
...
," ,
            ...
            # 创建事件
            ..events.types import MatchCreatedEvent
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.error(f"发布比赛创建事件失,)
    败: {e}"


# 事件驱动的用户服务
class EventDrivenUserService:
    """事件驱动的用户服务"" 处理用户相关的事件发布.
    Handles user-related event publishing.
    """" def __init__(self, user_repository) -> None: """初始化用户服务""",
    Args:
    "user_repository": 用户仓储
        """" self._user_repository = user_repositoryself._event_bus = get_event_bus()
"
        self._event_source = "user_service async def register_user(self,)
"    "username": str,
    "email": str,
    "password_hash": str,
    ...
    "user_agent": 用户代理
        ...
        """" # 创建用户(简化实现)
        _user = {)
            "id": 1,  # 实际应该由数据库生成", "username": username,", "email": email,", "password_hash": password_hash,", "registration_date": datetime.utcnow()
," 
    ...
    async def _publish_user_registered_event(self,)
    "user": Dict[str, Any],
    ...
    # 创建事件数据
            ...
            # 创建事件
            ..events.types import UserRegisteredEvent
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.error(f"发布用户注册事件失,)
    败: {e}"
