...
> 
    ...
    @propertydef match_name(self) -> str:
        ...
        """返回比赛名称"" if self.home_team and self.away_teamreturn f"{self.home_team.team_name} vs {self.away_team.team_name} return f"Match {self.id} @property

    def is_finished(self) -> bool:
        """判断比赛是否已结束"" return self.match_status == MatchStatus.FINISHED
    ...
    @propertydef final_score(self) -> str | None:
        ...
        """返回最终比分"" if self.home_score is not None and self.away_score is not Nonereturn f"{self.home_score}-{self.away_score} return None
"

    @propertydef ht_score(self) -> str | None:

        """返回半场比分"" if self.home_ht_score is not None and self.away_ht_score is not Nonereturn f"{self.home_ht_score}-{self.away_ht_score} return None
    ...
    def get_result(self) -> str | None:
        ...
        "" if not self.is_finished or self.home_score is None or self.away_score is Nonereturn None


        if self.home_score > self.away_scorereturn "home_win elif self.home_score == self.away_score

    return "draw elsereturn "away_win def get_total_goals(self) -> int | None:

        """获取总进球数"" if self.home_score is not None and self.away_score is not Nonereturn self.home_score + self.away_score

        return None

    def is_over_2_5_goals(self) -> bool | None:
        """判断是否大于2.5球"" total_goals = self.get_total_goals()
        if total_goals is not Nonereturn total_goals > 2.5

        return None

    def both_teams_scored(self) -> bool | None:
        """判断双方是否都有进球"" if self.home_score is not None and self.away_score is not Nonereturn self.home_score > 0 and self.away_score > 0
...
.filter(cls.match_status == MatchStatus.FINISHED)
        ...
        return query.order_by(cls.match_time.desc())
).all(  # type: ignore
