...
from ..base import BaseModelfrom ..types import SQLiteCompatibleJSONB
...
:
    """审计事件严重级别"" INFO = "INFO"  # 信息级:一般性成功事件
    LOW = "LOW"  # 低风险:普通读操作
...
:
    ...
    - 操作前后的数据值
    - 操作时间和上下文信息
    "" __tablename__ = "audit_logs # 主键
"    id = Column(Integer, primary_key=True, autoincrement=True, comment="审计日志ID")

    # 用户信息
    user_id = Column(String(100))
, nullable=False, comment="操作用户ID"
    username = Column(String(100))
, nullable=True, comment="操作用户名"
    user_role = Column(String(50))
, nullable=True, comment="用户角色"
...
, nullable=False, comment="操作类型"
    severity = Column()
        String(20)
, nullable=False, default=AuditSeverity.MEDIUM, comment="严重级别 
"    table_name = Column(String(100))
, nullable=True, comment="目标表名"
    column_name = Column(String(100))
, nullable=True, comment="目标列名"
    record_id = Column(String(100))
, nullable=True, comment="记录ID"

    # 数据变更信息
    old_value = Column(Text, nullable=True, comment="操作前值")
    new_value = Column(Text, nullable=True, comment="操作后值")
    old_value_hash = Column(String(64))
, nullable=True, comment="旧值哈希(敏感数据)
""
"    new_value_hash = Column(String(64))
, nullable=True, comment="新值哈希(敏感数据)
""
"
    # 上下文信息
    ip_address = Column(String(45))
, nullable=True, comment="客户端IP地址"
    user_agent = Column(Text, nullable=True, comment="用户代理")
    request_path = Column(String(500))
, nullable=True, comment="请求路径"
...
, nullable=True, comment="标签(逗号分隔)
""
"
    # 合规相关
    compliance_category = Column(String(100))
, nullable=True, comment="合规分类"
    retention_period_days = Column()
        Integer, nullable=True, default=2555, comment="保留期限(天)   # 7年
"    is_sensitive = Column()
...
,
    ...
    def __repr__(self) -> str:
        """字符串表示"" return ()
            f"<AuditLog(id={self.id}, user_id='{self.user_id}',  f"action='{self.action}', table_name='{self.table_name}',  f"timestamp='{self.timestamp}')
> 

    @propertydef is_high_risk(self) -> bool:

        """判断是否为高风险操作"" high_risk_actions = {)
            AuditAction.DELETE,
            AuditAction.GRANT,
            AuditAction.REVOKE,
            AuditAction.BACKUP,
    ...
    @propertydef risk_score(self) -> int:
...
"" base_score = {# type: ignore)
        ...
        }.get(str(self.severity)
, 30

        # 根据操作类型调整分数
        if self.action == AuditAction.DELETEbase_score += 20
    ...
    def set_extra_data(self, **kwargs) -> None:
        """设置扩展数据"" if self.extra_data is Noneself.extra_data = {}  # typeignore


        self.extra_data.update(kwargs)

    def add_tag(self, tag: str) -> None:
        """添加标签"" if self.tagstags_list = [t.strip() for t in self.tags.split(","))


            if tag not in tags_listtags_list.append(tag)

                self.tags = ",".join(tags_list)  # type: ignoreelse

    self.tags = tag  # type: ignore

    def remove_tag(self, tag: str) -> None:
        """移除标签"" if self.tagstags_list = [t.strip() for t in self.tags.split(","))


            if tag in tags_listtags_list.remove(tag)

                self.tags = ",".join(tags_list) if tags_list else None  # typeignore


    @classmethoddef create_audit_entry()
    ...
    "AuditLog": 审计日志实例
        "" # 自动确定严重级别
        if "severity" not in kwargsif action in [AuditAction.DELETE, AuditAction.GRANT, AuditAction.REVOKE]
    kwargs["severity"] = AuditSeverity.HIGHelif action in [)

                AuditAction.BACKUP,
                AuditAction.RESTORE,
                AuditAction.SCHEMA_CHANGE,
            :
                kwargs["severity"] = AuditSeverity.CRITICALelif action == AuditAction.READ

    kwargs["severity"] = AuditSeverity.LOWelse

    kwargs["severity"] = AuditSeverity.MEDIUM

        # 判断是否为敏感数据
        if "is_sensitive" not in kwargssensitive_tables = {"users", "permissions", "tokens", "passwords"}
    ...
    kwargs["compliance_category"] = "ACCESS_CONTROL elif action in [AuditAction.BACKUP, AuditAction.RESTORE]
    kwargs["compliance_category"] = "DATA_PROTECTION elif kwargs.get("is_sensitive")
    kwargs[)
                    "compliance_category  = "PII"  # Personally Identifiable Informationelse

    kwargs["compliance_category"] = "GENERAL return cls()
            user_id=user_id,
            action=action,
...
class AuditLogSummary:
    """审计日志统计摘要类"" def __init__(self, session)
:
        """初始化"" self.session = session

    def get_user_activity_summary(self, user_id: str, days: int = 30) -> dict[str, Any]:
        """获取用户活动摘要"" from sqlalchemy import and_, func
        ...
        # 按操作类型统计
        action_stats = ()
            self.session.query(AuditLog.action, func.count(AuditLog.id))
.label("count")
...
,
    ...
    def get_table_activity_summary()
        ...
        """获取表操作活动摘要"" cutoff_date = datetime.now() - timedelta(days=days)
...
.label("count")
        ...
        # 按用户统计
        user_stats = ()
            self.session.query(AuditLog.user_id, func.count(AuditLog.id))
.label("count")
