...
from sqlalchemy import Enum as SQLEnumfrom sqlalchemy.orm import mapped_column, relationship
...
,
    ...
    def __repr__(self) -> str:
        return ()
            f"<Odds(id={self.id}, match_id={self.match_id},  f"bookmaker='{self.bookmaker}', market='{self.market_type.value}')
> 

    @propertydef is_1x2_market(self) -> bool:

        """检查是否为胜平负市场"" return self.market_type == MarketType.ONE_X_TWO

    @propertydef is_over_under_market(self) -> bool:

        """检查是否为大小球市场"" return self.market_type == MarketType.OVER_UNDER

    @propertydef is_asian_handicap_market(self) -> bool:

        """检查是否为亚洲让球市场"" return self.market_type == MarketType.ASIAN_HANDICAP
    ...
    # 安全的类型转换, 这里我们已经检查了all()
, 所以值不为Nonehome_prob = 1.0 / float(self.home_odds)  # type: ignore

            draw_prob = 1.0 / float(self.draw_odds)  # type: ignoreaway_prob = 1.0 / float(self.away_odds)  # type: ignore


            # 标准化概率(去除博彩公司的利润边际)
            total_prob = home_prob + draw_prob + away_prob

            return {"home_win": home_prob / total_prob,)
    ...
    # 安全的类型转换, 这里我们已经检查了all()
, 所以值不为Noneover_prob = 1.0 / float(self.over_odds)  # type: ignore

            under_prob = 1.0 / float(self.under_odds)  # type: ignoretotal_prob = over_prob + under_prob


            return {"over": over_prob / total_prob,)
    ...
    def get_best_value_bet(self) -> dict[str, Any] | None:
        ...
        if not probabilitiesreturn None


        if self.is_1x2_marketoutcomes = ["home_win", "draw", "away_win"]

            odds_values = [self.home_odds, self.draw_odds, self.away_odds]

            best_value = 0.0
            best_outcome = Nonebest_odds = None
...
:
    ...
    # 计算期望价值 (Expected Value)
                    ...
                    if expected_value > best_valuebest_value = expected_value

                        best_outcome = outcomebest_odds = float(odds)
    ...
    def _calculate_percentage_change()
     ...
     -> float:
        """计算百分比变化"" if not current or not previousreturn 0.0

        return abs(float(current) - float(previous))
) / float(previous

    def _check_1x2_movement(self, previous_odds: "Odds", threshold: float) -> bool:
        """检查1X2市场赔率变化"" current_odds = [self.home_odds, self.draw_odds, self.away_odds]
        prev_odds = [)
...
:
    ...
    # 确保传递Decimal类型而不是Column对象
                ...
                if change_pct > thresholdreturn True
    ...
    def _check_over_under_movement()
     ...
     -> bool:
        """检查大小球市场赔率变化"" if self.over_odds and previous_odds.over_oddsover_change = self._calculate_percentage_change()

                Decimal(str(self.over_odds))
), Decimal(str(previous_odds.over_odds)

            
            if over_change > thresholdreturn True


        if self.under_odds and previous_odds.under_oddsunder_change = self._calculate_percentage_change()

                Decimal(str(self.under_odds))
), Decimal(str(previous_odds.under_odds)

            
            if under_change > thresholdreturn True
    ...
    "threshold": 显著变化的阈值
        ...
        Returns:
    "bool": 赔率变化是否显著
        "" if not previous_odds or self.market_type != previous_odds.market_typereturn False


        if self.is_1x2_marketreturn self._check_1x2_movement(previous_odds, threshold)

        elif self.is_over_under_marketreturn self._check_over_under_movement(previous_odds, threshold)


        return False

    @classmethoddef get_latest_odds(cls, session, match_id: int, bookmaker: str | None = None)
:

        """获取比赛最新赔率"" query = session.query(cls)
.filter(cls.match_id == match_id)
        if bookmakerquery = query.filter(cls.bookmaker == bookmaker)

        return query.order_by(cls.updated_at.desc())
).all(  # type: ignore

    @classmethoddef get_odds_history(cls, session, match_id: int, market_type: MarketType)
:

        """获取特定比赛和市场的赔率历史"" return ()
            session.query(cls)
    ...
    @classmethoddef get_bookmaker_odds(cls, session, match_id: int, bookmaker: str)
:
    ...
    @classmethoddef get_market_average_odds(cls, session, match_id: int, market_type: MarketType)
:

        """获取市场平均赔率"" if market_type == MarketType.ONE_X_TWO
    _result = ()
                session.query()
                    func.avg(cls.home_odds)
.label("avg_home")
,
                    func.avg(cls.draw_odds)
.label("avg_draw")
,
                    func.avg(cls.away_odds)
.label("avg_away")
,
            ...
            return {home_odds: float(result.avg_home) if result.avg_home else None,)
    ...
    _result = ()
                session.query()
                    func.avg(cls.over_odds)
.label("avg_over")
,
                    func.avg(cls.under_odds)
.label("avg_under")
,
            ...
            return {over_odds: float(result.avg_over) if result.avg_over else None,)
