...
:
    """特征实体类型"" MATCH = "match TEAM = "team PLAYER = "player LEAGUE = "league class FeatureEntity(BaseModel)
:
    """特征实体"" __tablename__ = "feature_entities id: Mapped[int] = mapped_column(Integer, primary_key=True)
"    name: Mapped[str] = mapped_column(String(100))
, nullable=False
    entity_type: Mapped[FeatureEntityType] = mapped_column()
        SQLEnum(FeatureEntityType)
, nullable=False
    
    value: Mapped[float] = mapped_column(DECIMAL(10, 4))
, nullable=True
...
:
    """特征元数据"" __tablename__ = "feature_metadata id: Mapped[int] = mapped_column(Integer, primary_key=True)
    feature_entity_id: Mapped[int] = mapped_column()
        ForeignKey("feature_entities.id")
, nullable=False
...
> # 为了向后兼容, 保留原始的名称
feature_entity = FeatureEntityfeature_metadata = FeatureMetadata
...
:
    """简化的 Features 类以保持向后兼容"" __tablename__ = "features id: Mapped[int] = mapped_column(Integer, primary_key=True)
