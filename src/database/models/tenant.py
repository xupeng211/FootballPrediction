...
class Tenant(BaseModel):
    ...
    slug = Column(
        String(50), unique=True, nullable=False, index=True, comment="租户标识符"
    )
    ...
    # 状态和计划
    status = Column(
        String(20), default=TenantStatus.TRIAL, nullable=False, comment="租户状态"
    )
    plan = Column(
        String(20), default=TenantPlan.BASIC, nullable=False, comment="租户计划"
    )

    # 配额和限制
    max_users = Column(Integer, default=10, nullable=False, comment="最大用户数")
    max_predictions_per_day = Column(
        Integer, default=100, nullable=False, comment="每日最大预测数"
    )
    max_api_calls_per_hour = Column(
        Integer, default=1000, nullable=False, comment="每小时最大API调用数"
    )
    storage_quota_mb = Column(
        Integer, default=100, nullable=False, comment="存储配额(MB)"
    )

    # 时间相关
    trial_ends_at = Column(
        DateTime(timezone=True), nullable=True, comment="试用结束时间"
    )
...
class TenantPermission(BaseModel):
    ...
    # 基本信息
    tenant_id = Column(
        Integer, ForeignKey("tenants.id"), nullable=False, comment="租户ID"
    )
    ...
    # 权限范围和资源
    scope = Column(
        String(20), default=PermissionScope.TENANT, nullable=False, comment="权限范围"
    )
...
class TenantRole(BaseModel):
    ...
    is_system_role = Column(
        Boolean, default=False, nullable=False, comment="是否为系统角色"
    )
...
class RolePermission(BaseModel):
    ...
    # 关联字段
    role_id = Column(
        Integer, ForeignKey("tenant_roles.id"), nullable=False, comment="角色ID"
    )
    permission_id = Column(
        Integer, ForeignKey("tenant_permissions.id"), nullable=False, comment="权限ID"
    )

    # 额外属性
    granted_by = Column(
        Integer, ForeignKey("users.id"), nullable=True, comment="授权者ID"
    )
    granted_at = Column(
    ...
    role: Mapped["TenantRole"] = relationship(
        "TenantRole", back_populates="role_permissions"
    )
    permission: Mapped["TenantPermission"] = relationship(
        "TenantPermission", back_populates="role_permissions"
    )
...
class UserRoleAssignment(BaseModel):
    ...
    # 分配信息
    assigned_by = Column(
        Integer, ForeignKey("users.id"), nullable=True, comment="分配者ID"
    )
    assigned_at = Column(
        DateTime(timezone=True),
        default=datetime.utcnow,
