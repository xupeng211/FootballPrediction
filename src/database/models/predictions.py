...
:
    ...
    存储各种机器学习模型的预测结果
    "" __tablename__ = "predictions # 关联信息
...
,
    ...
    status: Mapped[str] = mapped_column()
        String(20)
, nullable=False, default="pending", comment="预测状态 
...
, nullable=True, comment="实际比赛结果 
    ...
    def get_created_at(self) -> datetime:
        """兼容性方法:获取预测创建时间"" return self.predicted_at  # type: ignore

    @propertydef confidence(self) -> float | None:

        """兼容性属性:获取置信度"" if self.confidence_scorereturn float(self.confidence_score)
...
,
    ...
    def __repr__(self) -> str:
        return ()
            f"<Predictions(id={self.id}, match_id={self.match_id},  f"model='{self.model_name}', _result ='{self.predicted_result.value}')
> 

    @propertydef prediction_summary(self) -> str:

        """返回预测摘要"" probabilities = [)
...
,
        ...
        return f"{probabilities[0][0]} ({probabilities[0][1]:.1%}) @propertydef max_probability(self) -> float:
"
        """返回最高概率"" return max()
...
,
    ...
    @propertydef prediction_confidence_level(self) -> str:

        """返回预测信心等级"" max_prob = self.max_probability

        if max_prob >= 0.7
    return "High elif max_prob >= 0.5
"    return "Medium elsereturn "Low def get_probabilities_dict(self) -> dict[str, float]:

        """获取概率字典"" return {home_win: float(self.home_win_probability))
,
...
,
    ...
    def get_predicted_score(self) -> str | None:
        """获取预测比分"" if ()
            self.predicted_home_score is not Noneand self.predicted_away_score is not None

        
:
            return f"{self.predicted_home_score:.1f}-{self.predicted_away_score:.1f} return None
"
    def get_feature_importance_dict(self) -> dict[str, float] | None:
        """获取特征重要性字典"" if self.feature_importanceif isinstance(self.feature_importance, str)
    return json.loads(self.feature_importance)  # type: ignore
            # 如果是JSON类型, 直接返回
            return ()
                self.feature_importanceif isinstance(self.feature_importance, dict[str, Any])  # type
    ...
    "top_n": 返回前N个重要特征
        ...
        if not feature_importancereturn []
...
, key=lambda x: x[1], reverse=True
    ...
    "actual_result": 实际比赛结果 ('home_win', 'draw', 'away_win')
        ...
        # 预测的概率
        actual_predicted_prob = self.get_probabilities_dict()
[actual_result]
        ...
        return {"prediction_correct": prediction_correct,)
...
:
            if outcome in odds_dataodds = odds_data[outcome]
  ...
  # 限制最大25%
        ...
        "predicted_probability": predicted_prob,
                    ...
                    "recommendation": ()
                        "BET if expected_value > 0.05
                        else "CONSIDER if expected_value > 0
                        else "AVOID 
,
        ...
        # 按期望价值排序
        recommendations.sort(key=lambda x: float(x["expected_value"]))
, reverse=True  # type: ignore

        return recommendations

    def generate_explanation(self) -> dict[str, Any]:
        """生成预测解释"" top_features = self.get_top_features()
        probabilities = self.get_probabilities_dict()

        explanation = {"prediction_summary": self.prediction_summary,)
        "confidence_level": self.prediction_confidence_level,
...
,
            ...
            "additional_predictions": {"over_2_5_goals": ())
                    float(self.over_under_prediction)
                    if self.over_under_predictionelse None
...
,
    ...
    @classmethoddef get_match_predictions(cls, session, match_id: int)
:
    ...
    @classmethoddef get_latest_prediction()

        cls, session, match_id: int, model_name: str | None = None
    ) -> Optional["Predictions":
        """获取最新预测"" query = session.query(cls)
.filter(cls.match_id == match_id)
        if model_namequery = query.filter(cls.model_name == model_name)

        return query.order_by(cls.predicted_at.desc())
).first(  # type: ignore  # type: ignore

    @classmethoddef get_model_predictions(cls, session, model_name: str, limit: int = 100)
:
    ...
    @classmethoddef calculate_model_accuracy(cls, session, model_name: str, days: int = 30)
:
    ...
    "days": 天数
        ...
        predictions = ()
            ...
            .filter()
                and_(  # type: ignorecls.model_name == model_name,)
        ...
        if not predictionsreturn {"total_predictions 0}
        ...
        for prediction in predictions:
            match = prediction.matchactual_result = match.get_result()


            if actual_resultaccuracy = prediction.calculate_accuracy(actual_result)
    ...
    correct_predictions += 1
        ...
        accuracy_rate = correct_predictions / total_predictionsavg_brier_score = total_brier_score / total_predictions
