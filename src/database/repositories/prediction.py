...
class PredictionRepository(BaseRepository[Predictions]):
  ...
  # 添加pass语句
    ...
    async def get_by_user(self, user_id: str, status: Optional[str] = None) -> List[Prediction]:
        """
        获取指定用户的预测
    ...
    async def get_by_status(self, status: PredictionStatus, limit: Optional[int] = None) -> List[Prediction]:
        """
        根据状态获取预测
