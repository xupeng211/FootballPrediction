"""数据库配置模块"" import osfrom dataclasses import dataclass



@dataclassclass DatabaseConfig:

    """封装数据库连接配置及常用连接URL. "" host: strport: int

    database: strusername: str

    password: str | Nonepool_size: int = 10
    ...
    echo: bool = Falseecho_pool: bool = False


    def _is_sqlite(self) -> bool:
        return self.database.endswith(".db") or self.database == ":memory: @propertydef sync_url(self) -> str:
"
        if self._is_sqlite()
    if self.database ==  memory
    ":
"                return "sqlite:///:memory: return f"sqlite:///{self.database} password_part = f":{self.password}" if self.password else " return ()
"            f"postgresql+psycopg2://{self.username}{password_part} f"@{self.host}:{self.port}/{self.database} 

    @propertydef async_url(self) -> str:

        if self._is_sqlite()
    if self.database ==  memory
    ":
"                return "sqlite+aiosqlite:///:memory: return f"sqlite+aiosqlite:///{self.database} password_part = f":{self.password}" if self.password else " return ()
"            f"postgresql+asyncpg://{self.username}{password_part} f"@{self.host}:{self.port}/{self.database} 

    @propertydef alembic_url(self) -> str:
...
def _get_env_bool(key: str, default: bool = False) -> bool:
    value = os.getenv(key)
    if value is Nonereturn default
...
.lower()
    ...
    if env == "test default_db =  memoryelsedefault_db = "football_prediction_dev host = os.getenv(f"{prefix}DB_HOST", "localhost")
...
def get_test_database_config() -> DatabaseConfig:
    """返回测试环境数据库配置. "" return get_database_config("test")


def get_production_database_config() -> DatabaseConfig:
    """返回生产环境数据库配置. "" return get_database_config("production")
