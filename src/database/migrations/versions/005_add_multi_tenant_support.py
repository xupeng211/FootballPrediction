...
down_revision: Union[str, None] = "004_configure_database_permissions"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("max_predictions_per_day", sa.Integer(), nullable=False, comment="每日最大预测数"),
        sa.Column("max_api_calls_per_hour", sa.Integer(), nullable=False, comment="每小时最大API调用数"),
        sa.Column("storage_quota_mb", sa.Integer(), nullable=False, comment="存储配额(MB)"),
        ...
        sa.Column("settings", postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment="租户设置"),
        sa.Column("features", postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment="启用的功能"),
        sa.Column("branding", postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment="品牌定制"),
        sa.Column("statistics", postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment="租户统计"),
        sa.Column("usage_metrics", postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment="使用指标"),
    ...
    op.create_index("idx_tenant_subscription", "tenants", ["subscription_ends_at"], unique=False)
    ...
    op.create_table(
        ...
        sa.Column("actions", postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment="允许的动作列表"),
        sa.Column("conditions", postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment="权限条件"),
        sa.Column("restrictions", postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment="权限限制"),
        sa.Column("is_active", sa.Boolean(), nullable=False, comment="是否激活"),
    ...
    # 创建租户权限索引
    op.create_index("idx_tenant_permission_resource", "tenant_permissions", ["resource_type", "resource_id"], unique=False)
    op.create_index("idx_tenant_permission_scope", "tenant_permissions", ["tenant_id", "scope"], unique=False)
    ...
    op.create_table(
        ...
        sa.Column("is_system_role", sa.Boolean(), nullable=False, comment="是否为系统角色"),
        sa.Column("permissions", postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment="权限列表"),
        sa.Column("restrictions", postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment="角色限制"),
        sa.Column("is_active", sa.Boolean(), nullable=False, comment="是否激活"),
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], ),
        sa.PrimaryKeyConstraint("id"),
    ...
    # 创建租户角色索引
    op.create_index("idx_tenant_role_level", "tenant_roles", ["tenant_id", "level"], unique=False)
    ...
    op.create_table(
        ...
        sa.Column("granted_at", sa.DateTime(timezone=True), nullable=False, comment="授权时间"),
        sa.Column("conditions", postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment="特殊条件"),
        sa.ForeignKeyConstraint(["granted_by"], ["users.id"], ),
        sa.ForeignKeyConstraint(["permission_id"], ["tenant_permissions.id"], ),
    ...
    # 创建角色权限关联索引
    op.create_index("idx_role_permission_granted", "role_permissions", ["granted_at"], unique=False)
    ...
    # 创建用户角色分配索引
    op.create_index("idx_role_assignment_expiry", "user_role_assignments", ["expires_at"], unique=False)
    op.create_index("idx_user_role_assignment", "user_role_assignments", ["user_id", "tenant_id"], unique=False)
    op.create_index(op.f("ix_user_role_assignments_id"), "user_role_assignments", ["id"], unique=False)

    # 为用户表添加租户关联字段
    op.add_column("users", sa.Column("tenant_id", sa.Integer(), nullable=True, comment="所属租户ID"))
    ...
    op.create_index("idx_predictions_tenant_id", "predictions", ["tenant_id"], unique=False)

    # 比赛表
    op.add_column("matches", sa.Column("tenant_id", sa.Integer(), nullable=True, comment="所属租户ID"))
    op.create_foreign_key(
