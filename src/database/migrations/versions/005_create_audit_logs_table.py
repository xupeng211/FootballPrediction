...
import sqlalchemy as safrom alembic import context, op

from sqlalchemy.exc import SQLAlchemyError, DatabaseError

from sqlalchemy import textfrom sqlalchemy.dialects import postgresql


# 版本标识
revision = "005 down_revision = "004_configure_permissions branch_labels = Nonedepends_on = None



def upgrade()
:
    """升级数据库架构 - 创建audit_logs表"" # 创建审计日志表
    op.create_table()
        "audit_logs",
        # 主键
        sa.Column()
            "id", sa.Integer()
, autoincrement=True, nullable=False, comment="审计日志ID 
,
        # 用户信息
        sa.Column()
            "user_id", sa.String(length=100)
...
,
        sa.Column()
            "table_name", sa.String(length=100)
, nullable=True, comment="目标表名 
,
        sa.Column()
            "column_name", sa.String(length=100)
, nullable=True, comment="目标列名 
,
        sa.Column("record_id", sa.String(length=100))
, nullable=True, comment="记录ID"
,
        # 数据变更信息
        sa.Column("old_value", sa.Text())
, nullable=True, comment="操作前值"
,
        sa.Column("new_value", sa.Text())
, nullable=True, comment="操作后值"
,
        sa.Column()
            "old_value_hash",
            sa.String(length=64)
...
,
        # 上下文信息
        sa.Column()
            "ip_address", sa.String(length=45)
, nullable=True, comment="客户端IP地址 
,
        sa.Column("user_agent", sa.Text())
, nullable=True, comment="用户代理"
,
        sa.Column()
            "request_path", sa.String(length=500)
, nullable=True, comment="请求路径 
,
        sa.Column()
            "request_method", sa.String(length=10)
...
def downgrade(: """降级数据库架构 - 删除audit_logs表"" # 检查是否在离线模式)
    if not context.is_offline_mode()
    connection = op.get_bind()
