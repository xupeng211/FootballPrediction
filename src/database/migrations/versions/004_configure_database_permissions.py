...
revision = "004_configure_permissions down_revision = "f48d412852cc branch_labels = Nonedepends_on = None
...
def upgrade() -> None:
    ...
    # type: ignorelogger.info("⚠️  离线模式:跳过数据库权限配置")

        # 在离线模式下执行注释, 确保 SQL 生成正常
        op.execute("-- offline mode: skipped database user creation")  # type: ignoreop.execute("-- offline mode: skipped database permission configuration")  # type: ignore
    ...
    # 获取当前数据库名称(支持测试环境)
    db_name = os.getenv("TEST_DB_NAME", "football_prediction")  # type: ignoreif os.getenv("ENVIRONMENT") == "test # typeignoredb_name = os.getenv("TEST_DB_NAME", "football_prediction_test")  # typeignore
"
...
def downgrade() -> None:
    """回滚数据库权限配置"" # 检查是否在离线模式
    if context.is_offline_mode()
    # type: ignorelogger.info("⚠️  离线模式:跳过数据库权限回滚")
    ...
    if os.getenv("ENVIRONMENT") == "test db_name = os.getenv("TEST_DB_NAME", "football_prediction_test")
...
"
)  # type: ignoreconnection.execute(text("DROP USER IF EXISTS football_writer;)
"
  # type: ignore
