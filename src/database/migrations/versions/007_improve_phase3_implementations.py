...
revision = "007_improve_phase3_implementations down_revision = "d3bf28af22ff branch_labels = Nonedepends_on = None
...
:  # 创建当前年份和未来2年的分区
        op.execute()
            f"" CREATE TABLE IF NOT EXISTS raw_scores_data_y{year_offset}
        ...
        SELECTm.id,
            ...
            CASE
                WHEN m.match_status = 'IN_PROGRESS' THEN
                    EXTRACT(EPOCH FROM (NOW() - m.match_time))
 / 60 as minutes_elapsed
                ELSE NULL
            END,
            -- 最新赔率信息
            (SELECT JSON_AGG())
...
;
            ...
            SELECT
                ...
                COUNT(*)
::BIGINT,
                'Odds with invalid values (<= 0 or > 1000)
''::TEXT
'            FROM odds
