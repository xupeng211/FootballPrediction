...
Create Date: 2025-09-10 20:4,
    2:25.754318

"" # revision identifiers, used by Alembic.
    "revision": str = "f48d412852cc "down_revision": Union[str, None] = "d56c8d0d5aa0"  # type: ignore
...
, nullable=False, comment="开始时间"
,  # type: ignoresa.Column("end_time", sa.DateTime())
...
, nullable=True, comment="错误信息"
,  # type: ignoresa.Column("created_at", sa.DateTime())
, nullable=False, comment="创建时间"
,  # type: ignore

        sa.PrimaryKeyConstraint("id")
,  # type: ignorecomment="数据采集日志表",
...
, nullable=False, comment="原始JSON数据"
,  # type: ignoresa.Column("collected_at", sa.DateTime())
...
,
        sa.Column("match_time", sa.DateTime())
, nullable=True, comment="比赛时间"
,  # type: ignoresa.Column("created_at", sa.DateTime())
, nullable=False, comment="创建时间"
,  # type: ignore

        sa.PrimaryKeyConstraint("id")
,  # type: ignorecomment="Bronze层原始比赛数据表",

    

    # 为Bronze层比赛数据表创建索引
    op.create_index("idx_raw_match_data_source", "raw_match_data", ["data_source"])  # type: ignoreop.create_index(  # type: ignore)

        "idx_raw_match_data_collected_at", "raw_match_data", ["collected_at"]
    
    op.create_index("idx_raw_match_data_processed", "raw_match_data", ["processed"])  # type: ignoreop.create_index(  # type: ignore)
...
,
        sa.Column()
            "bookmaker", sa.String(length=100)
, nullable=True, comment="博彩公司 
",
        sa.Column()
            "market_type", sa.String(length=50)
...
,
    ...
    # 添加新的特征字段以支持更复杂的ML特征
        ...
        # recent_5_goals_against, h2h_draws, h2h_lossesnew_columns = [)

            ("home_advantage_score", sa.Numeric(5, 2))
, "主场优势评分"
,
            ()
                "form_trend",
...
"            
,
            ("odds_movement", sa.String(length=20))
, "赔率变动趋势"
,
            ("market_sentiment", sa.Numeric(5, 4))
, "市场情绪指数"
,
            ("feature_version", sa.String(length=20))
, "特征版本"
,
...
,
        ...
        for column_name, column_type, column_comment in new_columns: tr,
    y:
                op.add_column()
                    "features",
                    sa.Column()
...
,
                ...
                # 如果字段已存在,忽略错误
                logger.info(f"Warning: Column {column_name} already exist,)
"    s: {e}"
"
    except (SQLAlchemyError, DatabaseError, ConnectionError, TimeoutError) as e:
        # 如果features表不存在或字段已存在,忽略错误但记录日志
        logger.info(f"Warning: Could not add columns to features tabl,)
"    e: {e}"
"
...
def downgrade() -> None:
    ...
    # 只删除在此迁移中添加的字段
        ...
        for column_name in columns_to_drop: tr,
    y:
        ...
        # 忽略字段不存在的错误但记录日志
        logger.info(f"Warning: Could not drop columns from features tabl,)
"    e: {e}"
"
    # 删除Bronze层赔率数据表
