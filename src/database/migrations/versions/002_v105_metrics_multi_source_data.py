...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("source_name", sa.String(length=50), nullable=False, comment="数据源名称 (如: Entity_P)"),
        ...
        # 完整性评分和验证
        sa.Column("integrity_score", sa.Float(), nullable=True, comment="完整性评分 (1/P1 + 1/P2 + 1/P3)"),
        ...
        sa.Column("updated_at", sa.DateTime(), nullable=False, server_default=sa.text("CURRENT_TIMESTAMP"), comment="记录更新时间"),

        # 主键约束
        sa.PrimaryKeyConstraint("id", name="pk_metrics_multi_source_data"),

        # 唯一约束：同一比赛同一数据源只能有一条记录
        sa.UniqueConstraint("match_id", "source_name", name="uq_metrics_multi_source_data_match_source"),
...
def downgrade() -> None:
    ...
    # 删除索引
    op.drop_index("idx_metrics_multi_source_data_match_valid", table_name="metrics_multi_source_data")
    op.drop_index("idx_metrics_multi_source_data_data_timestamp", table_name="metrics_multi_source_data")
    ...
    op.drop_index("idx_metrics_multi_source_data_source_name", table_name="metrics_multi_source_data")
