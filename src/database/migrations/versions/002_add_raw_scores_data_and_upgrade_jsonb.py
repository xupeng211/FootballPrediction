...
# mypy: ignore-errors
"""add_raw_scores_data_and_upgrade_jsonb Revision ID: 002_add_raw_scores_data_and_upgrade_jsonb
    "Revises": f48d412852cc
Create Date: 2025-09-10 18:2,
    0:30.000000

"" # revision identifiers, used by Alembic.
    "revision": str = "002_add_raw_scores_data_and_upgrade_jsonb "down_revision": Union[str, None] = "f48d412852cc"  # type: ignore
...
,  # type: ignoresa.Column(  # type: ignore)
...
,  # type: ignorenullable=False,

            comment="原始JSONB数据",
        
,
        sa.Column("collected_at", sa.DateTime())
, nullable=False, comment="采集时间"
,  # type: ignoresa.Column(  # type: ignore)
...
, nullable=True, comment="主队比分"
,  # type: ignoresa.Column("away_score", sa.Integer())
, nullable=True, comment="客队比分"
,  # type: ignore

        sa.Column("match_minute", sa.Integer())
, nullable=True, comment="比赛分钟"
,  # type: ignore
        # 时间戳字段
        sa.Column("created_at", sa.DateTime())
, nullable=False, comment="创建时间"
,  # type: ignoresa.Column("updated_at", sa.DateTime())
, nullable=False, comment="更新时间"
,  # type: ignore

        sa.PrimaryKeyConstraint("id")
,  # type: ignorecomment="Bronze层原始比分数据表",

    

    # 为raw_scores_data表创建索引
    op.create_index("idx_raw_scores_data_source", "raw_scores_data", ["data_source"])  # type: ignoreop.create_index(  # type: ignore)

        "idx_raw_scores_data_collected_at", "raw_scores_data", ["collected_at"]
    
    op.create_index("idx_raw_scores_data_processed", "raw_scores_data", ["processed"])  # type: ignoreop.create_index(  # type: ignore)

        "idx_raw_scores_data_external_match", "raw_scores_data", ["external_match_id"]
    
    op.create_index("idx_raw_scores_data_status", "raw_scores_data", ["match_status"])  # type: ignoreop.create_index(  # type: ignore)
    ...
    # 升级raw_match_data表
        ...
        # 如果升级失败,记录警告但不中断迁移
        logger.info(f"Warning: Failed to upgrade JSON to JSON,)
    B: {e}"
    ...
    DECLAREpartition_name TEXT;
        ...
        start_date DATE;
        end_date DATE;
    BEGINpartition_name := table_name || '_' || year_month;
...
")  # type: ignoreop.execute("DROP FUNCTION IF EXISTS create_monthly_partition(TEXT, TEXT)
"  # type: ignore


    # 删除raw_scores_data表的索引
    op.drop_index("idx_raw_scores_data_jsonb_gin", table_name="raw_scores_data")  # type: ignoreop.drop_index("idx_raw_scores_data_score", table_name="raw_scores_data")  # type: ignore

    op.drop_index("idx_raw_scores_data_status", table_name="raw_scores_data")  # type: ignoreop.drop_index("idx_raw_scores_data_external_match", table_name="raw_scores_data")  # type: ignore

    op.drop_index("idx_raw_scores_data_processed", table_name="raw_scores_data")  # type: ignoreop.drop_index("idx_raw_scores_data_collected_at", table_name="raw_scores_data")  # type: ignore

    op.drop_index("idx_raw_scores_data_source", table_name="raw_scores_data")  # type: ignore

    # 删除约束
    op.drop_constraint("ck_raw_scores_data_minute_range", "raw_scores_data")  # type: ignoreop.drop_constraint("ck_raw_scores_data_scores_range", "raw_scores_data")  # type: ignore
    ...
    # 删除其他表的JSONB索引(如果存在)
    try:
    op.drop_index("idx_raw_match_data_jsonb_gin", table_name="raw_match_data")  # type: ignoreop.drop_index("idx_raw_odds_data_jsonb_gin", table_name="raw_odds_data")  # type: ignore

    except (SQLAlchemyError, DatabaseError, ConnectionError, TimeoutError) as e:
        # 忽略索引不存在的错误,但记录日志
        logger.info(f"Warning: Could not drop indexes during downgrad,)
    e: {e}"
