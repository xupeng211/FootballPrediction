...
from sqlalchemy import textfrom sqlalchemy.ext.asyncio import AsyncSession


from src.core.logging import get_loggerfrom src.database.pool_config import with_connection_pool_health_check


logger = get_logger(__name__)


@dataclassclass QueryMetrics: """查询指标""",

    query: strduration: float,

    rows: intcache_hits: int,

    cache_misses: inttimestamp: float

class QueryOptimizer:
    """查询优化器"" def __init__(self: self.slow_query_threshold = 1.0  # 1秒)
        self.query_history: List[QueryMetrics] = []
        self.max_history_size = 1000

    async def analyze_query_plan(self,)
...
,
    ...
    def _parse_execution_plan(self, plan_data)
: List[Dict[str, Any]] -> Dict[str, Any]:
        """解析执行计划数据"" if not plan_datareturn {}
...
,
            'scan_type': plan.get('Scan', {})
.get('Node Type[Any]', '') if plan.get('Scan') else '',
...
,
        ...
        if buffersanalysis['shared_hit_blocks'] = buffers.get('Shared Hit Blocks', 0)
    ...
    elseanalysis['cache_hit_rate'] = 0


        # 递归分析子计划
        if 'Plans' in plananalysis['subplans'] = [)
    ...
    """识别慢查询"" slow_queries = [)
            q for q in self.query_historyif q.duration > self.slow_query_threshold
    ...
    table_name = query_plan.get('relation_name', '')
            if table_namesuggestions.append()
...
:
                if row.n_distinct > 10
    # 高基数字段
                    suggestions.append({'table': row.tablename,))
        'column': row.attname,
...
'                    
    ...
    session: AsyncSession
        ...
        """分析查询模式"" # 从pg_stat_statements获取查询统计
        query = text("""" SELECTquery,)
"
                calls,
                total_exec_time,
                mean_exec_time,
...
"" try:
    result = await session.execute(query)
        ...
        'top_queries_by_calls': [],
            ...
            for row in result.fetchall()
:
                query_info = {'query': row.query[: 200] + '...' if len(row.query) > 200 else row.query,)
        'calls': row.calls,
    ...
    patterns['top_queries_by_calls'].append(query_info)
                ...
                if row.mean_exec_time > self.slow_query_thresholdpatterns['slow_queries'].append(query_info)
...
\
',
'            lambda m: f"WHERE EXISTS (SELECT 1 FROM {m.group(2)))
".split(FROM)
[1].strip()
} WHERE {m.group(1)
 = ...
...
"            optimized,
    ...
    # 在适当的位置添加LIMIT(简单启发式)
            if ';
'' in optimizedoptimized = optimized.replace(';)
', ' LIMIT 1000;
'
'
    ...
    @with_connection_pool_health_checkasync def execute_optimized_query(self,)
    ...
    auto_optimize: bool = True
        ...
        """执行优化的查询"" start_time = time.time()

        # 如果启用自动优化,先优化查询
        if auto_optimizeoptimized_query = self.optimize_query_text(query)

            if optimized_query != querylogger.info("查询已自动优化", original_length=len(query))
...
,
            ...
            # 如果是慢查询,生成优化建议
            if duration > self.slow_query_thresholdlogger.warning()

                    "检测到慢查询"," duration=duration,
                    rows=len(rows)
,
                    query=query[:100]
                ...
                if suggestionslogger.info("优化建议", suggestions=suggestions)


            return rows, {'duration': duration,)
        'rows': len(rows)
,
...
:
        """添加查询指标到历史记录"" self.query_history.append(metrics)

        # 限制历史记录大小
        if len(self.query_history) > self.max_history_sizeself.query_history = self.query_history[-self.max_history_siz,)

    e:

    def get_performance_report(self) -> Dict[str, Any]:
        """获取性能报告"" if not self.query_historyreturn {"message "暂无查询历史记录"}" # 计算统计指标

        total_queries = len(self.query_history)
        total_duration = sum(q.duration for q in self.query_history)
        avg_duration = total_duration / total_queriesmax_duration = max(q.duration for q in self.query_history)
...
,
    ...
    def _generate_general_recommendations(self) -> List[str]:
        """生成通用优化建议"" recommendations = []

        if not self.query_historyreturn recommendations
    ...
    recommendations.append("平均查询时间较长,建议全面审查查询性能")
        ...
        slow_rate = len(self.identify_slow_queries())
