...
from sqlalchemy import Column, DateTime, Integerfrom sqlalchemy.orm import DeclarativeBase



class Base(DeclarativeBase)
:
    """SQLAlchemy基础模型类"" class TimestampMixin:
    """时间戳混入类,为模型添加创建时间和更新时间字段"" created_at = Column()
...
:
    ...
    def to_dict(self, exclude_fields: set | None = None) -> dict[str, Any]:
        ...
        Args: "exclude_fields": 需要排除的字段集合,
    Returns:
            Dict[str, Any]: 模型字典表示
        "" if exclude_fields is Noneexclude_fields = set()


    result: dict[str, Any] = {}
        for column in self.__table__.columns:
            column_name = column.nameif column_name not in exclude_fields

    value = getattr(self, column_name)
                if isinstance(value, datetime)
    # 将datetime转换为ISO格式字符串
                    result[column_name] = value.isoformat()
                elseresult[column_name] = value

        return result

    @classmethoddef from_dict(cls, data: dict[str, Any])
:
    ...
    "exclude_fields": 需要排除的字段集合
        "" if exclude_fields is Noneexclude_fields = {"id", "created_at"}  # 默认排除ID和创建时间
