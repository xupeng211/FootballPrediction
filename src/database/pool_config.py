...
import asyncpgfrom sqlalchemy import event

from sqlalchemy.engine import Enginefrom sqlalchemy.ext.asyncio import AsyncEngine, create_async_engine

from sqlalchemy.pool import QueuePool

from src.core.config import get_settingsfrom src.core.logging import get_logger


logger = get_logger(__name__)
class DatabasePoolConfig:
    """数据库连接池配置管理器"" def __init__(self)
:
        self.settings = get_settings()
        self._engine: AsyncEngine | None = None

    def get_pool_config(self) -> dict[str, Any]:
        """获取优化的连接池配置"" # 基于系统资源动态调整
        cpu_count = os.cpu_count() or 4

        # 根据环境配置不同的池大小
        if self.settings.environment == "production pool_size = min(30, cpu_count * 5)
"            max_overflow = min(50, pool_size * 2)
            pool_timeout = 30
            pool_recycle = 3600
        elif self.settings.environment == "staging pool_size = min(20, cpu_count * 3)
"            max_overflow = min(30, pool_size * 1.5)
            pool_timeout = 20
            pool_recycle = 1800
    "else":  # developmentpool_size = min(10, cpu_count * 2)
    ...
    async def create_engine(self) -> AsyncEngine:
        """创建优化的数据库引擎"" if self._engine is Nonepool_config = self.get_pool_config()
...
:
        """注册数据库事件监听器"" @event.listens_for(self._engine.sync_engine, "connect")
        def receive_connect(dbapi_connection, connection_record)
:
            """连接建立时的回调"" logger.debug("数据库连接已建立")

        @event.listens_for(self._engine.sync_engine, "checkout")
        def receive_checkout(dbapi_connection, connection_record, connection_proxy)
:
            """连接从池中取出时的回调"" # 记录连接池使用情况
...
:
            """连接返回池中时的回调"" logger.debug("连接已返回池中")

        @event.listens_for(Engine, "before_execute")
        def receive_before_execute()
            conn, clauseelement, multiparams, params, execution_options
        : """SQL执行前的回调"" if execution_options.get("log_query", False)
    logger.info("执行SQL", sql=str(clauseelement))
        ...
        : """SQL执行后的回调"" if execution_options.get("log_query", False)
    logger.info("SQL执行完成", rows=result.rowcount)

    async def monitor_pool_health(self)
:
        """监控连接池健康状态"" if not self._enginereturn
    ...
    async def create_test_connection(self) -> bool: """创建测试连接验证数据库可用性""",
    try:
...
:
        """关闭数据库引擎"" if self._engineawait self._engine.dispose()
...
:
    ...
    def get_pool_config(self) -> dict[str, Any]:
        """获取AsyncPG连接池配置"" return {"min_size": 5,)
        "max_size": 20,
            "max_queries": 50000,
            "max_inactive_connection_lifetime 300,
            "timeout": 60,
            "command_timeout": 30,
            "server_settings": {"application_name": "football_prediction",)
        "timezone": "UTC",
...
:
        """创建AsyncPG连接池"" config = self.get_pool_config()
...
# 连接池健康检查装饰器
def with_connection_pool_health_check(func: """连接池健康检查装饰器"" async def wrapper(*args, **kwargs))
:
        # 在执行前检查连接池健康状态
        if pool_config._enginepool = pool_config._engine.pool
...
class PoolStatsCollector:
    """连接池统计信息收集器"" def __init__(self: self.stats = {"total_requests": 0,))
...
:
        """收集连接池统计信息"" if not pool_config._enginereturn


        pool = pool_config._engine.poolstatus = pool.status()
