...
"                f"错误: {error_message}",
    ...
    async def log_api_failure(self,)
    "task_name": str,
    "api_endpoint": str,
    ...
    "response_data": 响应数据
        ...
        try:
            ...
            logger.warning()
                f"API调用失败: {api_endpoint} (状态,)
"    码: {http_status}, 重试: {retry_count}, "
"                f"错误: {error_message}",
                extra={)
                    "task_name": task_name,
                    "api_endpoint": api_endpoint,
        ...
        except (RuntimeError, ValueError, ConnectionError) as log_error: logger.error(f"记录API错误日志失,)
"    败: {str(log_error)}"
"
    async def log_data_collection_error(self,)
    "data_source": str,
    "collection_type": str,
    "error_message": str,
    ...
    "error_count": 错误数量
        ...
        except (RuntimeError, ValueError, ConnectionError) as log_error: logger.error(f"记录数据采集错误失,)
"    败: {str(log_error)}"
"            return None
    ...
    async def _save_error_to_db(self, error_details: dict[str, Any]) -> None:
        ...
        except (RuntimeError, ValueError, ConnectionError) as db_error: logger.error(f"保存错误到数据库失,)
"    败: {str(db_error)}"
"
    async def _get_db_type(self) -> str:
        """获取数据库类型"""
        if self._db_type is None: tr,
    y:
                # 获取数据库引擎来检测类型
    ...
    async def _ensure_error_logs_table_exists(self, session) -> None: """确保 error_logs 表存在""",
    try:
            db_type = await self._get_db_type()
        ...
        except (RuntimeError, ValueError, ConnectionError) as create_error: logger.warning(f"创建 error_logs 表失,)
"    败: {str(create_error)}"
"
    ...
    Returns:
        ...
        except (RuntimeError, ValueError, ConnectionError) as stats_error: logger.error(f"获取错误统计失,)
"    败: {str(stats_error)}"
"            return {)
                "total_errors": 0,
                "task_errors": [],
    ...
    Returns:
        ...
        except (RuntimeError, ValueError, ConnectionError) as cleanup_error: logger.error(f"清理错误日志失,)
"    败: {str(cleanup_error)}"
"            return 0
