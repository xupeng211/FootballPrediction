...
class TaskMonitor:
    ...
    def __init__(self, registry: CollectorRegistry | None = None):
        ...
        Args: registr,
    y: 可选的 Prometheus 注册表实例,主要用于测试隔离
        """
    ...
    def record_task_start(self, task_name: str, task_id: str) -> None:
        ...
        logger.info(f"任务开始: {task_name} (I,))
"    D: {task_id}"
"
    def record_task_completion(self, task_name: str, task_id: str, duration: float, status: str = "success")
     -> None:
        """
        记录任务完成
    ...
    status: 任务状态 (success / failed)
        ...
        logger.info()
            f"任务完成: {task_name} (I,)
"    D: {task_id}), 状态: {status}, 耗时: {duratio,
    n:.2fs"
"        
    ...
    async def calculate_error_rates(self) -> dict[str, float]:
        ...
        except (RuntimeError, ValueError, ConnectionError) as e: logger.error(f"计算错误率失,)
"    败: {str(e)}"
"            return {}
    ...
    Returns:
        ...
        except (RuntimeError, ValueError, ConnectionError) as e: logger.error(f"获取任务统计失,)
"    败: {str(e)}"
...
"                    
        ...
        except (RuntimeError, ValueError, ConnectionError) as e: logger.error(f"检查任务健康状态失,)
"    败: {str(e)}"
"            return {)
    ...
    async def _get_queue_sizes(self) -> dict[str, int]:
        ...
        try:
            ...
            redis_client = redis.from_url()
                os.getenv("REDIS_URL", "redis: //localhos,)
"    t:6379 / 0"
"            
        ...
        except (RuntimeError, ValueError, ConnectionError) as e: logger.warning(f"获取队列大小失,)
"    败: {str(e)}"
"            return {}
    ...
    async def _check_task_delays(self) -> dict[str, float]:
        ...
        except (RuntimeError, ValueError, ConnectionError) as e: logger.warning(f"检查任务延迟失,)
"    败: {str(e)}"
"            return {}
