...
class LeagueCollector(FootballDataCollector):
            ...
            for competition in supported_competitions:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to collect standings for {comp_code}: {e}")
    ...
    async def collect_league_details(self, competition_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to collect league details for {competition_code}: {e}")
            raise
    ...
    def normalize_standings_data(self, standings: List[Dict[str, Any]], competition: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        标准化积分榜数据格式
                ...
                for idx, team_standing in enumerate(table):
                    normalized_team = {
                        ...
                        'team': {
                            ...
                            'short_name': team_standing.get('team', {}).get('shortName'),
        ...
        try:
            ...
            stats = {
                ...
                'statistics': {
                    ...
                    'average_goals_per_game': round((total_goals_for + total_goals_against) / max(total_games_played, 1), 2),
                    'average_points_per_game': round(total_points / max(total_games_played, 1), 2),
                    'highest_points': max(team['points'] for team in standings) if standings else 0,
                    'lowest_points': min(team['points'] for team in standings) if standings else 0
                },
            ...
            logger.error(f"Failed to collect league statistics for {competition_code}: {e}")
            return {}

    async def collect_top_teams(self, competition_code: str, limit: int = 10) -> List[Dict[str, Any]]:
        """
        收集联赛排名靠前的球队
        ...
        try:
            ...
            # 按积分排序并返回前N名
            top_teams = sorted(standings, key=lambda x: x['points'], reverse=True)[:limit]
    ...
    async def collect_league_matches_by_round(self, competition_code: str, matchday: Optional[int] = None) -> Dict[str, Any]:
        """
        收集特定轮次的比赛
        ...
        try:
            ...
            if not matchday:
                raise ValueError(f"No matchday specified and no current matchday found for {competition_code}")
            ...
            logger.info(f"Collected {len(matches)} matches for {competition_code} matchday {matchday}")
            return result

        except Exception as e:
            logger.error(f"Failed to collect league matches by round for {competition_code}: {e}")
            return {}
        ...
        try:
            ...
            for competition in all_competitions:
                ...
                if (search_term_lower in name or
                    search_term_lower in code or
                    competition.get('id') == search_term):

                    normalized_league = self.normalize_league_data(competition)
                    if normalized_league and 'error' not in normalized_league:
                        matching_competitions.append(normalized_league)

            logger.info(f"Found {len(matching_competitions)} competitions matching '{search_term}'")
