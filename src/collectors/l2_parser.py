...
class L2Parser:
    ...
    def parse_match_data(
        self, raw_data: Union[Dict[str, Any], str]
    ) -> L2DataProcessingResult:
        """
        解析比赛数据的主入口方法
        ...
        try:
            ...
            ctx = ParsingContext(
                raw_data=raw_data, parsed_sections=[], errors=[], logger=self.logger
            )
            ...
            self.logger.info(
                f"Successfully parsed {len(ctx.parsed_sections)} data sections: {ctx.parsed_sections}"
            )
            return result
    ...
    def _parse_all_sections(self, ctx: ParsingContext) -> L2MatchData:
        ...
        return L2MatchData(
            ...
            if team_stats and team_stats.get("home")
            else TeamStats(),
            away_stats=team_stats.get("away")
            if team_stats and team_stats.get("away")
            else TeamStats(),
            # 比赛事件
            events=match_events,
            # 射门数据
            ...
            # 球员评分数据 (转换为字典格式)
            player_ratings={
                str(r.player_id or i): r for i, r in enumerate(player_ratings)
            }
            if player_ratings
            else {},
            # 元数据
    ...
    def _extract_match_basic_info(self, ctx: ParsingContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if score_str and score_str != "0-0":
                ...
                self.logger.debug(
                    f"Step 2: Converting score string '{score_str}' to integers"
                )
                ...
                self.logger.info(
                    f"ULTIMATE: Extracted scores from string: home={home_score}, away={away_score}"
                )
            else:
                # 备用方案：尝试直接路径提取
                self.logger.warning(
                    "Score string extraction failed, trying direct path extraction"
                )

                home_score_raw = self._get_nested_value_robust(
                    ctx.raw_data, ["header", "teams", 0, "score"]
                )
                away_score_raw = self._get_nested_value_robust(
                    ctx.raw_data, ["header", "teams", 1, "score"]
                )
                ...
                if home_score_raw is not None and away_score_raw is not None:
                    ...
                    self.logger.info(
                        f"BACKUP: Extracted scores from direct path: home={basic_info['home_score']}, away={basic_info['away_score']}"
                    )
                else:
                    basic_info["home_score"] = 0
                    basic_info["away_score"] = 0
                    self.logger.warning(
                        "All score extraction methods failed, defaulting to 0-0"
                    )
            ...
            # 最终比分验证
            self.logger.info(
                f"Final extracted scores: home={basic_info['home_score']}, away={basic_info['away_score']}"
            )
        ...
        for key in path:
            ...
            if isinstance(current, list) and isinstance(key, int):
                if 0 <= key < len(current):
                    ...
                    self.logger.debug(
                        f"Robust path matching: list index {key} out of range (len={len(current)})"
                    )
                    return None
            ...
            # 处理列表索引 (字符串形式的数字)
                ...
                if 0 <= index < len(current):
                    ...
                    self.logger.debug(
                        f"Robust path matching: list index {index} out of range (len={len(current)})"
                    )
                    return None
            ...
            # 处理字典访问
                ...
                if found_key is not None:
                    ...
                    self.logger.debug(
                        f"Robust path matching: found '{found_key}' for '{key}'"
                    )
                else:
                    self.logger.debug(
                        f"Robust path matching: key '{key}' not found in {list(current.keys())}"
                    )
                    return None
            else:
                self.logger.debug(
                    f"Robust path matching: unexpected type {type(current)} for key '{key}'"
                )
                return None
    ...
    def _parse_team_stats(self, ctx: ParsingContext) -> Optional[Dict[str, TeamStats]]:
        """解析球队统计数据"""
        try:
            stats_data = self._get_nested_value_robust(
                ctx.raw_data, ["content", "stats"]
            )
            if not stats_data:
                return None
    ...
    def _parse_single_team_stats(
        self, stats_dict: Dict[str, Any], team_index: int
    ) -> TeamStats:
        """
        解析单个球队的统计数据
        ...
        return TeamStats(
            ...
            shots_on_target=self._extract_stat_value(
                stats_dict, "shotsOnTarget", team_index
            ),
            corners=self._extract_stat_value(stats_dict, "corners", team_index),
            fouls=self._extract_stat_value(stats_dict, "fouls", team_index),
            yellow_cards=self._extract_stat_value(
                stats_dict, "yellowCards", team_index
            ),
    ...
    def _extract_stat_value(
        self, stats_dict: Dict[str, Any], stat_type: str, team_index: int
    ) -> Optional[float]:
        """从统计数据字典中提取指定类型的值"""
    ...
    def _parse_match_events(self, ctx: ParsingContext) -> List[L2MatchEvent]:
        """解析比赛事件"""
        try:
            events_data = self._get_nested_value_robust(
                ctx.raw_data, ["content", "events"]
            )
            if not events_data:
                return []
                ...
                for event in events_data:
                    if isinstance(event, dict):
                        ...
                        parsed_event = L2MatchEvent(
                            ...
                            is_goal=event_type_str
                            in ["Goal", "OwnGoal", "PenaltyGoal"],
                            is_own_goal=event_type_str == "OwnGoal",
                            card_type=event.get("cardType")
                            if "card" in event_type_str.lower()
                            else None,
                            substituted_player=event.get("substitutedPlayer", {}).get(
                                "name"
                            )
                            if event_type_str == "Substitution"
                            and event.get("substitutedPlayer")
                            else None,
                        )
    ...
    def _parse_shot_data(self, ctx: ParsingContext) -> List[ShotData]:
        """解析射门数据"""
        try:
            shot_data = self._get_nested_value_robust(
                ctx.raw_data, ["content", "shotmap", "shots"]
            )
            if not shot_data:
                return []
                ...
                for shot in shot_data:
                    if isinstance(shot, dict):
                        parsed_shot = ShotData(
                            ...
                            x=shot.get("position", {}).get("x", 0.0)
                            if shot.get("position")
                            else 0.0,
                            y=shot.get("position", {}).get("y", 0.0)
                            if shot.get("position")
                            else 0.0,
    ...
    def _parse_player_ratings(self, ctx: ParsingContext) -> List[L2PlayerRating]:
        """解析球员评分"""
        try:
            ratings_data = self._get_nested_value_robust(
                ctx.raw_data, ["content", "playerRatings"]
            )
            if not ratings_data:
                return []
    ...
    def _parse_metadata(self, ctx: ParsingContext) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # 体育场信息
            stadium_name = self._get_nested_value_robust(
                ctx.raw_data, ["content", "ground", "name"]
            )
            if stadium_name:
                metadata["stadium"] = StadiumInfo(name=stadium_name)

            # 裁判信息
            referee_data = self._get_nested_value_robust(
                ctx.raw_data, ["content", "referee"]
            )
            ...
            # 天气信息
            weather_data = self._get_nested_value_robust(
                ctx.raw_data, ["content", "weather"]
            )
            ...
            # 观众数量
            attendance = self._get_nested_value_robust(
                ctx.raw_data, ["content", "attendance"]
            )
            if attendance:
    ...
    def _extract_formations(
        ...
        try:
            # 基于实际数据结构的正确路径
            lineup_base = self._get_nested_value_robust(
                ctx.raw_data, ["content", "lineup"]
            )
            ...
            if "homeTeam" in lineup_base:
                ...
                if isinstance(home_team_data, dict) and "formation" in home_team_data:
                    home_formation = self._validate_formation(
            ...
            if "awayTeam" in lineup_base:
                ...
                if isinstance(away_team_data, dict) and "formation" in away_team_data:
                    away_formation = self._validate_formation(
            ...
            if not home_formation or not away_formation:
                ...
                for path in backup_paths:
                    ...
                    if lineups_data:
                        backup_home, backup_away = (
                            self._extract_formations_from_generic(lineups_data)
                        )
            ...
            if home_formation or away_formation:
                ...
                self.logger.debug(
                    f"Extracted formations: home={home_formation}, away={away_formation}"
                )
        ...
        try:
            ...
            if "homeTeam" in lineup_base:
                ...
                if isinstance(home_team_data, dict):
                    home_lineup = self._parse_single_lineup(
                        home_team_data, is_home=True
                    )
            ...
            if "awayTeam" in lineup_base:
                ...
                if isinstance(away_team_data, dict):
                    away_lineup = self._parse_single_lineup(
                        away_team_data, is_home=False
                    )

            if home_lineup or away_lineup:
                ctx.parsed_sections.append("lineups")
                self.logger.debug(
                    f"Extracted lineups: home={len(home_lineup.starters) if home_lineup else 0} starters, away={len(away_lineup.starters) if away_lineup else 0} starters"
                )
    ...
    def _parse_single_lineup(
        ...
        try:
            ...
            lineup = TeamLineup(
                ...
                manager=team_data.get("coach", {}).get("name")
    ...
    def _extract_formations_from_generic(
        self, lineups_data: Any
    ) -> Tuple[Optional[str], Optional[str]]:
        """
        从通用格式中提取阵型（备用方法）
        ...
        if "home" in lineups_data and "away" in lineups_data:
            home_formation = self._validate_formation(
                lineups_data["home"].get("formation")
            )
            away_formation = self._validate_formation(
                lineups_data["away"].get("formation")
            )
                ...
                if stats_data and isinstance(stats_data, list):
                    for stat in stats_data:
                        if isinstance(stat, dict) and stat.get("type") == "xg":
                            ...
                            if len(xg_values) >= 2:
                                return {
                                    "home": float(xg_values[0])
                                    if xg_values[0]
                                    else 0.0,
                                    "away": float(xg_values[1])
                                    if xg_values[1]
                                    else 0.0,
                                }
    ...
    def _parse_score(self, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            score_from_status = self._get_nested_value_robust(
                data, ["header", "status", "score"]
            )
            if (
                score_from_status
                and isinstance(score_from_status, str)
                and "-" in score_from_status
            ):
                ...
                if (
                    ...
                    self.logger.debug(
                        f"Found score string from status: {cleaned_score} (original: {score_from_status})"
                    )
                    return cleaned_score

            # 方法2: 从 teams 数组提取比分
            home_score = self._get_nested_value_robust(
                data, ["header", "teams", 0, "score"]
            )
            away_score = self._get_nested_value_robust(
            ...
            for path in backup_paths:
                ...
                if (
                    score_candidate
                    and isinstance(score_candidate, str)
                    and "-" in score_candidate
                ):
                    # 处理带空格的比分字符串，例如 "2 - 2" -> "2-2"
                    ...
                    if (
                        ...
                        self.logger.debug(
                            f"Found score from backup path {path}: {cleaned_score} (original: {score_candidate})"
                        )
                        return cleaned_score
