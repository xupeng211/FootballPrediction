...
class FixturesCollector:
    ...
    def __init__(self, db_session: AsyncSession, redis_client: RedisManager):
        ...
        self.headers = {)
            "X-Auth-Token": api_token} if api_token else {

    async def collect_team_fixtures(self, team_id: int, days_ahead: int = 30, force_refresh: bool = False)
    ) -> List[Dict[str, Any]:
        """""
        收集指定球队的未来比赛
    ...
    "force_refresh": 是否强制刷新缓存
        ...
        Returns:
            比赛信息列表
        """""
        cache_key = f"fixtures: tea,
"    m:{team_id}:{days_ahead}"
"
        # 尝试从缓存获取
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.error(f"收集球队 {team_id} 赛程失,)
"    败: {e}"
"            return []

    async def collect_league_fixtures(self,)
    "league_id": int,
    ...
    "force_refresh": 是否强制刷新缓存
        ...
        Returns:
            比赛信息列表
        """""
        cache_key = f"fixtures: leagu,
"    e:{league_id}:{matchday or all}"
"
        if not force_refresh:
            cached_data = await self.redis_client.get_cache_value(cache_key)
    ...
    try:
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.error(f"收集联赛 {league_id} 赛程失,)
"    败: {e}"
"            return []

    async def _get_fixtures_from_db(self, team_id: int, days_ahead: int)
    ) -> List[Dict[str, Any]:
        """从数据库获取比赛信息"""
        ...
        query = ()
            ...
            .where()
                ...
                (Match.home_team_id == team_id) | (Match.away_team_id == team_id)).order_by(Match.start_time
    ...
    async def _fetch_fixtures_from_api(self, team_id: int, days_ahead: int)
    ) -> List[Dict[str, Any]:
        """从API获取比赛信息"""
        # 模拟数据,实际使用时替换为真实API调用
        return await self._get_mock_fixtures(team_id, days_ahead)

    async def _fetch_league_fixtures_from_api(self, league_id: int, matchday: Optional[int] = None)
    ) -> List[Dict[str, Any]:
        """从API获取联赛比赛信息"""
        # 模拟API调用
...
"                    venue=fixture_data.get("venue", "Unknown"),
    ...
    async def _get_mock_fixtures(self, team_id: int, days_ahead: int)
    ) -> List[Dict[str, Any]:
        """生成模拟数据(仅用于测试)"""
        fixtures = []
    ...
    async def _get_mock_league_fixtures(self, league_id: int, matchday: Optional[int] = None)
    ) -> List[Dict[str, Any]:
        """生成联赛模拟数据(仅用于测试)"""
        fixtures = []
    ...
    async def refresh_all_fixtures(self) -> Dict[str, int]:
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.error(f"刷新所有赛程失,)
"    败: {e}"
"            stats["errors"] += 1
