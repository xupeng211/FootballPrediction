...
class L2Fetcher:
    ...
    def __init__(
        ...
        """
        self.config = config or L2FetchConfig(timeout=timeout, max_retries=max_retries)
    ...
    async def _ensure_client(self) -> httpx.AsyncClient:
        ...
        if self._client is None or self._client.is_closed:
            ...
            self.logger.debug(
                "HTTP client initialized with timeout=%.1fs", self.config.timeout
            )
    ...
    async def _execute_request_with_stats(
        ...
        try:
            ...
            raise L2FetchError(status_code=e.response.status_code, message=error_msg)

        except httpx.RequestError as e:
            self._stats["failed_requests"] += 1
            raise L2FetchError(status_code=-1, message=f"Request error: {str(e)}")

        except Exception as e:
            self._stats["failed_requests"] += 1
            raise L2FetchError(status_code=-2, message=f"Unexpected error: {str(e)}")
    ...
    async def _fetch_with_compression(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise L2FetchError(status_code=-3, message=f"JSON parsing error: {str(e)}")
    ...
    async def _fetch_without_compression(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise L2FetchError(
                status_code=-4, message=f"JSON parsing error (uncompressed): {str(e)}"
            )
    ...
    async def fetch_match_details(self, match_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(
                "Unexpected error fetching match details for %s: %s", match_id, e
            )
            raise L2FetchError(
                status_code=-5, message=f"Unexpected error: {str(e)}", match_id=match_id
            )
