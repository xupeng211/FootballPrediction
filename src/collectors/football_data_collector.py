...
class FootballDataCollector(BaseCollector):
    ...
    async def collect_players(
        ...
        try:
            ...
            if result.success and result.data:
                ...
                logger.info(f"Successfully collected {len(squad)} players for team {team_id}")
    ...
    async def collect_leagues(self, areas: Optional[List[int]] = None) -> CollectionResult:
        """采集联赛数据"""
        try:
            ...
            if result.success and result.data:
                ...
                logger.info(f"Successfully collected {len(supported_competitions)} competitions")
    ...
    async def collect_standings(self, league_id: int) -> CollectionResult:
        ...
        try:
            ...
            return CollectionResult(
                success=False,
                error=f"Failed to collect standings: {str(e)}"
            )
        ...
        try:
            ...
            for competition in target_competitions:
                ...
                try:
                    ...
                    comp_data['teams'] = teams_result.data if teams_result.success else None
                    ...
                    comp_data['matches_finished'] = matches_finished_result.data if matches_finished_result.success else None
                    comprehensive_data['stats']['total_requests'] += 1
                    ...
                    matches_scheduled_result = await self.collect_matches(
                        comp_id, date_from=datetime.now(), date_to=date_to, status='SCHEDULED'
                    )
                    comp_data['matches_scheduled'] = matches_scheduled_result.data if matches_scheduled_result.success else None
                    comprehensive_data['stats']['total_requests'] += 1
                    ...
                    comp_data['standings'] = standings_result.data if standings_result.success else None
                    comprehensive_data['stats']['total_requests'] += 1
                    ...
                    error_msg = f"Failed to collect comprehensive data for {comp_code}: {e}"
            ...
            return CollectionResult(
                success=False,
                error=f"Failed to collect comprehensive data: {str(e)}"
            )
