...
class ExternalLeague(Base):
    ...
    external_id = Column(String(50), unique=True, nullable=False, index=True, comment='外部API的联赛ID')
    ...
    def season_progress(self) -> Optional[float]:
        """赛季进度百分比"""
        if not self.current_season_start or not self.current_season_end or not self.current_matchday:
            return None
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'current_season_start': self.current_season_start.isoformat() if self.current_season_start else None,
            'current_season_end': self.current_season_end.isoformat() if self.current_season_end else None,
    ...
    @classmethod
    def from_api_data(cls, data: Dict[str, Any], is_supported: bool = False) -> 'ExternalLeague':
        """从API数据创建实例"""
        try:
            # 解析地区信息
            area = data.get('area', {})
            ...
            if season:
                ...
                if start_date_str:
                    try:
                        current_season_start = datetime.fromisoformat(start_date_str.replace('Z', '+00:00'))
                    except ValueError:
                        pass
                if end_date_str:
                    try:
                        current_season_end = datetime.fromisoformat(end_date_str.replace('Z', '+00:00'))
                    except ValueError:
                        pass
    ...
    def update_from_api_data(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if season:
                ...
                self.current_matchday = season.get('currentMatchday', self.current_matchday)
                ...
                if start_date_str:
                    try:
                        self.current_season_start = datetime.fromisoformat(start_date_str.replace('Z', '+00:00'))
                    except ValueError:
                        pass
                if end_date_str:
                    try:
                        self.current_season_end = datetime.fromisoformat(end_date_str.replace('Z', '+00:00'))
                    except ValueError:
                        pass
    ...
    def update_statistics(self, total_teams: int = None, total_matches: int = None) -> None:
        """更新统计信息"""
...
class ExternalLeagueStandings(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    league_id = Column(Integer, ForeignKey('external_leagues.id'), nullable=False, comment='联赛ID')
    ...
    @classmethod
    def from_api_data(cls, data: Dict[str, Any], league_id: int, external_league_id: str) -> 'ExternalLeagueStandings':
        """从API数据创建实例"""
        try:
            team = data.get('team', {})
