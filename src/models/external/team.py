...
class ExternalTeam(Base):
    ...
    external_id = Column(String(50), unique=True, nullable=False, index=True, comment='外部API的球队ID')

    # 球队基本信息
    name = Column(String(100), nullable=False, comment='球队名称')
    short_name = Column(String(50), nullable=True, comment='球队简称')
    tla = Column(String(3), nullable=True, comment='三字母缩写')
    crest = Column(Text, nullable=True, comment='队徽URL')
    ...
    club_colors = Column(String(100), nullable=True, comment='俱乐部颜色')
    venue = Column(String(100), nullable=True, comment='主场球场')
    ...
    # 联赛关联信息
    competition_id = Column(Integer, nullable=True, comment='所属联赛ID')
    competition_name = Column(String(100), nullable=True, comment='所属联赛名称')
    competition_code = Column(String(10), nullable=True, comment='所属联赛代码')
    ...
    def _calculate_data_quality_score(data: Dict[str, Any]) -> int:
        ...
        # 详细信息 (30分)
        detail_fields = ['address', 'website', 'founded', 'clubColors', 'venue']
    ...
    def update_from_api_data(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.short_name = data.get('shortName', self.short_name)
            self.tla = data.get('tla', self.tla)
            self.crest = data.get('crest', self.crest)

            # 更新详细信息
            self.address = data.get('address', self.address)
            self.website = data.get('website', self.website)
            self.founded = data.get('founded', self.founded)
            self.club_colors = data.get('clubColors', self.club_colors)
            self.venue = data.get('venue', self.venue)
