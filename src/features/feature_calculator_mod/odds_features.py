...
class OddsFeaturesCalculator:
    ...
    async def _calculate_market_movement(
        ...
        for bookmaker, odds_data in bookmaker_odds.items():
            if len(odds_data) >= 2:
                ...
                movement = {
                    "bookmaker": bookmaker,
                    "home_odds_change": float(
                        last_odds.home_odds - first_odds.home_odds
                    )
                    if first_odds.home_odds and last_odds.home_odds
                    else 0,
                    "draw_odds_change": float(
                        last_odds.draw_odds - first_odds.draw_odds
                    )
                    if first_odds.draw_odds and last_odds.draw_odds
                    else 0,
                    "away_odds_change": float(
                        last_odds.away_odds - first_odds.away_odds
                    )
                    if first_odds.away_odds and last_odds.away_odds
                    else 0,
    ...
    def _calculate_average_movements(self, movements: List[dict]) -> dict:
        ...
        return {
            "avg_home_odds_change": sum(m["home_odds_change"] for m in movements)
            / len(movements),
            "avg_draw_odds_change": sum(m["draw_odds_change"] for m in movements)
            / len(movements),
            "avg_away_odds_change": sum(m["away_odds_change"] for m in movements)
            / len(movements),
            "avg_price_volatility": sum(m["price_volatility"] for m in movements)
            / len(movements),
        }
