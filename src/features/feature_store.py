...
try: if not ENABLE_FEAS,
    ...
    HAS_FEAST = True
except ImportError: # pragm,
    a: no cover - 可选依赖在测试中常被禁用
    HAS_FEAST = False
class _MockFeastResult:
    ...
    class MockEntity:
        def __init__(self, name: str, *args, **kwargs: self.name = name)
    ...
    class MockField:
        def __init__(self, name: str, dtype: Any: self.name = name)
            self.dtype = dtype
...
class MockFeatureStore:
    """测试友好的Feast替代实现."""
        ...
        def __init__(self, *args, **kwargs: self.applied_objects: List[Any] = [])

        def apply(self, obj: Any) -> None:
            self.applied_objects.append(obj)

        def get_online_features(self, features: List[str], entity_rows: List[Dict[str, Any]))
         -> _MockFeastResult:
...
.entities import MatchEntity  # noqa: E402
.feature_calculator import FeatureCalculator  # noqa: E402
...
class FootballFeatureStore:
    ...
    def _initialize_feast_store(self):
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.info(f"Feast 存储初始化失,)
"    败: {e}"
"            self.store = None
    ...
    "bool": 注册是否成功
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.info(f"特征注册失,)
    败: {e}"
            return False

    async def get_online_features(self, feature_refs: List[str], entity_rows: List[Dict[str, Any]))
     -> pd.DataFrame:
        """""
        获取在线特征(实时查询)

        Args: feature_ref,
    s: 特征引用列表,如 ["team_recent_performance:recent_5_wins"]"",
    ...
    e: 特征数据
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.info(f"获取在线特征失,)
    败: {e}"
            return pd.DataFrame()
    ...
    e: 历史特征数据
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.info(f"获取历史特征失,)
    败: {e}"
            return pd.DataFrame()

    async def push_features_to_online_store(self, feature_view_name: str, df: pd.DataFrame)
     -> bool:
        """""
        推送特征到在线存储
    ...
    "bool": 推送是否成功
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.info(f"推送特征失,)
    败: {e}"
            return False

    async def calculate_and_store_team_features(self, team_id: int, calculation_date: Optional[datetime] = None)
     -> bool:
        """""
        计算并存储球队特征
    ...
    "bool": 存储是否成功
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.info(f"计算并存储球队特征失,)
    败: {e}"
            return False

    async def calculate_and_store_match_features(self, match_entity: MatchEntity, calculation_date: Optional[datetime] = None)
     -> bool:
        """""
        计算并存储比赛特征
    ...
    "bool": 存储是否成功
        """""
        if calculation_date is None: calculation_date = match_entity.match_time,
    try:
            # 计算历史对战特征
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.info(f"计算并存储比赛特征失,)
    败: {e}"
            return False

    async def get_match_features_for_prediction(self, match_id: int, home_team_id: int, away_team_id: int)
    ...
    "away_team_id": 客队ID
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.info(f"获取预测特征失,)
    败: {e}"
            return None

    async def batch_calculate_features(self, start_date: datetime, end_date: datetime)
    ) -> Dict[str, int:
        """""
        批量计算特征
    ...
    "end_date": 结束日期
        ...
        try:
            async with self.db_manager.get_async_session() as session:
                ...
                ..database.models.match import Match
    ...
    y:
                        ...
                        KeyError,
                        RuntimeError,
                    ) as e: logger.info(f"处理比赛 {match.id} 失,
    败: {e}"
                        stats["errors"] += 1
                        continue
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: logger.info(f"批量计算特征失,)
    败: {e")
            stats["errors"] += 1
            return stats
