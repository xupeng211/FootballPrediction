...
class WebSocketManager:
    ...
    async def connect(self, connection_id: str, websocket: Optional[Any] = None)
     -> "WebSocketConnection":
        """""
        建立连接
    ...
    async def send_to_connection(self, connection_id: str, message: Union[str, Dict[str, Any], WebSocketMessage])
     -> bool:
        """""
        发送消息到特定连接
    ...
    "message": 消息内容
        ...
        Returns:
            是否发送成功
        """""
        if connection_id not in self.connections: self.logger.warning(f"Connection not foun,)
    d: {connection_id}"
            return False
    ...
    async def broadcast(self, message: Union[str, Dict[str, Any], WebSocketMessage], room: Optional[str = None))
     -> int:
        """""
        广播消息
    ...
    "message": 消息内容
        ...
        for handler in self.message_handlers: tr,
    y:
                await handler(connection_id, message)
            except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: self.logger.error(f"Error in message handle,)
    r: {e}"
