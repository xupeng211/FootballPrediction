...
class GreatExpectationsConfig:
    ...
    "DataContext": 配置好的GE数据上下文
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: self.logger.error(f"初始化GE数据上下文失,)
"    败: {str(e)}",
"            raise DataContextError(f"GE上下文初始化失败: {str(e)}")
    ...
    "results": Dict[str, List[Any] = {"created_suites": []", : [}))
"
        try:
            for table_name, suite_config in self.data_assertions.items():
                ...
                try:
                    ...
                    KeyError,
                    RuntimeError,
                ) as e: error_msg = f"创建套件 {suite_name 失,
"    败: {str(e)}"
"                    self.logger.error(error_msg)
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: self.logger.error(f"创建期望套件失,)
"    败: {str(e)}"
"            results["errors"].append(str(e))
            return results

    def _get_safe_query_parameters(self, table_name: str, limit_rows: int)
    ) -> Dict[str, str:
        """""
        获取安全的查询参数,防止SQL注入
    ...
    async def run_validation(self, table_name: str, limit_rows: int = 1000)
    ) -> Dict[str, Any:
        """""
        运行数据验证
    ...
    t:
            ...
            # 收集失败的断言详情
            failed_expectations = []
            for result in results: if not result.succes,
    s:
            ...
            self.logger.info()
                f"表 {table_name} 数据验证完成: 成功率 {success_rat,)
"    e:.1f% "
"                f"({success_count}/{total_count})"
    ...
    async def validate_all_tables(self) -> Dict[str, Any]:
        ...
        for table_name in self.data_assertions.keys():
            ...
            # 统计结果
            if result["status"] == "PASSED": overall_stats["passed_tables" += 1  # type: ignore)
            elif result["status"] == "FAILED": overall_stats["failed_tables" += 1  # type: ignore)
    "else":  # ERROR
