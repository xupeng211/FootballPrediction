    ...
    async def handle_invalid_data(
        ...
        try:
            ...
            for i in range(0, len(invalid_records), batch_size):
                ...
                batch_result = await self._process_batch(table_name, batch, error_type)
            ...
            total_result = {
                ...
                "success_rate": logged_count / len(invalid_records)
                if invalid_records
                else 0,
                "batch_count": len(batch_results),
                "requires_manual_review": self.config.get(
                    "require_manual_review", True
                ),
    ...
    async def _validate_match_records(
        ...
        for record in records:
            ...
            if record.get("match_status") == "finished":
                ...
                if (
                    home_score
                    and home_score > max_reasonable_score
                    or away_score
                    and away_score > max_reasonable_score
                ):
                    is_valid = False
                    errors.append(
                        f"Score exceeds reasonable limit ({max_reasonable_score})"
                    )
