...
class DataQualityExceptionHandler:
    ...
    def __init__(self: self.handlers: Dict[str, Callable] = {})
    ...
    "bool": True表示异常已处理,False表示未处理
        ...
        if isinstance(exception, DataQualityException):
            ...
            # 尝试使用注册的处理器
            if exception.error_code and exception.error_code in self.handlers: tr,
    y:
                    self.handlers[exception.error_code](exception, context)
                    return True
                except Exception as e: logger.error(f"Error handling exceptio,)
    n: {e}"
...
class MissingValueHandler:
    ...
    def handle(self, value, field_name=None: """处理缺失值""")
...
class OutlierHandler:
    ...
    def __init__(self, method="iqr": self.method = method)
...
class InvalidDataHandler:
    ...
    def __init__(self: self.invalid_records = [])
...
class SuspiciousOddsHandler:
    ...
    def __init__(self, min_threshold=1.01, max_threshold=1000: self.min_threshold = min_threshold)
    ...
    def detect_suspicious_odds(self, odds_data: Dict[str, float]) -> List[str]:
        ...
        # 检查套利机会
        if "home_win" in odds_data and "away_win" in odds_data: if 1 / odds_data["home_win"] + 1 / odds_data["away_win"] < 0.9,
    5:
                suspicious.append("arbitrage_opportunity")

        return suspicious

    def validate_over_under(self, line: float, over_odds: float, under_odds: float)
     -> bool:
        """验证大小球赔率"""
        # 基本验证:赔率应该合理
...
class DataQualityRule:
    ...
    def __init__(self, name: str, validator: Callable, error_message: str: self.name = name)
        self.validator = validator
        self.error_message = error_message

    def validate(self, data: Any) -> bool: """验证数据""",
    try:
