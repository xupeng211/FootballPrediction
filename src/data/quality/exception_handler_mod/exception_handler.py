...
class DataQualityExceptionHandler:
    ...
    async def get_quality_dashboard(
        ...
        try:
            ...
            dashboard = {
                "table_name": table_name,
                "last_24h_stats": results[0] if not isinstance(results[0], Exception) else {},
                "daily_trend": results[1] if not isinstance(results[1], Exception) else [],
                "top_issues": results[2] if not isinstance(results[2], Exception) else [],
                "quality_score": results[3] if not isinstance(results[3], Exception) else {},
                "pending_reviews": results[4] if not isinstance(results[4], Exception) else [],
                "timestamp": datetime.now().isoformat(),
            }
    ...
    async def process_quality_report(
        ...
        try:
            ...
            raise DataQualityException(
                f"生成质量报告失败: {str(e)}",
                error_code="QUALITY_REPORT_ERROR"
            )
