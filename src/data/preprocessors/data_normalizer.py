...
class DataFormatNormalizer:
    ...
    STANDARD_FIELDS = [
        ...
        # 主队统计
        'home_expected_goals', 'home_shots_on_target', 'home_possession',
        'home_shots', 'home_corners',

        # 客队统计
        'away_expected_goals', 'away_shots_on_target', 'away_possession',
        'away_shots', 'away_corners',

        # 合并统计（用于向后兼容）
        'expected_goals', 'shots_on_target', 'possession',
        'shots', 'corners',
    ]
    ...
    def detect_format(self, raw_data: Dict[str, Any]) -> Optional[str]:
        """检测数据格式类型
    ...
    def normalize(self, raw_data: Dict[str, Any], format_hint: Optional[str] = None) -> Dict[str, Any]:
        """标准化数据格式
    ...
    def _normalize_stats_format(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        normalized['home_shots'] = self._extract_field(
            home_stats, ['totalShots', 'shots']
        )
        normalized['home_corners'] = self._extract_field(
            home_stats, ['corners']
        )
        ...
        normalized['away_shots'] = self._extract_field(
            away_stats, ['totalShots', 'shots']
        )
        normalized['away_corners'] = self._extract_field(
            away_stats, ['corners']
        )
    ...
    def _extract_field(self, data: Dict[str, Any], possible_names: list) -> Optional[float]:
        """从字典中提取字段（尝试多个可能的名称）
    ...
    def normalize_dataframe(self, df: pd.DataFrame, format_hint: Optional[str] = None) -> pd.DataFrame:
        """标准化 DataFrame 格式
...
def detect_data_format(raw_data: Dict[str, Any]) -> Optional[str]:
    """检测数据格式（便捷函数）
