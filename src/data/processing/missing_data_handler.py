...
class MissingDataHandler:
    ...
    def impute_missing_data(self, data: pd.DataFrame, strategy: str = 'adaptive') -> pd.DataFrame:
        """缺失数据插补
    ...
    def handle_columns_with_excessive_missing(self, data: pd.DataFrame, threshold: float = 0.5) -> pd.DataFrame:
        """处理缺失值过多的列
    ...
    def validate_imputation_quality(self, original_data: pd.DataFrame, imputed_data: pd.DataFrame) -> Dict[str, Any]:
        """验证插补质量
        ...
        if total_original_missing > 0:
            validation['imputation_success_rate'] = (
                (total_original_missing - total_remaining_missing) / total_original_missing
            ) * 100
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        missing_ratio = self.missing_patterns['total_missing'] / (
            sum(len(pattern.split()) for pattern in self.missing_patterns.get('missing_patterns', {}).keys())
            if self.missing_patterns.get('missing_patterns') else 1
        )
        ...
        extensive_missing = [
            col for col, info in self.missing_patterns.get('missing_by_column', {}).items()
...
# 便捷函数
def handle_missing_data(data: pd.DataFrame, strategy: str = 'adaptive',
