...
class FootballDataCleaner:
    ...
    async def clean_odds_data(
        ...
        """
        return await self.odds_processor.clean_odds_data(raw_odds)  # type: ignore

    # 向后兼容的方法
    def _validate_match_data(self, raw_data: Dict[str, Any]) -> bool:
        """验证比赛数据的基础字段（向后兼容）"""
        return self.data_validator.validate_match_data(raw_data)  # type: ignore

    def _validate_odds_data(self, raw_odds: Dict[str, Any]) -> bool:
        """验证赔率数据的基础字段（向后兼容）"""
        return self.data_validator.validate_odds_data(raw_odds)  # type: ignore

    def _to_utc_time(self, time_str: Optional[str]) -> Optional[str]:
        """时间统一转换为UTC（向后兼容）"""
        return self.time_processor.to_utc_time(time_str)  # type: ignore

    async def _map_team_id(
        self, team_data: Dict[str, Any], *, league_id: Optional[int] = None
    ) -> Optional[int]:
        """球队ID映射到标准ID（向后兼容）"""
        return await self.id_mapper.map_team_id(team_data, league_id=league_id)  # type: ignore

    async def _map_league_id(self, league_data: Dict[str, Any]) -> Optional[int]:
        """联赛ID映射到标准ID（向后兼容）"""
        return await self.id_mapper.map_league_id(league_data)  # type: ignore

    def _deterministic_id(self, value: str, *, modulus: int) -> int:
        """基于稳定哈希生成确定性ID（向后兼容）"""
        return self.id_mapper._deterministic_id(value, modulus=modulus)  # type: ignore

    def _standardize_match_status(self, status: Optional[str]) -> str:
        """标准化比赛状态（向后兼容）"""
        return self.data_validator.standardize_match_status(status)  # type: ignore

    def _validate_score(self, score: Any) -> Optional[int]:
        """验证比分数据（向后兼容）"""
        return self.data_validator.validate_score(score)  # type: ignore

    def _extract_season(self, season_data: Dict[str, Any]) -> Optional[str]:
        """提取赛季信息（向后兼容）"""
        return self.time_processor.extract_season(season_data)  # type: ignore

    def _clean_venue_name(self, venue: Optional[str]) -> Optional[str]:
        """清洗场地名称（向后兼容）"""
        return self.data_validator.clean_venue_name(venue)  # type: ignore

    def _clean_referee_name(
        self, referees: Optional[List[Dict[str, Any]]]
    ) -> Optional[str]:
        """清洗裁判姓名（向后兼容）"""
        return self.data_validator.clean_referee_name(referees)  # type: ignore

    def _validate_odds_value(self, price: Any) -> bool:
        """验证赔率值（向后兼容）"""
        return self.odds_processor.validate_odds_value(price)  # type: ignore

    def _standardize_outcome_name(self, name: Optional[str]) -> str:
        """标准化结果名称（向后兼容）"""
        return self.odds_processor.standardize_outcome_name(name)  # type: ignore

    def _standardize_bookmaker_name(self, bookmaker: Optional[str]) -> str:
        """标准化博彩公司名称（向后兼容）"""
        return self.odds_processor.standardize_bookmaker_name(bookmaker)  # type: ignore

    def _standardize_market_type(self, market_type: Optional[str]) -> str:
        """标准化市场类型（向后兼容）"""
        return self.odds_processor.standardize_market_type(market_type)  # type: ignore

    def _validate_odds_consistency(self, outcomes: List[Dict[str, Any]]) -> bool:
        """验证赔率合理性（向后兼容）"""
        return self.odds_processor.validate_odds_consistency(outcomes)  # type: ignore

    def _calculate_implied_probabilities(
        self, outcomes: List[Dict[str, Any]]
    ) -> Dict[str, float]:
        """计算隐含概率（向后兼容）"""
        return self.odds_processor.calculate_implied_probabilities(outcomes)  # type: ignore
