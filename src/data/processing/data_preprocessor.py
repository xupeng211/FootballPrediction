...
class DataPreprocessor:
    ...
    def preprocess_dataset(self, raw_data: pd.DataFrame, data_type: str = 'matches') -> Dict[str, Any]:
        """
        预处理完整数据集
        ...
        try:
            ...
            processing_result['reports']['cleaning'] = self.cleaner.get_cleaning_report()
            ...
            if self.config['cleaning']['handle_missing']:
                ...
                # 分析缺失模式
                missing_analysis = self.missing_handler.analyze_missing_patterns(cleaned_data)
                processing_result['reports']['missing_analysis'] = missing_analysis

                # 处理缺失值过多的列
                drop_threshold = self.config['missing_data']['drop_threshold']
                data_after_dropping = self.missing_handler.handle_columns_with_excessive_missing(
                ...
                # 验证插补质量
                imputation_validation = self.missing_handler.validate_imputation_quality(
                    cleaned_data, final_data
                )
                processing_result['reports']['imputation_validation'] = imputation_validation
                processing_result['reports']['missing_report'] = self.missing_handler.get_missing_data_report()
            else:
    ...
    def _assess_data_quality(self, original_data: pd.DataFrame, processed_data: pd.DataFrame,
        ...
        if completeness_score < 0.95:
            assessment['quality_issues'].append(f"数据完整性较低: {completeness_score:.2%}")

        # 2. 一致性评估
        if data_type == 'matches':
            consistency_issues = self._check_match_consistency(processed_data)
            assessment['consistency_score'] = 1.0 - len(consistency_issues) / 10.0  # 假设最多10个问题
            assessment['quality_issues'].extend(consistency_issues)

        # 3. 有效性评估
        validity_issues = self._check_data_validity(processed_data, data_type)
        assessment['validity_score'] = 1.0 - len(validity_issues) / 20.0  # 假设最多20个问题
        ...
        # 5. 生成改进建议
        assessment['improvements'] = self._generate_improvement_suggestions(assessment, data_type)
    ...
    def _check_match_consistency(self, data: pd.DataFrame) -> List[str]:
        ...
        if 'match_date' in data.columns and 'status' in data.columns:
            future_finished = data[
                (data['match_date'] > pd.Timestamp.now()) & (data['status'] == 'FINISHED')
            ]
    ...
    def _check_data_validity(self, data: pd.DataFrame, data_type: str) -> List[str]:
        ...
        # 检查极端值
        if data_type == 'matches' and 'home_score' in data.columns and 'away_score' in data.columns:
            extreme_scores = data[
                (data['home_score'] > 15) | (data['away_score'] > 15)
            ]
    ...
    def _generate_improvement_suggestions(self, assessment: Dict[str, Any], data_type: str) -> List[str]:
        """生成数据改进建议"""
        suggestions = []
    ...
    def get_processing_summary(self) -> Dict[str, Any]:
        ...
        successful_processed = sum(1 for record in self.processing_history if record['success'])
    ...
    def _calculate_processing_stats(self) -> Dict[str, Any]:
        ...
        for record in self.processing_history:
            ...
            if 'final_shape' in record:
                data_types[data_type]['total_rows_processed'] += record['final_shape'][0]

        return {
            'by_data_type': data_types,
            'average_rows_per_job': sum(
                stat['total_rows_processed'] / stat['count']
                for stat in data_types.values() if stat['count'] > 0
            ) / len(data_types) if data_types else 0
        }


# 便捷函数
def preprocess_football_data(raw_data: pd.DataFrame, data_type: str = 'matches',
