...
class FootballDataCleaner:
    ...
    def clean_dataset(self, raw_data: pd.DataFrame, data_type: str = 'matches') -> pd.DataFrame:
        """清洗完整数据集
    ...
    def _validate_basic_structure(self, data: pd.DataFrame, data_type: str) -> pd.DataFrame:
        """基础数据结构验证"""
        required_columns = {
    ...
    def _remove_duplicates(self, data: pd.DataFrame, data_type: str) -> pd.DataFrame:
        ...
        if data_type == 'matches':
            subset_cols = ['match_id'] if 'match_id' in data.columns else ['home_team_id', 'away_team_id', 'match_date']
        ...
        for column in data.columns:
            ...
            if missing_count > 0:
                ...
                missing_info[column] = {
                    'count': missing_count,
                    'ratio': missing_ratio,
                    'strategy': self._determine_missing_strategy(column, data[column], missing_ratio)
                }

                # 应用相应的处理策略
                data[column] = self._apply_missing_strategy(data[column], missing_info[column]['strategy'])
    ...
    def _determine_missing_strategy(self, column: str, series: pd.Series, missing_ratio: float) -> str:
        """确定缺失值处理策略"""
    ...
    def _detect_and_handle_outliers(self, data: pd.DataFrame, data_type: str) -> pd.DataFrame:
        """检测和处理异常值"""
    ...
    def _handle_outliers(self, data: pd.DataFrame, column: str, outliers: pd.Series) -> pd.DataFrame:
        """处理异常值"""
        try:
        ...
        if data_type == 'matches':
            ...
            if 'home_score' in data.columns and 'away_score' in data.columns:
                invalid_scores = data[
                    ...
                    (data['home_score'] > 20) | (data['away_score'] > 20)
                ]
                if not invalid_scores.empty:
                    validation_errors.append(f"发现 {len(invalid_scores)} 条无效比分记录")

            # 验证时间逻辑
            if 'match_date' in data.columns:
                future_matches = data[data['match_date'] > datetime.now() + timedelta(days=365)]
                if not future_matches.empty:
                    validation_errors.append(f"发现 {len(future_matches)} 条超过1年的未来比赛")
            ...
            if 'home_team_id' in data.columns and 'away_team_id' in data.columns:
                ...
                if not same_team_matches.empty:
                    validation_errors.append(f"发现 {len(same_team_matches)} 条主队相同比赛")
            ...
            for col in odds_columns:
                if col in data.columns:
                    ...
                    if not invalid_odds.empty:
                        validation_errors.append(f"发现 {len(invalid_odds)} 条无效赔率记录 ({col})")
    ...
    def _generate_cleaning_report(self, raw_data: pd.DataFrame, cleaned_data: pd.DataFrame, steps: List[str]):
        """生成清洗报告"""
        self.data_quality_report.update({
            'original_shape': raw_data.shape,
            'cleaned_shape': cleaned_data.shape,
            'cleaning_steps': steps,
            'data_reduction_ratio': 1 - (len(cleaned_data) / len(raw_data)) if len(raw_data) > 0 else 0,
...
# 便捷函数
def clean_football_data(raw_data: pd.DataFrame, data_type: str = 'matches', config: Optional[Dict] = None) -> pd.DataFrame:
    """
    便捷的足球数据清洗函数
