...
class ScoresCollector(BaseCollector):
    ...
    async def collect_live_scores(self, match_ids: list[int]) -> CollectionResult:
        ...
        try:
            if not match_ids:
                return CollectionResult(success=False, error="No match IDs provided")
            ...
            return CollectionResult(
                success=True, data=all_scores, count=collected_count, cached=False
            )
    ...
    def _process_match_statistics(self, raw_data: dict) -> dict:
        ...
        processed_stats = {"match_id": match.get("id"), "teams": {}}
        ...
        while True:
            try:
                ...
                if score_result.success:
                    ...
                    logger.warning(
                        f"无法获取比赛 {match_id} 的数据: {score_result.error}"
                    )
