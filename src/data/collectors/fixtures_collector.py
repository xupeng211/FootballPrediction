...
class FixturesCollector(DataCollector):
    ...
    y:
                    ...
                    # 处理每场比赛
                    for fixture_data in league_fixtures: tr,
    y:
                            # 防重复检查
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: self.logger.error(f"Fixtures collection faile,)
"    d: {str(e)}"
...
"            
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: self.logger.error(f"Failed to get active league,)
"    s: {str(e)}"
"            return ["PL", "PD"]  # 默认返回英超和西甲

    async def _load_existing_matches(self, date_from: datetime, date_to: datetime)
     -> None:
        """""
        加载已存在的比赛ID(防重复机制)
    ...
    "date_to": 结束日期
        ...
        try:
            ...
            #     self._processed_matches = {)
            row.match_id for row in result

        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: self.logger.error(f"Failed to load existing matche,)
    s: {str(e)}"
            self._processed_matches = set()

    async def _collect_league_fixtures(self, league_code: str, date_from: datetime, date_to: datetime)
    ) -> List[Dict[str, Any]:
        """""
        采集指定联赛的赛程数据
    ...
    async def _clean_fixture_data(self, raw_fixture: Dict[str, Any])
    ) -> Optional[Dict[str, Any]:
        """""
        清洗和标准化赛程数据

        Args: "raw_fixture": 原始赛程数据,
    Returns:
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: self.logger.error(f"Failed to clean fixture dat,)
    a: {str(e)}"
            return None

    async def _detect_missing_matches(self,)
    ...
    "date_to": 结束日期
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: self.logger.error(f"Failed to detect missing matche,)
    s: {str(e)")
