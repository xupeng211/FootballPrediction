...
class OddsCollector(BaseCollector):
    ...
    async def collect_match_odds(self, match_id: int) -> CollectionResult:
        ...
        try:
            ...
            return CollectionResult(
                success=False, error=f"Exception occurred: {str(e)}"
            )
    ...
    async def collect_league_odds(
        ...
        try:
            ...
            if result.success and result.data:
                ...
                return CollectionResult(
                    success=False, error=f"Failed to fetch league odds: {result.error}"
                )
    ...
    async def collect_historical_odds(
        self, match_id: int, days_back: int = 30
    ) -> CollectionResult:
        """
        采集历史赔率数据
    ...
    def _process_odds_data(self, raw_data: dict) -> dict:
        ...
        processed = {
            ...
            "match_info": {
                ...
                "competition": raw_data.get("match", {})
                .get("competition", {})
                .get("name"),
    ...
    def _process_bet_type(self, bets: list[dict]) -> dict:
        ...
        processed = []
        for bet in bets:
            bet_info = {"name": bet.get("name"), "id": bet.get("id"), "values": []}
            ...
            if isinstance(bet_data, dict) and "values" in bet_data:
                for value in bet_data.get("values", []):
                    if "price_change" in value:
                        changes[f"{bet_type}_{value.get('value', '')}"] = value[
                            "price_change"
                        ]
    ...
    def _count_bookmakers(self, odds_data: dict) -> int:
        ...
        for _bet_type, bet_data in odds_data.items():
    ...
    async def monitor_odds_changes(
        self, match_ids: list[int], check_interval: int = 300
    ) -> None:
        """
        监控赔率变化
            ...
            try:
                for match_id in match_ids:
                    ...
                    if result.success:
                        ...
                        logger.warning(
                            f"无法获取比赛 {match_id} 的赔率: {result.error}"
                        )
...
async def collect_odds_for_match(match_id: int, api_key: str, base_url: str) -> dict:
    ...
    Returns:
        Dict: 采集结果
    """
    collector = OddsCollector(api_key=api_key, base_url=base_url)
