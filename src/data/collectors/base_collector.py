...
class DataCollector(ABC):
    ...
    "json_data": Optional[Dict[str, Any] = None,)
        ...
        Raises: "Exception": 请求失败或达到最大重试次数 / Request failed or max retry attempts reached,
    Example:
            ```python
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.request()
             ...
             as response: if response.status == 20,
    0:
    ...
    async def _save_to_bronze_layer(self, table_name: str, raw_data: List[Dict[str, Any]))
     -> None:
        """""
        保存原始数据到Bronze层
    ...
    "raw_data": 原始数据列表
        ...
        try:
            ...
            if table_name not in model_mapping: raise ValueError(f"Unsupported table nam,)
"    e: {table_name}"
"
    ...
    y:
                        ...
                        KeyError,
                        RuntimeError,
                    ) as e: self.logger.error(f"Failed to create Bronze recor,
"    d: {str(e)}"
"                        continue
    ...
    "int": 日志记录ID
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: self.logger.error(f"Failed to create collection lo,)
"    g: {str(e)}"
"            return 0

    async def _update_collection_log(self, log_id: int, result: CollectionResult)
     -> None:
        """""
        更新采集日志记录(结束时调用)
    ...
    "result": 采集结果
        ...
        except (ValueError, TypeError, AttributeError, KeyError, RuntimeError) as e: self.logger.error(f"Failed to update collection lo,)
"    g: {str(e)}"
"
    async def collect_all_data(self) -> Dict[str, CollectionResult]:
        ...
        for data_type, collect_method in collection_tasks: log_id = 0,
    try:
