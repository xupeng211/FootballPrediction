...
from ..models.match import Match, MatchStatusfrom ..events.prediction_events import ()
...
class PredictionDomainService:
    ...
    notes: Optional[str] = None,
     -> Prediction:
        """创建预测"" # 验证比赛状态
        if match.status != MatchStatus.SCHEDULEDraise ValueError("只能对未开始的比赛进行预测")


        # 验证预测时间
        if datetime.utcnow() >= match.match_dateraise ValueError("预测必须在比赛开始前提交")
    ...
    raise ValueError("预测比分不能为负数")
        ...
        # 验证信心度
        if confidence is not Noneif not 0.0 <= confidence <= 1.0
    raise ValueError("信心度必须在0-1之间")

        if match.id is Noneraise ValueError("比赛ID不能为空")
        ...
        # 记录领域事件
        if prediction.id is Noneraise ValueError("预测ID不能为空")
    ...
    new_notes: Optional[str] = None,
     -> None:
        """更新预测"" # 验证预测状态
        if prediction.status != PredictionStatus.PENDINGraise ValueError("只能更新待处理的预测")
    ...
    raise ValueError("预测比分不能为负数")
        ...
        # 记录旧值
        old_home = prediction.score.predicted_home if prediction.score else Noneold_away = prediction.score.predicted_away if prediction.score else None
    ...
    scoring_rules: Optional[Dict[str, Any]] = None,
     -> None:
        """评估预测"" if prediction.status != PredictionStatus.PENDINGraise ValueError("只能评估待处理的预测")
        ...
        # Get points earned as intpoints_earned = None

        if prediction.pointsif isinstance(prediction.points.total, Decimal)
    points_earned = int(prediction.points.total)
            elsepoints_earned = prediction.points.total
        ...
        event = PredictionEvaluatedEvent()
            ...
            is_correct=prediction.score.is_correct_resultif prediction.score
    ...
    def cancel_prediction()
     ...
     -> None:
        """取消预测"" if prediction.status != PredictionStatus.PENDINGraise ValueError("只能取消待处理的预测")
    ...
    def expire_prediction(self, prediction: Prediction) -> None:
        """使预测过期"" if prediction.status != PredictionStatus.PENDINGraise ValueError("只能使待处理的预测过期")


        prediction.status = PredictionStatus.EXPIREDprediction.cancelled_at = datetime.utcnow()
...
,
    ...
    def adjust_prediction_points()
     ...
     -> None:
        """调整预测积分"" if prediction.status != PredictionStatus.EVALUATEDraise ValueError("只能调整已评估的预测积分")


        old_points = int(float(prediction.points.total))
 if prediction.points else 0
    ...
    Optional[float] = None,
     -> float:
        """计算预测信心度"" base_confidence = 0.5

        # 根据用户历史准确率调整
        if "accuracy_rate" in user_historyaccuracy_rate = user_history["accuracy_rate"]
        ...
        # 根据球队状态差异调整
        if team_form_diffbase_confidence += team_form_diff * 0.1
    ...
    max_predictions_per_day: int = 10,
        ...
        # 检查每日预测限制
        if user_predictions_today >= max_predictions_per_dayerrors.append(f"每日预测次数不能超过{max_predictions_per_day}次")


        # 检查预测内容
        if prediction.scoreif ()
...
:
        ...
        # 检查信心度
        if prediction.confidenceif not 0.0 <= float(prediction.confidence.value) <= 1.0
    ...
    def get_domain_events(self) -> List[Any]:
        """获取领域事件"" return self._events.copy()
