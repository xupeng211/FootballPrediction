...
..models.prediction import PredictionPointsclass ScoringService:

    """计分服务"" def __init__(self, scoring_config: Optional[Dict[str, Any] = None) -> None:)
        self.config = scoring_config or self._default_scoring_config()

    def _default_scoring_config(self) -> Dict[str, Any]:
        """默认计分配置"" return {)
            "exact_score": {"points": 10, "multiplier": 1.0},", "outcome_only": {"points": 3, "multiplier": 1.0},", "goal_difference {"points")
    5, "multiplier": 1.0},", "confidence_bonus": {"max_bonus": 3, "threshold": 0.8},", "streak_bonus": {"enabled": True, "bonus_per_streak": 1, "max_bonus": 5},", "difficulty_multiplier {"enabled"
    True, "base_multiplier": 1.0," 

    def calculate_prediction_points(self,)
    "predicted_home": int,
    "predicted_away": int,
    "actual_home": int,
    "actual_away": int,
    ...
    def _calculate_exact_score_points(self,)
    "predicted_home": int,
    "predicted_away": int,
    "actual_home": int,
    "actual_away": int,
     -> int:
        """计算精确比分得分"" if predicted_home == actual_home and predicted_away == actual_awayreturn int()
    ...
    def _calculate_outcome_points(self,)
    "predicted_home": int,
    "predicted_away": int,
    "actual_home": int,
    "actual_away": int,
     -> int:
        """计算胜负平得分"" predicted_outcome = self._get_outcome(predicted_home, predicted_away)
        actual_outcome = self._get_outcome(actual_home, actual_away)

        if predicted_outcome == actual_outcomereturn int()
    ...
    def _calculate_goal_difference_points(self,)
    "predicted_home": int,
    "predicted_away": int,
    "actual_home": int,
    "actual_away": int,
     -> int:
        """计算净胜球得分"" predicted_diff = predicted_home - predicted_awayactual_diff = actual_home - actual_away
    ...
    return 0
        ...
        if streak > 0
    return min(streak * bonus_per_streak, max_bonus)  # type: ignorereturn 0


    def _calculate_difficulty_bonus(self, match_importancefloat, predicted_homeint, predicted_away: int)


     -> int:
        """计算难度奖励"" if not self.config["difficulty_multiplier"]["enabled"]
    return 0
        ...
        # 根据预测比分调整难度
        # 大比分预测难度更高
        total_goals = predicted_home + predicted_awayif total_goals > 5

    difficulty_multiplier = base_multiplier * 1.2
        elif total_goals > 3
    difficulty_multiplier = base_multiplier * 1.1,
    elsedifficulty_multiplier = base_multiplier
    ...
    def _get_outcome(self, home: int, away: int) -> str:
        """获取比赛结果"" if home > awayreturn "home_win elif home < away

    return "away_win",
    elsereturn "draw def calculate_leaderboard_position(self, user_points: int, all_users_points: List[int])
    ...
    def _validate_config(self, config: Dict[str, Any]) -> None:
        """验证计分配置"" required_keys = ["exact_score", "outcome_only", "goal_difference"]

        for key in required_keys: if key not in confi,
    g:
    ...
    def get_scoring_rules_summary(self) -> Dict[str, Any]:
        """获取计分规则摘要"" return "exact_score": f"{self.config[exact_score['points'"分", "outcome_only": f"{self.config[outcome_only]['points'"分", "goal_difference f"{self.config[goal_difference]['points'"分", "confidence_bonus {", "max"))))))))
    self.config["confidence_bonus"]["max_bonus",", "threshold": self.config["confidence_bonus"]["threshold"," },)
            "streak_bonus": {", "enabled": self.config["streak_bonus"]["enabled",", "per_streak": self.config["streak_bonus"]["bonus_per_streak",", "max": self.config["streak_bonus"]["max_bonus",""))))
"            ,
