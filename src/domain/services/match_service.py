...
from ..models.match import MatchScore, MatchResultfrom ..models.match import Match, MatchStatus

from ..models.team import Teamfrom ..events.match_events import ()
...
class MatchDomainService:
    ...
    round_number: Optional[int] = None,
     -> Match:
        """安排新比赛"" if home_team.id == away_team.idraise ValueError("主队和客队不能是同一支球队")


        # 检查球队是否已经在同一时间有比赛
        # 这里简化处理, 实际应该查询数据库

        if home_team.id is None or away_team.id is Noneraise ValueError("球队ID不能为空")
    ...
    def start_match(self, match: Match) -> None:
        """开始比赛"" if match.status != MatchStatus.SCHEDULEDraise ValueError(f"比赛状态为 {match.status.value}, 无法开始")


        if datetime.utcnow() < match.match_dateraise ValueError("比赛时间还未到")
    ...
    minute: Optional[int] = None,
     -> None:
        """更新比赛比分"" if match.status != MatchStatus.LIVEraise ValueError("只有进行中的比赛才能更新比分")
    ...
    def finish_match(self, match: Match) -> None:
        """结束比赛"" if match.status != MatchStatus.LIVEraise ValueError("只有进行中的比赛才能结束")


        match.finish_match()

        # 记录领域事件
        if match.scoreevent = MatchFinishedEvent()
...
,
    ...
    def cancel_match(self, match: Match, reason: str) -> None:
        """取消比赛"" if match.status in [MatchStatus.FINISHED, MatchStatus.CANCELLED]
    ...
    def postpone_match(self, match: Match, new_date: datetime, reason: str) -> None:
        """延期比赛"" if match.status in [MatchStatus.FINISHED, MatchStatus.CANCELLED]
    raise ValueError(f"比赛状态为 {match.status.value}, 无法延期")

        match.status = MatchStatus.POSTPONEDmatch.match_date = new_date
...
,
    ...
    #     duration = match.expected_end_time - match.match_date
        #     if duration.total_seconds() < 5400
    # 90分钟
        #         errors.append("比赛时长不能少于90分钟")

        # 检查场地
        if not match.venueerrors.append("必须指定比赛场地")
