...
# 便捷工厂函数
def create_match_created_data(match_id: int, home_team: str, away_team: str, league: str, scheduled_start: datetime, **kwargs) -> MatchCreatedEventData:
    """创建比赛创建事件数据"""
...
def create_prediction_made_data(prediction_id: str, user_id: int, match_id: int, predicted_outcome: str, confidence: float, stake: float, odds: float) -> PredictionMadeEventData:
    """创建预测事件数据"""
...
def create_system_error_data(error_id: str, error_type: str, error_message: str, stack_trace: str, **kwargs) -> SystemErrorEventData:
    """创建系统错误事件数据"""
