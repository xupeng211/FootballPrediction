...
:
    """集成方法枚举"" WEIGHTED_AVERAGE = "weighted_average"  # 加权平均
...
@dataclassclass StrategyWeight:

    """策略权重配置"" "strategy_name": str
    "base_weight": float
    "dynamic_weight": Optional[float] = None
    "performance_weight": Optional[float] = None
    "recent_accuracy": Optional[float] = None
@dataclassclass EnsembleResult:

    """集成结果"" "final_prediction": Tuple[int, int]
    "confidence": float
...
:
    """集成预测策略"" 组合多个子策略的预测结果,通过智能加权提高整体准确性.
...
"        super()
.__init__(name, StrategyType.ENSEMBLE)
        self._sub_strategies: Dict[str, PredictionStrategy] = {}
        self._strategy_weights: Dict[str, StrategyWeight] = {}
        self._ensemble_method = EnsembleMethod.WEIGHTED_AVERAGEself._performance_history: Dict[str, List[float]] = {}

        self._consensus_threshold = 0.7
        self._max_disagreement = 2.0

    async def initialize(self, config: Dict[str, Any]) -> None: """初始化集成策略""",
    Args:
    "config": 配置参数,包含: - sub_strategie,
    s: 子策略配置列表
                - ensemble_method: 集成方法
                - strategy_weights: 策略权重配置
                - consensus_threshold: 共识阈值
                - performance_window: 性能评估窗口
        """" self.config = configself._ensemble_method = EnsembleMethod()
        ...
        self._is_initialized = Trueprint()

            f"集成策略 {self.name] 初始化成功,包含 {len(self._sub_strategies))
 个子策略 

    async def _initialize_sub_strategies(self, strategies_config: List[Dict[str, Any]))
     -> None:
        """初始化子策略"" .ml_model import MLModelStrategy
        .statistical import StatisticalStrategy
        .historical import HistoricalStrategy

        strategy_classes = {)
            "ml_model": MLModelStrategy,", "statistical": StatisticalStrategy,", "historical": HistoricalStrategy," 
"
        for strategy_config in strategies_config:
            ...
            if not strategy_enabledcontinue
    ...
    async def _initialize_strategy_weights(self, weights_config: Dict[str, float])
     -> None:
        """初始化策略权重"" # 默认权重分配
...
:
    ...
    async def predict(self, input_data: PredictionInput) -> PredictionOutput:
        """执行集成预测"" if not self._is_initializedraise RuntimeError("策略未初始化")
    ...
    raise ValueError("输入数据无效")
        ...
        # 根据集成方法组合结果
        if self._ensemble_method == EnsembleMethod.WEIGHTED_AVERAGEensemble_result = await self._weighted_average_ensemble()

                input_data, strategy_predictions
            
        elif self._ensemble_method == EnsembleMethod.MAJORITY_VOTINGensemble_result = await self._majority_voting_ensemble()

                input_data, strategy_predictions
            
        elif self._ensemble_method == EnsembleMethod.DYNAMIC_WEIGHTINGensemble_result = await self._dynamic_weighting_ensemble()
    ...
    # 默认使用加权平均
        ...
        output = PredictionOutput()
            ...
            metadata={"method": f"ensemble_{self._ensemble_method.value",))
        "strategies_used": list(strategy_predictions.keys())
),", "consensus_score": ensemble_result.consensus_score,", "disagreement_level": ensemble_result.disagreement_level,", "strategy_predictions": {" "name": {)
                        "prediction": (" pred.predicted_home_score,)
...
"                    for name, pred in strategy_predictions.items()
    ...
    async def _execute_all_strategies(self, input_data: PredictionInput)
...
:
    ...
    logger.info(f"策略 {name] 预测失败: {result]")
            elsepredictions[name] = result


        return predictions  # type: ignore

    async def _weighted_average_ensemble(self,)
        "input_data": PredictionInput,
    "strategy_predictions": Dict[str, PredictionOutput],
     -> EnsembleResult:
        """加权平均集成"" total_home = 0.0
        total_away = 0.0
        total_weight = 0.0
...
:
            ...
            total_home += prediction.predicted_home_score * confidence_adjusted_weighttotal_away += prediction.predicted_away_score * confidence_adjusted_weight

            total_weight += confidence_adjusted_weight

            strategy_contributions[name] = {)
                "weight": confidence_adjusted_weight,", "contribution": (" prediction.predicted_home_score * confidence_adjusted_weight,)
...
,
        ...
        # 计算加权平均
        if total_weight > 0
    final_home = total_home / total_weightfinal_away = total_away / total_weight

        elsefinal_home, final_away = 1, 1
        ...
        return EnsembleResult()
            final_prediction=(int(round(final_home)))
), int(round(final_away)

,
            confidence=np.mean([p.confidence for p in strategy_predictions.values()))
,  # type: ignorestrategy_contributions=strategy_contributions,

            consensus_score=consensus_score,
            disagreement_level=disagreement_level,
        

    async def _majority_voting_ensemble(self,)
    "input_data": PredictionInput,
    "strategy_predictions": Dict[str, PredictionOutput],
     -> EnsembleResult:
        """多数投票集成"" # 收集所有预测结果
        ...
        # 找到最常见的预测
        prediction_counts = {}  # type: ignorefor pred in predictions:

            key = predprediction_counts[key] = prediction_counts.get(key, 0) + 1
    ...
    return await self._weighted_average_ensemble()
        ...
        away_counts = {}  # type: ignorefor pred in away_predictions:

            key = predaway_counts[key] = away_counts.get(key, 0) + 1
    ...
    final_away = int(np.mean(away_predictions))
        ...
        elsefinal_away = most_common_away[0]
...
,  # type: ignorestrategy_contributions=strategy_contributions,
    ...
    async def _dynamic_weighting_ensemble(self,)
    "input_data": PredictionInput,
    "strategy_predictions": Dict[str, PredictionOutput],
     -> EnsembleResult:
        """动态权重集成"" # 根据策略最近的性能调整权重
        for name, weight in self._strategy_weights.items()
:
            if name in self._performance_historyrecent_performance = self._performance_history[name][-1,)

    0:  # 最近10次
                if recent_performanceavg_performance = np.mean(recent_performance)

                    # 性能越好,权重越高
                    weight.dynamic_weight = weight.base_weight * (0.5 + avg_performance)  # type: ignore,
    elseweight.dynamic_weight = weight.base_weight
...
:
            weight = ()
                self._strategy_weights[name].dynamic_weight  # type: ignoreor self._strategy_weights[name].base_weight

            

            total_home += prediction.predicted_home_score * weight  # type: ignoretotal_away += prediction.predicted_away_score * weight  # type: ignore

            total_weight += weight  # type: ignore

            strategy_contributions[name] = {)
                "weight": weight,", "dynamic_weight": self._strategy_weights[name.dynamic_weight,", "base_weight": self._strategy_weights[name.base_weight,", "confidence": prediction.confidence," ])
...
,  # type: ignorestrategy_contributions=strategy_contributions,
    ...
    async def _calculate_consensus_score(self, strategy_predictions: Dict[str, PredictionOutput])
     -> float:
        """计算策略共识度"" if len(strategy_predictions) < 2
    return 1.0
    ...
    async def _calculate_disagreement_level(self, strategy_predictions: Dict[str, PredictionOutput])
     -> float:
        """计算策略分歧度"" if len(strategy_predictions) < 2
    return 0.0
    ...
    async def _calculate_ensemble_confidence(self, input_data: PredictionInput, ensemble_result: EnsembleResult)
     -> float:
        """计算集成预测的置信度"" base_confidence = np.mean()
            [)
    ...
    async def _calculate_ensemble_probabilities(self, strategy_predictions: Dict[str, PredictionOutput])
    ) -> Dict[str, float:
        """计算集成概率分布"" # 收集所有策略的概率分布
        all_probabilities = []
        weights = []

        for name, prediction in strategy_predictions.items()
:
            if prediction.probability_distributionall_probabilities.append(prediction.probability_distribution)

                weights.append(self._strategy_weights[name].base_weight)

        if not all_probabilitiesreturn {"home_win")
...
:
    ...
    async def update_metrics(self, actual_results: List[Tuple[Prediction, Dict[str, Any])))
     -> None:
        """更新集成策略性能指标"" if not actual_resultsreturn
        ...
        for pred, actual in actual_results:
            ...
            # 精确匹配
            if ()
                pred.predicted_home == actual_home  # type: ignoreand pred.predicted_away == actual_away  # type: ignore
...
:
            ...
            # 计算得分误差
            error = abs(pred.predicted_home - actual_home) + abs(  # type: ignorepred.predicted_away - actual_away  # type: ignore)
...
,
        ...
        for strategy_name in self._sub_strategies.keys()
:
            if strategy_name not in self._performance_historyself._performance_history[strategy_name] = []
            ...
            # 限制历史记录长度
            if len(self._performance_history[strategy_name]) > self._performance_windowself._performance_history[strategy_name] = self._performance_history[)

                    strategy_name
                ][-self._performance_window :

    def add_strategy(self, strategy: PredictionStrategy, weight: float = 1.0) -> None:
        """动态添加子策略"" self._sub_strategies[strategy.name] = strategyself._strategy_weights[strategy.name] = StrategyWeight()
    ...
    def remove_strategy(self, strategy_name: str) -> None:
        """移除子策略"" if strategy_name in self._sub_strategiesdel self._sub_strategies[strategy_name]

        if strategy_name in self._strategy_weightsdel self._strategy_weights[strategy_name]

        if strategy_name in self._performance_historydel self._performance_history[strategy_name]

        logger.info(f"移除子策略: {strategy_name]")

    def update_strategy_weight(self, strategy_name: str, new_weight: float) -> None:
        """更新策略权重"""
        if strategy_name in self._strategy_weightsself._strategy_weights[strategy_name.base_weight = new_weight)
