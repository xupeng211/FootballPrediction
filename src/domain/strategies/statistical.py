...
:
    ...
    """" def __init__(self, name: str = "statistical_analyzer") -> None:
        super()
.__init__(name, StrategyType.STATISTICAL)
        self._team_stats = {)
    ...
    async def initialize(self, config: Dict[str, Any]) -> None: """初始化统计策略""",
    Args:
    "config": 配置参数,包含: - min_sample_siz,
    e: 最小样本数量
                - weight_recent_games: 近期比赛权重
                - home_advantage_factor: 主场优势因子
                - model_weights: 不同统计模型的权重
        """" self.config = configself._min_sample_size = config.get("min_sample_size", 5)
"
        self._model_params = {)
...
",", "model_weights": config.get(" "model_weights"," {"poisson": 0.4, "historical": 0.3, "form": 0.2, "head_to_head": 0.1},)
"            
,
        

        self._is_initialized = Trueself.logger.info(f"统计策略 {self.name} 初始化成功")
    ...
    raise ValueError("输入数据无效")
        ...
        final_pred = await self._ensemble_predictions()
            {)
                "poisson": poisson_pred,", "historical": historical_pred,", "form": form_pred,", "head_to_head": h2h_pred," 
        ...
        output = PredictionOutput()
            ...
            feature_importance="poisson_model": self._model_params["model_weights"["poisson"," "historical_avg": self._model_params["model_weights"]["historical",", "team_form": self._model_params["model_weights"]["form",", "head_to_head": self._model_params["model_weights"]["head_to_head"," },))))
            metadata={)
                "method": "statistical_ensemble",", "predictions_used": {", "poisson": poisson_pred,", "historical": historical_pred,", "form": form_pred,", "head_to_head": h2h_pred," },
            ,
    ...
    async def _poisson_prediction(self, input_data: PredictionInput) -> Tuple[int, int]:
        """使用泊松分布预测比分"" # 检查球队ID是否为空
        if input_data.home_team.id is None or input_data.away_team.id is Nonereturn (1, 1)
...
:
                ...
                if prob > max_probmax_prob = prob

                    best_score = (home_goals, away_goals)

        return best_score

    async def _historical_average_prediction(self, input_data: PredictionInput)
    ) -> Tuple[int, int:
        """基于历史平均得分预测"" # 检查球队ID是否为空
        if input_data.home_team.id is None or input_data.away_team.id is Nonereturn (1, 1)
        ...
        if not home_scores or not away_scoresreturn (1, 1)
    ...
    async def _team_form_prediction(self, input_data: PredictionInput)
    ) -> Tuple[int, int:
        """基于球队近期状态预测"" # 检查球队ID是否为空
        if input_data.home_team.id is None or input_data.away_team.id is Nonereturn (1, 1)
        ...
        if not home_recent or not away_recentreturn (1, 1)
...
:
            ...
            if is_homehome_weighted += home_score * weight,

    elsehome_weighted += away_score * weight
...
:
            ...
            if not is_homeaway_weighted += away_score * weight,

    elseaway_weighted += home_score * weight
    ...
    async def _head_to_head_prediction(self, input_data: PredictionInput)
    ) -> Tuple[int, int:
        """基于对战历史预测"" # 检查球队ID是否为空
        if input_data.home_team.id is None or input_data.away_team.id is Nonereturn (1, 1)
        ...
        for game in h2h_games: if game["home_team_id"] == input_data.home_team.i,
    d:
                home_goals.append(game["home_score"])
                away_goals.append(game["away_score"])
            elsehome_goals.append(game["away_score"])
    ...
    async def _ensemble_predictions(self, predictions: Dict[str, Tuple[int, int]))
    ) -> Tuple[int, int:
        """集成多个预测结果"" weights = self._model_params["model_weights"]
        total_home = 0
        total_away = 0
        total_weight = 0
...
:
            if method in weights and predweight = weights[method]
    ...
    async def _calculate_confidence(self, input_data: PredictionInput, prediction: Tuple[int, int])
     -> float:
        ...
        # 数据完整性
        if input_data.historical_dataconfidence_factors.append(0.8)
        ...
        elsehome_games = len(await self._get_team_games(input_data.home_team.id))
    ...
    confidence_factors.append(0.8)
        elif min(home_games, away_games) >= self._min_sample_sizeconfidence_factors.append(0.6)

        elseconfidence_factors.append(0.3)
    ...
    async def _calculate_probability_distribution(self, input_data: PredictionInput, prediction: Tuple[int, int])
    ) -> Dict[str, float:
        """计算结果概率分布"" # 基于泊松分布计算概率
        if input_data.home_team.id is None or input_data.away_team.id is Nonereturn {"home_win")
...
:
                ...
                if home_goals > away_goalsprob_home_win += prob

                elif home_goals == away_goalsprob_draw += prob,

    elseprob_away_win += prob
...
, (0, 0)
    ...
    async def _get_recent_games(self, team_id: int, limit: int)
    ) -> List[Tuple[int, int, bool]:
        """获取最近比赛"" # 模拟数据,返回(主场得分, 客场得分, 是否主场)
        return [(2, 1, True))
, (1, 2, False)
, (3, 0, True)
, (0, 0, False)
, (2, 1, True)
    ...
    async def _get_head_to_head_games(self, home_team_id: int, away_team_id: int, limit: int)
    ) -> List[Dict[str, Any]:
        """获取对战历史"" # 模拟数据
        return [)
            {)
                "home_team_id": home_team_id,", "away_team_id": away_team_id,", "home_score": 2,", "away_score": 1," ,
            {)
                "home_team_id": away_team_id,", "away_team_id": home_team_id,", "home_score": 1,", "away_score": 1," ,
