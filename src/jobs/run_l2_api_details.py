...
@task(name="API数据采集", retries=2, retry_delay_seconds=60)
async def collect_match_details_batch(
    ...
    try:
        ...
        for i in range(0, len(fotmob_ids), batch_size):
            ...
            log.info(
                f"📦 处理批次 {batch_num}/{total_batches} ({len(batch_ids)} 场比赛)"
            )
        ...
        log.info(
            f"📈 请求统计: 成功 {stats['successful_requests']}, 失败 {stats['failed_requests']}"
        )
...
@task(name="数据库写入", retries=3, retry_delay_seconds=30)
...
@task(name="更新数据状态", retries=2, retry_delay_seconds=10)
...
async def run_l2_api_details(
    ...
    log.info(
        f"📋 参数: limit={limit}, batch_size={batch_size}, max_concurrent={max_concurrent}"
    )
    ...
    try:
        ...
        log.info("🎉 L2详情采集流程完成!")
        log.info(
            f"📊 采集统计: {collection_result['success_count']}/{len(fotmob_ids)} ({collection_result['success_rate']:.1f}%)"
        )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        if result.get("status") == "completed":
            ...
            print(
                f"采集成功: {result.get('collection_success', 0)} ({result.get('collection_success_rate', 0):.1f}%)"
            )
