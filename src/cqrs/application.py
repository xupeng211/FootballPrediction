...
.bus import get_command_bus, get_query_bus
.commands import ()
...
class PredictionCQRSService:
    ...
    async def get_user_stats(self, user_id: int, include_predictions: bool = False)
...
class MatchCQRSService:
    ...
    # 查询操作
    async def get_match_by_id(self, match_id: int, include_predictions: bool = False)
    ) -> Optional[MatchDTO:
        """根据ID获取比赛"""
...
async def initialize_cqrs():
    ...
    # 注册中间件
    .bus import LoggingMiddleware, ValidationMiddleware
