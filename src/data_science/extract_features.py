...
def main():
    ...
    try:
        ...
        print(f"   日期范围: {eda_report['basic_stats']['date_range']['earliest']} 到 {eda_report['basic_stats']['date_range']['latest']}")

        print("\n🔍 数据质量:")
        print(f"   总比赛数: {eda_report['data_quality']['total_matches']}")
        print(f"   xG数据可用性: {eda_report['data_quality']['xg_data_available']} 场比赛 ({eda_report['data_quality']['xg_data_rate']:.1f}%)")
        print(f"   完整阵容数据: {eda_report['data_quality']['lineups_available']} 场比赛 ({eda_report['data_quality']['lineups_rate']:.1f}%)")
        print(f"   完整赔率数据: {eda_report['data_quality']['odds_available']} 场比赛 ({eda_report['data_quality']['odds_rate']:.1f}%)")

        print("\n🎯 目标变量分布:")
        print(f"   主队获胜: {eda_report['target_distribution']['home_win']} ({eda_report['target_distribution']['percentages']['home_win_pct']:.1f}%)")
        print(f"   平局: {eda_report['target_distribution']['draw']} ({eda_report['target_distribution']['percentages']['draw_pct']:.1f}%)")
        print(f"   客队获胜: {eda_report['target_distribution']['away_win']} ({eda_report['target_distribution']['percentages']['away_win_pct']:.1f}%)")
