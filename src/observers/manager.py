...
class CacheSubject(Subject):
    ...
    async def record_cache_hit(self, cache_name: str, key: str, hit: bool = True) -> None:
        """记录缓存事件"""
        event = CacheEvent(
...
class MetricsObserver(Observer):
    ...
    async def update(self, subject: Subject, event_data: Any) -> None:
        ...
        if isinstance(event_data, PredictionEvent):
            ...
            # 更新平均响应时间
            total_time = self._metrics['avg_response_time'] * (self._metrics['total_predictions'] - 1)
            total_time += event_data.response_time_ms
            self._metrics['avg_response_time'] = total_time / self._metrics['total_predictions']
