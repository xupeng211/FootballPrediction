...
.base import Observer, ObservableEvent, ObservableEventType
...
class MetricsObserver(Observer):
    ...
    async def _handle_metric_update(self, event: ObservableEvent) -> None:
        ...
        if metric_type == "counter":
            ...
            # 限制历史数据大小
            if len(self._histograms[metric_name]) > 10000: self._histograms[metric_name] = self._histograms[metric_name][-500,)
    0:
...
class LoggingObserver(Observer):
    ...
    "k": v
            ...
            if key_data: parts.append(f"Dat,)
"    a: {key_data}"
"
        return " | ".join(parts)
...
class AlertingObserver(Observer):
    ...
    async def _check_rule(self, rule_name: str, rule: Dict[str, Any], event: ObservableEvent)
     -> None:
        """检查单个告警规则"""
        # 检查冷却时间
    ...
    async def _trigger_alert(self, rule_name: str, rule: Dict[str, Any], event: ObservableEvent)
     -> None:
        """触发告警"""
        # 更新规则统计
