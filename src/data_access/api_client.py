...
class FotMobAPIClient:
    ...
    def _make_request(self, url: str, params: dict[str, Any] = None, timeout: int = None) -> dict[str, Any] | None:
        """发起API请求"""
        self._rate_limit()
    ...
    def get_league_matches(self, league_id: str = None, tab: str = "results") -> dict[str, Any] | None:
        """获取联赛比赛列表"""
        if league_id is None:
            league_id = self.config.harvest.league_id

        url = f"{self.config.api.fotmob_base_url}/leagues"
        params = {"id": league_id, "tab": tab}
    ...
    def get_team_stats(self, team_id: str, league_id: str = None) -> dict[str, Any] | None:
        """获取球队统计"""
        if league_id is None:
            league_id = self.config.harvest.league_id

        url = f"{self.config.api.fotmob_base_url}/teamStats"
        params = {"teamId": team_id, "leagueId": league_id}
    ...
    def search_team(self, team_name: str) -> list[dict[str, Any]] | None:
        """搜索球队"""
        url = f"{self.config.api.fotmob_base_url}/searchAPI"
        params = {"term": team_name, "type": "team"}
