    ...
    def aggregate_from_shotmap(data: Dict[str, Any]) -> Tuple[float, float]:
        ...
        def extract_from_shots(shots_data: Dict[str, Any]):
            ...
            for team_key, shot_list in shots_data.items():
                ...
                if any(keyword in team_key_lower for keyword in ['home', '0', team_data.get('homeTeam', {}).get('name', '').lower()]):
                    home_xg += team_xg
                else:
                    away_xg += team_xg
...
class AdvancedFeatureExtractor:
    ...
    def extract_complete_features(self, match_data: Dict[str, Any], external_id: str) -> MatchFeatures:
        ...
        if (features_dict.get('home_possession') is not None and
            features_dict.get('away_possession') is not None):
        ...
        try:
            ...
            logger.info(f"[SUCCESS] Match ID: {external_id}, xG: {features.home_xg}-{features.away_xg}, "
    ...
    def _extract_possession_features(self, data: Dict[str, Any]) -> Tuple[Optional[float], Optional[float]]:
                    ...
                    for stat_item in top_stats:
                        if isinstance(stat_item, dict) and stat_item.get('key') == 'BallPossesion':
                            ...
                            if isinstance(possession_values, list) and len(possession_values) >= 2:
                                ...
                                logger.debug(f"从FotMob新API提取控球率: 主队={home_possession}%, 客队={away_possession}%")
                                break
        ...
        # 最后的备用方案：使用递归提取器
        if home_possession is None:
            home_possession = self.recursive_extractor.extract_value(data, self.config.POSSESSION_PATTERNS['home'], 'possession.home')
        if away_possession is None:
            away_possession = self.recursive_extractor.extract_value(data, self.config.POSSESSION_PATTERNS['away'], 'possession.away')

        # 数据质量检查：如果仍然找不到真实数据，返回NULL而不是50默认值
        if home_possession is None or away_possession is None:
            logger.warning(f"⚠️ 无法获取真实控球率数据，返回NULL以避免默认值污染 - 主队: {home_possession}, 客队: {away_possession}")
            ...
            try:
                ...
                for key in path:
                    ...
                    if isinstance(current, dict):
                        # 查找各种赔率字段名
                        home_odds = home_odds or current.get('homeWin') or current.get('homeOdds') or current.get('home_win')
                        away_odds = away_odds or current.get('awayWin') or current.get('awayOdds') or current.get('away_win')
                        draw_odds = draw_odds or current.get('draw') or current.get('drawOdds') or current.get('draw_odds')
    ...
    def _extract_corners_features(self, data: Dict[str, Any]) -> Dict[str, Any]:
                ...
                for key in path:
                    ...
                    if isinstance(current, dict):
                        if 'home' in current and 'away' in current:
                            home_corners = self._safe_float(current['home'])
                            away_corners = self._safe_float(current['away'])
                            break
            except (KeyError, TypeError, AttributeError):
                continue
        ...
        if home_corners is None or away_corners is None:
            try:
                ...
                if isinstance(stats, list):
                    for stat in stats:
                        if isinstance(stat, dict) and stat.get('type') == 'cornerKicks':
                            home_corners = self._safe_float(stat.get('home'))
                            away_corners = self._safe_float(stat.get('away'))
                            break
            except Exception:
                pass
        ...
        if 'cards' in data and isinstance(data['cards'], list):
            ...
            for card in data['cards']:
                if isinstance(card, dict):
                    team_type = card.get('teamType', '').lower()
                    card_type = card.get('cardType', '').lower()

                    if 'yellow' in card_type:
                        if team_type == 'home' or card.get('isHome', False):
                            home_yellow += 1
                        else:
                            away_yellow += 1
