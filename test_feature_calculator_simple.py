    ...
    try:
            ...
            if isinstance(node, ast.ClassDef):
                ...
                for item in node.body:
                    if isinstance(item, ast.FunctionDef):
                        ...
                        if any(isinstance(d, ast.Name) and d.id == 'async' for d in item.decorator_list):
        ...
        for cls in classes:
            if cls["name"] == "FeatureCalculator":
                ...
                # 分类方法
                async_methods = [m for m in cls['methods'] if m in [af.split('.')[1] for af in async_functions]]
...
def test_feature_calculator_concepts():
    ...
    try:
        ...
        features_to_add = [
            ...
            {"name": "odds_implied_prob", "type": "numerical", "description": "赔率隐含概率"}
        ]
...
def test_feature_calculation_algorithms():
    ...
    try:
        ...
        avg_goals_scored = sum(m["goals_scored"] for m in recent_matches) / total_matches if total_matches > 0 else 0
        avg_goals_conceded = sum(m["goals_conceded"] for m in recent_matches) / total_matches if total_matches > 0 else 0
        ...
        team_a_wins = sum(1 for m in head_to_head if
                          (m["home_team"] == "A" and m["home_score"] > m["away_score"]) or
                          (m["away_team"] == "A" and m["away_score"] > m["home_score"]))
        team_b_wins = sum(1 for m in head_to_head if
                          (m["home_team"] == "B" and m["home_score"] > m["away_score"]) or
                          (m["away_team"] == "B" and m["away_score"] > m["home_score"]))
        ...
        for i, odds in enumerate(odds_data):
            ...
            if total_prob > 0:
                ...
                print(f"  ✅ 赔率组合{i+1}: 主队{home_prob_norm:.2%}, 平局{draw_prob_norm:.2%}, 客队{away_prob_norm:.2%}")
