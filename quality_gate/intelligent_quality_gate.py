...
class IntelligentQualityGate:
    ...
    def _assess_test_coverage(self, project_path: Path) -> float:
        ...
        try:
            ...
            # 测试密度奖励
            test_density = len(test_files) / max(len(src_files), 1) * 100
            density_bonus = min(test_density, 10.0)

            total_score = min(base_score + density_bonus, 100.0)
            print(f"    📊 Test files: {len(test_files)}, Source files: {len(src_files)}")
    ...
    def generate_report(self, result: QualityResult) -> str:
        ...
        report_lines.extend([
            "",
            "---",
            f"*Report generated by Intelligent Quality Gate System*",
            f"*Phase 6 - Intelligent Upgrade*"
        ])
