...
try:
    ...
    httpx = MockHttpx()  # type: ignore[assignment]
...
class TestAPIPredictions:
    ...
    @pytest.fixture
    def test_api_client(self, api_base_url):
        """创建测试API客户端"""
        if HAS_HTTPX:
            ...
            mock_client.get = lambda url: MockResponse(
                200,
                {
                    "match_id": 12345,
                    ...
                    "prediction_timestamp": "2025-09-14T00:00:00Z",
                },
            )
            mock_client.post = lambda url, **kwargs: MockResponse(
                200,
                {
                    "predictions": [
                        {
    ...
    async def test_get_prediction_success(
        ...
        try:
            ...
            if isinstance(e, (ConnectionError, httpx.ConnectError)):
                pytest.skip("无法连接到API服务器")
            else:
                raise

    @pytest.mark.asyncio
    async def test_probability_distribution_validation(
        ...
        try:
            ...
            if isinstance(e, (ConnectionError, httpx.ConnectError)):
                ...
                assert abs(total_prob - 1.0) < 0.01, "概率分布验证失败"

    @pytest.mark.asyncio
    async def test_batch_predictions_endpoint(self, test_api_client, sample_match_ids):
        """测试批量预测端点"""
        batch_request = {
        ...
        try:
            response = await test_api_client.post(
                "/predictions/batch", json=batch_request
            )
    ...
    @pytest.mark.asyncio
    async def test_invalid_match_id_handling(self, test_api_client):
        """测试无效比赛ID的处理"""
        invalid_match_ids = [-1, 0, 999999999, "invalid", None]

        for invalid_id in invalid_match_ids:
            try:
                response = await test_api_client.get(f"/predictions/{invalid_id}")
    ...
    @pytest.mark.asyncio
    async def test_nonexistent_match_handling(self, test_api_client):
        """测试不存在比赛的处理"""
        nonexistent_match_id = 999999999  # 极大概率不存在的ID

        try:
            response = await test_api_client.get(f"/predictions/{nonexistent_match_id}")
    ...
    @pytest.mark.asyncio
    async def test_api_rate_limiting(self, test_api_client, sample_match_ids):
        """测试API限流机制"""
        match_id = sample_match_ids[0]
        ...
        try:
            ...
            if rate_limit_detected:
                ...
                assert (
                    "rate limit" in rate_limit_response.json().get("detail", "").lower()
                )
    ...
    @pytest.mark.asyncio
    async def test_prediction_consistency(self, test_api_client, sample_match_ids):
        """测试预测结果的一致性"""
        match_id = sample_match_ids[0]
        ...
        try:
            ...
            if len(predictions) >= 2:
                ...
                for subsequent_prediction in predictions[1:]:
                    # 检查关键字段是否一致
                    assert (
                        subsequent_prediction["match_id"]
                        == first_prediction["match_id"]
                    )

                    # 如果是同一模型版本，预测应该相同
                    if subsequent_prediction.get(
                        "model_version"
                    ) == first_prediction.get("model_version"):
                        assert (
                            abs(
    ...
    @pytest.mark.asyncio
    async def test_features_metadata_completeness(
        self, test_api_client, sample_match_ids
    ):
        """测试特征元数据的完整性"""
        match_id = sample_match_ids[0]
        ...
        try:
            ...
            response = await test_api_client.get(
                f"/predictions/{match_id}?include_features=true"
            )
    ...
    @pytest.mark.asyncio
    async def test_api_response_time(self, test_api_client, sample_match_ids):
        """测试API响应时间"""
        match_id = sample_match_ids[0]
    ...
    @pytest.mark.asyncio
    async def test_concurrent_api_requests(self, test_api_client, sample_match_ids):
        """测试并发API请求处理"""

        async def make_prediction_request(match_id):
            try:
                response = await test_api_client.get(f"/predictions/{match_id}")
        ...
        try:
            # 并发请求多个预测
            tasks = [make_prediction_request(mid) for mid in sample_match_ids[:3]]
    ...
    @pytest.mark.asyncio
    async def test_sql_injection_prevention(self, test_api_client):
        """测试SQL注入防护"""
        malicious_inputs = [
        ...
        for malicious_input in malicious_inputs:
            try:
                response = await test_api_client.get(f"/predictions/{malicious_input}")
    ...
    @pytest.mark.asyncio
    async def test_api_authentication_if_required(self, api_base_url):
        """测试API认证（如果需要）"""
        # 如果API需要认证，测试无认证访问
        if HAS_HTTPX:
            async with httpx.AsyncClient() as client:
                try:
                    response = await client.get(f"{api_base_url}/predictions/12345")
