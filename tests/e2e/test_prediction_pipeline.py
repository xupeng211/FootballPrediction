...
from tests.mocks import (
    ...
    MockDataProcessingService, MockPredictionService, MockFeatureStore,
    MockAPIClient
)
...
class TestPredictionPipeline:
    ...
    async def test_complete_prediction_pipeline(self, mock_services):
        ...
        features = await self._simulate_feature_generation(mock_services, processed_data)

        # Step 5: Make predictions
        predictions = await self._simulate_prediction_generation(mock_services, features)
    ...
    async def test_pipeline_error_handling(self, mock_services):
        ...
        with pytest.raises(Exception, match="Data processing failed"):
            await self._simulate_data_processing(mock_services, match_data)
    ...
    async def test_pipeline_performance(self, mock_services):
        ...
        assert processing_time < 2.0, f"Pipeline took {processing_time:.2f}s, expected < 2.0s"
    ...
    async def test_batch_prediction_pipeline(self, mock_services):
        ...
        for match in matches:
            processed = await self._simulate_data_processing(mock_services, match)
            processed_matches.append(processed)

        # Generate features for all matches
        all_features = []
        for processed in processed_matches:
            features = await self._simulate_feature_generation(mock_services, processed)
    ...
    async def _simulate_data_collection(self, services, match_data, team_data):
        ...
        async with services['database'].get_session() as session:
            session.set_query_result("first", match_data)
        ...
        await services['storage'].upload_file(
            f"raw_matches/{match_data['match_id']}.json",
            str(match_data).encode('utf-8')
        )

    async def _simulate_data_processing(self, services, match_data):
        """Simulate data processing."""
        return await services['data_processor'].process_match_data(match_data)

    async def _simulate_feature_generation(self, services, processed_data):
    ...
    async def _simulate_api_delivery(self, services, predictions):
        ...
        await services['redis'].set(
            f"prediction:{predictions['match_id']}",
            predictions,
            ttl=1800
        )
        ...
        async with services['database'].get_session() as session:
            session.set_query_result("first", predictions)
    ...
    def _validate_pipeline_results(self, api_response, original_match, predictions):
        ...
        assert data["match_id"] == original_match["match_id"]
        ...
        assert 0 <= data["confidence"] <= 1, f"Confidence {data['confidence']} outside valid range"
