...
from tests.mocks import (
    MockDatabaseManager, MockRedisManager, MockDataLakeStorage,
    MockDataProcessingService, MockFootballDataAPI, MockWebSocketClient
)
...
class TestDataCollectionPipeline:
    ...
    async def test_football_data_collection_pipeline(self, mock_services):
        ...
        processed_data = await self._process_collected_data(mock_services, live_matches, historical_matches, team_data)
        ...
        self._validate_data_collection_results(live_matches, historical_matches, team_data, processed_data)
    ...
    async def test_real_time_data_processing(self, mock_services):
        ...
        connected = await mock_services['websocket'].connect("ws://football-api.example.com/live")
        assert connected, "WebSocket connection failed"

        # Simulate receiving live match updates
        live_updates = [
            {"type": "match_start", "match_id": 12345, "home_team": "Team A", "away_team": "Team B"},
            {"type": "goal", "match_id": 12345, "team": "home", "score": 1, "minute": 25},
            {"type": "goal", "match_id": 12345, "team": "away", "score": 1, "minute": 67},
            {"type": "match_end", "match_id": 12345, "final_score": "1-1"}
        ]
    ...
    async def test_data_collection_performance(self, mock_services):
        ...
        assert collection_time < 5.0, f"Data collection took {collection_time:.2f}s, expected < 5.0s"

    @pytest.mark.asyncio
    @pytest.mark.e2e
    async def test_data_quality_validation(self, mock_services):
        """Test data quality validation in collection pipeline."""
        # Collect data with quality issues
        raw_data = [
            {"match_id": 12345, "home_team": "Team A", "away_team": ""},  # Missing away team
            {"match_id": None, "home_team": "Team B", "away_team": "Team C"},  # Missing match_id
            {"match_id": 12346, "home_team": "Team D", "away_team": "Team E"}  # Valid data
        ]

        validated_data = []
        for data in raw_data:
            validation_result = await mock_services['data_processor'].validate_data_quality(data)
            if validation_result['is_valid']:
                validated_data.append(data)

        # Validate quality filtering
        assert len(validated_data) == 1  # Only valid data should remain
        assert validated_data[0]['match_id'] == 12346  # Should be the valid entry

    async def _collect_live_match_data(self, services):
        """Collect live match data via WebSocket."""
        await services['websocket'].connect("ws://football-api.example.com/live")

        live_matches = []
        for i in range(3):  # Simulate 3 live matches
            match_data = {
                "match_id": 1000 + i,
            ...
            services['websocket'].simulate_message(match_data)
            live_matches.append(match_data)

        await services['websocket'].disconnect()
        return live_matches

    async def _collect_historical_match_data(self, services):
        """Collect historical match data via REST API."""
        response = await services['football_api'].get("/matches")
        return response['data']['matches']

    async def _collect_team_data(self, services):
        """Collect team and league data."""
        teams_response = await services['football_api'].get("/teams")
        leagues_response = await services['football_api'].get("/leagues")
    ...
    async def _process_collected_data(self, services, live_matches, historical_matches, team_data):
        ...
        for match in all_matches:
            try:
                processed = await services['data_processor'].process_match_data(match)
    ...
    async def _store_processed_data(self, services, processed_data):
        ...
        async with services['database'].get_session() as session:
            session.set_query_result("all", processed_data["matches"])
        ...
        # Store in data lake
        await services['storage'].upload_file(
            f"processed_data/{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            str(processed_data).encode('utf-8')
        )

    async def _process_live_update(self, services, update):
        """Process a single live update."""
        processed = await services['data_processor'].process_match_data(update)
        processed['processed'] = True
        processed['original_update'] = update
        return processed

    def _validate_data_collection_results(self, live_matches, historical_matches, team_data, processed_data):
        ...
        assert len(team_data["teams"]) > 0, "No team data collected"
        assert len(team_data["leagues"]) > 0, "No league data collected"

        # Validate processing results
        assert "matches" in processed_data, "No processed matches in result"
        assert "teams" in processed_data, "No processed teams in result"
        assert "leagues" in processed_data, "No processed leagues in result"

        # Validate processing timestamps
        assert "processed_at" in processed_data, "No processing timestamp"

        # Validate match data quality
        for match in processed_data["matches"]:
            assert "match_id" in match, f"Missing match_id in {match}"
            assert "home_team" in match, f"Missing home_team in {match}"
            assert "away_team" in match, f"Missing away_team in {match}"
            assert "processed_features" in match, f"Missing processed_features in {match}"
