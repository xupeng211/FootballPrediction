...
class TestUpdateStatementIntegrity:
    ...
    def test_no_null_tags_in_database(self):
        ...
        mock_records = [
            ...
            {'match_id': 2, 'league_id': None, 'season_id': None, 'home_team': 'Unknown', 'away_team': 'Unknown'},  # 失败
        ]
...
class TestZeroHardcodingConfig:
    ...
    def test_config_unified_no_hardcoded_strings(self):
        ...
        if os.getenv('DOCKER_ENV'):
            assert db.name not in ['football_prediction_dev', 'test_db'], \
                f"检测到硬编码数据库名: {db.name}"
    ...
    def test_harvester_config_returns_valid_settings(self):
        ...
        assert settings.dimension_fuse_count == 5, f"dimension_fuse_count 应该是 5，实际是 {settings.dimension_fuse_count}"
