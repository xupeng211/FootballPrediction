    ...
    try:
        ...
        async with FootballDataCollector() as collector:
            ...
            if validated_data and "competitions" in validated_data:
                logger.info(
                    f"     ✅ 数据验证成功，验证了 {validated_data.get('count', 0)} 个联赛"
                )
            else:
                logger.error("     ❌ 数据验证失败")
                return False
            ...
            if cleaned_data and "competitions" in cleaned_data:
                ...
                if cleaned_data["competitions"]:
                    ...
                    if all(field in first_comp for field in required_fields):
                        ...
                        logger.warning(
                            f"     ⚠️ 数据结构不完整，缺少字段: {[f for f in required_fields if f not in first_comp]}"
                        )
            else:
                logger.error("     ❌ 数据清洗失败")
                return False
    ...
    try:
        ...
        async with TeamCollector() as collector:
            ...
            if teams:
                ...
                if all(
                    key in first_team
                    for key in ["external_id", "name", "short_name", "crest"]
                ):
...
async def test_data_integrity():
    ...
    try:
        ...
        async with (
            ...
            LeagueCollector() as league_collector,
        ):
            ...
            if teams_not_in_standings:
                logger.warning(
                    f"     ⚠️ {len(teams_not_in_standings)} 支队在列表中但不在积分榜中"
                )
            if standings_not_in_teams:
                logger.warning(
                    f"     ⚠️ {len(standings_not_in_teams)} 支队在积分榜中但不在列表中"
                )
