...
docker compose -p "$project" -f "$compose_file" up --abort-on-container-exit --exit-code-from restore-baseline-verifier \
  ...
  --attach runner --attach canary-bootstrap --attach canary-owner-phase-seal --attach canary-runner \
  --attach restore-verifier --attach restore-baseline-verifier restore-baseline-verifier
