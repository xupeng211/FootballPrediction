...
class TestAsyncManagerIntegration:
    ...
    async def test_database_connection_and_crud(self, setup_test_database):
        ...
        # 3. 查询单条记录
        alice = await fetch_one(text("SELECT * FROM integration_users WHERE name = 'Alice'"))
        assert alice is not None, "应该找到Alice"
        assert alice["email"] == "alice@test.com", "Alice的邮箱应该正确"

        # 4. 更新数据
        await execute(text("UPDATE integration_users SET age = 26 WHERE name = 'Alice'"))
        updated_alice = await fetch_one(text("SELECT age FROM integration_users WHERE name = 'Alice'"))
        assert updated_alice["age"] == 26, "Alice的年龄应该被更新"

        # 5. 删除数据
        await execute(text("DELETE FROM integration_users WHERE name = 'Bob'"))
        remaining_users = await fetch_all(text("SELECT COUNT(*) as count FROM integration_users"))
    ...
    async def test_transaction_isolation(self, setup_test_database):
        ...
        # 直接使用execute方法（会自动提交）
        await execute(text("INSERT INTO integration_transactions (data) VALUES ('trans1')"))
        await execute(text("INSERT INTO integration_transactions (data) VALUES ('trans2')"))

        # 验证结果
        records = await fetch_all(text("SELECT * FROM integration_transactions ORDER BY data"))
    ...
    async def test_error_handling_and_rollback(self, setup_test_database):
        ...
        # 插入第一条记录
        await execute(text("INSERT INTO integration_errors (name) VALUES ('unique_name')"))
        ...
        # 验证第一条记录仍然存在（事务回滚）
        record = await fetch_one(text("SELECT * FROM integration_errors WHERE name = 'unique_name'"))
        assert record is not None, "原始记录应该仍然存在"

        # 验证只有一条记录
        count = await fetch_one(text("SELECT COUNT(*) as count FROM integration_errors"))
    ...
    async def test_batch_operations(self, setup_test_database):
        ...
        # 验证批量插入结果
        records = await fetch_all(text("SELECT COUNT(*) as count FROM integration_batch"))
        assert records[0]["count"] == 10, "应该插入10条记录"

        # 验证数据完整性
        first_record = await fetch_one(text("SELECT * FROM integration_batch WHERE id = 1"))
        assert first_record["name"] == "item_1", "第一条记录应该正确"
        assert first_record["value"] == 10, "第一条记录的值应该正确"

        last_record = await fetch_one(text("SELECT * FROM integration_batch WHERE id = 10"))
