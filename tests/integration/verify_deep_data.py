...
from src.data.collectors.fotmob_browser_v2 import FotmobBrowserScraperV2, FotmobMatchDataV2
...
class DeepDataValidator:
    ...
    async def run_deep_validation(self) -> bool:
        ...
        try:
            # 执行V2深度采集
            async with FotmobBrowserScraperV2(max_concurrent_details=2) as scraper:  # 降低并发度便于观察
                matches = await scraper.scrape_matches_deep(test_date)

                if not matches:
                    print("❌ 深度测试失败: 未采集到任何比赛数据")
                    self.test_results['critical_failures'].append("未采集到任何比赛数据")
    ...
    def _perform_mandatory_assertions(self, matches: list[FotmobMatchDataV2], test_date: str) -> bool:
        """执行强制断言 - 首席代码审计师级别"""
        ...
        major_league_matches = [
            ...
            if any(league in match.league_name.lower() for league in
        ...
        for i, match in enumerate(test_matches, 1):
            print(f"\n{'─'*60}")
            print(f"🧪 深度测试 {i}/{len(test_matches)}: {match.home_team_name} vs {match.away_team_name}")
            print(f"{'─'*60}")
            ...
            try:
                ...
                # 检查阵容结构
                has_home_lineup = any('home' in str(k).lower() for k in match.lineups.keys())
                has_away_lineup = any('away' in str(k).lower() for k in match.lineups.keys())
                has_players = any('player' in str(k).lower() for k in match.lineups.keys())

                print(f"  ✅ 阵容数据: 通过 (结构: {has_home_lineup}/{has_away_lineup}, 球员: {has_players})")
                self.test_results['lineups_found'] += 1

            except AssertionError as e:
                print(f"  ❌ 阵容数据: 失败 - {str(e)}")
                self.test_results['critical_failures'].append(f"比赛{match.match_id}阵容缺失: {str(e)}")
                passed_all = False
            ...
            try:
                ...
                has_possession = any(stat in stats_content for stat in ['possession', 'ball'])
                has_shots = any(stat in stats_content for stat in ['shot', 'attempt', 'ontarget'])
                has_corners = any(stat in stats_content for stat in ['corner'])
                has_fouls = any(stat in stats_content for stat in ['foul', 'yellow'])

                print(f"  ✅ 统计数据: 通过 (控球:{has_possession}, 射门:{has_shots}, 角球:{has_corners})")
                ...
                self.test_results['critical_failures'].append(f"比赛{match.match_id}统计缺失: {str(e)}")
                passed_all = False
            ...
            if match.odds: completeness_score += 1
            if match.venue: completeness_score += 1
        ...
        print(f"📋 阵容成功率: {lineups_rate:.1%} ({self.test_results['lineups_found']}/{total_tests})")
        print(f"📊 统计成功率: {stats_rate:.1%} ({self.test_results['stats_found']}/{total_tests})")
    ...
    def print_failure_report(self):
        ...
        if self.test_results['critical_failures']:
            ...
            if len(self.test_results['critical_failures']) > 5:
                print(f"  ❌ ... 还有 {len(self.test_results['critical_failures']) - 5} 个失败")
