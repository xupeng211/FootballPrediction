...
class DataQualityTester:
    ...
    def print_data_richness_report(self, match_data_list: list[FotmobMatchData], raw_api_data: list[dict]):
        """打印数据丰度报告"""
        print("\n📊 数据丰度报告:")
        ...
        for match in match_data_list:
            # 基础字段检查
            basic_fields = ['match_id', 'home_team_name', 'away_team_name', 'home_score', 'away_score']
            has_basic = all(hasattr(match, field) and getattr(match, field) is not None for field in basic_fields)

            if has_basic:
                rich_matches += 1

            # 收集所有字段
            for field in ['match_id', 'league_name', 'home_team_name', 'away_team_name'
                         'status', 'kickoff_time', 'utc_time']:
            ...
            def find_advanced_fields(obj, path=""):
                if isinstance(obj, dict):
                    for key, value in obj.items():
                        ...
                        # 检查xG相关字段
                        if any(xg_term in key.lower() for xg_term in ['xg', 'expected', 'rating']):
                            advanced_fields_found.add(current_path)
                            self.test_results['xg_data_found'] = True

                        # 检查阵容相关字段
                        if any(lineup_term in key.lower() for lineup_term in ['lineup', 'bench', 'formation', 'starter']):
                            advanced_fields_found.add(current_path)
                            self.test_results['lineup_data_found'] = True

                        # 检查统计相关字段
                        if any(stats_term in key.lower() for stats_term in ['possession', 'shots', 'corner', 'foul', 'offside', 'card']):
                            advanced_fields_found.add(current_path)
        ...
        if advanced_fields_found:
            print("\n🎯 高级数据字段发现:")
            xg_fields = [f for f in advanced_fields_found if any(xg in f.lower() for xg in ['xg', 'expected', 'rating'])]
            lineup_fields = [f for f in advanced_fields_found if any(lineup in f.lower() for lineup in ['lineup', 'bench', 'formation'])]
            stats_fields = [f for f in advanced_fields_found if any(stat in f.lower() for stat in ['possession', 'shots', 'corner'])]
    ...
    def assert_xg_data(self, raw_api_data: list[dict]) -> bool:
        """断言xG数据存在"""
        xg_keywords = ['xg', 'expected', 'rating', 'expectedgoals', 'xg_home', 'xg_away']
    ...
    def assert_lineup_data(self, raw_api_data: list[dict]) -> bool:
        """断言阵容数据存在"""
        lineup_keywords = ['lineup', 'bench', 'formation', 'starter', 'substitute', 'player']
    ...
    def has_lineup_structure(self, obj: Any) -> bool:
        ...
        if isinstance(obj, dict):
            for key, value in obj.items():
                if any(lineup_term in key.lower() for lineup_term in ['lineup', 'bench']):
    ...
    def assert_stats_data(self, raw_api_data: list[dict]) -> bool:
        """断言统计数据存在"""
        stats_keywords = ['possession', 'shots', 'corner', 'foul', 'offside', 'card', 'pass', 'tackle']
    ...
    def has_stats_values(self, obj: Any) -> bool:
        ...
        if isinstance(obj, dict):
            for key, value in obj.items():
                if any(stat_term in key.lower() for stat_term in ['possession', 'shots', 'corner']):
    ...
    def save_test_sample(self, match_data_list: list[FotmobMatchData], raw_api_data: list[dict], test_date: str):
        """保存测试样本"""
        try:
            ...
            sample_data = {
                ...
                'matches': [asdict(match) for match in match_data_list[:5]],  # 保存前5场比赛作为样本,
