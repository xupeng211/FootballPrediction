...
from benchmark_framework import (
    PerformanceBenchmarkFramework, BenchmarkConfig, run_performance_benchmark,
    create_benchmark_config, BenchmarkDatabase
)
...
class BenchmarkRunner:
    ...
    async def check_regressions(self, benchmark_name: Optional[str] = None, environment: Optional[str] = None) -> Dict[str, Any]:
        """检查性能回归"""
        logger.info("🔍 检查性能回归")
    ...
    async def generate_performance_report(self, output_dir: str = "performance_reports") -> Dict[str, Any]:
        ...
        # 获取基准测试摘要
        benchmark_names = ["football_prediction_system", "ml_inference_performance", "data_processing_performance"]
                    ...
                    if recent_results:
                        ...
                        if successful_results:
                            ...
                            report_data['benchmarks'][f"{bench_name}_{env}"] = {
                                'latest_result': successful_results[0].execution_time_ms,
                                'avg_execution_time_ms': statistics.mean(execution_times),
                                'min_execution_time_ms': min(execution_times),
                                'max_execution_time_ms': max(execution_times),
                                'test_count': len(successful_results),
                                'last_updated': successful_results[0].timestamp.isoformat()
                            }
    ...
    async def _generate_comparison_report(self, benchmark_name: str, comparison_results: Dict[str, Any]) -> Dict[str, Any]:
        """生成环境对比报告"""
        ...
        for metric in ['avg_execution_time_ms', 'p95_execution_time_ms', 'success_rate']:
            values = {}
            for env in envs:
            ...
            if all(v > 0 for v in values.values()):
                ...
                improvement = ((values[worst_env] - values[best_env]) / values[worst_env]) * 100
    ...
    async def _analyze_trends(self, historical_results: List) -> Dict[str, Any]:
        ...
        return {
            ...
            'time_range_days': (sorted_results[-1].timestamp - sorted_results[0].timestamp).days,
            'avg_execution_time_ms': statistics.mean(execution_times),
...
async def main():
    ...
    compare_parser.add_argument('--environments', nargs='+', default=['development', 'staging'], help='要比较的环境')
    ...
    report_parser.add_argument('--output-dir', default='performance_reports', help='报告输出目录')
        ...
        if args.command == 'run':
            ...
            if result.get('comparison_metrics'):
                for metric, data in result['comparison_metrics'].items():
                    print(f"\n{metric}:")
                    print(f"  最佳环境: {data['best_environment']} ({data['best_value']:.2f})")
                    print(f"  最差环境: {data['worst_environment']} ({data['worst_value']:.2f})")
            ...
            if result['regression_details']:
                ...
                for regression in result['regression_details']:
                    print(f"  - {regression['benchmark_name']} ({regression['environment']}): "
