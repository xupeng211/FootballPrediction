...
def assert_valid_prediction_response(response_data):
    ...
    assert isinstance(response_data["match_id"], int), \
        f"match_id should be int, got {type(response_data['match_id'])}"
    assert isinstance(response_data["confidence"], (int, float)), \
        f"confidence should be numeric, got {type(response_data['confidence'])}"
    ...
    assert response_data["predicted_outcome"] in valid_outcomes, \
    ...
    assert isinstance(response_data["model_version"], str), \
        f"model_version should be string, got {type(response_data['model_version'])}"
    assert response_data["model_version"], "model_version should not be empty"
...
def assert_probabilities_valid(probabilities):
    ...
    assert abs(total_prob - 1.0) < 1e-6, \
        f"Probabilities sum to {total_prob:.6f}, expected 1.0 (difference: {abs(total_prob - 1.0):.6f})"


def assert_performance_requirements(response_time_ms, operation_name="API call"):
    ...
    error_msg = (
        f"{operation_name} response time {response_time_ms:.2f}ms exceeds "
        f"critical threshold of {critical_threshold}ms"
    )

    assert response_time_ms < critical_threshold, error_msg

    # Performance analysis
    performance_level = "excellent" if response_time_ms < warning_threshold else "acceptable"
    if response_time_ms >= warning_threshold:
        pytest.warns(
            UserWarning,
            match=f"{operation_name} response time {response_time_ms:.2f}ms "
                 f"exceeds warning threshold of {warning_threshold}ms"
        )


def assert_performance_consistency(response_times, operation_name="API calls"):
    ...
    assert max_deviation <= consistency_threshold, \
        f"{operation_name} performance inconsistent. Max deviation {max_deviation:.2f}ms " \
        f"exceeds threshold {consistency_threshold:.2f}ms. " \
        f"Stats: avg={avg_time:.2f}ms, min={min_time:.2f}ms, max={max_time:.2f}ms, " \
        f"std={std_dev:.2f}ms"
...
class TestAPIPerformanceWithImprovedAssertions:
    ...
    async def test_prediction_api_latency_under_100ms(self, performance_services):
        ...
        assert isinstance(match_features, dict), "Generated features should be a dictionary"
        assert match_features, "Generated features should not be empty"
    ...
    @pytest.mark.asyncio
    @pytest.mark.performance
    async def test_cached_prediction_performance_superiority(self, performance_services):
        ...
        cache_key = f"prediction:{match_features['match_id']}"
        cache_success = await performance_services['redis'].set(
            cache_key,
            prediction1,
            ttl=300
        )
        ...
        cached_data = await performance_services['redis'].get(cache_key)
        assert cached_data == prediction1, "Cached data differs from original prediction"

        # Second request (cache hit)
        start_time = time.time()
        cached_prediction = await performance_services['redis'].get(cache_key)
        second_request_time = (time.time() - start_time) * 1000

        # Cache performance validation
        cache_performance_threshold = 10.0  # 10ms for cache reads
        assert second_request_time < cache_performance_threshold, \
        ...
        improvement_ratio = first_request_time / max(second_request_time, 0.001)  # Avoid division by zero

        assert performance_improvement > 0, \
            f"Cached response should be faster, but took {second_request_time:.2f}ms vs {first_request_time:.2f}ms"
    ...
    async def test_batch_prediction_performance_efficiency(self, performance_services):
        ...
        # Validate test setup
        assert len(match_features_list) == batch_size, \
        ...
        assert avg_time_per_prediction < batch_efficiency_threshold, \
        ...
        assert len(batch_predictions) == batch_size, \
            f"Expected {batch_size} predictions, got {len(batch_predictions)}. " \
            f"Missing: {batch_size - len(batch_predictions)} predictions"

        # Batch quality validation
        failed_predictions = [pred for pred in batch_predictions if 'error' in pred]
        assert not failed_predictions, \
            f"Batch contained {len(failed_predictions)} failed predictions: {failed_predictions}"

        # Individual prediction validation
        for i, prediction in enumerate(batch_predictions):
            assert_valid_prediction_response(prediction)
            expected_match_id = match_features_list[i]['match_id']
            assert prediction['match_id'] == expected_match_id, \
    ...
    async def test_api_performance_under_concurrent_load(self, performance_services):
        ...
        start_time = time.time()
        tasks = [make_prediction_with_validation(features, i) for i, features in enumerate(match_features_list)]
        ...
        performance_stats = assert_performance_consistency(response_times, "Concurrent API calls")
        ...
        avg_response_time = performance_stats["avg_time"]
        ...
        # Performance tier should be at least "good"
        assert performance_stats["performance_tier"] in ["excellent", "good"], \
