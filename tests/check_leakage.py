...
class DataLeakageDetector:
    ...
    def detect_mutual_info_leakage(self, df: pd.DataFrame, target_col: str = "actual_result") -> dict:
        ...
        try:
            ...
            for i, col in enumerate(numeric_cols):
                ...
                mi_results.append({"feature": col, "mutual_info": mi_score})
    ...
    def random_feature_detection(self, df: pd.DataFrame, sample_size: int = 20) -> dict:
        ...
        for col in sampled_features:
            # 检查标准差
            if df[col].std() == 0:
                suspicious.append({"feature": col, "issue": "constant", "severity": "MEDIUM"})
                continue
...
def main():
    """主函数 - 示例用法"""
    # 创建检测器
    detector = DataLeakageDetector(correlation_threshold=0.9, mutual_info_threshold=0.5)
