...
import src.streaming.kafka_consumer as kafka_consumer_module
...
async def test_start_consuming_processes_message_success(consumer_fixture):
    ...
    message.topic.return_value = "matches"
    message.partition.return_value = 0
    message.offset.return_value = 1

    async def process_side_effect(passed_msg):
        consumer.running = False
        return True

    process_mock = AsyncMock(side_effect=process_side_effect)

    with patch.object(consumer, "_process_message", process_mock):
        mock_consumer.poll.side_effect = [message]
        await asyncio.wait_for(consumer.start_consuming(timeout=0.01), timeout=1)

    process_mock.assert_awaited_once_with(message)
    mock_consumer.commit.assert_called_once_with(message)
...
async def test_start_consuming_parse_failure_triggers_retry(consumer_fixture):
    ...
    async def failure_side_effect(passed_msg):
        consumer.running = False
        return False
    ...
    warning_messages = [call.args[0] for call in consumer.logger.warning.call_args_list]
...
async def test_start_consuming_handles_broker_error(consumer_fixture):
    ...
    error = MagicMock()
    partition_eof = getattr(kafka_consumer_module.KafkaError, "_PARTITION_EOF", -191)
    if not isinstance(partition_eof, int):
        partition_eof = -191
    error.code.return_value = partition_eof + 1
    error.__str__.return_value = "broker unavailable"
    ...
    with patch("src.streaming.kafka_consumer.KafkaException", DummyKafkaException):
        await asyncio.wait_for(consumer.start_consuming(timeout=0.01), timeout=1)

    error_messages = [call.args[0] for call in consumer.logger.error.call_args_list]
    assert any("消费过程中出现错误" in msg for msg in error_messages)
    mock_consumer.commit.assert_not_called()
    mock_consumer.close.assert_called_once()
