...
class TestRawHTMLPreservation:
    ...
    def collector(self, temp_data_dir):
        ...
        with patch("src.data.collectors.fbref_collector.requests.Session"):
            ...
            # 覆盖数据目录为临时目录
            collector.raw_data_dir = (
                Path(temp_data_dir) / "data" / "raw_landing" / "fbref"
            )
            return collector
    ...
    @pytest.mark.asyncio
    async def test_raw_html_saving_should_create_file(
        self, collector, mock_html_content, temp_data_dir
    ):
        """
        ...
        with patch.object(collector, "fetch_html", return_value=mock_html_content):
            ...
            if not result.empty:
                ...
                if "raw_file_path" in result.columns:
                    ...
                    pytest.fail(
                        "DataFrame should contain 'raw_file_path' column after HTML saving implementation"
                    )
            else:
                ...
                if expected_base_dir.exists():
                    pytest.skip(
                        "HTML parsing failed but raw saving might be implemented - test skipped"
                    )
                else:
                    pytest.fail(
                        "Both HTML parsing and raw saving failed - test fails as expected"
                    )

    @pytest.mark.asyncio
    async def test_raw_html_file_content_correctness(
        self, collector, mock_html_content, temp_data_dir
    ):
        """
        测试保存的HTML文件内容正确性
    ...
    @pytest.mark.asyncio
    async def test_raw_html_saving_failure_isolation(
        self, collector, mock_html_content
    ):
        """
            ...
            with patch("gzip.open", side_effect=mock_file_save_failure):
                with patch("builtins.open", side_effect=mock_file_save_failure):
                    ...
                    if not result.empty:  # 如果解析成功
                        assert "home" in result.columns or any(
                            "home" in str(col).lower() for col in result.columns
                        )
    ...
    @pytest.mark.asyncio
    async def test_integration_with_existing_functionality(
        self, collector, mock_html_content
    ):
        """
        测试与现有功能的集成
