"""
Issue #83-C 数据驱动测试: domain.strategies.historical
覆盖率目标: 60% → 85%
...
class TestHistoricalDataDriven:
    """Issue #83-C 数据驱动测试 - domain.strategies.historical"""
    ...
    def sample_match_data(self):
        """示例比赛数据"""
        return {'home_team': 'Manchester United', 'away_team': 'Liverpool', 'home_score': 2, 'away_score': 1, 'match_date': '2024-01-15', 'league': 'Premier League', 'season': '2023-2024', 'venue': 'Old Trafford', 'attendance': 75000, 'weather': 'Clear', 'temperature': 15.5}

    @pytest.fixture
    def sample_prediction_data(self):
        """示例预测数据"""
        return {'match_id': 12345, 'home_win_prob': 0.65, 'draw_prob': 0.25, 'away_win_prob': 0.1, 'predicted_home_goals': 2.1, 'predicted_away_goals': 0.8, 'confidence': 0.85, 'model_version': 'v2.1', 'created_at': '2024-01-15T10:30:00Z'}

    @pytest.fixture
    def sample_team_stats(self):
        """示例球队统计数据"""
        return {'team_id': 1, 'team_name': 'Manchester United', 'matches_played': 25, 'wins': 15, 'draws': 6, 'losses': 4, 'goals_for': 42, 'goals_against': 18, 'points': 51, 'league_position': 3, 'form': 'WWDLW'}

    @pytest.mark.unit
    @pytest.mark.parametrize("scenario", [{'name': 'historical_performance', 'input': {'team_form': 'WWWWW', 'h2h_record': 'WWLDW'}, 'expected_output': {'performance_score': '>0.7'}}, {'name': 'ensemble_prediction', 'input': {'model_predictions': [0.6, 0.65, 0.58, 0.62]}, 'expected_output': {'final_prediction': '0.6-0.65'}}])
    def test_strategy_with_real_data_scenarios(self, scenario, sample_match_data, sample_team_stats):
        """测试策略使用真实数据场景"""
        try:
            import importlib
            module = importlib.import_module('domain.strategies.historical')
