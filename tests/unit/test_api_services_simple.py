...
class TestCoreExceptions:
    ...
    def test_model_error_inheritance(self):
        ...
        error = ModelError(
            ...
            details={"model_path": "/path/to/model.pkl"}
        )
...
class TestSimpleConfigurationFunctions:
    ...
    def test_simple_database_connection_string(self):
        ...
        class MockDatabaseSettings:
            def __init__(self, host="localhost", port=5432, name="test_db", user="test_user", password="test_pass"):
                self.host = host
                self.port = port
                self.name = name
                self.user = user
    ...
    def test_simple_fotmob_headers(self):
        ...
        settings = MockFotMobSettings(
            x_mas_header="env-mas-header",
            x_foo_header="env-foo-header"
        )

        headers = settings.get_headers()
        expected = {
            "X-MAS": "env-mas-header",
            "X-FOO": "env-foo-header"
        }
