...
def mock_prediction_service():
    ...
    mock_load_model.call_count = 0
    service.get_production_model_with_retry = mock_load_model
    return service
...
def mock_data_collector():
    ...
    collector.make_request = mock_make_request
    return collector
        ...
        for attempt in range(allowed_attempts):
            try:
                ...
                if allowed_attempts <= 3 or attempt == allowed_attempts - 1:
                    raise DatabaseError("数据库连接失败：超过最大重试次数") from error
        return db_manager
...
class TestMLflowRetry:
    ...
    async def test_mlflow_model_loading_max_retry_attempts(
        ...
        async def mock_load_model_always_fail(*args, **kwargs):
            raise MlflowException("Model loading failed")

        mock_prediction_service.get_production_model_with_retry = (
            mock_load_model_always_fail
        )
...
class TestExternalAPIRetry:
    ...
    async def test_external_api_call_max_retry_attempts(self, mock_data_collector):
        ...
        mock_data_collector.make_request = mock_make_request_always_fail
