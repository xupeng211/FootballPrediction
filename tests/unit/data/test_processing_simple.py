...
class TestFootballDataCleanerSimple:
    ...
    def test_clean_numeric_data_simple(self, dirty_data):
        ...
        cleaned_data = dirty_data[
            (dirty_data['home_score'] >= 0) & (dirty_data['home_score'] <= 20) &
            (dirty_data['away_score'] >= 0) & (dirty_data['away_score'] <= 20)
        ].copy()
...
class TestDataQualitySimple:
    ...
    def generate_quality_report_simple(self, sample_data):
        ...
        report = {
            'overall_score': (overall_completeness + (1 if not validity_issues else 0) +
                            (1 if not consistency_issues else 0) + freshness_score) / 4,
...
class TestDataProcessingIntegrationSimple:
    ...
    def test_end_to_end_processing_pipeline_simple(self):
        ...
        cleaned_data = deduplicated_data[
            (deduplicated_data['home_score'] >= 0) & (deduplicated_data['home_score'] <= 20) &
            (deduplicated_data['away_score'] >= 0) & (deduplicated_data['away_score'] <= 20)
        ].copy()
    ...
    def test_processing_performance_simple(self):
        ...
        cleaned_data = large_dataset[
            (large_dataset['home_score'] >= 0) & (large_dataset['home_score'] <= 10) &
            (large_dataset['away_score'] >= 0) & (large_dataset['away_score'] <= 10)
        ].copy()
    ...
    def test_data_quality_monitoring_simple(self):
        ...
        test_data = pd.DataFrame({
            ...
            'date': ['2024-01-01', '2024-01-02', '2024-01-03', 'invalid_date', '2024-01-05']
        })

        # 计算质量指标
        completeness_score = (test_data.count().sum()) / (len(test_data) * len(test_data.columns))
