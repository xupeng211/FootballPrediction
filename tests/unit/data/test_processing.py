...
class TestFootballDataCleaner:
    ...
    def sample_raw_data(self):
        ...
        return pd.DataFrame({
            ...
            'home_team': ['Team A', 'Team B', 'Team A', 'Team C', 'Team B', 'Team D'],
            'away_team': ['Team B', 'Team A', 'Team C', 'Team B', 'Team A', 'Team C'],
    ...
    def dirty_data(self):
        ...
        return pd.DataFrame({
            ...
            'home_team': ['Team A', 'Team B', 'Team A', 'Team C', 'Team B', 'Team D', 'Team A', 'Team E'],
            'away_team': ['Team B', 'Team A', 'Team C', 'Team B', 'Team A', 'Team C', 'Team B', 'Team F'],
            'home_score': [2, 1, 3, 0, 2, 1, -5, 100],  # 异常值：-5, 100
            'away_score': [1, 2, 0, 1, 2, 3, -1, 50],   # 异常值：-1, 50
            'date': ['2024-01-01', 'invalid_date', '2024-01-03', '2024-01-04', '2024-01-05', '2024-01-06', '2024-01-07', '2024-01-08'],
    ...
    def test_clean_numeric_data(self, cleaner, dirty_data):
        """测试清洗数值数据"""
        cleaned_data = cleaner.clean_numeric_data(dirty_data, ['home_score', 'away_score'])
    ...
    def test_clean_text_data(self, cleaner):
        ...
        dirty_text = pd.DataFrame({
            'team_name': ['  Team A  ', 'TEAM B', 'team c', 'Team-D', 'Team_E_F.C.'],
            'league': ['pl', 'Premier League', 'premier league', 'EPL', 'English Premier League']
        })
...
class TestMissingDataHandler:
    ...
    def data_with_missing(self):
        ...
        return pd.DataFrame({
            ...
            'date': ['2024-01-01', '2024-01-02', None, '2024-01-04', None, '2024-01-06', '2024-01-07', '2024-01-08']
        })
    ...
    def test_mean_imputation(self, handler, data_with_missing):
        """测试均值插补"""
        imputed_data = handler.mean_imputation(data_with_missing.copy(), ['home_score', 'away_score'])
        ...
        assert any(abs(val - mean_home) < 0.001 for val in imputed_data['home_score'] if pd.notna(val))

    def test_median_imputation(self, handler, data_with_missing):
        """测试中位数插补"""
        imputed_data = handler.median_imputation(data_with_missing.copy(), ['possession'])
        ...
        assert any(abs(val - median_possession) < 0.001 for val in imputed_data['possession'])

    def test_mode_imputation(self, handler, data_with_missing):
        """测试众数插补"""
        # 添加分类数据进行众数插补测试
        test_data = data_with_missing.copy()
        test_data['weather'] = ['sunny', 'rainy', None, 'sunny', 'cloudy', 'sunny', None, 'rainy']
    ...
    def test_interpolation_imputation(self, handler, data_with_missing):
        """测试插值法"""
        imputed_data = handler.interpolation_imputation(data_with_missing.copy(), ['shots'])
    ...
    def test_validate_imputation_quality(self, handler, data_with_missing):
        ...
        quality_report = handler.validate_imputation_quality(data_with_missing, imputed_data)
    ...
    def test_handle_high_missing_columns(self, handler):
        ...
        processed_data = handler.handle_high_missing_columns(high_missing_data, threshold=0.5)
...
class TestDataPreprocessor:
    ...
    def test_feature_engineering(self, preprocessor, raw_data):
        ...
        # 可能的新特征
        potential_features = ['goal_difference', 'total_goals', 'home_win', 'away_win', 'draw']
...
class TestDataQualityMonitor:
    ...
    def test_monitor_data_drift(self, monitor):
        ...
        baseline_data = pd.DataFrame({
            'score': [1, 2, 3, 4, 5],
            'possession': [50, 55, 60, 52, 58]
        })

        # 新数据（有轻微漂移）
        new_data = pd.DataFrame({
            'score': [2, 3, 4, 5, 6],
            'possession': [52, 57, 62, 54, 60]
        })
...
class TestAdvancedAnomalyDetector:
    ...
    def test_statistical_anomaly_detection(self, detector, data_with_anomalies):
        ...
        for idx in anomalies:
            ...
            assert abs(value - data_with_anomalies['home_score'].mean()) > 2 * data_with_anomalies['home_score'].std()
    ...
    def test_anomaly_explanation(self, detector, data_with_anomalies):
        """测试异常解释"""
        anomalies = detector.detect_statistical_anomalies(data_with_anomalies['home_score'])

        if anomalies:
            explanation = detector.explain_anomaly(
                data_with_anomalies.iloc[anomalies[0]],
                data_with_anomalies
            )
...
class TestDataProcessingIntegration:
    ...
    def test_end_to_end_processing_pipeline(self):
        ...
        raw_data = pd.DataFrame({
            ...
            'home_team': ['Team A', 'Team A', 'Team B', 'Team C', 'Team D', 'Team E', 'Team F', 'Team G', 'Team H', 'Team I'],
            'away_team': ['Team B', 'Team B', 'Team A', 'Team D', 'Team C', 'Team F', 'Team E', 'Team J', 'Team K', 'Team L'],
            'home_score': [2, 2, 1, None, 0, 2, 1, -5, 100, 3],  # 包含缺失和异常值
            'away_score': [1, 1, None, 0, 1, 2, 3, -1, 50, 1],   # 包含缺失和异常值
            'date': ['2024-01-01', '2024-01-01', 'invalid_date', '2024-01-03', None, '2024-01-05', '2024-01-06', '2024-01-07', '2024-01-08', '2024-01-09'],
        ...
        anomaly_report = detector.detect_anomalies_batch(processed_data)
