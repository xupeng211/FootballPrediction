...
class TestStreamingDataCollector:
    ...
    def test_streaming_collector_initialization_default(self, default_config):
        """测试默认配置初始化"""
    ...
    async def test_collect_with_streaming_disabled(self, sample_data):
        ...
        # Mock基础采集方法
        with patch.object(collector, "collect_fixtures") as mock_collect:
            mock_collect.return_value = CollectionResult(
                data_source="test_source",
                collection_type="fixtures",
                records_collected=len(sample_data),
    ...
    async def test_kafka_producer_send_error_handling(
        ...
        with patch(
            ...
            # Mock Kafka发送失败
            mock_kafka_producer.send.side_effect = Exception("Kafka send failed")
            mock_producer_class.return_value = mock_kafka_producer
            ...
            with patch.object(collector, "_send_to_stream") as mock_send_stream:
                ...
                with patch.object(
                    ...
                    mock_collect.return_value = CollectionResult(
                        data_source="test_source",
                        collection_type="fixtures",
                        records_collected=len(large_data),
                        success_count=len(large_data),
                        error_count=0,
                        status="success",
    ...
    async def test_topic_name_generation(self, mock_kafka_producer):
        ...
        with patch(
            ...
            # 验证主题名称格式
            if mock_kafka_producer.send.called:
    ...
    async def test_collect_with_retry_on_stream_failure(
        ...
        with patch(
            ...
            with patch(
                "src.data.collectors.base_collector.DataCollector.collect_fixtures",
    ...
    async def test_cleanup_resources(self, mock_kafka_producer):
        ...
        with patch(
            ...
            # 测试清理方法（如果存在）
            if hasattr(collector, "close") or hasattr(collector, "cleanup"):
                cleanup_method = getattr(collector, "close", None) or getattr(
                    collector, "cleanup", None
                )
    ...
    async def test_error_recovery_and_logging(self, mock_kafka_producer, sample_data):
        ...
        with patch(
            ...
            # Mock连续失败
            mock_kafka_producer.send.side_effect = Exception("Persistent failure")
            mock_producer_class.return_value = mock_kafka_producer

            collector = StreamingDataCollector(
                data_source="test_source", enable_streaming=True, max_retries=1
            )
