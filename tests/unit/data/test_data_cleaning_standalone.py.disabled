...
class TestDataCleaningStandalone:
    """数据清洗独立测试类"""
    ...
    def test_remove_duplicates_general(self, cleaner):
        """测试通用去重功能"""
        data = pd.DataFrame({"col1": [1, 2, 2, 3], "col2": ["a", "b", "b", "c"]})
    ...
    def test_handle_missing_values_numeric(self, cleaner):
        """测试数值型缺失值处理"""
        data = pd.DataFrame(
            {"numeric_col": [1, 2, np.nan, 4, 5], "text_col": ["A", "B", "C", "D", "E"]}
        )
    ...
    def test_handle_missing_values_text(self, cleaner):
        """测试文本型缺失值处理"""
        data = pd.DataFrame(
            {"numeric_col": [1, 2, 3, 4, 5], "text_col": ["A", "B", np.nan, "D", "E"]}
        )
    ...
    def test_edge_cases(self, cleaner):
        ...
        single_row = pd.DataFrame(
            {"match_id": [1], "home_score": [1], "away_score": [0]}
        )
