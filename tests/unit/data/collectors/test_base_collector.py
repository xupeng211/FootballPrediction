...
class TestCollectionResult:
    ...
    def test_collection_result_creation_full(self):
        ...
        result = CollectionResult(
            ...
            error_count=3,
            status="partial",
            error_message="Some data failed to collect",
        ...
        assert result.error_message == "Some data failed to collect"
...
class TestDataCollector:
    ...
    async def test_make_request_with_retry_success(self):
        ...
        with patch('aiohttp.ClientSession', return_value=mock_session):
            result = await collector._make_request_with_retry("https://api.test.com/data")
    ...
    async def test_make_request_with_retry_http_error(self):
        ...
        with patch('aiohttp.ClientSession', return_value=mock_session):
            with pytest.raises(aiohttp.ClientResponseError):
                await collector._make_request_with_retry("https://api.test.com/data")
    ...
    async def test_make_request_success(self):
        ...
        with patch.object(collector, '_make_request_with_retry', return_value={"data": "test"}) as mock_retry:
    ...
    async def test_make_request_with_retries(self):
        """测试HTTP请求带重试机制"""
        collector = MockDataCollector(data_source="test_source", max_retries=2, retry_delay=0.1)
        ...
        with patch.object(collector, '_make_request_with_retry', side_effect=mock_retry) as mock_retry_method:
            result = await collector._make_request("https://api.test.com/data")
    ...
    async def test_make_request_max_retries_exceeded(self):
        ...
        with patch.object(collector, '_make_request_with_retry', side_effect=Exception("Always fails")) as mock_retry:
    ...
    async def test_save_to_bronze_layer_success(self):
        ...
        with patch('src.database.models.raw_data.RawMatchData') as mock_model:
            mock_instance = MagicMock()
            mock_model.return_value = mock_instance

            test_data = [{"raw_data": {"match_id": 123}, "external_match_id": "ext_123"}]
        ...
        assert kwargs["data_source"] == "test_source"
        assert kwargs["raw_data"] == {"match_id": 123}
    ...
    async def test_save_to_bronze_layer_empty_data(self):
        ...
        mock_logger.assert_called_once_with("No data to save to raw_match_data")
    ...
    async def test_save_to_bronze_layer_invalid_table(self):
        ...
        with pytest.raises(ValueError, match="Unsupported table name: invalid_table"):
            await collector._save_to_bronze_layer("invalid_table", [{"data": "test"}])
    ...
    def test_is_duplicate_record_true(self):
        ...
        new_record = {"id": 1, "name": "test", "value": "A"}
        existing_records = [
            {"id": 1, "name": "test", "value": "B"},  # Same id and name
            {"id": 2, "name": "other", "value": "C"}
        ]

        result = collector._is_duplicate_record(new_record, existing_records, ["id", "name"])
    ...
    def test_is_duplicate_record_false(self):
        ...
        new_record = {"id": 3, "name": "test", "value": "A"}
        existing_records = [
    ...
    def test_is_duplicate_record_missing_fields(self):
        ...
        new_record = {"id": 1, "name": "test"}
        existing_records = [
            {"id": 1, "value": "A"}  # Missing 'name' field
        ]
    ...
    async def test_create_collection_log_success(self):
        ...
        with patch('src.database.models.data_collection_log.DataCollectionLog', return_value=mock_log_entry):
    ...
    async def test_create_collection_log_failure(self):
        ...
        with patch('src.database.models.data_collection_log.DataCollectionLog', side_effect=Exception("DB error")):
            with patch.object(collector.logger, 'error') as mock_logger:
                log_id = await collector._create_collection_log("fixtures")

        assert log_id == 0
        mock_logger.assert_called_once_with("Failed to create collection log: DB error")
    ...
    async def test_update_collection_log_success(self):
        ...
        with patch('src.database.models.data_collection_log.CollectionStatus') as mock_status:
            result = CollectionResult(
                data_source="test_source",
                collection_type="fixtures",
                records_collected=10,
    ...
    async def test_update_collection_log_invalid_log_id(self):
        ...
        mock_logger.assert_called_once_with("Invalid log_id (0), cannot update collection log.")
    ...
    async def test_collect_all_data_success(self):
        ...
        with patch.object(collector, '_create_collection_log', return_value=1) as mock_create, \
             patch.object(collector, '_update_collection_log') as mock_update:
    ...
    async def test_collect_all_data_with_failure(self):
        ...
        async def failing_collect_odds(**kwargs):
            raise Exception("Odds collection failed")

        collector.collect_odds = failing_collect_odds

        # Mock the log creation and update methods
        with patch.object(collector, '_create_collection_log', return_value=1) as mock_create_log, \
             patch.object(collector, '_update_collection_log') as mock_update:
        ...
        assert results["odds"].status == "failed"
        assert results["live_scores"].status == "success"

        # Verify the failed result has correct error message
        assert results["odds"].error_message == "Odds collection failed"
        assert results["odds"].success_count == 0
        assert results["odds"].error_count == 1
    ...
    async def test_collect_all_data_log_creation_failure(self):
        ...
        with patch.object(collector, '_create_collection_log', return_value=0), \
             patch.object(collector, '_update_collection_log') as mock_update:
