...
class TestOddsCollector:
    ...
    def test_init_with_custom_values(self):
        """测试自定义初始化"""
        collector = OddsCollector(
            data_source="custom_odds_api",
            api_key="custom_key",
            base_url="https://custom.api.com",
            time_window_minutes=10
        )

        assert collector.data_source == "custom_odds_api"
        assert collector.api_key == "custom_key"
        assert collector.base_url == "https://custom.api.com"
        assert collector.time_window_minutes == 10
    ...
    async def test_collect_odds_success(self):
        """测试成功采集赔率数据"""
        # Mock helper methods
        with patch.object(self.collector, '_get_active_bookmakers', return_value=["bet365", "pinnacle"]), \
             patch.object(self.collector, '_get_upcoming_matches', return_value=["1", "2"]), \
             patch.object(self.collector, '_clean_expired_odds_cache'), \
             patch.object(self.collector, '_collect_match_odds') as mock_collect_match, \
             patch.object(self.collector, '_has_odds_changed', return_value=True), \
             patch.object(self.collector, '_clean_odds_data') as mock_clean, \
            ...
            assert result.success_count == 2  # 2 matches * 1 market each
            assert result.error_count == 0
    ...
    async def test_collect_odds_with_custom_parameters(self):
        ...
        with patch.object(self.collector, '_clean_expired_odds_cache'), \
             patch.object(self.collector, '_collect_match_odds', return_value=[]), \
             patch.object(self.collector, '_save_to_bronze_layer') as mock_save:
    ...
    async def test_collect_odds_duplicate_prevention(self):
        ...
        with patch.object(self.collector, '_get_active_bookmakers', return_value=["bet365"]), \
             patch.object(self.collector, '_get_upcoming_matches', return_value=["1"]), \
             patch.object(self.collector, '_clean_expired_odds_cache'), \
             patch.object(self.collector, '_collect_match_odds', return_value=[odds_data]), \
             patch.object(self.collector, '_has_odds_changed', return_value=True), \
             patch.object(self.collector, '_clean_odds_data') as mock_clean, \
             patch.object(self.collector, '_save_to_bronze_layer'):
            ...
            assert result.status == "success"
            assert result.success_count == 0  # Should be skipped due to duplicate
            assert result.records_collected == 0
    ...
    async def test_collect_odds_no_change_detection(self):
        ...
        with patch.object(self.collector, '_get_active_bookmakers', return_value=["bet365"]), \
             ...
             patch.object(self.collector, '_clean_expired_odds_cache'), \
             patch.object(self.collector, '_collect_match_odds', return_value=[odds_data]), \
             patch.object(self.collector, '_has_odds_changed', return_value=False), \
             patch.object(self.collector, '_save_to_bronze_layer'):

            result = await self.collector.collect_odds()

            assert result.status == "success"
            assert result.success_count == 0  # Should be skipped due to no change
            assert result.records_collected == 0
    ...
    async def test_collect_odds_with_partial_success(self):
        ...
        with patch.object(self.collector, '_get_active_bookmakers', return_value=["bet365", "pinnacle"]), \
            ...
            mock_collect_match.return_value = [
                {"match_id": "1", "bookmaker": "bet365", "market_type": "h2h", "outcomes": [{"name": "home", "price": 2.5}]},  # Valid
                {"match_id": "2", "bookmaker": "pinnacle", "market_type": "h2h", "outcomes": []}  # Invalid (no outcomes)
            ]
            mock_clean.side_effect = [
                {"external_match_id": "1", "bookmaker": "bet365", "market_type": "h2h", "outcomes": [{"name": "home", "price": 2.5}]},  # Valid
                None  # Invalid
            ]
            ...
            assert result.error_count == 2  # Both invalid data processing error AND odds-level error are counted
    ...
    async def test_collect_odds_with_match_error(self):
        ...
        with patch.object(self.collector, '_get_active_bookmakers', return_value=["bet365"]), \
            ...
            def side_effect(match_id, bookmakers, markets):
                if match_id == "1":
                    return [{"match_id": "1", "bookmaker": "bet365", "market_type": "h2h", "outcomes": [{"name": "home", "price": 2.5}]}]
                else:
                    raise Exception("API Error")

            mock_collect_match.side_effect = side_effect
    ...
    async def test_get_active_bookmakers_success(self):
        ...
        expected_bookmakers = [
            "bet365", "pinnacle", "williamhill", "betfair", "unibet", "marathonbet"
        ]
    ...
    async def test_collect_match_odds_success(self):
        ...
        with patch.object(self.collector, '_make_request', return_value=mock_response):
            odds = await self.collector._collect_match_odds("1", ["bet365"], ["h2h"])
    ...
    def test_generate_odds_key(self):
        ...
        # Should be consistent for same data within same time window
        key2 = self.collector._generate_odds_key(odds_data)
        assert key == key2

        # Should be different for different data
        different_data = odds_data.copy()
        different_data["match_id"] = "54321"
        assert key != self.collector._generate_odds_key(different_data)
    ...
    async def test_has_odds_changed_different_odds(self):
        ...
        # Second call with different odds
        changed = await self.collector._has_odds_changed(odds_data2)
        assert changed is True
    ...
    async def test_clean_odds_data_success(self):
        ...
        assert cleaned["outcomes"][0]["price"] == 2.5
    ...
    async def test_clean_odds_data_invalid_price(self):
        ...
        assert cleaned is not None
        assert len(cleaned["outcomes"]) == 1
        assert cleaned["outcomes"][0]["name"] == "away"
...
class TestOddsCollectorEdgeCases:
    """测试赔率采集器边界情况"""
    ...
    async def test_collect_odds_empty_match_ids(self):
        """测试空比赛ID列表"""
        with patch.object(self.collector, '_get_upcoming_matches', return_value=[]):
            result = await self.collector.collect_odds()
    ...
    async def test_collect_odds_empty_bookmakers(self):
        """测试空博彩公司列表"""
        with patch.object(self.collector, '_get_active_bookmakers', return_value=[]):
            result = await self.collector.collect_odds(match_ids=["1"])
    ...
    async def test_collect_odds_api_timeout(self):
        ...
        with patch.object(self.collector, '_get_active_bookmakers', return_value=["bet365"]), \
             patch.object(self.collector, '_get_upcoming_matches', return_value=["1"]), \
             patch.object(self.collector, '_clean_expired_odds_cache'), \
             patch.object(self.collector, '_collect_match_odds', side_effect=TimeoutError("API timeout")):

            result = await self.collector.collect_odds()

            # When all matches fail due to timeout, status is "failed" since success_count = 0
            assert result.status == "failed"
    ...
    async def test_collect_odds_rate_limit(self):
        ...
        with patch.object(self.collector, '_get_active_bookmakers', return_value=["bet365"]), \
             patch.object(self.collector, '_get_upcoming_matches', return_value=["1"]), \
             patch.object(self.collector, '_clean_expired_odds_cache'), \
             patch.object(self.collector, '_collect_match_odds', side_effect=Exception("Rate limit exceeded")):

            result = await self.collector.collect_odds()

            # When all matches fail due to rate limit, status is "failed" since success_count = 0
            assert result.status == "failed"
