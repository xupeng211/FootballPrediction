...
def metadata_manager():
    ...
    with patch('requests.Session') as mock_session_class:
        ...
        mock_session_instance.headers = {"Content-Type": "application/json", "Accept": "application/json"}
        mock_session_class.return_value = mock_session_instance

        manager = MetadataManager(marquez_url="http://test:5000")
...
class TestMetadataManagerInitialization:
    ...
    def test_init_default_url(self):
        ...
        with patch('requests.Session') as mock_session_class:
            ...
            assert manager.session == mock_session_instance
    ...
    def test_init_custom_url(self):
        ...
        with patch('requests.Session') as mock_session_class:
            ...
            manager = MetadataManager(marquez_url="http://custom:8080")

            assert manager.marquez_url == "http://custom:8080"
            assert manager.base_url == "http://custom:8080"
            assert manager.api_url == "http://custom:8080/api/v1/"
...
class TestCreateNamespace:
    ...
    def test_create_namespace_success(self, metadata_manager):
        ...
        # Verify the call
        expected_url = "http://test:5000/api/v1/namespaces"
        expected_data = {
            "name": "test_namespace",
            "description": "Test namespace",
            "ownerName": "test_owner"
        }
        metadata_manager.session.post.assert_called_once_with(
            expected_url, json=expected_data
        )
    ...
    def test_create_namespace_minimal(self, metadata_manager):
        """Test namespace creation with minimal parameters."""
        mock_response = Mock()
        mock_response.status_code = 201
        mock_response.json.return_value = {"name": "minimal_namespace"}
        metadata_manager.session.post.return_value = mock_response

        result = metadata_manager.create_namespace(name="minimal_namespace")

        expected_data = {"name": "minimal_namespace"}
        metadata_manager.session.post.assert_called_once_with(
            "http://test:5000/api/v1/namespaces", json=expected_data
        )
        assert result["name"] == "minimal_namespace"

    def test_create_namespace_http_error(self, metadata_manager):
        """Test namespace creation with HTTP error."""
        mock_response = Mock()
        mock_response.status_code = 409
        mock_response.raise_for_status.side_effect = Exception("Namespace already exists")
        metadata_manager.session.post.return_value = mock_response

        with pytest.raises(Exception, match="Namespace already exists"):
            metadata_manager.create_namespace(name="existing_namespace")

    def test_create_namespace_connection_error(self, metadata_manager):
        """Test namespace creation with connection error."""
        metadata_manager.session.post.side_effect = Exception("Connection failed")
...
class TestGetNamespace:
    ...
    def test_get_namespace_success(self, metadata_manager):
        ...
        result = metadata_manager.get_namespace("test_namespace")

        expected_url = "http://test:5000/api/v1/namespaces/test_namespace"
        metadata_manager.session.get.assert_called_once_with(expected_url)
        assert result["name"] == "test_namespace"

    def test_get_namespace_not_found(self, metadata_manager):
        ...
        with pytest.raises(Exception, match="Not found"):
            metadata_manager.get_namespace("nonexistent_namespace")
...
class TestListNamespaces:
    ...
    def test_list_namespaces_empty(self, metadata_manager):
...
class TestCreateDataset:
    ...
    def test_create_dataset_success(self, metadata_manager):
        ...
        expected_url = "http://test:5000/api/v1/namespaces/test_namespace/datasets"
        expected_data = {
            "name": "test_dataset",
            "description": "Test dataset"
        }
    ...
    def test_create_dataset_with_fields(self, metadata_manager):
        """Test dataset creation with schema fields."""
        mock_response = Mock()
        mock_response.status_code = 201
        mock_response.json.return_value = {"name": "test_dataset"}
        metadata_manager.session.post.return_value = mock_response
        ...
        metadata_manager.session.post.assert_called_once_with(
            "http://test:5000/api/v1/namespaces/test_namespace/datasets",
            json=expected_data
        )
...
class TestGetDataset:
    ...
    def test_get_dataset_success(self, metadata_manager):
        ...
        expected_url = "http://test:5000/api/v1/namespaces/test_namespace/datasets/test_dataset"
        metadata_manager.session.get.assert_called_once_with(expected_url)
        assert result["name"] == "test_dataset"
...
class TestCreateJob:
    ...
    def test_create_job_success(self, metadata_manager):
        ...
        expected_url = "http://test:5000/api/v1/namespaces/test_namespace/jobs"
        expected_data = {
            "name": "test_job",
            "description": "Test job",
            "type": "BATCH"
...
class TestGetJob:
    ...
    def test_get_job_success(self, metadata_manager):
        ...
        expected_url = "http://test:5000/api/v1/namespaces/test_namespace/jobs/test_job"
        metadata_manager.session.get.assert_called_once_with(expected_url)
        assert result["name"] == "test_job"
...
class TestCreateRun:
    ...
    def test_create_run_success(self, metadata_manager):
        ...
        result = metadata_manager.create_run(
            namespace="test_namespace",
            job_name="test_job",
            nominal_start_time="2024-01-01T00:00:00Z",
            nominal_end_time="2024-01-01T01:00:00Z"
        )

        expected_url = "http://test:5000/api/v1/namespaces/test_namespace/jobs/test_job/runs"
        expected_data = {
            "nominalStartTime": "2024-01-01T00:00:00Z",
            "nominalEndTime": "2024-01-01T01:00:00Z"
        }
...
class TestGetRun:
    ...
    def test_get_run_success(self, metadata_manager):
        ...
        result = metadata_manager.get_run("test_namespace", "test_job", "test_run_id")

        expected_url = "http://test:5000/api/v1/namespaces/test_namespace/jobs/test_job/runs/test_run_id"
        metadata_manager.session.get.assert_called_once_with(expected_url)
        assert result["id"] == "test_run_id"
...
class TestListRuns:
    ...
    def test_list_runs_success(self, metadata_manager):
        ...
        mock_response.json.return_value = {
            "runs": [
                {"id": "run1", "nominalStartTime": "2024-01-01T00:00:00Z"},
                {"id": "run2", "nominalStartTime": "2024-01-02T00:00:00Z"}
            ]
        }
        metadata_manager.session.get.return_value = mock_response
...
class TestGetLatestRun:
    ...
    def test_get_latest_run_success(self, metadata_manager):
        ...
        result = metadata_manager.get_latest_run("test_namespace", "test_job")

        expected_url = "http://test:5000/api/v1/namespaces/test_namespace/jobs/test_job/runs/latest"
        metadata_manager.session.get.assert_called_once_with(expected_url)
        assert result["id"] == "latest_run_id"
...
class TestSearchMetadata:
    ...
    def test_search_metadata_success(self, metadata_manager):
        ...
        mock_response.json.return_value = {
            "results": [
                {"name": "dataset1", "type": "dataset"},
                {"name": "job1", "type": "job"}
            ]
        }
        metadata_manager.session.get.return_value = mock_response

        result = metadata_manager.search_metadata("test_query")

        expected_url = "http://test:5000/api/v1/search"
        expected_params = {"q": "test_query"}
        metadata_manager.session.get.assert_called_once_with(expected_url, params=expected_params)
        assert len(result["results"]) == 2

    def test_search_metadata_with_filters(self, metadata_manager):
        ...
        filters = {"namespace": "test_namespace", "type": "dataset"}
        result = metadata_manager.search_metadata("test_query", filters=filters)

        expected_params = {"q": "test_query", "namespace": "test_namespace", "type": "dataset"}
        metadata_manager.session.get.assert_called_once_with(
...
class TestHealthCheck:
    ...
    def test_health_check_success(self, metadata_manager):
        ...
        result = metadata_manager.health_check()

        expected_url = "http://test:5000/api/v1/health"
        metadata_manager.session.get.assert_called_once_with(expected_url)
        assert result["status"] == "healthy"

    def test_health_check_unhealthy(self, metadata_manager):
...
class TestMetadataManagerErrorHandling:
    ...
    def test_http_error_handling(self, metadata_manager):
        ...
        with pytest.raises(Exception, match="Internal server error"):
            metadata_manager.get_namespace("test_namespace")

    def test_connection_error_handling(self, metadata_manager):
        """Test connection error handling."""
        metadata_manager.session.get.side_effect = Exception("Connection refused")
    ...
    def test_timeout_error_handling(self, metadata_manager):
        ...
        metadata_manager.session.get.side_effect = Exception("Request timeout")
...
class TestMetadataManagerEdgeCases:
    ...
    def test_special_characters_in_parameters(self, metadata_manager):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"name": "test@namespace"}
        metadata_manager.session.get.return_value = mock_response

        result = metadata_manager.get_namespace("test@namespace")
        assert result["name"] == "test@namespace"


class TestMetadataManagerModule:
    """Test cases for metadata manager module imports and functionality."""

    def test_module_imports(self):
        """Test that the module can be imported successfully."""
        from src.lineage import metadata_manager
        assert hasattr(metadata_manager, 'MetadataManager')
    ...
    def test_class_methods_exist(self):
        ...
        methods = [
            ...
            'create_run', 'get_run', 'list_runs', 'get_latest_run',
            'search_metadata', 'health_check'
        ]

        for method in methods:
            assert hasattr(MetadataManager, method)