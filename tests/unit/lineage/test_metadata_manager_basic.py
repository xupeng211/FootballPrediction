...
class TestMetadataManager:
    ...
    assert hasattr(manager, "marquez_url")
    assert manager.marquez_url == "http:_/localhost:5000"

    def test_metadata_manager_custom_url(self):
        """测试自定义URL的元数据管理器初始化"""
    ...
    def test_create_namespace_basic(self, mock_put):
        """测试创建命名空间基本功能"""
        mock_response = Mock()
        ...
        manager = MetadataManager()
        result = manager.create_namespace("test_namespace", "Test namespace")
    ...
    @patch("src.lineage.metadata_manager.requests.Session.put")
    def test_create_dataset_basic(self, mock_put):
        """测试创建数据集基本功能"""
        mock_response = Mock()
        mock_response.status_code = 201
    ...
    @patch("src.lineage.metadata_manager.requests.Session.put")
    def test_create_job_basic(self, mock_put):
        """测试创建作业基本功能"""
        mock_response = Mock()
        mock_response.status_code = 201
        mock_response.json.return_value = {"name": "test_job"}
        mock_put.return_value = mock_response
...
class TestMetadataManagerUtilities:
    ...
    def test_build_url_basic(self):
        ...
        # 测试内部URL构建（如果有此方法）
        if hasattr(manager, "_build_url"):
            url = manager._build_url("_api/v1/namespaces")
    assert isinstance(url, str)
    assert "/api/v1/namespaces" in url

    @patch("src.lineage.metadata_manager.urljoin")
    def test_url_joining(self, mock_urljoin):
        """测试URL拼接功能"""
        mock_urljoin.return_value = "http:_/localhost:5000/api/v1/test"

        manager = MetadataManager()
        # 测试URL拼接是否被调用
        if hasattr(manager, "_build_url"):
            manager._build_url("/api/v1/test")
