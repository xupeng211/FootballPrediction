...
class TestJobRunLifecycle:
    ...
    def test_complete_job_run_minimal(self):
        """测试完成作业运行（最小参数）"""
        with patch(
            "src.lineage.lineage_reporter.OpenLineageClient"
        ) as mock_client_class:
...
class TestActiveRunsManagement:
    ...
    def test_get_active_runs(self):
        ...
        run_id1 = str(uuid4())
        run_id2 = str(uuid4())
        reporter._active_runs["job1"] = run_id1
        reporter._active_runs["job2"] = run_id2
    ...
    def test_mixed_success_failure_jobs(self):
        ...
        with patch(
            ...
            with patch.multiple(
                ...
                # 启动多个作业
                success_jobs = ["success_1", "success_2"]
                failed_jobs = ["failed_1", "failed_2"]
...
class TestLineageReporterErrorHandling:
        ...
        with patch(
            ...
            with patch.multiple(
                ...
                with pytest.raises(
                    ...
                    reporter.report_data_collection(
    ...
    def test_report_data_transformation_empty_sources(self):
        """测试报告数据转换（空数据源）"""
        with patch(
            "src.lineage.lineage_reporter.OpenLineageClient"
        ) as mock_client_class:
