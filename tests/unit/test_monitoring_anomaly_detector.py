...
def test_anomaly_result_to_dict():
    ...
    assert result_dict["anomaly_score"] == 0.5679  # Rounded to 4 decimal places
    assert result_dict["detection_method"] == "test_method"
    assert result_dict["description"] == "test description"


def test_detect_three_sigma_anomalies(anomaly_detector, sample_numeric_data):
...
def test_detect_range_anomalies(anomaly_detector):
    ...
    # Data with values outside defined ranges
    data = pd.Series([1, 2, 3, 150, -50, 5, 6])  # Some values outside normal range
    ...
    for result in results:
        ...
        assert result.detection_method == "range_check"
        assert isinstance(result.anomaly_score, float)
...
def test_detect_frequency_anomalies(anomaly_detector, sample_categorical_data):
    ...
    for result in results:
        ...
        assert result.detection_method == "frequency"
        assert isinstance(result.anomaly_score, float)
...
def test_detect_table_anomalies(anomaly_detector):
    ...
    with patch.object(anomaly_detector, '_get_table_data') as mock_get_data:
        sample_df = pd.DataFrame({
            ...
            'match_time': [datetime.now()] * 6,
            'match_status': ['scheduled', 'scheduled', 'live', 'finished', 'scheduled', 'scheduled']
        })
...
def test_detect_anomalies(anomaly_detector):
    ...
    with patch.object(anomaly_detector, '_detect_table_anomalies') as mock_detect_table:
        mock_detect_table.return_value = []
        
        results = anomaly_detector.detect_anomalies(["matches"])
...
def test_get_anomaly_summary(anomaly_detector):
    ...
    anomalies = [
        AnomalyResult(
            "table1", "col1", AnomalyType.OUTLIER, AnomalySeverity.HIGH, [1], 0.1, "method1", "desc1"
        ),
        AnomalyResult(
            "table2", "col2", AnomalyType.VALUE_RANGE, AnomalySeverity.CRITICAL, [2], 0.2, "method2", "desc2"
        )
    ]
    ...
    assert "outlier" in summary["by_type"] 
    assert "value_range" in summary["by_type"]
    assert "table1" in summary["by_table"]
    assert "table2" in summary["by_table"]


def test_get_anomaly_summary_empty(anomaly_detector):
    ...
    assert len(summary["by_table"]) == 0


def test_three_sigma_no_anomalies(anomaly_detector):
    """Test 3-sigma detection with no anomalies"""
    # Data with no real outliers
    data = pd.Series([10, 10.1, 9.9, 10.2, 9.8] * 20)
...
def test_frequency_no_anomalies(anomaly_detector):
    """Test frequency detection with no anomalies"""
    # Data with uniform frequency
    data = pd.Series(["A", "B", "C"] * 10)
...
def test_detect_time_gap_anomalies_zero_iqr(anomaly_detector):
    ...
    # Data with consistent intervals
    base_time = datetime.now()
    times = [base_time - timedelta(minutes=i) for i in range(10)]  # Consistent 1-min intervals
