...
class TestUnifiedCacheManager:
    ...
    def test_update_local_cache_lru_eviction(self, cache_manager):
        ...
        timestamp1 = cache_manager.local_cache[key1]['timestamp']

        # 添加第二个缓存项
        key2 = "test_type:key2"
        value2 = "value2"
        cache_manager._update_local_cache(key2, value2)
        timestamp2 = cache_manager.local_cache[key2]['timestamp']
    ...
    async def test_invalidate_pattern(self, cache_manager):
        ...
        for key, value in cache_items:
            cache_key = cache_manager._generate_cache_key(key[0], key[1])
        ...
        # 应该删除test_type下所有包含"key"的缓存项
        remaining_keys = [k for k in cache_manager.local_cache.keys() if not k.startswith("test_type:key")]
        assert len(remaining_keys) == 1
        assert "other_type:key1" in remaining_keys
    ...
    async def test_serialize_deserialize_error_handling(self, cache_manager):
        ...
        with patch('pickle.dumps', side_effect=Exception("Serialization error")):
            result = await cache_manager.set("key", "value", "type")
            assert result is False

        # 测试反序列化错误
        invalid_bytes = b"invalid pickle data"
        result = cache_manager._deserialize_data(invalid_bytes)
        assert result is None
...
class TestCacheDecorators:
    ...
    @pytest.mark.asyncio
    async def test_cached_decorator_sync_function(self, cache_manager):
        """测试缓存装饰器同步函数"""
        call_count = 0
    ...
    @pytest.mark.asyncio
    async def test_cached_decorator_async_function(self, cache_manager):
        """测试缓存装饰器异步函数"""
        call_count = 0
    ...
    @pytest.mark.asyncio
    async def test_performance_monitor_decorator(self, cache_manager):
        """测试性能监控装饰器"""
    ...
    @pytest.mark.asyncio
    async def test_combined_decorators(self, cache_manager):
        """测试组合装饰器"""
        call_count = 0
...
class TestCacheIntegration:
    ...
    @pytest.mark.asyncio
    async def test_cache_workflow(self, cache_manager):
        """测试完整缓存工作流"""
        # 1. 设置多个缓存项
        cache_items = [
            ("prediction:match_123", {"prediction": "home_win", "confidence": 0.75}),
            ("team_stats:team_456", {"played": 10, "won": 6, "draw": 1}),
            ("user_data:user_789", {"name": "Test User", "preferences": ["news", "notifications"]}),
        ]

        for key, value in cache_items:
            await cache_manager.set(key, value, "test")
        ...
        for key, expected_value in cache_items:
            cached_value = await cache_manager.get(key, "test")
        ...
        # 5. 清理特定模式的缓存
        await cache_manager.invalidate_pattern("team_stats", "test")

        # 6. 验证部分缓存被清除
        team_stats_cached = await cache_manager.get("team_stats:team_456", "test")
        assert team_stats_cached is None

        # 7. 其他缓存应该仍然存在
        prediction_cached = await cache_manager.get("prediction:match_123", "test")
    ...
    async def test_cache_performance(self, cache_manager):
        ...
        for i in range(operations):
            ...
            value = {"iteration": i, "data": "x" * 100}  # 较大的数据
            await cache_manager.set(key, value, "performance_test", ttl=300)
        ...
        for i in range(operations):
            key = f"performance_test_{i}"
            await cache_manager.get(key, "performance_test")
        ...
        assert stats['local_cache_size'] == min(operations, cache_manager.local_cache_size)