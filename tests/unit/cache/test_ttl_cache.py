...
class TestCacheEntry:
    ...
    def test_cache_entry_is_expired_with_ttl(self):
        ...
        ttl = timedelta(minutes=5)  # 5分钟TTL
    ...
    def test_cache_entry_is_not_expired_with_ttl(self):
        ...
        value = "test_value"
        created_at = datetime.now() - timedelta(minutes=2)  # 2分钟前创建
        ttl = timedelta(minutes=5)  # 5分钟TTL
    ...
    def test_cache_entry_is_not_expired_without_ttl(self):
        ...
        value = "test_value"
        created_at = datetime.now() - timedelta(days=365)  # 一年前创建
...
class TestTTLCache:
    ...
    async def test_ttl_cache_set_get_with_ttl(self, ttl_cache):
        ...
        key = "test_key"
        value = "test_value"
        ttl = timedelta(milliseconds=100)  # 100毫秒TTL
    ...
    async def test_ttl_cache_expiration(self, ttl_cache):
        ...
        key = "test_key"
        value = "test_value"
        ttl = timedelta(milliseconds=50)  # 50毫秒TTL
    ...
    async def test_ttl_cache_eviction_lru(self, ttl_cache):
        ...
        await small_cache.set("key4", "value4")  # 这应该触发LRU淘汰
