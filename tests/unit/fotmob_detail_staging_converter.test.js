...
test("C21: shuffled source index order produces identical outputs", async () => {
  ...
  const entries = pairs.map((p, i) =>
    sourceIndexEntry(
      p.payload.source_match_id,
      `/tmp/p${i}.payload.json`,
      `/tmp/p${i}.manifest.json`,
    ),
  );
  const run = async (order) => {
    const index = buildSourceIndex(order.map((i) => entries[i]));
    return convertAll({
      sourceIndex: index,
      loader: async (entry) => {
        const idx = entries.findIndex(
          (e) => e.source_match_id === entry.source_match_id,
        );
        return {
          ...pairArgs(pairs[idx]),
...
test("no wall clock: artifact generated_at comes from the manifest, observation_id is deterministic", () => {
  ...
  const b = convertPair(pairArgs(buildPair()));
  assert.strictEqual(
    a.artifact.generated_at,
    pair.manifest.response_received_at,
  );
  assert.strictEqual(a.artifact.observation_id, b.artifact.observation_id);
  assert.match(
    a.artifact.observation_id,
...
test("artifact preserves the five allowlisted sections verbatim (byte-faithful)", () => {
  ...
  for (const section of SECTIONS) {
    ...
    assert.strictEqual(
      result.artifact.sections[section].version,
      "fotmob-match-detail-parsed/v1",
    );
  }
  assert.deepStrictEqual(
    result.artifact.sections.player_stats.json,
    pair.payload.normalized.player_stats,
  );
  assert.deepStrictEqual(
    result.artifact.sections.events.json,
    pair.payload.normalized.events,
  );
});
...
test("quarantine-class observations carry quarantine status and code", () => {
  ...
  assert.strictEqual(
    result.terminal_state,
    TERMINAL_STATES.QUARANTINED_VALIDATION_FAIL,
  );
  ...
  const run = async () =>
    convertAll({
      ...
      loader: async (entry) =>
        pairArgs(
          pairs.find(
            (p) => p.payload.source_match_id === entry.source_match_id,
          ),
        ),
    });
  const r1 = await run();
