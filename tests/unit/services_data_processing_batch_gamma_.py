...
class TestDataProcessingServiceBatchGamma001:
    ...
    def sample_dataframe(self):
        ...
        return pd.DataFrame({
            ...
            'home_score': [2, 1, 0],
            'away_score': [1, 1, 2],
            'match_date': pd.to_datetime(['2025-01-15', '2025-01-16', '2025-01-17'])
        })
    ...
    @pytest.mark.asyncio
    async def test_process_raw_match_data_success(self, data_processing_service, sample_match_data):
        """测试处理原始比赛数据成功场景"""
        # Mock数据清洗和存储
        data_processing_service.data_cleaner.clean_match_data = AsyncMock(return_value=sample_match_data)
        ...
        assert result["processed_id"] == 1001
        assert result["message"] == "比赛数据处理完成"

        # 验证处理步骤被调用
        data_processing_service.data_cleaner.clean_match_data.assert_called_once_with(sample_match_data)
        data_processing_service.db_manager.execute.assert_called_once()
        data_processing_service.data_lake.store.assert_called_once()

    @pytest.mark.asyncio
    async def test_process_raw_match_data_failure(self, data_processing_service, sample_match_data):
        """测试处理原始比赛数据失败场景"""
        # Mock数据清洗失败
        data_processing_service.data_cleaner.clean_match_data = AsyncMock(side_effect=Exception("清洗失败"))

        result = await data_processing_service.process_raw_match_data(sample_match_data)

        # 验证处理失败
        assert result["success"] is False
        assert "清洗失败" in result["error"]

    @pytest.mark.asyncio
    async def test_process_raw_odds_data_success(self, data_processing_service, sample_odds_data):
        """测试处理原始赔率数据成功场景"""
        # Mock数据清洗和存储
        data_processing_service.data_cleaner.clean_odds_data = AsyncMock(return_value=sample_odds_data)
        data_processing_service.db_manager.execute = AsyncMock()
        ...
        assert result["success"] is True
        assert result["processed_id"] == 1001
        assert result["message"] == "赔率数据处理完成"

    @pytest.mark.asyncio
    async def test_process_features_data(self, data_processing_service, sample_dataframe):
        """测试处理特征数据"""
        # Mock特征处理
        data_processing_service.data_cleaner.extract_features = AsyncMock(return_value=sample_dataframe)
    ...
    @pytest.mark.asyncio
    async def test_process_batch_matches(self, data_processing_service, sample_match_data):
        """测试批量处理比赛数据"""
        batch_data = [sample_match_data, {**sample_match_data, "match_id": 1002}]

        # Mock批量处理
        data_processing_service._process_single_match_data = AsyncMock(side_effect=batch_data)
    ...
    @pytest.mark.asyncio
    async def test_validate_data_quality_success(self, data_processing_service, sample_dataframe):
        """测试数据质量验证成功场景"""
        # Mock质量验证
    ...
    @pytest.mark.asyncio
    async def test_validate_data_quality_failure(self, data_processing_service, sample_dataframe):
        """测试数据质量验证失败场景"""
        # Mock质量验证失败
        data_processing_service.data_cleaner.validate_data = AsyncMock(return_value={
            "is_valid": False,
    ...
    def test_process_text(self, data_processing_service):
        """测试文本处理"""
        text = "Team A vs Team B: 2-1"

        result = data_processing_service.process_text(text)
    ...
    async def test_process_batch(self, data_processing_service, sample_match_data):
        ...
        # Mock处理单个数据
        data_processing_service.process_raw_match_data = AsyncMock(
            side_effect=[
    ...
    async def test_process_bronze_to_silver(self, data_processing_service):
        """测试青铜层到银层数据处理"""
        # Mock各层数据处理
        data_processing_service._process_raw_matches_bronze_to_silver = AsyncMock(return_value=100)
        data_processing_service._process_raw_odds_bronze_to_silver = AsyncMock(return_value=50)
        data_processing_service._process_raw_scores_bronze_to_silver = AsyncMock(return_value=25)

        result = await data_processing_service.process_bronze_to_silver(batch_size=100)

        # 验证处理结果
        assert result["total_processed"] == 175
        assert result["matches_processed"] == 100
        assert result["odds_processed"] == 50
        assert result["scores_processed"] == 25
    ...
    async def test_get_bronze_layer_status(self, data_processing_service):
        ...
        # 验证状态信息
        assert result["matches_count"] == 100
        assert result["odds_count"] == 200
        assert result["scores_count"] == 150
        assert result["total_records"] == 450

    @pytest.mark.asyncio
    async def test_handle_missing_scores(self, data_processing_service, sample_dataframe):
        """测试处理缺失比分数据"""
        # Mock缺失值处理
        data_processing_service.missing_handler.handle_missing_scores = AsyncMock(return_value=sample_dataframe)
    ...
    async def test_detect_anomalies(self, data_processing_service, sample_dataframe):
        """测试异常检测"""
        # Mock异常检测
        data_processing_service.data_cleaner.detect_anomalies = AsyncMock(return_value=[
            {"type": "outlier", "value": 10, "field": "home_score"}
        ])
    ...
    @pytest.mark.asyncio
    async def test_store_processed_data(self, data_processing_service, sample_dataframe):
        """测试存储处理后的数据"""
        # Mock数据存储
        data_processing_service.data_lake.store = AsyncMock()
        data_processing_service.db_manager.execute = AsyncMock()

        result = await data_processing_service.store_processed_data(sample_dataframe, "matches")
    ...
    async def test_cache_processing_results(self, data_processing_service):
        ...
        # 验证缓存成功
        assert result["success"] is True
        data_processing_service.cache_manager.setex.assert_called_once()
    ...
    @pytest.mark.asyncio
    async def test_process_with_retry_success(self, data_processing_service, sample_match_data):
        """测试带重试的处理成功场景"""
        # Mock处理成功
    ...
    @pytest.mark.asyncio
    async def test_process_with_retry_failure(self, data_processing_service, sample_match_data):
        """测试带重试的处理失败场景"""
        # Mock处理失败
    ...
    async def test_collect_performance_metrics(self, data_processing_service):
        ...
        # 验证性能指标
        assert result["avg_processing_time"] == 1.75
    ...
    async def test_process_large_dataset(self, data_processing_service):
        ...
        # Mock分批处理
        data_processing_service._process_in_batches = AsyncMock(return_value={
            "processed": 1000,
            "failed": 0
        })
    ...
    def test_error_handling_strategies(self, data_processing_service):
        """测试错误处理策略"""
        # 验证服务能够优雅地处理各种错误情况
        assert hasattr(data_processing_service, 'logger')
        assert data_processing_service.name is not None
    ...
    async def test_concurrent_processing(self, data_processing_service):
        ...
        # 创建多个并发任务
        tasks = [
            data_processing_service.process_raw_match_data({"match_id": i})
            for i in range(5)
        ]
    ...
    def test_data_integrity_validation(self, data_processing_service, sample_match_data):
        """测试数据完整性验证"""
        # 验证数据结构完整性
        required_fields = ["match_id", "home_team", "away_team", "home_score", "away_score"]

        for field in required_fields:
            assert field in sample_match_data
    ...
    async def test_memory_efficiency(self, data_processing_service):
        ...
        data_processing_service._process_in_batches = AsyncMock(
            return_value={"processed": 10000, "failed": 0}
        )
