...
describe('L1ConfigManager', () => {
    ...
    it('seasonless 联赛与 J1 单年份历史页联赛应保留 canonical slug 与 URL 策略', () => {
        ...
        assert.deepStrictEqual(mls.additionalResultsPaths, ['/football/{country}/{league}/']);
        assert.deepStrictEqual(mls.additionalHistoricalResultsPaths, ['/football/{country}/{league}-{year}/']);
    ...
    it('辅助 getter 与字符串 helper 应覆盖默认和数组类型分支', () => {
        ...
        assert.deepStrictEqual(
            runtimeManager.getActiveLeagues({ tier: 'P1' }).map(league => league.id),
            [120]
        );
    ...
    it('expected matches 回退与配置异常分支应可预测', () => {
        ...
        const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'l1-config-'));
        ...
        try {
            ...
            assert.throws(() => runtimeManager._assertValidReconLeague('BROKEN', null), /联赛配置损坏: BROKEN/);
            assert.throws(
