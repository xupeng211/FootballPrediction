...
class TestSystemMetrics:
    ...
    def test_create_histogram_with_mock(self, metrics):
        ...
        histogram = metrics._create_histogram("test_histogram", "Test histogram", ["label"])
...
class TestSystemCollector:
    ...
    def test_get_system_info(self, collector):
        """测试获取系统信息"""
        with patch('psutil.boot_time', return_value=1609459200), \
...
class TestHealthChecker:
    ...
    async def test_check_all(self, checker):
        """测试执行所有检查"""
        # 模拟各个检查方法
        checker._check_database = AsyncMock(return_value={
            "status": "healthy",
            "message": "数据库正常"
        })
        checker._check_redis = AsyncMock(return_value={
            "status": "healthy",
            "message": "Redis正常"
        })
        checker._check_system_resources = AsyncMock(return_value={
            "status": "healthy",
            "message": "系统资源正常"
        })
        checker._check_application_health = AsyncMock(return_value={
            "status": "healthy",
            "message": "应用正常"
        })
        checker._check_external_services = AsyncMock(return_value={
            "status": "healthy",
            "message": "外部服务正常"
        })
        checker._check_data_pipeline = AsyncMock(return_value={
            "status": "healthy",
            "message": "数据管道正常"
        })
    ...
    async def test_check_database(self, checker):
        ...
        checker.database_collector.get_query_stats = MagicMock(return_value={
...
class TestGlobalFunctions:
    ...
    def test_record_functions(self):
        ...
        with patch('src.monitoring.system.get_system_monitor') as mock_get_monitor:
            ...
            mock_monitor.record_request.assert_called_once_with("GET", "/test", 200, 0.123)

            record_db_query("SELECT", "test", 0.456)
            mock_monitor.record_database_query.assert_called_once_with("SELECT", "test", 0.456, False)

            record_cache_op("get", "redis", "hit")
            mock_monitor.record_cache_operation.assert_called_once_with("get", "redis", "hit")
