...
class TestAnomalyResult:
    ...
    def test_anomaly_result_creation_minimal(self):
        ...
        assert result.description == "Outlier values detected"
    ...
    def test_anomaly_result_to_dict(self):
        ...
        result = AnomalyResult(
            ...
            description="Unknown value frequency anomaly",
        )
...
class TestAnomalyDetectorDetailed:
    """Detailed test cases for AnomalyDetector class."""

    @patch('src.monitoring.anomaly_detector.DatabaseManager')
    @patch('src.monitoring.anomaly_detector.logger')
    def test_anomaly_detector_initialization(self, mock_logger, mock_db_manager):
        ...
        mock_logger.info.assert_called_with("异常检测器初始化完成")
    ...
    def test_detection_config_structure(self):
        ...
        assert odds_config["thresholds"]["home_odds"]["min"] == 1.01
        ...
        assert predictions_config["thresholds"]["home_win_probability"]["max"] == 1.0

    @patch('src.monitoring.anomaly_detector.DatabaseManager')
    @patch('src.monitoring.anomaly_detector.logger')
    def test_detect_anomalies_table_error_handling(self, mock_logger, mock_db_manager):
        ...
        anomalies = detector.detect_anomalies.__wrapped__(detector, table_names=["matches"])

        assert isinstance(anomalies, list)
        mock_logger.error.assert_called_with("检测表 matches 异常失败: Database error")

    @patch('src.monitoring.anomaly_detector.logger')
    def test_detect_table_anomalies_no_config(self, mock_logger):
        """Test _detect_table_anomalies with no configuration."""
        detector = AnomalyDetector()
        mock_session = AsyncMock()

        anomalies = detector._detect_table_anomalies(mock_session, "nonexistent_table", ["three_sigma"])

        assert anomalies == []
        mock_logger.warning.assert_called_with("表 nonexistent_table 没有检测配置")

    @patch('src.monitoring.anomaly_detector.logger')
    def test_detect_table_anomalies_empty_data(self, mock_logger):
        ...
        with patch.object(detector, '_get_table_data', return_value=pd.DataFrame()):
            anomalies = detector._detect_table_anomalies(mock_session, "matches", ["three_sigma"])

        assert anomalies == []
        mock_logger.warning.assert_called_with("表 matches 没有数据")
    ...
    def test_get_table_data_success(self):
        ...
        mock_row = MagicMock()
        mock_row._mapping = {"id": 1, "home_score": 2, "away_score": 1}
        mock_result = MagicMock()
        mock_result.fetchall.return_value = [mock_row]
        mock_session.execute.return_value = mock_result

        data = detector._get_table_data.__wrapped__(detector, mock_session, "matches")
    ...
    @patch('src.monitoring.anomaly_detector.logger')
    def test_get_table_data_error(self, mock_logger):
        ...
        assert isinstance(data, pd.DataFrame)
        assert data.empty
        mock_logger.error.assert_called_with("获取表 matches 数据失败: Connection failed")

    def test_detect_column_anomalies_empty_data(self):
        """Test _detect_column_anomalies with empty data."""
        detector = AnomalyDetector()

        # Create DataFrame with empty column after dropna
        data = pd.DataFrame({"column": [None, None, None]})
        table_name = "matches"
        ...
        anomalies = detector._detect_column_anomalies(
            data, table_name, column_name, methods, column_type
        )
    ...
    def test_detect_three_sigma_anomalies_success(self):
        ...
        anomalies = detector._detect_three_sigma_anomalies(data, table_name, column_name)
    ...
    @patch('src.monitoring.anomaly_detector.logger')
    def test_detect_three_sigma_anomalies_error(self, mock_logger):
        """Test _detect_three_sigma_anomalies error handling."""
        detector = AnomalyDetector()

        # Create invalid data
        data = pd.Series(["invalid", "data"])
        table_name = "matches"
        column_name = "home_score"
    ...
    def test_detect_iqr_anomalies_success(self):
        ...
        anomalies = detector._detect_iqr_anomalies(data, table_name, column_name)
    ...
    @patch('src.monitoring.anomaly_detector.logger')
    def test_detect_iqr_anomalies_error(self, mock_logger):
    ...
    def test_detect_z_score_anomalies_success(self):
        ...
        anomalies = detector._detect_z_score_anomalies(data, table_name, column_name, threshold=2.0)
    ...
    def test_detect_z_score_anomalies_zero_std(self):
        ...
        anomalies = detector._detect_z_score_anomalies(data, table_name, column_name)

        assert len(anomalies) == 0

    @patch('src.monitoring.anomaly_detector.logger')
    def test_detect_z_score_anomalies_error(self, mock_logger):
    ...
    def test_detect_range_anomalies_success(self):
        ...
        detector.detection_config["matches"]["thresholds"]["home_score"] = {"min": 0, "max": 10}

        # Create data with range violations
        data = pd.Series([5, 15, -2, 8])  # 15 and -2 are out of range
        table_name = "matches"
        column_name = "home_score"

        anomalies = detector._detect_range_anomalies(data, table_name, column_name)
    ...
    @patch('src.monitoring.anomaly_detector.logger')
    def test_detect_range_anomalies_error(self, mock_logger):
    ...
    def test_detect_frequency_anomalies_success(self):
        ...
        # Create data with frequency anomalies
        data = pd.Series(["normal", "normal", "rare", "very_frequent", "very_frequent", "very_frequent", "very_frequent", "very_frequent"])
        table_name = "matches"
        column_name = "status"

        anomalies = detector._detect_frequency_anomalies(data, table_name, column_name)
    ...
    @patch('src.monitoring.anomaly_detector.logger')
    def test_detect_frequency_anomalies_error(self, mock_logger):
    ...
    def test_detect_time_gap_anomalies_success(self):
        """Test _detect_time_gap_anomalies with time gaps."""
        detector = AnomalyDetector()

        # Create time data with gaps
        ...
        anomalies = detector._detect_time_gap_anomalies(data, table_name, column_name)

        assert len(anomalies) >= 0  # May or may not detect depending on gap size

    def test_detect_time_gap_anomalies_insufficient_data(self):
        """Test _detect_time_gap_anomalies with insufficient data."""
        detector = AnomalyDetector()

        # Create time data with only one point
    ...
    @patch('src.monitoring.anomaly_detector.logger')
    def test_detect_time_gap_anomalies_error(self, mock_logger):
        """Test _detect_time_gap_anomalies error handling."""
        detector = AnomalyDetector()

        # Create invalid time data
        data = pd.Series(["invalid_time", "not_a_date"])
        table_name = "matches"
        column_name = "match_time"
    ...
    def test_get_anomaly_summary_with_data(self):
        ...
        summary = detector.get_anomaly_summary([anomaly1, anomaly2, anomaly3])
        ...
        assert summary["high_priority_anomalies"] == 3  # critical + high
...
class TestAnomalyDetectorIntegration:
    ...
    def test_complete_detection_workflow(self):
        ...
        test_data = pd.DataFrame({
            ...
            "match_status": ["completed", "completed", "scheduled", "completed", "completed", "scheduled"]
        })

        with patch.object(detector, '_get_table_data', return_value=test_data):
            anomalies = detector._detect_table_anomalies(
                mock_session, "matches", ["three_sigma", "iqr", "z_score", "range_check", "frequency"]
            )

        assert len(anomalies) > 0
        # Should detect the outlier in home_score
        home_score_anomalies = [a for a in anomalies if a.column_name == "home_score"]
        assert len(home_score_anomalies) > 0

        # Verify anomaly structure
        for anomaly in anomalies:
            assert isinstance(anomaly, AnomalyResult)
            assert anomaly.table_name == "matches"
            assert anomaly.detection_method in ["3sigma", "iqr", "z_score", "range_check", "frequency"]
            assert 0 <= anomaly.anomaly_score <= 1

    def test_multiple_tables_detection(self):
        ...
        # Create test data for different tables
        ...
        with patch.object(detector, '_get_table_data', side_effect=mock_get_table_data):
            anomalies = detector._detect_table_anomalies(mock_session, "matches", ["three_sigma"])
            odds_anomalies = detector._detect_table_anomalies(mock_session, "odds", ["range_check"])
    ...
    def test_error_resilience(self):
        ...
        # Test with data that will cause various types of errors
        ...
        with patch.object(detector, '_get_table_data', return_value=error_data):
            ...
            anomalies = detector._detect_table_anomalies(
                mock_session, "matches", ["three_sigma", "iqr", "z_score", "range_check"]
            )
    ...
    def test_severity_calculation_consistency(self):
        ...
        for score, expected_severity in test_cases:
            actual_severity = detector._calculate_severity(score)
            assert actual_severity == expected_severity, f"Score {score} should have severity {expected_severity}"

    def test_realistic_data_scenario(self):
        ...
        realistic_data = pd.DataFrame({
            "id": list(range(1, 21)),
            "home_score": [1, 2, 3, 1, 2, 0, 4, 2, 1, 3, 2, 1, 0, 3, 2, 1, 2, 25, 1, 2],  # 25 is clear outlier
            "away_score": [0, 1, 2, 1, 0, 1, 3, 1, 2, 1, 0, 1, 2, 1, 3, 0, 1, 2, 1, 0],
            "minute": [90, 90, 90, 85, 90, 88, 90, 90, 87, 90, 89, 90, 85, 90, 88, 90, 90, 150, 90, 86],  # 150 is outlier
            "match_status": ["completed"] * 19 + ["extra_time"],  # "extra_time" might be frequency anomaly
        })

        with patch.object(detector, '_get_table_data', return_value=realistic_data):
            anomalies = detector._detect_table_anomalies(
                mock_session, "matches", ["three_sigma", "iqr", "z_score", "range_check", "frequency"]
            )
        ...
        minute_anomalies = [a for a in anomalies if a.column_name == "minute"]
        assert len(minute_anomalies) > 0
