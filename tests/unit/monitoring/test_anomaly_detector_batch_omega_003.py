...
class TestAnomalyDetectorBatchOmega003:
    ...
    def test_detect_time_gap_anomalies_abnormal_intervals(self, detector):
        ...
        data = pd.Series([
            ...
            base_time + timedelta(minutes=1),
            base_time + timedelta(minutes=2),
            base_time + timedelta(hours=1)  # 异常长的间隔
        ])
    ...
    def test_detect_column_anomalies_categorical_column(self, detector, sample_data):
        ...
        anomalies = detector._detect_column_anomalies(
            sample_data, "matches", "match_status", methods, "categorical"
        )
    ...
    async def test_detect_anomalies_error_handling(self, detector):
        ...
        with patch.object(detector.db_manager, 'get_async_session') as mock_get_session:
            ...
            with patch.object(detector, '_detect_table_anomalies', side_effect=Exception("Detection error")):
    ...
    def test_detection_method_coverage(self, detector):
        ...
        for method in methods:
            if method == detector._detect_time_gap_anomalies:
                result = method(time_data, "test_table", "test_column")
            else:
