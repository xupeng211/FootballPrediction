...
class TestQualityMonitorBatchOmega001:
    ...
    def sample_completeness_result(self):
        ...
        return DataCompletenessResult(
            table_name="matches",
            total_records=1000,
            missing_critical_fields={"home_score": 50, "away_score": 30},
            missing_rate=0.08,
    ...
    def test_data_completeness_result_initialization(self):
        ...
        missing_fields = {"home_score": 50, "away_score": 30}
    ...
    async def test_calculate_overall_quality_score(self, monitor):
        ...
        freshness_results = {
            ...
            "odds": Mock(is_fresh=False, records_count=500)
        }

        # 模拟完整性检查结果
        completeness_results = {
            "matches": Mock(completeness_score=0.95),
            "odds": Mock(completeness_score=0.80)
        }

        with patch.object(monitor, 'check_data_freshness', return_value=freshness_results), \
             patch.object(monitor, 'check_data_completeness', return_value=completeness_results):
    ...
    def test_missing_rate_calculation(self):
        ...
        # 8%缺失率
        result1 = DataCompletenessResult(
            table_name="test",
            total_records=1000,
            missing_critical_fields={"field1": 80},
            missing_rate=0.08,
            completeness_score=0.92
        )
        ...
        result2 = DataCompletenessResult(
            table_name="test",
            total_records=1000,
            missing_critical_fields={"field1": 1000},
            missing_rate=1.0,
    ...
    def test_large_numbers_handling(self):
        ...
        result2 = DataCompletenessResult(
            ...
            missing_critical_fields={"big_field": 100000},
            missing_rate=0.1,
    ...
    def test_threshold_boundary_conditions(self):
        ...
        result = DataFreshnessResult(
            table_name="test",
            last_update_time=boundary_time,
            records_count=100,
            freshness_hours=threshold_hours,
    ...
    async def test_concurrent_access(self, monitor):
        ...
        with patch.object(monitor.db_manager, 'get_async_session') as mock_get_session, \
            ...
            tasks = []
            for i in range(5):
                task = monitor.check_data_freshness(["matches"])
                tasks.append(task)
