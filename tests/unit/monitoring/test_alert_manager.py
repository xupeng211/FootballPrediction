...
class TestAlert:
    ...
    def test_alert_creation_with_all_parameters(self):
        ...
        alert = Alert(
            ...
            message="Complete alert with all params",
        ...
        assert alert.alert_id == "test-456"
    ...
    def test_alert_silence(self):
        ...
        alert = Alert(
            alert_id="test-silence",
            title="Silence Test",
            message="Test silencing",
            level=AlertLevel.INFO,
            source="test",
        )
...
class TestAlertRule:
    ...
    def test_alert_rule_defaults(self):
        ...
        assert rule.throttle_seconds == 300  # Default value
...
class TestPrometheusMetrics:
    ...
    @patch('src.monitoring.alert_manager.REGISTRY')
    def test_metrics_use_custom_registry(self, mock_registry):
...
class TestAlertManager:
    ...
    def test_alert_manager_initialization(self, mock_logger):
    ...
    @patch('src.monitoring.alert_manager.logger')
    def test_alert_manager_initializes_default_rules(self, mock_logger):
    ...
    def test_register_handler(self, mock_logger):
        ...
        mock_logger.info.assert_called_with("注册告警处理器: log")
    ...
    def test_add_rule(self):
        ...
        assert manager.rules["custom_rule"] == new_rule
    ...
    @patch('src.monitoring.alert_manager.logger')
    def test_fire_alert_success(self, mock_logger):
    ...
    @patch('src.monitoring.alert_manager.logger')
    def test_fire_alert_throttled(self, mock_logger):
    ...
    def test_generate_alert_id(self):
        ...
        # Test without labels
        id1 = manager._generate_alert_id("Test Alert", "test", None)
        assert len(id1) == 12

        # Test with labels
        labels = {"env": "test", "service": "api"}
        id2 = manager._generate_alert_id("Test Alert", "test", labels)
        assert len(id2) == 12
        assert id1 != id2  # Should be different due to labels

        # Test consistent input produces consistent ID
        id3 = manager._generate_alert_id("Test Alert", "test", labels)
        assert id2 == id3

    def test_should_throttle_no_rule(self):
        """Test throttling logic when no rule is specified."""
        manager = AlertManager()

        result = manager._should_throttle("test-alert", None)
        assert result is False

    def test_should_throttle_nonexistent_rule(self):
        """Test throttling logic for nonexistent rule."""
        manager = AlertManager()

        result = manager._should_throttle("test-alert", "nonexistent_rule")
        assert result is False

    def test_should_throttle_never_fired(self):
        """Test throttling logic for rule that has never fired."""
        manager = AlertManager()
        rule = manager.rules["data_freshness_critical"]
        assert rule.last_fired is None

        result = manager._should_throttle("test-alert", "data_freshness_critical")
        assert result is False

    def test_should_throttle_within_window(self):
        """Test throttling logic within throttle window."""
        manager = AlertManager()
        rule = manager.rules["data_freshness_critical"]
        rule.last_fired = datetime.now() - timedelta(minutes=10)  # 10 minutes ago

        result = manager._should_throttle("test-alert", "data_freshness_critical")
        assert result is True  # Should be throttled (1800 second window)

    def test_should_throttle_outside_window(self):
        """Test throttling logic outside throttle window."""
        manager = AlertManager()
        rule = manager.rules["data_freshness_warning"]
        rule.last_fired = datetime.now() - timedelta(minutes=61)  # 61 minutes ago

        result = manager._should_throttle("test-alert", "data_freshness_warning")
        assert result is False  # Should not be throttled (3600 second window)

    @patch('src.monitoring.alert_manager.logger')
    def test_send_alert_with_rule_channels(self, mock_logger):
        ...
        manager._send_alert(alert, "data_freshness_critical")  # Has LOG and PROMETHEUS

        mock_handler.assert_called_once_with(alert)

    @patch('src.monitoring.alert_manager.logger')
    def test_send_alert_without_rule(self, mock_logger):
        ...
        alert = Alert(
            alert_id="test-send-no-rule",
            title="Send Test No Rule",
            message="Test sending without rule",
            level=AlertLevel.INFO,
            source="test",
        )

        manager._send_alert(alert, None)  # No rule specified

        mock_handler.assert_called_once_with(alert)

    @patch('src.monitoring.alert_manager.logger')
    def test_send_alert_handler_error(self, mock_logger):
        ...
        manager.register_handler(AlertChannel.LOG, error_handler)
    ...
    def test_update_alert_metrics(self):
        ...
        manager = AlertManager()
        alert = Alert(
            alert_id="test-metrics",
            title="Metrics Test",
            message="Test metrics update",
            level=AlertLevel.WARNING,
            source="test",
        )
    ...
    def test_log_handler_info_level(self):
        ...
        with patch('src.monitoring.alert_manager.logger') as mock_logger:
            ...
            mock_logger.log.assert_called_with(
                logging.INFO,
                "[ALERT] Log Info Test: Test info logging (ID: test-log-info, Source: test, Labels: {'env': 'test'})"
            )

    def test_log_handler_critical_level(self):
        """Test log handler for CRITICAL level alerts."""
        manager = AlertManager()

        with patch('src.monitoring.alert_manager.logger') as mock_logger:
            alert = Alert(
                alert_id="test-log-critical",
                title="Log Critical Test",
                message="Test critical logging",
                level=AlertLevel.CRITICAL,
                source="test",
            )
            ...
            mock_logger.log.assert_called_with(
                logging.CRITICAL,
                "[ALERT] Log Critical Test: Test critical logging (ID: test-log-critical, Source: test, Labels: {})"
            )

    def test_prometheus_handler(self):
        """Test Prometheus handler."""
        manager = AlertManager()
        alert = Alert(
            alert_id="test-prometheus",
            title="Prometheus Test",
            message="Test Prometheus handler",
            level=AlertLevel.WARNING,
            source="test",
        )
    ...
    def test_resolve_alert_success(self):
        ...
        result = manager.resolve_alert("test-resolve-alert")
    ...
    def test_resolve_alert_not_found(self):
        ...
        result = manager.resolve_alert("nonexistent-alert")
    ...
    def test_resolve_alert_already_resolved(self):
        ...
        manager = AlertManager()
        alert = Alert(
            alert_id="test-already-resolved",
            title="Already Resolved",
            message="Test already resolved alert",
            level=AlertLevel.INFO,
            source="test",
        )
        alert.resolve()
        ...
        result = manager.resolve_alert("test-already-resolved")

        assert result is False  # Should not resolve already resolved alert
    ...
    def test_get_active_alerts_all(self):
        ...
        # Create test alerts
        active_alert1 = Alert("id1", "Active 1", "msg1", AlertLevel.INFO, "test")
        active_alert2 = Alert("id2", "Active 2", "msg2", AlertLevel.WARNING, "test")
        resolved_alert = Alert("id3", "Resolved", "msg3", AlertLevel.INFO, "test")
        resolved_alert.resolve()

        manager.alerts = [active_alert1, active_alert2, resolved_alert]
    ...
    def test_get_active_alerts_filtered_by_level(self):
        ...
        active_info = Alert("id1", "Info", "msg1", AlertLevel.INFO, "test")
        active_warning = Alert("id2", "Warning", "msg2", AlertLevel.WARNING, "test")
        active_critical = Alert("id3", "Critical", "msg3", AlertLevel.CRITICAL, "test")

        manager.alerts = [active_info, active_warning, active_critical]

        warning_alerts = manager.get_active_alerts(level=AlertLevel.WARNING)

        assert len(warning_alerts) == 1
        assert warning_alerts[0] == active_warning
    ...
    def test_get_alert_summary(self):
        ...
        # Create test alerts
        active_critical = Alert("id1", "Critical", "msg1", AlertLevel.CRITICAL, "source1")
        active_warning = Alert("id2", "Warning", "msg2", AlertLevel.WARNING, "source2")
        resolved_alert = Alert("id3", "Resolved", "msg3", AlertLevel.INFO, "source1")
        resolved_alert.resolve()

        manager.alerts = [active_critical, active_warning, resolved_alert]
    ...
    def test_update_quality_metrics_freshness(self):
        ...
        manager.metrics.data_freshness_hours.labels.assert_has_calls([
            call(table_name="matches"),
            call(table_name="teams"),
        ])
    ...
    def test_update_quality_metrics_overall_score(self):
        ...
        manager.metrics.data_quality_score.labels.assert_called_with(table_name="matches")
    ...
    def test_update_anomaly_metrics(self):
        ...
        class MockAnomaly:
            def __init__(self):
                ...
                self.column_name = "home_score"
                self.anomaly_type = MagicMock()
                self.anomaly_type.value = "outlier"
                self.severity = MagicMock()
                self.severity.value = "high"
        ...
        manager.metrics.anomalies_detected_total.labels.assert_called_with(
            table_name="matches", column_name="home_score",
            anomaly_type="outlier", severity="high"
        )
        manager.metrics.anomaly_score.labels.assert_called_with(
            table_name="matches", column_name="home_score"
        )
    ...
    def test_check_and_fire_anomaly_alerts_critical(self):
        ...
        class MockAnomaly:
            def __init__(self):
                ...
                self.anomaly_type = MagicMock()
                self.anomaly_type.value = "outlier"
                self.anomaly_score = 0.25  # > 0.2
                self.description = "High score detected"

        anomalies = [MockAnomaly()]

        alerts = manager.check_and_fire_anomaly_alerts(anomalies)
    ...
    def test_check_and_fire_anomaly_alerts_warning(self):
        ...
        class MockAnomaly:
            def __init__(self):
                ...
                self.anomaly_type = MagicMock()
                self.anomaly_type.value = "outlier"
                self.anomaly_score = 0.15  # > 0.1
                self.description = "Medium score detected"
    ...
    def test_check_and_fire_anomaly_alerts_low_score(self):
        ...
        class MockAnomaly:
            def __init__(self):
                ...
                self.anomaly_type = MagicMock()
                self.anomaly_type.value = "outlier"
                self.anomaly_score = 0.05  # < 0.1
                self.description = "Low score detected"
...
class TestAlertManagerIntegration:
    ...
    @patch('src.monitoring.alert_manager.logger')
    def test_full_alert_workflow(self, mock_logger):
    ...
    @patch('src.monitoring.alert_manager.logger')
    def test_quality_monitoring_workflow(self, mock_logger):
        ...
        assert summary["critical_alerts"] == 3  # freshness, completeness, score
        assert summary["by_level"]["warning"] == 1  # freshness warning

    @patch('src.monitoring.alert_manager.logger')
    def test_handler_error_handling(self, mock_logger):
