...
class TestEdgeCasesAndFailureScenarios:
    ...
    def prediction_service(
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch(
                    "src.models.prediction_service.DatabaseManager",
    ...
    async def test_predict_match_with_invalid_id(self, prediction_service):
        ...
        with patch.object(
            ...
            mock_model.predict = Mock(return_value=np.array(["draw"]))
            mock_get_model.return_value = (mock_model, "1.0")

            with pytest.raises(ValueError, match="比赛 0 不存在"):
                await prediction_service.predict_match(0)

            with pytest.raises(ValueError, match="比赛 -1 不存在"):
    ...
    async def test_database_query_timeout(self, prediction_service):
        ...
        mock_session.execute.side_effect = TimeoutError("数据库查询超时")
    ...
    async def test_database_transaction_rollback(self, prediction_service):
        ...
        # Mock数据库提交失败
        mock_session = AsyncMock()
        mock_session.commit.side_effect = Exception("数据库提交失败")
        mock_session_context = AsyncMock()
        mock_session_context.__aenter__.return_value = mock_session
    ...
    async def test_model_loading_failure(
        ...
        # 获取模型应该失败
        with pytest.raises(Exception, match="模型文件损坏"):
            await prediction_service.get_production_model()
    ...
    async def test_concurrent_model_loading(
        ...
        mock_model.predict_proba = Mock(return_value=np.array([[0.33, 0.33, 0.34]]))
        mock_model.predict = Mock(return_value=np.array(["draw"]))
        mock_mlflow_module.sklearn.load_model.return_value = mock_model
