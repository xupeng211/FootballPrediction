...
class TestPredictionResult:
    ...
    def test_prediction_result_to_dict_none_dates(self):
        """测试预测结果转换为字典（日期为None）"""
        result = PredictionResult(match_id=1, model_version="v1.0")
...
class TestPredictionService:
    ...
    def test_prepare_features_missing_values(self, prediction_service):
        """测试缺失特征值的处理"""
        features = {"home_recent_wins": 3}  # 只有部分特征
    ...
    async def test_store_prediction_success(self, prediction_service, mock_db_manager):
        ...
        await prediction_service._store_prediction(result)
    ...
    async def test_predict_match_no_match(self, prediction_service):
        ...
        ), patch.object(prediction_service, "_get_match_info", return_value=None):
            with pytest.raises(ValueError, match="比赛 999 不存在"):
                await prediction_service.predict_match(999)
    ...
    async def test_verify_prediction_success(self, prediction_service, mock_db_manager):
        ...
        mock_match_result.first.return_value = mock_match

        mock_session.execute.side_effect = [mock_match_result, Mock()]
