...
class TestBaselineModelTrainer:
    ...
    def mock_db_manager(self):
        """模拟数据库管理器"""
        with patch("src.models.model_training.DatabaseManager") as mock:
            mock.return_value.get_async_session = MagicMock()
            yield mock.return_value

    @pytest.fixture
    def mock_feature_store(self):
        """模拟特征存储"""
        with patch("src.models.model_training.FootballFeatureStore") as mock:
            yield mock.return_value

    @pytest.fixture
    def mock_mlflow(self):
        """模拟MLflow模块"""
        with patch("src.models.model_training.mlflow") as mock:
            mock.start_run.return_value.__enter__.return_value.info.run_id = (
                "test_run_id"
            )
            yield mock
    ...
    def mock_mlflow_client(self):
        """模拟MlflowClient"""
        with patch("src.models.model_training.MlflowClient") as mock:
            yield mock.return_value
    ...
    def trainer(
        ...
        with patch("src.models.model_training.mlflow.set_tracking_uri"):
            trainer_instance = BaselineModelTrainer()
            trainer_instance.db_manager = mock_db_manager
            trainer_instance.feature_store = mock_feature_store
    ...
    async def test_prepare_training_data_success(self, trainer, mock_feature_store):
        ...
        matches_df = pd.DataFrame(
            {
                "id": range(100),
                "home_team_id": range(100),
                "away_team_id": range(100, 200),
                "match_time": pd.to_datetime(
                    pd.date_range(start=start_date, periods=100, freq="h")
                ),
                "result": ["home"] * 50 + ["draw"] * 25 + ["away"] * 25,
            }
        )
    ...
    async def test_prepare_training_data_insufficient_samples(self, trainer):
        ...
        matches_df = pd.DataFrame({"id": range(10)})  # Only 10 samples
        trainer._get_historical_matches = AsyncMock(return_value=matches_df)

        with pytest.raises(ValueError, match="训练数据不足"):
            await trainer.prepare_training_data(start_date, end_date, min_samples=100)
    ...
    async def test_prepare_training_data_feature_store_fallback(
        ...
        simplified_features_df = pd.DataFrame(
            {"match_id": range(100), "home_recent_wins": [2] * 100}
        )
    ...
    async def test_get_historical_matches_with_mock_query(self, trainer):
        ...
        mock_query_result.fetchall.return_value = [
            MockMatch(1, 10, 20, 1, datetime(2023, 1, 1), 2, 1, "2023"),
            MockMatch(2, 30, 40, 1, datetime(2023, 1, 2), 0, 3, "2023"),
        ]
        mock_session.execute.return_value = mock_query_result
        trainer.db_manager.get_async_session.return_value.__aenter__.return_value = (
            mock_session
        )
    ...
    async def test_get_simplified_features_success(self, trainer):
        ...
        async def mock_calculate_team_features(session, team_id, match_time):
            return {"recent_wins": 3, "recent_goals_for": 8, "recent_goals_against": 4}

        trainer._calculate_team_simple_features = mock_calculate_team_features
    ...
    async def test_calculate_team_simple_features(self, trainer):
        ...
        mock_query_result = Mock()
        mock_scalars = Mock()
        mock_scalars.all.return_value = [mock_match1, mock_match2, mock_match3]
    ...
    def test_get_model_performance_summary(self, trainer):
        """测试获取模型性能摘要"""
        run_id = "test_run_123"
        ...
        mock_client = Mock()
        mock_run = Mock()
        mock_run.data.metrics = {"accuracy": 0.85, "precision": 0.82, "recall": 0.88}
        mock_run.data.params = {"n_estimators": "100", "max_depth": "6"}
