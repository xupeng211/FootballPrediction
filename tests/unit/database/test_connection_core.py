...
class TestDatabaseManagerCore:
    ...
    def test_manager_initialization(self, db_manager):
        ...
        # 测试基本属性存在
        try:
            _ = getattr(db_manager, "engine", None)
            # async_engine 在未初始化时会抛出 RuntimeError，这是预期行为
            try:
                _ = getattr(db_manager, "async_engine", None)
            except RuntimeError:
                pass  # 预期的异常，数据库未初始化
            _ = getattr(db_manager, "session_factory", None)
            _ = getattr(db_manager, "async_session_factory", None)
    ...
    async def test_initialize_with_error(self, db_manager):
        ...
        try:
            ...
            with patch(
                ...
                mock_conn_str.side_effect = Exception("Configuration error")
    ...
    async def test_close_all_connections_basic(self):
        ...
        try:
            ...
            with patch("src.database.connection.engine") as mock_engine:
                with patch("src.database.connection.async_engine") as mock_async_engine:
    ...
    async def test_database_health_check_success(self):
        ...
        try:
            ...
            with patch.object(db_manager, "test_connection") as mock_test:
                mock_test.return_value = {"status": "healthy", "response_time": 0.05}
...
if __name__ == "__main__":
    # 运行测试
    pytest.main([__file__, "-v", "--cov=src.database.connection"])
