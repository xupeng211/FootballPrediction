...
class TestFootballKafkaProducer:
    ...
    def test_serialize_message_string(self):
        """测试字符串消息序列化"""
        test_data = "Simple string message"
    ...
    def test_delivery_callback_failure(self):
        """测试消息投递失败回调"""
        mock_error = Mock()
        mock_error.str.return_value = "Message delivery failed"

        mock_message = Mock()
        mock_message.error.return_value = mock_error

        # 不应该抛出异常，但应该记录错误
        with patch.object(self.producer.logger, "error") as mock_log_error:
            self.producer._delivery_callback(mock_error, mock_message)
    ...
    async def test_send_batch_partial_failure(self, mock_producer_class):
        ...
        mock_producer = Mock()
        # 第二次调用produce时抛出异常
        mock_producer.produce.side_effect = [None, KafkaException("Error"), None]
        mock_producer.poll = Mock()
        mock_producer.flush = Mock()
        mock_producer_class.return_value = mock_producer
        ...
        batch_data = [{"match_id": 1}, {"match_id": 2}, {"match_id": 3}]
    ...
    def test_producer_health_check(self, mock_producer_class):
        ...
        mock_producer.list_topics.return_value.topics = {"test - topic": Mock()}
        mock_producer_class.return_value = mock_producer
    ...
    def test_producer_health_check_exception(self, mock_producer_class):
        """测试健康检查异常"""
        mock_producer = Mock()
        mock_producer.list_topics.side_effect = KafkaException("Connection error")
        mock_producer_class.return_value = mock_producer
...
class TestProducerPerformance:
    ...
    async def test_batch_processing_performance(self, mock_producer_class):
        ...
        # 创建大批量数据
        large_batch = [{"match_id": i, "data": f"test_{i}"} for i in range(1000)]

        import time

        start_time = time.time()

        result = await producer.send_batch(large_batch, "match")
    ...
    async def test_concurrent_sends(self, mock_producer_class):
        ...
        tasks = []
        for i in range(10):
            task = producer.send_match_data({"match_id": i})
            tasks.append(task)
...
class TestErrorHandling:
    ...
    async def test_producer_buffer_full(self, mock_producer_class):
        """测试生产者缓冲区满"""
        mock_producer = Mock()
        mock_producer.produce.side_effect = KafkaException("Local: Queue full")
        mock_producer_class.return_value = mock_producer
    ...
    async def test_producer_timeout(self, mock_producer_class):
        """测试生产者超时"""
        mock_producer = Mock()
        mock_producer.produce.side_effect = KafkaException("Request timed out")
        mock_producer_class.return_value = mock_producer
