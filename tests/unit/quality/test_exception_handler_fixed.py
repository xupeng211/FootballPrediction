    ...
    def test_handle_custom_exception(self):
        ...
        try:
            ...
            result = handler.handle_exception(
                custom_exception, context={"module": "api"}
            )
        ...
        try:
            ...
            class ExceptionNotifier:
                ...
                def add_notification_rule(
                    self, rule_name, condition, channels, cooldown_minutes=30
                ):
                    """添加通知规则"""
                    rule = {
                        "name": rule_name,
                            ...
                            for channel_name in rule["channels"]:
                                channel = next(
                                    (
                                        ...
                                        if c["type"] == channel_name
                                    ),
                                    None,
                                )
                                if channel and channel["enabled"]:
                                    notification = self._create_notification(
                                        exception_info, channel, rule
                                    )
                ...
                def _determine_severity(self, exception):
                    """确定异常严重程度"""
                    if isinstance(
                        exception, (CriticalError, SystemExit, KeyboardInterrupt)
                    ):
                        return "CRITICAL"
                ...
                def _create_notification(self, exception_info, channel, rule):
                    ...
                    if channel["type"] == "email":
                        ...
                        notification["channel"] = channel["config"].get(
                            "channel", "#alerts"
                        )
                ...
                def _send_notification(self, notification):
                    ...
                    self.sent_notifications.append(
                        {**notification, "sent_at": datetime.now(), "status": "sent"}
                    )
            ...
            # 添加通知渠道
            notifier.add_notification_channel(
            ...
            email_notification = next(
                n for n in notifications if n["channel"] == "email"
            )
            ...
            slack_notification = next(
                n for n in notifications if n["channel"] == "slack"
            )
            assert slack_notification["channel"] == "#alerts"

            # 测试冷却时间
            # 再次发送严重异常（没有冷却时间限制）
            notifications2 = notifier.send_exception_notification(
                CriticalError("Another critical error")
            )
            ...
            class ExceptionBatchProcessor:
                ...
                def get_statistics(self):
                    ...
                    total_exceptions = sum(
                        b["summary"]["total"] for b in self.processed_batches
                    )
                    all_types = {}
        ...
        try:
            ...
            class RecoveryStrategyManager:
                ...
                def register_strategy(
                    self, exception_type, strategy_name, strategy_func, max_attempts=3
                ):
                    """注册恢复策略"""
                ...
                def attempt_recovery(self, exception, context=None):
                    ...
                    for strategy in self.strategies[exception_type]:
                        ...
                        try:
                            ...
                            recovery_record["strategies_tried"].append(
                                f"{strategy['name']} (failed: {recovery_error})"
                            )
                            continue
            ...
            manager.register_strategy(
                ConnectionError, "retry_connection", retry_connection
            )
            manager.register_strategy(
                CacheError, "clear_cache", clear_cache, max_attempts=2
            )
            ...
            recovered, message = manager.attempt_recovery(
                conn_error, context={"retry_count": 1}
            )
