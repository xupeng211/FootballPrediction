...
class TestDataLakeStorageBatchDelta001:
        ...
        with patch('os.getcwd', return_value='/test/dir'):
            with patch('os.path.join', return_value='/test/dir/data/football_lake'):
                with patch.object(Path, 'mkdir'):
                    ...
                    # 验证默认设置
                    assert data_lake.compression == "snappy"
                    assert data_lake.partition_cols == ["year", "month"]
                    assert data_lake.max_file_size_mb == 100
    ...
    def test_data_lake_initialization_custom_path(self, temp_data_lake):
        ...
        assert data_lake.compression == "gzip"
        assert data_lake.partition_cols == ["year", "month", "day"]
        assert data_lake.max_file_size_mb == 200
    ...
    async def test_get_table_stats_success(self, data_lake):
        ...
        try:
            ...
            if result:  # 如果返回非空字典
                for key in ["total_files", "total_size_mb", "total_records", "partitions"]:
                    if key in result:
