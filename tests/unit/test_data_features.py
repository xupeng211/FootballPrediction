...
from src.data.features.feature_store import FootballFeatureStore, get_feature_store, initialize_feature_store
...
def feature_store():
    ...
    import tempfile
    import os
    temp_dir = tempfile.mkdtemp(prefix="feast_repo_test_")
...
def test_feature_store_initialization():
    ...
    store = FootballFeatureStore(project_name="test_project")
    assert store is not None
    assert store.project_name == "test_project"
...
def test_feature_store_initialize(feature_store):
    ...
    with patch('src.data.features.feature_store.RepoConfig') as mock_config, \
         patch('src.data.features.feature_store.FeatureStore') as mock_store, \
...
def test_feature_store_apply_features_not_initialized(feature_store):
...
def test_write_features(feature_store):
    ...
    with patch.object(feature_store, '_store') as mock_store:
        feature_store.write_features('test_feature_view', sample_data)
        mock_store.push.assert_called_once()


def test_write_features_not_initialized(feature_store):
...
def test_get_online_features(feature_store):
    ...
    with patch.object(feature_store, '_store') as mock_store:
        ...
        mock_feature_vector.to_df.return_value = pd.DataFrame({'feature1': [1, 2, 3]})
        
        result = feature_store.get_online_features('test_service', entity_data)
        assert isinstance(result, pd.DataFrame)


def test_get_online_features_not_initialized(feature_store):
    ...
    with pytest.raises(RuntimeError, match="特征仓库未初始化"):
        feature_store.get_online_features('test_service', entity_data)


def test_get_historical_features(feature_store):
    """Test getting historical features"""
    # Create sample entity data with timestamp
    entity_data = pd.DataFrame({
        'match_id': [1, 2, 3],
    ...
    with patch.object(feature_store, '_store') as mock_store:
        ...
        mock_training_df.to_df.return_value = pd.DataFrame({'feature1': [1, 2, 3]})
        
        result = feature_store.get_historical_features('test_service', entity_data)
        assert isinstance(result, pd.DataFrame)


def test_get_historical_features_not_initialized(feature_store):
    ...
    with pytest.raises(RuntimeError, match="特征仓库未初始化"):
        feature_store.get_historical_features('test_service', entity_data)


def test_create_training_dataset(feature_store):
    """Test creating a training dataset"""
    start_date = datetime.now() - timedelta(days=30)
    end_date = datetime.now()
    
    with patch.object(feature_store, '_store') as mock_store, \
         patch.object(feature_store, 'get_historical_features') as mock_get_hist:
        
        mock_get_hist.return_value = pd.DataFrame({'feature1': [1, 2, 3]})
        
        result = feature_store.create_training_dataset(start_date, end_date)
        assert isinstance(result, pd.DataFrame)


def test_get_feature_statistics(feature_store):
    """Test getting feature statistics"""
    with patch.object(feature_store, '_store') as mock_store:
        mock_feature_view = Mock()
        mock_feature_view.features = [Mock(), Mock()]  # 2 features
        mock_feature_view.entities = [Mock(), Mock()]  # 2 entities
        mock_feature_view.ttl = timedelta(days=30)
        mock_feature_view.tags = {"team": "data"}
        
        mock_store.get_feature_view.return_value = mock_feature_view
        
        stats = feature_store.get_feature_statistics('test_view')
        assert "feature_view_name" in stats
        assert stats["num_features"] == 2


def test_get_feature_statistics_not_initialized(feature_store):
    """Test getting feature statistics without initialization"""
    feature_store._store = None
    
    with pytest.raises(RuntimeError, match="特征仓库未初始化"):
        feature_store.get_feature_statistics('test_view')
...
def test_list_features(feature_store):
    ...
    with patch.object(feature_store, '_store') as mock_store:
        ...
        mock_feature_view1.name = "view1"
        mock_feature_view1.features = [Mock(), Mock()]
        mock_feature_view1.entities = [Mock()]
        mock_feature_view1.tags = {"team": "data"}
        
        mock_feature_view1.features[0].name = "feature1"
        mock_feature_view1.features[0].dtype.name = "INT32"
        mock_feature_view1.features[0].description = "First feature"
        
        mock_feature_view1.features[1].name = "feature2"
        mock_feature_view1.features[1].dtype.name = "FLOAT"
        mock_feature_view1.features[1].description = "Second feature"
        
        mock_store.list_feature_views.return_value = [mock_feature_view1]
        
        features_list = feature_store.list_features()
        assert isinstance(features_list, list)
        assert len(features_list) == 2  # 2 features from 1 view
...
def test_cleanup_old_features(feature_store):
...
def test_initialize_feature_store():
    ...
    with patch('src.data.features.feature_store.FootballFeatureStore') as mock_class:
        mock_instance = Mock()
        ...
        result = initialize_feature_store(
            ...
            redis_config={"connection_string": "redis://localhost:6379"}
        )
...
def test_write_features_with_auto_timestamp(feature_store):
    ...
    mock_df = Mock()
    mock_df.columns = ['match_id', 'feature_value']  # No timestamp column initially
    mock_df.__contains__ = Mock(return_value=False)  # timestamp_column not in df

    with patch.object(feature_store, '_store') as mock_store:
        feature_store.write_features('test_feature_view', mock_df)
...
def test_get_feature_statistics_with_error_handling(feature_store):
    ...
    with patch.object(feature_store, '_store') as mock_store:
        ...
        stats = feature_store.get_feature_statistics('nonexistent_view')
...
def test_list_features_with_error_handling(feature_store):
    ...
    with patch.object(feature_store, '_store') as mock_store:
        mock_store.list_feature_views.side_effect = Exception("List features failed")
        
        features_list = feature_store.list_features()
        assert features_list == []


def test_create_training_dataset_with_match_ids(feature_store):
    ...
    with patch.object(feature_store, '_store') as mock_store, \
        ...
        mock_get_hist.return_value = pd.DataFrame({'feature1': [1, 2, 3, 4, 5]})
        
        result = feature_store.create_training_dataset(
            start_date, end_date, match_ids=match_ids
        )
        assert isinstance(result, pd.DataFrame)
...
def test_feature_store_initialization_with_custom_config():
    ...
    custom_redis = {
        "connection_string": "redis://custom_host:6380/1"
    }
    
    store = FootballFeatureStore(
        project_name="custom_project",
        postgres_config=custom_postgres,
