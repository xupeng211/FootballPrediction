...
def storage(tmp_path, monkeypatch):
    ...
    storage = DataLakeStorage(base_path=str(tmp_path))
    return storage


@pytest.mark.asyncio
async def test_save_historical_data_writes_file(storage, caplog):
    caplog.set_level(logging.INFO, logger="storage.DataLakeStorage")
    df = pd.DataFrame(
        [
    ...
    file_path = await storage.save_historical_data("raw_matches", df)
...
async def test_save_historical_data_handles_write_failure(storage, monkeypatch, caplog):
    df = pd.DataFrame([{"match_id": 10, "league": "SerieA"}])

    with patch("src.data.storage.data_lake_storage.pq.write_table") as mock_write:
        mock_write.side_effect = OSError("disk full")

        with pytest.raises(OSError):
            await storage.save_historical_data("raw_matches", df)
...
async def test_save_historical_data_handles_missing_directory(
    ...
    storage = DataLakeStorage(base_path=str(base_path))
    df = pd.DataFrame([{"match_id": 99, "league": "Ligue1"}])
