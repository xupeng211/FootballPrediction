...
class TestRecoveryHandlerBatchOmega004:
    ...
    def test_task_failure_initialization(self):
        ...
        context = {"task_name": "测试任务", "priority": 1}
    ...
    def test_execute_recovery_strategy_unknown_strategy(self, recovery_handler, mock_task, sample_failure):
        ...
        recovery_handler.recovery_configs[FailureType.TIMEOUT]["strategy"] = MockStrategy("unknown_strategy")
    ...
    def test_fixed_delay_retry_success(self, mock_datetime, recovery_handler, mock_task, sample_failure):
        ...
        config = {"max_retries": 3, "base_delay": 120}
    ...
    def test_check_failure_patterns_repeated_failures(self, recovery_handler):
        """测试检查失败模式（重复失败）"""
        # 添加连续三次同类型失败
        recovery_handler.failure_patterns["task_001"] = [
            FailureType.TIMEOUT,
            FailureType.TIMEOUT,
            FailureType.TIMEOUT
        ]
    ...
    def test_send_alert_handler_exception(self, recovery_handler):
        ...
        mock_handler.__name__ = "error_handler"
    ...
    def test_register_alert_handler(self, recovery_handler):
        """测试注册告警处理器"""
    ...
    def test_get_failure_statistics_with_data(self, recovery_handler, sample_failure):
        """测试获取失败统计信息（有数据）"""
        # 添加一些失败记录
        recovery_handler.failure_history = [sample_failure]
        recovery_handler.total_failures = 1
        recovery_handler.successful_recoveries = 1
        recovery_handler.failed_recoveries = 0
        recovery_handler.failure_patterns = {sample_failure.task_id: [FailureType.TIMEOUT]}
    ...
    def test_clear_old_failures(self, mock_datetime, recovery_handler):
        ...
        old_failure = TaskFailure(
            task_id="old_task",
            failure_time=mock_now - timedelta(days=35),
            failure_type=FailureType.TIMEOUT,
        ...
        new_failure = TaskFailure(
            task_id="new_task",
            failure_time=mock_now - timedelta(days=10),
            failure_type=FailureType.TIMEOUT,
            error_message="New error"
        )

        recovery_handler.failure_history = [old_failure, new_failure]
    ...
    def test_clear_old_failures_no_old_records(self, recovery_handler):
        ...
        failure = TaskFailure(
            task_id="new_task",
            failure_time=datetime.now() - timedelta(days=10),
            failure_type=FailureType.TIMEOUT,
            error_message="New error"
        )

        recovery_handler.failure_history = [failure]
    ...
    def test_integration_complete_failure_handling_workflow(self, recovery_handler, mock_task):
        ...
        def mock_handler(alert_data):
            mock_calls.append(alert_data)
        mock_handler.__name__ = "integration_test_handler"
    ...
    def test_integration_statistics_after_multiple_failures(self, recovery_handler, mock_task):
        ...
        # 模拟多次失败处理
        for i in range(3):
            mock_task.task_id = f"task_{i:03d}"
            recovery_handler.handle_task_failure(mock_task, f"Error {i}")
    ...
    def test_integration_alert_handler_registration_and_execution(self, recovery_handler):
        ...
        calls2 = []
        def handler2(alert_data):
            calls2.append(alert_data)
        handler2.__name__ = "handler2"

        recovery_handler.register_alert_handler(handler1)
        recovery_handler.register_alert_handler(handler2)
    ...
    def test_error_handling_invalid_task_attributes(self, recovery_handler):
        ...
        # 应该能处理而不崩溃
        try:
            recovery_handler.handle_task_failure(invalid_task, "Test error")
