...
class TestBatchTaskExecution:
    ...
    def task_scheduler(self, mock_database, mock_feature_store):
        ...
        with patch('src.scheduler.task_scheduler.DatabaseManager', return_value=mock_database), \
             patch('src.scheduler.task_scheduler.FootballFeatureStore', return_value=mock_feature_store):
            scheduler = TaskScheduler()
            scheduler.db_manager = mock_database
    ...
    def failing_tasks(self):
        ...
        for i in range(3):
            task = ScheduledTask(
                ...
                func=lambda x: 1 / 0,  # This will raise ZeroDivisionError
                args=[i],
                cron_expression="0 8 * * *",
                enabled=True,
                timeout=300,
    ...
    class TestBulkTaskRegistration:
        ...
        def test_register_multiple_tasks_success(self, task_scheduler, sample_tasks):
            ...
            for task in sample_tasks:
                result = task_scheduler.register_task(task)
    ...
    class TestBatchTaskExecution:
        ...
        async def test_execute_all_tasks_success(self, task_scheduler, sample_tasks):
            ...
            original_execute = task_scheduler.execute_task
            executed_tasks = []

            def mock_execute(task_id):
                executed_tasks.append(task_id)
                return {"status": "success", "result": f"executed_{task_id}"}
            ...
            results = []
            for task in sample_tasks:
                result = await task_scheduler.execute_task(task.task_id)
                results.append(result)
        ...
        @pytest.mark.asyncio
        async def test_execute_batch_with_failures(self, task_scheduler, sample_tasks, failing_tasks):
            ...
            assert len(exceptions) == len(failing_tasks)  # Failed tasks raised exceptions
        ...
        async def test_concurrent_task_execution(self, task_scheduler, sample_tasks):
            ...
            # Mock execution with slight delay to simulate concurrent processing
            execution_order = []
    ...
    class TestQueueConsumption:
        ...
        def test_queue_task_processing(self, job_manager, mock_task_queue):
            ...
            # Mock queue with tasks
            mock_tasks = [
                {"task_id": "queue_task_1", "data": "data1"},
                {"task_id": "queue_task_2", "data": "data2"},
                {"task_id": "queue_task_3", "data": "data3"}
            ]

            mock_task_queue.get.side_effect = mock_tasks + [None]  # None to signal end
            ...
            def mock_process_task(task_data):
                ...
                return JobExecutionResult(
                    ...
                    status="success",
                    result=f"processed_{task_data['data']}",
        ...
        def test_queue_error_handling(self, job_manager, mock_task_queue):
            ...
            # Mock queue with a failing task
            failing_task = {"task_id": "failing_task", "data": "bad_data"}
            mock_task_queue.get.side_effect = [failing_task, None]

            processed_tasks = []
            errors = []

            def mock_process_task(task_data):
                if task_data["task_id"] == "failing_task":
    ...
    class TestResourceManagement:
        ...
        async def test_memory_usage_monitoring(self, job_manager):
            ...
            with patch('src.scheduler.job_manager.psutil') as mock_psutil:
                ...
                mock_psutil.virtual_memory.side_effect = [MagicMock(percent=val) for val in memory_values]
                ...
                assert all(0 <= sample <= 100 for sample in memory_samples)  # Valid percentages
        ...
        async def test_cpu_usage_monitoring(self, job_manager):
            ...
            with patch('src.scheduler.job_manager.psutil') as mock_psutil:
                ...
                assert all(0 <= sample <= 100 for sample in cpu_samples)  # Valid percentages
        ...
        async def test_resource_threshold_handling(self, job_manager):
            ...
            with patch('src.scheduler.job_manager.psutil') as mock_psutil:
                ...
                mock_psutil.virtual_memory.return_value = MagicMock(percent=95.0)  # High memory
                mock_psutil.cpu_percent.return_value = 90.0  # High CPU

                # Check resource status
                memory_ok = job_manager.resource_monitor.check_memory_usage(threshold=90.0)
    ...
    class TestCleanupAndRecovery:
        ...
        @pytest.mark.asyncio
        async def test_task_cleanup_after_completion(self, task_scheduler, sample_tasks):
        ...
        async def test_state_recovery_after_failure(self, task_scheduler, sample_tasks):
            ...
            def mock_execute(task_id):
                if task_id.endswith("3"):  # Simulate failure for task_3
                    failed_executions.append(task_id)
                    raise Exception(f"Execution failed for {task_id}")
                return {"status": "success", "result": f"executed_{task_id}"}
