...
test('ReconEngine 在可用代理少于 2 时应进入 SUSPEND，并在心跳恢复后自动 resume', async () => {
  let heartbeatCount = 0;
  const events = [];
  const snapshots = [
    { total: 22, healthy: 1, cooling: 21, dead: 0, available: 1 },
    { total: 22, healthy: 1, cooling: 21, dead: 0, available: 1 },
    { total: 22, healthy: 2, cooling: 20, dead: 0, available: 2 }
  ];
  ...
  const engine = new ReconEngine({
    ...
    proxyRotator: {
      getHealthStatus() {
        return snapshots[Math.min(heartbeatCount, snapshots.length - 1)];
      },
      proxyProvider: {
        async runHealthCheck() {
          heartbeatCount++;
          return { healthy: snapshots[Math.min(heartbeatCount, snapshots.length - 1)].available };
        }
      }
    },
  ...
  const snapshot = await engine.ensureProxyPoolCapacity({ context: 'unit-test' });

  assert.equal(snapshot.available, 2);
  assert.equal(engine.getRuntimeState().status, 'RUNNING');
  assert.ok(events.some((entry) => entry.event === 'recon_engine_suspended'));
  assert.ok(events.some((entry) => entry.event === 'recon_engine_suspend_heartbeat'));
  assert.ok(events.some((entry) => entry.event === 'recon_engine_resumed'));
});
