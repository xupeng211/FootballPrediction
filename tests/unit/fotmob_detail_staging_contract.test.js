...
test("A1: legal minimal payload+manifest passes L1-L8 and yields ACCEPTED_NEW-ready state", () => {
  ...
  assert.strictEqual(
    validation.layers[VALIDATION_LAYERS.L4_PROVENANCE_HASH_CHAIN].ok,
    true,
  );
});
...
test("A2: legal complete payload+manifest (all five sections) builds a valid artifact", () => {
  ...
  assert.deepStrictEqual(validation.coverage.stats.periods, [
    "All",
    "FirstHalf",
    "SecondHalf",
  ]);
  ...
  assert.deepStrictEqual(validation.coverage.lineup.sides.sort(), [
    "away",
    "home",
  ]);
});
...
test("A3: schema error (wrong schema_version) → REJECTED_SCHEMA_UNKNOWN E002", () => {
  ...
  assert.strictEqual(
    ...
    TERMINAL_STATES.REJECTED_SCHEMA_UNKNOWN,
  );
...
test("A6: response-derived=false → REJECTED_PROVENANCE_BROKEN E010", () => {
  ...
  assert.strictEqual(
    validation.terminal_state,
    TERMINAL_STATES.REJECTED_PROVENANCE_BROKEN,
  );
...
test("A9: payload file SHA wrong (physical bytes differ) → E008", () => {
  ...
  const modifiedBytes = Buffer.from(
    JSON.stringify(pair.payload, null, 2) + "\n\n",
    "utf8",
  );
  const validation = validateObservation({
    payload: pair.payload,
...
test("A13: source index with bad archive sha / duplicate ids fails closed", () => {
  const bad = validateSourceIndex({
    ...
    entries: [
      { source_match_id: "1", payload_file: "/tmp/a", manifest_file: "/tmp/b" },
    ],
  });
...
test("B16: player_stats-style ID keys present in sections verbatim after conversion", () => {
  ...
  assert.deepStrictEqual(
    Object.keys(result.artifact.sections.player_stats.json).sort(),
    ["1021586", "109222"],
  );
});
...
test("B17: same object content in different key insertion order → identical capture hash", () => {
  ...
  reordered.stable_payload_sha256 =
    computeStableCapturePayloadSha256(reordered);
...
test("B18: no duplicated canonical-JSON hash implementation exists in staging contract source", () => {
  ...
  assert.match(
    source,
    /require\(\s*['"]\.\/FotMobDetailCaptureContract['"]\s*\)/,
  );
...
test("B19: generated_at / observation_id never enter the business hash", () => {
  ...
  assert.strictEqual(
    computeStagingArtifactBusinessHash(mutated),
    artifact.business_hash,
  );
});
  ...
  const pair = buildPair({
    ...
    normalized: {
      ...
      events: [
        ...
        { id: null, minute: 45, event_kind: "marker_event", type: "AddedTime", playerName: null, synthetic_event_key: null, source_has_native_id: false },
        { id: null, minute: 45, event_kind: "marker_event", type: "Half", playerName: null, synthetic_event_key: null, source_has_native_id: false },
        { id: null, minute: 90, event_kind: "marker_event", type: "AddedTime", playerName: null, synthetic_event_key: null, source_has_native_id: false },
        { id: null, minute: 90, event_kind: "marker_event", type: "Half", playerName: null, synthetic_event_key: null, source_has_native_id: false },
...
test("F40/F41/F42: converter and contract modules import no fetcher, no DB client, no scripts/ops", () => {
  for (const rel of [
    ...
    assert.doesNotMatch(
      source,
      /scripts\/ops/,
      `${rel} must not reference scripts/ops`,
    );
    ...
    assert.doesNotMatch(
      source,
      /require\(['"]fuzzball['"]\)/,
      `${rel} must not import fuzzball`,
    );
    assert.doesNotMatch(
      source,
      /\.similarity\(|fuzzy_match/i,
      `${rel} must not contain fuzzy matching calls`,
    );
  }
});
...
test("F43: artifact sections contain only the five allowlisted sections, no odds fields", () => {
  ...
  assert.strictEqual(result.ok, true);
  assert.deepStrictEqual(
    Object.keys(result.artifact.sections).sort(),
    [...SECTIONS].sort(),
  );
...
test("F44/F45: canonical_match_id defaults to null and an unlinked detail is legal", () => {
  ...
  assert.strictEqual(
    result.artifact.canonical_link_status,
    "UNLINKED_NOT_ATTEMPTED",
  );
...
test("source index helper builds a legal index", () => {
  const index = buildSourceIndex([
    sourceIndexEntry(
      "3901023",
      "/tmp/pairs/1.payload.json",
      "/tmp/pairs/1.manifest.json",
    ),
  ]);
