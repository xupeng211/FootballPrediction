...
class TestQualityMonitorBatchGamma006:
    ...
    def test_freshness_thresholds_configuration(self, quality_monitor):
        ...
        # 验证所有必需的阈值都存在
        expected_tables = ["matches", "odds", "predictions", "teams", "leagues"]
        for table in expected_tables:
            assert table in thresholds, f"Missing threshold for table: {table}"
            assert isinstance(thresholds[table], (int, float)), f"Invalid threshold type for {table}"
            assert thresholds[table] > 0, f"Threshold should be positive for {table}"

    def test_critical_fields_configuration(self, quality_monitor):
        """测试关键字段配置"""
        critical_fields = quality_monitor.critical_fields

        # 验证所有必需表的字段定义
        expected_tables = ["matches", "odds", "predictions", "teams"]
        for table in expected_tables:
            assert table in critical_fields, f"Missing critical fields for table: {table}"
            assert isinstance(critical_fields[table], list), f"Fields should be a list for {table}"
            assert len(critical_fields[table]) > 0, f"Should have critical fields for {table}"

    def test_get_quality_level(self, quality_monitor):
        """测试质量等级评估"""
        # 测试不同分数的等级评估
        assert quality_monitor._get_quality_level(95.0) == "优秀"
        assert quality_monitor._get_quality_level(85.0) == "良好"
        assert quality_monitor._get_quality_level(75.0) == "一般"
        assert quality_monitor._get_quality_level(65.0) == "较差"
        assert quality_monitor._get_quality_level(0.0) == "很差"
        assert quality_monitor._get_quality_level(100.0) == "优秀"

    def test_get_quality_level_boundary_values(self, quality_monitor):
        """测试质量等级边界值"""
        # 测试边界值
        assert quality_monitor._get_quality_level(95.0) == "优秀"  # 边界
        assert quality_monitor._get_quality_level(94.9) == "良好"
        assert quality_monitor._get_quality_level(85.0) == "良好"  # 边界
        assert quality_monitor._get_quality_level(84.9) == "一般"
        assert quality_monitor._get_quality_level(70.0) == "一般"  # 边界
        assert quality_monitor._get_quality_level(69.9) == "较差"
        assert quality_monitor._get_quality_level(50.0) == "较差"  # 边界
        assert quality_monitor._get_quality_level(49.9) == "很差"

    def test_data_freshness_result_initialization(self):
    ...
    def test_data_freshness_result_to_dict(self, sample_freshness_result):
        ...
        assert isinstance(result_dict["freshness_hours"], float)
        assert result_dict["freshness_hours"] == round(sample_freshness_result.freshness_hours, 2)
    ...
    def test_generate_quality_recommendations(self, quality_monitor):
        ...
        result = quality_monitor._generate_quality_recommendations(quality_issues)
    ...
    def test_error_handling_strategies(self, quality_monitor):
        ...
        assert quality_monitor.freshness_thresholds is not None
    ...
    def test_configuration_completeness(self, quality_monitor):
        ...
        # 验证关键表都有配置
        essential_tables = {"matches", "odds", "predictions"}
        assert essential_tables.issubset(freshness_keys), "Missing essential tables in freshness thresholds"
        assert essential_tables.issubset(critical_keys), "Missing essential tables in critical fields"

    def test_threshold_values_reasonable(self, quality_monitor):
        """测试阈值设置的合理性"""
        # 验证阈值设置合理性
        assert quality_monitor.freshness_thresholds["odds"] < quality_monitor.freshness_thresholds["matches"]  # 赔率应该更频繁更新
        assert quality_monitor.freshness_thresholds["predictions"] < quality_monitor.freshness_thresholds["matches"]  # 预测应该更频繁更新
        assert quality_monitor.freshness_thresholds["teams"] > quality_monitor.freshness_thresholds["matches"]  # 球队数据更新频率较低
        assert quality_monitor.freshness_thresholds["leagues"] > quality_monitor.freshness_thresholds["teams"]  # 联赛数据更新频率最低

    def test_critical_fields_content(self, quality_monitor):
        """测试关键字段内容"""
        # 验证关键字段包含必要的字段
        matches_fields = quality_monitor.critical_fields["matches"]
        assert any("team" in field.lower() for field in matches_fields), "Matches should have team-related fields"
        assert any("time" in field.lower() for field in matches_fields), "Matches should have time-related fields"

        odds_fields = quality_monitor.critical_fields["odds"]
        assert any("odds" in field.lower() for field in odds_fields), "Odds should have odds-related fields"
        assert "match_id" in odds_fields, "Odds should reference match_id"
    ...
    async def test_calculate_overall_quality_score_basic(self, quality_monitor):
        ...
        quality_monitor.check_data_freshness = AsyncMock(return_value={
            ...
            "odds": Mock(is_fresh=True, freshness_hours=0.5)
        })
        quality_monitor.check_data_completeness = AsyncMock(return_value={
            "matches": Mock(completeness_score=95.0),
            "odds": Mock(completeness_score=90.0)
        })
        quality_monitor.check_data_consistency = AsyncMock(return_value={
            "overall_consistency_score": 88.0
        })
        ...
        assert result["quality_level"] in ["优秀", "良好", "一般", "较差", "很差"]
    ...
    async def test_get_quality_trends_empty_data(self, quality_monitor):
        ...
        assert "current_quality" in result
        assert result["trend_period_days"] == 7
        assert result["current_quality"]["overall_score"] == 0.0
    ...
    async def test_get_quality_trends_single_data_point(self, quality_monitor):
        ...
        assert isinstance(result, dict)
        assert "current_quality" in result
        assert result["current_quality"]["overall_score"] == 85.0
        assert result["trend_period_days"] == 7