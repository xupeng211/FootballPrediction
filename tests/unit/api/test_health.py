...
class TestHealthAPI:
    ...
    @pytest.mark.asyncio
    @patch('src.api.health.MlflowClient')
    @patch('src.api.health.mlflow')
    @patch('src.api.health.get_settings')
    async def test_check_mlflow_success(self, mock_get_settings, mock_mlflow, mock_mlflow_client_class):
        """测试MLflow检查成功"""
        # Mock settings
