...
spec = importlib.util.spec_from_file_location("auth_module", "/home/user/projects/FootballPrediction/src/api/auth.py")
...
class TestAuthModels:
    ...
    def test_user_register_model_valid(self):
        ...
        user = auth_module.UserRegister(**user_data)
    ...
    def test_user_login_model_valid(self):
        ...
        login = auth_module.UserLogin(**login_data)
    ...
    def test_token_response_model_valid(self):
        ...
        response = auth_module.TokenResponse(**token_data)
    ...
    def test_password_change_request_model_valid(self):
        ...
        request = auth_module.PasswordChangeRequest(**password_data)
    ...
    def test_password_reset_request_model_valid(self):
        ...
        request = auth_module.PasswordResetRequest(**reset_data)
...
class TestUserAuthentication:
    ...
    async def test_authenticate_user_success_by_username(self, auth_manager):
        """测试用户名认证成功"""
        user = await auth_module.authenticate_user("admin", "admin123", auth_manager)
    ...
    async def test_authenticate_user_success_by_email(self, auth_manager):
        """测试邮箱认证成功"""
        user = await auth_module.authenticate_user("user@football-prediction.com", "user123", auth_manager)
    ...
    async def test_authenticate_user_wrong_username(self, auth_manager):
        """测试用户名错误认证失败"""
        user = await auth_module.authenticate_user("nonexistent", "admin123", auth_manager)
        assert user is None

    @pytest.mark.asyncio
    async def test_authenticate_user_wrong_password(self, auth_manager):
        """测试密码错误认证失败"""
        user = await auth_module.authenticate_user("admin", "wrongpassword", auth_manager)
        assert user is None

    @pytest.mark.asyncio
    async def test_authenticate_user_nonexistent_email(self, auth_manager):
        """测试不存在的邮箱认证失败"""
        user = await auth_module.authenticate_user("nonexistent@example.com", "password123", auth_manager)
        assert user is None
...
class TestUserCreation:
    ...
    async def test_create_user_success(self, auth_manager):
        """测试创建用户成功"""
        user_data = auth_module.UserRegister(
            username="newuser",
        ...
        # 保存原始MOCK_USERS以便恢复
        original_users = auth_module.MOCK_USERS.copy()
        ...
        try:
            ...
            # 恢复原始MOCK_USERS
            auth_module.MOCK_USERS.clear()
            auth_module.MOCK_USERS.update(original_users)
...
class TestSecurityFeatures:
    ...
    def test_mock_users_data_structure(self):
        """测试模拟用户数据结构"""
        assert len(auth_module.MOCK_USERS) >= 2

        admin_user = auth_module.MOCK_USERS[1]
        ...
        regular_user = auth_module.MOCK_USERS[2]
...
class TestEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_authenticate_user_empty_credentials(self, auth_manager):
        """测试空凭据认证"""
        user = await auth_module.authenticate_user("", "", auth_manager)
        assert user is None

    @pytest.mark.asyncio
    async def test_authenticate_user_none_credentials(self, auth_manager):
        """测试None凭据认证"""
        with pytest.raises(Exception):
            await auth_module.authenticate_user(None, None, auth_manager)

    @pytest.mark.asyncio
    async def test_get_user_by_id_negative_id(self):
        """测试获取负数ID用户"""
        user = await auth_module.get_user_by_id(-1)
        assert user is None

    @pytest.mark.asyncio
    async def test_get_user_by_id_zero_id(self):
        """测试获取0 ID用户"""
        user = await auth_module.get_user_by_id(0)
        assert user is None

    def test_jwt_auth_manager_default_secret_key(self):
        """测试JWT认证管理器默认密钥生成"""
        manager = JWTAuthManager()  # 不提供密钥
        assert manager.secret_key is not None
        assert len(manager.secret_key) > 50  # 应该是一个长密钥
    ...
    @pytest.mark.asyncio
    async def test_verify_token_malformed(self, auth_manager):
        """测试验证格式错误的令牌"""
        malformed_tokens = [
            "not-a-jwt",
