...
def test_get_task_priority(task_name, expected_priority):
    assert utils.get_task_priority(task_name) == expected_priority
...
def test_calculate_next_collection_time(
    task_name, hour, minute, expected_delta_minutes
):
    now = datetime(2023, 1, 1, hour, minute)
    with patch("src.tasks.utils.datetime") as mock_dt:
...
async def test_should_collect_live_scores_with_matches(MockDatabaseManager):
...
@pytest.mark.asyncio
@patch("src.tasks.utils.DatabaseManager")
async def test_should_collect_live_scores_no_matches(MockDatabaseManager):
    mock_result = MagicMock()
...
@pytest.mark.asyncio
@patch("src.tasks.utils.DatabaseManager")
async def test_get_active_leagues_success(MockDatabaseManager):
    mock_row = MagicMock()
    mock_row.name = "Premier League"
...
@pytest.mark.asyncio
@patch("src.tasks.utils.DatabaseManager")
async def test_get_active_leagues_db_error(MockDatabaseManager):
    mock_db_manager = MockDatabaseManager.return_value
    mock_db_manager.get_async_session.side_effect = Exception("DB Error")

    leagues = await utils.get_active_leagues()
    assert leagues == ["Premier League", "La Liga", "Serie A", "Bundesliga"]


@pytest.mark.asyncio
@patch("src.tasks.utils.DatabaseManager")
async def test_cleanup_stale_tasks_success(MockDatabaseManager):
    mock_result = MagicMock()
...
@pytest.mark.asyncio
@patch("src.tasks.utils.DatabaseManager")
async def test_get_upcoming_matches_db_error(MockDatabaseManager):
    mock_db_manager = MockDatabaseManager.return_value
    mock_db_manager.get_async_session.side_effect = Exception("DB Error")
