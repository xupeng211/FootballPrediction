...
@pytest.fixture
def monitor_with_registry() -> tuple[TaskMonitor, CollectorRegistry]:
...
async def test_calculate_error_rates_updates_metrics(monitor_with_registry):
    ...
    ), patch.object(monitor, "_get_db_type", AsyncMock(return_value="postgresql")):
        error_rates = await monitor.calculate_error_rates()

    assert error_rates == {
        "collect_odds_task": pytest.approx(0.1),
        "collect_scores_task": pytest.approx(0.0),
    }

    assert mock_session.execute.await_count == 1

    odds_rate = registry.get_sample_value(
...
async def test_check_task_health_handles_dependency_failure(monitor_with_registry):
    ...
    with patch.object(
        ...
        monitor, "_check_task_delays", new=AsyncMock(return_value={})
    ), patch(
        "src.tasks.monitoring.logger"
    ) as mock_logger:
        health_status = await monitor.check_task_health()

    assert health_status["overall_status"] == "unknown"
    assert health_status["error"] == "redis down"
    assert health_status["issues"] == [
        {"type": "monitoring_error", "message": "redis down"}
    ]
...
async def test_check_task_health_triggers_alerts_when_thresholds_exceeded(
    ...
    with patch.object(
        ...
        new=AsyncMock(return_value={"collect_odds_task": 0.35}),
    ), patch.object(
        monitor,
        "_get_queue_sizes",
        new=AsyncMock(return_value={"fixtures": 150, "odds": 20}),
    ), patch.object(
        monitor,
        "_check_task_delays",
        new=AsyncMock(return_value={"collect_scores_task": 720}),
    ):
        health_status = await monitor.check_task_health()
    ...
    assert {"high_error_rate", "queue_backlog", "task_delays"}.issubset(issue_types)

    assert health_status["metrics"]["error_rates"][
        "collect_odds_task"
    ] == pytest.approx(0.35)
    assert health_status["metrics"]["queue_sizes"]["fixtures"] == 150
    assert health_status["metrics"]["task_delays"]["collect_scores_task"] == 720
...
async def test_get_queue_sizes_updates_prometheus_metrics(monitor_with_registry):
    ...
    redis_client.llen.side_effect = lambda key: queue_counts[key.split(".", 1)[1]]

    with patch("redis.from_url", return_value=redis_client):
        sizes = await monitor._get_queue_sizes()

    assert sizes == queue_counts

    for queue, expected in queue_counts.items():
...
async def test_check_task_delays_collects_results(monitor_with_registry):
    ...
    mock_session.execute = AsyncMock(
        return_value=[
            SimpleNamespace(task_name="collect_odds_task", avg_delay_seconds=123.4),
            SimpleNamespace(task_name="collect_scores_task", avg_delay_seconds=45.0),
        ]
    )
    ...
    query_builder.build_task_delay_query.return_value = "SELECT 1"
    ...
    assert delays == {
        ...
        "collect_scores_task": pytest.approx(45.0),
    }
    query_builder.build_task_delay_query.assert_called_once()
...
async def test_get_db_type_uses_engine_once(monitor_with_registry):
    ...
    with patch(
        ...
        result_first = await monitor._get_db_type()
        result_second = await monitor._get_db_type()

    assert result_first == "sqlite"
    assert result_second == "sqlite"
...
async def test_get_query_builder_caches_instance(monitor_with_registry):
    ...
    mock_builder.assert_called_once_with("postgresql")


def test_record_and_update_metrics(monitor_with_registry):
    monitor, registry = monitor_with_registry

    monitor.record_task_start("demo_task", "t-1")
    active_value = registry.get_sample_value(
        "football_active_tasks", {"task_name": "demo_task"}
    )
    assert active_value == 1

    monitor.record_task_completion("demo_task", "t-1", duration=2.5, status="success")
    ...
    monitor.record_task_retry("demo_task", retry_count=1)
    retry_value = registry.get_sample_value(
        "football_task_retries_total", {"task_name": "demo_task", "retry_count": "1"}
    )
    assert retry_value == 1

    monitor.update_queue_size("fixtures", 7)
    queue_value = registry.get_sample_value(
