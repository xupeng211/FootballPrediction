...
class TestMaintenanceTasks:
    ...
    def task_manager(self):
        """创建任务管理器"""
        with (
            patch("src.tasks.maintenance_tasks.database"),
    ...
    async def test_backup_task(self, task_manager):
        ...
        with (
            patch("src.tasks.maintenance_tasks.create_database_backup") as mock_backup,
    ...
    async def test_concurrent_task_execution(self, task_manager):
        """测试并发任务执行"""
        tasks = [DatabaseCleanupTask(), CacheCleanupTask(), SystemHealthCheckTask()]
    ...
    def test_task_configuration_validation(self):
        ...
        # 无效配置 - 缺少必需字段
        invalid_config = {"timeout": 3600, "retry_attempts": 3}
    ...
    async def test_task_dependency_management(self, task_manager):
        ...
        # 执行任务（应该先执行依赖）
        with (
            patch.object(cleanup_task, "execute") as mock_cleanup,
    ...
    def test_task_configuration_from_file(self, task_manager):
        ...
        config_data = {
            ...
            "cache_cleanup": {"schedule": "0 3 * * *", "enabled": True},
        }
