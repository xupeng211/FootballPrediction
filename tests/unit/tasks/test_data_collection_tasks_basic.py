...
def _make_fake_task(retries: int, retry_side_effect=None):
    return SimpleNamespace(
        ...
        name="src.tasks.data_collection_tasks.collect_fixtures_task",
    )


def _collect(task, **kwargs):
    run_fn = tasks_module.collect_fixtures_task.run.__func__
    return run_fn(task, **kwargs)
...
def test_collect_fixtures_success(monkeypatch):
    ...
    with patch(
        ...
        result = _collect(fake_task, leagues=["EPL"], days_ahead=2)

    assert result["status"] == "success"
    assert result["records_collected"] == 7
    mock_collector.collect_fixtures.assert_awaited_once()
    ...
    mock_logger.info.assert_any_call("赛程采集完成: 成功=7, 错误=0, 总数=7")


def test_collect_fixtures_retry(monkeypatch):
    fake_task = _make_fake_task(
        retries=0, retry_side_effect=RuntimeError("retry invoked")
    )

    mock_collector = MagicMock()
    mock_collector.collect_fixtures = AsyncMock(side_effect=RuntimeError("boom"))

    monkeypatch.setattr(
        tasks_module.TaskRetryConfig,
        "get_retry_config",
        MagicMock(return_value={"max_retries": 3, "retry_delay": 4}),
    )
    ...
    with patch(
        ...
        with pytest.raises(RuntimeError, match="retry invoked"):
            _collect(fake_task, leagues=["EPL"], days_ahead=5)

    fake_task.retry.assert_called_once()
    _, kwargs = fake_task.retry.call_args
    assert kwargs["countdown"] == 4
...
def test_collect_fixtures_final_failure_logs(monkeypatch):
    ...
    mock_collector = MagicMock()
    mock_collector.collect_fixtures = AsyncMock(side_effect=RuntimeError("final fail"))
    ...
    with patch(
        ...
        with pytest.raises(RuntimeError, match="final fail"):
            _collect(fake_task, leagues=None, days_ahead=1)
