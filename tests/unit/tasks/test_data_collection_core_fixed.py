...
class TestDataCollectionTask:
    ...
    def test_set_database_manager(self, mock_orchestrator_class):
        ...
        # 验证数据库管理器传递给收集器
        mock_orchestrator.collectors["collector1"].set_database_manager.assert_called_once_with(mock_db_manager)
        mock_orchestrator.collectors["collector2"].set_database_manager.assert_called_once_with(mock_db_manager)
...
@pytest.mark.skipif(DATA_COLLECTION_CORE_AVAILABLE, reason="Data collection core module should be available")
...
async def test_mock_scenarios():
    ...
    # 模拟任务执行
    try:
        result = {"collected": 10, "status": "success"}
