...
import src.streaming as streaming_pkg
import src.tasks.streaming_tasks as streaming_tasks
...
class DummyProducer:
    ...
    async def send_batch(self, data_list, data_type):
        ...
        self.sent.append((data_list, data_type))
        return {"success": len(data_list), "failed": 0}
...
class DummyConfig:
    def __init__(self, *, existing_topic=True):
        self.existing_topic = existing_topic

    def get_all_topics(self):
        return ["matches", "odds"]

    def is_valid_topic(self, name):
        return self.existing_topic and name in {"matches", "odds"}

    def get_topic_config(self, name):
        return {"name": name, "partitions": 1}


@pytest.fixture(autouse=True)
def patch_error_logger(monkeypatch):
    dummy = DummyErrorLogger()
    monkeypatch.setattr(streaming_tasks, "TaskErrorLogger", lambda: dummy)
    return dummy


def build_task(error_logger):
    base = streaming_tasks.StreamingTask()

    class TaskWrapper:
        def __init__(self):
            self.logger = SimpleNamespace(info=Mock(), error=Mock())
            self.error_logger = error_logger
            self.request = SimpleNamespace(id="task-123", retries=0)

        def run_async(self, coro):
            return base.run_async(coro)
...
def test_consume_kafka_streams_success(monkeypatch, patch_error_logger):
    ...
    monkeypatch.setattr(streaming_tasks, "FootballKafkaConsumer", lambda: consumer)
    task = build_task(patch_error_logger)

    result = call_task(
        streaming_tasks.consume_kafka_streams_task,
        task,
        topics=["matches"],
        batch_size=10,
        timeout=2.0,
    ...
    assert consumer.subscribed_topics == ["matches"]
...
def test_consume_kafka_streams_failure(monkeypatch, patch_error_logger):
    ...
    result = call_task(streaming_tasks.consume_kafka_streams_task, task, topics=None)

    assert result["status"] == "failed"
    assert consumer.subscribed_all is True
    assert consumer.stopped is True
    ...
    assert call["context"]["batch_size"] == 100


def test_start_continuous_consumer_success(monkeypatch, patch_error_logger):
    consumer = DummyConsumer()
    monkeypatch.setattr(
        streaming_tasks,
        "FootballKafkaConsumer",
        lambda consumer_group_id=None: consumer,
    )
    task = build_task(patch_error_logger)
...
def test_start_continuous_consumer_failure(monkeypatch, patch_error_logger):
    consumer = DummyConsumer(should_fail=True)
    monkeypatch.setattr(streaming_tasks, "FootballKafkaConsumer", lambda **_: consumer)
    task = build_task(patch_error_logger)
...
def test_produce_to_kafka_stream_success(monkeypatch, patch_error_logger):
    ...
    monkeypatch.setattr(streaming_tasks, "FootballKafkaProducer", lambda: producer)
    task = build_task(patch_error_logger)

    data = [{"id": 1}, {"id": 2}]
    result = call_task(
        streaming_tasks.produce_to_kafka_stream_task, task, data, "matches"
    )
...
def test_produce_to_kafka_stream_failure(monkeypatch, patch_error_logger):
    ...
    result = call_task(streaming_tasks.produce_to_kafka_stream_task, task, [], "odds")
...
def test_stream_health_check_success(monkeypatch, patch_error_logger):
    ...
    monkeypatch.setattr(streaming_tasks, "StreamProcessor", lambda: processor)
    task = build_task(patch_error_logger)

    result = call_task(streaming_tasks.stream_health_check_task, task)
...
def test_stream_data_processing(monkeypatch, patch_error_logger, start_fail):
    processor = DummyProcessor(start_fail=start_fail)
    monkeypatch.setattr(streaming_tasks, "StreamProcessor", lambda: processor)
    task = build_task(patch_error_logger)

    result = call_task(
        streaming_tasks.stream_data_processing_task,
        task,
        topics=["matches"],
...
def test_kafka_topic_management_list(monkeypatch, patch_error_logger):
    config = DummyConfig()
    monkeypatch.setattr(streaming_pkg, "StreamConfig", lambda: config)
    task = build_task(patch_error_logger)

    result = call_task(streaming_tasks.kafka_topic_management_task, task, action="list")

    assert result["topics"] == ["matches", "odds"]
    task.logger.info.assert_called_once()
...
def test_kafka_topic_management_create_existing(monkeypatch, patch_error_logger):
    ...
    result = call_task(
        streaming_tasks.kafka_topic_management_task,
        task,
        action="create",
        topic_name="matches",
    )
...
def test_kafka_topic_management_failure(monkeypatch, patch_error_logger):
    ...
    monkeypatch.setattr(streaming_pkg, "StreamConfig", broken_config)
    task = build_task(patch_error_logger)
    ...
    assert result["status"] == "failed"
    task.logger.error.assert_called_once()
    assert patch_error_logger.calls[0]["retry_count"] == 3


def test_kafka_topic_management_unsupported(monkeypatch, patch_error_logger):
    config = DummyConfig()
    monkeypatch.setattr(streaming_pkg, "StreamConfig", lambda: config)
    task = build_task(patch_error_logger)
