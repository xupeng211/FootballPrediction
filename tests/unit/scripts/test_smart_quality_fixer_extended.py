...
class TestSmartQualityFixerExtended:
    ...
    def test_fix_ruff_issues_detailed(self):
        """测试Ruff问题修复的详细功能"""
        initial_fix_count = self.fixer.fix_results["fixes_applied"].get("ruff_issues", 0)
    ...
    def test_fix_test_issues(self):
        """测试测试问题修复功能"""
        initial_fix_count = self.fixer.fix_results["fixes_applied"].get("test_issues", 0)
    ...
    def test_apply_refactor_suggestions(self):
        """测试应用重构建议功能"""
        initial_fix_count = self.fixer.fix_results["fixes_applied"].get("refactor_suggestions", 0)
    ...
    def test_fix_mypy_errors_functional(self):
        """测试MyPy错误修复功能"""
        initial_fix_count = self.fixer.fix_results["fixes_applied"].get("mypy_errors", 0)
    ...
    def test_fix_import_errors_functional(self):
        """测试导入错误修复功能"""
        initial_fix_count = self.fixer.fix_results["fixes_applied"].get("import_errors", 0)
    ...
    def test_fix_syntax_errors_functional(self):
        """测试语法错误修复功能"""
        initial_fix_count = self.fixer.fix_results["fixes_applied"].get("syntax_errors", 0)
    ...
    def test_quality_standards_structure(self):
        ...
        # 应该有基本的质量标准键（如果文件存在）
        if standards:
            expected_keys = ['max_line_length', 'max_function_length', 'max_class_length']
            # 至少应该有一些键
