...
class TestDataLakeStorage:
    ...
    def test_init_with_custom_params(self, temp_dir):
        ...
        storage = DataLakeStorage(
            base_path=temp_dir,
            compression="gzip",
            partition_cols=["league", "year"],
            max_file_size_mb=50,
        )

        assert storage.compression == "gzip"
        assert storage.partition_cols == ["league", "year"]
        assert storage.max_file_size_mb == 50
    ...
    async def test_save_historical_data_with_dataframe(
        ...
        result_path = await storage.save_historical_data("raw_matches", df)
    ...
    async def test_save_empty_data(self, storage):
        """测试保存空数据"""
        result_path = await storage.save_historical_data("raw_matches", [])
    ...
    async def test_save_with_invalid_table(self, storage, sample_match_data):
        ...
        with pytest.raises(ValueError, match="Unknown table"):
            await storage.save_historical_data("invalid_table", sample_match_data)

    @pytest.mark.asyncio
    async def test_load_historical_data_empty_table(self, storage):
        """测试从空表加载数据"""
        df = await storage.load_historical_data("raw_matches")
    ...
    async def test_load_with_invalid_table(self, storage):
        ...
        df = await storage.load_historical_data("invalid_table")
        assert isinstance(df, pd.DataFrame)
        assert len(df) == 0
    ...
    async def test_get_table_stats(self, storage, sample_match_data):
        ...
        assert "file_count" in stats  # 实际字段名
    ...
    async def test_cleanup_empty_partitions(self, storage):
        """测试清理空分区"""
        cleaned_count = await storage.cleanup_empty_partitions("raw_matches")
    ...
    def test_table_path_structure(self, storage):
        ...
        for table_name, expected_path in expected_structure.items():
            table_path = storage.tables[table_name]
            assert expected_path in str(table_path)
