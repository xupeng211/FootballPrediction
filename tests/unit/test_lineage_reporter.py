...
def lineage_reporter_instance():
    ...
    return LineageReporter(marquez_url="http://test-marquez:5000")


def test_lineage_reporter_initialization():
    """Test initialization of LineageReporter"""
    reporter = LineageReporter()
    assert reporter is not None
    assert reporter.namespace == "football_prediction"
    assert hasattr(reporter, '_active_runs')
...
def test_start_job_run(lineage_reporter_instance):
    ...
    run_id = lineage_reporter_instance.start_job_run(
        job_name="test_job",
        job_type="BATCH",
        inputs=[{"name": "input_dataset", "namespace": "test_namespace"}],
...
def test_complete_job_run_with_run_id(lineage_reporter_instance):
    ...
    lineage_reporter_instance._active_runs["test_job"] = test_run_id
    
    result = lineage_reporter_instance.complete_job_run(
        job_name="test_job",
        outputs=[{"name": "output_dataset", "namespace": "test_namespace"}],
...
def test_fail_job_run_with_run_id(lineage_reporter_instance):
...
def test_report_data_collection(lineage_reporter_instance):
    ...
    run_id = lineage_reporter_instance.report_data_collection(
        ...
        source_config={"schema": [{"name": "id", "type": "int"}]}
    )
...
def test_report_data_transformation(lineage_reporter_instance):
    ...
    run_id = lineage_reporter_instance.report_data_transformation(
        ...
        transformation_sql="SELECT * FROM table1 JOIN table2",
...
def test_get_active_runs(lineage_reporter_instance):
    ...
    active_runs = lineage_reporter_instance.get_active_runs()
    ...
    active_runs["test_job"] = "modified"
    assert lineage_reporter_instance._active_runs["test_job"] == "test_run_id"


def test_clear_active_runs(lineage_reporter_instance):
    """Test clearing active runs"""
    # Add some test runs
    lineage_reporter_instance._active_runs["test_job1"] = "run1"
    lineage_reporter_instance._active_runs["test_job2"] = "run2"
    
    lineage_reporter_instance.clear_active_runs()
...
def test_detect_exception_in_start_job_run(lineage_reporter_instance):
    ...
    with pytest.raises(Exception, match="Client error"):
        lineage_reporter_instance.start_job_run(
            job_name="failing_job"
        )


def test_detect_exception_in_complete_job_run(lineage_reporter_instance):
    ...
    test_run_id = "test-run-id-exception"
    lineage_reporter_instance._active_runs["test_job"] = test_run_id
