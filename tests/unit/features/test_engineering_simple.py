...
class TestFeatureEntitiesSimple:
    ...
    def test_match_entity_creation_simple(self):
        ...
        assert match_entity.home_score == 2
        assert match_entity.away_score == 1
        assert match_entity.status == "FT"
    ...
    def test_team_entity_creation_simple(self):
        ...
        assert team_entity.founded_year == 1900
        assert team_entity.home_city == "City A"

    def test_feature_key_creation_simple(self):
        """测试特征键创建"""
        feature_key = FeatureKey("team_performance", 1, datetime(2024, 1, 15))

        assert feature_key.name == "team_performance"
        assert feature_key.entity_id == 1
        assert feature_key.timestamp == datetime(2024, 1, 15)
    ...
    def test_feature_entity_creation_simple(self):
        ...
        assert feature_entity.feature_key.name == "team_performance"
        assert feature_entity.feature_key.entity_id == 1
        assert feature_entity.feature_value["goals_scored"] == 2.5
        assert feature_entity.feature_value["goals_conceded"] == 1.2
    ...
    def test_feature_entity_is_expired_simple(self):
        ...
        # 未过期
        current_time = datetime(2024, 1, 11, 10, 0)
        assert feature_entity.is_expired(current_time) is False

        # 已过期
        expired_time = datetime(2024, 1, 13, 10, 0)
        assert feature_entity.is_expired(expired_time) is True
...
class TestFeatureDefinitionsSimple:
    ...
    def test_recent_performance_features_simple(self):
        ...
        # 手动计算统计指标（简化版本）
        wins = sum(1 for match in recent_matches if match["home_score"] > match["away_score"])
        draws = sum(1 for match in recent_matches if match["home_score"] == match["away_score"])
        losses = sum(1 for match in recent_matches if match["home_score"] < match["away_score"])
        goals_scored = sum(match["home_score"] for match in recent_matches)
        goals_conceded = sum(match["away_score"] for match in recent_matches)

        # 验证计算结果
        assert wins == 3  # 2-1, 3-0, 2-1
        assert draws == 1  # 1-1
        assert losses == 1  # 0-2
        assert goals_scored == 8
        assert goals_conceded == 5
    ...
    def test_historical_matchup_features_simple(self):
        ...
        # 手动计算对战统计
        team_a_wins = sum(1 for match in historical_matches
                         if (match["home_team"] == "Team A" and match["home_score"] > match["away_score"]) or
                            (match["home_team"] == "Team B" and match["away_score"] > match["home_score"]))
        team_b_wins = sum(1 for match in historical_matches
                         if (match["home_team"] == "Team B" and match["home_score"] > match["away_score"]) or
                            (match["home_team"] == "Team A" and match["away_score"] > match["home_score"]))
        draws = sum(1 for match in historical_matches if match["home_score"] == match["away_score"])

        # 验证计算结果
        assert team_a_wins == 2
        assert team_b_wins == 2
        assert draws == 1
    ...
    def test_odds_features_simple(self):
        ...
        # 计算隐含概率
        home_win_implied_prob = 1 / odds_data["home_win"]
        draw_implied_prob = 1 / odds_data["draw"]
        away_win_implied_prob = 1 / odds_data["away_win"]

        # 验证概率计算
        assert abs(home_win_implied_prob - 0.476) < 0.01
        assert abs(draw_implied_prob - 0.294) < 0.01
        assert abs(away_win_implied_prob - 0.313) < 0.01

        # 验证庄家优势
        total_implied_prob = home_win_implied_prob + draw_implied_prob + away_win_implied_prob
        bookmaker_margin = total_implied_prob - 1.0
        assert bookmaker_margin > 0
    ...
    def test_all_match_features_simple(self):
        ...
        assert all_features.recent_performance is not None
        assert all_features.historical_matchup is not None
    ...
    def test_all_team_features_simple(self):
        ...
        assert all_features.additional_features["form"] == "good"
...
class TestFeatureCalculatorSimple:
    ...
    def test_calculate_recent_performance_simple(self, sample_team_matches):
        ...
        goals_scored = recent_matches['home_score'].sum() + recent_matches['away_score'].sum()
        goals_conceded = recent_matches['away_score'].sum() + recent_matches['home_score'].sum()
    ...
    def test_calculate_historical_matchup_simple(self):
        ...
        team_a_matches = historical_data[
            ((historical_data['home_team_id'] == team_a_id) | (historical_data['away_team_id'] == team_a_id)) &
            ((historical_data['home_team_id'] == team_b_id) | (historical_data['away_team_id'] == team_b_id))
        ]
        ...
        features = HistoricalMatchupFeatures(
            team_a_id=team_a_id,
            team_b_id=team_b_id,
            historical_matches=team_a_matches.to_dict('records'),
            calculation_date=calculation_date
        )
    ...
    def test_calculate_odds_features_simple(self):
        ...
        # 创建特征对象
        features = OddsFeatures(match_id=match_id, odds_data=odds_data)
        ...
        # 计算隐含概率
        home_implied_prob = 1 / odds_data["home_win"]
        away_implied_prob = 1 / odds_data["away_win"]
        draw_implied_prob = 1 / odds_data["draw"]
    ...
    def test_batch_feature_calculation_simple(self):
        ...
        for team_id, team_data in teams_data.items():
            ...
            features = RecentPerformanceFeatures(
                team_id=team_id,
                recent_matches=team_data.to_dict('records'),
                calculation_date=calculation_date
            )
    ...
    def test_feature_calculation_performance_simple(self):
        ...
        for team_id in team_ids:
            ...
            # 计算统计数据
            avg_goals_scored = team_data['goals_scored'].mean()
...
class TestFeatureEngineeringPipelineSimple:
    ...
    def test_simple_feature_pipeline(self):
        ...
        # 原始数据
        raw_matches = [
            {"match_id": 12345, "home_team_id": 1, "away_team_id": 2, "home_score": 2, "away_score": 1},
            {"match_id": 12346, "home_team_id": 2, "away_team_id": 3, "home_score": 1, "away_score": 1},
            {"match_id": 12347, "home_team_id": 3, "away_team_id": 4, "home_score": 3, "away_score": 0}
        ]
    ...
    def test_feature_transformation_simple(self):
        ...
        def transform_features(features):
            ...
            transformed = {
                ...
                "possession_difference": features["home_possession"] - features["away_possession"],
    ...
    def test_feature_selection_simple(self):
        ...
        def select_important_features(features):
            ...
            important_features = [
                ...
                "total_goals", "goal_difference", "possession_difference",
                "shots_difference", "home_win", "draw", "away_win"
            ]
        ...
        def validate_features(features_list):
            ...
            for features in features_list:
                validation = {
                    ...
                    "valid_goal_difference": isinstance(features["goal_difference"], int),
                    "valid_result_encoding": sum([features["home_win"], features["draw"], features["away_win"]]) == 1
                }
