...
from src.api.predictions import router, get_async_session, prediction_service
...
def test_get_match_prediction_success_with_cached_prediction(client, mock_prediction_service_fixture):
    ...
    mock_prediction.model_name = "football_model"
    ...
    with patch('src.api.predictions.get_async_session') as mock_session_gen:
        ...
        async def mock_execute(query):
            if 'Match' in str(query) or 'match' in str(query).lower():
                mock_query_result_match.scalar_one_or_none.return_value = mock_match
                return mock_query_result_match
            else:
                mock_query_result_prediction.scalar_one_or_none.return_value = mock_prediction
...
def test_get_match_prediction_success_with_real_prediction(client, mock_prediction_service_fixture):
    ...
    with patch('src.api.predictions.get_async_session') as mock_session_gen:
        ...
        mock_prediction_service_fixture.predict_match.return_value = mock_prediction_result
...
def test_predict_match_success(client, mock_prediction_service_fixture):
...
def test_predict_match_match_not_found(client):
    ...
    with patch('src.api.predictions.get_async_session') as mock_session_gen:
        ...
        response = client.post("/predictions/99999/predict")
        assert response.status_code == 404


def test_batch_predict_matches_success(client, mock_prediction_service_fixture):
    ...
    with patch('src.api.predictions.get_async_session') as mock_session_gen:
        ...
        # Mock the scalars method for the .in_ query
        mock_scalars_result = MagicMock()
        mock_scalars_result.all.return_value = [mock_match]
        ...
        mock_prediction_service_fixture.batch_predict_matches.return_value = [mock_prediction_result]

        response = client.post("/predictions/batch", json={"match_ids": [12345]})
        assert response.status_code in [200, 500]


def test_batch_predict_matches_too_many(client):
    """Test batch prediction with too many match IDs"""
    response = client.post("/predictions/batch", json={"match_ids": list(range(55))})
    assert response.status_code == 400
    assert "最多支持50场比赛" in response.json()["detail"]


def test_get_match_prediction_history_success(client):
    ...
    with patch('src.api.predictions.get_async_session') as mock_session_gen:
        ...
        async def mock_execute_side_effect(query):
            ...
            if 'Match' in str(query) or 'match' in str(query).lower():
                ...
                mock_scalars_result = MagicMock()
                mock_scalars_result.all.return_value = [mock_prediction]
...
def test_get_match_prediction_history_match_not_found(client):
    ...
    with patch('src.api.predictions.get_async_session') as mock_session_gen:
        ...
        response = client.get("/predictions/history/99999")
        assert response.status_code == 404


def test_get_recent_predictions_success(client):
    """Test successful retrieval of recent predictions"""
    # Mock result object for recent predictions query
    mock_result = MagicMock()
    mock_result.id = 1
    mock_result.match_id = 12345
    mock_result.model_version = "1.0"
    mock_result.model_name = "football_model"
    mock_result.predicted_result = "home"
    mock_result.confidence_score = 0.45
    mock_result.created_at = datetime.now()
    mock_result.is_correct = None
    mock_result.home_team_id = 10
    mock_result.away_team_id = 20
    mock_result.match_time = datetime.now()
    mock_result.match_status = "scheduled"
    ...
    with patch('src.api.predictions.get_async_session') as mock_session_gen:
        ...
        mock_query_result.fetchall.return_value = [mock_result]
        mock_session.execute.return_value = mock_query_result
...
def test_verify_prediction_success(client, mock_prediction_service_fixture):
...
def test_verify_prediction_error(client, mock_prediction_service_fixture):
    ...
    with patch('src.api.predictions.get_async_session') as mock_session_gen:
        ...
        mock_prediction_service_fixture.verify_prediction.side_effect = Exception("Verification error")
