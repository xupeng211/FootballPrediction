...
class TestSystemMonitor:
    ...
    def test_get_current_metrics(self, system_monitor):
        ...
        with patch('psutil.cpu_percent', return_value=45.5), \
            ...
            assert metrics.memory_used_mb == mock_memory.used / (1024 * 1024)
            assert metrics.memory_available_mb == mock_memory.available / (1024 * 1024)
            assert metrics.disk_usage_percent == 30.0
    ...
    def test_get_metrics_summary_with_data(self, system_monitor):
        ...
        assert cpu_stats['avg'] == 55.0  # (50 + 68) / 2
    ...
    def test_check_performance_alerts_high_cpu(self, system_monitor):
        ...
        with patch('psutil.cpu_percent', return_value=85.0), \
            ...
            assert len(alerts) >= 1
            cpu_alert = next((alert for alert in alerts if alert['type'] == 'cpu_high'), None)
    ...
    def test_check_performance_alerts_memory(self, system_monitor):
        ...
        with patch('psutil.cpu_percent', return_value=30.0), \
            ...
            memory_alert = next((alert for alert in alerts if alert['type'] == 'memory_high'), None)
    ...
    def test_check_performance_alerts_response_time(self, system_monitor):
        ...
        with patch('psutil.cpu_percent', return_value=30.0), \
            ...
            # 模拟慢响应时间
            with patch.object(system_monitor, '_get_avg_response_time', return_value=2.5):
                metrics = system_monitor.get_current_metrics()
                alerts = system_monitor.check_performance_alerts()

                response_alert = next((alert for alert in alerts if alert['type'] == 'response_time_high'), None)
...
class TestPerformanceAnalyzer:
    ...
    def test_analyze_trends_with_data(self, performance_analyzer):
        ...
        assert cpu_trend['trend'] in ['increasing', 'decreasing', 'stable', 'insufficient_data']
    ...
    def test_get_performance_recommendations(self, performance_analyzer):
        ...
        recommendations = performance_analyzer.get_performance_recommendations(metrics_summary)

        assert isinstance(recommendations, list)
        assert len(recommendations) > 0

        # 应该有CPU使用率建议
        cpu_recommendation = next((rec for rec in recommendations if 'CPU' in rec), None)
        assert cpu_recommendation is not None

        # 应该有内存使用率建议
        memory_recommendation = next((rec for rec in recommendations if '内存' in rec), None)
        assert memory_recommendation is not None

        # 应该有响应时间建议
        response_recommendation = next((rec for rec in recommendations if '响应时间' in rec), None)
