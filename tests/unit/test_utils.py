...
def test_api_response_error():
    ...
    response = APIResponse.error(message="Error occurred", data={"info": "details"})

    assert response["success"] is False
    assert response["message"] == "Error occurred"
    assert response["data"]["info"] == "details"
    assert "code" in response
    assert response["code"] == 500


def test_api_response_with_none_data():
    """Test APIResponse with None data"""
    response = APIResponse.success(data=None, message="Success")

    assert response["success"] is True
    assert response["message"] == "Success"
    assert "data" not in response  # None data is not included
...
def test_filter_none_values():
    ...
    filtered = DictUtils.filter_none_values(dict_with_nones)
...
def test_truncate_string():
    ...
    truncated = StringUtils.truncate(long_string, length=20)
    assert len(truncated) <= 20  # Account for the appended "..."
    assert truncated.endswith("...")
    
    # Test with no truncation needed
    short_string = "Short"
    result = StringUtils.truncate(short_string, length=20)
...
def test_clean_text():
    ...
    assert clean == "This is a test string"
...
def test_now_utc():
    ...
    # This test checks if the function returns a datetime object
...
def test_format_datetime():
    ...
    assert formatted == "2023-10-15 10:30:00"
...
def test_ensure_dir():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        new_dir = os.path.join(temp_dir, "test", "subdir")
        result_path = FileUtils.ensure_dir(new_dir)
        
        assert os.path.exists(new_dir)
        assert str(result_path) == new_dir
...
def test_read_write_json_file():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        FileUtils.write_json(data, json_file)
...
def test_read_json_file_nonexistent():
    ...
    with pytest.raises(FileNotFoundError):
        FileUtils.read_json("/nonexistent/path.json")
...
def test_crypto_utils_hash_string():
    ...
    hashed_md5 = CryptoUtils.hash_string(original, "md5")
    ...
    hashed_sha256 = CryptoUtils.hash_string(original, "sha256")
    assert len(hashed_sha256) == 64  # SHA256 hash length


def test_crypto_utils_verify_password():
    """Test password verification"""
    original = "test_password_123"
    wrong_password = "wrong_password"
    password_hash = CryptoUtils.hash_password(original)

    assert CryptoUtils.verify_password(original, password_hash) is True
...
def test_hash_consistency():
    ...
    hash1 = CryptoUtils.hash_string(original, "md5")
    hash2 = CryptoUtils.hash_string(original, "md5")
...
def test_truncate_string_shorter_than_limit():
    ...
    short_string = "short"
    result = StringUtils.truncate(short_string, length=20)
    assert result == short_string  # Should not be modified
...
def test_crypto_utils_with_empty_strings():
    ...
    assert empty_hash == hashlib.md5("".encode("utf-8"), usedforsecurity=False).hexdigest()

    # Should not be equal to non-empty hash
    non_empty_hash = CryptoUtils.hash_string("not_empty", "md5")
...
def test_filter_none_values_no_nones():
    ...
    input_dict = {"a": 1, "b": "value", "c": [1, 2, 3]}
    result = DictUtils.filter_none_values(input_dict)
...
def test_file_utils_read_json_file_alias():
...
def test_file_utils_read_json_file_nonexistent_alias():
    ...
    result = FileUtils.read_json_file("/nonexistent/path.json")
