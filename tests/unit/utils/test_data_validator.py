...
def test_validate_required_fields():
    data = {"a": 1, "b": None}
    missing = DataValidator.validate_required_fields(data, ["a", "b", "c"])
    assert missing == ["b", "c"]


def test_validate_data_types():
    data = {"count": "5", "enabled": True}
    type_specs = {"count": int, "enabled": bool}
    invalid = DataValidator.validate_data_types(data, type_specs)
    assert invalid == ["count: 期望 int, 实际 str"]
...
def test_validate_patterns():
    ...
    assert not DataValidator.validate_email("bad-email")
    assert DataValidator.validate_phone("13800138000")
    assert DataValidator.validate_phone("+12025550123")
