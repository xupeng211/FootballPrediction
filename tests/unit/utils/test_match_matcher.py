...
class TestFotmobMatchMatcher:
    ...
    @pytest.mark.asyncio
    async def test_find_match_by_fuzzy_match_exact_name(self, matcher, mock_fotmob_matches_response):
        """测试通过模糊匹配找到完全相同的队名"""
        # 直接Mock _get_matches_by_date方法返回测试数据
        with patch.object(matcher, '_get_matches_by_date', new_callable=AsyncMock) as mock_get_matches:
    ...
    @pytest.mark.asyncio
    async def test_find_match_by_fuzzy_match_similar_name(self, matcher, mock_fotmob_matches_response):
        """测试通过模糊匹配找到相似的队名"""
        # 直接Mock _get_matches_by_date方法返回测试数据
        with patch.object(matcher, '_get_matches_by_date', new_callable=AsyncMock) as mock_get_matches:
            mock_get_matches.return_value = mock_fotmob_matches_response
    ...
    @pytest.mark.asyncio
    async def test_find_match_by_fuzzy_match_no_match(self, matcher, mock_fotmob_matches_response):
        """测试找不到匹配的比赛"""
        mock_response = Mock()
    ...
    @pytest.mark.asyncio
    async def test_find_match_by_fuzzy_match_multiple_candidates(self, matcher, mock_fotmob_matches_response):
        """测试有多个候选匹配时选择最佳匹配"""
        # 直接Mock _get_matches_by_date方法返回测试数据
        with patch.object(matcher, '_get_matches_by_date', new_callable=AsyncMock) as mock_get_matches:
            mock_get_matches.return_value = mock_fotmob_matches_response
    ...
    def test_calculate_team_similarity_exact_match(self, matcher):
        """测试队名完全相同的相似度计算"""
        similarity = matcher._calculate_team_similarity("Manchester United", "Manchester United")
    ...
    @pytest.mark.asyncio
    async def test_get_matches_by_date_success(self, matcher, mock_fotmob_matches_response):
        """测试成功获取指定日期的比赛列表"""
        # 直接Mock多个联赛API响应
        ...
        mock_league_response.json.return_value = {
            "fixtures": {
                "allMatches": mock_fotmob_matches_response["matches"]
            }
        }
