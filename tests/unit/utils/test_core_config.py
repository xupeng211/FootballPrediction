...
class TestConfig:
    ...
    def test_config_init_without_file(self, tmp_path):
        """测试没有配置文件时的初始化"""
        with patch("src.core._config.Path.home", return_value=tmp_path):
            _config = Config()
            assert isinstance(_config._config, dict)
            assert len(_config._config) == 0
    ...
    def test_config_init_with_file(self, tmp_path):
        ...
        config_dir.mkdir()
        config_file = config_dir / "_config.json"
    ...
    def test_config_load_invalid_json(self, tmp_path):
        ...
        with patch("src.core._config.Path.home", return_value=tmp_path):
            with patch("src.core._config.logging.warning") as mock_warning:
                _config = Config()
    ...
    def test_config_get_with_default(self, tmp_path):
        ...
        with patch("src.core._config.Path.home", return_value=tmp_path):
            ...
            assert _config.get("nonexistent_key", 0) == 0

    def test_config_set_and_get(self, tmp_path):
        """测试设置和获取配置项"""
        with patch("src.core._config.Path.home", return_value=tmp_path):
            _config = Config()
            _config.set("key1", "value1")
            _config.set("key2", 123)
            _config.set("key3", {"nested": "value"})

            assert _config.get("key1") == "value1"
            assert _config.get("key2") == 123
            assert _config.get("key3") == {"nested": "value"}

    def test_config_save(self, tmp_path):
        """测试保存配置"""
        with patch("src.core._config.Path.home", return_value=tmp_path):
            _config = Config()
            _config.set("test", "value")
            _config.save()

            # 验证文件已创建并包含正确内容
            config_file = tmp_path / ".footballprediction" / "_config.json"
    ...
    def test_config_key_conversion(self, tmp_path):
        ...
        with patch("src.core._config.Path.home", return_value=tmp_path):
            _config = Config()
            _config.set("123", "numeric_key")
            assert _config.get("123") == "numeric_key"
            # 键会被自动转换为字符串
            assert _config.get(123) == "numeric_key"
    ...
    def test_config_complex_types(self, tmp_path):
        ...
        with patch("src.core._config.Path.home", return_value=tmp_path):
            ...
            _config.set("complex", complex_data)
            _config.save()
