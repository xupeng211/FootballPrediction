...
class TestDictUtilsComprehensive:
    ...
    def test_merge_overwrite(self):
        ...
        dict2 = {"b": 99, "c": 3}
        _result = DictUtils.merge(dict1, dict2)
        assert _result == {"a": 1, "b": 99, "c": 3}

    def test_merge_deep(self):
        """测试：深度合并"""
        dict1 = {"a": {"x": 1, "y": 2}}
        dict2 = {"a": {"y": 99, "z": 3}}
        _result = DictUtils.merge(dict1, dict2, deep=True)
        assert _result == {"a": {"x": 1, "y": 99, "z": 3}}
    ...
    def test_merge_deep_nested_levels(self):
        ...
        dict2 = {"a": {"b": {"c": 99, "e": 3}}}
        _result = DictUtils.merge(dict1, dict2, deep=True)
        assert _result == {"a": {"b": {"c": 99, "d": 2, "e": 3}}}
    ...
    def test_merge_three_dicts(self):
        ...
        dict2 = {"b": 2}
        dict3 = {"c": 3}
        _result = DictUtils.merge(dict1, dict2, dict3)
        assert _result == {"a": 1, "b": 2, "c": 3}
    ...
    def test_flatten_simple(self):
        """测试：简单扁平化"""
        _data = {"a": 1, "b": 2}
        _result = DictUtils.flatten(data)
    ...
    def test_flatten_list(self):
        """测试：包含列表的扁平化"""
        _data = {"a": [1, 2, 3], "b": {"c": [4, 5]}}
        _result = DictUtils.flatten(data)
        assert _result == {"a": [1, 2, 3], "b.c": [4, 5]}
    ...
    def test_pick_keys(self):
        ...
        keys = ["a", "c"]
        _result = DictUtils.pick(data, keys)
    ...
    def test_omit_keys(self):
        ...
        _data = {"a": 1, "b": 2, "c": 3, "d": 4}
        keys = ["b", "d"]
        _result = DictUtils.omit(data, keys)
        assert _result == {"a": 1, "c": 3}
    ...
    def test_rename_keys(self):
        """测试：重命名键"""
        _data = {"old_name": 1, "keep_name": 2}
        mapping = {"old_name": "new_name"}
        _result = DictUtils.rename_keys(data, mapping)
        assert _result == {"new_name": 1, "keep_name": 2}
    ...
    def test_rename_keys_conflict(self):
        ...
        mapping = {"a": "b"}  # 会覆盖现有的b
        _result = DictUtils.rename_keys(data, mapping)
        assert _result == {"b": 1}  # 原来的b被覆盖

    def test_filter_by_value(self):
        """测试：按值过滤"""
        _data = {"a": 1, "b": 2, "c": 3, "d": 1}
        _result = DictUtils.filter_by_value(data, lambda v: v == 1)
        assert _result == {"a": 1, "d": 1}

    def test_filter_by_key(self):
        """测试：按键过滤"""
        _data = {"apple": 1, "banana": 2, "apricot": 3}
        _result = DictUtils.filter_by_key(data, lambda k: k.startswith("a"))
        assert _result == {"apple": 1, "apricot": 3}

    def test_invert_dict(self):
        """测试：反转字典"""
        _data = {"a": 1, "b": 2, "c": 3}
        _result = DictUtils.invert(data)
        assert _result == {1: "a", 2: "b", 3: "c"}
