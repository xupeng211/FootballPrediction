...
def test_write_and_read_json(tmp_path):
    ...
    success = FileUtils.write_json_file(payload, file_path)
...
def test_get_file_hash_and_size(tmp_path):
    ...
    assert FileUtils.get_file_size(file_path) == len("hello world")
...
def test_cleanup_old_files(tmp_path):
    ...
    removed = FileUtils.cleanup_old_files(tmp_path, days=30)
    assert removed == 1
...
def test_read_json_file_missing(tmp_path):
    assert FileUtils.read_json_file(tmp_path / "missing.json") is None


def test_write_json_file_handles_error(monkeypatch, tmp_path):
    ...
    assert FileUtils.write_json_file({"a": 1}, file_path) is False
