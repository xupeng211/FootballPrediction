...
class TestBackupTasksBatchGamma010Additional:
    ...
    def test_run_backup_script_success_with_file_size(self, backup_task):
        ...
        mock_result.stdout = "Backup successful"
        mock_result.stderr = ""
        ...
        with patch('subprocess.run', return_value=mock_result):
            ...
            assert stats["backup_file_size_bytes"] == 1024000

    def test_run_backup_script_timeout_exception(self, backup_task):
        """测试备份脚本执行超时异常"""
    ...
    def test_get_latest_backup_size_full_backup_with_files(self, backup_task):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            size = backup_task._get_latest_backup_size("full")
    ...
    def test_get_latest_backup_size_incremental_backup(self, backup_task):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            mock_find_result.stdout = "/backup/incremental/20240101\n/backup/incremental/20240102"
            ...
            mock_run.side_effect = [mock_find_result, mock_du_result]

            size = backup_task._get_latest_backup_size("incremental")
    ...
    def test_verify_backup_timeout_exception(self, backup_task):
        ...
        with patch('subprocess.run', side_effect=subprocess.TimeoutExpired("restore.sh", 300)):
            with patch('src.tasks.backup_tasks.logger') as mock_logger:
                result = backup_task.verify_backup("/backup/test_backup.sql.gz")
    ...
    def test_verify_backup_general_exception(self, backup_task):
        ...
        with patch('subprocess.run', side_effect=OSError("File not found")):
            with patch('src.tasks.backup_tasks.logger') as mock_logger:
                result = backup_task.verify_backup("/backup/nonexistent.sql.gz")
    ...
    def test_get_backup_metrics_error_handling(self):
        ...
        with patch('prometheus_client.REGISTRY') as mock_registry:
            ...
            assert metrics["success_total"].inc is not None

    def test_backup_task_initialization_with_custom_registry(self):
    ...
    def test_backup_task_path_initialization(self, backup_task):
        ...
        # 验证备份目录从环境变量获取
        expected_backup_dir = os.getenv("BACKUP_DIR", "/backup/football_db")
        assert backup_task.backup_dir == expected_backup_dir

    def test_backup_config_environment_variables(self, backup_task):
        """测试备份配置环境变量"""
        config = backup_task.get_backup_config()

        # 验证环境变量被正确读取
        assert config["backup_dir"] == os.getenv("BACKUP_DIR", "/backup/football_db")
        assert config["max_backup_age_days"] == int(os.getenv("MAX_BACKUP_AGE_DAYS", "30"))
        assert config["compression"] is True

    def test_backup_config_retention_policy(self, backup_task):
        """测试备份配置保留策略"""
        config = backup_task.get_backup_config()
        ...
        # 验证默认值
        assert retention["full_backups"] == int(os.getenv("KEEP_FULL_BACKUPS", "7"))
        assert retention["incremental_backups"] == int(os.getenv("KEEP_INCREMENTAL_BACKUPS", "30"))
        assert retention["wal_archives"] == int(os.getenv("KEEP_WAL_ARCHIVES", "7"))

    def test_backup_config_notification_settings(self, backup_task):
        """测试备份配置通知设置"""
        config = backup_task.get_backup_config()
        ...
        # 验证环境变量处理
        expected_enabled = os.getenv("BACKUP_NOTIFICATIONS", "true").lower() == "true"
        assert notification["enabled"] == expected_enabled