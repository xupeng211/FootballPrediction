...
class TestCryptoUtils:
    ...
    assert re.match(uuid_pattern, generated_uuid)
    ...
    assert all(c in "0123456789abcdef-" for c in short_id.replace("-", ""))
    ...
    def test_hash_string_md5(self):
        ...
        expected_hash = hashlib.md5(
            test_string.encode("utf-8"), usedforsecurity=False
        ).hexdigest()

        result = CryptoUtils.hash_string(test_string, "md5")
    ...
    def test_hash_string_md5_empty_string(self):
        """测试空字符串的MD5哈希"""
        result = CryptoUtils.hash_string("", "md5")
    ...
    def test_hash_string_sha256(self):
        """测试字符串的SHA256哈希"""
        test_string = "Hello, SHA256!"
        expected_hash = hashlib.sha256(test_string.encode("utf-8")).hexdigest()

        result = CryptoUtils.hash_string(test_string, "sha256")
    ...
    def test_hash_password_bcrypt_available(self):
        """测试bcrypt可用时的密码哈希"""
    ...
    assert hashed.startswith("$2b$")  # bcrypt标识符

        except ImportError:
            pytest.skip("bcrypt not available")

    def test_hash_password_bcrypt_unavailable(self):
        """测试bcrypt不可用时的密码哈希回退"""
        password = "test_password_123"
    ...
    def test_all_methods_with_different_inputs(self):
        ...
        for data in test_data:
            # 测试哈希方法
            md5_hash = CryptoUtils.hash_string(data, "md5")
            sha256_hash = CryptoUtils.hash_string(data, "sha256")
    ...
    def test_edge_cases(self):
        ...
        md5_result = CryptoUtils.hash_string(long_string, "md5")
        sha256_result = CryptoUtils.hash_string(long_string, "sha256")
    ...
    @patch("src.utils.crypto_utils.secrets.token_hex")
    def test_generate_salt_secrets_fallback(self, mock_token_hex):
        """测试secrets模块异常时的处理"""
    ...
    @patch("src.utils.crypto_utils.secrets.token_hex")
    def test_generate_token_secrets_fallback(self, mock_token_hex):
        """测试secrets模块异常时的处理"""
        mock_token_hex.side_effect = Exception("secrets error")
