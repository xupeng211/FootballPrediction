...
from src.evaluation.visualizer import (
    EvaluationVisualizer, create_evaluation_plots,
)
...
class TestEvaluationVisualizer:
    ...
    def test_plot_backtest_results_empty(self, visualizer):
        ...
        empty_result = BacktestResult(
            initial_bankroll=1000.0, final_bankroll=1000.0, total_bets=0,
            winning_bets=0, losing_bets=0, total_stake=0.0, total_winnings=0.0,
            net_profit=0.0, roi=0.0, win_rate=0.0, avg_odds=0.0,
            max_consecutive_losses=0, max_consecutive_wins=0,
            max_drawdown=0.0, max_drawdown_percentage=0.0,
            sharpe_ratio=0.0, calmar_ratio=0.0, profit_factor=0.0,
            avg_profit_per_bet=0.0, std_profit_per_bet=0.0,
            bets=[], equity_curve=[1000.0], metadata={}
        )
    ...
    @patch('matplotlib.pyplot.savefig')
    def test_create_comprehensive_report(self, visualizer, sample_data, sample_backtest_result):
        """测试综合报告创建"""
        y_true, y_pred, y_proba = sample_data
        from src.evaluation.metrics import MetricsResult
        from datetime import datetime
    ...
    @patch('pandas.DataFrame.to_csv')
    def test_export_data_for_tableau_with_backtest(self, visualizer, sample_data, sample_backtest_result):
        """测试包含回测数据的Tableau导出"""
        y_true, y_pred, y_proba = sample_data
...
class TestConvenienceFunctions:
    ...
    def test_create_evaluation_plots(self, sample_data):
        ...
        with patch('src.evaluation.visualizer.EvaluationVisualizer') as mock_visualizer_class:
    ...
    def test_create_evaluation_plots_with_backtest(self, sample_data):
        ...
        backtest_result = BacktestResult(
            initial_bankroll=1000.0, final_bankroll=1050.0, total_bets=10,
            winning_bets=6, losing_bets=4, total_stake=100.0, total_winnings=150.0,
            net_profit=50.0, roi=50.0, win_rate=60.0, avg_odds=2.5,
            max_consecutive_losses=2, max_consecutive_wins=3,
            max_drawdown=20.0, max_drawdown_percentage=2.0,
            sharpe_ratio=1.0, calmar_ratio=25.0, profit_factor=2.5,
            avg_profit_per_bet=5.0, std_profit_per_bet=10.0,
            bets=[], equity_curve=[1000.0, 1050.0], metadata={}
        )
