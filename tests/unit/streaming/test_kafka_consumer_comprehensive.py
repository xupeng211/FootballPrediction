...
class TestFootballKafkaConsumerComprehensive:
    ...
    def mock_config(self):
        """创建模拟流配置"""
        config = Mock()
        config.kafka_servers = ["localhost:9092"]
        config.kafka_topics = ["matches", "odds", "scores"]
        config.kafka_group_id = "football_consumer_group"
        config.kafka_auto_offset_reset = "earliest"
        config.kafka_enable_auto_commit = True
        return config
    ...
    def mock_kafka_message(self):
        """创建模拟 Kafka 消息"""
        mock_message = Mock()
        mock_message.topic = "matches"
        mock_message.partition = 0
        mock_message.offset = 123
        mock_message.key = b"match_12345"
    ...
    def test_deserialize_message_valid_json(self, consumer):
        """测试有效JSON消息反序列化"""
        message_data = {"key": "value", "number": 123}
    ...
    def test_deserialize_message_invalid_json(self, consumer):
        """测试无效JSON消息反序列化"""
        invalid_bytes = b"invalid json string"
    ...
    async def test_process_kafka_message_with_error(self, consumer):
        """测试处理带错误的Kafka消息"""
        error_message = Mock()
        error_message.error = Exception("Kafka error")
    ...
    async def test_consume_batch_success(self, consumer, mock_kafka_message):
        ...
        with patch.object(consumer._consumer, 'consume') as mock_consume, \
            ...
            # 模拟批量消息
            mock_consume.return_value = [mock_kafka_message]
            mock_process.return_value = True
    ...
    def test_deserialize_large_message(self, consumer):
        ...
        large_bytes = json.dumps(large_data).encode('utf-8')
    ...
    async def test_batch_processing_workflow(self, consumer, mock_kafka_message):
        ...
        with patch.object(consumer._consumer, 'consume') as mock_consume, \
            ...
            # 模拟批量消息
            messages = [mock_kafka_message] * 10
            mock_consume.return_value = messages
            mock_process.return_value = True
    ...
    async def test_reconnection_handling(self, consumer):
        ...
        with patch.object(consumer._consumer, 'poll') as mock_poll:
            ...
            mock_poll.side_effect = mock_poll_with_recovery
    ...
    def test_consumer_group_configuration(self, mock_config):
        ...
        custom_group_id = "custom_consumer_group"
