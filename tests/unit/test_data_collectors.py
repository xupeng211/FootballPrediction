    ...
    async def collect_odds(self, **kwargs):
        ...
        return CollectionResult(
            ...
            collected_data=[
                ...
                {"match_id": 2, "home_odds": 1.8, "draw_odds": 3.6, "away_odds": 4.5},
            ],
        )
...
class TestDataCollector:
    ...
    def mock_collector(self):
        ...
        with patch("src.data.collectors.base_collector.DatabaseManager"):
            return MockDataCollector(
                data_source="test_api", max_retries=2, retry_delay=1
            )
    ...
    async def test_collect_all_data(self, mock_collector):
        """测试完整数据采集流程"""
        # Mock数据库相关方法
        with patch.object(mock_collector, "_create_collection_log", return_value=1):
            with patch.object(
                mock_collector, "_update_collection_log", return_value=None
            ):
...
class TestDataCollectorErrorHandling:
    ...
    def error_collector(self):
        ...
        with patch("src.data.collectors.base_collector.DatabaseManager"):
            return ErrorDataCollector(data_source="error_api")

    @pytest.mark.asyncio
    async def test_collect_all_data_with_errors(self, error_collector):
        """测试带错误的完整采集流程"""
        with patch.object(error_collector, "_create_collection_log", return_value=1):
            with patch.object(
                error_collector, "_update_collection_log", return_value=None
            ):
