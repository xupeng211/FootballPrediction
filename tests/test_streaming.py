...
1. 环境配置：
   ...
   export KAFKA_GROUP_ID="football-data-consumers"
...
class FakeKafkaProducer:
    ...
    def __init__(self, configs):
        ...
        self.messages = []  # 存储所有"发送"的消息
        self.simulate_failure = False  # 🛡️ 用于模拟发送失败的开关
    ...
    def produce(self, topic, key, value, callback):
        ...
        if self.simulate_failure:
            ...
            if callback:
                # 模拟发送失败，传递错误对象
                error = Exception(f"模拟Kafka发送失败 #{self.failure_count}")
                callback(error, None)  # err=error表示失败
            # 抛出异常让producer的try-catch捕获到失败
            raise Exception(f"模拟Kafka发送失败 #{self.failure_count}")
            return
...
class MockMessage:
    ...
    def create_error_message(
        ...
        error_mock.str.return_value = error_description or f"Kafka error {error_code}"
        return cls(error=error_mock, value=None)
    ...
    def create_normal_message(
        ...
        return cls(
            value=serialized_data, topic=topic, partition=partition, offset=offset
        )
...
class TestFootballKafkaConsumer:
    ...
    async def test_process_match_message(self, mock_db_manager_class):
        """测试处理比赛消息"""
    ...
    @pytest.mark.asyncio
    @patch("src.streaming.kafka_consumer.DatabaseManager")
    async def test_process_odds_message(self, mock_db_manager_class):
        """测试处理赔率消息"""
        # 🔧 修复AsyncMock协程警告 - 确保session.add和session.commit正确模拟
        mock_session = AsyncMock()
        # 将add设为普通Mock，因为在真实代码中它不是协程
    ...
    @pytest.mark.asyncio
    @patch("src.streaming.kafka_consumer.DatabaseManager")
    async def test_process_scores_message(self, mock_db_manager_class):
        """测试处理比分消息"""
        # 🔧 修复AsyncMock协程警告 - 确保session.add和session.commit正确模拟
        mock_session = AsyncMock()
        # 将add设为普通Mock，因为在真实代码中它不是协程
    ...
    async def test_consumer_error_simulation(self):
        ...
        # 添加正常消息和错误消息混合
        normal_message = MockMessage(
            value=json.dumps({"data_type": "match", "data": {"match_id": 1}}).encode()
        )

        consumer.consumer.messages = [
            normal_message,  # 第1条：正常
            None,  # 第2条：会触发错误模拟
            normal_message,  # 第3条：正常
        ]
...
class TestStreamingDataCollector:
    ...
    async def test_collect_fixtures_with_streaming(self):
        ...
        with patch(
            ...
            "src.data.collectors.streaming_collector.DataCollector.__init__",
            return_value=None,
        ), patch.object(
            StreamingDataCollector, "collect_fixtures", new_callable=AsyncMock
    ...
    async def test_collect_odds_with_streaming(self):
        ...
        with patch(
            ...
            StreamingDataCollector, "collect_odds", new_callable=AsyncMock
        ) as mock_base_collect:
            # 设置 mock 返回值
            from src.data.collectors.base_collector import CollectionResult
            ...
            mock_result = CollectionResult(
                ...
                error_count=0,
                status="success",
                collected_data=[{"bookmakers": [{"name": "test", "bets": []}]}],
            )
            mock_base_collect.return_value = mock_result
    ...
    async def test_collect_live_scores_with_streaming(self):
        ...
        with patch(
            ...
            StreamingDataCollector, "collect_live_scores", new_callable=AsyncMock
        ) as mock_base_collect:
            # 设置 mock 返回值
            from src.data.collectors.base_collector import CollectionResult
...
class TestKafkaIntegration:
    ...
    @pytest.mark.asyncio
    @patch("src.streaming.kafka_consumer.DatabaseManager")
    async def test_end_to_end_flow(self, mock_db_manager_class):
        """测试端到端流程"""
        config = StreamConfig()
        ...
        if "data_type" not in message_content:
            message_content["data_type"] = "match"
            message_content["source"] = "test_source"
            updated_message = json.dumps(message_content)
            consumer.consumer.messages = [MockMessage(value=updated_message.encode())]
    ...
    def test_data_serialization(self):
        """测试数据序列化"""
        config = StreamConfig()
        with patch("src.streaming.kafka_producer.Producer", new=FakeKafkaProducer):
            producer = FootballKafkaProducer(config)
            test_data = {
                "match_id": 12345,
