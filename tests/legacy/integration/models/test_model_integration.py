...
try:
    ...
    from src.database.models import League, Team, Match, Predictions
    ...
    # Mock imports for testing = DatabaseManager MagicMock
    League = Team = Match = Predictions = MagicMock
    BaselineModelTrainer = PredictionService = MagicMock
    ModelMetricsExporter = MagicMock
...
class TestModelIntegration:
    ...
    async def db_session(self):
        ...
        if not hasattr(db_manager, "_async_engine[") or db_manager._async_engine is None:": db_manager.initialize()": async with db_manager.get_async_session() as session:": yield session"
    ...
    async def test_model_training_workflow(
        ...
        "]]]""测试模型训练完整流程"""
        trainer = BaselineModelTrainer(mlflow_tracking_uri="http://test5002[")""""
        ...
        with patch.object(
            ...
            feature_data = pd.DataFrame(
                {
                    ...
                    "]event_timestamp[: ": datetime(2024, 6, 1) + timedelta(days = i) for i in range(10)""""
                    ",""""
            ...
            mock_feature_store.get_historical_features.return_value = feature_data

            # 模拟XGBoost训练
            with patch("]src.models.model_training.xgb.XGBClassifier[") as mock_xgb:": mock_model = MagicMock()": mock_model.predict.return_value = np.array(["]home["] * 8 + ["]away["] * 2)": mock_model.predict_proba.return_value = np.array("""
                    [
                ...
                # 测试训练
                run_id = await trainer.train_baseline_model("]test_experiment[")": assert run_id =="]test_run_id_123[" mock_xgb.assert_called_once()""""
    ...
    async def test_prediction_service_workflow(
        ...
        "]""测试预测服务完整流程"""
        prediction_service = PredictionService(mlflow_tracking_uri="http://test5002[")""""

        # 模拟MLflow客户端
        with patch("]src.models.prediction_service.MlflowClient[") as mock_client_class:": mock_client = MagicMock()": mock_client_class.return_value = mock_client[""
            ...
            with patch(
            ...
            ) as mock_load_model = mock_model MagicMock()
                mock_model.predict.return_value = np.array(["]home["])": mock_model.predict_proba.return_value = np.array("""
                    [[0.2, 0.3, 0.5]]
                )  # away, draw, home
                ...
                with patch.object(
                    ...
                    mock_feature_store.get_match_features_for_prediction.return_value = {
                        "]home_recent_wins[": 3,""""
                        "]away_recent_wins[": 2,""""
                        "]home_recent_goals_for[": 15,""""
                        "]home_recent_goals_against[": 8,""""
                        "]away_recent_goals_for[": 12,""""
                        "]away_recent_goals_against[": 10,""""
        ...
        assert result.match_id ==sample_match_data["]]match["].id[" assert result.model_version =="]]1[" assert result.predicted_result in ["]home[", "]draw[", "]away["]" assert 0 <= result.confidence_score <= 1["""
        ...
        assert stored_prediction.model_name =="]]football_baseline_model[" assert stored_prediction.model_version =="]1["""""
    ...
    async def test_prediction_verification(
        ...
        assert prediction.actual_result =="]]]home["  # 主队获胜[" assert prediction.is_correct is True["""
    ...
    async def test_batch_prediction(
        ...
        with patch.object(
            ...
            with patch(
                ...
                mock_model.predict.return_value = np.array(["]home[", "]away[", "]draw["])": mock_model.predict_proba.return_value = np.array(["""
                    [0.5, 0.3, 0.2],
                    [0.2, 0.3, 0.5],
                    [0.3, 0.4, 0.3]
                ])
    ...
    async def test_model_accuracy_calculation(
        ...
        # 测试准确率计算
        accuracy = await prediction_service.get_model_accuracy("]test_model[", days=30)": expected_accuracy = correct_predictions / total_predictions[": assert accuracy ==expected_accuracy[""
    ...
    async def test_prediction_statistics(
        ...
        # 验证v1版本统计
        v1_stats = next(s for s in stats["]]statistics["] if s["]model_version["] =="]v1[")": assert v1_stats["]total_predictions["] ==3[" assert v1_stats["]]verified_predictions["] ==2[" assert v1_stats["]]accuracy["] ==0.5  # 1 correct out of 2 verified[" def test_prediction_result_to_dict(self):"""
        ...
        assert result_dict["]match_id["] ==123[" assert result_dict["]]model_version["] =="]v1[" assert result_dict["]predicted_result["] =="]home[" assert result_dict["]confidence_score["] ==0.5[" assert result_dict["]]features_used["] =={"]feature1[" 1.0, "]feature2[" 2.0}" assert result_dict["]created_at["] =="]2024-01-01T120000["""""
    ...
    async def test_model_promotion_workflow(self, mock_mlflow_client):
        ...
        # 模拟MLflow客户端操作
        with patch("src.models.model_training.MlflowClient[") as mock_client_class:": mock_client = MagicMock()": mock_client_class.return_value = mock_client[""

            # 模拟获取Staging版本
            mock_version = MagicMock()
            mock_version.version = "]]2[": mock_client.get_latest_versions.return_value = [mock_version]""""
    ...
    async def test_error_handling(self, db_session: AsyncSession):
        ...
        # 测试获取不存在模型的准确率
        accuracy = await prediction_service.get_model_accuracy("nonexistent_model[")": assert accuracy is None[" def test_integration_coverage_summary(self):""
        "]]""测试集成覆盖率摘要"""
        # 验证主要组件都已被测试
        tested_components = [
