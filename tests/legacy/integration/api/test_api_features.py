...
def mock_team():
    ...
    def setup_database_mocks(self):
        """自动设置数据库模拟"""
        with patch("src.database.connection.get_async_session[") as mock_get_session:": mock_session = AsyncMock()": mock_get_session.return_value = mock_session[": yield mock_session"
    ...
    async def test_get_match_features_success(self, client, mock_match):
        ...
        with patch("src.api.features.feature_store[") as mock_feature_store:""""
            ...
            # 🔧 FIXED: 正确配置数据库会话Mock
            with patch("]src.api.features.get_async_session[") as mock_get_session:": mock_session = AsyncMock()"""
                ...
                data = response.json()
                assert data["]success["] is True[" assert "]]data[" in data[""""
                assert "]]match_info[" in data["]data["]": assert "]features[" in data["]data["]""""

                # 验证比赛信息
                match_info = data["]data["]["]match_info["]": assert match_info["]match_id["] ==1[" assert match_info["]]home_team_id["] ==1[" assert match_info["]]away_team_id["] ==2[""""

                # 验证特征数据
                features = data["]]data["]["]features["]": assert "]team_features[" in features[""""
    ...
    async def test_get_match_features_not_found(self, client):
        ...
        with patch("src.api.features.select["):""""
            ...
            with patch("]src.api.features.get_async_session[") as mock_get_session:": mock_session = AsyncMock()": mock_session.execute = AsyncMock(return_value=mock_result)": mock_get_session.return_value.__aenter__.return_value = mock_session"

                response = client.get("]_api/v1/features/999[")": assert response.status_code ==404[" data = response.json()""
    ...
    async def test_get_team_features_success(self, client, mock_team):
        ...
        with patch("src.api.features.select["), patch(""""
            ...
            with patch("]src.api.features.get_async_session[") as mock_get_session:": mock_session = AsyncMock()": mock_session.execute = AsyncMock(return_value=mock_result)": mock_get_session.return_value.__aenter__.return_value = mock_session"
                ...
                response = client.get("]_api/v1/features/teams/1[")": assert response.status_code ==200[" data = response.json()""
            assert data["]]success["] is True[""""

            # 验证球队信息
            team_info = data["]]data["]["]team_info["]": assert team_info["]team_id["] ==1[" assert team_info["]]team_name["] =="]测试球队["""""

            # 验证特征数据
            features = data["]data["]["]features["]": assert features["]recent_5_wins["] ==3[" assert features["]]recent_5_goals_for["] ==8[""""
    ...
    async def test_get_team_features_with_raw_data(self, client, mock_team):
        ...
        with patch("src.api.features.select["), patch(""""
            ...
            with patch("]src.api.features.get_async_session[") as mock_get_session:": mock_session = AsyncMock()": mock_session.execute = AsyncMock(return_value=mock_result)": mock_get_session.return_value.__aenter__.return_value = mock_session"
                ...
                response = client.get("]_api/v1/features/teams/1?include_raw=true[")": assert response.status_code ==200[" data = response.json()""
                assert data["]]success["] is True[" assert "]]raw_features[" in data["]data["]""""
    ...
    async def test_calculate_match_features_success(self, client, mock_match):
        ...
        with patch("src.api.features.select["), patch(""""
            ...
            with patch("]src.api.features.get_async_session[") as mock_get_session:": mock_session = AsyncMock()": mock_session.execute = AsyncMock(return_value=mock_result)": mock_get_session.return_value.__aenter__.return_value = mock_session"
                ...
                response = client.post("]_api/v1/features/calculate/1[")": assert response.status_code ==200[" data = response.json()""
                assert data["]]success["] is True[" result_data = data["]]data["]": assert result_data["]match_id["] ==1[" assert result_data["]]match_features_stored["] is True[" assert result_data["]]home_team_features_stored["] is True[" assert result_data["]]away_team_features_stored["] is True[" assert "]]calculation_time[" in result_data[""""
    ...
    async def test_calculate_team_features_success(self, client, mock_team):
        ...
        with patch("src.api.features.select["), patch(""""
            ...
            with patch("]src.api.features.get_async_session[") as mock_get_session:": mock_session = AsyncMock()": mock_session.execute = AsyncMock(return_value=mock_result)": mock_get_session.return_value.__aenter__.return_value = mock_session"
                ...
                response = client.post("]_api/v1/features/calculate/teams/1[")": assert response.status_code ==200[" data = response.json()""
                assert data["]]success["] is True[" result_data = data["]]data["]": assert result_data["]team_id["] ==1[" assert result_data["]]team_name["] =="]测试球队[" assert result_data["]features_stored["] is True[" assert "]]calculation_date[" in result_data[""""
    ...
    async def test_batch_calculate_features_success(self, client):
        ...
        with patch("src.api.features.feature_store[") as mock_feature_store:""""
            ...
            response = client.post(
                "]_api/v1/features/batch/calculate[",": params={"""
            ...
            assert response.status_code ==200
            data = response.json()
            assert data["]success["] is True[" result_data = data["]]data["]": assert "]date_range[" in result_data[""""
            assert result_data["]]date_range["]["]start_date["] == "]2025-09-10T000000[" assert result_data["]date_range["]["]end_date["] == "]2025-09-17T000000[" assert result_data["]statistics["] ==mock_stats[" assert "]]completion_time[" in result_data[""""
    ...
    async def test_get_historical_features_success(self, client, mock_match):
        ...
        with patch("src.api.features.select["), patch(""""
            ...
            with patch("]src.api.features.get_async_session[") as mock_get_session:": mock_session = AsyncMock()": mock_session.execute = AsyncMock(return_value=mock_result)": mock_get_session.return_value.__aenter__.return_value = mock_session"
                ...
                response = client.get(
                    "]_api/v1/features/historical/1[",": params = {"]feature_refs[": ["]team_recent_performancerecent_5_wins["]},""""
                )

                assert response.status_code ==200
                data = response.json()
                assert data["]success["] is True[" result_data = data["]]data["]": assert result_data["]match_id["] ==1[" assert result_data["]]feature_count["] ==5  # DataFrame列数[" assert result_data["]]record_count["] ==2  # DataFrame行数[" assert len(result_data["]]features["]) ==2  # 转换为records的长度[""""
    ...
    async def test_api_error_handling(self, client):
        ...
        # 测试数据库连接错误
        with patch(
            "src.api.features.get_async_session[",": side_effect=Exception("]数据库连接错误["),""""
        ):
            response = client.get("]_api/v1/features/1[")": assert response.status_code ==500["""

        # 测试特征存储错误
        with patch("]]src.api.features.select["), patch(""""
            "]src.api.features.feature_store["""""
        ) as mock_feature_store = mock_result AsyncMock()
            ...
            with patch("]src.api.features.get_async_session[") as mock_get_session:": mock_session = AsyncMock()": mock_session.execute = AsyncMock(return_value=mock_result)": mock_get_session.return_value.__aenter__.return_value = mock_session"
                ...
                response = client.get("]/api/v1/features/1[")": assert response.status_code ==500[" def test_api_route_registration(self, client):""
        "]]""测试API路由注册"""
        # 测试特征相关路由是否正确注册
        response = client.get("_api/v1/features/1[")""""
        ...
        response = client.get("]/api/v1/features/teams/1[")": assert response.status_code != 404[" response = client.post("]]/api/v1/features/calculate/1[")": assert response.status_code != 404[" response = client.post(""
