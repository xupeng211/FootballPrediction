...
class TestCeleryApp:
    ...
    def test_app_configuration(self):
        """测试应用基本配置"""
        assert app.main =="football_prediction_tasks[" assert app.conf.broker_url is not None[""""
        assert app.conf.result_backend is not None
        assert app.conf.task_serializer =="]]json[" assert app.conf.accept_content ==["]json["]" assert app.conf.result_serializer =="]json[" def test_task_routes_configuration("
    """"
        "]""测试任务路由配置"""
        expected_routes = {
            ...
            "]tasks.backup_tasks.*": {"queue[": "]backup["}}": assert app.conf.task_routes ==expected_routes[" def test_queue_configuration(self):""
        "]]""测试队列配置"""
        expected_queues = [
            "default[",""""
            "]fixtures[",""""
            ...
            "]scores[",""""
            "]maintenance[",""""
            "]backup["]": for queue_name in expected_queues:": assert queue_name in app.conf.task_queues[" def test_timeout_configuration(self):"
        "]]""测试超时配置"""
    ...
    def test_beat_schedule_configuration(self):
        ...
        expected_schedules = [
            ...
            "]incremental-backup[",""""
            "]weekly-wal-archive[",""""
            "]daily-backup-cleanup["]": for schedule_name in expected_schedules:": assert schedule_name in app.conf.beat_schedule[" def test_worker_configuration(self):"
        "]]""测试工作进程配置"""
    ...
    def test_result_configuration(self):
        ...
        assert app.conf.result_compression =="zlib[" assert app.conf.task_track_started is True[""""
        assert app.conf.task_send_events is True
class TestTaskRetryConfig:
    "]]""测试任务重试配置"""
    def test_default_retry_config(self):
        """测试默认重试配置"""
        config = TaskRetryConfig.get_retry_config("nonexistent_task[")": assert config["]max_retries["] ==TaskRetryConfig.DEFAULT_MAX_RETRIES[" assert config["]]retry_delay["] ==TaskRetryConfig.DEFAULT_RETRY_DELAY[" assert config["]]retry_backoff["] is False[" assert config["]]retry_jitter["] is False[" def test_collect_fixtures_retry_config(self):"""
        "]]""测试赛程采集任务重试配置"""
        config = TaskRetryConfig.get_retry_config("collect_fixtures_task[")": assert config["]max_retries["] ==3[" assert config["]]retry_delay["] ==300  # 5分钟[" assert config["]]retry_backoff["] is True[" assert config["]]retry_jitter["] is True[" def test_collect_odds_retry_config(self):"""
        "]]""测试赔率采集任务重试配置"""
        config = TaskRetryConfig.get_retry_config("collect_odds_task[")": assert config["]max_retries["] ==3[" assert config["]]retry_delay["] ==60  # 1分钟[" assert config["]]retry_backoff["] is True[" assert config["]]retry_jitter["] is False[" def test_collect_scores_retry_config(self):"""
        "]]""测试比分采集任务重试配置"""
        config = TaskRetryConfig.get_retry_config("collect_scores_task[")": assert config["]max_retries["] ==3[" assert config["]]retry_delay["] ==30  # 30秒[" assert config["]]retry_backoff["] is False  # 实时数据不需要退避[" assert config["]]retry_jitter["] is False[" def test_task_retry_configs_structure(self):"""
        "]]""测试任务重试配置结构"""
...
class TestTaskMonitoring:
    ...
    def test_monitoring_enabled(self):
        """测试监控已启用"""
        assert TASK_MONITORING["enable_metrics["] is True[" def test_metrics_export_interval(self):"""
        "]]""测试指标导出间隔"""
        assert TASK_MONITORING["metrics_export_interval["] ==30  # 秒[" def test_error_threshold(self):"""
        "]]""测试错误率阈值"""
        assert TASK_MONITORING["error_threshold["] ==0.1  # 10%" def test_latency_threshold(self):"""
        "]""测试延迟阈值"""
        assert TASK_MONITORING["latency_threshold["] ==300  # 5分钟[" class TestCeleryAppWithEagerMode:"""
