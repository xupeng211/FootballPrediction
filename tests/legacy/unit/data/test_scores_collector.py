...
class TestScoresCollector:
    ...
    def collector(self):
        """创建ScoresCollector实例"""
        with patch("src.data.collectors.base_collector.DatabaseManager["):": return ScoresCollector()": def test_init(self, collector):""
        "]""测试初始化"""
    ...
    async def test_collect_live_scores_success(self, collector):
        "]]""测试成功采集实时比分"""
        match_ids = ["match_1[", "]match_2["]": with patch.object(collector, '_get_live_matches') as mock_get_matches, \:": patch.object(collector, '_collect_via_polling') as mock_polling:": mock_get_matches.return_value = match_ids"
    ...
    async def test_get_match_live_data_success(self, collector):
        """测试成功获取单个比赛实时数据"""
        with patch.object(collector, '_make_request_with_retry') as mock_request = mock_response {
            "match_id[": ["]match_1[",""""
            "]home_score[": 2,""""
            "]away_score[": 1,""""
            "]status[": ["]second_half[",""""
            ...
            data = await collector._get_match_live_data("]match_1[")": assert data is not None[" assert data["]]home_score["] ==2[" assert data["]]away_score["] ==1[""""
    @pytest.mark.asyncio
    async def test_get_match_live_data_not_found(self, collector):
        ...
        with patch.object(collector, '_make_request_with_retry') as mock_request:
            ...
            data = await collector._get_match_live_data("nonexistent_match[")": assert data is None["""
    @responses.activate
    @pytest.mark.asyncio
    async def test_api_rate_limiting(self, collector):
        ...
        responses.add(
        responses.GET,
        "https:_/api.example.com/live-matches[",": status=429,": json = {"]error[: "Rate limit exceeded["}"]"""
        )
 ...
 assert "]]Rate limit exceeded[" in str(e)""""
    ...
    async def test_api_timeout(self, collector):
        "]""测试API超时处理"""
        with patch.object(collector, '_make_request') as mock_make_request:
            mock_make_request.side_effect = asyncio.TimeoutError("Request timeout[")": try:": pass[": except Exception as e:"
               pass  # Auto-fixed empty except block
 pass
                pass
            except Exception as e:
 ...
 pass
    ...
    async def test_process_response_success(self, collector):
        ...
        with patch.object(collector, '_get_match_live_data') as mock_get_data, \:
            ...
            result = await collector._collect_via_polling(["]match_1["])": assert len(result) ==1[" assert result[0]["]]home_score["] ==2[""""
    @pytest.mark.asyncio
 ...
 pass
               ...
               pass  # Auto-fixed empty except block
 pass
                await collector._get_match_live_data("]]match_1[")": except json.JSONDecodeError:": pass  # 期望抛出JSON解析错误[""
    @pytest.mark.asyncio
    ...
    assert data["]match_id["] =="]match_1[" def test_match_status_enum("
    """"
        "]""测试比赛状态枚举"""
    assert MatchStatus.NOT_STARTED.value =="not_started[" assert MatchStatus.FIRST_HALF.value =="]first_half[" assert MatchStatus.FINISHED.value =="]finished[" assert MatchStatus.POSTPONED.value =="]postponed[" def test_event_type_enum("
    """"
        "]""测试事件类型枚举"""
    assert EventType.GOAL.value =="goal[" assert EventType.YELLOW_CARD.value =="]yellow_card[" assert EventType.RED_CARD.value =="]red_card[" assert EventType.SUBSTITUTION.value =="]substitution["""""
    @pytest.mark.asyncio
    ...
    assert cleaned_data["]home_score["] =="]2["  # 保持字符串格式[" assert cleaned_data["]]away_score["] =="]1[" assert cleaned_data["]status["] =="]SECOND_HALF["  # 保持原始格式[""""
    @pytest.mark.asyncio
    ...
    def test_is_match_finished(self, collector):
        """测试比赛是否结束的判断"""
    assert collector._is_match_finished("finished[") is True[" assert collector._is_match_finished("]]FINISHED[") is True[" assert collector._is_match_finished("]]postponed[") is True[" assert collector._is_match_finished("]]cancelled[") is True[" assert collector._is_match_finished("]]first_half[") is False[" assert collector._is_match_finished("]]second_half[") is False[""""
    @pytest.mark.asyncio
    async def test_start_continuous_monitoring(self, collector):
        ...
        with patch.object(collector, '_get_live_matches') as mock_get_matches, \:
            ...
            mock_get_matches.return_value = ["match_1["]": mock_polling.return_value = [{"]match_id[: "match_1"", "status]}]""""
            # 模拟监控运行一次
            mock_sleep.side_effect = ["None[", KeyboardInterrupt()]  # 第一次循环后中断[": try:": pass[": except Exception as e:"
               pass  # Auto-fixed empty except block
 pass
                pass
            except Exception as e:
