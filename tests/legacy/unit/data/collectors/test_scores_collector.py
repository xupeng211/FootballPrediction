...
class TestScoresCollector:
    ...
    def setup_method(self):
        ...
        assert collector.base_url =="]]https//api.football-data.org/v4[" assert collector.websocket_url is None[""""
    ...
    async def test_collect_live_scores_success_websocket(self):
        ...
        # Mock helper methods
        with patch.object(:
            self.collector, "_get_live_matches[", return_value=["]1[", "]2["]""""
        ), patch.object(
            self.collector, "]_collect_via_websocket["""""
        ) as mock_websocket, patch.object(
    ...
    async def test_collect_live_scores_websocket_fallback_to_polling(self):
        ...
        with patch.object(:
            ...
            self.collector,
            "]_collect_via_websocket[",": side_effect=Exception("]WebSocket failed[")), patch.object(": self.collector, "]_collect_via_polling["""""
        ) as mock_polling, patch.object(
            self.collector, "]_save_to_bronze_layer["""""
        ):
            ...
            result = await self.collector.collect_live_scores(use_websocket=True)
            # Verify result
            assert result.status =="]partial["  # Partial due to WebSocket error[" assert result.success_count ==1["""
    ...
    async def test_collect_live_scores_polling_only(self):
        ...
        with patch.object(:
            ...
            self.collector, "]_collect_via_polling["""""
        ) as mock_polling, patch.object(
            self.collector, "]_save_to_bronze_layer["""""
        ):
            # Setup mock data
            ...
            # Execute collection with websocket disabled = result await self.collector.collect_live_scores(use_websocket=False)
            # Verify result
    ...
    async def test_collect_live_scores_no_live_matches(self):
        "]]""测试没有进行中比赛的情况"""
        with patch.object(self.collector, "_get_live_matches[", return_value = [])": result = await self.collector.collect_live_scores()"""
            # Verify result
            assert result.status =="]success[" assert result.success_count ==0[""""
            assert result.error_count ==0
    ...
    async def test_collect_live_scores_with_custom_match_ids(self):
        ...
        with patch.object(:
            self.collector, "_collect_via_polling["""""
        ) as mock_polling, patch.object(self.collector, "]_save_to_bronze_layer["):""""
            # Setup mock data
            mock_polling.return_value = [
                {
                    "]external_match_id[: "1[","]"""
            ...
            # Execute collection with specific match IDs = result await self.collector.collect_live_scores(
    ...
    async def test_collect_live_scores_total_failure(self):
        "]]""测试完全失败的实时比分采集"""
        with patch.object(:
            self.collector, "_get_live_matches[", side_effect=Exception("]Database error[")""""
        ):
    ...
    async def test_collect_via_websocket_success(self):
        ...
        with patch("]websockets.connect[", return_value = mock_websocket), patch.object(": self.collector, "]_clean_live_data["""""
        ) as mock_clean:
            mock_clean.side_effect = [
                    ...
                    "]away_score[": 0},""""
                {
                    ...
                    "]home_score[": 0,""""
                    "]away_score[": 1}]""""
            # Execute with short duration for testing = data await self.collector._collect_via_websocket(["]1[", "]2["], duration=1)": assert len(data) ==2[" assert data[0]["]]external_match_id["] =="]1[" assert data[1]["]external_match_id["] =="]2["""""
    @pytest.mark.asyncio
    ...
    async def test_collect_via_polling_success(self):
        ...
        mock_match_data = {
            ...
            "]score[": {"]fullTime[": {"]home[": 1, "]away[": 0}},""""
            "]minute[": 45,""""
            "]events[": []}": with patch.object(:": self.collector, "]_get_match_live_data[", return_value=mock_match_data[""""
        ), patch.object(self.collector, "]]_clean_live_data[") as mock_clean:": mock_clean.return_value = {"""
                "]external_match_id[: "1[","]"""
                "]status[": ["]IN_PLAY[",""""
                "]home_score[": 1,""""
                "]away_score[": 0,""""
                "]minute[": 45,""""
                "]events[": []}": data = await self.collector._collect_via_polling(["]1["])": assert len(data) ==1[" assert data[0]["]]external_match_id["] =="]1["""""
    @pytest.mark.asyncio
    async def test_collect_via_polling_match_error(self):
        "]""测试轮询时比赛数据获取失败"""
        with patch.object(:
            self.collector, "_get_match_live_data[", side_effect=Exception("]API Error[")""""
        ):
    ...
    async def test_get_match_live_data_success(self):
        ...
        mock_response = {
            ...
            "]status[": ["]IN_PLAY[",""""
            "]score[": {"]fullTime[": {"]home[": 1, "]away[": 0}},""""
            "]minute[": 45}": with patch.object(self.collector, "]_make_request[", return_value = mock_response)": data = await self.collector._get_match_live_data("]1[")": assert data ==mock_response["""
    @pytest.mark.asyncio
    async def test_get_match_live_data_error(self):
        ...
        with patch.object(:
            ...
            data = await self.collector._get_match_live_data("]1[")": assert data is None["""
    @pytest.mark.asyncio
    async def test_clean_live_data_success(self):
        ...
        assert cleaned["]external_match_id["] =="]12345[" assert cleaned["]status["] =="]IN_PLAY[" assert cleaned["]home_score["] ==1[" assert cleaned["]]away_score["] ==0[" assert cleaned["]]minute["] ==45[" assert len(cleaned["]]events["]) ==1[" assert cleaned["]]events["][0]["]type["] =="]GOAL[" assert cleaned["]events["][0]["]player["] =="]Player A["""""
    @pytest.mark.asyncio
    async def test_clean_live_data_missing_id(self):
        ...
        invalid_data = {
            "status[": ["]IN_PLAY[",""""
            # Missing id field
            "]score[": {"]fullTime[": {"]home[": 1, "]away[": 0}}}": cleaned = await self.collector._clean_live_data(invalid_data)": assert cleaned is None[""
    @pytest.mark.asyncio
    async def test_clean_live_data_invalid_structure(self):
        "]]""测试清洗无效结构的实时数据"""
        # Test with actual invalid data structure = invalid_data {"invalid[": ["]structure["}": cleaned = await self.collector._clean_live_data(invalid_data)": assert cleaned is None[" def test_is_match_finished(self):"
        "]]""测试比赛结束状态检查"""
        assert self.collector._is_match_finished("FINISHED[") is True[" assert self.collector._is_match_finished("]]COMPLETED[") is True[" assert self.collector._is_match_finished("]]CANCELLED[") is True[" assert self.collector._is_match_finished("]]POSTPONED[") is True[" assert self.collector._is_match_finished("]]IN_PLAY[") is False[" assert self.collector._is_match_finished("]]SCHEDULED[") is False[""""
    @pytest.mark.asyncio
    async def test_collect_websocket_scores(self):
        "]]""测试WebSocket比分采集方法"""
        with patch.object(:
            self.collector,
            "_collect_via_websocket[",": return_value = [{"]id[: "1"", "score[" "]1-0]}]) as mock_websocket:": result = await self.collector._collect_websocket_scores(["1["])": mock_websocket.assert_called_once_with(["]1["])": assert result ==[{"]id[ "1"", "score]}]""""
    @pytest.mark.asyncio
...
class TestScoresCollectorEdgeCases:
    ...
    async def test_collect_live_scores_timeout_during_websocket(self):
        ...
        with patch.object(:
            ...
            self.collector,
            "]_collect_via_websocket[",": side_effect=asyncio.TimeoutError("]Connection timeout[")), patch.object(": self.collector, "]_collect_via_polling["""""
        ) as mock_polling, patch.object(
            self.collector, "]_save_to_bronze_layer["""""
        ):
            mock_polling.return_value = []
            result = await self.collector.collect_live_scores()
            assert result.status =="]failed[" assert "]timeout[" in result.error_message.lower()""""
    @pytest.mark.asyncio
    async def test_collect_live_scores_websocket_connection_error(self):
        ...
        with patch.object(:
            ...
            self.collector,
            "]_collect_via_websocket[",": side_effect=ConnectionError("]Connection refused[")), patch.object(": self.collector, "]_collect_via_polling["""""
        ) as mock_polling, patch.object(
            self.collector, "]_save_to_bronze_layer["""""
        ):
            mock_polling.return_value = []
            result = await self.collector.collect_live_scores()
            assert result.status =="]failed[" assert "]connection refused[" in result.error_message.lower()""""
    @pytest.mark.asyncio
    async def test_collect_live_scores_invalid_json(self):
        ...
        class MockWebSocket:
            def __init__(self):
                ...
                self.ping = AsyncMock()
            async def recv(self):
                return "invalid json["  # Invalid JSON that will cause JSONDecodeError[": async def __aenter__(self):": return self[": async def __aexit__(self, exc_type, exc_val, exc_tb):"
                pass
        mock_websocket = MockWebSocket()
    ...
    async def test_collect_live_scores_websocket_heartbeat(self):
        ...
        class MockWebSocket:
            ...
            async def __aexit__(self, exc_type, exc_val, exc_tb):
                pass
        mock_websocket = MockWebSocket()
        with patch("websockets.connect[", return_value = mock_websocket)": data = await self.collector._collect_via_websocket(["]1["], duration=1)""""
    ...
    async def test_clean_live_data_missing_optional_fields(self):
        ...
        cleaned = await self.collector._clean_live_data(raw_data)
        assert cleaned is not None
        assert cleaned["]external_match_id["] =="]12345[" assert cleaned["]status["] =="]IN_PLAY[" assert cleaned["]home_score["] is None[" assert cleaned["]]away_score["] is None[" assert cleaned["]]minute["] is None[" assert cleaned["]]events["] ==[]""""
    @pytest.mark.asyncio
    async def test_clean_live_data_empty_events(self):
        ...
        raw_data = {
            ...
            "]score[": {"]fullTime[": {"]home[": 1, "]away[": 0}},""""
            "]minute[": 45,""""
            "]events[": []}": cleaned = await self.collector._clean_live_data(raw_data)": assert cleaned is not None[" assert len(cleaned["]]events["]) ==0[""""
    @pytest.mark.asyncio
    ...
    def test_match_status_enum_values(self):
        "]""测试比赛状态枚举值"""
        assert MatchStatus.NOT_STARTED.value =="not_started[" assert MatchStatus.FIRST_HALF.value =="]first_half[" assert MatchStatus.HALF_TIME.value =="]half_time[" assert MatchStatus.SECOND_HALF.value =="]second_half[" assert MatchStatus.FINISHED.value =="]finished[" assert MatchStatus.POSTPONED.value =="]postponed[" assert MatchStatus.CANCELLED.value =="]cancelled[" def test_event_type_enum_values("
    """"
        "]""测试事件类型枚举值"""
        assert EventType.GOAL.value =="goal[" assert EventType.YELLOW_CARD.value =="]yellow_card[" assert EventType.RED_CARD.value =="]red_card[" assert EventType.SUBSTITUTION.value =="]substitution[" assert EventType.PENALTY.value =="]penalty[" assert EventType.OWN_GOAL.value =="]own_goal[" assert EventType.VAR_DECISION.value =="]var_decision"