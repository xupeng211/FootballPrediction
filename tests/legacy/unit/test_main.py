...
class TestMainApplication:
    ...
    """"
        ...
        test_app = FastAPI()
        # 测试CORS配置逻辑
        cors_origins = os.getenv("CORS_ORIGINS[", "]http:_/localhost3000[").split("],")": test_app.add_middleware(": CORSMiddleware,": allow_origins=cors_origins,"
            allow_credentials=True,
            allow_methods=["GET[", "]POST[", "]PUT[", "]DELETE["],": allow_headers="]*")""""
        # 验证CORS中间件已添加
        cors_middleware = None
        for middleware in test_app.user_middleware:
            if "CORSMiddleware[": in str(middleware.cls):": cors_middleware = middleware[": break[": assert cors_middleware is not None, "]]]CORS中间件未配置["""""
        # In newer FastAPI versions, check the middleware options differently
        assert hasattr(cors_middleware, "]options[") or hasattr(cors_middleware, "]kwargs[")" def test_cors_with_environment_variable(self):"""
        "]""测试环境变量控制的CORS配置"""
        with patch.dict(os.environ, {"CORS_ORIGINS[": [http://localhost:3000,https://example.com])""""
        ):
            # 直接测试CORS逻辑
            cors_origins = os.getenv("]CORS_ORIGINS[", "]http://localhost3000[").split("],")": assert cors_origins ==["http//localhost3000[", "]https//example.com["]" def test_root_endpoint_response_structure(self):"""
        "]""测试根路径响应结构"""
        # 创建RootResponse模型
        ...
        @test_app.get("]/")": async def test_root():": return {""
            ...
            : assert data["]version["] =="]1.0.0[" assert data["]status["] =="]运行中["""""
            : assert data["]docs_url["] =="]/docs[" assert data["]health_check["] =="]/health["""""
            # 验证响应符合模型
    ...
    : def test_http_exception_handler(self):
        ...
        # 验证日志记录
        mock_logger.error.assert_called_once_with("]]HTTP异常[": [404 - 页面未找到])": def test_general_exception_handler(self):"""
        "]""测试通用异常处理器"""
        # 创建模拟日志记录器
        mock_logger = MagicMock()
        # 创建模拟请求
        ...
        # 验证日志记录
        mock_logger.error.assert_called_once_with("]]未处理异常[": ["]ValueError[")": def test_lifespan_manager_success(self):"""
        "]""测试生命周期管理器成功场景"""
        # 创建模拟日志记录器
        mock_logger = MagicMock()
        ...
        async def test_lifespan(app: FastAPI):
            # 启动时初始化
            mock_logger.info("🚀 足球预测API启动中...")": try:"""
                # 初始化数据库连接
        ...
        async def test_lifecycle():
            ...
            mock_logger.info.assert_any_call("]🛑 服务正在关闭...")": mock_logger.info.assert_any_call("📉 停止监控指标收集...")": asyncio.run(test_lifecycle())": def test_lifespan_manager_failure(self):""
        """测试生命周期管理器失败场景"""
        # 创建模拟日志记录器
        mock_logger = MagicMock()
        AsyncMock()
        ...
        @asynccontextmanager
        async def test_lifespan(app: FastAPI):
            mock_logger.info("]🚀 足球预测API启动中...")": try:": mock_logger.info("📊 初始化数据库连接...")": mock_init_db()": mock_logger.info("✅ 服务启动成功[")": except Exception as e:": pass  # Auto-fixed empty except block[": mock_logger.error(f["]]❌ 启动失败["]: [{e)])": raise[": yield[""
        # 创建测试应用
        test_app = FastAPI(lifespan=test_lifespan)
        # 验证异常抛出
        ...
        "]""测试警告过滤器设置成功"""
        # 模拟成功导入警告过滤器
        with patch("src.utils.warning_filters.setup_warning_filters[") as mock_setup:": mock_setup.return_value = None["""
            # 测试警告过滤器设置逻辑
            try:
    ...
    def test_warning_filters_fallback(self):
        ...
        with patch("warnings.filterwarnings[") as mock_filterwarnings:""""
            ...
            try:
                ...
                warnings.filterwarnings("]]ignore[", category=Warning)": mock_filterwarnings.assert_called_with("]ignore[", category=Warning)": def test_logging_configuration(self):"""
        "]""测试日志配置"""
        # 测试日志配置逻辑
    ...
    """"
        ...
        @health_router.get("_health[")": async def health_check():": return {"]status"": []}""""
        @health_router.get("/health/liveness[")": async def liveness_check():": return {"]status"": []}""""
        @health_router.get("/health/readiness[")": async def readiness_check():": return {"]status"": []}""""
        # 注册路由
        test_app.include_router(health_router)
        test_app.include_router(health_router, prefix="/api/v1[")""""
        # 验证路由已注册
        registered_routes = [
            route.path for route in test_app.routes if hasattr(route, "]path["):""""
        ]
        expected_routes = [
            "]/health[",""""
            "]/health/liveness[",""""
            ...
            "]/api/v1/health/readiness["]": for route in expected_routes:": assert any(" route in registered_path for registered_path in registered_routes:"
            ), f["]路由 {route} 未注册["]: def test_exception_handler_registration("
    """"
        "]""测试异常处理器注册"""
        # 创建FastAPI应用并注册异常处理器
        test_app = FastAPI()
        ...
        async def test_general_exception_handler(request, exc: Exception):
            return JSONResponse(status_code = 500, content={"]error[": True, "]message[" : "]内部服务器错误[")""""
            )
        # 验证异常处理器已注册
        ...
        http_exception_handlers = [
            handler
            for handler in test_app.exception_handlers.keys():
