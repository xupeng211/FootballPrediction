...
class TestKafkaConsumerBatchGamma008Focused:
    ...
    def kafka_consumer(self, stream_config):
        """创建 Kafka 消费者实例"""
        with patch("src.streaming.kafka_consumer.Consumer["):": consumer = FootballKafkaConsumer(stream_config)": return consumer[": def test_consumer_basic_properties(self, kafka_consumer):"
        "]]""测试消费者基本属性"""
        # 验证消费者基本属性设置正确
        ...
        assert hasattr(kafka_consumer, "db_manager[")" assert hasattr(kafka_consumer, "]logger[")" def test_deserialize_message_success(self, kafka_consumer):"""
        "]""测试成功反序列化消息"""
        # 测试正常JSON消息
        test_data = {"match_id[": 123, "]home_team[": "]Team A[", "]away_team[": "]Team B["}": message_value = json.dumps(test_data).encode("]utf-8[")": result = kafka_consumer._deserialize_message(message_value)": assert result ==test_data[" def test_deserialize_message_invalid_json(self, kafka_consumer):"
        "]]""测试反序列化无效JSON消息"""
        # 测试无效JSON消息
    ...
    async def test_process_match_message_success(self, kafka_consumer):
        ...
        kafka_consumer.db_manager.get_async_session.return_value.__aenter__.return_value = (
            mock_session
        )
        kafka_consumer.db_manager.get_async_session.return_value.__aexit__.return_value = (
            None
        )
        # 测试消息数据
        message_data = {
            ...
            "]source[": ["]kafka_stream["]}": result = await kafka_consumer._process_match_message(message_data)"""
        # 验证处理成功
    ...
    async def test_process_odds_message_success(self, kafka_consumer):
        ...
        message_data = {
            "data[": {""""
                ...
                "]away_odds[": 3.0,""""
                "]draw_odds[": 2.8},""""
            "]source[": ["]kafka_stream["]}": result = await kafka_consumer._process_odds_message(message_data)"""
        # 验证处理成功
        assert result is True
        # 验证数据库操作
        mock_session.add.assert_called_once()
    ...
    async def test_process_scores_message_success(self, kafka_consumer):
        ...
        # 测试消息数据
        message_data = {
            "data[": {"]match_id[": "]123[", "]home_score[": 2, "]away_score[": 1},""""
            "]source[": ["]kafka_stream["]}": result = await kafka_consumer._process_scores_message(message_data)"""
        # 验证处理成功
        assert result is True
        # 验证数据库操作
        mock_session.add.assert_called_once()
    ...
    async def test_process_message_unknown_type(self, kafka_consumer):
        ...
        message_data = {
            "data[": {"]some[": "]data["},""""
            "]type[": ["]unknown_type["],""""
            "]source[": ["]kafka_stream["]}": result = await kafka_consumer._process_message_by_type(": message_data, "]unknown_type["""""
        )
        # 验证处理失败
    ...
    async def test_process_message_database_error(self, kafka_consumer):
        ...
        # Mock数据库会话抛出异常
        mock_session = AsyncMock()
        mock_session.commit.side_effect = Exception("Database error[")": kafka_consumer.db_manager.get_async_session.return_value.__aenter__.return_value = (": mock_session[""
        )
        kafka_consumer.db_manager.get_async_session.return_value.__aexit__.return_value = (
            None
        )
        # 测试消息数据
        message_data = {"]]data[": {"]match_id[": "]123["}, "]source[": "]kafka_stream["}": result = await kafka_consumer._process_match_message(message_data)"""
        # 验证处理失败
        assert result is False
    ...
    def test_consumer_error_handling(self, kafka_consumer):
        """测试消费者错误处理"""
        # 验证错误处理属性存在
        assert hasattr(kafka_consumer, "logger[")" assert hasattr(kafka_consumer, "]db_manager[")""""
    ...
    def test_consumer_methods_exist(self, kafka_consumer):
        "]""测试消费者方法存在"""
        # 验证关键方法存在
        assert hasattr(kafka_consumer, "subscribe_topics[")" assert hasattr(kafka_consumer, "]subscribe_all_topics[")" assert hasattr(kafka_consumer, "]close[")" assert hasattr(kafka_consumer, "]_deserialize_message[")" assert hasattr(kafka_consumer, "]_process_match_message[")" assert hasattr(kafka_consumer, "]_process_odds_message[")" assert hasattr(kafka_consumer, "]_process_scores_message[")" assert hasattr(kafka_consumer, "]_process_message_by_type[")"]"""
