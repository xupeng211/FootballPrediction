...
class TestDataLakeStorageBatchGamma005:
    ...
    def test_data_lake_storage_initialization_default(self, temp_dir):
        ...
        assert storage.compression =="snappy[" assert storage.partition_cols ==["]year[", "]month["]" assert storage.max_file_size_mb ==100["""
        # 验证目录结构创建
        bronze_dir = Path(temp_dir) / "]]bronze[": silver_dir = Path(temp_dir) / "]silver[": gold_dir = Path(temp_dir) / "]gold[": assert bronze_dir.exists()" assert silver_dir.exists()"""
    ...
    def test_data_lake_storage_initialization_custom(self, temp_dir):
        ...
        custom_config = {
            ...
            "]max_file_size_mb[": 200}": storage = DataLakeStorage(**custom_config)"""
        # 验证自定义配置
        assert storage.base_path ==Path(temp_dir)
        assert storage.compression =="]gzip[" assert storage.partition_cols ==["]year[", "]month["]" assert storage.max_file_size_mb ==200["""
    ...
    @pytest.mark.asyncio
    async def test_save_historical_data_with_dataframe_success(
        self, storage, sample_dataframe
    ):
        ...
        with patch("pathlib.Path.mkdir["), patch(:""""
            ...
            assert result.endswith("].parquet[")" assert "]raw_matches[" in result[""""
    @pytest.mark.asyncio
    async def test_save_historical_data_with_dict_list(self, storage, sample_data):
        ...
        with patch("pathlib.Path.mkdir["), patch(:""""
            ...
            # 验证结果
            assert result["]success["] is True[" assert result["]]records_count["] ==2[""""
    @pytest.mark.asyncio
    async def test_save_historical_data_empty_data(self, storage):
        "]]""测试保存空数据"""
        # 测试空列表
        empty_list = []
        result = await storage.save_historical_data("raw_matches[", empty_list)": assert result["]success["] is False[" assert "]]数据为空[" in result["]error["]""""
        # 测试空DataFrame
        empty_df = pd.DataFrame()
        result = await storage.save_historical_data("]raw_matches[", empty_df)": assert result["]success["] is False[" assert "]]数据为空[" in result["]error["]""""
    @pytest.mark.asyncio
    async def test_save_historical_data_invalid_table_name(
        ...
        "]""测试无效表名处理"""
        # 测试不存在的表名
        result = await storage.save_historical_data("invalid_table[", sample_dataframe)": assert result["]success["] is False[" assert "]]表名无效[" in result["]error["]""""
    @pytest.mark.asyncio
    async def test_save_historical_data_file_creation_error(
        self, storage, sample_dataframe
    ):
        "]""测试文件创建错误处理"""
        # Mock文件创建失败
        with patch("pathlib.Path.mkdir[", side_effect = OSError("]权限不足["))": result = await storage.save_historical_data("]raw_matches[", sample_dataframe)": assert result["]success["] is False[" assert "]]权限不足[" in result["]error["]""""
    @pytest.mark.asyncio
    async def test_save_historical_data_parquet_error(self, storage, sample_dataframe):
        ...
        with patch("pathlib.Path.mkdir["), patch(:""""
            ...
            "]]pandas.DataFrame.to_parquet[", side_effect=Exception("]Parquet写入失败[")""""
        ):
            result = await storage.save_historical_data("]raw_matches[", sample_dataframe)": assert result["]success["] is False[" assert "]]Parquet写入失败[" in result["]error["]""""
    @pytest.mark.asyncio
    async def test_load_historical_data_success(self, storage, sample_dataframe):
        "]""测试加载历史数据成功场景"""
        # Mock数据加载
        with patch("pathlib.Path.exists[", return_value = True), patch(""""
            "]pathlib.Path.rglob[", return_value=[Path("]test.parquet[")]""""
        ), patch("]pandas.read_parquet[", return_value = sample_dataframe)": result = await storage.load_historical_data("]raw_matches[")": assert result["]success["] is True[" assert len(result["]]data["]) ==2[" assert "]]file_count[" in result[""""
    @pytest.mark.asyncio
    async def test_load_historical_data_no_files(self, storage):
        ...
        with patch("pathlib.Path.exists[", return_value = True), patch(""""
            ...
            result = await storage.load_historical_data("]raw_matches[")": assert result["]success["] is False[" assert "]]没有找到数据文件[" in result["]error["]""""
    @pytest.mark.asyncio
    async def test_load_historical_data_invalid_table(self, storage):
        "]""测试无效表名加载"""
        result = await storage.load_historical_data("invalid_table[")": assert result["]success["] is False[" assert "]]表名无效[" in result["]error["]""""
    @pytest.mark.asyncio
    async def test_load_historical_data_read_error(self, storage):
        ...
        with patch("pathlib.Path.exists[", return_value = True), patch(""""
            "]pathlib.Path.rglob[", return_value=[Path("]test.parquet[")]""""
        ), patch("]pandas.read_parquet[", side_effect = Exception("]读取失败["))": result = await storage.load_historical_data("]raw_matches[")": assert result["]success["] is False[" assert "]]读取失败[" in result["]error["]: def test_tables_attribute("
    """"
        "]""测试表属性"""
        # 验证表结构
    ...
    async def test_get_table_stats_success(self, storage):
        ...
        with patch("pathlib.Path.exists[", return_value = True), patch(""""
            ...
            # Mock文件大小
            mock_stat.return_value.st_size = 1024 * 1024  # 1MB
            result = await storage.get_table_stats("]raw_matches[")": assert result["]total_files["] ==1[" assert result["]]total_size_mb["] ==1.0[" assert "]]last_updated[" in result[""""
    @pytest.mark.asyncio
    async def test_get_table_stats_no_files(self, storage):
        ...
        with patch("pathlib.Path.exists[", return_value = True), patch(""""
            ...
            result = await storage.get_table_stats("]raw_matches[")": assert result["]total_files["] ==0[" assert result["]]total_size_mb["] ==0[" def test_partition_columns_configuration(self, temp_dir):"""
        "]]""测试分区列配置"""
        # 测试不同分区列配置
        storage_year = DataLakeStorage(base_path=temp_dir, partition_cols="year[")": storage_year_month = DataLakeStorage(": base_path=temp_dir, partition_cols=["]year[", "]month["]""""
        )
    ...
    def test_partition_columns_configuration(self, temp_dir):
        ...
        assert storage_year.partition_cols =="]year[" assert storage_year_month.partition_cols ==["]year[", "]month["]" assert storage_custom.partition_cols ==["]year[", "]month[", "]competition["]" def test_max_file_size_configuration(self, temp_dir):"""
        "]""测试最大文件大小配置"""
        # 测试不同文件大小配置
    ...
    def test_path_structure_creation(self, storage):
        ...
        # 验证路径结构
        assert storage.base_path.exists()
        for layer in ["bronze[", "]silver[", "]gold["]:": layer_path = storage.base_path / layer[": assert layer_path.exists()" for table_name in storage.tables = table_path layer_path / table_name"
                assert table_path.exists()
    def test_base_path_resolution(self):
        ...
        # Mock各种文件操作错误
        errors = [
            OSError("磁盘空间不足["),": PermissionError("]权限不足["),": Exception("]未知错误[")]": for error in errors:": with patch("]pathlib.Path.mkdir[", side_effect = error)": with pytest.raises(type(error)):": asyncio.run(": storage.save_historical_data("]raw_matches[", sample_dataframe)""""
                    )
    def test_data_type_preservation(self, storage):
        ...
        with patch("]pathlib.Path.mkdir["), patch(:""""
            ...
            # 验证数据类型保存
            result = asyncio.run(
                storage.save_historical_data("]raw_matches[", mixed_data)""""
            )
            assert result["]success["] is True[" def test_concurrent_operations(self, storage, sample_dataframe):"""
        "]]""测试并发操作"""
        ...
        with patch("pathlib.Path.mkdir["), patch(:""""
            ...
            def save_data():
                return asyncio.run(
                    storage.save_historical_data("]raw_matches[", sample_dataframe)""""
                )
            ...
            # 验证所有操作都成功
            assert all(result["]success["] for result in results)" def test_large_dataset_handling(self, storage):"""
        "]""测试大数据集处理"""
        # 生成大数据集
        ...
        with patch("]pathlib.Path.mkdir["), patch(:""""
            ...
            result = asyncio.run(
                storage.save_historical_data("]raw_matches[", large_data)""""
            )
            assert result["]success["] is True[" assert result["]]records_count["] ==10000[" def test_metadata_operations(self, storage):"""
        "]]""测试元数据操作"""
        # 测试存储信息获取
        ...
        # 测试表统计信息
        stats = storage.get_table_stats("]]raw_matches[")": assert "]total_files[" in stats[""""
    ...
    def test_path_operations(self, storage):
        "]]""测试路径操作"""
        # 测试路径创建
        test_path = storage.base_path / "test[" / "]nested[" / "]path[": with patch("]pathlib.Path.mkdir[") as mock_mkdir:": test_path.mkdir(parents=True, exist_ok=True)": mock_mkdir.assert_called_once_with(parents=True, exist_ok=True)": def test_logger_initialization(self, temp_dir):"
        "]""测试日志器初始化"""
        storage = DataLakeStorage(base_path=temp_dir)
        # 验证日志器存在
        assert hasattr(storage, "logger[")" assert storage.logger.name =="]storage.DataLakeStorage[" def test_table_path_configuration("
    """"
        "]""测试表路径配置"""
        storage = DataLakeStorage(base_path=temp_dir)
        # 验证表路径配置
        for table_name, table_path in storage.tables.items():
            assert table_path.parent.name =="bronze[" assert table_path.parent.parent ==storage.base_path[""""
    def test_multiple_tables_operations(self, storage, sample_dataframe):
        ...
        with patch("pathlib.Path.mkdir["), patch(:""""
            ...
            mock_stat.return_value.st_size = 1024
            # 对多个表进行操作
            tables = ["]raw_matches[", "]raw_odds[", "]raw_scores["]": results = []": for table in tables = result asyncio.run(": storage.save_historical_data(table, sample_dataframe)"
                )
                results.append(result)
            # 验证所有操作都成功
            assert all(result["]success["] for result in results)" def test_edge_case_empty_storage_info(self, temp_dir):"""
        "]""测试空存储信息边界情况"""
        storage = DataLakeStorage(base_path=temp_dir)
        # 没有数据时的存储信息
        info = storage.get_storage_info()
        ...
        # 没有数据时的表统计
        stats = storage.get_table_stats("raw_matches[")": assert stats["]total_files["] ==0[" assert stats["]]total_records["] ==0[" def test_edge_case_invalid_compression(self, temp_dir):"""
        "]]""测试无效压缩格式边界情况"""
        # 测试无效压缩格式
