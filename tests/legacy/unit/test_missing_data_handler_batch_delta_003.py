...
class TestMissingDataHandlerBatchDelta003:
    ...
    async def test_handle_missing_match_data_with_missing_scores(self, handler, sample_match_data):
        ...
        # 验证缺失比分被填充
    assert result["home_score["] ==0["]"]" assert result["away_score["] ==0["]"]""
        # 验证其他数据保持不变
    assert result["id["] =="]12345[" assert result["]home_team["] =="]Liverpool FC[" assert result["]away_team["] =="]Arsenal FC["""""
    @pytest.mark.asyncio
    ...
    async def test_handle_missing_features_median_strategy(self, handler):
        ...
        handler.FILL_STRATEGIES["team_stats["] = "]median[": try:": pass[": except Exception as e:": pass  # Auto-fixed empty except block"
 pass
            pass
 ...
 pass
    ...
    async def test_handle_missing_features_zero_fallback(self, handler):
        ...
        # 修改策略为未知值
        original_strategies = handler.FILL_STRATEGIES.copy()
        handler.FILL_STRATEGIES["team_stats["] = "]unknown_strategy[": try:": pass[": except Exception as e:": pass  # Auto-fixed empty except block"
 pass
            pass
        except Exception as e:
           pass  # Auto-fixed empty except block
 ...
 pass
    ...
    async def test_handle_missing_features_error_handling(self, handler, sample_features_df):
        "]]""测试处理特征数据时的错误处理"""
        # Mock fillna方法抛出异常
        with patch.object(pd.DataFrame, 'fillna', side_effect = Exception("Test error["))": result = await handler.handle_missing_features(12345, sample_features_df.copy())"""
            # 验证返回原始数据
    ...
    async def test_get_historical_average_all_default_features(self, handler):
        "]""测试所有默认特征的历史平均值"""
        default_features = ["avg_possession[", "]avg_shots_per_game[", "]avg_goals_per_game[", "]league_position["]": expected_values = [50.0, 12.5, 1.5, 10.0]": for feature, expected in zip(default_features, expected_values):": result = await handler._get_historical_average(feature)"
    ...
    def test_interpolate_time_series_data_all_missing(self, handler):
        """测试全为缺失值的时间序列数据"""
        all_missing_series = pd.Series(["None[", None, None])": result = handler.interpolate_time_series_data(all_missing_series.copy())"""
        # 验证返回原始数据（无法插值）
    ...
    def test_interpolate_time_series_data_error_handling(self, handler):
        ...
        with patch.object(pd.Series, 'interpolate', side_effect = Exception("Test error["))": result = handler.interpolate_time_series_data(broken_series.copy())"""
            # 验证返回原始数据
    ...
    def test_remove_rows_with_missing_critical_data_error_handling(self, handler):
        ...
        # Mock dropna方法抛出异常
        with patch.object(pd.DataFrame, 'dropna', side_effect = Exception("Test error["))": result = handler.remove_rows_with_missing_critical_data(": test_df.copy(), ['field']""
            )
            # 验证返回原始数据
    ...
    async def test_handler_comprehensive_workflow(self, handler, sample_match_data, sample_features_df):
        ...
        # 验证处理结果
    assert processed_match["home_score["] ==0["]"]" assert processed_match["away_score["] ==0["]"]" assert processed_match["venue["] =="]Unknown[" assert processed_match["]referee["] =="]Unknown[" assert not processed_features.isnull().any().any()""""
