...
class TestAPIResponseModel:
    ...
    def test_response_model_valid_data(self):
        ...
        "]message[: "操作成功[","]"""
        "]data[": {"]id[": 1, "]name[": "]测试["},""""
        "]code[: "200["}"]": model = APIResponseModel(**response_data)": assert model.success is True"
    assert model.message =="]操作成功[" assert model.data =={"]id[" 1, "]name[" "]测试["}" assert model.code =="]200[" def test_response_model_minimal_data("
    """"
        "]""测试最小化响应模型数据"""
        response_data = {"success[": False, "]message[": "]操作失败["}": model = APIResponseModel(**response_data)": assert model.success is False[" assert model.message =="]]操作失败[" assert model.data is None[""""
...
class TestAPIResponse:
    ...
    assert response["success["] is True["]"]" assert response["message["] =="]操作成功[" assert "]timestamp[" in response[""""
    assert "]]data[" not in response[""""
        # 验证时间戳格式
        timestamp = response["]]timestamp["]: datetime.fromisoformat(timestamp)  # 如果格式错误会抛出异常[": def test_success_with_data(self):"""
        "]]""测试带数据的成功响应"""
        test_data = {
        "id[": 123,""""
        "]name[: "测试用户[","]"""
    ...
    assert response["success["] is False["]"]" assert response["message["] =="]操作失败[" assert response["]code["] ==500  # 默认错误代码[" assert "]]timestamp[" in response[""""
    assert "]]data[" not in response[""""
    ...
    assert response["]]message["] ==error_message[" assert response["]]code["] ==error_code[" assert "]]timestamp[" in response[""""
    ...
    assert "]]timestamp[" in error_response[""""
        # 验证时间戳格式
        datetime.fromisoformat(success_response["]]timestamp["])": datetime.fromisoformat(error_response["]timestamp["])": def test_response_with_complex_data_types(self):"""
        "]""测试复杂数据类型的响应"""
        complex_data = {
        "string[: "测试字符串[","]"""
        "]number[": 42,""""
        ...
        "]boolean[": True,""""
            "]null[": None,""""
            "]list[": [1, "]two[", {"]three[": 3}],""""
            "]dict[": {"]nested[": {"]deep[": "]value["}, "]chinese[": "]中文测试["}}": response = APIResponse.success(data=complex_data)": assert response["]success["] is True[" assert response["]]data["] ==complex_data[" def test_all_response_fields_present(self):"""
        "]]""测试响应包含所有必要字段"""
        # 成功响应
        success_response = APIResponse.success(data={"test[": True))": required_success_fields = {"]success[", "]message[", "]timestamp[", "]data["}": assert required_success_fields.issubset(success_response.keys())"""
        # 错误响应
        error_response = APIResponse.error(message = "]错误[", code=400, data={"]error[": True)""""
        )
        required_error_fields = {"]success[", "]message[", "]timestamp[", "]code[", "]data["}": assert required_error_fields.issubset(error_response.keys())" def test_response_immutability(self):""
        "]""测试响应对象的数据完整性"""
        original_data = {"mutable[": ["]test["}": response = APIResponse.success(data=original_data)"""
        # 修改原始数据
        original_data["]mutable["] = "]changed["""""
        # 响应中的数据应该保持不变（如果实现了深拷贝）
        # 注意：当前实现没有深拷贝，这是一个测试记录行为
    assert response["]data["]"]mutable[" =="]changed["  # 当前行为[" def test_edge_case_empty_strings(self):"""
        "]]""测试边界情况：空字符串"""
        response = APIResponse.success(data="", message="")": assert response["success["] is True["]"]" assert response["message["] =="]"""""
    assert response["data["] =="]"""""
    def test_edge_case_zero_values(self):
        """测试边界情况：零值"""
        response = APIResponse.error(message="零值测试[", code=0, data=0)": assert response["]success["] is False[" assert response["]]code["] ==0[" assert response["]]data["] ==0[" def test_large_data_response(self):"""
        "]]""测试大数据量响应"""
        large_data = {f["item_{i}"]: f["value_{i}"] for i in range(1000)": response = APIResponse.success(data=large_data)": assert response["success["] is True["]"]" assert len(response["data["]) ==1000["]"]" assert response["data["]"]item_999[" =="]value_999"