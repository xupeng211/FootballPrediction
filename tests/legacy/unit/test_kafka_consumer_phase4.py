...
async def test_start_consuming_processes_message_success(consumer_fixture):
    ...
    message.topic.return_value = os.getenv("TEST_KAFKA_CONSUMER_PHASE4_RETURN_VALUE_30"): message.partition.return_value = 0[": message.offset.return_value = 1[": async def process_side_effect(passed_msg):": consumer.running = False"
        return True
...
async def test_start_consuming_parse_failure_triggers_retry(consumer_fixture):
    ...
    message = MagicMock()
    message.error.return_value = None
    message.topic.return_value = os.getenv("TEST_KAFKA_CONSUMER_PHASE4_RETURN_VALUE_30"): message.partition.return_value = 1[": message.offset.return_value = 42[": async def failure_side_effect(passed_msg):": consumer.running = False"
        return False
