...
class TestFeatureCalculator:
    ...
    async def test_calculate_all_match_features(
        ...
        with patch.object(:
            feature_calculator.db_manager, "]get_async_session["""""
        ) as mock_session = mock_session_instance AsyncMock()
    ...
    async def test_batch_calculate_team_features(self, feature_calculator):
        ...
        with patch.object(:
            ...
            def mock_execute(query):
                ...
                result = Mock()
                if hasattr(query, "]whereclause[") and "]team_id[": in str(:": query.whereclause["""
                ):
                # 这里简化处理，实际应该解析查询条件
            ...
            # 执行批量计算
            results = feature_calculator.batch_calculate_team_features(
    ...
    assert isinstance(results["]]team_id[", RecentPerformanceFeatures)" assert results["]team_id[".team_id ==team_id[" assert results["]]team_id[".calculation_date ==calculation_date[" def test_feature_property_calculations(self):"""
        "]]""测试特征属性计算"""
        # 测试近期战绩特征的计算属性
    ...
    assert performance_dict["team_id["] ==1[" assert performance_dict["]]recent_5_wins["] ==3[" assert performance_dict["]]recent_5_goals_for["] ==8[" assert "]]calculation_date[" in performance_dict[""""
    ...
    assert h2h_dict["]]home_team_id["] ==1[" assert h2h_dict["]]away_team_id["] ==2[" assert h2h_dict["]]h2h_total_matches["] ==5[""""
        # 测试实体序列化
        match_dict = sample_match_entity.to_dict()
    assert match_dict["]]match_id["] ==1[" assert match_dict["]]home_team_id["] ==1[" assert match_dict["]]away_team_id["] ==2[" assert "]]match_time[" in match_dict[""""
        team_dict = sample_team_entity.to_dict()
    assert team_dict["]]team_id["] ==1[" assert team_dict["]]team_name["] =="]测试球队[" import sqlalchemy as sa["]"]": from src.database.connection import get_database_manager