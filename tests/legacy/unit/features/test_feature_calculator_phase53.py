...
try:
 ...
 pass
    ...
    print(f["]Import failed["]: [{e)])": IMPORT_SUCCESS = False[": class TestFootballFeatureCalculator:""
    "]]""足球特征计算器测试"""
    @pytest.fixture
    def calculator(self):
        """创建特征计算器实例"""
        if not IMPORT_SUCCESS:
        pytest.skip("Cannot import FootballFeatureCalculator[")": return FootballFeatureCalculator()": def test_calculator_initialization(self, calculator):""
        "]""测试计算器初始化"""
        print("🧪 测试特征计算器初始化...")""""
        # 测试基本属性
    ...
    assert hasattr(calculator, 'feature_cache')
        print("✅ 特征计算器初始化测试通过[")": def test_feature_definitions_loading(self, calculator):"""
        "]""测试特征定义加载"""
        print("🧪 测试特征定义加载...")""""
        # 测试特征定义
    ...
    assert feature in definitions, f["Missing feature {feature}"]" print("✅ 特征定义加载测试通过[")""""
    @pytest.mark.asyncio
    ...
    async def test_calculation_error_handling(self, calculator):
        "]""测试计算错误处理"""
        print("🧪 测试计算错误处理...")": with patch.object(calculator, '_get_historical_matches', side_effect = Exception("Database connection failed["))": result = await calculator.calculate_team_strength_features('Team A')": assert result is not None[" assert 'error' in result"
    assert 'Database connection failed' in result['error']
        print("]]✅ 计算错误处理测试通过[")": def test_feature_quality_metrics(self, calculator):"""
        "]""测试特征质量指标"""
        print("🧪 测试特征质量指标...")""""
        # Mock特征数据
        feature_data = [
    ...
    assert 'data_completeness' in quality_metrics
        print("✅ 特征质量指标测试通过[")": def test_feature_calculator_comprehensive():"""
    "]""特征计算器全面测试"""
    print("🚀 开始 Phase 5.3.2[": [特征计算器全面测试...])": test_instance = TestFootballFeatureCalculator()"""
    # 执行所有测试
    tests = [
    # 基础功能测试
 ...
 pass
            ...
            test(calculator)
            passed += 1
            print(f["]  ✅ {test.__name__)"])": except Exception as e:": pass  # Auto-fixed empty except block[": failed += 1"
            print(f["]  ❌ {test.__name__}"]: [{e)])": print(f["\n📊 测试结果["]: [{passed} 通过, {failed) 失败])"]": if failed ==0[": print("]🎉 Phase 5.3.2[": [特征计算器测试完成])": print("]\n📋 测试覆盖的功能:")": print("  - ✅ 计算器初始化和配置[")": print("]  - ✅ 特征定义管理[")": print("]  - ✅ 球队强度计算[")": print("]  - ✅ 近期表现评级[")": print("]  - ✅ 交锋历史分析[")": print("]  - ✅ 联赛排名特征[")": print("]  - ✅ 主场优势计算[")": print("]  - ✅ 伤病停赛影响[")": print("]  - ✅ 天气条件因素[")": print("]  - ✅ 综合特征计算[")": print("]  - ✅ 特征验证和标准化[")": print("]  - ✅ 特征缓存机制[")": print("]  - ✅ 质量指标计算[")": print("]  - ✅ 错误处理[")": else:": print("]❌ 部分测试失败[")": if __name__ =="]__main__[": test_feature_calculator_comprehensive()"]": import sys": from src.features.feature_calculator import FootballFeatureCalculator