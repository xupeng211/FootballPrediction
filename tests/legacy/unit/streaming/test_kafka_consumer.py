...
class TestFootballKafkaConsumer:
    ...
    def consumer(self):
        ...
        with patch("src.streaming.kafka_consumer.StreamConfig[") as mock_config, \:": patch("]src.streaming.kafka_consumer.DatabaseManager[") as mock_db_manager:": mock_config.return_value = Mock()": mock_config.return_value.get_consumer_config.return_value = {""
            "]bootstrap.servers[: "localhost:9092[","]"""
            "]group.id[: "test-group"""""
            }
            with patch("]confluent_kafka.Consumer[") as mock_consumer_class:": mock_consumer = Mock()": mock_consumer_class.return_value = mock_consumer[": return FootballKafkaConsumer()"
    ...
    async def test_create_consumer_success(self, consumer):
        ...
        with patch('confluent_kafka.Consumer') as mock_consumer_class = mock_consumer Mock()
    ...
    async def test_consume_messages_success(self, consumer):
        ...
        consumer.consumer.poll.side_effect = ["]]mock_message[", None]": with patch.object(consumer, '_process_message') as mock_process:": mock_process.return_value = True[": result = await consumer.consume_messages(timeout=1.0)"
    ...
    async def test_process_message_database_error(self, consumer):
        ...
        consumer.db_manager = Mock()
        consumer.db_manager.save_match_data = AsyncMock(side_effect=Exception("]数据库错误["))": result = await consumer._process_match_message(message_data)": assert result is False[""
    @pytest.mark.asyncio
    ...
    async def test_deserialize_message_invalid_json(self, consumer):
        "]""测试反序列化无效JSON"""
        invalid_json = b'{"invalid[": json}'": try:": pass[": except Exception as e:"
 ...
 pass
    ...
    async def test_message_backlog_handling(self, consumer):
        ...
        consumer.consumer.poll.side_effect = messages + ["None["  # 10条消息后结束[": with patch.object(consumer, '_process_message') as mock_process:": mock_process.return_value = True[": result = await consumer.consume_messages(timeout=5.0)"
    assert result is True
    ...
    async def test_connection_error_handling(self, consumer):
        "]]]""测试连接错误处理"""
        consumer.consumer = Mock()
        consumer.consumer.poll = Mock(side_effect=Exception("连接断开["))": result = await consumer.consume_messages()": assert result is False[""
    @pytest.mark.asyncio
    async def test_consumer_initialization_error(self, consumer):
        "]]""测试消费者初始化错误"""
        with patch('confluent_kafka.Consumer') as mock_consumer_class:
            mock_consumer_class.side_effect = Exception("初始化失败[")": result = consumer._create_consumer()": assert result is False[""
    @pytest.mark.asyncio
