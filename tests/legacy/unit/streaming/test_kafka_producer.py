...
class TestFootballKafkaProducer:
    ...
    def producer(self):
        ...
        with patch("src.streaming.kafka_producer.StreamConfig[") as mock_config:": mock_config.return_value = Mock()": mock_config.return_value.get_producer_config.return_value = {""
            "]bootstrap.servers[: "localhost:9092"""""
            }
            with patch("]confluent_kafka.Producer[") as mock_producer_class:": mock_producer = Mock()": mock_producer_class.return_value = mock_producer[": return FootballKafkaProducer()"
    ...
    async def test_create_producer_success(self, producer):
        ...
        with patch('confluent_kafka.Producer') as mock_producer_class = mock_producer Mock()
 ...
 pass
                producer._create_producer()
    assert False, "]应该抛出异常[" except Exception:""""
            pass  # 期望抛出异常
    ...
    assert call_args[1]['topic'] =="]matches-stream[" assert call_args[1]['key'] =="]match_1["""""
        # 解析消息内容
        message_data = json.loads(call_args[1]['value'])
    assert message_data['data_type'] =="]match[" assert message_data['source'] =="]data_collector[" assert message_data['data'] ==match_data[""""
    @pytest.mark.asyncio
    ...
    async def test_send_match_data_producer_not_initialized(self, producer):
        "]]]""测试生产者未初始化时的发送"""
        producer.producer = None
        match_data = {"match_id[: "match_1"", "home_team]}": result = await producer.send_match_data(match_data)": assert result is False[""
    @pytest.mark.asyncio
    ...
    assert result is True
        ...
        call_args = producer.producer.produce.call_args
        message_data = json.loads(call_args[1]['value'])
    assert message_data['data_type'] =="]]odds[" assert message_data['data'] ==odds_data[""""
    @pytest.mark.asyncio
    ...
    assert result is True
        ...
        call_args = producer.producer.produce.call_args
        message_data = json.loads(call_args[1]['value'])
    assert message_data['data_type'] =="]scores[" assert message_data['data'] ==scores_data[""""
    @pytest.mark.asyncio
    async def test_send_batch_success(self, producer):
        ...
        producer.producer.flush = Mock()
        messages = [
        {"match_id[: "match_1"", "data_type]}},""""
        {"match_id[: "match_2"", "data_type]}},""""
        {"match_id[: "match_3"", "data_type]: 2.50}}""""
        ]
    ...
    async def test_send_batch_with_failures(self, producer):
        ...
        producer.producer.flush = Mock()
        # 前两条成功，第三条失败
        producer.producer.produce.side_effect = ["None[", None, Exception("]发送失败[")]": messages = ["""
        {"]match_id[: "match_1"", "data_type]}},""""
        {"match_id[: "match_2"", "data_type]}},""""
        {"match_id[: "match_3"", "data_type]: 2.50}}""""
        ]
    ...
    async def test_serialize_message_failure(self, producer):
        ...
        class UnserializableObject:
            pass
        message = {
            "object[": UnserializableObject()": try:": pass[": except Exception as e:"
           pass  # Auto-fixed empty except block
 pass
            pass
        except Exception as e:
 ...
 pass
    ...
    async def test_delivery_callback_error(self, producer):
        "]""测试消息发送失败回调"""
        mock_err = Exception("消息发送失败[")": mock_msg = Mock()"""
        # 不应该抛出异常，但应该记录日志
        producer._delivery_callback(mock_err, mock_msg)
    @pytest.mark.asyncio
    async def test_reconnect_on_connection_loss(self, producer):
        ...
        # 初始生产者失效
        producer.producer = Mock()
        producer.producer.flush.side_effect = Exception("连接断开[")": match_data = {"]match_id[: "match_1"", "home_team]}""""
        # 第一次发送失败
    ...
    async def test_timeout_handling(self, producer):
        ...
        producer.producer = Mock()
        producer.producer.produce = Mock()
        producer.producer.flush.side_effect = asyncio.TimeoutError("操作超时[")": match_data = {"]match_id[: "match_1"", "home_team]}": result = await producer.send_match_data(match_data)": assert result is False[""
    @pytest.mark.asyncio
    async def test_kafka_broker_unavailable(self, producer):
        "]""测试Kafka代理不可用"""
        with patch('confluent_kafka.Producer') as mock_producer_class:
            mock_producer_class.side_effect = Exception("无法连接到Kafka代理[")": result = producer._create_producer()": assert result is False[""
    @pytest.mark.asyncio
    async def test_validate_message_data(self, producer):
        "]]""测试验证消息数据"""
        # 有效数据
        valid_data = {"match_id[: "match_1"", "home_team]}": assert producer._validate_message_data(valid_data) is True["""
        # 缺少必要字段
        invalid_data = {"]home_team[: "Team A["}"]": assert producer._validate_message_data(invalid_data) is False[""
        # None值
        none_data = None
    ...
    async def test_flush_timeout_handling(self, producer):
        ...
        producer.producer = Mock()
        producer.producer.produce = Mock()
        producer.producer.flush.side_effect = Exception("Flush超时[")": match_data = {"]match_id[: "match_1"", "home_team]}": result = await producer.send_match_data(match_data)": assert result is False[""
    @pytest.mark.asyncio
    ...
    async def test_send_with_retry_mechanism(self, producer):
        ...
        def mock_produce_with_retry(*args, **kwargs):
            ...
            call_count += 1
            if call_count < 3:
            raise Exception("临时失败[")": return None[": producer.producer = Mock()": producer.producer.produce = mock_produce_with_retry"
        producer.producer.flush = Mock()
