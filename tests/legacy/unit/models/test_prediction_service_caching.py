...
class TestPredictionServiceCaching:
    ...
    def prediction_service(
        ...
        with patch("src.models.prediction_service.mlflow.set_tracking_uri["):": service = PredictionService()": return service[""
    @pytest.mark.asyncio
    ...
    async def test_batch_predict_matches_caching(self, prediction_service):
        ...
        return PredictionResult(match_id=match_id, model_version="v1.0[")": with patch.object(:": prediction_service, "]get_production_model[", return_value=(Mock(), "]v1.0[")""""
            ...
            with patch.object(:
                prediction_service,
                "]predict_match[",": side_effect=predict_match_side_effect):"""
                # 第一次批量预测
