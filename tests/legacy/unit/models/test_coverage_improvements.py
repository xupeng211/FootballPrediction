...
class TestPredictionServiceCoverage:
    ...
    mock_staging_version.version = "0.9.0-staging[": mock_staging_version.current_stage = os.getenv("TEST_COVERAGE_IMPROVEMENTS_CURRENT_STAGE_31"): mock_latest_version = Mock()": mock_latest_version.version = os.getenv("TEST_COVERAGE_IMPROVEMENTS_VERSION_31"): mock_latest_version.current_stage = os.getenv("TEST_COVERAGE_IMPROVEMENTS_CURRENT_STAGE_31")""""
        ...
        mock_client.get_latest_versions.side_effect = [[], "]mock_staging_version[": with patch(:""""
            ...
            "]]src.models.prediction_service.mlflow.sklearn.load_model[",": return_value=Mock()):": model, version = await service.get_production_model("]fallback_model[")": assert version =="]0.9.0-staging[" assert mock_client.get_latest_versions.call_count ==2[""""
        # 测试场景2：Production 和 Staging 均无版本，只有最新版本
        # 创建新的mock_client实例避免状态污染
        mock_client_2 = MagicMock()
        mock_client_2.get_latest_versions.side_effect = [[], [], "]]mock_latest_version[": with patch(:""""
            "]src.models.prediction_service.MlflowClient[", return_value=mock_client_2[""""
        ), patch(
            "]]src.models.prediction_service.mlflow.sklearn.load_model[",": return_value=Mock()):": model, version = await service.get_production_model("]latest_model[")": assert version =="]0.8.0-latest[" assert mock_client_2.get_latest_versions.call_count ==3[""""
    @pytest.mark.asyncio
    async def test_store_prediction_and_metrics_export(self, service):
        "]]""测试 _store_prediction 和 metrics_exporter 的调用"""
        result = PredictionResult(match_id=1, model_version="1.0[")": mock_session = AsyncMock()": service.db_manager.get_async_session = MagicMock()": service.db_manager.get_async_session.return_value.__aenter__.return_value = ("
        mock_session
        )
    ...
    def test_prepare_features_with_missing_values(self, service):
        ...
        # 故意缺少一些特征
        features = {"home_recent_wins[": 0.8, "]away_recent_goals_for[": 1.2}"]": feature_array = service._prepare_features_for_prediction(features)": assert isinstance(feature_array, np.ndarray)"
