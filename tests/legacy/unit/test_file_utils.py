...
class TestFileUtils:
    ...
    def test_ensure_dir_creates_directory(self, tmp_path):
        """测试确保目录存在功能"""
        test_dir = tmp_path / "test_directory[" / "]nested[": result = FileUtils.ensure_dir(test_dir)": assert test_dir.exists()" assert test_dir.is_dir()""
    assert result ==test_dir
    def test_ensure_dir_existing_directory(self, tmp_path):
        "]""测试在已存在目录上调用ensure_dir"""
        existing_dir = tmp_path / "existing[": existing_dir.mkdir()": result = FileUtils.ensure_dir(existing_dir)": assert existing_dir.exists()" assert result ==existing_dir"
    ...
    def test_write_json_success(self, tmp_path):
        ...
        test_data = {"alias[": ["]write["}": json_file = tmp_path / "]alias_write.json[": result = FileUtils.write_json_file(test_data, json_file)": assert result is True[" assert json_file.exists()""
    def test_write_json_file_alias_failure(self, tmp_path):
        "]]""测试write_json_file别名方法失败情况"""
        test_data = {"test[": ["]data["}""""
        # 使用只读目录模拟写入失败
        read_only_dir = tmp_path / "]readonly[": read_only_dir.mkdir()": read_only_dir.chmod(0o444)  # 只读权限[": json_file = read_only_dir / "]]test.json[": try:": pass[": except Exception as e:": pass  # Auto-fixed empty except block"
 pass
            pass
 ...
 pass
    ...
    def test_get_file_hash_binary_file(self, tmp_path):
        "]]""测试获取二进制文件的哈希值"""
        test_content = b["\x00\x01\x02\x03\xff["]"]": test_file = tmp_path / "binary_file.bin[": with open(test_file, "]wb[") as f:": f.write(test_content)": hash_value = FileUtils.get_file_hash(test_file)": assert isinstance(hash_value, str)"
    ...
    assert file_size ==len(test_content.encode("]]utf-8["))" def test_get_file_size_empty_file(self, tmp_path):"""
        "]""测试获取空文件大小"""
        empty_file = tmp_path / "empty.txt[": empty_file.touch()": file_size = FileUtils.get_file_size(empty_file)": assert file_size ==0[" def test_cleanup_old_files(self, tmp_path):"
        "]]""测试清理旧文件功能"""
        # 创建一些测试文件
        old_file1 = tmp_path / "old1.txt[": old_file2 = tmp_path / "]old2.txt[": new_file = tmp_path / "]new.txt["""""
        # 创建文件
    ...
    def test_cleanup_old_files_non_existent_directory(self, tmp_path):
        "]""测试清理不存在目录的旧文件"""
        non_existent = tmp_path / "non_existent_dir[": removed_count = FileUtils.cleanup_old_files(non_existent)": assert removed_count ==0[" def test_cleanup_old_files_no_old_files(self, tmp_path):""
        "]]""测试清理目录中没有旧文件的情况"""
        # 创建一些新文件
        for i in range(3):
        new_file = tmp_path / f["new_file_{i}.txt["]"]": new_file.write_text(f["content {i)"])": removed_count = FileUtils.cleanup_old_files(tmp_path, days=30)": assert removed_count ==0[""
        # 验证文件仍然存在
    assert len(list(tmp_path.iterdir())) ==3
    def test_cleanup_old_files_with_subdirectories(self, tmp_path):
        "]""测试清理时忽略子目录"""
        # 创建文件和子目录
        old_file = tmp_path / "old.txt[": sub_dir = tmp_path / "]subdir[": sub_dir.mkdir()": old_file.write_text("]old content[")""""
        # 设置旧时间戳
        import os
    ...
    @patch("]src.utils.file_utils.Path.iterdir[")": def test_cleanup_old_files_exception_handling(self, mock_iterdir, tmp_path):"""
        "]""测试清理文件时的异常处理"""
        mock_iterdir.side_effect = Exception("权限错误[")": removed_count = FileUtils.cleanup_old_files(tmp_path)": assert removed_count ==0[" def test_all_methods_with_string_paths(self, tmp_path):"
        "]]""测试所有方法都支持字符串路径"""
        test_dir_str = str(tmp_path / "string_test[")": test_file_str = str(tmp_path / "]string_test.json[")": test_data = {"]string_path[": True}"]"""
        # 测试字符串路径
