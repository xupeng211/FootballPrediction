...
class TestConfig:
    ...
    def setup_method(self):
        ...
        self.config_dir = Path(self.temp_dir) / ".footballprediction[": self.config_file = self.config_dir / "]config.json[": def test_config_init_creates_empty_config("
    """"
        "]""测试初始化创建空配置"""
        with patch("pathlib.Path.home[", return_value = Path(self.temp_dir))": config = Config()": assert config._config =={}" assert config.config_dir ==self.config_dir"
    ...
    """"
        "]""测试获取不存在的配置项使用默认值"""
        with patch("pathlib.Path.home[", return_value = Path(self.temp_dir))": config = Config()": result = config.get("]nonexistent[", "]default_value[")": assert result =="]default_value[" def test_get_nonexistent_key_without_default("
    """"
        "]""测试获取不存在的配置项不提供默认值"""
        with patch("pathlib.Path.home[", return_value = Path(self.temp_dir))": config = Config()": result = config.get("]nonexistent[")": assert result is None[" def test_set_config_value(self):""
        "]]""测试设置配置项"""
        with patch("pathlib.Path.home[", return_value = Path(self.temp_dir))": config = Config()": config.set("]test_key[", "]test_value[")": assert config._config["]test_key["] =="]test_value[" def test_save_config_creates_directory("
    """"
        "]""测试保存配置时创建目录"""
        with patch("pathlib.Path.home[", return_value = Path(self.temp_dir))": config = Config()": config.set("]test_key[", "]test_value[")": config.save()": assert self.config_dir.exists()" assert self.config_file.exists()"
    def test_save_config_writes_correct_content(self):
        ...
        with patch("pathlib.Path.home[", return_value = Path(self.temp_dir))""""
            # 创建第一个配置实例并保存
            config1 = Config()
            config1.set("]test_key[", "]test_value[")": config1.set("]number[", 123)": config1.save()"""
            # 创建第二个配置实例，应该加载之前保存的值
            config2 = Config()
            assert config2.get("]test_key[") =="]test_value[" assert config2.get("]number[") ==123[" class TestGlobalConfig:"""
    "]]""测试全局配置实例"""
