...
class TestAnomalyDetectorBatchGamma007:
    ...
    def sample_time_data(self):
        ...
        return pd.Series(
            ["base_time[",": base_time + timedelta(minutes=1),": base_time + timedelta(minutes=2),": base_time + timedelta(hours=5),  # 异常间隔"
            ]
        )
    def test_anomaly_detector_initialization_coverage(self, detector):
        ...
        # 验证 odds 表配置
        odds_config = detector.detection_config["]]odds["]: assert "]numeric_columns[" in odds_config[""""
        assert "]]home_odds[" in odds_config["]thresholds["]""""
        # 验证 predictions 表配置
        predictions_config = detector.detection_config["]predictions["]: assert "]numeric_columns[" in predictions_config[""""
        assert "]]home_win_probability[" in predictions_config["]thresholds["]: def test_detect_three_sigma_anomalies_edge_cases("
    """"
        "]""测试3σ规则边缘情况"""
        # 测试包含None和NaN的数据
        data_with_nulls = pd.Series([1, 2, None, 4, 5, float("nan["), 100])": anomalies = detector._detect_three_sigma_anomalies(": data_with_nulls, "]test[", "]column["""""
        )
        # 应该处理空值并检测异常
    ...
    def test_detect_three_sigma_anomalies_mixed_types(self, detector):
        "]""测试3σ规则混合类型数据"""
        # 混合类型数据（字符串和数字）
        mixed_data = pd.Series([1, 2, "invalid[", 4, 100])": anomalies = detector._detect_three_sigma_anomalies(mixed_data, "]test[", "]column[")""""
        # 应该优雅处理类型错误
        assert isinstance(anomalies, list)
    def test_detect_iqr_anomalies_coverage(self, detector):
        ...
        anomalies_same = detector._detect_iqr_anomalies(same_values, "]test[", "]column[")": assert len(anomalies_same) ==0[" def test_detect_iqr_anomalies_mixed_data(self, detector):""
        "]]""测试IQR方法混合数据"""
        # 包含非数值的数据
        mixed_data = pd.Series([1, 2, "text[", 3, 100])": anomalies = detector._detect_iqr_anomalies(mixed_data, "]test[", "]column[")""""
        # 应该优雅处理错误
        assert isinstance(anomalies, list)
    def test_detect_z_score_anomalies_coverage(self, detector):
        ...
        anomalies = detector._detect_z_score_anomalies(normal_data, "test[", "]column[")": assert isinstance(anomalies, list)"""
        # 测试异常数据
        outlier_data = pd.Series([1, 2, 3, 4, 100])
        anomalies_outlier = detector._detect_z_score_anomalies(
            outlier_data, "]test[", "]column["""""
        )
        # 验证异常检测
    ...
    def test_detect_z_score_anomalies_edge_cases(self, detector):
        ...
        anomalies = detector._detect_z_score_anomalies(single_value, "test[", "]column[")": assert len(anomalies) ==0["""
        # 测试两个相同值
        two_same = pd.Series([5, 5])
        anomalies_two = detector._detect_z_score_anomalies(two_same, "]]test[", "]column[")": assert len(anomalies_two) ==0[" def test_detect_range_anomalies_threshold_coverage(self, detector):""
        "]]""测试范围检查阈值覆盖"""
        # 设置测试配置
        ...
        # 测试正常数据
        normal_data = pd.Series([1, 2, 3, 4, 5])
        anomalies = detector._detect_range_anomalies(
            normal_data, "]test_table[", "]test_column["""""
        )
        assert len(anomalies) ==0
        # 测试超出范围数据
        range_data = pd.Series([1, 2, 15, -5])
        anomalies_range = detector._detect_range_anomalies(
            range_data, "]test_table[", "]test_column["""""
        )
    ...
    def test_detect_frequency_anomalies_coverage(self, detector):
        "]""测试频率异常检测覆盖"""
        # 测试均匀分布
        uniform_data = pd.Series(["A[", "]B[", "]C[", "]A[", "]B[", "]C["])": anomalies = detector._detect_frequency_anomalies(uniform_data, "]test[", "]column[")": assert len(anomalies) ==0["""
        # 测试频率不均衡
        skewed_data = pd.Series(["]]A[", "]A[", "]A[", "]A[", "]B[", "]C["])": anomalies_skewed = detector._detect_frequency_anomalies(": skewed_data, "]test[", "]column["""""
        )
        assert isinstance(anomalies_skewed, list)
    def test_detect_frequency_anomalies_single_value(self, detector):
        "]""测试频率异常检测单值情况"""
        # 所有值相同
        single_value_data = pd.Series(["A[", "]A[", "]A[", "]A["])": anomalies = detector._detect_frequency_anomalies(": single_value_data, "]test[", "]column["""""
        )
        assert isinstance(anomalies, list)
    def test_detect_time_gap_anomalies_coverage(self, detector, sample_time_data):
        "]""测试时间间隔异常检测覆盖"""
        # 测试时间数据
        anomalies = detector._detect_time_gap_anomalies(
            sample_time_data, "test[", "]column["""""
        )
        assert isinstance(anomalies, list)
        # 测试短时间序列
        short_time_data = pd.Series([datetime.now()])
        anomalies_short = detector._detect_time_gap_anomalies(
            short_time_data, "]test[", "]column["""""
        )
    ...
    def test_detect_time_gap_anomalies_string_dates(self, detector):
        ...
        string_time_data = pd.Series(
            ["2025-01-01 10:00:00[", "]2025-01-01 10:01:00[", "]2025-01-01 15:00:00["]""""
        )
        anomalies = detector._detect_time_gap_anomalies(
            string_time_data, "]test[", "]column["""""
        )
        # 应该处理字符串格式并检测异常
        assert isinstance(anomalies, list)
    ...
    async def test_detect_anomalies_error_handling_coverage(self, detector):
        """测试异常检测错误处理覆盖"""
        # Mock数据库会话抛出异常
        with patch.object(detector.db_manager, "get_async_session[") as mock_get_session:": mock_session = AsyncMock()": mock_get_session.return_value.__aenter__.return_value = mock_session[": mock_get_session.return_value.__aexit__.return_value = None"
            # Mock _detect_table_anomalies 抛出异常
            with patch.object(:
                detector, "]]_detect_table_anomalies[", side_effect=Exception("]Test error[")""""
            ):
    ...
    async def test_detect_table_anomalies_no_data(self, detector):
        ...
        with patch.object(detector, "_get_table_data[", return_value = pd.DataFrame())": detector.detection_config["]test_table["] = {""""
    ...
    async def test_get_table_data_database_error(self, detector):
        "]""测试获取表数据数据库错误"""
        mock_session = AsyncMock()
        mock_session.execute.side_effect = Exception("Database error[")": data = await detector._get_table_data(mock_session, "]test_table[")": assert data.empty["""
    @pytest.mark.asyncio
    async def test_get_anomaly_summary_coverage(self, detector):
        "]]""测试异常摘要覆盖"""
        # 测试空摘要
        empty_summary = await detector.get_anomaly_summary([])
        assert empty_summary["total_anomalies["] ==0["]"]" assert empty_summary["by_severity["] =={}"]" assert empty_summary["by_type["] =={}"]" assert empty_summary["by_table["] =={}"]" assert "summary_time[" in empty_summary[""""
    @pytest.mark.asyncio
    ...
    """"
        ...
        for method in methods = result method(test_data, "]test_table[", "]test_column[")": assert isinstance(result, list)" def test_configuration_validation_coverage(self, detector):""
        "]""测试配置验证覆盖"""
        # 验证配置结构
        config = detector.detection_config
        # 检查必需的表
        required_tables = ["matches[", "]odds[", "]predictions["]": for table in required_tables:": assert table in config[""
        # 检查matches表配置的完整性
        matches_config = config["]]matches["]: required_keys = [""""
            "]numeric_columns[",""""
            "]time_columns[",""""
            "]categorical_columns[",""""
            "]thresholds["]": for key in required_keys:": assert key in matches_config[" def test_anomaly_result_comprehensive_coverage(self):"
    ...
    def test_anomaly_detection_edge_case_data(self, detector):
        "]""测试异常检测边缘情况数据"""
        # 测试包含极大值的数据
        extreme_data = pd.Series([1, 2, 3, 4, float("inf[")])": anomalies = detector._detect_three_sigma_anomalies(": extreme_data, "]test[", "]column["""""
        )
        # 应该处理无限大值
        assert isinstance(anomalies, list)
    def test_error_logging_coverage(self, detector):
        ...
        # 这些方法应该记录错误但不抛出异常
        assert isinstance(
            detector._detect_three_sigma_anomalies(invalid_data, "]test[", "]test["), list[""""
        )
        assert isinstance(
            detector._detect_iqr_anomalies(invalid_data, "]]test[", "]test["), list[""""
        )
        assert isinstance(
            detector._detect_z_score_anomalies(invalid_data, "]]test[", "]test["), list[""""
        )
        assert isinstance(
            detector._detect_range_anomalies(invalid_data, "]]test[", "]test["), list[""""
        )
        assert isinstance(
            detector._detect_frequency_anomalies(invalid_data, "]]test[", "]test["), list["]"]""
        )