...
def quality_monitor(mock_db_manager):
    ...
    with pytest.MonkeyPatch.context() as m:
        m.setattr("src.database.connection.DatabaseManager[", lambda: mock_db_manager[0])": monitor = QualityMonitor()": yield monitor, mock_db_manager[1]""
@pytest.mark.asyncio
async def test_check_table_freshness_is_fresh(quality_monitor):
    ...
    result = await monitor._check_table_freshness(mock_session, "matches[")": assert result.is_fresh is True[" assert result.records_count ==100[""
@pytest.mark.asyncio
async def test_check_table_freshness_is_stale(quality_monitor):
    ...
    mock_result.first.return_value = mock_row
    mock_session.execute.return_value = mock_result
    result = await monitor._check_table_freshness(mock_session, "matches[")": assert result.is_fresh is False[" assert result.freshness_hours > 24[""
@pytest.mark.asyncio
async def test_check_table_completeness(quality_monitor):
    ...
    critical_fields_count = len(monitor.critical_fields.get("matches[", []))": mock_session.execute.side_effect = ["]mock_total_result[" + ["]mock_missing_result[" * critical_fields_count[": result = await monitor._check_table_completeness(mock_session, "]]matches[")": assert result.total_records ==100[" assert result.completeness_score < 100[""
