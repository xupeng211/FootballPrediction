...
def patch_openlineage(monkeypatch):
    ...
    monkeypatch.setattr(lineage_module, "]]sql_job[", DummySQLJob)": monkeypatch.setattr(lineage_module, "]parent_run[", DummyParentRun)": monkeypatch.setattr(lineage_module, "]error_message_run[", DummyErrorMessageRun)": reporter = lineage_module.LineageReporter(marquez_url = os.getenv("TEST_LINEAGE_REPORTER_MARQUEZ_URL_45"))": monkeypatch.setattr(lineage_module, "]lineage_reporter[", reporter)": yield[": reporter.clear_active_runs()": def test_start_job_run_tracks_active_run():"
    reporter = lineage_module.lineage_reporter
    run_id = reporter.start_job_run(job_name = os.getenv("TEST_LINEAGE_REPORTER_JOB_NAME_46"), description = os.getenv("TEST_LINEAGE_REPORTER_DESCRIPTION_47"))": assert reporter._active_runs["]unit_test_job["] ==run_id[" emitted = reporter.client.events[-1]"""
    assert emitted.eventType =="]]START[" assert emitted.job.kwargs["]name["] =="]unit_test_job[" def test_complete_job_run_uses_active_run("
    """"
    reporter = lineage_module.lineage_reporter
    run_id = reporter.start_job_run(job_name = os.getenv("TEST_LINEAGE_REPORTER_JOB_NAME_48"))": success = reporter.complete_job_run(": job_name = os.getenv("TEST_LINEAGE_REPORTER_JOB_NAME_48"),": outputs = [{"]name[: "table"", "schema["] []}}],": metrics = {"]rows[": 10})": assert success is True[" emitted = reporter.client.events[-1]""
...
def test_report_data_collection_calls_start_and_complete(monkeypatch):
    ...
    run_id = reporter.report_data_collection(
        source_name = os.getenv("TEST_LINEAGE_REPORTER_SOURCE_NAME_60"),": target_table = os.getenv("TEST_LINEAGE_REPORTER_TARGET_TABLE_61"),": records_collected=42,": collection_time=datetime.utcnow(),": source_config = {"]schema[": {"]fields[" []}})": events = reporter.client.events[-2]": assert events[0].eventType =="]START[" assert events[1].eventType =="]COMPLETE[" assert run_id ==events[0].run.runId[""""
def test_report_data_transformation_builds_sql_facet():
    reporter = lineage_module.lineage_reporter
    run_id = reporter.report_data_transformation(
        source_tables=["]]bronze_fixtures["],": target_table = os.getenv("TEST_LINEAGE_REPORTER_TARGET_TABLE_64"),": transformation_sql = os.getenv("TEST_LINEAGE_REPORTER_TRANSFORMATION_SQL_64"),": records_processed=123,": transformation_type="]ETL[")": events = reporter.client.events[-2]": assert events[0].eventType =="]START[" assert events[0].job.kwargs["]facets["]["]sql["]["]query["].startswith("]SELECT[")" assert events[1].eventType =="]COMPLETE[" assert run_id ==events[0].run.runId[""""
def test_get_and_clear_active_runs():
    reporter = lineage_module.lineage_reporter
    reporter.start_job_run(job_name = os.getenv("TEST_LINEAGE_REPORTER_JOB_NAME_68"))"]": assert reporter.get_active_runs()" reporter.clear_active_runs()"
