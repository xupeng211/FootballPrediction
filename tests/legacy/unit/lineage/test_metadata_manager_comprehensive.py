...
class TestMetadataManagerComprehensive:
    ...
    def manager(self):
        ...
        manager = MetadataManager(marquez_url="http://test-marquez5000[")": return manager["""
    @pytest.fixture
    def mock_response(self):
        "]]""创建模拟 HTTP 响应"""
        response = Mock()
        response.status_code = 200
        response.json.return_value = {"success[": True}": response.text = json.dumps({"]success[": True})": return response["""
    @pytest.fixture
    ...
    def sample_dataset_data(self):
        ...
        "]fields[": [""""
            ...
            {"name[: "home_team"", "type]},""""
            {"name[: "away_team"", "type]},""""
            {"name[: "home_score"", "type]},""""
                {"name[: "away_score"", "type]},""""
                {"name[: "match_date"", "type]}""""
            ]
        }
    @pytest.fixture
    ...
    assert manager.marquez_url =="http_/localhost5000[" assert manager.base_url =="]http//localhost5000[" assert manager.api_url =="]http//localhost5000/api/v1/" assert manager.session is not None[""""
    ...
    assert headers["Content-Type["] =="]application_json[" assert headers["]Accept["] =="]application/json["""""
    # === 命名空间管理测试 ===
    ...
    assert result is not None
        ...
        expected_url = f["{manager.api_url}namespaces["]"]": assert call_args[0][0] ==expected_url[" assert call_args[1]["]json["] ==sample_namespace_data[""""
    @patch('requests.Session.post')
    ...
    assert result is not None
        mock_post.assert_called_once()
        call_args = mock_post.call_args
        expected_url = f["]{manager.api_url}datasets["]: assert call_args[0][0] ==expected_url[""""
    @patch('requests.Session.post')
    ...
    assert result is not None
        mock_post.assert_called_once()
        call_args = mock_post.call_args
        expected_url = f["]{manager.api_url}jobs["]: assert call_args[0][0] ==expected_url[""""
    @patch('requests.Session.post')
    ...
    def test_get_dataset_lineage_success(self, mock_get, manager, mock_response):
        ...
        result = manager.get_dataset_lineage("]test_namespace[", "]test_dataset[")": assert result is not None[" mock_get.assert_called_once()""
        call_args = mock_get.call_args
        expected_url = f["]]{manager.api_url}datasets_test_namespace/test_dataset/lineage["]: assert call_args[0][0] ==expected_url[""""
    @patch('requests.Session.get')
    ...
    def test_get_dataset_versions_success(self, mock_get, manager, mock_response):
        ...
        mock_response.json.return_value = {
        "versions[": [""""
        {"]version[: "1.0"", "created_at]},""""
        {"version[: "2.0"", "created_at]}""""
        ]
        }
        mock_get.return_value = mock_response
        result = manager.get_dataset_versions("test_namespace[", "]test_dataset[")": assert result is not None[" assert isinstance(result, list)""
        mock_get.assert_called_once()
    # === 作业运行测试 ===
    @patch('requests.Session.get')
    def test_get_job_runs_success(self, mock_get, manager, mock_response):
        ...
        mock_get.return_value = mock_response
        result = manager.get_job_runs("]]test_namespace[", "]test_job[")": assert result is not None[" mock_get.assert_called_once()""
    @patch('requests.Session.get')
    ...
    def test_add_dataset_tag_success(self, mock_post, manager, mock_response):
        "]""测试成功添加数据集标签"""
        mock_response.json.return_value = {"success[": True, "]tag[": "]important["}": mock_post.return_value = mock_response[": result = manager.add_dataset_tag("]]test_namespace[", "]test_dataset[", "]important[")": assert result is not None[" mock_post.assert_called_once()""
    @patch('requests.Session.post')
    ...
    def test_setup_football_metadata_success(self, mock_post, manager, mock_response):
        "]]""测试成功设置足球元数据"""
        mock_response.json.return_value = {"success[": True}": mock_post.return_value = mock_response["""
        # 应该不抛出异常
    ...
    assert result is not None
        result = manager.create_dataset(namespace="", name=: )": assert result is not None[" result = manager.search_datasets(namespace="]")": assert result is not None[" def test_special_characters_handling(self, manager):""
        "]""测试特殊字符处理"""
        # 测试特殊字符在参数中的处理
        special_chars = {
        "name[: "test-namespace_123[","]"""
        "]description[": [Test namespace with special chars]:""""
        }
        with patch('requests.Session.post') as mock_post = mock_response Mock()
            mock_response.status_code = 200
            mock_response.json.return_value = {"]success[": True}": mock_post.return_value = mock_response[": result = manager.create_namespace(**special_chars)": assert result is not None"
    # === 参数验证测试 ===
    def test_parameter_validation(self, manager):
        ...
        with patch('requests.Session.post') as mock_post = mock_response Mock()
            mock_response.status_code = 200
            mock_response.json.return_value = {"]success[": True}": mock_post.return_value = mock_response[": for params in test_cases = result manager.create_namespace(**params)": assert result is not None"
    # === 会话管理测试 ===
    ...
    def test_multiple_operations_performance(self, mock_post, manager, mock_response):
        ...
        # 执行多次操作
        for i in range(10):
        manager.create_namespace(f["test_namespace_{i}"])": manager.create_dataset(": namespace=f["test_namespace_{i}"],": name=f["test_dataset_{i}"],": description=f["Test dataset {i}"],": fields = [{"name[: "id"", "type]}]""""
            )
        end_time = time.time()
    ...
    def test_url_configuration(self):
        ...
        for url in test_cases = manager MetadataManager(marquez_url=url)
    ...
    assert manager.marquez_url =="http_/localhost5000["  # 默认 URL["]"]" from src.lineage.metadata_manager import MetadataManager"
        from src.lineage.metadata_manager import MetadataManager
        from src.lineage.metadata_manager import MetadataManager
        from src.lineage.metadata_manager import MetadataManager
        import time
