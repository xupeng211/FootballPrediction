...
class TestMainApplicationWithMocks:
    ...
    def test_fastapi_app_creation(self):
        ...
        app = main_module.app
        # 验证应用配置
    assert app.title =="足球预测API[" assert app.description =="]基于机器学习的足球比赛结果预测系统[" assert app.version =="]1.0.0[" assert app.docs_url =="]_docs[" assert app.redoc_url =="]/redoc[" assert hasattr(app, 'lifespan')""""
    def test_cors_middleware_configuration(self):
        ...
        if "CORSMiddleware[": in str(middleware.cls):": cors_middleware = middleware[": break[": assert cors_middleware is not None, "]]]CORS中间件未配置[" def test_lifespan_function_exists("
    """"
        "]""测试生命周期函数存在"""
        import src.main as main_module
    ...
    def test_environment_variable_parsing(self):
        ...
        with patch.dict(os.environ, {'CORS_ORIGINS': 'http:_/localhost:3000,https://example.com')):
            ...
            cors_middleware = None
            for middleware in app.user_middleware:
            if "CORSMiddleware[": in str(middleware.cls):": cors_middleware = middleware[": break[": assert cors_middleware is not None"
    ...
    assert hasattr(app, attr), f["缺少属性 {attr}"]" assert getattr(app, attr) is not None, f["属性为空 {attr}"]""""
        # 验证中间件
