...
class MatchFactory(SQLAlchemyModelFactory):
    ...
    away_possession = factory.LazyAttribute(
        lambda _: lambda obj: 100 - obj.home_possession
    )
    ...
    data_source = factory.Iterator(["]api_football[", "]opta[", "]stats_perform[", "]manual["])": processed = True[": created_at = factory.LazyAttribute(lambda _: datetime.now())": updated_at = factory.LazyAttribute(lambda _: datetime.now())"

    @factory.post_generation
    def ensure_valid_scores(obj, create, extracted, **kwargs):
        "]]""Ensure scores are valid for finished matches."""""""
        if obj.status =="finished[": and obj.home_score is None:": obj.home_score = random.randint(0, 5)": if obj.status =="]finished[": and obj.away_score is None:": obj.away_score = random.randint(0, 5)": class HistoricalMatchFactory(MatchFactory):""
