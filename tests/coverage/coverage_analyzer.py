...
class CoverageAnalyzer:
    ...
    async def _run_coverage_collection(self) -> dict[str, Any]:
        ...
        # 创建Coverage对象
        cov = coverage.Coverage(source=["src"], config_file="tests/coverage/.coveragerc")
...
'''
    ...
    async def _save_analysis_report(self, report_data: dict[str, Any]) -> Path:
        ...
        report_file = (
            output_dir / f"coverage_deep_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
async def main():
    ...
    # 深度分析命令
    subparsers.add_parser("deep-analysis", help="执行深度覆盖率分析")

    # 改进计划命令
    subparsers.add_parser("improvement-plan", help="生成覆盖率改进计划")

    # 复杂度分析命令
    subparsers.add_parser("complexity-analysis", help="执行代码复杂度分析")
