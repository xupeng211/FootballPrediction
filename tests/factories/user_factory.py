...
class UserFactory:
    ...
    def generate_user_preferences(cls):
        ...
        return {
            ...
            "preferred_odds_format": random.choice(["decimal", "fractional", "american"]),
            "default_stake_amount": random.choice([5, 10, 20, 50, 100]),
            "risk_tolerance": random.choice(["low", "medium", "high"]),
            "favorite_markets": random.sample([
                "1X2", "BTTS", "Over/Under", "Correct Score", "Both Teams to Score"
            ], random.randint(1, 3)),
    ...
    def generate_user_prediction_history(cls, num_predictions=10):
        ...
        for _ in range(num_predictions):
            prediction = {
                ...
                "prediction_time": datetime.now() - timedelta(days=random.randint(1, 90)),
    ...
    def generate_new_user(cls):
        ...
        profile = cls.generate_user_profile()
        profile.update({
            "account_created": datetime.now() - timedelta(days=random.randint(1, 7)),
            "last_login": datetime.now() - timedelta(hours=random.randint(1, 24)),
            "subscription_type": "free",
            "total_predictions": 0,
    ...
    def generate_veteran_user(cls):
        ...
        profile = cls.generate_user_profile()
        profile.update({
            "account_created": datetime.now() - timedelta(days=random.randint(365, 1000)),
    ...
    def generate_user_batch(cls, batch_size=10, user_types=None):
        ...
        if user_types is None:
            user_types = ["active", "inactive", "new", "veteran"]

        users = []
        for _ in range(batch_size):
            user_type = random.choice(user_types)
            if user_type == "active":
                users.append(cls.generate_active_user())
            elif user_type == "inactive":
                users.append(cls.generate_inactive_user())
            elif user_type == "new":
                users.append(cls.generate_new_user())
            elif user_type == "veteran":
                users.append(cls.generate_veteran_user())
