...
class MatchFactory(BaseFactory, DataFactoryMixin, TimestampMixin):
    ...
    def create(cls, **kwargs) -> Dict[str, Any]:
        ...
        if total_possession != 100:
            default_data['home_possession'] = round(default_data['home_possession'] / total_possession * 100, 1)
...
class DerbyMatchFactory(MatchFactory):
    ...
    DERBY_NAMES = [
        "国家德比", "米兰德比", "伦敦德比", "曼彻斯特德比",
        "西班牙德比", "德国德比", "法国德比", "意大利德比"
    ]
...
class CupMatchFactory(MatchFactory):
    ...
    def create(cls, **kwargs) -> Dict[str, Any]:
        ...
        match.update({
            'is_cup_match': True,
            'cup_round': kwargs.get('cup_round', random.choice([
                'first_round', 'second_round', 'quarter_final', 'semi_final', 'final'
            ])),
