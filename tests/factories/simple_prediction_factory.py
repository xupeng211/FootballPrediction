...
class SimpleMatchFactory:
    ...
    def create_match(cls, **overrides) -> Dict[str, Any]:
        ...
        base_match = {
            ...
            "home_team": home_team,
            "away_team": away_team,
            "match_date": overrides.get('match_date', datetime.now(timezone.utc) + timedelta(days=random.randint(1, 30))),
...
class EdgeCaseFactory:
    ...
    def create_high_scoring_match(cls) -> Dict[str, Any]:
        """创建高比分比赛"""
        return SimpleMatchFactory.create_match(
            home_score=random.randint(5, 8),
            away_score=random.randint(5, 8)
        )
...
if __name__ == "__main__":
    ...
    elo_diff = (extreme_match['team_stats']['home']['elo_rating'] -
                extreme_match['team_stats']['away']['elo_rating'])
    print(f"\n✅ 极端Elo差异: {elo_diff} 分")
