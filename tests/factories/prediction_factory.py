...
class PredictionFactory(BaseFactory, DataFactoryMixin, TimestampMixin):
    ...
    # 预测模型类型
    MODEL_TYPES = [
        "neural_network", "random_forest", "logistic_regression",
        "xgboost", "svm", "ensemble", "deep_learning"
    ]

    # 联赛名称
    LEAGUE_NAMES = [
        "西甲", "英超", "意甲", "德甲", "法甲", "中超", "日职联", "韩K联"
    ]
    ...
    def create(cls, **kwargs) -> Dict[str, Any]:
        ...
        # 确保概率总和为1
        probs = [default_data['home_win_prob'], default_data['draw_prob'], default_data['away_win_prob']]
        total = sum(probs)
        if total > 0:
    ...
    def create_for_future(cls, days_ahead: int = 7, **kwargs) -> Dict[str, Any]:
        """创建未来预测"""
        future_date = datetime.now(timezone.utc).replace(day=datetime.now(timezone.utc).day + days_ahead)
        return cls.create(
...
class MatchPredictionFactory(PredictionFactory):
    ...
    def create(cls, **kwargs) -> Dict[str, Any]:
        ...
        # 添加比赛特有字段
        prediction.update({
            'match_venue': kwargs.get('match_venue', random.choice(['主场', '客场', '中性场地'])),
            'weather_condition': kwargs.get('weather_condition', random.choice(['晴', '雨', '雪', '多云'])),
...
class TournamentPredictionFactory(PredictionFactory):
    ...
    def create(cls, **kwargs) -> Dict[str, Any]:
        ...
        # 添加锦标赛特有字段
        prediction.update({
            'tournament_name': kwargs.get('tournament_name', random.choice([
                '世界杯', '欧洲杯', '亚洲杯', '美洲杯', '非洲杯'
            ])),
            'tournament_stage': kwargs.get('tournament_stage', random.choice([
                'group_stage', 'round_of-16', 'quarter-finals', 'semi-finals', 'final'
            ])),
...
class MLModelPredictionFactory(PredictionFactory):
    ...
    # ML模型类型
    ML_MODEL_TYPES = [
        "deep_neural_network", "random_forest", "gradient_boosting",
        "logistic_regression", "support_vector_machine", "ensemble_stacking",
        "lstm_neural_network", "transformer_model", "cnn_model"
    ]
    ...
    def create(cls, **kwargs) -> Dict[str, Any]:
        ...
        prediction.update({
            ...
            'model_framework': kwargs.get('model_framework', random.choice([
            ...
            'model_explainability': kwargs.get('model_explainability', random.choice(['high', 'medium', 'low'])),
            ...
            'model_performance_trend': kwargs.get('model_performance_trend', random.choice(['improving', 'stable', 'declining'])),
        })
    ...
    def _generate_feature_importance(cls) -> Dict[str, float]:
        """生成特征重要性"""
        features = [
            'home_team_form', 'away_team_form', 'head_to_head_stats',
            'home_goals_scored', 'away_goals_conceded', 'team_rankings',
            'injury_status', 'weather_conditions', 'home_advantage',
            'recent_performance', 'historical_patterns'
        ]
    ...
    def _generate_shap_values(cls) -> Dict[str, float]:
        ...
        return {
            feature: round(random.uniform(-0.2, 0.2), 4) for feature in features
        }