...
class OddsFactory(BaseFactory, DataFactoryMixin, TimestampMixin):
    ...
    def create_for_match(cls, match_id: int, **kwargs) -> Dict[str, Any]:
        """为特定比赛创建赔率"""
        return cls.create(match_id=match_id, **kwargs)
    ...
    def create_from_bookmaker(cls, bookmaker: str, **kwargs) -> Dict[str, Any]:
        """创建指定博彩公司的赔率"""
        return cls.create(bookmaker=bookmaker, **kwargs)

    @classmethod
    def create_with_margin(cls, margin: float, **kwargs) -> Dict[str, Any]:
        """创建指定利润率的赔率"""
        return cls.create(margin=round(margin, 4), **kwargs)
    ...
    def create_correct_score_odds(cls, **kwargs) -> Dict[str, Any]:
        ...
        for score in scores:
            if score in ["0-0", "1-1", "2-2"]:  # 平局比分
                correct_scores[f"correct_score_{score.replace('-', '_')}"] = round(
                    random.uniform(8.0, 25.0), 2
                )
            elif score in ["1-0", "2-0", "2-1"]:  # 常见比分
                correct_scores[f"correct_score_{score.replace('-', '_')}"] = round(
                    random.uniform(6.0, 15.0), 2
                )
            else:  # 其他比分
                correct_scores[f"correct_score_{score.replace('-', '_')}"] = round(
                    random.uniform(10.0, 50.0), 2
                )
...
class HistoricalOddsFactory(OddsFactory):
    ...
    def create(cls, **kwargs) -> Dict[str, Any]:
        ...
        historical_date = datetime.now(timezone.utc).replace(
        ...
        return super().create(
            ...
            settlement_result=kwargs.get(
                "settlement_result", random.choice(["home_win", "draw", "away_win"])
            ),
            **kwargs,
        )
