...
class OddsFactory(SQLAlchemyModelFactory):
    ...
    asian_handicap_home = factory.LazyAttribute(
        lambda _: round(random.uniform(-2.0, 2.0), 1)
    )
    asian_handicap_away = factory.LazyAttribute(
        lambda _: round(random.uniform(-2.0, 2.0), 1)
    )
    ...
    # Double chance
    home_win_or_draw = factory.LazyAttribute(
        lambda _: round(random.uniform(1.1, 1.5), 2)
    )
    home_win_or_away = factory.LazyAttribute(
        lambda _: round(random.uniform(1.2, 1.8), 2)
    )
