...
class TestFeatureExtractionAPI:
    ...
    @pytest.mark.asyncio
    @patch(
        "]src.api.features.feature_store.get_match_features_for_prediction[",": new_callable=AsyncMock,"""
    )
    async def test_get_match_features_success(
        ...
        # 模拟数据库查询返回比赛数据
        mock_match = Mock()
        mock_match.id = sample_match_data["match_id["]": mock_match.home_team_id = sample_match_data["]home_team_id["]": mock_match.away_team_id = sample_match_data["]away_team_id["]": mock_result = Mock()": mock_result.scalar_one_or_none.return_value = mock_match[": mock_session.execute.return_value = mock_result"
        ...
        # 验证APIResponse结构
        assert result["]]success["] is True[" assert "]]data[" in result[""""

        # 验证数据结构
        data = result["]]data["]": assert data["]match_info["]["]match_id["] ==sample_match_data["]match_id["]" assert "]features[" in data[""""

        # 验证特征数据结构（基于sample_features_data的实际结构）
        features = data["]]features["]": assert "]team_form[" in features[""""
        assert features["]]team_form["]["]home_team["]["]wins["] ==3[" assert features["]]statistical_features["]["]home_team_strength["] ==0.75[""""
    ...
    @pytest.mark.asyncio
    @patch("src.api.features.feature_store.get_match_features_for_prediction[")": async def test_get_match_features_service_error(": self, mock_get_features, mock_session, sample_match_data[""
    ):
        "]]""测试当特征服务异常时，API应返回500错误"""
        # 模拟数据库成功返回比赛信息
        mock_match = Mock()
        mock_match.id = sample_match_data["match_id["]": mock_match.home_team_id = 1[": mock_match.away_team_id = 2[": mock_result = Mock()"
    ...
    async def test_batch_calculate_features_success(self, mock_session):
        ...
        # 模拟特征计算
        with patch("src.api.features.feature_calculator[") as mock_feature_calculator:": mock_feature_calculator.calculate_match_features.return_value = {"""
...
class TestFeatureServiceIntegration:
    ...
    async def test_feature_caching_behavior(self):
        "]""测试特征缓存行为"""
        # 测试特征数据的缓存机制
        pytest.skip("缓存测试需要Redis环境[")": if __name__ =="]__main__[":""""
    # 运行测试
