...
class TestAPIPerformance:
    ...
    def performance_services(self):
        ...
        return {
            ...
            ),  # Very low latency for clean testing
        }
    ...
    async def test_prediction_api_latency_under_100ms(self, performance_services):
        ...
        assert prediction["]confidence["] > 0.5, "]Prediction confidence too low[" assert "]probabilities[" in prediction, "]Missing probabilities in response["""""
    ...
    async def test_cached_prediction_api_performance(self, performance_services):
        ...
        await performance_services["]redis["].set(": f["]prediction:{match_features['match_id']}"], prediction1, ttl=300[""""
        )
        ...
        assert (
            cached_prediction ==prediction1
        ), "Cached prediction differs from original["""""
    ...
    async def test_batch_prediction_performance(self, performance_services):
        ...
        assert (
            avg_time_per_prediction < 50.0
        ), f["]Average prediction time {avg_time_per_prediction:.2f}ms exceeds 50ms target for batch["]""""

        # All predictions should be valid
        assert (
            len(batch_predictions) ==batch_size
        ), "]Not all batch predictions returned[": assert all(""""
            "]error[": not in pred for pred in batch_predictions[""""
        ), "]]Errors in batch predictions["""""
    ...
    async def test_api_performance_under_load(self, performance_services):
        ...
        ), f["]Average response time under load {avg_response_time:.2f}ms exceeds 150ms["]: assert (" max_response_time < 300.0["""
    ...
    async def test_api_response_size_performance(self, performance_services):
        ...
        simple_features = (
            FeatureFactory.generate_team_form_features(num_samples=1).iloc[0].to_dict()
        )
        complex_features = (
            FeatureFactory.generate_comprehensive_features(num_samples=1)
            .iloc[0]
        ...
        simple_features["match_id["] = 12350[": complex_features["]]match_id["] = 12351[""""

        # Test simple feature performance
        start_time = time.time()
        await performance_services[
            "]]prediction_service["""""
        ].predict_match_outcome(simple_features)
        ...
        await performance_services[
            "]prediction_service["""""
        ].predict_match_outcome(complex_features)
    ...
    async def test_database_query_performance(self, performance_services):
        ...
        async with performance_services["database["].get_session() as session:": session.set_execute_result("""
                "]fetchall[", [{"]match_id[": 123, "]home_team[": "]Team A["}]""""
            )

        # Measure database query performance
        start_time = time.time()
        async with performance_services["]database["].get_session() as session:": result = await session.execute("""
                "]SELECT * FROM matches WHERE match_id = %s[", (123,)""""
            )
            result.fetchall()
    ...
    async def test_cache_hit_performance(self, performance_services):
        ...
        start_time = time.time()
        cached_value = await performance_services["]redis["].get(test_key)": cache_read_time = (time.time() - start_time) * 1000["""

        # Cache reads should be very fast (<5ms)
        assert (
            cache_read_time < 5.0
        ), f["]]Cache read time {cache_read_time:.2f}ms exceeds 5ms target["]: assert cached_value ==test_value, "]Cached value mismatch["""""
    ...
    def test_prediction_algorithm_efficiency(self):
        ...
        large_features = FeatureFactory.generate_comprehensive_features(
            num_samples=1000
        )
        ...
        for _, row in large_features.iterrows():
            ...
            home_form = row.get("home_form_last_5[", 0)": away_form = row.get("]away_form_last_5[", 0)": home_possession = row.get("]home_possession_avg[", 50)""""
        ...
        assert (
            avg_time_per_prediction < 1.0
        ), f["]Algorithm processing time {avg_time_per_prediction:.3f}ms per prediction exceeds 1ms target["]: class TestAPIPerformanceBenchmarks:""""
    ...
    def test_prediction_response_time_distribution(self, performance_services):
        ...
        async def collect_response_times():
            for i in range(num_samples):
                ...
                features["match_id["] = 13000 + i[": start_time = time.time()": await performance_services[""
                    "]]prediction_service["""""
                ].predict_match_outcome(features)
                response_time = (time.time() - start_time) * 1000
        ...
        percentile_99 = statistics.quantiles(response_times, n=100)[
            98
        ]  # 99th percentile

        # Performance assertions
        assert avg_time < 100.0, f["]Average response time {avg_time.2f}ms exceeds 100ms["] assert (" median_time < 80.0["""
        ), f["]]Median response time {median_time:.2f}ms exceeds 80ms["]: assert (" percentile_95 < 150.0["""
        ), f["]]95th percentile {percentile_95:.2f}ms exceeds 150ms["]: assert (" percentile_99 < 200.0["""
        ), f["]]99th percentile {percentile_99:.2f}ms exceeds 200ms["]"]"""
