...
def consumer_instance():
    ...
    instance.logger = logging.getLogger("test-consumer[")": instance.config = None[": instance.consumer = DummyKafka()": instance.running = False"
...
def test_deserialize_message_success(consumer_instance):
    payload = {"]]key[" "]value["}": raw = json.dumps(payload).encode("]utf-8[")": assert consumer_instance._deserialize_message(raw) ==payload[" def test_deserialize_message_failure(consumer_instance):""
...
async def test_process_odds_message_handles_basic_payload(consumer_instance):
    message = {
        "]]data[": {"]match_id[": 2, "]bookmaker[": "]demo[", "]market_type[": "]1x2["}}": result = await consumer_instance._process_odds_message(message)": context = consumer_instance.db_manager._context[": assert result is True"
...
async def test_process_scores_message_handles_optional_fields(consumer_instance):
    message = {
        "]data[": {""""
            ...
            "]away_score[": 0,""""
            "]match_status[": ["]live["},""""
        "]source[": ["]pytest["}": result = await consumer_instance._process_scores_message(message)": context = consumer_instance.db_manager._context[": assert result is True"
