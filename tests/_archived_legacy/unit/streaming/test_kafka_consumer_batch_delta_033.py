...
class TestFootballKafkaConsumerBatchDelta033:
    ...
    def consumer_instance(self, mock_stream_config, mock_consumer):
        ...
        with patch("src.streaming.kafka_consumer.DatabaseManager[") as mock_db_manager, \:": patch("]confluent_kafka.Consumer[") as mock_consumer_class:": mock_consumer_class.return_value = mock_consumer[": mock_db_manager.return_value = Mock()": return FootballKafkaConsumer(config=mock_stream_config)"
    @pytest.fixture
    ...
    def test_create_consumer_when_none(self, consumer_instance, mock_consumer):
        ...
        with patch("confluent_kafka.Consumer[") as mock_consumer_class:": mock_consumer_class.return_value = mock_consumer[": consumer_instance._create_consumer()": assert consumer_instance.consumer ==mock_consumer"
    def test_create_consumer_when_exists(self, consumer_instance, mock_consumer):
        ...
        with patch("confluent_kafka.Consumer[") as mock_consumer_class:": mock_consumer_class.return_value = Mock()": consumer_instance._create_consumer()""
            # 应该保持原有的consumer
    ...
    def test_initialize_consumer_success(self, mock_stream_config, mock_consumer):
        ...
        with patch("src.streaming.kafka_consumer.DatabaseManager[") as mock_db_manager, \:": patch("]confluent_kafka.Consumer[") as mock_consumer_class:": mock_consumer_class.return_value = mock_consumer[": mock_db_manager.return_value = Mock()": consumer = FootballKafkaConsumer(config=mock_stream_config)"
            consumer.consumer = None
            consumer._initialize_consumer("]]test_group[")": mock_consumer_class.assert_called_once_with({"""
            "]bootstrap.servers[: "localhost:9092[","]"""
            "]group.id[": ["]test_group[",""""
            "]auto.offset.reset[": ["]earliest[",""""
                "]enable.auto.commit[": True[""""
    ...
    def test_deserialize_message_success(self, consumer_instance):
        "]]""测试成功反序列化消息"""
        original_data = {"key[: "value"", "number]: 123}": message_bytes = json.dumps(original_data).encode('utf-8')": result = consumer_instance._deserialize_message(message_bytes)": assert result ==original_data"
    def test_deserialize_message_invalid_json(self, consumer_instance):
        """测试反序列化无效JSON消息"""
        invalid_bytes = b'{"invalid[": json}'": with pytest.raises(json.JSONDecodeError):": consumer_instance._deserialize_message(invalid_bytes)": def test_deserialize_message_unicode_error(self, consumer_instance):"
        "]""测试反序列化Unicode错误"""
        unicode_bytes = "测试中文[".encode('gbk')  # 非UTF-8编码[": with pytest.raises(UnicodeDecodeError):": consumer_instance._deserialize_message(unicode_bytes)""
    # === 消息处理测试 ===
    @pytest.mark.asyncio
    ...
    assert call_args.data_source =="]test_source[" assert call_args.external_match_id =="]match_123["""""
    @pytest.mark.asyncio
    ...
    async def test_process_message_database_error(self, consumer_instance):
        ...
        consumer_instance.db_manager = Mock()
        consumer_instance.db_manager.get_async_session.side_effect = Exception("]数据库连接失败[")": result = await consumer_instance._process_match_message(message_data)": assert result is False[""
    @pytest.mark.asyncio
    async def test_process_main_message_routing(self, consumer_instance):
        ...
        with patch.object(consumer_instance, '_process_match_message') as mock_match, \:
            ...
            # 测试比赛消息路由
            match_msg = Mock()
            match_msg.value.return_value = json.dumps({"data_type[": ["]match[")).encode('utf-8')": await consumer_instance._process_message(match_msg)": mock_match.assert_called_once()""
            # 测试赔率消息路由
            odds_msg = Mock()
            odds_msg.value.return_value = json.dumps({"]data_type[": ["]odds[")).encode('utf-8')": await consumer_instance._process_message(odds_msg)": mock_odds.assert_called_once()""
            # 测试比分消息路由
            scores_msg = Mock()
            scores_msg.value.return_value = json.dumps({"]data_type[": ["]scores[")).encode('utf-8')": await consumer_instance._process_message(scores_msg)": mock_scores.assert_called_once()""
    # === 订阅管理测试 ===
    ...
    def test_subscribe_topics_with_validation(self, consumer_instance, mock_consumer):
        "]""测试订阅主题时的验证"""
        # 模拟一些主题无效
        consumer_instance.config.is_valid_topic.side_effect = lambda topic topic != "invalid-topic[": topics = ["]valid-topic[", "]invalid-topic[", "]another-valid-topic["]": consumer_instance.subscribe_topics(topics)"""
        # 应该只订阅有效主题
        mock_consumer.subscribe.assert_called_once_with(["]valid-topic[", "]another-valid-topic["])": def test_subscribe_topics_all_invalid(self, consumer_instance, mock_consumer):"""
        "]""测试所有主题都无效的情况"""
        consumer_instance.config.is_valid_topic.return_value = False
        topics = ["invalid-topic1[", "]invalid-topic2["]": consumer_instance.subscribe_topics(topics)"""
        # 不应该调用订阅
 ...
 pass
    ...
    async def test_consume_batch_success(self, consumer_instance, sample_kafka_message):
        """测试批量消费成功"""
        stats = {"processed[": 0, "]failed[": 0}": start_time = datetime.now()"""
        # 模拟消息处理
        with patch.object(consumer_instance, '_process_message') as mock_process:
            mock_process.return_value = True
            # 简化测试：直接测试方法逻辑
    ...
    def test_get_session_function(self):
        ...
        with patch("src.streaming.kafka_consumer.DatabaseManager[") as mock_db_manager:": mock_db_manager.return_value = Mock()": mock_session = AsyncMock()": mock_db_manager.return_value.get_async_session.return_value = mock_session"
            session = get_session()
 ...
 pass
    ...
    async def test_offset_commit_failure(self, consumer_instance, sample_kafka_message):
        """测试偏移量提交失败"""
        consumer_instance.consumer.poll.return_value = sample_kafka_message
        consumer_instance.consumer.commit.side_effect = Exception("提交失败[")": with patch.object(consumer_instance, '_process_message') as mock_process:": mock_process.return_value = True[""
            # 即使提交失败也应该继续运行
            task = asyncio.create_task(consumer_instance.start_consuming(timeout=0.1))
            await asyncio.sleep(0.05)
            consumer_instance.stop_consuming()
