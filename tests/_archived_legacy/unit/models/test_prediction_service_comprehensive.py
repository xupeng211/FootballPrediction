...
class TestPredictionResult:
    ...
    assert result_dict["]]match_id["] ==123[" assert result_dict["]]model_version["] =="]1.0.0[" assert result_dict["]created_at["] ==created_at.isoformat()" assert "]home_win_probability[" in result_dict[""""
    ...
    def test_prediction_result_validation_probabilities_sum(self):
        ...
        result = PredictionResult(
        match_id=123,
        model_version="1.0.0[",": home_win_probability=0.5,": draw_probability=0.3,": away_win_probability=0.2)"
        total_prob = (
...
class TestPredictionService:
    ...
    async def test_get_production_model_fallback_to_staging(self, mock_dependencies):
        ...
        mock_client.get_latest_versions.side_effect = [[], "]mock_version_info[": mock_model = Mock()": with patch(:"""
            ...
            "]]src.models.prediction_service.mlflow.sklearn.load_model[",": return_value=mock_model):": service = PredictionService()": model, version = await service.get_production_model("]test_model[")": assert version =="]0.9.0["""""
    ...
    async def test_get_production_model_caching(self, mock_dependencies):
        ...
        mock_version_info.version = "1.0.0[": mock_version_info.current_stage = "]Production[": mock_client.get_latest_versions.return_value = ["]mock_version_info[": mock_model = Mock()": with patch(:"""
            ...
            # 第二次调用（应该使用缓存）
            model2, version2 = await service.get_production_model("]test_model[")": assert model1 ==model2[" assert version1 ==version2[""
        # 验证只加载了一次模型
    ...
    async def test_predict_match_success(self, mock_dependencies):
        ...
        with patch.object(:
            ...
            service,
            "]_prepare_features_for_prediction[",": return_value=np.array([[0.8, 0.6, 0.7]])):": result = await service.predict_match(123)": assert isinstance(result, PredictionResult)"
    assert result.match_id ==123
    assert result.model_version =="]1.0.0[" assert result.predicted_result =="]home[" assert result.home_win_probability ==0.5[""""
    ...
    async def test_predict_match_no_features_fallback(self, mock_dependencies):
        ...
        mock_model.predict.return_value = np.array("draw[")": mock_match_info = {"]home_team_id[": 1, "]away_team_id[": 2}": mock_default_features = {"]default_feature[": 0.5}": async def mock_get_match_info(match_id):": return mock_match_info[": async def mock_get_features(*args, **kwargs):"
        return None
        # 模拟异步方法
        service._get_match_info = AsyncMock(return_value=mock_match_info)
        service.feature_store.get_match_features_for_prediction = AsyncMock(
    ...
    assert result.predicted_result =="]draw" assert result.features_used ==mock_default_features