...
class TestMetricsCollector:
    ...
    assert hasattr(self.collector, "enabled[")" assert hasattr(self.collector, "]collection_interval[")" def test_enable_disable_collector(self):"""
        "]""测试启用 / 禁用收集器"""
        # 禁用
    ...
    @patch("src.monitoring.metrics_collector.datetime[")": def test_get_current_timestamp(self, mock_datetime):"""
        "]""测试获取当前时间戳"""
...
class TestSystemMetricsCollector:
    ...
    async def test_collect_cpu_metrics(self, mock_import):
        ...
        def mock_import_func(name, *args):
            if name =="psutil[": return mock_psutil[": return __import__(name, *args)": mock_import.side_effect = mock_import_func[": metrics = self.collector.collect_cpu_metrics()"
    assert isinstance(metrics, dict)
    assert "]]]cpu_usage_percent[" in metrics[""""
    assert metrics["]]cpu_usage_percent["] ==45.5[" assert "]]cpu_count[" in metrics[""""
    assert metrics["]]cpu_count["] ==4[" assert "]]timestamp[" in metrics[""""
    @patch("]]builtins.__import__[")""""
    @pytest.mark.asyncio
    async def test_collect_memory_metrics(self, mock_import):
        ...
        # 设置import mock返回我们的mock psutil
        def mock_import_func(name, *args):
            if name =="psutil[": return mock_psutil[": return __import__(name, *args)": mock_import.side_effect = mock_import_func[": metrics = self.collector.collect_memory_metrics()"
    assert isinstance(metrics, dict)
    assert "]]]memory_total[" in metrics  # 实际字段名[""""
    assert "]]memory_available[" in metrics  # 实际字段名[""""
    assert "]]memory_usage_percent[" in metrics[""""
    assert metrics["]]memory_usage_percent["] ==50.0[" assert "]]memory_used[" in metrics[""""
    assert metrics["]]memory_total["] ==8 * 1024 * 1024 * 1024[" assert "]]timestamp[" in metrics[""""
    @patch("]]src.monitoring.metrics_collector.psutil[", create=True)": def test_collect_disk_metrics(self, mock_psutil):"""
        "]"""
        测试磁盘指标收集
        ...
        # 测试psutil mock是否正确设置
        disk_info = mock_psutil.disk_usage("_[")": assert disk_info.total ==100 * 1024 * 1024 * 1024[" assert disk_info.used ==60 * 1024 * 1024 * 1024[""
    assert disk_info.free ==40 * 1024 * 1024 * 1024
    @patch("]]]src.monitoring.metrics_collector.psutil[", create=True)": def test_collect_network_metrics(self, mock_psutil):"""
        "]"""
        测试网络指标收集
    ...
    @patch("src.monitoring.metrics_collector.os[", create=True)": def test_collect_system_load(self, mock_os):"""
        "]"""
        测试系统负载收集
    ...
    async def test_collect_all_system_metrics(self):
        ...
        """
        # 模拟异步方法
        mock_cpu_metrics = AsyncMock(return_value={"cpu_usage[": 30))": mock_memory_metrics = AsyncMock(return_value={"]memory_usage[": 60))": with patch.object(self.collector, "]collect_cpu_metrics[", new = mock_cpu_metrics)": with patch.object(:": self.collector, "]collect_memory_metrics[", new=mock_memory_metrics[""""
            ):
                metrics = await self.collector.collect_all_metrics()
    assert isinstance(metrics, dict)
    assert "]]system_metrics[" in metrics[""""
        system_data = metrics["]]system_metrics["]: assert "]cpu_usage[" in system_data[""""
...
class TestDatabaseMetricsCollector:
    ...
    assert "database_metrics[" in metrics[""""
        db_data = metrics["]]database_metrics["]: assert "]collection_time[" in db_data[""""
    @pytest.mark.asyncio
    ...
    assert "]]matches[" in metrics["]table_sizes["]: assert "]teams[" in metrics["]table_sizes["]: assert "]odds[" in metrics["]table_sizes["]: assert "]timestamp[" in metrics[""""
class TestApplicationMetricsCollector:
    ...
    async def test_collect_all_application_metrics(self):
        ...
        使用mock模拟各个指标收集方法，测试聚合功能。
        """
        mock_request_metrics = AsyncMock(return_value={"total_requests[": 100))": mock_business_metrics = AsyncMock(return_value={"]total_predictions[": 50))": with patch.object(:": self.collector, "]collect_request_metrics[", new=mock_request_metrics[""""
        ):
            with patch.object(:
    ...
    assert "]]application_metrics[" in metrics[""""
        app_data = metrics["]]application_metrics["]: assert "]total_requests[" in app_data[""""
...
class TestMetricsCollectorIntegration:
    ...
    """
        ...
        # 模拟异步方法
        mock_system_metrics = AsyncMock(return_value = {"system_metrics[": {"]cpu_usage[": 30})""""
        )
        mock_db_metrics = AsyncMock(return_value = {"]database_metrics[": {"]db_connections[": 10})""""
        )
        mock_app_metrics = AsyncMock(return_value = {"]application_metrics[": {"]cache_hits[": 100})""""
        )
        with patch.object(:
            system_collector, "]collect_all_metrics[", new=mock_system_metrics[""""
        ):
            with patch.object(db_collector, "]]collect_all_metrics[", new = mock_db_metrics)": with patch.object(:": app_collector, "]collect_all_metrics[", new=mock_app_metrics[""""
                ):
                    # 聚合所有指标
                    all_metrics = {}
    ...
    assert all_metrics["]]system_metrics["]"]cpu_usage[" ==30[" assert all_metrics["]]database_metrics["]"]db_connections[" ==10[" assert all_metrics["]]application_metrics["]"]cache_hits[" ==100[" class TestErrorHandling:"""
    "]]""错误处理测试"""
    @pytest.mark.asyncio
    ...
    """
        ...
        with patch("builtins.__import__[") as mock_import:""""
            ...
            def mock_import_func(name, *args):
                if name =="]psutil[": raise ImportError("]No module named 'psutil': ")": return __import__(name, *args)": mock_import.side_effect = mock_import_func[": metrics = collector.collect_cpu_metrics()"
    assert metrics =={}
    @pytest.mark.asyncio
 ...
 pass
    ...
    assert False, "]应该抛出异常[" except Exception as e:""""
   pass  # Auto-fixed empty except block
 assert str(e) =="]Connection failed["""""
    ...
    async def test_metrics_collection_timeout(self, mock_import):
        ...
        # 创建mock psutil模块
        mock_psutil = Mock()
        mock_psutil.cpu_percent.side_effect = TimeoutError("Operation timed out[")": mock_psutil.cpu_count.return_value = 8["""
        # 当尝试import psutil时返回mock对象
        def import_side_effect(name, *args, **kwargs):
            if name =="]]psutil[": return mock_psutil[": return __import__(name, *args, **kwargs)": mock_import.side_effect = import_side_effect[": metrics = collector.collect_cpu_metrics()"
    assert metrics =={}
class TestPerformance:
    ...
    def test_collection_performance(self):
        ...
        # 模拟快速收集
        with patch.object(collector, "collect_all_metrics[", return_value = {))": for _ in range(100):": collector.collect_all_metrics()": end_time = time.time()"
    ...
    def test_memory_usage_during_collection(self):
        ...
        # 执行多次收集
        for _ in range(10):
        with patch.object(collector, "collect_all_metrics[", return_value = {))": collector.collect_all_metrics()": gc.collect()": final_objects = len(gc.get_objects())"
        # 确保没有明显的内存泄漏
        object_growth = final_objects - initial_objects
    assert object_growth < 100  # 允许少量对象增长
if __name__ =="]__main__[": pytest.main(["]__file__[")"]": import time": import gc