...
class TestMetadataManagerBatchOmega005:
    ...
    def metadata_manager(self):
        ...
        manager = MetadataManager(marquez_url="http://localhost5000[")": return manager["""
    @pytest.fixture
    ...
    assert manager.marquez_url =="http_/localhost5000[" assert manager.api_url =="]http//localhost5000/api/v1/" def test_create_namespace_basic("
    """"
        """测试创建命名空间基本功能"""
    ...
    assert call_args[1]["]json["]["]name["] =="]test_namespace[" assert call_args[1]["]json["]["]description["] =="]Test namespace[" assert call_args[1]["]json["]["]ownerName["] =="]test_owner[" def test_create_namespace_minimal("
    """"
        "]""测试创建命名空间最小参数"""
        with patch.object(metadata_manager.session, 'put', return_value = mock_response)
    ...
    assert result is not None
            # 验证调用参数
            call_args = metadata_manager.session.put.call_args
            payload = call_args[1]["]json["]": assert payload["]name["] =="]test_dataset[" assert payload["]description["] =="]Test dataset[" assert payload["]type["] =="]DB_TABLE[" assert len(payload["]fields["]) ==2[" assert payload["]]tags["] ==tags[" assert payload["]]sourceName["] =="]test_source[" def test_create_dataset_minimal("
    """"
        "]""测试创建数据集最小参数"""
        with patch.object(metadata_manager.session, 'put', return_value = mock_response)
    ...
    assert result is not None
        # 验证调用参数
        call_args = metadata_manager.session.put.call_args
            payload = call_args[1]["]json["]": assert payload["]name["] =="]minimal_dataset[" assert payload["]description["] =="]数据集 minimal_dataset[" assert payload["]type["] =="]DB_TABLE[" assert "]fields[" not in payload[""""
    ...
    def test_create_dataset_with_schema(self, metadata_manager, mock_response):
        ...
        with patch.object(metadata_manager.session, 'put', return_value = mock_response)
            ...
            # 验证Schema字段处理
            call_args = metadata_manager.session.put.call_args
            fields = call_args[1]["]]json["]["]fields["]": assert len(fields) ==3[" assert fields[0]["]]name["] =="]id[" assert fields[0]["]description["] =="]ID[" assert fields[1]["]name["] =="]name[" assert "]description[" not in fields[1]""""
    assert fields[2]["]description["] =="]数值[" def test_create_dataset_error("
    """"
        "]""测试创建数据集错误处理"""
        error_response = Mock()
    ...
    def test_create_job_basic(self, metadata_manager, mock_response):
        "]""测试创建作业基本功能"""
        input_datasets = [
            {"namespace[: "input_ns"", "name]},""""
            {"namespace[: "input_ns"", "name]}""""
        ]
        output_datasets = [
            {"namespace[: "output_ns"", "name]}""""
        ]
        with patch.object(metadata_manager.session, 'put', return_value = mock_response)
    ...
    assert result is not None
            # 验证调用参数
            call_args = metadata_manager.session.put.call_args
            payload = call_args[1]["]json["]": assert payload["]name["] =="]test_job[" assert payload["]description["] =="]Test job[" assert payload["]type["] =="]BATCH[" assert len(payload["]inputs["]) ==2[" assert len(payload["]]outputs["]) ==1[" assert payload["]]location["] =="]/path/to/job.py[" def test_create_job_minimal("
    """"
        "]""测试创建作业最小参数"""
        with patch.object(metadata_manager.session, 'put', return_value = mock_response)
            result = metadata_manager.create_job(
    ...
    assert result is not None
        # 验证调用参数
        call_args = metadata_manager.session.put.call_args
            payload = call_args[1]["]json["]": assert payload["]name["] =="]minimal_job[" assert payload["]description["] =="]作业 minimal_job[" assert payload["]type["] =="]BATCH[" assert "]inputs[" not in payload[""""
    ...
    def test_create_job_stream_type(self, metadata_manager, mock_response):
        ...
        with patch.object(metadata_manager.session, 'put', return_value = mock_response)
            ...
            # 验证作业类型
            call_args = metadata_manager.session.put.call_args
            payload = call_args[1]["]json["]": assert payload["]type["] =="]STREAM[" def test_create_job_error("
    """"
        "]""测试创建作业错误处理"""
        error_response = Mock()
    ...
    assert "]]lineage[" in call_args[0][0]""""
        params = call_args[1]["]params["]": assert params["]nodeId["] =="]datasettest_namespacetest_dataset[" assert params["]depth["] =="]5[" def test_get_dataset_lineage_default_depth("
    """"
        "]""测试获取数据集血缘关系默认深度"""
        with patch.object(metadata_manager.session, 'get', return_value = mock_response)
            ...
            # 验证默认深度
            call_args = metadata_manager.session.get.call_args
            params = call_args[1]["]params["]": assert params["]depth["] =="]3[" def test_get_dataset_lineage_error("
    """"
        "]""测试获取数据集血缘关系错误处理"""
        error_response = Mock()
    ...
    assert "]search[" in call_args[0][0]""""
        params = call_args[1]["]params["]": assert params["]q["] =="]test_query[" assert params["]namespace["] =="]test_namespace[" assert params["]limit["] =="]50[" def test_search_datasets_minimal("
    """"
        "]""测试搜索数据集最小参数"""
        search_result = Mock()
    ...
    assert params["]]limit["] =="]10[" def test_search_datasets_error("
    """"
        "]""测试搜索数据集错误处理"""
        error_response = Mock()
        ...
        versions_result.json.return_value = {
            "versions[": [""""
            {"]version[: "v1"", "created_at]},""""
            {"version[: "v2"", "created_at]}""""
            ]
        }
    ...
    assert "]namespaces_test_namespace/jobs/test_job/runs[" in call_args[0][0]""""
        params = call_args[1]["]params["]": assert params["]limit["] ==100[" def test_get_job_runs_default_limit(self, metadata_manager, mock_response):"""
        "]]""测试获取作业运行历史默认限制"""
        runs_result = Mock()
        runs_result.json.return_value = {"runs[" []}": runs_result.raise_for_status.return_value = None[": with patch.object(metadata_manager.session, 'get', return_value = runs_result)": metadata_manager.get_job_runs("
            ...
            # 验证默认限制
            call_args = metadata_manager.session.get.call_args
            params = call_args[1]["]params["]": assert params["]limit["] ==20[" def test_get_job_runs_error(self, metadata_manager):"""
        "]]""测试获取作业运行历史错误处理"""
        error_response = Mock()
    ...
    def test_setup_football_metadata_success(self, metadata_manager):
        ...
        success_response.json.return_value = {"id[": ["]success["}": success_response.raise_for_status.return_value = None[": with patch.object(metadata_manager.session, 'put', return_value = success_response)""
            # 应该不抛出异常
    ...
    def test_setup_football_metadata_namespace_exists(self, metadata_manager):
        ...
        # Mock 命名空间已存在的情况
        error_response = Mock()
        error_response.raise_for_status.side_effect = RequestException("Already exists[")": success_response = Mock()": success_response.json.return_value = {"]id[": ["]success["}": success_response.raise_for_status.return_value = None[": with patch.object(metadata_manager.session, 'put') as mock_put:""
    ...
    assert manager.marquez_url =="]http//custom5000[" def test_url_construction_edge_cases("
    """"
        "]""测试URL构建边界情况"""
        from src.lineage.metadata_manager import MetadataManager
    ...
    """"
        ...
        manager = MetadataManager()
        # 验证session headers配置
    assert manager.session.headers["Content-Type["] =="]application_json[" assert manager.session.headers["]Accept["] =="]application/json[" def test_payload_construction_for_datasets("
    """"
        "]""测试数据集载荷构建"""
        schema_fields = [
        {"name[: "id"", "type]},""""
    ...
    assert schema_fields[0]["name["] =="]id[" assert schema_fields[0]["]description["] =="]Primary key[" assert schema_fields[1]["]name["] =="]name[" assert schema_fields[1]["]type["] =="]VARCHAR[" def test_payload_construction_for_jobs("
    """"
        "]""测试作业载荷构建"""
        input_datasets = [
        {"namespace[: "input_ns"", "name]}""""
        ...
        # 验证载荷构建逻辑
        expected_inputs = [{"namespace[: "input_ns"", "name[" "]input_ds]}]": expected_outputs = [{"namespace[: "output_ns"", "name]}]": assert len(input_datasets) ==1[" assert input_datasets[0]["]namespace["] =="]input_ns[" assert len(output_datasets) ==1[""""
    assert output_datasets[0]["]]namespace["] =="]output_ns[" def test_logging_functionality("
    """"
        "]""测试日志记录功能"""
        ...
        "]""测试错误日志记录"""
        with patch('src.lineage.metadata_manager.logger') as mock_logger = error_response Mock()
            error_response.raise_for_status.side_effect = RequestException("API Error[")": with patch.object(metadata_manager.session, 'put', return_value = error_response)": try:": pass"
 ...
 pass
    ...
    def test_marquez_url_processing(self):
        ...
        ("https://marquez.example.com[", "]https://marquez.example.com/api/v1/")]": for base_url, expected_api_url in test_cases = manager MetadataManager(marquez_url=base_url)": assert manager.api_url ==expected_api_url" from src.lineage.metadata_manager import MetadataManager"
        from src.lineage.metadata_manager import MetadataManager
        from src.lineage.metadata_manager import MetadataManager
        from src.lineage.metadata_manager import MetadataManager
        from src.lineage.metadata_manager import get_metadata_manager, _metadata_manager
