...
class TestMetricsCollectorBatchGamma009:
    ...
    def test_collector_initialization_complete(self, metrics_collector):
        ...
        assert metrics_collector.enabled
    ...
    async def test_start_success_with_signal_handler(self, metrics_collector):
        ...
        with patch("signal.signal[") as mock_signal, patch.object(:": metrics_collector, "]_collection_loop["""""
        ) as mock_loop:
            mock_loop.return_value = Mock()
            mock_task = Mock()
            mock_task.__aenter__ = Mock(return_value=mock_task)
            mock_task.__aexit__ = Mock(return_value=None)
            with patch("]asyncio.create_task[", return_value = mock_task)": await metrics_collector.start()"""
                # 验证信号处理器注册
                assert mock_signal.call_count ==2
                mock_signal.assert_any_call(
                    signal.SIGINT, metrics_collector._signal_handler
                )
                mock_signal.assert_any_call(
                    signal.SIGTERM, metrics_collector._signal_handler
                )
    @pytest.mark.asyncio
    async def test_start_signal_handler_value_error(self, metrics_collector):
        ...
        with patch(:
            "signal.signal[", side_effect=ValueError("]Non-main thread[")""""
        ), patch.object(metrics_collector, "]_collection_loop[") as mock_loop:": mock_loop.return_value = Mock()": mock_task = Mock()": mock_task.__aenter__ = Mock(return_value=mock_task)"
            mock_task.__aexit__ = Mock(return_value=None)
    ...
    async def test_collection_loop_basic(self, metrics_collector):
        ...
        with patch.object(:
            metrics_collector.metrics_exporter,
            "collect_all_metrics[",": new_callable=AsyncMock) as mock_collect:"""
            # 设置短间隔用于快速测试
    ...
    async def test_collection_loop_exception_handling(self, metrics_collector):
        ...
        with patch.object(:
            metrics_collector.metrics_exporter,
            "collect_all_metrics[",": side_effect=Exception("]Collection error[")), patch("]asyncio.sleep[") as mock_sleep:": metrics_collector.collection_interval = 0.1[": metrics_collector.running = True[""
            # 让循环运行一段时间后停止
    ...
    def test_collect_system_metrics_success(self, metrics_collector):
        "]]]""测试成功收集系统指标"""
        # Mock psutil import
        with patch("builtins.__import__[") as mock_import:": mock_psutil = Mock()": mock_psutil.cpu_percent.return_value = 50.0[": mock_memory = Mock()"
            ...
            def mock_import_func(name, *args):
                if name =="]]psutil[": return mock_psutil[": return __import__(name, *args)": mock_import.side_effect = mock_import_func[": result = metrics_collector.collect_system_metrics()"
            # 验证返回的指标
            assert result["]]]cpu_usage_percent["] ==50.0[" assert result["]]memory_usage_percent["] ==60.0[" assert result["]]disk_usage_percent["] ==30.0[" assert "]]timestamp[" in result[""""
    def test_collect_system_metrics_exception(self, metrics_collector):
        "]]""测试系统指标收集异常处理"""
        # Mock psutil import抛出异常
        with patch("builtins.__import__[") as mock_import:": def mock_import_func(name, *args):": if name =="]psutil[": raise ImportError("]No module named 'psutil': ")": return __import__(name, *args)": mock_import.side_effect = mock_import_func[": result = metrics_collector.collect_system_metrics()"
    ...
    def test_enable_disable_methods(self, metrics_collector):
        ...
        assert not metrics_collector.enabled
        # 启用
    ...
    def test_metrics_exporter_integration(self, metrics_collector):
        ...
        assert hasattr(metrics_collector, "metrics_exporter[")" assert metrics_collector.metrics_exporter is not None["""
    ...
    async def test_collect_once_success(self, metrics_collector):
        ...
        with patch.object(:
            metrics_collector.metrics_exporter,
            "collect_all_metrics[",": new_callable=AsyncMock) as mock_collect = result await metrics_collector.collect_once()"""
            # 验证结果结构
            assert result["]success["] is True[" assert "]]collection_time[" in result[""""
            assert "]]duration_seconds[" in result[""""
            assert result["]]message["] =="]指标收集成功[" assert isinstance(result["]duration_seconds["], float)""""
            # 验证mock方法被调用
    ...
    async def test_collect_once_failure(self, metrics_collector):
        ...
        with patch.object(:
            metrics_collector.metrics_exporter,
            "collect_all_metrics[",": side_effect=Exception("]Collection error[")):": result = await metrics_collector.collect_once()"""
            # 验证失败结果结构
            assert result["]success["] is False[" assert "]]collection_time[" in result[""""
            assert "]]error[" in result[""""
            assert result["]]message["] =="]指标收集失败[" assert result["]error["] =="]Collection error[" def test_collector_string_representation("
    """"
        "]""测试收集器的字符串表示"""
        ...
        # 验证基本属性可以访问
        assert hasattr(metrics_collector, "]]collection_interval[")" assert hasattr(metrics_collector, "]enabled[")" assert metrics_collector.collection_interval ==5["""
        assert metrics_collector.enabled
