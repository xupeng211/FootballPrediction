...
class TestMainApplicationMinimal:
    ...
    """"
        ...
        for middleware in test_app.user_middleware:
            if "CORSMiddleware[": in str(middleware.cls):": cors_middleware = middleware[": break[": assert cors_middleware is not None, "]]]CORS中间件未配置[" def test_root_endpoint_structure("
    """"
        "]""测试根路径基本结构"""
        ...
        @test_app.get("_[")": async def test_root():": return {""
            ...
            "]status[: "运行中[","]"""
            "]docs_url[": ["]_docs[",""""
                "]health_check[: "/health["}"]": from fastapi.testclient import TestClient[": with TestClient(test_app) as client = response client.get("]]/")": assert response.status_code ==200[" data = response.json()""
            assert data["]service["] =="]足球预测API[" assert data["]version["] =="]1.0.0[" assert data["]status["] =="]运行中[" assert data["]docs_url["] =="]/docs[" assert data["]health_check["] =="]/health[" def test_http_exception_handler_structure("
    """"
        "]""测试HTTP异常处理器基本结构"""
        # 创建模拟请求
 ...
 pass
    ...
    """"
        ...
        @health_router.get("_health[")": async def health_check():": return {"]status[": ["]healthy["}""""
            ...
            f["]路由 {route} 未注册["]: def test_exception_handler_registration_basic("
    """"
        "]""测试异常处理器注册（简化版）"""
        # 创建FastAPI应用并注册异常处理器
        test_app = FastAPI()
