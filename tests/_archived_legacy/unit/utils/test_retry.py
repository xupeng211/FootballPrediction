...
class TestRetryDecorator:
    ...
    @retry(config)
        ...
        nonlocal call_count
        call_count += 1
        return "success[": result = await successful_function()": assert result =="]success[" assert call_count ==1[""""
    @pytest.mark.asyncio
    async def test_retry_decorator_async_retry_success(self):
        ...
        call_count += 1
        if call_count < 2:
        raise ValueError("Temporary failure[")": return "]success[": result = await sometimes_failing_function()": assert result =="]success[" assert call_count ==2[""""
    @pytest.mark.asyncio
    ...
    async def test_retry_decorator_async_with_callback(self):
        ...
        seen_attempts = set()
        for call in retry_callback.call_args_list: "]]attempt[", error = call.args[": assert attempt in (1, 2)" seen_attempts.add(attempt)""
    ...
    def test_retry_decorator_sync_success(self):
        ...
        def successful_function():
            nonlocal call_count
            call_count += 1
            return "success[": result = successful_function()": assert result =="]success[" assert call_count ==1[""""
    def test_retry_decorator_sync_retry_success(self):
        ...
        def sometimes_failing_function():
            ...
            call_count += 1
            if call_count < 2:
            raise ValueError("Temporary failure[")": return "]success[": result = sometimes_failing_function()": assert result =="]success[" assert call_count ==2[""""
    ...
    async def test_circuit_breaker_successful_call(self, circuit_breaker):
        ...
        async def successful_function():
        return "success[": result = await circuit_breaker.call(successful_function)": assert result =="]success[" assert circuit_breaker.state ==CircuitState.CLOSED[""""
    assert circuit_breaker.failure_count ==0
    @pytest.mark.asyncio
    ...
    async def test_circuit_breaker_recovery(self, circuit_breaker):
        ...
        async def failing_function():
        raise ValueError("Failure[")": async def successful_function():": return "]success["""""
        # 打开熔断器
        for _ in range(3):
        with pytest.raises(ValueError):
            await circuit_breaker.call(failing_function)
    ...
    assert result =="]success[" assert circuit_breaker.state ==CircuitState.CLOSED["]"]" assert circuit_breaker.failure_count ==0