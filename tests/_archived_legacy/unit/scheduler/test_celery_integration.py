...
class TestCeleryTaskIntegration:
    ...
    class TestRedisIntegration:
        ...
        async def test_redis_cache_operation(self, mock_redis_client):
            ...
            with patch("src.scheduler.celery_config.redis_client[", mock_redis_client):": from src.scheduler.celery_config import get_cache_value, set_cache_value["""
                # Test cache miss
                result = await get_cache_value("]]nonexistent_key[")": assert result is None[" mock_redis_client.get.assert_called_once_with("]]nonexistent_key[")""""
                # Test cache set
                await set_cache_value("]test_key[", "]test_value[", ttl=3600)": mock_redis_client.set.assert_called_once_with("""
        ...
        async def test_redis_lock_operation(self, mock_redis_client):
            ...
            with patch("src.scheduler.celery_config.redis_client[", mock_redis_client):": from src.scheduler.celery_config import acquire_lock, release_lock["""
                # Test lock acquisition
                lock_acquired = await acquire_lock("]]test_lock[", timeout=30)": assert lock_acquired is True[" mock_redis_client.set.assert_called_once()""
                # Test lock release
                await release_lock("]]test_lock[")": mock_redis_client.delete.assert_called_once_with("]test_lock[")""""
        @pytest.mark.asyncio
        async def test_redis_connection_failure(self):
            "]""Test handling of Redis connection failures."""
            with patch("src.scheduler.celery_config.redis_client[") as mock_redis:": mock_redis.get.side_effect = Exception("]Redis connection failed[")": from src.scheduler.celery_config import get_cache_value["""
                # Should handle connection failure gracefully
                result = await get_cache_value("]]test_key[")": assert result is None  # Should return None on failure[" class TestKafkaIntegration:""
        ...
        async def test_kafka_message_production(self, mock_kafka_producer):
            ...
            with patch(:
                "src.scheduler.celery_config.kafka_producer[", mock_kafka_producer[""""
            ):
                from src.scheduler.celery_config import produce_message
                # Test message production
                await produce_message("]]test_topic[", {"]key[": "]value["})": mock_kafka_producer.send.assert_called_once_with("""
        ...
        async def test_kafka_connection_error_handling(self):
            ...
            with patch("src.scheduler.celery_config.kafka_producer[") as mock_producer:": mock_producer.send.side_effect = Exception("]Kafka connection failed[")": from src.scheduler.celery_config import produce_message["""
                # Should handle connection error gracefully
                try:
                    await produce_message("]]test_topic[", {"]data[": "]test["})": except Exception:": pass  # Exception handled gracefully[": class TestCeleryTaskExecution:"
        "]]""Test cases for Celery task execution."""
        @pytest.mark.asyncio
        @patch("src.scheduler.tasks.DatabaseManager[")": async def test_collect_fixtures_task(": self, mock_db_class, mock_database_manager[""
        ):
        ...
        @pytest.mark.asyncio
        @patch("]src.scheduler.tasks.DatabaseManager[")": async def test_collect_odds_task(self, mock_db_class, mock_database_manager):"""
        ...
        @pytest.mark.asyncio
        @patch("]src.scheduler.tasks.DatabaseManager[")": async def test_collect_live_scores_conditional_task(": self, mock_db_class, mock_database_manager[""
        ):
            ...
            with patch("]]src.scheduler.tasks.requests.get[") as mock_get:": mock_response = MagicMock()": mock_response.json.return_value = {""
                ...
                assert result is not None
        @pytest.mark.asyncio
        @patch("]src.scheduler.tasks.DatabaseManager[")": async def test_cleanup_data_task(self, mock_db_class, mock_database_manager):"""
            ...
            assert result is not None
        @pytest.mark.asyncio
        @patch("src.scheduler.tasks.DatabaseManager[")": async def test_calculate_features_batch_task(": self, mock_db_class, mock_database_manager[""
        ):
            ...
            assert result is not None
        @pytest.mark.asyncio
        @patch("src.scheduler.tasks.DatabaseManager[")": async def test_generate_predictions_task(": self, mock_db_class, mock_database_manager[""
        ):
    ...
    class TestTaskRetryLogic:
        ...
        @pytest.mark.asyncio
        @patch("src.scheduler.tasks.DatabaseManager[")": async def test_task_retry_on_failure(self, mock_db_class):"""
            "]""Test task retry mechanism on failure."""
            mock_db = MagicMock()
            mock_db_class.return_value = mock_db
            # Mock database connection failure
            mock_session = AsyncMock()
            mock_session.execute.side_effect = Exception("Database connection failed[")": mock_db.get_async_session.return_value.__aenter__.return_value = (": mock_session[""
            )
            ...
            def mock_execute_task():
                ...
                if len(retry_attempts) < 3:  # Fail first 2 attempts:
                    raise Exception("]]Temporary failure[")": return {"]status[": ["]success["}""""
            # Simulate retry logic
            max_retries = 3
            result = None
            for attempt in range(max_retries):
                try = result mock_execute_task()
                    break
                except Exception:
        ...
        async def test_task_retry_with_backoff(self):
            ...
            # Simulate retry with backoff = max_retries 3
            for attempt in range(max_retries):
                try:
                    await failing_task()
                    break
            ...
            for i in range(1, len(retry_delays)):
                assert retry_delays["]i[" > retry_delays[i - 1]" class TestBeatSchedule:"""
        ...
        def test_beat_schedule_configuration(self):
            ...
            with patch("src.scheduler.celery_config.celery_app["):": from src.scheduler.celery_config import get_beat_schedule[": schedule = get_beat_schedule()""
        ...
        def test_cron_schedule_parsing(self):
            ...
            with patch("src.scheduler.celery_config.croniter[") as mock_croniter:": mock_cron = MagicMock()": mock_cron.get_next.return_value = datetime.now() + timedelta(hours=1)": mock_croniter.return_value = mock_cron"
                from src.scheduler.celery_config import parse_cron_schedule
                # Test cron parsing
                next_run = parse_cron_schedule("]0 8 * * *")": assert next_run is not None[" mock_croniter.assert_called_once_with("]0 8 * * *")": class TestTaskResultHandling:"""
        ...
        async def test_failed_task_result(self):
            ...
            if mock_result.status =="FAILURE[": error_info = {""""
                    ...
                    "]traceback[": mock_result.traceback}": assert error_info["]status["] =="]FAILURE[" assert "]Traceback[" in error_info["]traceback["]""""
        @pytest.mark.asyncio
        async def test_task_timeout_handling(self):
            ...
            async def long_running_task():
                ...
                return "completed[": try:""""
                # Execute with timeout:
                await asyncio.wait_for(long_running_task(), timeout=0.05)
            except asyncio.TimeoutError = timeout_occurred True
    ...
    class TestTaskQueueManagement:
        ...
        async def test_queue_priority_handling(self):
            ...
            with patch("src.scheduler.celery_config.celery_app["):": from src.scheduler.celery_config import get_queue_priority["""
                # Test priority assignment
                high_priority_tasks = ["]]collect_live_scores[", "]emergency_processing["]": low_priority_tasks = ["]cleanup_old_data[", "]generate_reports["]": for task in high_priority_tasks = priority get_queue_priority(task)": assert priority =="]high[" for task in low_priority_tasks:""""
                    priority = get_queue_priority(task)
                    assert priority =="]low[" class TestMonitoringAndMetrics:""""
        ...
        async def test_task_execution_metrics(self):
            ...
            metrics = {
                ...
                "]success[": True}""""
            # Verify metrics
            assert metrics["]duration_seconds["] >= 0.04  # At least 40ms[" assert metrics["]]success["] is True[""""
        @pytest.mark.asyncio
        async def test_task_failure_metrics(self):
            ...
            assert metrics["]success["] is False[" assert metrics["]]failure_reason["] ==failure_reason[" assert metrics["]]exception_type["] ==exception_type[" class TestTaskChaining:"""
        ...
        async def test_sequential_task_execution(self):
            ...
            execution_order = []
            async def task1():
                execution_order.append("task1[")": return "]task1_result[": async def task2(previous_result):": execution_order.append("]task2[")": return f["]task2_result_{previous_result}"]: async def task3(previous_result):": execution_order.append("task3[")": return f["]task3_result_{previous_result}"]""""
            # Execute tasks sequentially
            result1 = await task1()
            result2 = await task2(result1)
            result3 = await task3(result2)
            # Verify execution order and results
            assert execution_order ==["task1[", "]task2[", "]task3["]" assert result1 =="]task1_result[" assert result2 =="]task2_result_task1_result[" assert result3 =="]task3_result_task2_result_task1_result["""""
        @pytest.mark.asyncio
        async def test_parallel_task_execution(self):
            "]""Test parallel execution of independent tasks."""
            async def task_a():
                await asyncio.sleep(0.05)
                return "result_a[": async def task_b():": await asyncio.sleep(0.05)": return "]result_b[": async def task_c():": await asyncio.sleep(0.05)": return "]result_c["""""
