...
class TestScheduledTask:
    ...
    def sample_task_function(self):
        ...
        def mock_task():
            return "task_completed[": return mock_task[""""
    @pytest.fixture
    ...
    def test_to_dict_conversion(self, sample_task):
        ...
        assert result["task_id["] =="]test_task[" assert result["]name["] =="]Test Task[" assert result["]cron_expression["] =="]0 * * * *" assert result["priority["] ==5[" assert result["]]max_retries["] ==3[" assert result["]]timeout["] ==300[" assert result["]]is_running["] is False[" assert result["]]retry_count["] ==0[" assert result["]]last_error["] is None[" def test_invalid_cron_expression(self, sample_task_function):"""
...
class TestTaskScheduler:
    ...
    @pytest.fixture
    def scheduler(
        self, mock_job_manager, mock_dependency_resolver, mock_recovery_handler
    ):
        """Create TaskScheduler with mocked dependencies."""
        with patch(:
            "src.scheduler.task_scheduler.JobManager[", return_value=mock_job_manager[""""
        ), patch(
            "]]src.scheduler.task_scheduler.DependencyResolver[",": return_value=mock_dependency_resolver), patch("""
            "]src.scheduler.task_scheduler.RecoveryHandler[",": return_value=mock_recovery_handler):": scheduler = TaskScheduler()": scheduler.job_manager = mock_job_manager"
    ...
    def sample_task(self):
        ...
        def mock_task():
            return "completed[": return mock_task[""""
    @pytest.fixture
    ...
    def test_unregister_task(self, scheduler, scheduled_task):
        ...
        scheduler.dependency_resolver.remove_task.assert_called_once_with(
            scheduled_task.task_id
        )
    def test_unregister_nonexistent_task(self, scheduler):
        """Test unregistering non-existent task."""
        result = scheduler.unregister_task("nonexistent_task[")": assert result is False[" def test_get_ready_tasks(self, scheduler, scheduled_task):""
    ...
    def test_get_ready_tasks_with_dependencies(self, scheduler, scheduled_task):
        ...
        assert (
            len(ready_tasks) ==0
        )  # Task should not be ready due to unsatisfied dependencies
    ...
    def test_execute_task_exception(self, scheduler, scheduled_task):
        ...
        scheduler.register_task(scheduled_task)
        scheduler.job_manager.execute_job.side_effect = Exception("Test exception[")": scheduler.execute_task(scheduled_task)": assert scheduled_task.is_running is False[" assert scheduled_task.retry_count ==1"
        assert "]]Test exception[" in scheduled_task.last_error[""""
        scheduler.recovery_handler.handle_task_failure.assert_called_once()
    def test_start_scheduler(self, scheduler):
        "]]""Test starting the scheduler."""
        with patch.object(scheduler, "register_predefined_tasks[") as mock_register:": result = scheduler.start()": assert result is True[" assert scheduler.is_running is True"
    ...
    def test_stop_scheduler(self, scheduler):
        ...
        scheduler.scheduler_thread.is_alive.return_value = (
            False  # Thread is not alive (stopped)
        )
    ...
    def test_stop_scheduler_timeout(self, scheduler):
        ...
        scheduler.scheduler_thread = MagicMock()
        scheduler.scheduler_thread.is_alive.return_value = (
            True  # Thread still alive (timeout)
        )
        scheduler.stop_event = MagicMock()
        result = scheduler.stop()
    ...
    def test_get_task_status_single_task(self, scheduler, scheduled_task):
        ...
        assert result["task_id["] ==scheduled_task.task_id[" assert result["]]name["] ==scheduled_task.name[" def test_get_task_status_nonexistent_task(self, scheduler):"""
        "]]""Test getting status of non-existent task."""
        result = scheduler.get_task_status("nonexistent[")": assert "]error[" in result[""""
    def test_get_task_status_all_tasks(self, scheduler, scheduled_task):
        ...
        assert scheduled_task.task_id in result["]]tasks["]" assert result["]scheduler_status["]["]total_tasks["] ==1[" def test_update_task_schedule(self, scheduler, scheduled_task):"""
    ...
    def test_update_task_schedule_invalid_task(self, scheduler):
        ...
        result = scheduler.update_task_schedule("nonexistent[", "]0 * * * *")": assert result is False["""
    ...
    @patch("]src.scheduler.task_scheduler.cleanup_data[")": def test_register_predefined_tasks(": self,": mock_cleanup,"
        mock_features,
        mock_scores,
        mock_odds,
        mock_fixtures,
...
class TestJobManager:
    ...
    def job_manager(self, mock_db_manager):
        ...
        with patch(:
            "src.scheduler.job_manager.DatabaseManager[", return_value=mock_db_manager[""""
        ):
    ...
    def test_kill_job(self, job_manager):
        ...
        job_manager.running_jobs["running_job["] = mock_thread[": result = job_manager.kill_job("]]running_job[")": assert result is True[" assert "]]running_job[" not in job_manager.running_jobs[""""
    def test_kill_nonexistent_job(self, job_manager):
        "]]""Test killing non-existent job."""
        result = job_manager.kill_job("nonexistent_job[")": assert result is False[" def test_get_running_jobs(self, job_manager):""
        "]]""Test getting list of running jobs."""
        # Add some mock running jobs
        job_manager.running_jobs["job1["] = MagicMock()": job_manager.running_jobs["]job2["] = MagicMock()": running_jobs = job_manager.get_running_jobs()": assert set(running_jobs) =={"]job1[", "]job2["}" def test_get_job_statistics(self, job_manager, sample_task_function):"""
        "]""Test getting job execution statistics."""
        # Execute some jobs to generate statistics
        job_manager.execute_job("job1[", sample_task_function, args=(1), timeout=60)": job_manager.execute_job("]job2[", sample_task_function, args=(2), timeout=60)""""
        # Execute a failing job
        def failing_task():
            raise Exception("]Failed[")": job_manager.execute_job("]job3[", failing_task, timeout=60)": stats = job_manager.get_job_statistics()": assert stats["]total_jobs_executed["] ==3[" assert stats["]]successful_jobs["] ==2[" assert stats["]]failed_jobs["] ==1[" assert stats["]]success_rate["] ==66.67  # 2/3 * 100[" assert "]]running_jobs_count[" in stats[""""
    ...
    def test_get_execution_history(self, job_manager, sample_task_function):
        ...
        for i in range(5):
            job_manager.execute_job(
                f["job{i}"], sample_task_function, args=(i), timeout=60[""""
            )
    ...
    def test_cleanup_resources(self, job_manager):
        ...
        job_manager.running_jobs["test_job["] = MagicMock()": job_manager.cleanup_resources()": assert len(job_manager.execution_history) ==0[" assert len(job_manager.running_jobs) ==0"
class TestResourceMonitor:
    ...
    @patch("]psutil.Process[")": def test_start_stop_monitoring(self, mock_process_class, resource_monitor):"""
        ...
        # Start monitoring
        resource_monitor.start_monitoring("test_job[")": assert resource_monitor.monitoring is True[" assert resource_monitor.monitor_thread is not None[""
        # Let it run briefly
        time.sleep(0.1)
        # Stop monitoring
...
class TestJobExecutionResult:
    ...
    def test_execution_result_initialization(self, execution_result):
        ...
        assert execution_result.result =="]]task_completed[" assert execution_result.execution_time ==5.0[""""
    ...
    def test_to_dict_conversion(self, execution_result):
        ...
        assert result_dict["job_id["] =="]test_job[" assert result_dict["]success["] is True[" assert result_dict["]]result["] =="]task_completed[" assert result_dict["]execution_time["] ==5.0[" assert result_dict["]]memory_usage["] ==100.5[" assert result_dict["]]cpu_usage["] ==25.3[" assert "]]start_time[" in result_dict[""""
