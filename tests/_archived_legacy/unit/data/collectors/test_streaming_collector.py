...
class TestStreamingDataCollector:
    ...
    def test_init_with_custom_stream_config(self):
        ...
        with patch(:
            "src.data.collectors.streaming_collector.FootballKafkaProducer["""""
        ) as mock_producer = mock_config Mock()
            mock_producer.return_value = Mock()
            collector = StreamingDataCollector(
                enable_streaming=True, stream_config=mock_config
            )
            assert collector.stream_config ==mock_config
    @patch("]src.data.collectors.streaming_collector.FootballKafkaProducer[")": def test_initialize_kafka_producer_success(self, mock_producer):"""
        "]""测试Kafka生产者初始化成功"""
        mock_instance = Mock()
    ...
    @patch("src.data.collectors.streaming_collector.FootballKafkaProducer[")": def test_initialize_kafka_producer_failure(self, mock_producer):"""
        "]""测试Kafka生产者初始化失败"""
        mock_producer.side_effect = Exception("连接失败[")": collector = StreamingDataCollector(enable_streaming=False)": collector._initialize_kafka_producer()": assert collector.kafka_producer is None"
        assert collector.enable_streaming is False
    @pytest.mark.asyncio
    async def test_send_to_stream_success(self):
        "]""测试发送数据到流成功"""
        mock_producer = AsyncMock()
        mock_producer.send_batch.return_value = {"success[": 5, "]failed[": 0}": self.collector.kafka_producer = mock_producer[": data_list = [{"]]id[": 1}, {"]id[": 2}, {"]id[": 3}, {"]id[": 4}, {"]id[": 5}]": result = await self.collector._send_to_stream(data_list, "]test_stream[")": assert result["]success["] ==5[" assert result["]]failed["] ==0[" mock_producer.send_batch.assert_called_once_with(data_list, "]]test_stream[")""""
    @pytest.mark.asyncio
    async def test_send_to_stream_failure(self):
        "]""测试发送数据到流失败"""
        mock_producer = AsyncMock()
        mock_producer.send_batch.side_effect = Exception("发送失败[")": self.collector.kafka_producer = mock_producer[": data_list = [{"]]id[": 1}, {"]id[": 2}]": result = await self.collector._send_to_stream(data_list, "]test_stream[")": assert result["]success["] ==0[" assert result["]]failed["] ==2[""""
    @pytest.mark.asyncio
    async def test_send_to_stream_disabled(self):
        "]]""测试流式处理禁用时发送数据"""
        self.collector.enable_streaming = False
        data_list = [{"id[": 1}]": result = await self.collector._send_to_stream(data_list, "]test_stream[")": assert result["]success["] ==0[" assert result["]]failed["] ==0[""""
    @pytest.mark.asyncio
    ...
    async def test_send_to_stream_public_interface_match_data(self):
        """测试公共接口发送比赛数据"""
        mock_producer = AsyncMock()
        mock_producer.send_batch.return_value = {"success[": 1, "]failed[": 0}": self.collector.kafka_producer = mock_producer[": data = [{"]]match_id[": 123, "]home_team[": "]Team A[", "]away_team[": "]Team B["}]": result = await self.collector.send_to_stream(data)": assert result is True[" mock_producer.send_batch.assert_called_once_with(data, "]]match[")""""
    @pytest.mark.asyncio
    async def test_send_to_stream_public_interface_odds_data(self):
        "]""测试公共接口发送赔率数据"""
        mock_producer = AsyncMock()
        mock_producer.send_batch.return_value = {"success[": 1, "]failed[": 0}": self.collector.kafka_producer = mock_producer[": data = [{"]]odds[": 1.5, "]bookmaker[": "]Bet365["}]": result = await self.collector.send_to_stream(data)": assert result is True[" mock_producer.send_batch.assert_called_once_with(data, "]]odds[")""""
    @pytest.mark.asyncio
    async def test_send_to_stream_public_interface_scores_data(self):
        "]""测试公共接口发送比分数据"""
        mock_producer = AsyncMock()
        mock_producer.send_batch.return_value = {"success[": 1, "]failed[": 0}": self.collector.kafka_producer = mock_producer[": data = [{"]]score[: "2-1"", "minute["] True}]": result = await self.collector.send_to_stream(data)": assert result is True[" mock_producer.send_batch.assert_called_once_with(data, "]]scores[")""""
    @pytest.mark.asyncio
    ...
    async def test_collect_live_scores_with_streaming_success(self):
        ...
        mock_result = CollectionResult(
            ...
            mock_producer = AsyncMock()
            mock_producer.send_batch.return_value = {"]]success[": 2, "]failed[": 0}": self.collector.kafka_producer = mock_producer[": result = await self.collector.collect_live_scores_with_streaming()": assert result.status =="]]success[" assert "]流处理[" in result.error_message[""""
    @pytest.mark.asyncio
        ...
        mock_result = CollectionResult(
            ...
            with patch.object(:
                ...
                configs = [
                    ...
                    {"type[: "odds"", "kwargs]: {}}]": result = await self.collector.batch_collect_and_stream(configs)": assert result["total_collections["] ==2[" assert result["]]successful_collections["] ==2[" assert result["]]failed_collections["] ==0[" assert result["]]total_stream_success["] ==2[" assert result["]]total_stream_failed["] ==0[""""
    @pytest.mark.asyncio
        ...
        mock_success_result = CollectionResult(
            ...
            "]collect_fixtures_with_streaming[",": return_value=mock_success_result):": with patch.object(:": self.collector,"
                "]collect_odds_with_streaming[",": side_effect=Exception("]采集失败[")):": configs = ["""
                    {"]type[: "fixtures"", "kwargs]: {}},""""
                    {"type[: "odds"", "kwargs]: {}}]": result = await self.collector.batch_collect_and_stream(configs)": assert result["total_collections["] ==2[" assert result["]]successful_collections["] ==1[" assert result["]]failed_collections["] ==1[""""
    @pytest.mark.asyncio
    async def test_batch_collect_unknown_type(self):
        "]]""测试批量采集未知类型"""
        configs = [{"type[: "unknown"", "kwargs] {}}]": result = await self.collector.batch_collect_and_stream(configs)": assert result["total_collections["] ==1[" assert result["]]successful_collections["] ==0[" assert result["]]failed_collections["] ==0[" def test_toggle_streaming_enable(self):"""
        "]]""测试启用流式处理"""
        self.collector.enable_streaming = False
    ...
    def test_get_streaming_status_enabled(self):
        """测试获取流式处理状态（启用）"""
        mock_config = Mock()
        mock_config.kafka_config.bootstrap_servers = ["localhost9092["]": mock_config.topics = {"]match[: "match_topic"", "odds]}": self.collector.stream_config = mock_config[": status = self.collector.get_streaming_status()": assert status["]streaming_enabled["] is True[" assert status["]]kafka_producer_initialized["] is True[" assert status["]]stream_config["]["]bootstrap_servers["] ==["]localhost9092["]" assert "]match[" in status["]stream_config["]["]topics["]": def test_get_streaming_status_disabled(self):"""
        "]""测试获取流式处理状态（禁用）"""
        collector = StreamingDataCollector(enable_streaming=False)
        status = collector.get_streaming_status()
        assert status["streaming_enabled["] is False[" assert status["]]kafka_producer_initialized["] is False[" def test_close(self):"""
        "]]""测试关闭采集器"""
        mock_producer = Mock()
    ...
    def test_del(self):
        ...
        mock_producer = Mock()
        self.collector.kafka_producer = mock_producer
        with patch.object(self.collector, "close[") as mock_close:": self.collector.__del__()": mock_close.assert_called_once()""
    @pytest.mark.asyncio
...
class TestStreamingDataCollectorEdgeCases:
    ...
    def setup_method(self):
        """设置测试"""
        with patch("src.data.collectors.streaming_collector.StreamConfig["):": with patch("]src.data.collectors.streaming_collector.FootballKafkaProducer["):": self.collector = StreamingDataCollector(enable_streaming=True)"""
    @pytest.mark.asyncio
    async def test_send_to_stream_no_producer(self):
        "]""测试没有生产者时发送数据"""
        self.collector.kafka_producer = None
        result = await self.collector.send_to_stream([{"data[": 1}])": assert result is True["""
    @pytest.mark.asyncio
