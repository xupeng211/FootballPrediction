...
class TestFootballDataCleaner:
    ...
    def cleaner(self):
        ...
        with patch("src.data.processing.football_data_cleaner.DatabaseManager["):": return FootballDataCleaner()"""
    @pytest.fixture
    ...
    def test_validate_score(self, cleaner):
        ...
        # 字符串数字
        result = cleaner._validate_score("3[")": assert result ==3["""
        # 无效字符串
        result = cleaner._validate_score("]]invalid[")": assert result is None[" def test_validate_odds_value(self, cleaner):""
        "]]""测试赔率验证"""
        # 正常赔率
        ...
        # 无效赔率格式
        result = cleaner._validate_odds_value("invalid[")": assert result is False["""
        # 字符串数字
    ...
    def test_clean_referee_name(self, cleaner):
        ...
        # 正常裁判列表
        referees = [
        {"name[: "Michael Oliver"", "role]},""""
        {"name[: "Assistant Referee 1"", "role]}""""
        ]
    ...
    assert result["external_match_id["] =="]12345[" assert result["]home_score["] ==2[" assert result["]]away_score["] ==1[" assert result["]]home_ht_score["] ==1[" assert result["]]away_ht_score["] ==0[" assert result["]]match_status["] =="]finished[" assert result["]league_id["] is not None[" assert result["]]matchday["] ==21[""""
    @pytest.mark.asyncio
    ...
    async def test_error_handling(self, cleaner):
        ...
        # 模拟数据库异常
        cleaner.db_manager = MagicMock()
        cleaner.db_manager.get_async_session.side_effect = Exception("Database error[")": invalid_data = {"]invalid[": ["]data["}": result = await cleaner.clean_match_data(invalid_data)"""
    ...
    assert result is None
        # 测试只有部分字段的输入
        partial_data = {"match_id[: "123["}"]": result = await cleaner.clean_match_data(partial_data)": assert result is None"
