...
class TestDataProcessingService:
    ...
    def setup_method(self):
        ...
        "]utcDate[: "2024-01-15T15:00:00Z[","]"""
            ...
            "]status[": ["]FINISHED[",""""
            "]competition[": {"]id[": "]PL[", "]name[": "]Premier League["}}": self.sample_odds_data = ["""
            {
            "]match_id[: "123456[","]"""
            "]bookmaker[": ["]Bet365[",""""
            "]market_type[": ["]h2h[",""""
    ...
    async def test_process_raw_match_data_success(self):
        ...
        "]match_time[: "2024-01-15T15:00:00+00:00[","]"""
            ...
            "]away_score[": 1,""""
            "]status[": ["]FINISHED[",""""
            "]league_id[": 1}": self.service.data_cleaner.clean_match_data.return_value = cleaned_data[": self.service.missing_handler.handle_missing_match_data.return_value = (": cleaned_data"
        )
        result = await self.service.process_raw_match_data(self.sample_match_data)
        assert result is not None
        assert result["]]external_match_id["] =="]123456[" self.service.data_cleaner.clean_match_data.assert_called_once_with(""""
            self.sample_match_data
        )
    ...
    async def test_process_raw_match_data_cleaning_failed(self):
        ...
        # 模拟初始化的服务
        self.service.data_cleaner = AsyncMock()
        self.service.data_cleaner.clean_match_data.side_effect = ValueError("清洗失败[")": await self.service.process_raw_match_data(self.sample_match_data)": assert result is None[""
    @pytest.mark.asyncio
    async def test_process_raw_odds_data_success(self):
        ...
        "]market_type[": ["]h2h[",""""
                ...
                "]last_update[: "2024-01-15T14:00:00+00:00["},"]"""
            {
                ...
                "]outcome_name[: "2[","]"""
                "]outcome_price[": 2.80,""""
                "]last_update[: "2024-01-15T14:00:00+00:00["}]"]": self.service.data_cleaner.clean_odds_data.return_value = cleaned_odds[": result = await self.service.process_raw_odds_data(self.sample_odds_data)"
        assert result is not None
        assert len(result) ==3
    ...
    async def test_validate_data_quality_match_success(self):
        ...
        "]match_time[: "2024-01-15T15:00:00+00:00[","]"""
            ...
            "]away_score[": 1,""""
            "]status[": ["]FINISHED[",""""
            "]league_id[": 1}": await self.service.validate_data_quality(valid_match_data, "]match[")": assert result["]is_valid["] is True[""""
    @pytest.mark.asyncio
    async def test_validate_data_quality_match_missing_fields(self):
        ...
        await self.service.validate_data_quality(invalid_match_data, "]match[")": assert result["]is_valid["] is False[""""
    @pytest.mark.asyncio
    async def test_validate_data_quality_negative_scores(self):
        ...
        "]match_time[: "2024-01-15T15:00:00+00:00[","]"""
            ...
            "]]away_score[": 1,""""
            "]status[": ["]FINISHED[",""""
            "]league_id[": 1}": await self.service.validate_data_quality(invalid_match_data, "]match[")": assert result["]is_valid["] is False[""""
    @pytest.mark.asyncio
    async def test_validate_data_quality_odds_success(self):
        ...
        await self.service.validate_data_quality(valid_odds_data, "]]odds[")": assert result["]is_valid["] is True[""""
    @pytest.mark.asyncio
    async def test_validate_data_quality_invalid_odds_price(self):
        ...
        await self.service.validate_data_quality(invalid_odds_data, "]]odds[")": assert result["]is_valid["] is False[" class TestFootballDataCleaner:"""
    "]]""足球数据清洗器测试类"""
    def setup_method(self):
        """设置测试环境"""
    ...
    async def test_clean_match_data_success(self):
        "]""测试成功清洗比赛数据"""
        await self.cleaner.clean_match_data(self.sample_match_data)
    assert result is not None
    ...
    async def test_clean_match_data_invalid_input(self):
        "]""测试无效输入数据的清洗"""
        invalid_data = {"invalid[" "]data["}": try = result await self.cleaner.clean_match_data(invalid_data)": assert result is None or result =={}" except (ValueError, KeyError):"
            # 清洗器应该抛出适当的异常
            pass
    @pytest.mark.asyncio
    async def test_clean_odds_data_success(self):
        ...
        assert len(result) >= 1
        # 检查第一个结果的结构
        if result = first_result result[0]
            assert "]external_match_id[" in first_result or "]match_id[": in first_result[": assert "]]bookmaker[" in first_result[""""
    @pytest.mark.asyncio
    async def test_clean_odds_data_invalid_price(self):
        ...
        if result is not None:
            # 如果返回结果，检查无效价格是否被过滤
            assert all(odds["]outcome_price["] > 0 for odds in result)" class TestMissingDataHandler:"""
    "]""缺失数据处理器测试类"""
    def setup_method(self):
        """设置测试环境"""
    ...
    async def test_handle_missing_match_data_with_missing_scores(self):
        ...
        "]match_time[: "2024-01-15T15:00:00+00:00[","]"""
            ...
            "]]league_id[": 1}": await self.handler.handle_missing_match_data(match_data)": assert result is not None[""
    ...
    async def test_handle_missing_match_data_no_missing_values(self):
        ...
        "]match_time[: "2024-01-15T15:00:00+00:00[","]"""
            ...
            "]away_score[": 1,""""
            "]status[": ["]FINISHED[",""""
            "]league_id[": 1}": await self.handler.handle_missing_match_data(complete_match_data)": assert result is not None[" assert result ==complete_match_data  # 数据应该保持不变"
    @pytest.mark.asyncio
    async def test_handle_missing_features_with_historical_average(self):
        "]]""测试使用历史平均值处理缺失特征"""
        # 使用实际存在的方法名
        await self.handler.handle_missing_match_data(
        {
        "team_id[": 1,""""
        "]goals_scored[": None,""""
