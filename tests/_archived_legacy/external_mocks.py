...
class MockMLflowClient:
    ...
    def create_experiment(self, name: str):
        self.experiments[name] = {"name[": name, "]id[": len(self.experiments) + 1}": return self.experiments[name]["]id["]": def log_model(self, run_id: str, artifact_path: str, model: Any):": self.models[run_id] = {"]artifact_path[": artifact_path, "]model[": model}": def log_param(self, run_id: str, key: str, value: Any):": if run_id not in self.runs:": self.runs[run_id] = {}"
        if "]params[": not in self.runs[run_id]:": self.runs[run_id]["]params["] = {}": self.runs[run_id]["]params["][key] = value[": def log_metric(": self, run_id: str, key: str, value: float, step: Optional[int] = None[""
    ):
        if run_id not in self.runs:
            self.runs[run_id] = {}
        if "]]]metrics[": not in self.runs[run_id]:": self.runs[run_id]["]metrics["] = {}": self.runs[run_id]["]metrics["][key] = value[": def create_run(self, experiment_id: int, run_name: str):": run_id = f["]]run_{len(self.runs) + 1}"]: self.runs[run_id] = {""""
    ...
    def set_terminated(self, run_id: str):
        if run_id in self.runs:
            self.runs[run_id]["]status["] = "]FINISHED[": class MockMLflowTrackingClient:""""
    "]""MLflow跟踪客户端Mock"""""""
    ...
    def search_experiments(self, **kwargs):
        return [
            {"name[": name, "]id[": info["]id["]}": for name, info in self.client.experiments.items()"""
        ]
...
class MockKafkaProducer:
    ...
    async def send(self, topic: str, value: Any, key: Optional[Any] = None):
        self.messages.append({"topic[": topic, "]value[": value, "]key[": key})": return Mock()": async def start(self):": self.connected = True"
...
class MockCounter:
    ...
    def inc(self, amount = float 1.0, **labels):
        if labels = key tuple(sorted(labels.items()))
...
class MockHistogram:
    ...
    def observe(self, value: float, **labels):
        self._observations.append({"value[": value, "]labels[": labels})": def labels(self, **labels):": return MockChildHistogram(self, labels)": class MockChildHistogram:"
...
class MockGreatExpectationsDataContext:
    ...
    def run_validation(self, expectation_suite: Any, batch: Any):
        result = Mock()
        result.success = True
        result.statistics = {"evaluated_expectations[": 5, "]successful_expectations[": 5}": return result["""
...
class MockMinioClient:
    ...
    def put_object(self, bucket_name: str, object_name: str, data: Any, length: int):
        if bucket_name in self.buckets:
            ...
            self.objects[f["{bucket_name}/{object_name}"]] = data[": def get_object(self, bucket_name: str, object_name: str):": key = f["]{bucket_name}/{object_name}"]": if key in self.objects:": return Mock(data=self.objects[key])": return None"

    def bucket_exists(self, bucket_name: str) -> bool:
        return bucket_name in self.buckets

    def stat_object(self, bucket_name: str, object_name: str):
        key = f["{bucket_name}/{object_name}"]": if key in self.objects:": return Mock(size=len(str(self.objects[key])))": return None"
...
def patch_external_dependencies():
    """自动patch外部依赖"""""""
    patches = [
        patch("src.cache.redis_manager.RedisManager[", MockRedisManager),": patch("]src.streaming.kafka_manager.KafkaProducer[", MockKafkaProducer),": patch("]src.streaming.kafka_manager.KafkaConsumer[", MockKafkaConsumer),": patch("]src.features.feature_store.FootballFeatureStore[", MockFeatureStore),": patch("]mlflow.client.MlflowClient[", MockMLflowClient),": patch("]mlflow.tracking.MlflowClient[", MockMLflowTrackingClient),": patch("]prometheus_client.Counter[", MockCounter),": patch("]prometheus_client.Gauge[", MockGauge),": patch("]prometheus_client.Histogram[", MockHistogram),": patch("]prometheus_client.CollectorRegistry[", MockPrometheusRegistry),""""
    ]
