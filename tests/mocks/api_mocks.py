...
class MockAPIClient:
    ...
    async def post(self, endpoint: str, data: Dict = None, json_data: Dict = None, headers: Dict = None) -> Dict:
        """Mock POST request."""
        return await self._make_request("POST", endpoint, data=data, json_data=json_data, headers=headers)

    async def put(self, endpoint: str, data: Dict = None, json_data: Dict = None, headers: Dict = None) -> Dict:
        """Mock PUT request."""
    ...
    async def patch(self, endpoint: str, data: Dict = None, json_data: Dict = None, headers: Dict = None) -> Dict:
        """Mock PATCH request."""
        return await self._make_request("PATCH", endpoint, data=data, json_data=json_data, headers=headers)
    ...
    async def _make_request(self, method: str, endpoint: str, **kwargs) -> Dict:
        ...
        if endpoint in self.rate_limits:
            last_request = self.rate_limits[endpoint]
    ...
    def set_error_response(self, method: str, endpoint: str, error: str, status_code: int = 500):
        """Set an error response for a specific endpoint."""
        self.set_response(method, endpoint, {
            "status": "error",
            "error": error,
...
class MockWeatherAPI(MockAPIClient):
    ...
    def _setup_default_responses(self):
        ...
        self.set_response("GET", "/forecast", {
            "status": "success",
            "data": {
                "forecast": [
                    {
...
class MockNotificationAPI(MockAPIClient):
    ...
    def _setup_default_responses(self):
        ...
        self.set_response("POST", "/send", {
            "status": "success",
            "message": "Notification sent successfully",
            "notification_id": f"notif_{int(time.time())}"
        })

    async def send_notification(self, recipient: str, message: str, notification_type: str = "email") -> Dict:
        """Send a notification."""
        notification = {
            "recipient": recipient,
            "message": message,
...
class MockAnalyticsAPI(MockAPIClient):
    ...
    def _setup_default_responses(self):
        ...
        self.set_response("POST", "/track", {
            "status": "success",
            "message": "Event tracked successfully"
        })

        self.set_response("GET", "/metrics", {
            "status": "success",
            "data": {
                "page_views": 1250,
                "unique_visitors": 450,
...
class MockWebSocketClient:
    ...
    async def receive(self) -> Dict:
        ...
        await asyncio.sleep(0.01)  # Simulate waiting for message
        return {"type": "update", "data": {"timestamp": time.time()}}
    ...
    def simulate_message(self, message: Dict):
        ...
        if self.connected:
            ...
            if message_type in self.handlers:
                if asyncio.iscoroutinefunction(self.handlers[message_type]):
                    asyncio.create_task(self.handlers[message_type](message))
                else:
...
class MockRateLimiter:
    ...
    async def check_rate_limit(self, user_id: str) -> bool:
        ...
        # Count user requests
        user_requests = len([req for req in self.requests if req["user_id"] == user_id])
