...
class MockDataLakeStorage:
    ...
    async def upload_file(self, path: str, data: Union[str, bytes], metadata: Dict = None) -> bool:
        ...
        self.metadata[path]["uploaded_at"] = time.time()
        self.metadata[path]["size"] = len(data)
    ...
    async def download_text(self, path: str) -> Optional[str]:
    ...
    async def list_files(self, prefix: str = "", recursive: bool = True) -> List[str]:
        ...
        if self.should_fail:
            raise Exception("Storage listing failed")
    ...
    async def copy_file(self, source_path: str, dest_path: str) -> bool:
        ...
        if source_path in self.storage:
            self.storage[dest_path] = self.storage[source_path]
            self.metadata[dest_path] = self.metadata[source_path].copy()
            self.metadata[dest_path]["copied_from"] = source_path
            self.metadata[dest_path]["copied_at"] = time.time()
            return True
        return False
    ...
    async def create_directory(self, path: str) -> bool:
        ...
        self.storage[path] = b""
        self.metadata[path] = {"type": "directory", "created_at": time.time()}
        return True
    ...
    async def upload_directory(self, local_path: str, remote_path: str) -> bool:
        ...
        for i in range(3):  # Simulate 3 files
            file_path = f"{remote_path}/file_{i}.txt"
            await self.upload_file(file_path, f"Content of file {i}")
    ...
    async def download_directory(self, remote_path: str, local_path: str) -> bool:
        ...
        if self.should_fail:
            raise Exception("Directory download failed")
    ...
    async def get_storage_stats(self) -> Dict[str, Any]:
        ...
        file_count = len([path for path in self.metadata.keys() if not path.endswith('/')])

        return {
            "total_files": file_count,
            "total_size": total_size,
            "total_directories": len([path for path in self.metadata.keys() if path.endswith('/')]),
            **self.stats
        }
    ...
    def set_file_content(self, path: str, content: Union[str, bytes], metadata: Dict = None):
        ...
        self.metadata[path] = metadata or {}
        self.metadata[path]["size"] = len(content)
        self.metadata[path]["created_at"] = time.time()

    def simulate_file_corruption(self, path: str):
...
class MockObjectStorage:
    ...
    async def upload_model(self, model_name: str, version: str, model_data: bytes, metadata: Dict = None) -> bool:
        """Mock model upload."""
        if self.should_fail:
        ...
        self.objects[key] = model_data
        self.versions.setdefault(model_name, []).append(version)

        if metadata:
            self.tags[key] = metadata
    ...
    def set_model_version(self, model_name: str, version: str, data: bytes):
