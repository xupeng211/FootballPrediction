...
class MockAsyncSession:
    ...
    async def execute(self, query, params=None):
        ...
        mock_result.scalar.return_value = self._execute_results.get("scalar", None)
        mock_result.fetchall.return_value = self._execute_results.get("fetchall", [])
        mock_result.first.return_value = self._execute_results.get("first", None)
        return mock_result
    ...
    def query(self, model):
        ...
        mock_query.all.return_value = self._query_results.get("all", [])
        mock_query.first.return_value = self._query_results.get("first", None)
        mock_query.count.return_value = self._query_results.get("count", 0)
        return mock_query
...
class MockDatabaseManager:
    ...
    async def execute_query(self, query: str, params: Dict = None) -> List[Dict]:
        ...
        return [{"id": 1, "result": "mock_result"}]
    ...
    async def execute_many(self, query: str, params_list: List[Dict]) -> List[Dict]:
        ...
        await asyncio.sleep(0.05)  # Simulate batch execution time
        return [{"id": i, "result": f"mock_result_{i}"} for i in range(len(params_list))]
    ...
    def set_current_session_result(self, key: str, result: Any):
        ...
        if self.current_session:
            if key.startswith("query_"):
                self.current_session.set_query_result(key[6:], result)
            elif key.startswith("execute_"):
                self.current_session.set_execute_result(key[8:], result)
