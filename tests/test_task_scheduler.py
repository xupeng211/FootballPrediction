...
class TestCeleryAppConfiguration:
    ...
    @patch("src.tasks.celery_app.Celery")
    def test_celery_app_creation(self, mock_celery_class):
        """
        测试Celery应用创建和基本配置
        ...
        # 创建mock的Celery应用实例
        mock_app = MagicMock()
        mock_app.main = "football_prediction_tasks"
    ...
    def test_worker_configuration(self, mock_celery_app):
        ...
        mock_conf.accept_content = ["json"]
        mock_conf.task_serializer = "json"
...
class TestDataCollectionTasks:
    ...
    def test_collect_fixtures_task_success(self, mock_db_manager, mock_error_logger):
        ...
        with patch(
            ...
            mock_result.records_collected = 50
            mock_result.success_count = 45
            mock_result.error_count = 5

            mock_collector.collect_fixtures.return_value = mock_result
    ...
    def test_collect_odds_task_success(self, mock_db_manager, mock_error_logger):
        ...
        with patch(
            ...
            mock_result = MagicMock()
            mock_result.status = "success"
            mock_result.records_collected = 100
            mock_result.success_count = 95
            mock_result.error_count = 5

            mock_collector.collect_odds.return_value = mock_result
            mock_collector_class.return_value = mock_collector
    ...
    def test_collect_scores_task_success(self, mock_db_manager, mock_error_logger):
        ...
        with patch(
            ...
            mock_result = MagicMock()
            mock_result.status = "success"
            mock_result.records_collected = 15
            mock_result.success_count = 12
            mock_result.error_count = 3

            mock_collector.collect_live_scores.return_value = mock_result
            mock_collector_class.return_value = mock_collector
...
class TestErrorLogger:
    ...
    async def test_log_data_collection_error(self, error_logger):
        ...
        with patch("src.tasks.error_logger.DataCollectionLog") as mock_log_class:
    ...
    async def test_get_error_statistics(self, error_logger):
        ...
        async def mock_session_context():
            ...
            mock_result_tasks = [
                MagicMock(task_name="collect_odds_task", error_count=5),
                MagicMock(task_name="collect_scores_task", error_count=2),
            ]

            mock_result_types = [
                MagicMock(error_type="API_FAILURE", error_count=4),
                MagicMock(error_type="DATABASE_ERROR", error_count=3),
            ]
        ...
        # Mock查询构建器
        with patch.object(error_logger, "_get_query_builder") as mock_builder_getter:
            mock_builder = MagicMock()
            mock_builder.build_error_statistics_query.return_value = (
    ...
    async def test_cleanup_old_logs(self, error_logger):
        ...
        with patch.object(error_logger, "_get_query_builder") as mock_builder_getter:
            mock_builder = MagicMock()
            mock_builder.build_cleanup_old_logs_query.return_value = (
...
class TestMaintenanceTasks:
    ...
    def test_system_health_check_task(self, mock_db_manager):
        ...
        with patch("redis.Redis") as mock_redis_class:
            ...
            mock_redis.ping.return_value = True  # 模拟Redis服务正常
...
class TestTaskMonitoring:
    ...
    def test_update_queue_size(self, task_monitor):
        ...
        task_monitor.update_queue_size("fixtures", 5)
        task_monitor.update_queue_size("odds", 12)
        task_monitor.update_queue_size("scores", 3)
    ...
    async def test_calculate_error_rates(self, task_monitor):
        """测试计算错误率"""
        # 正确Mock数据库管理器以支持异步操作
        with patch("src.tasks.monitoring.DatabaseManager") as mock_db_manager_class:
    ...
    async def test_check_task_health(self, task_monitor):
        ...
        with patch.object(
            ...
            # 设置Mock返回值
            mock_error_rates.return_value = {"collect_odds_task": 0.05}  # 5%错误率
            mock_queue_sizes.return_value = {
                "fixtures": 5,
                "odds": 12,
            }  # 少于100的队列大小
            mock_task_delays.return_value = {
...
class TestTaskUtils:
    ...
    async def test_should_collect_live_scores(self, mock_db_manager):
        ...
        with patch("src.tasks.utils.DatabaseManager") as mock_db_class:
            mock_db_instance = AsyncMock()
            mock_db_instance.get_async_session = mock_session_context
    ...
    def test_calculate_next_collection_time(self):
        ...
        with patch("src.tasks.utils.datetime") as mock_datetime:
            ...
            mock_datetime.timedelta = timedelta  # 保持timedelta正常工作
...
class TestIntegration:
    ...
    def test_monitoring_integration(self):
        ...
        monitor.record_task_start("collect_odds_task", "test-task-789")
        monitor.record_task_completion(
            "collect_odds_task", "test-task-789", 45.2, "success"
        )

        # 使用正确的单队列更新方法
        monitor.update_queue_size("odds", 5)
...
class TestEdgeCases:
    ...
    async def test_large_error_log_handling(self):
        ...
        with patch("src.tasks.error_logger.DatabaseManager") as mock_manager_class:
            ...
            with patch.object(logger, "_get_query_builder") as mock_builder_getter:
                mock_builder = MagicMock()
                mock_builder.build_cleanup_old_logs_query.return_value = (
