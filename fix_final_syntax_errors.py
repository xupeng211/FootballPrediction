...
def fix_all_syntax_errors(content):
    ...
    content = re.sub(
        ...
        r"from typing import\1, Dict, Any\2",
        content,
    )
    ...
    content = re.sub(
        r"Dict\[str,\s*Any\],\s*Any,\s*List\[Any\]", "Dict, Any, List", content
    )

    # 修复 "Dict[str, Any], Any, List[Any], Optional" -> "Dict, Any, List, Optional"
    content = re.sub(
        r"Dict\[str,\s*Any\],\s*Any,\s*List\[Any\],\s*Optional",
    ...
    content = re.sub(
        r"Any,\s*Dict\[str,\s*Any\],\s*List\[Any\]", "Any, Dict, List", content
    )

    # 修复 "Any, Dict[str, Any], Any, List[Any]" -> "Any, Dict, Any, List"
    content = re.sub(
        r"Any,\s*Dict\[str,\s*Any\],\s*Any,\s*List\[Any\]",
        "Any, Dict, Any, List",
        content,
    )
    ...
    content = re.sub(
        r"Union,\s*Any,\s*Dict\[str,\s*Any\],\s*Optional",
        "Union, Any, Dict, Optional",
        content,
    )
    ...
    content = re.sub(
        ...
        lambda m: m.group(0)
        if "]=" in m.group(0)
        else f"def {m.group(1)}({m.group(2)}): {m.group(3)}",
        content,
    )
    ...
    content = re.sub(
        r"Dict\[str,\s*Any\]\[([^\]]+?),\s*([^\]]+)\]", r"Dict[\1, \2]", content
    )
