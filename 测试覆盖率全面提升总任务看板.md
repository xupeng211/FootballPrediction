...
| ID | 任务 | 命令/动作 | 成果 | 状态 |
|----|------|------------|------|------|
| P1-1 | 扫描ImportError与坏测试 | `pytest --collect-only -r E` | 生成 `error_report.md` | [ ] |
| P1-2 | 自动禁用无效测试 | 将错误文件移至 `tests/disabled/` | pytest可完整收集 | [ ] |
| P1-3 | 扫描pytest.skip/skipif | `grep -R "pytest.skip"` | `skip_map.txt` | [ ] |
| P1-4 | 修正sys.path/import路径 | 自动添加 `sys.path.append("src")` | 可执行率提升 | [ ] |
| P1-5 | 运行修复后基线测试 | `pytest --maxfail=10 --cov=src` | `coverage_baseline.txt` | [ ] |
...
* 2025-10-17 22:30：完成 P1-1/P1-2，生成 error_report.md，覆盖率提升至 3.8%
```
