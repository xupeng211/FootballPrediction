...
| **Feature build** | Primary canonical | `npm run l3:stitch` | Schema must already be provisioned from `database/migrations/V26.4__create_l3_features_table.sql`; the stitch command only checks the precondition and writes feature data. Confirm input/write scope; `npm run smelt` is a specialized internal alternative |
