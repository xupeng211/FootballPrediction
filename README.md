...
| **Stage D market-evidence operations** | Canonical pre-authorization offline contract / controlled adapter implemented but disabled | `node scripts/ops/stage_d_cycle.js --dry-run --ledger-root <explicit-path> --quota-config config/stage_d_quota_budget.json` | The public one-cycle entrypoint remains offline-only: it loads Stage C authority, evaluates the lock/ledger/quota gate and emits a no-provider-request/no-canonical-write plan. The non-secret `config/stage_d_quota_budget.json` binds the Owner-declared Starter Free 500-credit plan, 50-credit reserve, 450-credit automated ceiling and one-credit `h2h × uk` cost; the module-level `executeStageDOneCycle` adapter is factory-bound and production-capable only with explicit runtime authorization plus reconciled provider headers. Its default path is disabled, tests use a networkless fake transport, and the scheduler remains off. No direct Stage C live path, hidden retry or fallback provider is permitted. |
