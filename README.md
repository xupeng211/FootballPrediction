...
| **Stage D market-evidence operations** | Canonical pre-authorization offline contract / controlled adapter implemented but disabled | Offline plan: `node scripts/ops/stage_d_cycle.js --dry-run --ledger-root <explicit-path> --quota-config config/stage_d_quota_budget.json [--quota-adjudication <external-artifact>]`; offline quota adjudication: `node scripts/ops/stage_d_quota_adjudication.js --ledger-root <path> --quota-config <path> --run-lock-trust-root <path> --output <new-trust-root-child> --historical-request-id <request-id> --source-main-sha <sha> --source-main-tree <tree> --adjudication-id <id>`; future separately authorized single-cycle binder: `node scripts/ops/stage_d_controlled_initialization.js --authorization <direct-child-of-trust-root> --authority-root <path> --allocation-authority <path> --ledger-root <path> --quota-config <path> [--quota-adjudication <external-artifact>] --fixture-universe-raw <path> --evidence-root <path> --run-lock-trust-root <path>` | The public plan and adjudication CLI are provider-free and transmission-free. The adjudication preserves local consumed accounting and explicit `PROVIDER_QUOTA_ACTUAL_EFFECT=UNKNOWN`; it can only admit a current-period one-credit dry-run when the immutable epoch, ledger generation, quota hash, historical request binding, artifact hash, permissions and conservative automated-ceiling bound validate. The binder is the sole future production-facing Stage D single-cycle entrypoint: it validates the read-only owner-controlled authorization plus the optional v2 adjudication hash, creates the module-private capability internally, binds `the_odds_api / h2h / uk / max=1 / cost=1` and authority pre-state, consumes authorization once, and delegates to the factory-bound cycle. It accepts no runtime token/boolean, retry count, provider override or scheduler mode. Tests use networkless fakes; Stage D remains `GATE_3=NOT_AUTHORIZED`, `STAGE_D_STARTED=NO`, and blocker #2/#3 remain closed as previously adjudicated. |
...
- V-next (`canonical_prematch/vnext-v1`) is a separate, versioned 17-feature
  definition in the same registry and is **not activated**. Its training
  projection currently accepts only nine registry-approved features; blocked
  SOT/standings/ELO and excluded possession remain outside the candidate.
  Production runtime and model-schema defaults remain V1.
...
- 新 Agent 先阅读 `AGENTS.md`；它指向唯一详细工作流 `docs/AGENT_WORKFLOW.md`。
  Claude-specific 权限差异见 `CLAUDE.md`，业务命令仍以本表为准；历史 workflow 文档不再是必读 authority。Agentic review policy is canonical in `AGENTS.md` / `docs/AGENT_WORKFLOW.md`: NORMAL defaults to DeepSeek, STRICT requires Codex, and CRITICAL requires both independent backends.
