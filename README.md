...
| **Feature build** | Primary canonical | `npm run l3:stitch` | Schema must already be provisioned from `database/migrations/V26.4__create_l3_features_table.sql`; the main stitch pipeline checks the precondition and writes feature data. Its legacy `scripts/maintenance/recalculate_elo.js` child is `SPECIALIZED_INTERNAL_RUNTIME_DDL`, not a schema authority, and remains separately authorized. Confirm input/write scope; `npm run smelt` is a specialized internal alternative |
