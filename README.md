...
| **Stage D market-evidence operations** | Canonical pre-authorization offline contract / controlled adapter implemented but disabled | Offline plan: `node scripts/ops/stage_d_cycle.js --dry-run --ledger-root <explicit-path> --quota-config config/stage_d_quota_budget.json`; future separately authorized single-cycle binder: `node scripts/ops/stage_d_controlled_initialization.js --authorization <direct-child-of-trust-root> --authority-root <path> --allocation-authority <path> --ledger-root <path> --quota-config <path> --fixture-universe-raw <path> --evidence-root <path> --run-lock-trust-root <path>` | The public plan remains offline-only. The new binder is the sole production-facing Stage D single-cycle entrypoint: it validates a read-only, owner-controlled bounded authorization artifact, creates the module-private capability internally, binds `the_odds_api / h2h / uk / max=1 / cost=1` plus the accounting epoch and authority pre-state, consumes the authorization once, and then delegates to the existing factory-bound cycle. It accepts no operator runtime token/boolean, retry count, provider override or scheduler mode. Tests inject only a networkless fake transport; the real provider path remains unauthorized, quota remains fail-closed, and blocker #2 (runtime filesystem permissions) and blocker #3 (independent backup fault-domain) remain open. |
