...
| **FotMob detail capture** | Controlled implementation surface | `node scripts/ops/fotmob_detail_capture.js plan` / `... replay` are fully offline; `... capture` requires `--execute` plus every documented gate | PLAN and REPLAY run offline with zero network and zero DB writes. CAPTURE is off by default and still requires explicit authorization: `--execute`, `CONFIRM_REAL_FOTMOB_DETAIL_CAPTURE=1`, `CONFIRM_MAX_FOTMOB_REQUESTS`, `--authorization-id`, `--expected-plan-sha256` (matching the plan), clean git worktree, 40-hex HEAD, repository-external non-symlink output root, safe `--run-id`, and a minimum 60 s inter-request delay. No real FotMob request has been made and no capture executed; all tests are mocked |
