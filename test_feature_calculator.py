    ...
    try:
        ...
        with patch.dict('sys.modules', modules_to_mock):
            ...
            try:
                ...
                print(f"  ✅ 滚动均值计算: {len(rolling_result) if rolling_result else 'None'} 个值")
            ...
            try:
                ...
                mock_matches = [
                    ...
                    {"id": 2, "home_team": "C", "away_team": "D", "home_score": 0, "away_score": 2}
                ]
...
def test_feature_algorithms():
    ...
    try:
        ...
        team_a_wins = sum(1 for m in head_to_head if (m["home_team"] == "A" and m["home_score"] > m["away_score"]) or
...
async def test_feature_pipeline():
    ...
    try:
        ...
        async def mock_get_matches():
            ...
            return [
                {"id": 1, "home_team": "Team A", "away_team": "Team B", "date": "2023-01-01"},
                {"id": 2, "home_team": "Team C", "away_team": "Team D", "date": "2023-01-02"}
            ]
        ...
        async def mock_batch_calculation():
            tasks = []
            for i in range(3):
                task = mock_calculate_features([{"id": i, "home_team": f"Team {i}", "away_team": f"Team {i+1}"}])
                tasks.append(task)
