...
class V33UnifiedMiner:
    ...
    def __init__(self):
        ...
        # 特征统计
        self.feature_counts = {"shotmap": 0, "momentum": 0, "environment": 0, "technical": 0, "total": 0}
    ...
    def extract_all_features(self, json_data: dict) -> dict[str, Any]:
        ...
        # 5. 计算交互特征
        interaction_dict = self._calculate_interaction_features(shotmap_features, momentum_features, env_features)
    ...
    def _calculate_interaction_features(self, shotmap: Any, momentum: Any, env: Any) -> dict[str, float]:
        """
        计算交互特征 (特征交叉)
        ...
        try:
            ...
            if shotmap and momentum:
                shot_pressure_interaction = shotmap.avg_shot_distance_home * momentum.pressure_diff
                features["inter_shot_distance_x_pressure"] = shot_pressure_interaction

                big_chance_momentum = shotmap.big_chances_created_home * momentum.last_15_min_pressure_home
                features["inter_big_chance_x_late_momentum"] = big_chance_momentum

            # 环境 x 射门交互
            if shotmap and env:
                weather_shot_accuracy = (1.0 if not env.is_bad_weather else 0.8) * shotmap.xg_per_shot_home
                features["inter_weather_x_shot_xg"] = weather_shot_accuracy

                fatigue_shots = shotmap.total_shots_home / max(1, env.days_since_last_match_home)
                features["inter_fatigue_x_shots"] = fatigue_shots

            # 动量 x 环境交互
            if momentum and env:
                night_momentum_boost = float(env.is_night_match) * momentum.last_15_min_pressure_home
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print()
    print("📊 Shotmap 特征: ~25 维")
    print("📈 Momentum 特征: ~15 维")
    print("🌍 Environment 特征: ~20 维")
    print("⚙️  Technical 特征: ~152 维")
    print("🔗 Interaction 特征: ~10 维")
    print()
    print("🎯 总计: ~220+ 维核心特征")
    print()
