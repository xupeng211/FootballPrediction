...
**当前版本**: V184.0.0-stable | **最后更新**: 2026-03-04

> ⚠️ **重要提示**: README.md 中部分命令已过时。以下脚本**不可用**：
...
docker-compose -f docker-compose.dev.yml exec dev python scripts/maintenance/fix_zombie_matches.py
docker-compose -f docker-compose.dev.yml exec dev python scripts/maintenance/database_detox.py
...
| 组件 | 功能 | 位置 |
|------|------|------|
| `ProductionHarvester` | L2/L3 数据收割主入口 (V179 无人值守) | `src/infrastructure/harvesters/` |
...
| `src/infrastructure/harvesters/ProductionHarvester.js` | L2/L3 收割器 (V179) | P0 |
...
**1. 深度隐身脚本 (`_injectStealthScripts`)**
```javascript
// 绕过 WebGL 指纹检测
...
**2. 弹性重试逻辑 (`_harvestWithRetry`)**
- 最大重试 3 次
- 指数退避：1s → 2s → 4s
- 自动端口避障：失败时切换代理节点

**3. L3 特征熔炼**
- `GoldenFeatureExtractor`: 身价/伤病/评分
- `TacticalMomentumExtractor`: xG/控球/射门
- `EloRatingExtractor`: Elo 动态评分
- `OddsMovementExtractor`: 赔率变动

### 数据资产 (2026-03-04)
| 表 | 记录数 | 状态 |
|---|---|---|
| `matches` | 562 | ✅ 已完赛 |
| `raw_match_data` | 562 | ✅ 原始 JSON |
| `l3_features` | 562 | ✅ 特征完整 (99.5% xG) |
