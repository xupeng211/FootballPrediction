#!/usr/bin/env python3
"""
GitHub Issues 批量创建脚本 (非交互式版本)
Create GitHub Issues for optimization tasks (non-interactive version)

使用方法:
python3 create_issues_silently.py
"""
...
class GitHubIssueCreator:
    ...
    def create_issue(self, title: str, body: str, labels: list, assignees: list = None):
        ...
        assignees_str = ' '.join([f'"{assignee}"' for assignee in assignees]) if assignees else ""
...
- 运行帮助命令: `make help`
- 查看质量报告: `python3 scripts/generate_quality_report.py`""".strip()
...
3. 连接池: 配置合适的连接池大小
4. 分页优化: 使用LIMIT/OFFSET或游标分页""".strip()
