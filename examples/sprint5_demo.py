...
from src.testing.backtester import BacktestEngine, BacktestConfig, run_backtest
...
async def demo_elo_rating_system():
    ...
    # 预测下一场比赛
    prediction = elo_system.predict_match_probabilities(
        home_team_id="Manchester United",
...
async def demo_poisson_features():
    ...
    # 创建泊松计算器
    poisson_calc = PoissonFeatureCalculator(
...
def demo_odds_movement_analysis():
    ...
    for i, odds_point in enumerate(odds_data):
        ...
        print(f"添加赔率数据点 {i+1}: 主胜 {odds_point['home_odds']} 平局 {odds_point['draw_odds']} 客胜 {odds_point['away_odds']}")
...
async def demo_model_ensemble():
    ...
    print(f"  概率分布: 客胜 {ensemble_result['probabilities'][0]:.1%} | "
...
def demo_backtest_engine():
    ...
    print(f"  1. 准备历史数据文件")
    print(f"  2. 配置正确的数据路径")
    print(f"  3. 运行: engine.run_backtest()")
...
async def demo_di_container_integration():
    ...
    try:
        ...
        print(f"\n服务容器集成演示完成!")
        print(f"所有Sprint 5高级分析服务已成功注册和初始化")
