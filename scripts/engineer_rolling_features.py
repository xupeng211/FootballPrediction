...
class RollingFeatureEngineer:
    ...
    def extract_team_identifiers(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'home_team_name' not in df.columns and 'away_team_name' not in df.columns:
            ...
            df['home_team_name'] = df.apply(lambda x: f"Team_Home_{x['match_id'] % 100}", axis=1)
            df['away_team_name'] = df.apply(lambda x: f"Team_Away_{x['match_id'] % 100}", axis=1)
    ...
    def build_team_history(self, df: pd.DataFrame) -> dict:
        ...
        for team in team_history:
            team_history[team] = sorted(team_history[team], key=lambda x: x['match_date'])

        logger.info(f"   ✅ 构建 {len(team_history)} 支球队的历史数据")
        return team_history

    
    def apply_rolling_features(self, df: pd.DataFrame, team_history: dict) -> pd.DataFrame:
        """应用滚动特征到数据集 - 确保时序安全"""
        ...
        for window_size in self.rolling_windows:
            ...
            for i, row in df.iterrows():
                ...
                home_history = [m for m in team_history.get(home_team, []) if m['match_date'] < current_date]
                away_history = [m for m in team_history.get(away_team, []) if m['match_date'] < current_date]

                # 计算主队滚动特征
                home_rolling = self._calculate_team_rolling_features(home_history, window_size)
                for feature_name, feature_value in home_rolling.items():
                    df.loc[i, f'home_{feature_name}'] = feature_value

                # 计算客队滚动特征
                away_rolling = self._calculate_team_rolling_features(away_history, window_size)
    ...
    def _calculate_team_rolling_features(self, matches: list, window_size: int) -> dict:
        ...
        # 计算胜率
        wins = sum(1 for m in window_matches if
                  (m['result'] == 'H' and m['venue'] == 'home') or
                  (m['result'] == 'A' and m['venue'] == 'away'))
        ...
        return {
            ...
            f'last_{window_size}_form_score': (wins * 3 + positive_goal_diff) / len(window_matches)
        }
    ...
    def build_safe_feature_dataset(self, input_path: str, output_path: str) -> pd.DataFrame:
        """构建安全的无泄露特征数据集"""
        ...
        # 定义安全的前赛特征（无数据泄露）
        base_safe_features = [
            'match_id', 'match_date', 'year', 'month', 'day_of_week', 'is_weekend',
    ...
    def print_dataset_summary(self, df: pd.DataFrame):
        ...
        print(f"\n📋 数据样本 (前3场):")
        rolling_cols = [col for col in df.columns if 'last_' in col][:8]  # 显示前8个滚动特征

        sample_cols = ['match_id', 'home_team_name', 'away_team_name', 'home_score', 'away_score'] + rolling_cols
