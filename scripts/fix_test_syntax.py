...
class EnhancedTestSyntaxFixer:
    ...
    def fix_try_except_blocks(self, content: str) -> Tuple[str, List[str]]:
        ...
        while i < len(lines):
            ...
            if line.startswith('try:'):
                ...
                if try_stack:
                    try_stack.pop()
            elif line and not line.startswith('#') and not line.startswith(('except ', 'except:', 'finally:', 'try:')):
        ...
        for i, line in enumerate(lines):
            ...
            if re.match(r'async\s+def\s+\w+', stripped):
                ...
                if i + 1 < len(lines) and not lines[i + 1].strip():
                    if i + 2 < len(lines) and not lines[i + 2].startswith(' ' * (len(line) - len(stripped) + 4)):
    ...
    def fix_string_literals(self, content: str) -> str:
        ...
        for pattern, closer in triple_patterns:
            if re.search(pattern, content, re.MULTILINE):
                content = re.sub(pattern, lambda m: m.group(0) + closer, content, flags=re.MULTILINE)

        # Fix single quotes
        single_quote_pattern = r"'([^']*|'(?!'))*$"
        if re.search(single_quote_pattern, content, re.MULTILINE):
            content = re.sub(single_quote_pattern, lambda m: m.group(0) + "'", content, flags=re.MULTILINE)

        # Fix double quotes
        double_quote_pattern = r'"([^"]*|"(?!"))*$'
        if re.search(double_quote_pattern, content, re.MULTILINE):
            content = re.sub(double_quote_pattern, lambda m: m.group(0) + '"', content, flags=re.MULTILINE)
    ...
    def process_file(self, file_path: Path) -> FixResult:
        ...
        try:
            ...
            # Apply fixes
            if original_error:
                content = self.fix_indentation_error(content,
                    int(original_error.split(':')[0].split()[-1]) if 'Line' in original_error else 1,
                    original_error)
    ...
    def generate_report(self, results: Dict[str, List[FixResult]], output_dir: Path):
        ...
        if results['unfixable']:
            with open(unfixed_file, 'w', encoding='utf-8') as f:
                ...
                f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
                f.write("These files could not be automatically fixed and require manual attention:\n\n")
