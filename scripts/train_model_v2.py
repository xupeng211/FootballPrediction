#!/usr/bin/env python3
"""
真实足球预测模型训练脚本 - 直接从数据库读取数据
Real Football Prediction Model Training Script - Direct Database Integration

该脚本避免了CSV文件的数据泄露问题，直接从数据库的features和matches表中读取数据。
"""
...
def train_model_with_proper_split(X, y):
    """使用时间序列拆分训练XGBoost模型，避免未来信息泄露"""
    logger.info("🚀 开始模型训练...")
...
def main():
    """主训练函数"""
    logger.info("=" * 60)
    logger.info("🎯 足球预测模型训练开始 (数据库直连版本)")
    logger.info("=" * 60)
    ...
    try:
        ...
        # 3. 训练模型
        model, label_encoder, accuracy, feature_importance = train_model_with_proper_split(X, y)
