...
cmd = [
    ...
    "-vv",
    "-ra",
    "--maxfail=5",  # 保持与pytest.ini一致
    "--cov=src",
    "--cov-report=json:coverage.json",
    "--cov-report=term-missing",
    "-p",
    "no:xdist",  # 保持与pytest.ini一致
]
...
if result.returncode != 0:
    ...
    with open(log_file, "w", encoding="utf-8") as f:
        proc = subprocess.run(
            ["pytest", "--maxfail=10", "--tb=short"], text=True, stdout=f, stderr=f
        )
...
low_cov = sorted(
    [(p, f["summary"]["percent_covered"]) for p, f in files.items()], key=lambda x: x[1]
)[:10]
