...
def execute_once():
    ...
    # 2. 运行测试并生成报告
    run(
        "pytest --maxfail=5 --disable-warnings -q --cov=. --cov-report=json:coverage.json || true",
        check=False,
    )
    ...
    run(
        f"git commit -m 'chore: auto coverage improvement round {now}' || true",
        check=False,
    )
    ...
    # 6. 创建并合并 PR
    run(
        f"gh pr create --base main --head {branch} --title 'ci: auto coverage improvement {now}' --body 'Automated coverage improvement iteration.' || true",
        check=False,
    )
