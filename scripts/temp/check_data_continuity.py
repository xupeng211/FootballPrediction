...
class DataContinuityAnalyzer:
    ...
    async def analyze_date_gaps(self, daily_counts: pd.DataFrame) -> dict[str, Any]:
        ...
        return {
            ...
            'data_coverage_percentage': (len(daily_counts) / len(full_date_range)) * 100,
    ...
    async def identify_sparse_dates(self, daily_counts: pd.DataFrame, threshold: int = 10) -> dict[str, Any]:
        """识别比赛稀疏日期"""
        ...
        low_activity_dates = sparse_dates[(sparse_dates['match_count'] >= 1) & (sparse_dates['match_count'] < 5)]
        medium_sparse_dates = sparse_dates[(sparse_dates['match_count'] >= 5) & (sparse_dates['match_count'] < 10)]
        ...
        return {
            ...
            'low_activity_dates': {
                ...
                'avg_matches': low_activity_dates['match_count'].mean() if len(low_activity_dates) > 0 else 0
            },
            'medium_sparse_dates': {
                'count': len(medium_sparse_dates),
                'dates': medium_sparse_dates['match_day'].tolist(),
                'avg_matches': medium_sparse_dates['match_count'].mean() if len(medium_sparse_dates) > 0 else 0
            }
        }

    async def analyze_weekly_patterns(self, daily_counts: pd.DataFrame) -> dict[str, Any]:
        """分析周度模式"""
        logger.info("📊 分析周度比赛模式...")

        # 添加星期信息
        daily_counts['weekday'] = pd.to_datetime(daily_counts['match_day']).dt.day_name()
        daily_counts['weekday_num'] = pd.to_datetime(daily_counts['match_day']).dt.dayofweek

        # 计算每周模式
        weekly_stats = daily_counts.groupby('weekday_num').agg({
            'match_count': ['mean', 'std', 'min', 'max', 'count'],
            'unique_leagues': 'mean'
        }).round(2)

        # 重命名列
        weekly_stats.columns = ['avg_matches', 'std_matches', 'min_matches', 'max_matches', 'total_days', 'avg_leagues']
        ...
        return {
            ...
            'weekend_vs_weekday': {
                'weekend_avg': weekly_stats.loc[['Saturday', 'Sunday'], 'avg_matches'].mean(),
                'weekday_avg': weekly_stats.loc[['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'], 'avg_matches'].mean()
            }
        }

    def create_density_calendar(self, daily_counts: pd.DataFrame, save_path: str = '/app/data_density_calendar.png'):
        """创建数据密度日历图"""
            ...
            for i, year in enumerate(years):
                ...
                for month in range(1, 13):
                    ...
                    for _, row in month_data.iterrows():
                        day = row['day']
                        weekday = pd.to_datetime(f"{year}-{month:02d}-{day:02d}").dayofweek
                        week = day // 7

                        # 对数值进行对数变换以便可视化
                        match_count = row['match_count']
                        if match_count > 0:
                            calendar_matrix[week, weekday] = np.log1p(match_count)  # log(1 + x)

                    # 绘制日历热图
                    im = ax.imshow(calendar_matrix, cmap='YlOrRd', aspect='auto', vmin=0, vmax=np.log1p(50))
    ...
    async def generate_patch_plan(self, gap_analysis: dict, sparse_analysis: dict) -> dict[str, Any]:
        """生成数据补漏计划"""
    ...
    async def generate_comprehensive_report(self) -> dict[str, Any]:
        ...
        comprehensive_report = {
            ...
            "data_overview": {
                ...
                "date_range": {
                    "start": daily_counts['match_day'].min().isoformat() if len(daily_counts) > 0 else None,
                    "end": daily_counts['match_day'].max().isoformat() if len(daily_counts) > 0 else None
                },
            ...
            "visualizations": {
                "density_calendar_path": calendar_path
            },
            "elo_feasibility_assessment": {
                "recommended": gap_analysis['data_coverage_percentage'] >= 75,
                "confidence_level": "High" if gap_analysis['data_coverage_percentage'] >= 90 else "Medium" if gap_analysis['data_coverage_percentage'] >= 75 else "Low",
                "key_considerations": [
    ...
    def print_strategic_summary(self, report: dict[str, Any]):
        ...
        print(f"   时间跨度: {overview['date_range']['start']} 至 {overview['date_range']['end']}")
        ...
        print(f"   周末平均: {weekly['weekend_vs_weekday']['weekend_avg']:.1f}场 vs 工作日: {weekly['weekend_vs_weekday']['weekday_avg']:.1f}场")
