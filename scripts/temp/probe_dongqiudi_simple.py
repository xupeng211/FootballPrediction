...
class DongqiudiSimpleProbe:
    ...
    def _analyze_response(self, data: Any) -> dict:
        ...
        def search_xg(obj, depth=0):
            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    if any(xg_term in str(key).lower() for xg_term in ['xg', 'expected_goal', '期望进球']):
                        return True
        ...
        def search_lineup(obj, depth=0):
            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    if any(lineup_term in str(key).lower() for lineup_term in ['lineup', 'formation', '首发', 'players']):
                        return True
        ...
        def search_stats(obj, depth=0):
            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    if any(stat_term in str(key).lower() for stat_term in ['statistic', 'technical', 'possession', '射门', '控球']):
                        return True
        ...
        if isinstance(data, dict):
            for key, value in list(data.items())[:5]:
                if isinstance(value, (dict, list)):
                    ...
                    str_val = str(value)
                    analysis['sample_structure'][key] = str_val[:50] + "..." if len(str_val) > 50 else str_val
        ...
        for match_id in self.test_match_ids:
            ...
            if result['has_data']:
                ...
                for res in result['results']:
                    ...
                    logger.info("  📊 数据分析:")
                    logger.info(f"    - 比赛数据: {'✅' if analysis['is_match_data'] else '❌'}")
                    logger.info(f"    - xG数据: {'✅' if analysis['has_xg'] else '❌'}")
                    logger.info(f"    - 阵容数据: {'✅' if analysis['has_lineup'] else '❌'}")
                    logger.info(f"    - 技术统计: {'✅' if analysis['has_stats'] else '❌'}")

                    if analysis['data_keys']:
                        logger.info(f"    - 数据键: {', '.join(analysis['data_keys'][:5])}")
