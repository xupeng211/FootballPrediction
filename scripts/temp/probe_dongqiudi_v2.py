...
class DongqiudiDeepProbe:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    for pattern in json_patterns:
                        ...
                        for match_json in matches:
                            try:
                                ...
                                if matches_info:
                                    logger.info(f"✅ 从 {url} 提取到 {len(matches_info)} 场比赛")
                                    return matches_info
                    ...
                    if match_ids:
                        ...
                        return [{'id': mid, 'source': 'html_link'} for mid in set(match_ids[:10])]
        ...
        def find_objects_with_key(obj, target_keys):
            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    if any(target_key in str(key).lower() for target_key in target_keys):
                        found.append(obj)
    ...
    def _analyze_match_data(self, data: Any, expected_id: str) -> dict:
        ...
        if isinstance(data, dict):
            analysis['data_structure'] = {k: type(v).__name__ for k, v in data.items()[:10]}
        ...
        # 搜索xG相关数据
        xg_patterns = [
            r'xg', r'expected_goal', r'期望进球', r'xG', r'expected_goals',
            r'xg_total', r'xg_home', r'xg_away', r'xg[\'\"]?\s*:\s*[\d.]+'
        ]
        ...
        # 搜索阵容相关数据
        lineup_patterns = [
            r'lineup', r'formation', r'首发', r'starting', r'players',
            r'squad', r'阵容', r'lineups', r'team_lineup'
        ]
        ...
        # 搜索技术统计数据
        stats_patterns = [
            r'statistic', r'technical', r'possession', r'射正', r'传球',
            r'控球率', r'射门', r'passes', r'shots', r'corners'
        ]
    ...
    def print_summary(self, all_results: list[dict]):
        ...
        endpoints_with_lineup = sum(1 for r in all_results if r['validation']['has_lineup'])
        endpoints_with_stats = sum(1 for r in all_results if r['validation']['has_stats'])
