    ...
    if coverage_file.exists():
        try:
            with open(coverage_file, 'r') as f:
                data = json.load(f)
                return {k: v.get('summary', {}).get('percent_covered', 0)
...
def main():
    ...
    for i, candidate in enumerate(candidates[:10], 1):
        ...
        print(f"{file_name:<40} {candidate['module_type']:<10} "
              f"{candidate['code_lines']:<8} {candidate['coverage']:.1f}%{' ':<5} "
              f"{candidate['missing_lines']:<8}")

    print(f"\n📈 分析完成:")
    print(f"   总候选文件数: {len(candidates)}")
    print(f"   Top 5 潜在覆盖提升: {sum(c['missing_lines'] for c in candidates[:5]):.0f} 行代码")
