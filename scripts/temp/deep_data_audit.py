...
class DataAuditor:
    ...
    async def load_data(self):
        ...
        try:
            ...
            async with async_session() as session:
                ...
                for row in rows:
                    try:
                        match_data = row.match_data if isinstance(row.match_data, dict) else {}

                        # 解析关键字段 - 从样本数据看，数据存储在match_data中
                        raw_data = match_data.get('raw_data', {})
                        match_date = match_data.get('time', '') or match_data.get('match_time', '')
    ...
    def analyze_timeline(self):
        ...
        # 按月统计
        monthly_stats = valid_dates.groupby(['year', 'month']).agg({
            'external_id': 'count',
            'parsed_date': ['min', 'max']
        }).reset_index()

        monthly_stats.columns = ['year', 'month', 'match_count', 'earliest_date', 'latest_date']
        ...
        for year in range(min_year, current_year + 1):
            for month in range(1, 13):
                # 检查是否存在这个月份的数据
                month_data = monthly_stats[(monthly_stats['year'] == year) & (monthly_stats['month'] == month)]
    ...
    def analyze_leagues(self):
        ...
        # 识别杯赛
        cup_keywords = ['Cup', 'cup', 'Trophy', 'trophy', 'Champions', 'champions', 'Europa', 'europa']
        ...
        league_stats['type'] = league_stats['is_cup'].apply(lambda x: '杯赛' if x else '联赛')

        # 按类型统计
        type_stats = league_stats.groupby('type').agg({
            'league_name': 'count',
            'match_count': 'sum'
        }).reset_index()
        ...
        print(f"🏆 联赛: {type_stats[type_stats['type'] == '联赛']['total_matches'].iloc[0]} 场")
        print(f"🏆 杯赛: {type_stats[type_stats['type'] == '杯赛']['total_matches'].iloc[0]} 场")
    ...
    def analyze_quality(self):
        ...
        # 分数完整性
        self.data['has_score'] = (self.data['home_score'].notna() &
                                self.data['away_score'].notna() &
                                ((self.data['home_score'] > 0) | (self.data['away_score'] > 0)))

        # 队伍名称完整性
        self.data['has_teams'] = (self.data['home_team'].notna() &
        ...
        quality_metrics = {
            ...
            'finished_matches': len(self.data[self.data['status_category'] == 'Finished']),
            'finished_with_score': len(self.data[(self.data['status_category'] == 'Finished') &
                                              self.data['has_score']]),
        ...
        # 计算质量得分
        scores = {
            'completion_rate': min(100, (quality_metrics['finished_with_score'] / quality_metrics['total_records']) * 100),
            'team_completeness': min(100, (quality_metrics['has_complete_teams'] / quality_metrics['total_records']) * 100),
            'date_completeness': min(100, (quality_metrics['has_valid_date'] / quality_metrics['total_records']) * 100),
            'league_completeness': min(100, (quality_metrics['has_league_info'] / quality_metrics['total_records']) * 100),
        }
    ...
    def generate_recommendations(self):
        ...
        # 基于联赛覆盖
        cup_ratio = len(self.report['leagues']['cup_competitions']) / self.report['leagues']['total_leagues']
        if cup_ratio < 0.2:
            recommendations.append({
                'priority': 'Medium',
                'category': '赛事覆盖',
...
"""
        ...
        if 'year' in self.data.columns and 'quarter' in self.data.columns:
            quarterly_stats = self.data.groupby(['year', 'quarter']).size().unstack(fill_value=0)
...
async def main():
    ...
    if report:
        ...
        # 保存报告到文件
        report_filename = f"data_quality_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
