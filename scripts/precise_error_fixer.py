...
'''
            ...
            if "class TestDateTimeUtilsPart20:" in content:
                ...
                for i, line in enumerate(lines):
                    ...
                    if "class TestDateTimeUtilsPart20:" in line and i+1 < len(lines):
                        next_line = lines[i+1]
                        # 如果下一行不是测试方法，添加pass
                        if not (next_line.strip().startswith("def ") or next_line.strip().startswith("@")):
    ...
    def run_quick_coverage_test(self):
        ...
        try:
            result = subprocess.run(
                ["python", "-m", "pytest", "tests/unit/utils/test_validators_complete.py", "--cov=src.utils", "--cov-report=term", "-q"],
                cwd=self.project_root,
                capture_output=True,
                text=True,
                timeout=120
