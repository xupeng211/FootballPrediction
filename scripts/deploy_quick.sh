...
# =================================================================
# 快速试运营环境部署脚本
# Quick Staging Environment Deployment Script
# =================================================================
...
# 配置变量
COMPOSE_FILE="docker/docker-compose.quick.yml"
...
start_services() {
    ...
    print_message $GREEN "✅ 快速试运营服务启动完成"
}
...
# 函数：显示访问信息
show_access_info() {
    print_message $BLUE "🌐 快速试运营环境访问信息:"
    echo "----------------------------------------"
    ...
    echo "  用户名: postgres"
    echo "  密码: quick_db_password_2024"
    echo ""
    echo "🔑 Redis连接:"
    echo "  主机: localhost:6379"
    echo "  密码: quick_redis_password_2024"
    echo ""
    echo "📝 查看日志:"
...
# 函数：显示帮助信息
show_help() {
    echo "足球预测系统快速试运营环境部署脚本"
    echo ""
    echo "用法: $0 [选项]"
    echo ""
    echo "选项:"
    echo "  deploy     部署快速试运营环境"
    echo "  status     显示当前状态"
    echo "  logs       显示服务日志"
    echo "  stop       停止所有服务"
    echo "  restart    重启所有服务"
...
# 主部署函数
deploy() {
    print_message $PURPLE "🚀 开始部署足球预测系统快速试运营环境..."
    ...
    # 9. 部署成功
    print_message $GREEN "🎉 快速试运营环境部署完成！"
    print_message $CYAN "📊 系统状态: 快速试运营就绪"
    print_message $CYAN "🌐 访问地址: http://localhost:8000"
    echo ""

    print_message $YELLOW "📋 快速试运营建议:"
...
case "$1" in
    ...
    logs)
        ...
        ;;
    stop)
        print_message $BLUE "🛑 停止快速试运营服务..."
        docker-compose -f $COMPOSE_FILE down
        print_message $GREEN "✅ 服务已停止"
        ;;
    restart)
        print_message $BLUE "🔄 重启快速试运营服务..."
