...
class FirstWeekLauncher:
    ...
    async def _check_and_configure_environment(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for var in required_vars:
                if not hasattr(self.settings, var.lower()) or not getattr(
                    self.settings, var.lower(), None
                ):
    ...
    async def _verify_api_connections(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"all_healthy": all_healthy, "connection_status": connection_status}
    ...
    async def _configure_kelly_safety(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(
                ...
                json={
                    "max_stake_percentage": safety_config[
                        "max_stake_percentage_of_bankroll"
                    ],
                    "max_daily_stake_percentage": safety_config[
                        "max_daily_stake_percentage"
                    ],
                    "emergency_stop_enabled": safety_config["emergency_stop_enabled"],
                    "manual_override_required": safety_config[
    ...
    async def _configure_observation_mode(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if current_mode.value != self.first_week_config["observation_mode"]:
                logger.info(
                    f"🔍 启用观察模式: {current_mode.value} → {self.first_week_config['observation_mode']}"
                )
    ...
    async def _setup_monitoring(self) -> Dict[str, Any]:
        ...
        try:
            ...
            monitoring_config = {
                "health_check_interval": self.first_week_config[
                    "health_check_interval"
                ],
                "performance_check_interval": self.first_week_config[
                    "performance_check_interval"
                ],
                "observation_check_interval": self.first_week_config[
                    "observation_check_interval"
                ],
            }

            logger.info("✅ 监控配置完成")
            logger.info(
                f"📊 健康检查间隔: {monitoring_config['health_check_interval']}秒"
            )
            logger.info(
                f"📈 性能检查间隔: {monitoring_config['performance_check_interval']}秒"
            )
            logger.info(
                f"🔍 观察检查间隔: {monitoring_config['observation_check_interval']}秒"
            )
    ...
    async def _final_verification(self) -> Dict[str, Any]:
        """最终验证"""
        try:
            verification_data = {"timestamp": datetime.now().isoformat(), "checks": {}}
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    verification_data["checks"]["kelly_safety"] = safety_data[
                        "safety_enabled"
                    ]
                    verification_data["checks"]["kelly_multiplier"] = (
                        safety_data.get("max_stake_percentage") == 5.0
                    )
                else:
            ...
            try:
                ...
                await manager.initialize()
                current_mode = await manager._get_current_observation_mode()
                verification_data["checks"]["observation_mode"] = (
                    current_mode.value == self.first_week_config["observation_mode"]
                )
            except:
            ...
            logger.info("✅ 最终验证完成")
            logger.info(
                f"API健康: {'✅' if verification_data['checks']['api_health'] else '❌'}"
            )
            logger.info(
                f"Kelly安全: {'✅' if verification_data['checks']['kelly_safety'] else '❌'}"
            )
            logger.info(
                f"观察模式: {'✅' if verification_data['checks']['observation_mode'] else '❌'}"
            )
    ...
    async def _generate_startup_report(self, launch_data: Dict[str, Any]):
        ...
        try:
            ...
            report_file = (
                report_dir
                / f"first_week_launch_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
    ...
    def print_launch_summary(self, launch_data: Dict[str, Any]):
        ...
        print(
            f"  日最大投注: {self.first_week_config['max_daily_stake_percentage']:.0%}"
        )
...
async def main():
    ...
    launch_parser.add_argument(
        "--configure-only", action="store_true", help="仅配置，不启动服务"
    )
