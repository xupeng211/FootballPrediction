...
class CIQualityChecker:
    ...
    def __init__(self, project_root: Path = None):
        ...
        self.quality_gates = {
            ...
            "tests": {"min_pass_rate": 85.0, "max_failures": 5, "min_total": 100},
            "code_quality": {"max_ruff_errors": 10, "max_mypy_errors": 10, "format_required": True},
            "security": {"max_vulnerabilities": 2, "max_secrets": 3},
        }
    ...
    def run_command(
        ...
        try:
            result = subprocess.run(
                command, cwd=self.project_root, capture_output=True, text=True, timeout=timeout
            )
    ...
    def check_test_coverage(self) -> Dict[str, Any]:
        ...
        if not coverage_file.exists():
            return {"passed": False, "overall_coverage": 0.0, "error": "覆盖率报告文件未生成"}
        ...
        try:
            ...
            return {"passed": False, "overall_coverage": 0.0, "error": f"解析覆盖率报告失败: {e}"}
    ...
    def _parse_test_results(self, output: str) -> Dict[str, Any]:
        ...
        if pass_rate < gates["min_pass_rate"]:
            errors.append(f"测试通过率 {pass_rate:.1f}% 低于要求 {gates['min_pass_rate']:.1f}%")

        if failed_tests > gates["max_failures"]:
            errors.append(f"失败测试数 {failed_tests} 超过限制 {gates['max_failures']}")

        if total_tests < gates["min_total"]:
            warnings.append(f"测试总数 {total_tests} 少于建议值 {gates['min_total']}")
    ...
    def check_code_quality(self) -> Dict[str, Any]:
        ...
        ruff_result = self.run_command(
            ["ruff", "check", "src/", "--output-format=text"], "Ruff代码检查"
        )

        results["ruff"]["success"] = ruff_result["success"]
        results["ruff"]["details"] = ruff_result["stdout"] + ruff_result["stderr"]
        results["ruff"]["errors"] = len(
            [line for line in ruff_result["stdout"].split("\n") if line.strip()]
        )

        # MyPy检查
        mypy_result = self.run_command(["mypy", "src/"], "MyPy类型检查")
        ...
        # 格式检查
        format_result = self.run_command(["ruff", "format", "--check", "src/"], "代码格式检查")
    ...
    def check_security(self) -> Dict[str, Any]:
        ...
        secret_matches = (
            secrets_result["stdout"].strip().split("\n") if secrets_result["stdout"].strip() else []
        )
        # 过滤掉测试文件和示例
        real_secrets = [
            match
            for match in secret_matches
            if "/test_" not in match and "example" not in match.lower()
        ]
    ...
    def _generate_summary(self, results: Dict) -> Dict[str, Any]:
        """生成检查摘要"""
        summary = {"coverage": "N/A", "tests": "N/A", "code_quality": "N/A", "security": "N/A"}
