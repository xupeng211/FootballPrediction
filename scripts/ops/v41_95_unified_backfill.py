#!/usr/bin/env python3
"""
V41.95 "清除干扰与逻辑归宗" - 统一回填引擎
===============================================

V41.95 重构：
1. ❌ 删除内部 get_db_connection() - 使用统一配置
2. ✅ 从 src.config_unified 导入数据库配置
3. ✅ 从 src.api.collectors.fotmob_core 导入解析逻辑
4. ✅ 统一使用 psycopg2 连接工厂模式
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/v41_95_unified_backfill.log'),
...
# ============================================================================
# V41.95: 数据库连接工厂 - 使用统一配置
# ============================================================================

def get_db_connection():
    """
    V41.95: 获取数据库连接 - 使用统一配置

    ⚠️ 禁止在此函数内部硬编码任何连接参数！
    所有连接参数必须从 src.config_unified.get_settings() 获取
    ...
    try:
        ...
        raise RuntimeError(
            f"❌ 数据库连接失败: {db.host}:{db.port}/{db.name}\n"
            f"   错误: {e}\n"
