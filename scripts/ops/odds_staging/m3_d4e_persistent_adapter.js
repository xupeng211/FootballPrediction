...
function createScopedPersistenceConflict(scope, key, reason) {
    const error = new PersistenceConflictError(reason);
    error.conflict_scope = scope;
    error.conflict_key_hash = sha256Text(String(key));
    error.conflict_reason = reason;
    return error;
}
...
class M3D4EPersistentAdapter {
    ...
    createPersistenceConflict(scope, key, reason) { return createScopedPersistenceConflict(scope, key, reason); }
    async createImportRun(run) {
        ...
        if (existing.rowCount) { const prior = existing.rows[0]; if (!sameText(prior.mode, run.mode) || !sameText(prior.source_type, run.source_type) || !sameText(prior.pipeline_version, run.pipeline_version) || !sameText(prior.pipeline_code_sha, run.pipeline_code_sha) || !sameText(prior.manifest_hash, run.manifest_hash) || !sameText(prior.candidate_business_hash, run.candidate_business_hash) || Number(prior.expected_accepted_count) !== run.expected_accepted_count || Number(prior.expected_quarantine_count) !== run.expected_quarantine_count || !sameJson(prior.metadata, run.metadata)) throw createScopedPersistenceConflict('import_run', run.run_key, 'divergent import run conflict'); this.runId = prior.id; this.completedReplay = prior.status === 'completed'; return { id: prior.id, already_present: true }; }
    ...
    async registerSourceFile(source) {
        ...
        if(existing.rowCount){const prior=existing.rows[0]; if(prior.source_provider!==source.source_provider||prior.logical_path!==source.logical_path||prior.content_hash!==source.content_hash||prior.hash_algorithm!==source.hash_algorithm||prior.manifest_hash!==source.manifest_hash||prior.competition!==source.competition||prior.season!==source.season||Number(prior.row_count)!==source.row_count||!sameJson(prior.provenance,source.provenance)) throw createScopedPersistenceConflict('source_file', source.content_hash, 'divergent source file conflict'); this.sourceFileId=prior.id;return {id:prior.id,already_present:true};}
...
module.exports = { D4E_LOCK_KEY, M3D4EPersistentAdapter, createScopedPersistenceConflict };
