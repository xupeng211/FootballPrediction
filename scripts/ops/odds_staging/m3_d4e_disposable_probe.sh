...
docker run -i --rm --network "$net" --read-only --tmpfs /tmp:rw,noexec,nosuid,size=32m --env-file "$dir/writer.env" -e PGHOST="$name" -e M3_D4E_PROBE_DB="$db" -e M3_D4E_DATABASE=fp_m3_persistent_sandbox -e ALLOW_DB_WRITE=yes -e FINAL_DB_WRITE_CONFIRMATION=yes -e ALLOW_ODDS_WRITE=yes -e DRY_RUN=false -v "$ROOT:/app:ro" -w /app node:20-alpine node - <<'NODE'
