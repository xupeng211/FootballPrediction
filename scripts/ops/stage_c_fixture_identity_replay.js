...
function main() {
    ...
    const report = { fixture_count: universe.snapshot.fixtures.length, event_count: universe.snapshot.fixtures.length, odds_event_count: JSON.parse(oddsRaw).length, matched: resolution.aliases.length, quarantined: resolution.quarantines.length, observation_count: observations.length, hashes: Object.fromEntries(Object.keys(payload).filter(k => k.endsWith('_snapshot') || k.endsWith('_registry') || k === 'fixture_universe').map(k => [k, semanticReplayHash(payload[k])])), identity_replay_sha256: resolution.semantic_sha256, market_replay_sha256: payload.market_projection_sha256 };
