            ...
            if 'class ' in content:
                ...
                if class_idx is not None:
                    ...
                    while method_idx < len(lines) and (
                        lines[method_idx].startswith('"""') or
                        lines[method_idx].strip() == '' or
                        lines[method_idx].strip().startswith('#')
                    ):
                        method_idx += 1

                    if method_idx < len(lines) and ('def ' in lines[method_idx] or 'async def ' in lines[method_idx]):
                        # 在方法前添加 logger
                        indent = '    '
...
def main():
    ...
    # 从 MyPy 输出中获取需要修复的文件
    result = os.popen('mypy src 2>&1 | grep "logger" | grep "not defined" | cut -d: -f1 | sort -u').read()
