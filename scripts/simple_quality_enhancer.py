...
def _enhance_code_quality_handle_error():
...
def _enhance_code_quality_check_condition():
...
def _enhance_code_quality_process_logic():
        current_score = 4.37
...
def enhance_code_quality():
    ...
    # 获取当前质量分数
    _enhance_code_quality_handle_error()
        result = subprocess.run(
            ['python3', 'scripts/quality_guardian.py', '--check-only'],
            capture_output=True,
            text=True,
        ...
        if '综合质量分数' in result.stdout:
            ...
            _enhance_code_quality_check_condition()
                current_score = float(match.group(1))
                print(f"   当前质量分数: {current_score}/10")
            else:
                current_score = 4.37
            ...
            current_score = 4.37
    _enhance_code_quality_process_logic()
        current_score = 4.37
