...
class AutomatedMaintenanceSystem:
            ...
            if result.returncode == 0:
                ...
                for line in lines:
                    if "Found" in line and "errors" in line:
                        ...
                        break
                health_results["code_quality"] = {
                    "status": "⚠️ 需要改进",
                    "details": f"{errors}个质量问题",
                }
        except Exception as e:
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "test_basic_pytest.py",
                    "--cov=src",
                    "--cov-report=json:maintenance_coverage.json",
                    "--quiet",
                ],
                capture_output=True,
                text=True,
    ...
    def generate_maintenance_report(self):
        ...
        # 保存报告
        report_file = Path(
            f"maintenance_reports/maintenance_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        report_file.parent.mkdir(exist_ok=True)
