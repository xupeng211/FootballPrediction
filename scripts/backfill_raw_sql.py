...
class RawSQLBackfillService:
    ...
    async def collect_daily_data(
        ...
        try:
            ...
            if "all" in sources or "football-data" in sources:
                ...
                if matches_result.success:
                    ...
                    result["errors"].append(
                        f"Football-Data.org收集失败: {matches_result.error}"
                    )
            ...
            result["success"] = (
                result["total_matches"] > 0 or len(result["errors"]) == 0
            )
            ...
            async for db in get_async_db():
                ...
                for match_data in data_result["football_data_matches"]:
                    try:
                        ...
                        raw_date = datetime.fromisoformat(
                            match_data.get(
                                "utcDate", f"{data_result['date']}T15:00:00Z"
                            )
                        )
                        match_date = (
                ...
                for match_data in data_result["fotmob_matches"]:
                    try:
                        ...
                        saved_count += 1
                        print(
                            f"✅ FotMob比赛插入成功: {home_team_id} vs {away_team_id}"
                        )
