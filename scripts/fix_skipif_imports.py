...
def fix_module_imports(filepath, module_name, actual_import):
    ...
    try:
        ...
        # 查找并替换导入检查
        rf"(# 测试导入\s+try:\s+from {module_name} import.*?except ImportError as e:.*?{module_name.upper()}_AVAILABLE = False)"
            ...
            if "# 测试导入" in line and i + 10 < len(lines):
                ...
                if module_name in import_block:
                    ...
                    if j < len(lines):
                        ...
                        new_lines.append('    print(f"Import error: {e}")')
...
def main():
    ...
    modules_to_fix = [
        (
            ...
            "src.api.features",
            "router, get_feature_store, validate_match_id, check_feature_store_availability, get_match_info, get_features_data, build_response_data, feature_store",
        ),
        (
            "tests/unit/api/test_monitoring.py",
            "src.monitoring",
            "SystemMonitor, MetricsCollector, HealthChecker",
        ),
        (
            "tests/unit/services/test_audit_service.py",
            "src.services.audit",
            "AuditService, AuditLogger, AuditQuery",
        ),
        (
            "tests/unit/utils/test_helpers_new.py",
            "src.utils.helpers",
            "validate_email, format_phone, generate_uuid",
        ),
    ]
