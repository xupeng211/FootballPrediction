    ...
    def collect_ruff_stats(self) -> dict:
        ...
        if not result or result.returncode != 0:
            return {
                ...
                "output": result.stderr if result else "No output",
            }
        ...
        for module in modules:
            if (self.project_root / module).exists():
                ...
                if module_result and module_result.returncode == 0:
                    ...
                    stats["files"].append(
                        {"module": module, "error_count": 0, "error": "Command failed"}
                    )
...
"""
        ...
        if ruff_stats.get("error_types"):
            for error_code, count in sorted(
                ruff_stats["error_types"].items(), key=lambda x: x[1], reverse=True
            ):
...
"""
        ...
        if ruff_stats.get("total_errors", 0) > 1000:
            recommendations.append(
                "🔴 **High Priority**: Ruff errors exceed 1000. Consider a focused cleanup sprint."
            )
        elif ruff_stats.get("total_errors", 0) > 500:
            recommendations.append(
                "🟡 **Medium Priority**: Ruff errors trending high. Monitor and plan cleanup."
            )

        if mypy_stats.get("status") != "passed":
            recommendations.append(
                "🔴 **High Priority**: MyPy type checking failed. Fix type errors immediately."
            )
        ...
        if not recommendations:
            recommendations.append(
                "🟢 **Good**: All quality gates passing. Maintain current standards."
            )
