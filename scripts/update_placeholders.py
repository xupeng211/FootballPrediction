    ...
    try:
        async with db_manager.get_async_session() as session:
            ...
            for team in placeholder_teams:
                team_id_str = (
                    team.name.split(" ")[1] if len(team.name.split(" ")) > 1 else None
                )
            ...
            for team_data in teams_data:
                ...
                if external_id in team_mapping:
                    ...
                    try:
                        ...
                        logger.info(
                            f"✅ 更新成功: {update_data['name']} (ID: {team_db_id})"
                        )
                        updated_count += 1
                    ...
                    logger.debug(
                        f"跳过非占位符球队: {team_data.get('name')} (ID: {external_id})"
                    )
