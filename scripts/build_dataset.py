...
from src.features.enhanced_feature_extractor import (
...
class DatasetBuilder:
            ...
            async with async_session() as session:
                ...
                for offset in range(0, total_matches, batch_size):
                    ...
                    result = await session.execute(
                        text(batch_query), {"batch_size": batch_size, "offset": offset}
                    )
                    ...
                    logger.info(
                        f"📦 提取批次 {offset // batch_size + 1}: {len(batch_dicts)} 场比赛"
                    )
    ...
    async def transform(self, matches: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            try:
                ...
                if features:
                    ...
                    if i % 100 == 0 or i == len(matches):
                        ...
                        logger.info(
                            f"⚡ 处理进度: {i}/{len(matches)} ({progress:.1f}%)"
                        )
                else:
        ...
        logger.info(
            f"✅ Transform 完成: 成功 {self.stats['processed_matches']:,}, 失败 {self.stats['failed_matches']:,}"
        )
        return features_list

    def load(
        self, features_list: List[Dict[str, Any]], output_path: str
    ) -> pd.DataFrame:
        """
        Load: 保存特征数据为CSV文件
    ...
    def generate_report(self, df: pd.DataFrame, output_path: str):
        ...
        print(
            f"   成功率:       {(self.stats['processed_matches'] / self.stats['total_matches'] * 100):.1f}%"
        )
        print(f"   处理耗时:     {duration:.1f} 秒")
        print(
            f"   处理速度:     {self.stats['processed_matches'] / duration:.1f} 场/秒"
        )
        ...
        feature_categories = {
            ...
            "上下文特征": [
                ...
                if any(
                    keyword in col
                    for keyword in ["referee", "stadium", "weather", "odds"]
                )
            ],
            "衍生特征": [
                col
        ...
        for category, features in feature_categories.items():
            ...
            if existing_features:
                ...
                print(
                    f"   {category:12}: {len(existing_features):2} 个特征, 覆盖率: {coverage:5.1f}%"
                )
        ...
        for feature in important_features:
            if feature in df.columns:
                ...
                coverage = (non_null_count / len(df)) * 100
                print(
                    f"   {feature:20}: {non_null_count:6,}/{len(df):6,} ({coverage:5.1f}%)"
                )
