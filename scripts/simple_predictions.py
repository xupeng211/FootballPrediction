...
def main():
    ...
    try:
        ...
        for col in feature_columns:
            if col in features_df.columns:
                ...
                if col == 'home_team_encoded':
                    features_df[col] = features_df['home_team_name'].apply(lambda x: hash(str(x)) % 1000)
                    available_features.append(col)
                elif col == 'away_team_encoded':
                    features_df[col] = features_df['away_team_name'].apply(lambda x: hash(str(x)) % 1000)
                    available_features.append(col)
                elif col == 'league_encoded':
                    features_df[col] = features_df['league_name'].apply(lambda x: hash(str(x)) % 100)
                    available_features.append(col)
                elif col == 'day_of_week':
                    features_df['collection_date'] = pd.to_datetime(features_df['collection_date'])
        ...
        logger.info(f"  - 高信心预测 (>60%): {len(high_confidence)} ({len(high_confidence)/total_predictions*100:.1f}%)")
        ...
        if len(completed_matches) > 0:
            correct_predictions = completed_matches[
                completed_matches['predicted_result'] == completed_matches['actual_result']
            ]
            accuracy = len(correct_predictions) / len(completed_matches)
            logger.info(f"  - 已完成比赛预测准确率: {accuracy:.3f} ({len(correct_predictions)}/{len(completed_matches)})")
        ...
        for _, pred in top_predictions.iterrows():
            ...
            score_info = f"({pred['home_score']}-{pred['away_score']})" if pred['home_score'] > 0 else ""
            actual_info = f" [实际: {pred['actual_result']}]" if pred['actual_result'] else ""

            logger.info(f"  📊 {match_info} {score_info}: {pred['predicted_result']} "
