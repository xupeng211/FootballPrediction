...
def analyze_changes():
    ...
    return {"modified": modified_files, "deleted": deleted_files, "untracked": untracked_files}
...
def categorize_changes(changes):
    ...
    for file_path in changes["modified"]:
        if any(
            path in file_path for path in ["src/core/", "src/api/", "src/database/", "src/models/"]
        ):
            categories["core_quality"].append(file_path)
        elif any(path in file_path for path in ["tests/", "test_", "conftest.py"]):
            categories["test_infrastructure"].append(file_path)
        elif "ISSUE88" in file_path:
            categories["issue88_deliverables"].append(file_path)
        elif any(path in file_path for path in ["docs/", ".md", "CLAUDE.md"]):
...
def main():
    ...
    print(
        f"  总计: {len(changes['modified']) + len(changes['deleted']) + len(changes['untracked'])} 个文件"
    )
