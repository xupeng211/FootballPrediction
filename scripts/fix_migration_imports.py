...
def fix_migration_imports(file_path):
    ...
    try:
        ...
        needs_logger = 'logger.' in content and 'logger = logging.getLogger' not in content and 'import logging' not in content
        ...
        for i, line in enumerate(lines):
            if line.startswith('# revision identifiers') or line.startswith('"""') or line.startswith('revision ='):
        ...
        if imports_to_add:
            ...
            if alembic_import_line >= 0:
                ...
                if len(parts) > 1:
                    ...
                    if new_imports:
                        lines[alembic_import_line] = f"{parts[0]}import {', '.join(current_imports + new_imports)}"
                        # 移除已添加的导入
                        imports_to_add = [imp for imp in imports_to_add if not imp.startswith('from alembic import')]
