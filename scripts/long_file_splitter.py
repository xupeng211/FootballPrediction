...
class LongFileSplitter:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    # 只包含不在类中的函数
                    if not any(
                        cls["line_start"] <= node.lineno <= cls["line_end"] for cls in classes
                    ):
    ...
    def _generate_split_strategy(
        ...
        """生成拆分策略"""
        strategy = {"method": "unknown", "files": [], "reasoning": []}
    ...
    def _generate_monitoring_split_strategy(
        self,
        file_path: str,
        classes: List[Dict],
        functions: List[Dict],
        ...
        for cls in classes:
            if any(
                keyword in cls["name"].lower() for keyword in ["anomaly", "detector", "detection"]
            ):
                anomaly_detection.append(cls)
            elif any(
                keyword in cls["name"].lower()
                for keyword in ["performance", "analyzer", "profiler"]
            ):
                performance_monitoring.append(cls)
            elif any(
                keyword in cls["name"].lower() for keyword in ["metric", "exporter", "collector"]
            ):
    ...
    def _generate_strategies_split_strategy(
        self,
        file_path: str,
        classes: List[Dict],
        functions: List[Dict],
    ...
    def _generate_decorators_split_strategy(
        self,
        file_path: str,
        classes: List[Dict],
        functions: List[Dict],
    ...
    def _generate_facades_split_strategy(
        self,
        file_path: str,
        classes: List[Dict],
        functions: List[Dict],
    ...
    def _generate_features_split_strategy(
        self,
        file_path: str,
        classes: List[Dict],
        functions: List[Dict],
    ...
    def _generate_api_split_strategy(
        self,
        file_path: str,
        classes: List[Dict],
        functions: List[Dict],
    ...
    def _generate_general_split_strategy(
        self,
        file_path: str,
        classes: List[Dict],
        functions: List[Dict],
    ...
    def _generate_file_content(
        self, file_plan: Dict, imports: List[Dict], constants: List[Dict]
    ) -> str:
        """生成拆分文件的内容"""
        content = []
