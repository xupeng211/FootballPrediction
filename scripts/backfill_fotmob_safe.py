...
class SafeFotMobCollector:
    ...
    async def collect_daily_fotmob_data(self, date_str: str) -> FotMobDailyResult:
        ...
        try:
            ...
            logger.info(
                f"📊 {date_str} FotMob采集完成: {result.total_matches} 场比赛, {result.new_matches} 场新比赛, {len(result.errors)} 个错误"
            )
        ...
        try:
            async with self.async_session() as session:
                    ...
                    try:
                        ...
                        if match_date_str:
                            try:
                                ...
                                logger.debug(
                                    f"✅ 日期解析成功: {match_date_str} -> {match_date}"
                                )
                            except ValueError:
                                try:
                                    ...
                                    match_date = datetime.strptime(
                                        f"{date_str} 15:00:00", "%Y-%m-%d %H:%M:%S"
                                    )
                        else:
                            match_date = datetime.strptime(
                                f"{date_str} 15:00:00", "%Y-%m-%d %H:%M:%S"
                        ...
                        if existing_match:
                            logger.warning(
                                f"⚠️ FotMob重复发现: DB ID {existing_match.id} - {home_team_id} vs {away_team_id} at {match_date}"
                            )
                            continue
                        else:
                            logger.info(
                                f"✅ 准备插入新FotMob比赛: {home_team_id} vs {away_team_id} at {match_date}"
                            )
                ...
                try:
                    ...
                    logger.info(
                        f"✅ FotMob数据保存成功: {date_str} - {saved_count} 场新比赛"
                    )
    ...
    async def run_safe_fotmob_backfill(
        ...
        logger.info(
            f"🚀 开始安全FotMob数据回填: {start_date.date()} 到 {end_date.date()}"
        )
        ...
        if stats["total_matches"] > 0:
            stats["success_rate"] = (
                stats["new_matches"] / stats["total_matches"]
            ) * 100
...
async def main():
    ...
    parser.add_argument(
        "--start-date", default="2022-01-01", help="开始日期 (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--end-date", default=None, help="结束日期 (YYYY-MM-DD, 默认今天)"
    )
    ...
    try:
        async with SafeFotMobCollector() as collector:
            # 执行FotMob安全回填任务
            await collector.run_safe_fotmob_backfill(
                start_date=start_date, end_date=end_date, dry_run=args.dry_run
            )
