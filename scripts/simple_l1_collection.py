                ...
                if not matches:
                    ...
                    if "fixtures" in data:
                        ...
                        if isinstance(fixtures, dict):
                            logger.error(
                                f"🔍 DEBUG - fixtures键: {list(fixtures.keys())}"
                            )
                            # 打印fixtures内容的前500字符
                            fixtures_str = str(fixtures)[:500]
                            logger.error(
                                f"🔍 DEBUG - fixtures内容(前500字符): {fixtures_str}"
                            )
                        elif isinstance(fixtures, list):
                            logger.error(
                                f"🔍 DEBUG - fixtures列表长度: {len(fixtures)}"
                            )
                            if fixtures:
                    ...
                    if "tabs" in data:
                        ...
                        if isinstance(tabs, list) and tabs:
                            ...
                            for i, tab in enumerate(tabs[:3]):  # 只看前3个tab
                                logger.error(
                                    f"🔍 DEBUG - tab[{i}]键: {list(tab.keys()) if isinstance(tab, dict) else type(tab)}"
                                )
                    ...
                    for key in potential_keys:
                        if key in data:
                            logger.error(
                                f"🔍 DEBUG - 找到潜在字段 '{key}': {type(data[key])}"
                            )
        ...
        try:
            async with get_db_session() as conn:
                ...
                existing = await conn.fetchval(
                    "SELECT id FROM leagues WHERE fotmob_id = $1", league_data["id"]
                )
                if existing:
    ...
    async def save_match_to_db(self, match_data: dict, league_id: int) -> bool:
        ...
        try:
            async with get_db_session() as conn:
                ...
                logger.info(
                    f"✅ 保存比赛: {home_team} vs {away_team} (ID: {fotmob_id})"
                )
                return True
