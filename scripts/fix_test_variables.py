...
def fix_variable_names():
    ...
    content = re.sub(
        r"DictUtils\.get_nested\(data,", "DictUtils.get_nested(data,", content
    )
    content = re.sub(
        r"DictUtils\.set_nested\(data,", "DictUtils.set_nested(data,", content
    )
    content = re.sub(r"DictUtils\.flatten\(data\)", "DictUtils.flatten(data)", content)
    content = re.sub(
        r"DictUtils\.merge\(([^,]+), ([^)]+)\)",
        lambda m: f"DictUtils.merge({m.group(1).replace('_data', 'data')}, {m.group(2).replace('_data', 'data')})",
        content,
    )
    ...
    content = re.sub(
        r"assert _result == expected", "assert result == expected", content
    )
    ...
    print("✅ 修复了测试文件中的变量名错误")


def verify_fixes():
    """验证修复结果"""
    Path("tests/unit/utils/test_dict_utils_comprehensive.py")
    ...
    result = subprocess.run(
        [
            ...
            "-m",
            "pytest",
            "tests/unit/utils/test_dict_utils_comprehensive.py::TestDictUtilsComprehensive::test_get_nested_simple",
            "-q",
        ],
        capture_output=True,
        text=True,
