...
class SimpleIntegrationChecker:
    ...
    def check_python_dependencies(self) -> bool:
        ...
        try:
            required_packages = [
                'fastapi', 'sqlalchemy', 'psycopg2', 'redis',
    ...
    def print_status_report(self):
        ...
        for service, status_info in self.services_status.items():
            ...
            if status == 'healthy':
                ...
                logger.info(f"✅ {service.replace('_', ' ').title():<20}: Healthy ({response_time:.2f}s) - {details}")
            else:
                error = status_info.get('error', 'Unknown error')
                logger.error(f"❌ {service.replace('_', ' ').title():<20}: Unhealthy - {error}")
...
def main():
    ...
    if checker.is_ready_for_integration_tests():
        ...
        logger.info("📝 Note: External services (PostgreSQL, Redis, Kafka) require Docker environment")
        return 0
    else:
