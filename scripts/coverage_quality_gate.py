...
class CoverageQualityGate:
    ...
    def check_quality_gate(self, coverage_data: Dict) -> Tuple[bool, List[str]]:
        ...
        for module in modules:
            threshold = (
                self.thresholds["critical"] if module["is_critical"] else self.thresholds["minimum"]
            )
        ...
        if zero_coverage_modules:
            violations.append(
                f"发现 {len(zero_coverage_modules)} 个零覆盖率模块: {', '.join(zero_coverage_modules[:3])}"
            )
...
"""
        ...
        if violations:
            ...
            low_coverage = len(
                [m for m in modules if 0 < m["coverage"] < self.thresholds["minimum"]]
            )
            if low_coverage > 0:
...
def main():
    ...
    parser.add_argument("--verbose", "-v", action="store_true", default=True, help="显示详细报告")
    parser.add_argument(
        "--overall-threshold", type=float, default=75.0, help="总体覆盖率阈值 (默认: 75.0)"
    )
    parser.add_argument(
        "--critical-threshold", type=float, default=80.0, help="关键模块覆盖率阈值 (默认: 80.0)"
    )
    parser.add_argument(
        "--minimum-threshold", type=float, default=60.0, help="最低覆盖率阈值 (默认: 60.0)"
    )
