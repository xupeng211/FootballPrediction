...
class Phase3TestGenerator:
    def __init__(self):
        ...
        self.target_modules = [
            # 高优先级核心模块
            {
                "path": "api/repositories.py",
                "priority": "HIGH",
                "module_type": "api",
                "reason": "API仓储核心模块",
            },
            {
                "path": "api/data_collector.py",
                "priority": "HIGH",
                "module_type": "api",
                "reason": "数据收集API",
            },
            {
                "path": "services/data_service.py",
                "priority": "HIGH",
                "module_type": "service",
                "reason": "数据服务核心",
            },
            ...
            {
                "path": "collectors/data_collector.py",
                "priority": "MEDIUM",
    ...
    def create_module_test(self, module_info):
        ...
        try:
            ...
            if module_info["module_type"] == "integration":
                test_file = (
                    f"tests/integration/{module_path.split('/')[-1].replace('.py', '_test.py')}"
                )
    ...
    def _generate_summary_report(self, created_files, failed_files):
        ...
        success_rate = (
            len(created_files) / (len(created_files) + len(failed_files)) * 100
            if (created_files or failed_files)
            else 0
        )
        ...
        print(
            f"  包含集成测试: {len([m for m in created_files if m['module_type'] == 'integration'])} 个"
        )
        print(
            f"  包含端到端测试: {len([m for m in created_files if m['module_type'] == 'e2e'])} 个"
        )
