...
class MyPyFixer:
    ...
    def _compile_error_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            'module_not_found': re.compile(r'Cannot find implementation or library stub for module named "([^"]+)"'),
            'no_any_return': re.compile(r'Returning Any from function declared to return "([^"]+)"'),
            ...
            'unexpected_keyword': re.compile(r'Unexpected keyword argument "([^"]+)" for "([^"]+)" of "([^"]+)"'),
        }
            ...
            if match := self.error_patterns['valid_type'].search(error):
                ...
                for i, line in enumerate(modified_lines):
                    if f'{var_name} =' in line and ':' not in line.split('=')[0]:
                        ...
                        modified_lines[i] = f"{' ' * indent}{var_name}: Any = {line.split('=', 1)[1].strip()}"
                ...
                for i, line in enumerate(modified_lines):
                    if f'{arg_name}=' in line:
                        ...
                        if 'str | None' in line or 'Optional[str]' in line:
                            modified_lines[i] = line.replace(f'{arg_name}=', f'{arg_name}={arg_name} or ""')
            ...
            if match := self.error_patterns['name_not_defined'].search(error):
                ...
                if name in ['HTTPError', 'RequestException']:
                    if not any('requests.exceptions' in line for line in modified_lines):
                        modified_lines = self._add_imports(modified_lines, {
                            f'from requests.exceptions import {name}'
                        })
    ...
    def _fix_return_type_issues(self, content: str, errors: List[str], tree: ast.AST) -> str:
        ...
        for error in errors:
            ...
            if match := self.error_patterns['no_any_return'].search(error):
                ...
                for i, line in enumerate(modified_lines):
                    if 'def ' in line and ':' in line and not line.strip().endswith('# type: ignore'):
    ...
    def _fix_unused_ignores(self, content: str, errors: List[str]) -> str:
        ...
        for line in lines:
            if '# type: ignore' in line:
                # 检查这行是否有实际的 MyPy 错误
                has_error = any(self.error_patterns['unused_ignore'].search(e) for e in errors if line.split(':')[0] in e)
    ...
    def print_report(self, report: Dict[str, Any]):
        ...
        print(f"📈 错误减少：{report['errors_fixed']} ({report['errors_fixed']/max(report['initial_errors'], 1)*100:.1f}%)")
