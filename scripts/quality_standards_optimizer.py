...
class QualityStandardsOptimizer:
    ...
    def __init__(self, project_root: Path = None):
        ...
        self.optimization_history_file = self.project_root / "monitoring-data" / "optimization_history.json"
        ...
        try:
            ...
            if quality_file.exists():
                with open(quality_file, 'r') as f:
                    data = json.load(f)
                return data.get("metrics", {}).get("tests_passed", 0) + data.get("metrics", {}).get("tests_failed", 0)
    ...
    def calculate_optimal_standards(self) -> Dict[str, Any]:
        ...
        standards = {
            ...
            "tests": {
                # 测试标准
                "min_pass_rate": max(85.0, 95.0 - (100.0 - coverage) * 0.5),  # 覆盖率越低，通过率要求越高
    ...
    def analyze_current_gaps(self) -> Dict[str, Any]:
        ...
        # 计算综合质量分数
        coverage_score = min(10.0, self.current_coverage / optimal_standards["coverage"]["target"] * 10)
    ...
    def print_optimization_report(self):
        ...
        print("🎯 优化后的质量标准:")
        print(f"  覆盖率目标: {standards['coverage']['target']:.1f}% (最低: {standards['coverage']['minimum']:.1f}%)")
