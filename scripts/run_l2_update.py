#!/usr/bin/env python3
"""
FootballPrediction L2数据标准化更新脚本
官方入口点：为所有完赛场次补充79维度战术统计数据

使用方法:
    python scripts/run_l2_update.py --batch-size 50 --delay 3
    python scripts/run_l2_update.py --help
"""
...
class L2BackfillProcessor:
    ...
    async def process_backfill(self):
        ...
        try:
            # 导入必要模块 - 直接导入独立解析器文件
            collectors_path = os.path.join(os.path.dirname(__file__), '..', 'src', 'collectors')
            sys.path.insert(0, collectors_path)
            from l2_parser import CompleteL2Parser as EnhancedL2Parser
            import aiohttp
