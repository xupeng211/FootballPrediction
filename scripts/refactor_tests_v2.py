...
class TestRefactorerV2:
                ...
                if isinstance(node, ast.FunctionDef):
                    if not node.name.startswith('_'):
                        ...
                        functions.append({
                            ...
                            'returns': ast.get_docstring(node.return_value) if hasattr(node, 'return_value') else None
                        })
    ...
    def generate_real_test_content(self, module_info, source_analysis):
        ...
        test_content.append(f'当前覆盖率: {module_info.get("current_coverage", 0)}% → 目标: {module_info.get("target_coverage", 50)}%')
        ...
        if source_analysis and source_analysis['functions']:
            ...
            for i, func in enumerate(source_analysis['functions'][:5]):  # 最多测试5个函数
                ...
                test_content.append('                print(f"函数 {func["name"]} 返回: {type(result)} - {str(result)[:50]}")')
        ...
        if source_analysis and source_analysis['classes']:
            ...
            for i, cls in enumerate(source_analysis['classes'][:3]):  # 最多测试3个类
                ...
                test_content.append('            assert instance is not None, f"类 {cls["name"]} 实例化失败"')
                test_content.append('            ')
                test_content.append('            # 验证实例类型')
                test_content.append(f'            assert type(instance).__name__ == "{cls["name"]}"')
                test_content.append('            ')
                ...
                if cls['methods']:
                    ...
                    for method in test_methods:
                        ...
                        if method in ['__init__', '__str__', '__repr__']:
                            ...
                            test_content.append('                try:')
                            test_content.append(f'                    method_result = instance.{method}()')
                            test_content.append(f'                    print(f"方法 {method} 返回: {{type(method_result)}}")')
                            test_content.append('                except Exception as me:')
                            test_content.append(f'                    print(f"方法 {method} 异常: {{me}}")')
                        test_content.append('                else:')
    ...
    def refactor_single_module(self, module_info):
        ...
        print(f"   📊 源代码分析: {len(source_analysis['functions'])} 函数, {len(source_analysis['classes'])} 类, {len(source_analysis['constants'])} 常量")
    ...
    def run_refactoring_phase1(self):
        ...
        print(f"📈 成功率: {successful_refactors/total_refactors*100:.1f}%" if total_refactors > 0 else "N/A")

        if successful_refactors > 0:
            print("\n🎉 阶段1重构成功完成!")
            print("📋 建议执行: python3 -m pytest tests/unit/core/config_test_refactored.py --cov=src --cov-report=term")
            return True
        else:
            print("\n⚠️ 阶段1重构需要进一步改进")
            return False
