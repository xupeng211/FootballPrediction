...
def analyze_data_structure(data: dict, path: str = "") -> None:
    ...
    if isinstance(data, dict):
        print(
            f"{'  ' * len(path.split('.'))}📁 Dictionary: {path or 'root'} ({len(data)} keys)"
        )
        for key, value in data.items():
            ...
            if any(
                ...
                print(
                    f"{'  ' * (len(path.split('.')) + 1)}🔑 Key: {key} (type: {type(value).__name__})"
                )

                if isinstance(value, dict):
                    if len(value) <= 10:  # 小字典直接显示
                        print(
                            f"{'  ' * (len(path.split('.')) + 2)}📋 Content: {list(value.keys())}"
                        )
                    else:
                        print(
                            f"{'  ' * (len(path.split('.')) + 2)}📋 Content: {len(value)} keys"
                        )
                        # 显示前5个键
                        first_keys = list(value.keys())[:5]
                        print(
                            f"{'  ' * (len(path.split('.')) + 2)}📋 First 5 keys: {first_keys}..."
                        )
                    ...
                    print(
                        f"{'  ' * (len(path.split('.')) + 2)}📋 Array: {len(value)} items"
                    )
                    if value and isinstance(value[0], dict):
                        print(
                            f"{'  ' * (len(path.split('.')) + 2)}📋 First item keys: {list(value[0].keys())[:5]}..."
                        )
                ...
                if isinstance(value, dict):
                    print(
                        f"{'  ' * (len(path.split('.')) + 1)}📁 {key}: Dictionary ({len(value)} keys)"
                    )
                elif isinstance(value, list):
                    print(
                        f"{'  ' * (len(path.split('.')) + 1)}📄 {key}: Array ({len(value)} items)"
                    )
                else:
                    print(
                        f"{'  ' * (len(path.split('.')) + 1)}💎 {key}: {type(value).__name__}"
                    )
        ...
        if data:
            print(
                f"{'  ' * (len(path.split('.')) + 1)}📋 First item type: {type(data[0]).__name__}"
            )
            if isinstance(data[0], dict):
                print(
                    f"{'  ' * (len(path.split('.')) + 1)}📋 First item keys: {list(data[0].keys())[:5]}..."
                )
    else:
        print(
            f"{'  ' * len(path.split('.'))}💎 {path}: {type(data).__name__} = {str(data)[:100]}"
        )
