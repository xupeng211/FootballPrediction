...
def analyze_data_leakage():
    ...
    for feature in post_match_leakage_features:
        if feature in df.columns:
            ...
            print(f"  {feature:25}: {non_null_count:4,}/{len(df):4,} ({non_null_count/len(df)*100:5.1f}%)")
    ...
    for feature in safe_pre_match_features:
        if feature in df.columns:
            non_null_count = df[feature].notna().sum()
            print(f"  {feature:20}: {non_null_count:4,}/{len(df):4,} ({non_null_count/len(df)*100:5.1f}%)")

    print(f"\n📋 建议的时序特征工程方案:")
    print(f"  1. 从进球数据推导历史表现指标")
    print(f"  2. 计算球队最近N场比赛的滚动平均")
    print(f"  3. 使用xG的时序趋势作为预测特征")
    print(f"  4. 构建主客场表现差异指标")
