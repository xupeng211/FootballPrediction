...
class TechnicalDebtCleaner:
    ...
    def run_full_cleanup(self) -> Dict[str, any]:
        ...
        logger.info(f"✅ 技术债务清理完成: {self.cleanup_results['issues_fixed']}/{self.cleanup_results['issues_found']} 问题已修复")
    ...
    def _find_python_files(self) -> List[Path]:
        ...
        for py_file in self.src_root.rglob("*.py"):
            # 排除测试文件和示例文件
            if not any(part in str(py_file) for part in ['tests/', 'examples/', '__pycache__']):
    ...
    def _verify_configuration_integrity(self):
        ...
        required_config_vars = [
            ...
            'LOG_DIR', 'DATA_DIR', 'MODEL_DIR', 'CACHE_DIR',
            'API_HOST', 'API_PORT',
            'ENVIRONMENT'
        ]
        ...
        if config_file.exists():
            ...
            self.cleanup_results['security_issues'].append({
                ...
                'details': 'Missing .env.example file'
            })
    ...
    def _cleanup_hardcoded_paths(self, python_files: List[Path]):
        ...
        for py_file in python_files:
            ...
            try:
                ...
                # 检查并修复FotMob API硬编码
                fotmob_matches = re.findall(self.hardcoded_patterns['fotmob_api'], content)
                if fotmob_matches:
                ...
                # 检查并修复文件路径硬编码
                path_matches = re.findall(self.hardcoded_patterns['file_paths'], content)
                if path_matches:
                    logger.warning(f"📍 在 {py_file} 中发现硬编码文件路径: {path_matches}")
                ...
                # 检查并修复localhost硬编码
                localhost_matches = re.findall(self.hardcoded_patterns['localhost'], content)
                if localhost_matches and 'config_secure' not in str(py_file):
                    ...
                    for match in localhost_matches:
                        if 'localhost' in match or '127.0.0.1' in match:
                            logger.info(f"📝 {py_file} 中发现localhost引用，但已在配置中处理")
                            # 这些通常通过配置处理，不需要修改代码
                ...
                if file_modified:
                    ...
                    self.cleanup_results['issues_fixed'] += content.count('os.path.join(os.getenv')
            ...
            try:
                ...
                for i, line in enumerate(lines):
                    ...
                    if re.search(self.hardcoded_patterns['debug_prints'], line):
                        # 排除示例和文档中的print语句
                        if not any(keyword in line.lower() for keyword in ['example', 'demo', 'example:', 'usage:', 'todo:']):
                    ...
                    if re.search(self.hardcoded_patterns['debug_logs'], line):
                        # 保留包含重要信息的debug日志
                        if any(keyword in line.lower() for keyword in ['error', 'exception', 'critical', 'security']):
                ...
                for issue_type, pattern in security_patterns.items():
                    ...
                    if matches:
                        ...
                        for match in matches:
                            ...
                            # 排除注释和测试代码
                            if not line_content.strip().startswith('#') and 'test' not in str(py_file).lower():
                                self.cleanup_results['security_issues'].append({
                                    'type': issue_type,
                                    'file': str(py_file),
                                    'line': line_num,
                                ...
                                logger.warning(f"🚨 安全问题发现 ({issue_type}): {py_file}:{line_num}")
    ...
    def _add_config_import(self, content: str) -> str:
        """添加配置导入语句"""
        # 检查是否已有导入语句
        if 'from src.config_secure import' in content or 'import src.config_secure' in content:
            return content
