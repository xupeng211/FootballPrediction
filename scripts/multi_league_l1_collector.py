...
class MultiLeagueCollector:
    ...
    def __init__(self):
        ...
        # 五大联赛配置
        self.target_leagues = [
            {"id": 47, "name": "Premier League", "country": "ENG", "short_name": "premier-league"},
            {"id": 87, "name": "LaLiga", "country": "ESP", "short_name": "laliga"},
            {"id": 55, "name": "Serie A", "country": "ITA", "short_name": "serie-a"},
            {"id": 54, "name": "Bundesliga", "country": "GER", "short_name": "bundesliga"},
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                if response.status_code != 200:
                    logger.error(f"❌ {league_info['name']} API请求失败: {response.status_code}")
                    return None
                ...
                if 'fixtures' in data and isinstance(data['fixtures'], dict):
                    if 'allMatches' in data['fixtures']:
                        ...
                        logger.info(f"✅ {league_info['name']}: 找到 {len(matches)} 场比赛")
                        return matches
                ...
                for match in matches:
                    ...
                    if existing_id:
                        ...
                        await conn.execute(
                            ...
                            """,
                            fotmob_id, home_team, away_team, league_info['name'], league_id,
                    ...
                    if saved_count <= 3:  # 只打印前3场比赛
                        logger.info(f"✅ 保存{league_info['name']}: {home_team} vs {away_team} ({match_date.strftime('%Y-%m-%d')})")
    ...
    async def run_collection(self):
        ...
        for league_info in self.target_leagues:
            ...
            if stats["total_matches"] > 0:
                ...
                logger.info(f"✅ {league_info['name']}: {stats['total_matches']}场总比赛, {stats['future_matches']}场未来比赛")
            else:
