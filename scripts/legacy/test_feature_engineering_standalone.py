...
def test_feature_transformer():
    ...
    assert abs(entropy - expected_entropy) < 1e-10, f"熵计算不正确"
    print("✅ 4. 赔率熵计算测试通过")

    # 测试完整转换
    result = transformer.transform(test_data)
    assert len(result) == len(test_data), "行数不匹配"
    expected_features = ['Implied_Prob_Home', 'Implied_Prob_Away', 'Odds_Entropy', 'Avg_Goals_Diff_L5']
    ...
    # 显示转换结果示例
    print("\n=== 转换结果示例 ===")
    display_cols = ['match_id', 'Implied_Prob_Home', 'Implied_Prob_Away', 'Odds_Entropy', 'Avg_Goals_Diff_L5']
