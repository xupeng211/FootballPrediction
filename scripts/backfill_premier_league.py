...
class PremierLeagueBackfill:
    ...
    async def collect_season(self, season_name: str, season_config: dict) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"🎉 {season_name}: 采集完成! {len(cleaned_data)} 场比赛已入库")
                return True
            else:
        ...
        for match in data:
            try:
                ...
                if match.get('status', '').lower() not in ['completed', 'final']:
                    logger.debug(f"跳过未完成比赛: {match.get('home_team')} vs {match.get('away_team')}")
                    continue
    ...
    async def _save_to_database(self, data: list[dict], season_name: str, season_id: str) -> bool:
        ...
        try:
            ...
            if success_count > 0:
                logger.info(f"✅ {season_name}: 成功保存 {success_count}/{len(data)} 场比赛")
                return True
            else:
    ...
    def print_final_report(self):
        ...
        logger.info(f"  成功率: {(self.stats['completed_seasons']/self.stats['total_seasons'])*100:.1f}%")
