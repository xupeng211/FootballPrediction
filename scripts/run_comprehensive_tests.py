...
class TestRunner:
    ...
    def run_tests(self):
        ...
        try:
            ...
            if redis.ping():
                print("✅ Redis连接成功")
                self.results['redis'] = {'success': True, 'description': 'Redis连接测试'}
            else:
                print("⚠️ Redis连接失败（可能未启动）")
                self.results['redis'] = {'success': False, 'description': 'Redis连接失败'}
        except Exception as e:
            print(f"⚠️ Redis测试跳过: {e}")
            self.results['redis'] = {'success': False, 'description': f'Redis不可用: {e}'}
