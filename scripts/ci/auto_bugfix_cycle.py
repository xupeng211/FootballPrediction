...
def run_tests():
    ...
    reports = sorted(
        REPORTS_DIR.glob("BUGFIX_REPORT_*.md"),
...
def auto_fix(report_path):
    ...
    run(
        f"git commit -m 'fix: auto bugfix based on {report_path.name}' || true",
        check=False,
    )
    run(f"git push origin {branch}", check=False)
    run(
        f"gh pr create --base main --head {branch} --title 'fix: auto bugfix from {report_path.name}' --body 'Automated bugfix from {report_path.name}' || true",
        check=False,
    )
