    ...
    try:
        ...
        with patch.dict('sys.modules', modules_to_mock):
            ...
            with patch('prometheus_client.Counter', return_value=mock_counter), \
                ...
                # 测试元数据
                metadata = {"detection_time": "2023-01-01T00:00:00", "model_version": "1.0"}
                ...
                with patch('src.data.quality.anomaly_detector.DatabaseManager'):
                    ...
                    for method in methods:
                        ...
                        print(f"  {'✅' if has_method and is_callable else '❌'} {method}")
                ...
                with patch('src.data.quality.anomaly_detector.DatabaseManager'):
                    ...
                    has_comprehensive = hasattr(adv_detector, 'run_comprehensive_detection')

                    print(f"  ✅ 统计检测器: {'存在' if has_statistical else '缺失'}")
                    print(f"  ✅ 机器学习检测器: {'存在' if has_ml else '缺失'}")
                    print(f"  ✅ 综合检测方法: {'存在' if has_comprehensive else '缺失'}")
                ...
                for method in async_methods:
                    ...
                    is_async = asyncio.iscoroutinefunction(getattr(adv_detector, method))
...
def test_detection_algorithms():
    ...
    try:
        ...
        # 模拟3σ检测
        mean = sum(d["value"] for d in test_data if d["expected"]) / sum(d["expected"] for d in test_data)
        variance = sum((d["value"] - mean) ** 2 for d in test_data if d["expected"]) / sum(d["expected"] for d in test_data)
        std = variance ** 0.5
        ...
        iqr_anomalies = [d for d in test_data if d["value"] < lower_bound or d["value"] > upper_bound]
