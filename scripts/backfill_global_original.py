                    ...
                    for team_id, name, short_name, crest, source in all_teams_to_save:
                        if team_id > 0:  # 只保存有效的球队ID
                            try:
                                ...
                                # 忽略DNS错误，继续处理其他球队
                                if "Temporary failure in name resolution" in str(team_error):
                                    logger.warning(f"⚠️ 球队 {team_id} ({name}) 因网络问题跳过: {team_error}")
                                else:
                ...
                if result.football_data_matches:
                    for match_data in result.football_data_matches:
                        try:
                            ...
                            if existing_match:
                                logger.warning(f"⚠️ Football-Data重复发现: DB ID {existing_match.id} - {home_team_id} vs {away_team_id} at {match_date}")
                                continue
                            else:
                                logger.info(f"✅ 准备插入新Football-Data比赛: {home_team_id} vs {away_team_id} at {match_date}")
