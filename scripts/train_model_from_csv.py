...
class SimpleModelTrainer:
    ...
    def __init__(self, model_path: str = "/app/data/models/football_prediction_model.pkl"):
    ...
    def load_and_prepare_data(self, csv_path: str = "/app/data/processed_features.csv") -> pd.DataFrame:
        """加载并准备训练数据"""
        ...
        try:
            ...
            # 特征工程 - 创建更多有用的特征
            df_clean['xg_ratio'] = df_clean['home_xg'] / (df_clean['away_xg'] + 0.01)  # 避免0除
    ...
    def select_features(self, df: pd.DataFrame) -> List[str]:
        ...
        # 基础特征列
        base_features = [
            'home_xg', 'away_xg', 'total_xg', 'xg_difference', 'xg_ratio'
        ]
        ...
        # 过滤掉不需要的列
        exclude_columns = [
            'id', 'fotmob_id', 'home_score', 'away_score', 'target',
        ...
        logger.info(f"选择了 {len(self.feature_columns)} 个特征: {self.feature_columns}")
    ...
    def print_training_summary(self):
        ...
        for class_name, metrics in stats['class_report'].items():
            if isinstance(metrics, dict) and 'f1-score' in metrics:
                print(f"   {class_name:10}: F1={metrics['f1-score']:.3f}, "
