    ...
    try:
        ...
        with patch.dict("sys.modules", modules_to_mock):
            ...
            spec = importlib.util.spec_from_file_location(
                "lineage_reporter", "src/lineage/lineage_reporter.py"
            )
            ...
            config_tests = [
                ...
                (
                    "完整配置",
                    {"marquez_url": "http://complete:5000", "namespace": "complete"},
                ),
            ]
            ...
            try:
                # 开始新作业
                reporter.start_job_run(job_name="fail_test_job", job_type="BATCH")
            ...
            for param_name, job_name in test_params:
                try:
                    if job_name is None:
                        ...
                        print(
                            f"  ✅ {param_name}: {job_name[:20]}{'...' if len(job_name) > 20 else ''}"
                        )
                except Exception as e:
...
def test_lineage_concepts():
    ...
    try:
        ...
        lineage_relations = [
            ...
            {
                "source": "raw_matches",
                "target": "clean_matches",
                "relationship": "cleaning",
            },
            {
                "source": "clean_matches",
                "target": "match_features",
        ...
        metadata_types = [
            {
                ...
                "description": "技术元数据（表结构、字段类型等）",
            },
            {
                "type": "business_metadata",
                "description": "业务元数据（业务含义、数据Owner等）",
            },
            {
                "type": "operational_metadata",
                "description": "操作元数据（创建时间、更新频率等）",
            },
            {
                "type": "lineage_metadata",
                "description": "血缘元数据（数据来源、转换历史等）",
            },
        ]
...
async def test_async_functionality():
    ...
    try:
        ...
        metadata_result = await mock_async_metadata_update(
            {"collection": collection_result, "processing": processing_result}
        )
