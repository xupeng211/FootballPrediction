...
def test_metrics_collector_structure():
    ...
    try:
        ...
        modules_to_mock["prometheus_client"].Histogram = Mock(
        ...
        ].get_metrics_exporter = mock_get_metrics_exporter
        ...
        with patch.dict("sys.modules", modules_to_mock):
            ...
            spec = importlib.util.spec_from_file_location(
                "metrics_collector", "src/monitoring/metrics_collector.py"
            )
            ...
            print("\n📋 指标格式化测试:")
            try:
                raw_metrics = {"cpu": 25.5, "memory": 65.2, "disk": 45.8}
            ...
            try:
                ...
                print(
                    f"  ✅ 系统指标收集器: 间隔={system_collector.collection_interval}秒"
                )

                # 测试数据库指标收集器
                db_collector = DatabaseMetricsCollector()
                print(
                    f"  ✅ 数据库指标收集器: 间隔={db_collector.collection_interval}秒"
                )

                # 测试应用指标收集器
                app_collector = ApplicationMetricsCollector()
                print(
                    f"  ✅ 应用指标收集器: 间隔={app_collector.collection_interval}秒"
                )
            ...
            try:
                ...
                for method in async_methods:
                    has_method = (
                        hasattr(system_collector, method)
                        or hasattr(db_collector, method)
                        or hasattr(app_collector, method)
                    )
            ...
            for param_name, interval in test_params:
                try:
                    ...
                    print(
                        f"  ✅ {param_name}: 间隔={test_collector.collection_interval}秒"
                    )
                except Exception as e:
        ...
        metric_categories = [
            {
                ...
                "description": "系统级指标（CPU、内存、磁盘、网络）",
            },
            {
                "category": "database_metrics",
                "description": "数据库指标（连接、查询性能、表大小）",
            },
            {
                "category": "application_metrics",
                "description": "应用指标（请求、错误率、业务指标）",
            },
            {
                "category": "business_metrics",
                "description": "业务指标（预测准确率、用户活跃度）",
            },
        ]
        ...
        monitoring_dimensions = [
            {
                ...
                "metrics": ["响应时间", "吞吐量", "资源利用率"],
            },
            {
                "dimension": "availability",
                "metrics": ["服务可用性", "错误率", "故障恢复时间"],
            },
            {
                "dimension": "reliability",
                "metrics": ["数据一致性", "服务稳定性", "容错能力"],
            },
            {
                "dimension": "scalability",
                "metrics": ["并发处理能力", "扩展性", "负载均衡"],
            },
        ]
...
async def test_async_collection():
    ...
    try:
        ...
        print(
            f"  ✅ 系统指标收集: CPU {system_result['cpu_usage']}%, 内存 {system_result['memory_usage']}%"
        )
        print(
            f"  ✅ 数据库指标收集: 连接数 {db_result['connection_count']}, 查询时间 {db_result['query_time_avg']}ms"
        )
        print(
            f"  ✅ 应用指标收集: 请求 {app_result['request_count']}, 错误率 {app_result['error_rate']*100}%"
        )
