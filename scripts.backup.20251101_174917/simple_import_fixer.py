        ...
        for model_file in models_files:
            try:
                ...
                if (
                    "class" in content
                    and "Base" in content
                    and "extend_existing=True" not in content
                ):
                    # 查找模型类定义
                    lines = content.split("\n")
                    modified_lines = []
                    ...
                    for line in lines:
                        if "class" in line and "(Base)" in line and "__table_args__" not in line:
                            ...
                            modified_lines.append(
                                " " * (indent + 4) + "__table_args__ = {'extend_existing': True}"
                            )
                        else:
                            modified_lines.append(line)
        ...
        for test_file in test_files:
            try:
                ...
                if "import " in content and (
                    "from domain.strategies" in content
                    or "from api.facades" in content
                    or "from database.repositories" in content
                ):
...
'''
    ...
    def run_fixes(self):
        ...
        # 生成报告
        total_fixes = (
            len(init_files) + len(model_fixes) + len(circular_fixes) + len(safe_import_files)
        )
        print("\n" + "=" * 50)
        print("📊 修复总结:")
