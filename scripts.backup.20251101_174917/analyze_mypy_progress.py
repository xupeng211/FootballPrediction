...
def analyze_progress():
    ...
    # 显示错误最多的文件
    print("📁 错误分布 (Top 10):")
    sorted_files = sorted(current_stats["errors_by_file"].items(), key=lambda x: x[1], reverse=True)
    ...
    # 显示错误类型分布
    print("🏷️  错误类型分布:")
    sorted_types = sorted(current_stats["error_types"].items(), key=lambda x: x[1], reverse=True)
...
def generate_progress_report(stats):
    ...
    report = {
        ...
        "analysis": {
            "top_files": list(
                sorted(stats["errors_by_file"].items(), key=lambda x: x[1], reverse=True)[:10]
            ),
            "common_errors": list(
                sorted(stats["error_types"].items(), key=lambda x: x[1], reverse=True)[:10]
            ),
        },
    }
