    ...
    def standardize_mock_imports(self, file_path: Path) -> bool:
        ...
        try:
            ...
            if mock_lines:
                ...
                # 添加其他不在标准列表中的组件
                other_components = [
                    comp for comp in used_mock_components if comp not in standard_order
                ]
            ...
            for i, line in enumerate(lines):
                ...
                for pattern, patch_decorator in external_patterns:
                    if re.search(pattern, line) and not any(
                        "patch" in prev_line for prev_line in lines[max(0, i - 5) : i]
                    ):
                        # 检查是否在测试函数中
        ...
        try:
            ...
            for mock_type, count in mock_patterns.items():
                if count > 3:  # 如果有超过3个相同的Mock创建
                    # 在文件开头添加fixture建议注释
                    if (
                        "# TODO: Consider creating a fixture for repeated Mock creation"
                        not in content
                    ):
                        lines.insert(0, "")
                        lines.insert(
                            0,
                            f"# TODO: Consider creating a fixture for {count} repeated {mock_type} creations",
                        )
                        content = "\n".join(lines)
                        break
