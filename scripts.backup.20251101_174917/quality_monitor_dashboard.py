...
class QualityMonitorDashboard:
    ...
    def run_ruff_check(self) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                ["make", "lint"], cwd=self.project_root, capture_output=True, text=True, timeout=60
            )
    ...
    def display_dashboard(self):
        ...
        ruff_status = (
            "✅"
            if self.metrics["ruff_errors"] < 100
            else "⚠️" if self.metrics["ruff_errors"] < 1000 else "❌"
        )
        ...
        test_status = (
            ...
            else "⚠️" if self.metrics["tests_failed"] < 10 else "❌"
        )
        print(
            f"🧪 测试状态: {test_status} {self.metrics['tests_passed']:,} 通过 / {self.metrics['tests_failed']:,} 失败 / {self.metrics['tests_skipped']:,} 跳过"
        )
        ...
        coverage_icon = (
            ...
            if self.metrics["test_coverage"] >= 80
                ...
                "👍"
                if self.metrics["test_coverage"] >= 50
                else "📈" if self.metrics["test_coverage"] >= 20 else "📉"
            )
        )
...
def main():
    ...
    parser.add_argument("--continuous", "-c", action="store_true", help="启动持续监控")
    parser.add_argument("--interval", "-i", type=int, default=300, help="监控间隔（秒，默认300秒）")
    parser.add_argument("--project-root", "-p", type=str, help="项目根目录路径")
