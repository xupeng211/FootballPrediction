...
class CoreStabilityValidator:
    ...
    def run_test_validation(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.results["tests"] = {
                    "status": "❌ 失败",
                    "details": {"error": result.stderr[:500], "exit_code": result.returncode},
                }
                print("❌ 测试验证失败")
            ...
            self.results["tests"] = {"status": "💥 异常", "details": {"error": str(e)}}
            print(f"💥 测试验证异常: {e}")
        ...
        try:
            ...
            if result.returncode == 0:
                self.results["code_quality"] = {
                    "status": "✅ 优秀",
                    "details": {"ruff_errors": 0, "status": "零错误"},
                }
                print("✅ Ruff检查通过: 零错误")
            else:
                # 解析错误统计
                ...
                self.results["code_quality"] = {
                    "status": "⚠️ 需要改进",
                    "details": {"ruff_errors": errors, "status": f"{errors}个错误"},
                }
                print(f"⚠️ 发现 {errors} 个代码质量问题")

        except Exception as e:
            self.results["code_quality"] = {"status": "💥 异常", "details": {"error": str(e)}}
            print(f"💥 代码质量检查异常: {e}")
    ...
    def run_coverage_validation(self):
        ...
        try:
            ...
            self.results["coverage"] = {"status": "💥 异常", "details": {"error": str(e)}}
            print(f"💥 覆盖率验证异常: {e}")
    ...
    def run_core_functionality_validation(self):
        ...
        self.results["core_functionality"] = {
            "status": (
                "✅ 稳定"
                if success_rate >= 75
                else "⚠️ 需要关注" if success_rate >= 50 else "❌ 不稳定"
            ),
            "details": {
