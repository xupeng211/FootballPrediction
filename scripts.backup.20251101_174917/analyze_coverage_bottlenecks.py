...
class CoverageBottleneckAnalyzer:
            ...
            try:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.FunctionDef):
                        functions.append(
                            {"name": node.name, "lineno": node.lineno, "args": len(node.args.args)}
                        )
                ...
                # 生成相对路径作为模块名
                module_name = (
                    str(src_file.relative_to(self.src_path)).replace("/", ".").replace(".py", "")
                )
    ...
    def identify_low_coverage_modules(self, source_info: Dict, test_coverage: Dict) -> List[Dict]:
        ...
        for module_name, module_info in source_info.items():
            ...
            # 特别关注业务逻辑模块
            business_modules = ["domain", "services", "api", "database", "cqrs"]
    ...
    def generate_deep_test_strategy(self, low_coverage_modules: List[Dict]) -> Dict[str, Any]:
        ...
        strategy["implementation_plan"] = [
            ...
            },
            {
                ...
                "modules": strategy["high_priority_modules"][5:]
                + strategy["medium_priority_modules"][:3],
                "focus": "测试完整的业务流程",
            },
            {
    ...
    def save_analysis_report(
        self, analysis_result: Dict[str, Any], output_file: str = "coverage_analysis_report.json"
    ):
        """保存分析报告"""
        import json
    ...
    def print_top_modules(self, low_coverage_modules: List[Dict], limit: int = 10):
        ...
        for i, module in enumerate(low_coverage_modules[:limit]):
            ...
            print(
                f"     代码行数: {module['code_lines']} | 函数: {module['functions_count']} | 类: {module['classes_count']}"
            )
            print(f"     测试数量: {module['test_count']} | 覆盖级别: {module['coverage_level']}")
            print(
                f"     优先级: {module['priority']} | 业务模块: {'是' if module['is_business'] else '否'}"
            )
            print()
