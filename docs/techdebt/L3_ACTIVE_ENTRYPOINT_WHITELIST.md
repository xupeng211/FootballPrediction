...
| `src/ml/**` executable modules | ML helper entrypoints | **do not run without ML task scope** | Multiple modules (`dataset_generator.py`, `xgboost_classifier.py`, etc.) have `if __name__ == "__main__"` blocks for standalone execution | Explicit ML task authorization |
| `src/services/**` modules with `__main__` | service self-tests | **do not run without service task scope** | `event_bus.py`, `league_router.py`, `match_aligner.py`, `odds_filter.py` have `__main__` or `argparse` for local testing | Task-specific service authorization |
