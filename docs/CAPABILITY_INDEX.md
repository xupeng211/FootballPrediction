...
| Odds | 真实历史赔率 import | BLOCKED | 无 | 无 | — | 未授权、未执行 | — | 真实 football-data CSV 已离线读取并进入 staging 审计（1,180 raw rows / 38,832 observations，D4B 38,616 accepted / 216 quarantined；data/real_odds_raw.csv），但从未正式 import 或写入任何数据库 / 生产（docs/PROJECT_STATUS.md、docs/M3_D4F_READINESS_REVIEW.md）；真实导入需后续单独授权，勿重复来源恢复 / 审计 |

## Domain: FotMob

| Domain | Capability | Status | Canonical entrypoint | Core implementation | Primary tests | Authorization & side effects | Legacy or forbidden alternatives | Current notes |
|---|---|---|---|---|---|---|---|---|
| FotMob | candidate exporter | CANONICAL/BLOCKED | 仅 `make data-fotmob-candidates-network-export`（需 NETWORK_AUTHORIZATION=yes） | `scripts/ops/fotmob_candidates_export.js` → `src/infrastructure/fotmob/FotMobCandidateExporter.js` | tests/unit/fotmob_candidate_exporter.test.js | 每次实际执行都会访问 FotMob 网络（CLI 对非 --help 调用强制 `--network-preview=true` + `--network-authorization=yes`，普通直接调用被 CLI 阻止）；`--retain-raw-responses` 是否落盘是另一项可选行为；不写数据库；当前未获得真实网络执行授权 | `n3_live_fotmob_raw_retain.js`、`pageprops_v2_*`、`adg60_*`、`titan_discovery.js` 及其它 Phase/ADG 历史采集脚本 | 唯一正式路径；不存在"本地无网络导出"模式；PR #1813 后支持 canonical-v2 |
...
| FotMob | canonical inventory contract | DOCUMENTED_ONLY | 无受控入口（依赖 canonical-v2，同 raw retention 行） | `src/infrastructure/canonical/CanonicalInventoryContract.js` | tests/unit/canonical_inventory_contract.test.js | 实现存在（PR #1813）；校验失败即 fail closed | — | `validateArtifactDocument()` 合同执行点（PR #1813）；当前无受控执行表面触发该能力 |
...
| DB | DB write guard | CANONICAL | scripts/ops 写脚本执行写操作前调用 | `scripts/ops/helpers/db_write_guard.js`（fail-closed；guard 自述仅供 ops scripts 调用，不构成统一 writer 层、不替代脚本自身检查） | 见 guard 相关测试 | 未通过 guard 的 ops 写路径被阻止（需 ALLOW_DB_WRITE=yes + FINAL_DB_WRITE_CONFIRMATION=yes + 表级门禁） | src/ 内模块（CanonicalInventoryWriter、FixtureRepository 等）不使用该 guard，各有自身授权机制 | 仅覆盖 scripts/ops 脚本；审计写路径时需逐路径确认授权（AGENTS.md §5.3） |
