...
| FotMob | detail capture plan | CANONICAL | `node scripts/ops/fotmob_detail_capture.js plan --candidate-artifact=<外部绝对路径> --season=... [--match-id=...] [--limit=N] --output=<外部绝对路径>`（直接 CLI，离线；README canonical 表登记为受控实现表面） | `src/infrastructure/fotmob/FotMobDetailCapturePlan.js` + `FotMobDetailCaptureContract.js` | tests/unit/fotmob_detail_capture_plan.test.js | 离线纯计算；输出到仓库外绝对非 symlink 路径；要求显式选择（--season/--match-id/--limit 至少其一），绝不默认全量 | 历史采集脚本 | 确定性 plan（fotmob-detail-capture-plan/v1 + plan_business_sha256）；输入 artifact v1/v2 双 hash 校验 |
| FotMob | detail capture (real network) | BLOCKED | `node scripts/ops/fotmob_detail_capture.js capture ...`（需 --execute + 全部 13 项授权门） | `src/infrastructure/fotmob/FotMobDetailCapturePipeline.js` + `FotMobDetailCaptureRetention.js` | tests/unit/fotmob_detail_capture_pipeline.test.js、fotmob_detail_capture_retention.test.js | 默认关闭：--execute + CONFIRM_REAL_FOTMOB_DETAIL_CAPTURE=1 + CONFIRM_MAX_FOTMOB_REQUESTS + authorization-id + expected-plan-sha256 + clean worktree + 40-hex HEAD + 仓库外非 symlink 输出根 + 非 symlink plan + 安全 run-id；单 URL 合同（https://www.fotmob.com/match/<digits>，GET、不跟随重定向、无 cookie/auth/proxy/browser、并发 1、重试 0、delay≥60s、30s 超时）；17 项内容有效性门；raw+manifest 配对原子保留；不写 DB；真实执行未授权 | `n3_live_fotmob_raw_retain.js`（LEGACY 网络/UPSERT 路径） | 每次真实执行都会访问 FotMob 网络；当前未获得真实执行授权（OWNER_REAL_CAPTURE_AUTHORIZATION=NO）；测试全部 mock（REAL_NETWORK_FORBIDDEN_IN_TEST） |
| FotMob | detail capture replay (offline) | CANONICAL | `node scripts/ops/fotmob_detail_capture.js replay --run-dir=<外部绝对路径>/runs/<run-id>` | `src/infrastructure/fotmob/FotMobDetailCaptureRetention.js` | tests/unit/fotmob_detail_capture_retention.test.js、fotmob_detail_capture_cli.test.js | 完全离线；raw hash 必须匹配 manifest；observed match id 必须匹配；零网络零 DB | — | 确定性结构化 artifact（fotmob-match-detail-artifact/v1）；输出到 run 目录 replay/ 子目录 |
