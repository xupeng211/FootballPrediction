...
| FotMob | detail staging (offline converter/validator) | CANONICAL | `make data-fotmob-detail-staging-{help,build,validate}`（`-build` 需 SOURCE_INDEX + OUTPUT_ROOT；`-validate` 需 OUTPUT_ROOT；直接 Node CLI `scripts/ops/fotmob_detail_staging.js` 为 internal engine） | `src/infrastructure/fotmob/FotMobDetailStaging{Contract,Converter,Retention}.js` | tests/unit/fotmob_detail_staging_{contract,converter,retention,cli}.test.js | 完全离线（OFFLINE ONLY / ZERO NETWORK / ZERO DATABASE / NO MIGRATION / NO CAPTURE）：无 fetcher/DB client import、无墙钟进业务字段（generated_at=manifest response_received_at；observation_id=确定性 UUIDv5）；输出仅限仓库外绝对非 symlink 路径；append-only 文件快照 + store-state.json 账本；原子 tmp+fsync+rename、冲突 fail-closed、both-or-neither；`-validate` 重验 artifact/summary/store ledger 并检测 partial output；不写 DB、无 migration、不执行 capture | — | 确定性 artifact（fotmob-detail-staging-artifact/v1）：5 个 versioned JSONB sections 逐字节保留、canonical_match_id 恒 null + UNLINKED_NOT_ATTEMPTED（不猜测 link）；直接复用 pipeline 捕获 hashing（computeStableCapturePayloadSha256，无自实现副本）；16 场归档（one/five/ten-match pilot archives）两次 build + 两次 validate 字节一致，派生输出已删除；marker_event（parser 注入 AddedTime/Half 分钟标记、无 id 设计）为合法变体；PR #1817 阻塞修复：commit-marker 原子提交/回滚、VERIFIED_PACKAGE_RECEIPT（真实 archive SHA-256 + 纯 Node tar reader）、A–E 38 项 validator（篡改检测）、REPEAT_EQUIVALENT 重建、LAYER_A/LAYER_B 完整性 hash；SC-002 状态：ENFORCEMENT_INFRASTRUCTURE=COMPLETE / STAGING_PRODUCTION_ROLE_DEPLOYMENT=PENDING / PR1817_CHANGES_SC002=NO；EXTERNAL_IMPLEMENTATION_ACCEPTANCE=PENDING、READY_TO_MERGE=NO |
