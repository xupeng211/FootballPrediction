...
| canonical candidate coverage      | implementation_proven_disposable_only  | Recovered FotMob artifact: 1,140 candidates; 888 exact + 4 isolated kickoff conflicts; v2 writer proof selects full inventory                      | Real status-complete artifact/provenance and separately authorized persistent execution        | Future canonical inventory write                                                     |
| local canonical database coverage | zero_for_scope                         | `matches` has zero Premier League/E0 rows in 2022/2023–2024/2025                                                                                   | Dedicated target selection, approved real artifact/provenance and separate write authorization | Future canonical inventory write                                                     |
| canonical schema freshness        | implementation_proven_disposable_only  | V26.10 provider/index/lineage contract and PostgreSQL 15 disposable compatibility proof                                                            | Separately authorized persistent migration/execution                                           | Future canonical inventory write                                                     |
...
| migration need                    | implementation_proven_disposable_only  | Additive V26.10 with exact scoped PUBLIC revoke; disposable migration replay/checks verified                                                       | Separately authorized persistent migration                                                     | Future canonical inventory write                                                     |
| rollback                          | disposable_rehearsal_complete          | Pre-write custom backup restored to a fresh disposable clone with schema/ACL/count verification                                                    | Dedicated-target backup/restore rehearsal                                                      | Future canonical inventory write                                                     |
...
At the time of this design review, no provider request, browser action,
migration, database/schema/row write, canonical/linkage/staging/import,
persistent M3 sandbox action, raw-payload storage, training, backtest or
prediction occurred. Section 16 records the later disposable-only
implementation proof; it does not authorize a persistent canonical or linkage
write.
...
- Labelled PostgreSQL 15 tmpfs proof, with generated `Synthetic …` candidates
  ...
  fresh replay was 1,140 exact duplicates with zero delta. Both one-row and
  ten-row canary-to-master proofs produced 1,140 matches; the ten-row case
  ended with 2 artifacts / 2 runs / 1,150 lineages.
