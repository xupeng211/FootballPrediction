...
class MockDataLoader:
    ...
    async def load_data(self) -> pd.DataFrame:
        ...
        for i in range(self.n_matches):
            ...
            # 随机选择主客场球队
            home_team_id, home_team_name = np.random.choice(list(teams.items()))

            # 确保客场球队不同
            away_teams = [k for k in teams.keys() if k != home_team_id]
            away_team_id, away_team_name = np.random.choice(away_teams)
