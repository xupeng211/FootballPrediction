...
export function usePredictions() {
    ...
    const successfulPredictions = computed(() => predictions.value.filter(p => p.success));
    const highConfidencePredictions = computed(() => predictions.value.filter(p => p.confidence > 0.8));
    return {
        predictions,
        loading,
        error,
